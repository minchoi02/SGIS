package egovframework.sgis.m2020.login.service.impl;
...
/**
 * @Class Name  : LoginViewServiceImpl.java
 * @Description : 지역현안 소통지도 로그인 서비스
...
@Service("loginViewService")
@PropertySource("classpath:globals.properties")
public class LoginViewServiceImpl extends EgovAbstractServiceImpl implements LoginViewService{
private final static Log logger = LogFactory.getLog(StringUtils.class);
	...
	public String loginProcess(HttpServletRequest request, HttpServletResponse response, Map<String, Object> params) {
		...
		for(int i=0;i<cookies.length;i++) {
			if("communityId".equals(cookies[i].getName())) {
				try {
					POI_ID = java.net.URLDecoder.decode(cookies[i].getValue(),"UTF-8");
		...
		if("Y".equals(LOGIN_YN)){
			...
			if(this.forceLogin(request,response)){
				...
				if(
					...
					return "redirect:"+CUR_URL.replaceAll(request.getContextPath(),"")+"?id="+POI_ID;
				}
				else {
					return "redirect:/";
				}
	...
	private String redirectErrorPage(
		...
		String query = "";
		System.out.println(request.getParameter("CUR_URL"));
		...
		return "redirect:/m2020/login/login.sgis"+query;
	}
