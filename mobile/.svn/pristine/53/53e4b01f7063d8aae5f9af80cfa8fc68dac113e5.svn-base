		...
		legendInfo : function(map) {
			...
			this.createLegendOptionColtrol = function(){
				...
				legendOptionPanel.onAdd = function(map) {
					...
					colorHtml = '<ul class="colorck02" style="display: flex; width: 100%;">';
					$.each(that.chooseLegendColor,function(cnt,node){
						html+='<li style="margin: 0 3px;"><a class="circle '+(cnt==0&&!that.useNegative?'active':'')+'" style="background-color:'+node.background+';" href="javascript:void(0)" data-start="'+node.start+'" data-end="'+node.end+'" data-original-start="'+node.start+'" data-original-end="'+node.end+'" data-negative="false"></a></li>';
						colorHtml+='<li style="margin: 0 3px;"><a class="circle '+(cnt==0?'active':'')+'" style="background-color:'+node.background+';" href="javascript:void(0)" data-start="'+node.start+'" data-end="'+node.end+'" data-original-start="'+node.start+'" data-original-end="'+node.end+'" data-negative="false"></a></li>';
					});
					colorHtml+='</ul>';
					if(that.useNegative){
					...
					if(that.useNegative){
						html+='							<label><input type="radio" name="selectEvenLegend_'+that.map.id+'" value="negative" checked="checked" onclick="javascript:srvLogWrite(\'O0\', \'51\', \'05\', \'04\', \'분할범례\', \'\');"> 분할범례 &nbsp;&nbsp;</label>';
					}else{
						html+='							<label><input type="radio" name="selectEvenLegend_'+that.map.id+'" value="auto" checked="checked" onclick="javascript:srvLogWrite(\'O0\', \'51\', \'05\', \'04\', \'자동범례\', \'\');"> 자동범례 &nbsp;&nbsp;</label>';
					}
					html+='							<label><input type="radio" name="selectEvenLegend_'+that.map.id+'" value="equal" onclick="javascript:srvLogWrite(\'O0\', \'51\', \'05\', \'04\', \'균등범례\', \'\');"> 균등범례</label><br><br> ';
					html+='						</td>';
					html+='					</tr>';
					html+='				</tbody>';
					html+='			</table>';
				...
				// color 추가
				$(".colorck02 li a").click(function(){
					$('#color_range').find(".colorck02 li a").removeClass("active");
					$(this).addClass("active");
					srvLogWrite('O0', '51', '05', '02', $(this).css('background-color'), '');
					
					// 색 변경.........
					var activeElement = $('#color_range').find(".colorck02 a.active");
...
//							srvLogWrite('M0','11','04','00','','');         // 범례 설정(일자리 통계정보)
			...
			this.addLegendItem = function(){
				...
				// color
				$("#color_list").empty();
				var n_ul = $("<ul/>", {"style":"display: flex; width: 100%;"});
				var ul = $("<ul/>");
				for(var i=1;i<=this.lv;i++){
					...
					n_ul.append($("<li/>",{"style":"width:"+(150/this.lv)+"px;background-color:"+this.legendColor[i]+";"}).append($("<span/>",{"text":""})));
				}
		...
		//통계소통지도 범례 삭제
			...
			this.updateLegendRangeColor = function(){
				...
				// color 추가
				$.each($("#color_list").find("li"),function(cnt,node){
					$(node).css({"background-color":that.legendColor[cnt]});
				});
			}
			/**
...
//							if(window.parent.$thematic != undefined){
//								if(thematicInfo.themaMapType == "03" || thematicInfo.themaMapType == "05"){
...
//						var colorList2 = getCalculColor("#0066aa", "#434471", that.lv);
			...
			this.getColor = function(value, valPerSlice) {
				...
				var activeElement = $(".colorck02 li a");
