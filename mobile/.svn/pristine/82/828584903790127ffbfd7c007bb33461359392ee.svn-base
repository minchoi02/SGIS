(function(W, D) {
	...
	$statsAnlsMap.ui = {
		...
		statsAnalsAnnualSalary : function(item) {
			...
			/*$("#statsGraphWrapDiv1").attr('style', 'position: fixed; top: 180px; z-index:1100; height: 100%; width:100%; overflow:hidden;');*/
			$("#statsGraphWrapDiv1").attr('style', 'position: fixed; top: 215px; z-index:700; height:calc(100% - 215px); width:100%; overflow:hidden;');
		...
		statsAnalsJobTransition : function() {
			...
			/*$("#statsGraphWrapDiv2").attr('style', 'position: fixed; top: 50px; z-index:990; padding-top: 60px; height: 100%;');	// 연령별 취업자수, 일자리추이, 일자리만족도 차트 영역 속성
*/			$("#statsGraphWrapDiv2").attr('style', 'position: fixed; top: 215px; z-index:700; padding-top: 0px; height:calc(100% - 215px); overflow-y:auto; padding-bottom:0px;');	// 연령별 취업자수, 일자리추이, 일자리만족도 차트 영역 속성
		...
		statsAnalsJobSatisfactionDegree : function(item) {
			...
			/*$("#statsGraphWrapDiv2").attr('style', 'position: fixed; top: 50px; z-index:990; padding-top: 60px;');*/
			$("#statsGraphWrapDiv2").attr('style', 'position: fixed; top: 215px; z-index:700; padding-top: 0px;overflow-y:auto; padding-bottom:0px;');
						            ...
						            point: {
						                events: {
						                    click: function () {
						                        ...
						                        /*$("#statsAnalsChartDataTitle3").html("<p>"+this.y.toFixed(1)+"</p><span>%</span>");	// 일자리만족도 값
...
*/			                                		$("#statsAnalsChartDataTitle3").html("<p>"+this.y+"</p><span>%</span>");	// 일자리만족도 값
				        ...
				        /* mng_s 20220705 iq2500snail 소수이하 자릿수 1자리로 통일 (start) */
				        var arr_text=[],  tmp_text = "";
				        arr_text = $('#statsAnalsChartMain3 .highcharts-container svg g.highcharts-data-labels g text ');
				        for(var i = 0; i < arr_text.length; i++) {
							tmp_text = Number(arr_text.eq(i).text()).toFixed(1);
							$('#statsAnalsChartMain3 .highcharts-container svg g.highcharts-data-labels g text ').eq(i).html(tmp_text);
						}	
				        /* mng_s 20220705 iq2500snail 소수이하 자릿수 1자리로 통일 (end) */
					} else {
						common_alert('failed!');
					}
				} ,
