...
function common_loading(p_flag) {
	if(p_flag) {
		...
		$(gv_common_loading.blockUI).css({"background-color":"rgba(3,3,3,0.8)"})
	}
	else {
...
function common_area(p_type, p_sido_cd, p_sgg_cd, p_emdong_cd, p_callback, p_callback2) {
	if(p_type == "sgg"){
		...
		$("#popupArea_emdong").css("display", "inline-block"); //2022-11-02 수정
	}
	...
	sidoCdSelect(p_sido_cd);	// 시도코드 조회
...
function sggCdSelect(p_sido_cd, p_sgg_cd){
	...
	$.ajax({
		...
		if(res.errCd == "0") {
			...
			//시도만 선택했을때 시군구 상위 ....셋팅
			emdongCdSelect(p_sido_cd, res.result[0].cd.slice(-3));
		}else if(res.errCd == "-401") {
			/** 2020.10.08[한광희] accessToken 만료시 재갱신 후 재 조회 START */
			accessTokenInfo(function(){
				sggCdSelect(p_sido_cd, p_sgg_cd);
