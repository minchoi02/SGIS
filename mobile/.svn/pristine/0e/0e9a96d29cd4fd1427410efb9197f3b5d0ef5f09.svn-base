(function(W, D) {
	...
	$(document).ready(function(){ //ready
		...
		if(undefined!=urlParam.get("from_ce") && null!=urlParam.get("from_ce")){
			from_ce = urlParam.get("from_ce");
		}
		console.log("from_ce"+ from_ce);
		
		if(null!=from_ce && ''!=from_ce){
			$("#from_ce").val(from_ce);
		}else{
			$("#from_ce").val('E');
		}
	...
	$communityIntro.ui = {
			...
			$.each(res.result.list,function(cnt,node){
				...
				list.append(
					$("<dl/>",{"class":(type=="hot"?(cnt==0?"M_on":""):(node.cmmnty_partcptn_grant_yn=="Y"?"lock":"")),style:(type=="hot"?(cnt==0?"":"display:none;"):"")}).append(
						$("<dt/>").append($("<a/>",{"href":"#"+ contextPath+"/m2020/map/withmap/map/communityMap.sgis?id="+node.cmmnty_map_id,"text":node.cmmnty_map_nm}),approval),
						...
						javascript:srvLogWrite('O0', '20', '02', '01', $("#from_ce").val(), '');
						apiLogWrite2("K1",node.cmmnty_map_id,node.cmmnty_map_nm,"없음","00","없음");

						location.href = contextPath+"/m2020/map/withmap/map/communityMap.sgis?id="+node.cmmnty_map_id;
					})
				);
		...
		getList : function(type){
			...
			obj.addParam("from_ce", $("#from_ce").val());
	...
	$communityIntro.event = {
		...
		setUIEvent: function() {
			...
			$("#community-search").submit(function(){
				srvLogWrite('O0', '20', '01', '03', $("#community-keyword").val(), '');
