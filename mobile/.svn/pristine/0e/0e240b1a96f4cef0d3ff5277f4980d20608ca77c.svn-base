	...
	$(document).ready(function() {
		$("#search-form").submit(function() {
			if ($searchList.searchValidate()) {
				location.href = contextPath+"/search.sgis?keywords=" + $("#searchKeyword").val();
			}
			return false;
		});
	...
	$searchList = {
		...
		initialize: function() {
			...
			if($searchList.sKeyword == ""){
				//$("#searchKeyword").val("검색어를 입력해 주세요");
				$("#searchKeyword").val("");
				$("#search_result").html("지역명 검색어 or 검색어를 입력해 주세요 <br /> 예) 서울시 인구");
		...
		searchValidate: function() {
			var searchword = $("#searchKeyword").val();
				...
				srvLogWrite("M0","01", "04", "00", "", searchword);		//모바일 검색
		...
		checkRequestEmptyCnt: function() {
			...
			if ($searchList.emptyRequestCnt < 5) {
				return;
			}
				...
				if (res.errCd == "0") {
					...
					for (var i = 0; i < (result.length > 4 ? 4 : result.length); i++) {
						$("#related-word").append(
							i > 0 ? "," : "",
							$("<a/>", {
								href: contextPath+"/search.sgis?keywords=" + result[i].rel_search_word,
			...
			onSuccess: function(status, res) {
				...
				if (res.errCd == "0") {
					...
					if (result.totalcount > 0) {
							...
							$(elementList).append(
								$("<a/>", {
									...
									if(type[url.substring(url.lastIndexOf("/")+1,url.length)]=="kosis"){
										messageAlert.open("알림", "kosis데이터는 모바일에서 조회할 수 없습니다");
									}else{
						...
						createPaging("#"+tableName+">.Pasing", result.totalcount, $searchList.openApiSOP, "page");
					}
				} else if (res.errCd == "-401") {
					accessTokenInfo(function(){
						$searchList.openApiSOP.makeLists();
				...
				if (res.errCd == "0") {
					...
					if (res.result.themeMapInfoListCount > 0) {
						...
						$.each(res.result.themeMapInfoList, function(cnt,node) {
							$(elementList).append(
								$("<a/>", {
									href: contextPath+"/map/thematic.sgis?id="+node.stat_thema_map_id,
									html: node.title
								})
							);
						});
						$("#" + tableName).append(
							$("<dt/>", {
								html: "통계주제도 검색결과 <span>(" + res.result.themeMapInfoListCount + ")</span>"
							}),
							elementList,
							$("<dd/>", {"class": "Pasing"})
						);
						createPaging("#"+tableName+">.Pasing", res.result.themeMapInfoListCount, $searchList.serviceApiThematic, "page");
					} else {
						setTimeout($searchList.checkRequestEmptyCnt(), 300);
					}
				} else if (res.errCd == "-100") {
			...
			onSuccess: function(status, res, options) {
				...
				if(res.errCd == "0") {
					...
					if(res.result.total !=0){
						$("#communityTable").append
						(
								$("<dt/>", {
									html: "지역현안 소통지도 <span>(" + res.result.total + ")</span>"
								}
								),elementList,
								$("<dd/>", {"class": "Pasing"})
						);
						createPaging("#communityTable>.Pasing", res.result.total, $searchList.getCmmntyList, "page");
					}else{
						setTimeout($searchList.checkRequestEmptyCnt(), 300);
					}
		...
		$.ajax({
			...
			success:function(data){
				...
				if(wordListLength > 0){
					...
					(
							$("<dt/>", {
								html: "통계용어 설명 <span>(" + wordListLength + ")</span>"
							}
							),elementList,
							$("<dd/>", {"class": "Pasing"})
					);
					//createPaging("#wordTable>.Pasing", wordListLength, "", "page");
				}else{
					setTimeout($searchList.checkRequestEmptyCnt(), 300);
				}
			},
...
//통계용어 설명
function communityListSearch(searchKwd){
	var word = decodeURIComponent(searchKwd);
	var word_idx = word.indexOf("=")+1;
	var post_title = word.substr(word_idx);
	var elementList = $("<dd/>", {
	...
	$.ajax({
		...
		data : {"type" : "all",
				...
				"pageSize" : 6,
				"bnd_year" : 2016,
				"search_word": post_title,
				"last_cmmnty_map_id":384,
				"search_word":"지역",
				"first_sort":"null",
				"second_sort":"Default",
				"from_ce" : "C"
		...
		success:function(data){
			console.log(data);
			var communityListLength = 0;
			var resultData = data.result;
			$.each(resultData.post_title_list,function(i){
				communityListLength++;
				var cmnty_map_id = resultData.summaryList[i].cmnty_map_id;
			//	var ptitle_word = resultData.post_title_list[i].post_title;
			//	var ptitle_word_start_idx = ptitle_word.indexOf("[");
				var ptitle_word_end_idx;
				var ptitle_word_kr;
			...
			if(communityListLength > 0){
				
				$("#communityTable").append
				(
						$("<dt/>", {
							html: "지역현안 소통지도 <span>(" + communityListLength + ")</span>"
						}
						),elementList,
						$("<dd/>", {"class": "Pasing"})
				);
				createPaging("#wordTable>.Pasing", communityListLength, "", "page");
			}else{
				setTimeout($searchList.checkRequestEmptyCnt(), 300);
			}
		},
