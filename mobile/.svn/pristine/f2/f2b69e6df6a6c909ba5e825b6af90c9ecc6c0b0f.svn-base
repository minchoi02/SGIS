				...
				*/
				...
				this.createMap = function (delegate, target, opt) {
					...
					if(document.location.href.match("interactiveMap")){
						var osm2 = new sop.TileLayer('http://sgisapi.kostat.go.kr/tiles/bmap4/L{z}/{y}/{x}.png', {minZoom: 0, maxZoom: 13, attribution: '' }); //운영
						//var osm2 = new sop.TileLayer('http://10.134.2.100:20083/tiles/bmap4/L{z}/{y}/{x}.png', {minZoom: 0, maxZoom: 13, attribution: '' }); //로컬
					...
					*/
					...
					if(location.href.match('interactiveMap/sharedata')){
								...
								mouseover : function (e) {
									...
									if (!that.isLayerMouseEventDisabled) {
										...
										if (that.delegate && 
											...
											console.log(">>>> didMouseOverPolygon");
...
//										fillColor : that.legend.legendColor[4]
										...
										for (param in layer.feature.info[0]) {
											if (layer.feature.info[0].showData == param) {
												if (parseFloat(layer.feature.info[0][param]) < 5 && 
														...
														param != "employee_cnt"
													    && parseFloat(layer.feature.info[0][param]) > 0) {	// 2020.09.17 추가  15세 미만 유소년 인구 음수(-) 값 표시
				...
				this.setPolygonDataGeojson = function (geoData) {
					...
					console.log("[2020 map.js] setPolygonDataGeojson() 호출");
			...
			miniMap : function() {
				...
				this.setPolygonGeoJsonStyle = function (type) {	
					console.log("setPolygonGeoJsonStyle type = " + type);
					// 일반경계일 경우, 색상을 채우지않고,
					// 데이터경계일 경우, 색상을 채운다.
					var color = "#ffb2a5";
