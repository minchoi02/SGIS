...
let catchmentApiPath = "/ServiceAPI";
...
*/			
			API_202001_URL : catchmentApiPath + "/OpenAPI3/catchmentArea/testStats.json",
			//API_202006_URL : "/ServiceAPI/OpenAPI3/catchmentArea/getSrvAreaGridStatDataList.json", //데이터보드 영향권 통계정보 조회
			API_202007_URL : catchmentApiPath + "/OpenAPI3/catchmentArea/getGridSrvAreaGridStatDataList.json", //격자 통계정보 조회
			API_202010_URL : catchmentApiPath + "/OpenAPI3/catchmentArea/getGridSrvAreaDataBoardList.json", //데이터 보드 : 격자 통계정보 조회
			API_202092_URL : catchmentApiPath + "/OpenAPI3/catchmentArea/serviceAreaStatistics.json", //데이터보드 영향권 통계정보 조회
			API_202093_URL : catchmentApiPath + "/OpenAPI3/catchmentArea/getCodeList.json", //공통 코드 조회
			API_202094_URL : catchmentApiPath + "/OpenAPI3/catchmentArea/characteristicStatistics.json", //특성별 통계 조회
			API_202095_URL : catchmentApiPath + "/OpenAPI3/catchmentArea/correlationAnalysis.json",//상관관계분석
			API_202097_URL : catchmentApiPath + "/OpenAPI3/catchmentArea/gridStatistics.json", //격자통계 통합(bsca) 조회
			API_202081_URL : catchmentApiPath + "/OpenAPI3/catchmentArea/ksicInfo.json", //한국표준산업분류 정보 조회
			API_202082_URL : catchmentApiPath + "/OpenAPI3/catchmentArea/settingInfo.json", //생활권역 설정정보 조회
			API_202090_URL : catchmentApiPath + "/OpenAPI3/catchmentArea/scopeInfo.json", //생활권역 임의값 범위정보조회
...
//					}
			...
			getCorrelationAnalysis : function(complexOption){
				...
				$.ajax({
				    type: 'POST',
				    url : '/ServiceAPI/OpenAPI3/catchmentArea/correlationAnalysis.json',
...
//					}
			...
			getGridInfo: function (option, url) {	// data{} 속성 추가 - 수정완료 2020-11-02 , 13:16
				url = url || '/ServiceAPI/OpenAPI3/catchmentArea/getGridSrvAreaGridStatDataList.json';
...
//	}());
	...
	/*********** 격자별 통계조회 Start **********/
	(function () {
		$class("sop.portal.getGridSrvAreaGridStatDataList.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {

				$catchmentAreaMask.endUnitWork(options.params.identifier);
				
				if (res.result != null && res.result.length > 0) {
					if(options.params.rangeType == "stats_radio_r_grid"){
						$catchmentAreaMap.ui.createSrvAreaShape(options.params.rangeType, options.params.rangeVal, options.params.mapId);
				    ...
				    var statSuffix = $catchmentAreaMenu.ui.getTypeUnit(type, coprType);
				    var statNm = "";
					if(type == "idlv"){
						statNm = statSuffix;
					}else{
	...
	/*********** 데이터보드 격자 통계조회 Start **********/
	(function () {
		$class("sop.portal.getGridSrvAreaDataBoardList.api").extend(sop.portal.absAPI).define({ //as
			onSuccess : function (status, res, options) {
				...
				if (res.result != null && res.result.gridStat != null && res.result.gridStat.length > 0) {
					...
					if(!options.promiseParam) {
						$catchmentAreaMenu.ui.settiongGridDataBoard(result, options); 
					} else {
	...
	/*********** 영향권 통계정보 조회 Start **********/
	(function() {
		$class("sop.portal.getServiceAreaStatistics.api").extend(sop.portal.absAPI)
		.define(
				{
					onSuccess : function(status, res, options) {
						...
						switch (parseInt(res.errCd)) {
							case 0:
								...
								$catchmentAreaMenu.ui.setServiceAreaStatisticsData(result, options);								
								break;
							case -100:
								// 검색결과가 존재하지 않습니다.
								break;
							case -401:
								accessTokenInfo(function() {
									$catchmentAreaMainApi.request.getServiceAreaStatistics(options.params.identifier);
								});
								break;	
						}
					},
	...
	/*********** 공통 코드 조회 Start **********/
	(function() {
		$class("sop.portal.serviceAreaCode.api").extend(sop.portal.absAPI)
		.define(
				{
					onSuccess : function(status, res, options) {
						switch (parseInt(res.errCd)) {
							case 0:
								var result  = res.result;
								$catchmentAreaMap.ui.setCodeData(result, options.params);								
								break;
							case -401:
								accessTokenInfo(function() {
	...
	/*********** 특성별 통계정보 조회 Start **********/
	(function() {
		$class("sop.portal.getCharacteristicsStats.api").extend(sop.portal.absAPI)
		.define(
				{
					onSuccess : function(status, res, options) {
						$catchmentAreaMask.endUnitWork(options.params.identifier);
						//$catchmentAreaMask.startProcess(options.params.identifier);
						...
						switch (parseInt(res.errCd)) {
							case 0:
								...
								$catchmentAreaMenu.ui.setChkDeatailData(result, options);
								$catchmentAreaMask.endUnitWork(options.params.identifier);
								break;
							case -401:
								accessTokenInfo(function() {
					...
					onSuccess : function(status, res, options) {
						...
						switch (parseInt(res.errCd)) {
							case 0:
								...
								$catchmentAreaMenu.ui.reportLoad(result, options);
								//SGIS4_생활권역_모바일_SG 끝
								break;
							case -401:
								accessTokenInfo(function() {
									$catchmentAreaMainApi.request.reportSrvAreaStats(options.params.identifier);
								});
								break;
						}
					},
					...
					onSuccess : function(status, res, options) {
						if (res.errCd == "0") {							
							if(options.params.workGb == "T"){
								...
								$catchmentAreaMenu.ui.setList(res, options);
							...
							if(options.params.workGb == "T"){
								...
								$catchmentAreaMenu.ui.setEmptyList();
							}else if(options.params.workGb == "D"){
								messageAlert.open("알림", res.errMsg);
							}							
						} else {
					...
					onSuccess : function(status, res, options) {
						if (res.errCd == "0") {							
							if(options.params.workGb == "recmd"){
								...
								$catchmentAreaMenu.ui.setLifeBizList(res, options);
							}
						} else if (res.errCd == "-401") {
							accessTokenInfo(function() {
...
//						alert('res.result.list; : ' + JSON.stringify(res.result.list));
						if (res.errCd == "0") {							
								$catchmentAreaMenu.ui.setScopeInfo(res, options);
						} else if (res.errCd == "-401") {
//							accessTokenInfo(function() {
//								$catchmentAreaMainApi.request.getSettingInfo(options.params);
//							});
	...
	(function() {
		$class("sop.portal.getCharacteristicsTotStats.api").extend(sop.portal.absAPI)
		.define(
				{
					onSuccess : function(status, res, options) {
						$catchmentAreaMask.endUnitWork(options.params.identifier);
