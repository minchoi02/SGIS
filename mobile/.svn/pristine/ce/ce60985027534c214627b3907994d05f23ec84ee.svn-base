...
(function(W, D) {
	...
	$(document).ready(function() {
		...
		//20200902 박은식 로그인 이후 돌아올 페이지 변수 (미완성) start
		var referrerUrl = document.referrer.replace(gv_full_context_path,"");
		var cmmnty_id = referrerUrl.split('id=')
		id = cmmnty_id[1];
		//20200902 박은식 로그인 이후 돌아올 페이지 변수 (미완성)	end
		//실패 인 경우
		if(gv_err_yn == "Y") {
			//자동로그인 취소
		...
		if(common_get_cookie("loginSaveAutoLogin") == "true") {
			...
			$form.append( 
				$("<input>", {
					type : "hidden",
					name : "CUR_URL",
					value : "/mobile/m2020/map/community/map/communityMap.sgis"
				})
			);
			$form.append( 
				$("<input>", {
					type : "hidden",
					name : "SYS_URL",
					value : gv_full_context_path+"/m2020/login/loginprocess.sgis"
				})
			);
			$form.append( 
				$("<input>", {
	...
	$login.ui = {
		...
		poiLogin : function() {
			...
			if(id != ""){
				common_set_cookie("communityId",id,365);
			} else {
				common_remove_cookie("communityId");
			}
