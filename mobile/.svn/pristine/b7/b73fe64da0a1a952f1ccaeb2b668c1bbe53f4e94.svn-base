...
(function(W, D) {
	...
	$(document).ready(function() {
		...
		$("#map-navigator-sido").change(function(){
			$("#map-navigator-sido option").removeClass("selected");
			$("#map-navigator-sido option:selected").addClass("selected");
		})
		...
		$("#administStatsRegionPopOk").click(function(){
			const selectAdmNm = $("#map-navigator-sido option.selected").text();
			$("#data-year").empty();
			$("#data-year").html($("#year-list option:selected").val());
			$("#location-text").empty();
			$("#location-text").html("<p>"+selectAdmNm+"</p>");
			$("#triggerBtn").trigger("click");
			$("#administStatsRegionPop").css("display","none");
			$("#common_popup_back").css("display","none");
		//	$("#year-container").css("display","none");
			//2022-10-14 [송은미] 이벤트 추가
			$(".locationboxwrap2").removeClass('active');
			
		})
		
		$("#administStatsYearPopOk").click(function(){
			const selectYear = $("#year-list option.selected").text();
			$("#data-year").empty();
...
//							_this.map.map.gMap.setView(defaultCenter,1);
		...
		createMap: function(id) {
			if($.heum.hasData($administStatsMap.ui.theme)){
				...
				//const year = $("#year-list option.selected").data("value");
				$("#data-year").text(year);
				//console.log(year);
		...
		didSelectedPolygon : function(event, feature, type, map){
			if(type==="data"){
				...
				if(map.target!=="tooltip-map"){
					...
					$("#location-text").append("<p>"+ $administStatsMap.ui.locationNameArray[0] +svg + $administStatsMap.ui.selectedAdmNm + "</p>");
					//$("#location-text").append($("<p/>",{"text":feature.info[0].result.region_nm}));
					$("#map-tooltip [data-id=title]").text($("#year-list option[aria-checked=true]").data("value")+"년 총 "+themeInfo.name);
					$("#map-navigator-sido option[data-value="+$administStatsMap.ui.admCd+"]").prop("selected" , true);
				}
				...
				success: function(res) {
					if (res.errCd == "0") {
						...
						if($.heum.hasData(res.result.resultList)&&res.result.resultList.length>0){
							...
							//const parameterYearButton = $("#year-list option").val();
...
//				}
			...
			$("#filter-form").submit(()=>{
				...
				let sidoButton = $("#map-navigator-sido option.selected:not([data-value=00])");
				...
				$("#location-text").empty();
				//console.log($administStatsMap.ui.locationNameArray);
				...
				if($administStatsMap.ui.locationNameArray.length =="2"){
					$("#location-text").append("<p>" + $administStatsMap.ui.locationNameArray[0] + svg + $administStatsMap.ui.locationNameArray[1] + "</p>");
				}else {
					$("#location-text").append("<p>" + $administStatsMap.ui.locationNameArray[0] + "</p>")
				}
