...
(function(W, D) {
	W.$catchmentAreaLegendInfo = W.$catchmentAreaLegendInfo || {};
	...
	cLegendInfo = {
		catchmentAreaLegendInfo : function(map) {
			// 변수
			var that = this;
			...
			//this.defaultStartLegendColor = ["#f4ebe2", "#e9d9a6","#d8bb6f", "#FFD75D", "#F09E43", "#d18049", "#aa6967"];
			//this.defaultLegendColor = ["#cd1103","#0e4000","#004574","#230064", "#00076a", "#783300", "#cccccc"];
			//this.defaultLegendColor = ["#f4ebe2", "#e9d9a6","#d8bb6f", "#FFD75D", "#F09E43", "#d18049", "#aa6967"];
			//this.legendColor = ["#ccc", "#c2b5b9", "#b99ea6", "#b08893", "#a77180", "#9e5a6d", "#95445a", "#8c2d47", "#831634", "#7a0021"];
			this.legendColor = ["#f2eee3", "#e5db9e","#ddc178", "#d29171", "#c97a5e", "#9d584a", "#7b3e30"];	//["#FDF3D6", "#E6D3B2","#D0B38E", "#BA936B", "#A47347", "#8E5323", "#783300"]	// ["#f4ebe2", "#e9d9a6","#d8bb6f", "#FFD75D", "#F09E43", "#d18049", "#aa6967"]
			...
			this.initialize = function(delegate) {
				...
				var tmpColors = ["#AA6967","#D18049", "#D8BB6F", "#D8BB6F", "#E9D9A6", "#F4EBE2", "#FFFFFF"];
				this.lv = this.initLv;
				this.map.setLegendColor(tmpColors);
				this.map.delegate = delegate;
				var currentdate = new Date();
				this.id = makeStamp(currentdate);
				
				//살고싶은 우리동네의 경우, default색상
				if (this.map.delegate != undefined && this.map.delegate.namespace != undefined && this.map.delegate.namespace === "houseAnalysisMap") {
					this.legendColor1 = "#7A0021";
					this.legendColor2 = "#cccccc";
				}else {
	    ...
	    this.removeDataOverlay = function() {
	    	...
	    	if (this.map.delegate.namespace != "interactiveCombineMap") {
	    		if (this.map.heatMap) {
	    ...
	    this.showNumberData = function(){
	    	...
	    	//Left Menu 통계표출 연동
	    	if(this.map.delegate.namespace == "interactiveMap") {
	    		$interactiveLeftMenu.ui.showNumberSetting();
	    	} else if(this.map.delegate.namespace == "bizStatsMap") {
		...
		this.calculateLegend = function(arData) {
			console.log("calculateLegend 범례를 계산한다.");
			if (arData == null || arData.length == 0 || arData[0].length == 0) {
				return;
			}
			this.data = arData;
			...
			if(this.legendType =="auto"){
				...
				that.updateGuganLegend();
				console.log(this.legendValue.equal);
		...
		/**
	     * 
	     * @name         : calCntPerGugan2
	     * @description  : 구간별 빈도수만 계산한다.
	     * @params valPerSlice : 범례기준
	     * @params data	: 데이터
	     */
		this.calCntPerGugan2 = function(valPerSlice, data) {
			var guganData = [];
			var cnt = 0;
			for (var i=0; i<that.lv; i++) {
				guganData[i] = 0;
...
/*			
			...
			if(valPerSlice !== undefined && data !== undefined){
				var loopCnt = valPerSlice.length;
				if(loopCnt > data.length){
					loopCnt = data.length;
				}
				
				for (var z=0; z<loopCnt; z++) {
					var guganData = [];
					var cnt = 0;
					for (var i=0; i<that.lv; i++) {
						guganData[i] = 0;
...
//					var colorList2 = getCalculColor("#a501b7", "#7c02cd", that.lv);
		...
		this.setUserLegend = function(type, lv) {
			...
			console.log("[catchmentAreaLegendInfo.js] setUserLegend() 호출");
			console.log("[catchmentAreaLegendInfo.js] setUserLegend() this.map.isInnerMapShow2 [" + this.map.isInnerMapShow2);
			console.log("[catchmentAreaLegendInfo.js] setUserLegend() this.map.isInnerMapShow3 [" + this.map.isInnerMapShow3);
			...
			if (this.guganTextShow) {
				...
				if (tmpData.length <= 1) {
					for(var i = 0;i <that.lv; i++){
						...
						console.log("tmpData1111");
						console.log(value);
						$("#colorStatus_"+that.id+">li:eq("+idx+")>span").text(value);
					}
				}else {
					var requestor = $catchmentAreaMenu.ui.gridMapRequestor;
					for(var i = 0;i <that.lv; i++){
						var idx = (that.lv-1) - i;
						if (i == 0) {
						...
						if( requestor === 'grid' ) {
							$("#lev"+(i+1)).html(value);							
						} else if( requestor === 'detailAnalysis') {
							$("#lev"+(i+1)+'_detail').html(value);	
						} else {
							// 그리드 격자 지도와 관련없는 것이면 여기에 코드 작성
						}
	     ...
	     * 
	     * @name         : setGoganSum
	     * @description  : 범례의 레벨별 구간합 정보를 설정한다.(원형 : setGoganText)
	     * @history 	 : 
	     */
		this.setGoganSum = function(valPerSlice, data, boundCnt, requestFromDetailChart){
			if (this.guganTextShow) {
				// 구간합 구하기 추가
				var cntPerSlice = this.calCntPerGugan2(valPerSlice, data);
				//SGIS4_0629_생활권역 시작
				var sumOfLgnd = 0; 
				//SGIS4_0629_생활권역 끝
				...
				for(var i = 0;i <that.lv; i++){
					if( requestor === 'grid' ) {
						$("#levCnt"+(i+1)).html(cntPerSlice[i] + " 개");	
					} else if(requestor === 'detailAnalysis' && (requestFromDetailChart === true || map.id === 0)) {
						$("#levCnt"+(i+1)+'_detail').html(cntPerSlice[i] + " 개");
					}
					
					//SGIS4_0629_생활권역 시작
					sumOfLgnd = sumOfLgnd + cntPerSlice[i]; 
					//SGIS4_0629_생활권역 끝
				}

				if( requestor === 'grid' ) {
					//SGIS4_0629_생활권역 시작
					//$("#levCnt0").html((boundCnt - data.length) + " 개");
					$("#levCnt0").html((boundCnt - sumOfLgnd) + " 개");
					//SGIS4_0629_생활권역 끝
				} else if(requestor === 'detailAnalysis' && (requestFromDetailChart === true || map.id === 0)) {
					//SGIS4_0629_생활권역 시작
					//$("#levCnt0_detail").html((boundCnt - data.length) + " 개");
					$("#levCnt0_detail").html((boundCnt - sumOfLgnd) + " 개");
					//SGIS4_0629_생활권역 끝
				}				
			}
		};
		...
		this.getColor = function(value, valPerSlice) {
			...
			if (valPerSlice.length < this.lv && that.legendType == "auto") {
				...
				var returnLevel=0;
				// 범례 보안 적용 - 2020-11-02 박상언
				if(value === 'N/A') {
					return [$.trim(that.legendColor[0]), 0]
				}
		...
		this.colorck = function(){
			...
			$("body").on("click",".colorck li>a",function(){
				...
				console.log("[catchmentAreaLegendInfo.js] colorck() color [" + color);
		...
		this.legendInit = function(isClear){
			...
			//살고싶은 우리동네의 경우, default색상
			if (that.delegate != undefined) {
				if(this.map.delegate.namespace != undefined){
					if(this.map.delegate.namespace === "houseAnalysisMap"){
		...
		this.showLegendFixed = function() {
			for (var i=0; i<this.map.delegate.mapList.length; i++) {
				var map = this.map.delegate.mapList[i];
				if (map != null) {
	     ...
	     * @author	     : 
	     */
		this.legendCopy = function(targetId, colorNoChange) {
			//해당 맵에 있는 범례정보를 저장한다.
			...
			if (this.legendList == null) {
				...
				for (var i=0; i<this.map.delegate.mapList.length; i++) {
					...
					this.legendList[i] = deepCopy(catchmentAreaLegendInfo);
				}
			}else {
				for (var i=0; i<this.map.delegate.mapList.length; i++) {
					var map = this.map.delegate.mapList[i];
					if(!map.legend.data) continue;	// 2020-12-09 : 상세분석지도 범례 기준을 변경하면 에러가 나는 문제 해결. 보고서에서 쓰이는 map 객체를 무시하면 됨.
			...
			if(curPage.match(matchStr)){ //mng_s 20180105 정책맵일경우
				...
				if (targetId==0) {
					this.data = cLegendInfo.catchmentAreaLegendInfo.fixed_legend_val1;
				} else if (targetId==1) {
					this.data = cLegendInfo.catchmentAreaLegendInfo.fixed_legend_val2;
				} else if (targetId==2) {
					this.data = cLegendInfo.catchmentAreaLegendInfo.fixed_legend_val3;
				}
			}
			...
			//혼합색상의 경우,
			if ($("#combineColor_"+this.map.delegate.mapList[targetId].legend.id+" >a").hasClass("on")) {
			...
			$("#legendDataSlider").slider({
		        ...
		        slide: function( event, ui ) {  //ui.value
		        	...
		        	if(map.isInnerMapShow2){
						...
						$('path[fill=#F0FFF0]').css('fill-opacity', '0.1');
						$catchmentAreaLegendInfo.ui.slideInfo.slideValue2 = ui.value;
		        		...
		        		$("path").css('fill-opacity', ui.value);
		        		$catchmentAreaLegendInfo.ui.slideInfo.slideValue1 = ui.value;
		        	}
			    }
		    });
			//mng_e
...
function legendColor(c01, c02, cls, max,id,obj){
	...
	console.log("[catchmentAreaLegendInfo.js] legendColor() 호출");
	console.log("[catchmentAreaLegendInfo.js] legendColor() obj [" + obj);
