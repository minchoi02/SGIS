...
(function(W, D) {
	...
	sLegendInfo = {
	    ...
	    this.drawBubbleMap = function(geojson) {
		    ...
		    geojson.eachLayer(function(layer) {
		    	...
		    	if (layer.feature.info.length > 0) {
		    		if (layer.feature.isEcountry) {
		    			info = layer.feature.info;
			    		data = info[0];
				    	unit = info[1];
				    	
				    	//mng_s 20200317 이진호
				    	//item_nm을 추가하여 base_item_nms 표기 하도록
				    	var item_nm = $interactiveMapEcountry.ecountry_base_item_nms[ $interactiveMapEcountry.tbl_id ];
				    	var kosisData = "";
				    	if (unit != undefined) {
				    		kosisData = item_nm+"&nbsp"+ appendCommaToNumber(data)+" ("+unit+")";
				    	}else {
				    		kosisData = item_nm+"&nbsp"+ appendCommaToNumber(data);
				    	}
				    	//mng_e 20200317 이진호
	    	...
	    	geojson.eachLayer(function(layer) {
		    	...
		    	if (layer.feature.info.length > 0) {
		    		if (layer.feature.isEcountry) {
				    	...
				    	//mng_s 20200317 이진호
				    	//기존 (위 주석 소스) 에는 unit 값이 없을 때 분기 처리가 되어있지 않아서 unit값의 유무에 따른 분기처리
				    	//item_nm을 추가하여 base_item_nms 표기 하도록
				    	var item_nm = $interactiveMapEcountry.ecountry_base_item_nms[ $interactiveMapEcountry.tbl_id ];
				    	if(unit != undefined && unit != null && unit != "" ){
				    		...
				    		toolTip += 			"<div style='font-size:14px;font-weight:bold;color:#3792de;'>"+ adm_nm +"</div>";
				    		toolTip +=				"<div style='height:5px;'></div>";
				    		toolTip += 			"<div style='font-size:12px;padding-left:5px;'>"+ item_nm + "&nbsp" +appendCommaToNumber(data)+" ("+unit+")</div>";
				    		toolTip += 	  "</div>";
				    	}else{
				    		var toolTip  = "<div style='margin:10px;'>";
				    		toolTip += 			"<div style='font-size:14px;font-weight:bold;color:#3792de;'>"+ adm_nm +"</div>";
				    		toolTip +=				"<div style='height:5px;'></div>";
				    		toolTip += 			"<div style='font-size:12px;padding-left:5px;'>"+ item_nm + "&nbsp" +  appendCommaToNumber(data)+ "</div>";
				    		toolTip += 	  "</div>";
				    	}
				    	//mng_e 20200317 이진호
	    ...
	    this.setLegendParams = function(type, color, level) {
	    	console.log("[setLegendParams] type = " + type + ", color = " + color+ ", level = " + level);
		...
		this.calculateLegend = function(arData) {
			if (arData == null || arData[0] == undefined || arData[0].length == 0) {
				return;
			}
			this.data = arData;
			...
			for (var i=0; i<arData.length; i++) {
				...
				//leekh 20190514 숫자가 아닌 데이터 때문에 오류나는 부분 수정함.
				var test = arData[i];
...
//					var colorList2 = getCalculColor("#a501b7", "#7c02cd", that.lv);
			...
			if (this.guganTextShow) {
				...
				if (tmpData.length <= 1) {
					...
					for(var i = 0;i <that.lv; i++){
						...
						$("#colorStatus_"+that.id+">li:eq("+idx+")>span").text(value);
						$("#colorStatus_"+that.id+">li:eq("+idx+")>span").autoTextSize(9, 10, false); // ie에서 제대로 동작안함, ...찍힘
						
						// hshsh
						$("#color_list_"+that.id+">li:eq("+idx+")>span").text(value);
						$("#color_list_"+that.id+">li:eq("+idx+")>span").autoTextSize(9, 10, false); // ie에서 제대로 동작안함, ...찍힘
					}
					$("#colorMaxNum").text(maxNum);
				}	
			}
		...
		this.getColor = function(value, valPerSlice) {
			...
			if (this.legendType == "negative" && this.negativeLegendColor != null && this.isNegativeColorShow) {
				...
				// bubble  // 
				setLegendColor(this.legendColor,"#colorStatus", this.id,that);
				
				setLegendColor(this.legendColor,"#color_list_"+this.id, this.id,that);
		...
		this.legendEvent = function(){
			...
			$("#colorStatus_"+that.id+">li>span").css("font-size", "10px");
...
//						srvLogWrite("C0", "08", "04", "00", "", cls);		//지도타입 4종
					}
...
//					srvLogWrite("C0", "08", "03", "00", "", "");		//범례고정
			...
			// color...
			// 색상 범례 값 선택 했을 때 이벤트
			body.on("click","#color_list_"+that.id+">li", function(){
				var val = $(this).find("span").text();
				//console.log("color_list_ click " + val);
				$("#colorMaxNum").text(val);
			});
		...
		this.colorck = function(){
			$(".colorck02 li").each(function(i){ 
				var selector = $(this).children("a");
				selector.css("background-color", selector.text());
			});
			
			$("body").on("click",".colorck02 li>a",function(){
				//$(this).parents(".colorck02").eq(0).find("a").removeClass("on");
				//$(this).addClass("on");
				$(this).parents(".colorck02").eq(0).find("a").removeClass("active");
				$(this).addClass("active");
				srvLogWrite('O0', '51', '05', '02', $(this).css('background-color'), '');
				
				var dataid = $(this).parents(".colorck02").eq(0).attr("id");
				var color = $(this).text();
				var startColor = $(this).attr("start-color");
				var listLegnth = $("#lvSelect_"+that.id).val();
				
				console.log("dataid = " + dataid + ", color = " + color + ", startColor = " + startColor + ", listLegnth = " + listLegnth);
				
				//mng_s
//				if(document.location.href.match("interactive")){
//					srvLogWrite("C0", "08", "01", "00", "", "");		//색상변경
//				}
				
				console.log("[legendInfo.js] colorck() color [" + color);
				
				if (map.isInnerMapShow2) {
					console.log("[legendInfo.js] map.isInnerMapShow2");
					if(color == "#cd1103") {
				...
				if (map.isInnerMapShow3) {
					console.log("[legendInfo.js] map.isInnerMapShow3");
					if(color == "#cd1103") {
						map.gridLegendClickNum = 0;
					} else if(color == "#0e4000") {
						map.gridLegendClickNum = 1;
				...
				if ($(this).hasClass("combined")){
					console.log("[legendInfo.js] combined");
				...
				if ($(this).parent().attr("class") == "negativeDefaultColor") {
					console.log("[legendInfo.js] negativeDefaultColor");
					that.isNegativeColorShow = true;
					that.map.setLegendColor();
				...
				if(dataid =="colorSetting_"+that.id){
					console.log("[legendInfo.js] colorSetting_");
					legendColor(startColor, color, "#colorSettingList01_"+that.id, listLegnth,that.id,that);
					
				}else if(dataid =="legendColor_"+that.id){
					
					// hshs
					console.log("[legendInfo.js] legendColor_ 원 색상 클릭");
					
					legendColor(startColor, color, "#colorStatus_"+that.id, listLegnth,that.id,that);
					// bubble //
					legendColor(startColor, color, "#colorStatus", listLegnth,that.id,that);
					
					// hshs (맵 삭상변경)
					// 색상리스트 변경
					legendColorNew(startColor, color, "#color_list_"+that.id, listLegnth,that.id,that);
//					legendColorNew(startColor, color, "#color_list", listLegnth,that.id,that);
		...
		this.heatTable = function(){
			...
			$("#heatRadiusSlider").slider({
			    ...
			    slide : function(e, ui) {
			       ...
			       console.log("that.map.heatBlur = " + that.map.heatBlur);
...
//			    	}
		...
		this.goganConfirm = function(){  // 범례구간설정 적용버튼
			$("body").on("click","#goganEvent_"+that.id, function(e){
				...
				if(document.location.href.match("interactive")){
//					srvLogWrite("C0", "08", "05", "00", "", "");		//단계설정 3종
				}
		...
		this.legendInit = function(isClear){
			...
			//20190218 원래 소스는 that.legendType는 auto로 하드코딩 되어있었음.
			...
			if (that.delegate != undefined) {
				if(this.delegate.namespace != undefined){
					if(this.delegate.namespace === "houseAnalysisMap"){
						...
						//20190218 대화형통계지도 초기화값 세팅함 leekh
...
//			$("#legendColor_"+that.id + ">li:eq(0) > a").addClass("on");
			$("#legendColor_"+that.id + ">li > a").removeClass("active");
			$("#legendColor_"+that.id + ">li:eq(0) > a").addClass("active");
			...
			legendColor(that.legendColor2, that.legendColor1, "#colorStatus_"+that.id, that.lv,that.id,that);
			// bubble
			legendColor(that.legendColor2, that.legendColor1, "#colorStatus", that.lv,that.id,that);
		...
		this.legendCopy = function(targetId) {
			...
			// bubbel x
			legendColor(this.legendColor2, this.legendColor1, "#colorStatus", this.lv, this.id, this);
		...
		this.createLegend = function() {
			...
			infoLegend.update = function(props) {
				...
				// hshs
				// 역순 버튼 id 변경
				//$('#reverseBtn').prop("id","reverseBtn_"+that.id);
				// id를 변경한다.
				$('#color_list').prop("id","color_list_"+that.id);
				$('.color_checkbox02 .colorck02').empty();
				$('.color_checkbox02 .colorck02').prop("id", "legendColor_"+that.id);
				// hshs
				var tmpHtml = "";
				//mng_s
				$.map(that.defaultLegendColor,function(value, index){					
					if (index == that.defaultLegendColor.length-1) {
						...
						// hshs
						tmpHtml += '<li><a id="Ngrid_lg_color_' + index + '" href="javascript:void(0)" class="circle '+(index==0?'active':'')+'" data-color="'+value+'" start-color="'+startColor+'"style=\"background-color:'+value+'">'+value+'</a></li>';
					}
				});
				
				// 원형 색상 처리
				$('.color_checkbox02 .colorck02').append(tmpHtml);
				...
				this._div.innerHTML = '<div data-html2canvas-ignore=true class="legendRing legendBox max" id="legendBox_'+that.id+'" data-ing="max">'
						...
						+'<li><a href="javascript:void(0); srvLogWrite(\'O0\', \'51\', \'05\', \'03\', \'색상\', \'\');" data-type="color" data-subj="색상지도 팁(Shaded Area map)" title="지도 내  경계영역 내 설정된 범례 범위값과 생상으로 시각화된 데이터를 통해 지역별로 비교할 수 있습니다."><span>색상</span></a>'
						+'<li><a href="javascript:void(0) srvLogWrite(\'O0\', \'51\', \'05\', \'03\', \'버블\', \'\');;" data-type="bubble" data-subj="버블지도 팁(Bubbles map)" title="지도내 경계영역 내 설정된 범례 범위값에 따라 경계영역 내 그려진 원형 크기로 시각화된 데이터를 통해 지역별로 비교할 수 있습니다."><span>버블</span></a>'
						+'<li><a href="javascript:void(0) srvLogWrite(\'O0\', \'51\', \'05\', \'03\', \'점\', \'\');" data-type="dot" data-subj="점지도 팁(Dot Destiny map)" title="지도내 경계영역 내 그려진 점으로 시각화된 데이터를 통해 지역별 밀집도를 비교할 수 있습니다."><span>점</span></a>'
						+'<li><a href="javascript:void(0) srvLogWrite(\'O0\', \'51\', \'05\', \'03\', \'열\', \'\');" data-type="heat" data-subj="열지도(Heat map)" title="지도내 경계여역 내 설정된 범례범위값과 색상으로 시각화된 데이터를 통해 지역벼로 비교할 수 있습니다."><span>열</span></a>'
						+'</ul>'
						...
						+(that.delegate != undefined && that.delegate.namespace === "thematicMap04" ? 
							...
							'<span>분류방법</span>'
							+'<a href="javascript:void(0);srvLogWrite(\'O0\', \'51\', \'05\', \'04\', \'분할범례\', \'\');" id="guganNagative_'+that.id+'" class="btnStyle01 al on" title="표출되는 통계값의 범위내에서 음수/양수로 자동으로 구간을 확정">분할범례</a>'
							+'<a href="javascript:void(0);srvLogWrite(\'O0\', \'51\', \'05\', \'04\', \'균등범례\', \'\');" id="guganEqual_'+that.id+'" class="btnStyle01 ac" title="표출되는 통계값의 범위내에서 균등하게 확정">균등범례</a>'
							+'<a href="javascript:void(0);srvLogWrite(\'O0\', \'51\', \'05\', \'04\', \'사용자정의\', \'\');" id="guganUser_'+that.id+'" class="btnStyle01 ar" title="표출 구간을 사용자가 정의">사용자정의</a>'
						:		
							'<span>분류방법</span>'
							+'<a href="javascript:void(0);srvLogWrite(\'O0\', \'51\', \'05\', \'04\', \'자동범례\', \'\');" id="guganAuto_'+that.id+'" class="btnStyle01 al on" title="표출되는 통계값에 의해서 자동으로 구간을 확정(Natural Break)">자동범례</a>'
							+'<a href="javascript:void(0);srvLogWrite(\'O0\', \'51\', \'05\', \'04\', \'균등범례\', \'\');" id="guganEqual_'+that.id+'" class="btnStyle01 ac" title="표출되는 통계값의 범위내에서 균등하게 확정">균등범례</a>'
							+'<a href="javascript:void(0);srvLogWrite(\'O0\', \'51\', \'05\', \'04\', \'사용자정의\', \'\');" id="guganUser_'+that.id+'" class="btnStyle01 ar" title="표출 구간을 사용자가 정의">사용자정의</a>'
						)
			...
			$("#legendDataSlider").slider({
		        ...
		        slide: function( event, ui ) {  //ui.value
		        	...
		        	if(document.location.href.match("interactive")){
//		        		srvLogWrite("C0", "08", "08", "00", "", "");		//투명도 설정
		        	}
			...
			if ($(".legendLine").length){
			   for (var i=0; i<10; i++) {
				   ...
				   // bubble 최초...
				   $("#bubbleLegendLine").prepend("<li><span style='margin-left:-30px;'>"+txt+"Lv</span></li>");
			   }
			}
			...
			legendColorNew(this.legendColor2, this.legendColor1, "#color_list_"+that.id, that.lv,that.id,that);
			// bubble x line
//			legendColor(this.legendColor2, this.legendColor1, "#colorStatus", that.lv,that.id,that);
...
//					srvLogWrite("C0", "08", "06", "00", "", "");		//색상설정
				...
				if(selectorTab =="al"){
				    ...
				    // bubble x
//				    legendColor(that.legendColor2, color, "#colorStatus", lv,that.id,that);
			...
			$("#reverseBtn_"+that.id).click(function(){
				console.log("reverseBtn_ click");
				if(document.location.href.match("interactive")){
//					srvLogWrite("C0", "08", "02", "00", "", "");		//범례역순
				}
				if(that.reverseOn==false){
					that.reverseOn=true;
				}else{
				...
				for ( var i = 0; i < that.legendColor.length; i++) {
					...
					//}else{
						$(cls).prepend("<li style='background:" + that.legendColor[i] + ";border:0px solid " + that.legendColor[i] + ";'><span style='font-size: 11px;'>"+txt+"Lv</span></li>");
					//}
				}
...
//				}
				...
				if ($("#color_list_"+that.id).attr("class")=="color_checkbox02"){
					var liHeight = parseInt(220/that.lv); 
					$(cls+" li").css("height",22+"px");
				}
			...
			// bubble 역순 버튼 클릭
			$("#reverseBtn2").click(function(){
				console.log("reverseBtn2 click");
//				if(document.location.href.match("interactive")){
//					srvLogWrite("C0", "08", "02", "00", "", "");		//범례역순
//				}
				if(that.reverseOn==false){
...
function setLegendColor(colorList, cls, id, obj) {
	...
	for ( var i = 0; i < arrColor.length; i++) {
		...
		if (obj.valPerSlice != null) {
			if ($(cls).parent().attr("class") == "ring") {
				...
				// bubble x
				//$("#colorStatus").find("li").eq(9-i).find("span").html(appendCommaToNumber(obj.valPerSlice[0][i]));
			}else {
	...
	if(cls=="#color_list_"+id){
		$("#color_list_"+id+">li").each(function(){
...
function legendColor(c01, c02, cls, max,id,obj){
	...
	console.log("obj.map.isInnerMapShow2 = " + obj.map.isInnerMapShow2);
	console.log("obj.map.isInnerMapShow3 = " + obj.map.isInnerMapShow3);
	...
	for (var i=0; i<10; i++) {
		...
		// bubble x
		$("#bubbleLegendLine").find("li").eq(i).find("span").show();
		$("#bubbleLegendLine").find("li").eq(9-i).find("span").html(txt+"Lv");
	}
...
//	}
	...
	for ( var i = 0; i < arrColor.length; i++) {
		...
		if (obj.valPerSlice != null) {
			if ($(cls).parent().attr("class") == "ring") {
				console.log("valPerSlice not null >>>>>  ring");
				$(cls).append("<li style='background:" + arrColor[i] + ";border:0px solid " + arrColor[i] + ";'><span>"+txt+"Lv</span></li>");
				$("#bubbleLegendLine_"+id).find("li").eq(9-i).find("span").html(appendCommaToNumber(obj.valPerSlice[0][i]));
				// bubble O 수치 
				$("#bubbleLegendLine").find("li").eq(9-i).find("span").html(appendCommaToNumber(obj.valPerSlice[0][i]));
//				$("#colorStatus").prepend("<li style='background:" + arrColor[9-i] + ";border:0px solid " + arrColor[9-i] + ";'><span>"+txt+"Lv</span></li>");
				$("#colorStatus").prepend("<li style='background:" + arrColor[i] + ";border:0px solid " + arrColor[i] + ";'><span>"+txt+"Lv</span></li>");
			}else {
				$(cls).prepend("<li style='background:" + arrColor[i] + ";border:0px solid " + arrColor[i] + ";'><span>"+txt+"Lv</span></li>");
				$("#colorStatus").prepend("<li style='background:" + arrColor[i] + ";border:0px solid " + arrColor[i] + ";'><span>"+txt+"Lv</span></li>");
			}
			...
			if ($(cls).parent().attr("class") == "ring") {
				console.log("valPerSlice null >>>>>  ring");
				$(cls).append("<li style='background:" + arrColor[i] + ";border:0px solid " + arrColor[i] + ";'><span>"+txt+"Lv</span></li>");
			}else {
				$(cls).prepend("<li style='background:" + arrColor[i] + ";border:0px solid " + arrColor[i] + ";'><span>"+txt+"Lv</span></li>");
			}
	...
	for (var i=0; i<hiddenLv; i++) {
		...
		// bubble 값없는 li 숨김
		$("#bubbleLegendLine").find("li").eq(i).find("span").hide();
	}
...
function legendColorNew(c01, c02, cls, max,id,obj){
	
	console.log("[legendInfo.js] legendColorNew() 호출");
	console.log("[legendInfo.js] legendColorNew() obj [" + obj);
	...
	console.log("#color_list_ = " + "#color_list_"+id);
	if(cls=="#color_list_"+id){
		$("#color_list_"+id+">li").each(function(){
			//$(this).width((100/$("#color_list"+id+">li").length)+"%");
			$(this).css("height","22px");
		});
	}
