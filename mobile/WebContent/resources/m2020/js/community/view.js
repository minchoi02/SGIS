	...
	$communityView.event = {
		...
		setUIEvent: function() {
			...
			$("#community-poiList").submit(function(){
				srvLogWrite('O0', '10', '02', '04', $("#community-keyword").val(), '');
...
function replyEdit(title, message, btnOptions, addMessageHtml,callback) {
	...
	var alertPopupWrapper = $("<div/>",{"class":"popWrapCommunity", "id":"modify_popup_confirm","style":"position:fixed;  background-color:#fff;" +
										" padding-top: 15px; border-radius:10px; left:50px; top:15%"});
	var buttonBox = $("<div/>",{"class":"btnBox", "style":"display:flex; align-items:center; justify-content:center;"});
	$.each(btnOptions,function(cnt,node){
		var button = $("<a/>",{"class":"btn_popType"+(cnt+1)+"_0001","text":node.title, "style":"display:flex; align-items:center; height:40px; margin:10px 0 15px 0;"}).click(function(){
			popupWrapper.remove();
			if(typeof node.func === "function"){
				node.func.call(popupWrapper);
			}
			if(!$(this).hasClass("btn_popType1_0001")){
				$('html, body').css({'overflow': 'auto', 'height': '100%'}).on("scroll", function(){
					event.preventDefault();     
					event.stopPropagation();
				});
