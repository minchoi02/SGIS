(function(W, D) {
	$(function(){
			...
			$interactive.ui.map.mapNavigation.move();
			var title = $(".gnb h2").text();
			var parameter = $("#map-title>h3").text();
			var zoomLevel = $interactive.ui.map.mapNavigation.zoom;
	...
	$interactive.search = {
			...
			if(data.length>0){
				...
				$.each(data,function(cnt,node){
					...
					dataArray.push({
						...
						adm_nm:node.adm_nm,
						y:y,
						color:$interactive.search.chartDefaultColor
					});
				});
				if(!hasAdmNm){
					$.ajax({
						...
						data : {
							...
							year : $interactive.ui.map.bnd_year,
				...
				function process(){
					...
					$interactive.search.setBarchart(dataArray,showData,showDataName,unit);
					$interactive.search.setTable(dataArray,showData);
				}
			}else{
		...
		setBarchart : function(data,showData,showDataName,unit){
			...
			console.log(data)
			$interactive.search.lastChartIndx = null;
			var chartHeight = (data.length * 25)+100;
			$highchartApi.chart("#chart-area>.chart","bar",$(window).width()-20, chartHeight, 20, showDataName, unit, data,  true, true, [], function(point){
				var adm_cd = point.options.adm_cd;
				if(map.dataBoundary&&map.isDrawStat){
					$.each(map.dataBoundary.getLayers(),function(cnt,node){
						if(node.feature.properties.adm_cd==adm_cd){
							node.fire("click");
							var center = node.getCenter();
							map.mapMove([center.x,center.y],map.gMap.getZoom(),false);
							return false;
						}
					});
				}else{
					$interactive.search.updateColor(adm_cd);
				}
			});
			$("#chart-area .highcharts-xaxis-labels text").click(function(){
				var chart = $("#chart-area>.chart").highcharts();
				if(chart){
					if(chart.series[0].data[$(this).index()]){
						chart.series[0].data[$(this).index()].firePointEvent("click");
					}
				}
			});
		},
		...
		setTable : function(data,showData){
			...
			$.each(data,function(cnt,node){
				...
				$.each($("#table-area table"),function(){
		...
		updateBarchartColor : function(adm_cd){
			var chart = $("#chart-area>.chart").highcharts();
			if($interactive.search.lastChartIndx!=null){
				chart.series[0].data[$interactive.search.lastChartIndx].update({ color: $interactive.search.chartDefaultColor });
			}
			$.each(chart.series[0].data,function(cnt,node){
				if(node.adm_cd===adm_cd){
					$interactive.search.lastChartIndx = cnt;
					node.update({ color: $interactive.search.activeChartColor});
					if($("#chart-area .highcharts-series rect[fill=\\"+$interactive.search.activeChartColor+"]").length>0){
						$("#chart-area").animate({
							scrollTop: $("#chart-area .highcharts-series rect[fill=\\"+$interactive.search.activeChartColor+"]").offset().top-$("#chart-area>.chart").offset().top-20
						}, 801);
					}
		...
		API_0301 : function(){
			...
			$interactive.ui.map.censusApi.setStatsMapCensusData(api_id,{
				...
				"callback" : function(data){
					if(checkItem.val()=="nongga_cnt"||checkItem.val()=="imga_cnt"||checkItem.val() == "naesuoga_cnt"||checkItem.val() == "haesuoga_cnt"){
						...
						$(".chartAreaTit").html(checkItem.data("show-name")+"(" + checkItem.data("unit") + ")[출처:통계청, 통계청농림어업총조사("+checkppltnyear+")");
						...
						$(".chartAreaTit").html(checkItem.data("show-name")+"(" + checkItem.data("unit") + ")[출처:통계청, 전국사업체조사("+checkppltnyear+")");
					}else{
						setMaptitle($("#map-title>h3"),checkItem.data("show-name")+"("+checkItem.data("unit")+")[출처:통계청, 인구주택총조사("+checkppltnyear+")");
						$(".chartAreaTit").html(checkItem.data("show-name")+"(" + checkItem.data("unit") + ")[출처:통계청, 인구주택총조사("+checkppltnyear+")");
					}
					$interactive.search.setArea(data, checkItem.val(), checkItem.data("show-name"), checkItem.data("unit"));
					api_log_write();
				}
			},{
				"year":checkItem.val()=="corp_cnt"?checkcorpyear:checkppltnyear,
				"bnd_year":$interactive.ui.map.bnd_year
			});
			return true;
		},
		/**
		...
		API_0302 : function(){
			...
			if(parameters.edu_level){
				...
				$.each(parameters.edu_level.split(","),function(cnt,node){
					title.push($("input[name=API_0302_edu_level][value="+node+"]:checkbox").parent().text());
				});
			...
			if(parameters.mrg_state){
				...
				$.each(parameters.mrg_state.split(","),function(cnt,node){
					title.push($("input[name=API_0302_mrg_state][value="+node+"]:checkbox").parent().text());
				});
				mapTitle.push(title.join(","));
			}
			...
			$interactive.ui.map.censusApi.setStatsMapCensusData(api_id,{
				...
				"callback" : function(data){
					...
					$(".chartAreaTit").html(mapTitle.join(" + ")+"(명)[출처:통계청, 인구주택총조사("+checkyear+")");
					$interactive.search.setArea(data, "population", "인구", "명");
					api_log_write();
				}
			},parameters);
			return true;
		...
		API_0304 : function(){
			...
			if($("#search-box>div[data-id="+api_id+"]>.TabArea").index(searchBox)==0){//산업분류검색
				parameters.class_code = $interactive.api.activeClassCode;
				if($("#API_0304_1_check").is(":checked") == true && $("#company-list input[name=company_list]:radio").is(":checked") == true){
				...
				mapTitle.push($interactive.api.activeClassName);
			}else if($("#search-box>div[data-id="+api_id+"]>.TabArea").index(searchBox)==1){//테마검색
			...
			$interactive.ui.map.censusApi.setStatsMapCensusData(api_id,{
				...
				"callback" : function(data){
					...
					$(".chartAreaTit").html(mapTitle.join(" + ")+"("+checkItem.data("unit")+")[출처:통계청, 전국사업체조사("+checkyear+")");
					$interactive.search.setArea(data, checkItem.val(), checkItem.data("show-name"), checkItem.data("unit"));
					api_log_write();
				}
			},parameters);
		...
		API_0305 : function(){
			...
			if(parameters.household_type){
				...
				$.each(parameters.household_type.split(","),function(cnt,node){
					title.push($("input[name=API_0305_household_type][value="+node+"]:checkbox").parent().text());
				});
				mapTitle.push(title.join(","));
			}
			if(parameters.ocptn_type){
				...
				$.each(parameters.ocptn_type.split(","),function(cnt,node){
					title.push($("input[name=API_0305_ocptn_type][value="+node+"]:checkbox").parent().text());
				});
				mapTitle.push(title.join(","));
			}
			...
			$interactive.ui.map.censusApi.setStatsMapCensusData(api_id,{
				...
				"callback" : function(data){
					...
					$(".chartAreaTit").html(mapTitle.join(" + ")+"(가구)[출처:통계청,인구주택총조사("+checkyear+")");
					$interactive.search.setArea(data, "household_cnt", "가구수", "가구");
					api_log_write();
				}
			},parameters);
			return true;
		...
		API_0306 : function(){
			...
			if(parameters.house_type){
				...
				$.each(parameters.house_type.split(","),function(cnt,node){
					title.push($("input[name=API_0306_house_type][value="+node+"]:checkbox").parent().text());
				});
				mapTitle.push(title.join(","));
			}
			...
			$interactive.ui.map.censusApi.setStatsMapCensusData(api_id,{
				...
				"callback" : function(data){
					...
					$(".chartAreaTit").html(mapTitle.join(" + ")+"(호)[출처:통계청,인구주택총조사("+checkyear+")");
					$interactive.search.setArea(data, "house_cnt", "주택수", "호");
					api_log_write();
				}
			},parameters);
			return true;
		...
		API_0310 : function(){
			...
			if($("#search-box>div[data-id="+api_id+"]>.TabArea").index(searchBox)==0){//가구원별 상세조건
				...
				mapTitle.push(searchBox.find("input[name=data_type]:radio:checked").parent().text());
				$interactive.ui.map.censusApi.setStatsMapCensusData(api_id,{
					"showData" : "population",
					"showDataName" : "인구",
					"unit" : "명",
					"callback" : function(data){
						...
						$(".chartAreaTit").html(mapTitle.join(" + ")+"(명)[출처:통계청,농림어업총조사("+checkyear+")");
						$interactive.search.setArea(data, "population", "인구", "명");
						api_log_write();
					}
				},parameters);
				...
				mapTitle.push(checkItem.parent().text());
				$interactive.ui.map.censusApi.setStatsMapCensusData(checkItem.data("api"),{
					"showData" : checkItem.val(),
					"showDataName" : checkItem.data("show-name"),
					"unit" : checkItem.data("unit"),
					"callback" : function(data){
						setMaptitle($("#map-title>h3"),mapTitle.join(" + ")+"("+checkItem.data("unit")+")[출처:통계청,농림어업총조사("+checkyearr+")");
						$(".chartAreaTit").html(mapTitle.join(" + ")+"("+checkItem.data("unit")+")[출처:통계청,농림어업총조사("+checkyearr+")");
						$interactive.search.setArea(data, checkItem.val(), checkItem.data("show-name"), checkItem.data("unit"));
						api_log_write();
					}
				},parameters);
	...
	function api_log_write(){
		var title = $(".gnb h2").text();				
		var parameter = $("#map-title>h3").text();
		var zoomLevel = $interactive.ui.subNavigation.menu.zoom;
