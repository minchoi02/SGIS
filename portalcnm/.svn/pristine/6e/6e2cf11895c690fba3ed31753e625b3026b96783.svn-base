...
var pluploader = {};
(function(W, D) {
	W.$DevbannerUpdate = W.$DevbannerUpdate || {};
	// get From URL
	var POST_NO = getParameter('POST_NO');
	var POST_TITLE_EN = getParameter('POST_TITLE_EN');
	var uploadFile = null;
	$(document)
	.ready(
			function() {
							...
							if (POST_TITLE.length == 0) {
								...
								if (getBytesCount(POST_TITLE) < 200) {
									if (getBytesCount(POST_CONTENT) < 4000) {
										if (getBytesCount(LINK_URL) < 4000) {
											getConfirmPopup('확인', '수정하시겠습니까?', 'confirm');
											$('#ok_confirmPopup')
											.click(
													function() {
														var PRIORITY_DISP_YN = $('#PRIORITY_DISP_YN').val();
														$DevbannerUpdate.updateBoardInner(POST_NO, encodeURIComponent(POST_TITLE),
																PRIORITY_DISP_YN, encodeURIComponent(POST_CONTENT), IMGURL[2].split("/")[4].slice(0,-1), encodeURIComponent(LINK_URL));
														confirmPopupRemove();
													});
											...
											getConfirmPopup('알림', '링크 URL이 지정된  입력범위를 4000Byte 초과하였습니다.', 'alert');
											$('#ok_alertPopup').click(function() {
												confirmPopupRemove();
											});
											$('#close_confirmPopup').click(function() {
	...
	$DevbannerUpdate = {
			...
			updateBoardInner : function(POST_NO, POST_TITLE, PRIORITY_DISP_YN, POST_CONTENT, IMGURL, LINK_URL) {
				...
				sopOpenApiupdateBoardInnerObj.addParam('POST_TITLE_EN', IMGURL);
				sopOpenApiupdateBoardInnerObj.addParam('LINK_URL', LINK_URL);
				sopOpenApiupdateBoardInnerObj.request({
					method : "POST",
					async : false,
					url : contextPath + "/ServiceAPI/QA/DevfaqManage/updateBanner.json"
				});
			}
	};
	// request Board
				...
				if (res.errCd == "0") {
					if (result != null) {
						if (result.board != null) {
							$("#POST_TITLE").html(result.board.POST_TITLE);
							var titleText = $("#POST_TITLE").text();
							...
							$('#LINK_URL').val(result.board.LINK_URL);
						}
					}
				} else {
		...
		$class("sop.openApi.updateBoardAfterFile.api").extend(sop.cnm.absAPI).define({
			onSuccess : function(status, res) {
				if (res.errCd == "0") {
					...
					$('#ok_alertPopup').click(function() {
						location.href = './../DT/BannerManage.html';
						confirmPopupRemove();
					});
					$('#close_confirmPopup').click(function() {
						location.href = './../DT/BannerManage.html';
		...
		$class("sop.openApi.updateBoardInner.api").extend(sop.cnm.absAPI).define(
				{
					onSuccess : function(status, res) {
						if (res.errCd == "0") {
							...
							$('#ok_alertPopup').click(
									function() {
										location.href = "./../DT/BannerManage.html?searchWordType=" + searchWordType + "&searchWord=" + searchWord
