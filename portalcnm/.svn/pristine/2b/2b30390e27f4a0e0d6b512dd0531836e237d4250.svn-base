...
/**
 * 
 * @ClassName: ReplyQuestion
 * @Description：
 * 
 * @author xuliguo
 * @date：2014年11月3日 下午1:38:22
 * @version V1.0
 * 
 */
public class ReplyQuestion extends AbsAuth<Success> {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(ReplyQuestion.class);
	@Resource
	private QAManageDao qaManageDao;

	@Override
	public String getApiId() {
		return "qamanage_reply";
	}
	...
	public Success executeAPI(HttpServletRequest req, HttpServletResponse res, String trId) throws AbsException {
		try {
			...
			Boolean flag = paramMap.get("POST_TITLE").toString().getBytes().length < 200;
			if (!flag)
			...
			return qaManageDao.replyQuestion(paramMap);
		} catch (AbsAPIException e) {
			logger.error(e);
			throw e;
		} catch (IllegalArgumentException e) {
			logger.error(e);
			throw new ApiException("입력값을 체크해 주세요");
		} catch (Exception e) {
			logger.error(e);
	...
	private enum MustParam {
		BOARD_CD, POST_NO, POST_DEPTH, POST_TITLE, POST_CONTENT, PARENT_POST_ID, LOW_RANK_S_CLASS_CD
	}
