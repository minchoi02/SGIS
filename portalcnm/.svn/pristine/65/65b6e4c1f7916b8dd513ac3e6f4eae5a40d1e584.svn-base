 ...
 * 
 */
public class DelThemaMapList extends AbsAuth<Success> {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(DelThemaMapList.class);
	@Resource
	private ThemaMapManageDao themaMapManageDao;

	@Override
	public String getApiId() {
		return "themamapmanage_delete";
	}
	...
	public Success executeAPI(HttpServletRequest req, HttpServletResponse res, String trId) throws AbsException {
		try {
			...
			String STAT_THEMA_MAP_ID_List = req.getParameter(OptionParam.STAT_THEMA_MAP_ID_List.name());
			if (STAT_THEMA_MAP_ID_List != null) {
				success = themaMapManageDao.deleteThemaMapList(STAT_THEMA_MAP_ID_List.split(","));
			}
			return success;
