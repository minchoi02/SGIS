...
(function(W, D) {
	...
	$(document).ready(
					function() {
						...
						CKEDITOR.replace('POST_CONTENT', {
							filebrowserUploadUrl : contextPath + "/js/plugins/ckeditor/ckeditorbannerImageUpload.jsp?realUrl=" + contextPath
						...
						$('#addButton').click(function() {							
							...
							//var POST_CONTENT =  $('#selectfile').val().replace(/(^\s*)|(\s*$)/g, '');
							var IMGURL = POST_CONTENT.split(" ");
							console.log(IMGURL[2]);
							console.log(IMGURL[2].split("/")[4].slice(0,-1));
									...
									if (getBytesCount(POST_CONTENT) < 4000) {
										if (getBytesCount(LINK_URL) < 4000) {
											...
											$('#ok_confirmPopup').click(function() {
												$DevbannerAdd.addBoard(POST_TITLE, PRIORITY_DISP_YN, POST_CONTENT, IMGURL[2].split("/")[4].slice(0,-1), LINK_URL);
												confirmPopupRemove();
											});
	...
	$DevbannerAdd = {
		addBoard : function(POST_TITLE, PRIORITY_DISP_YN, POST_CONTENT, IMGURL, LINK_URL) {			
			...
			sopOpenApiAddBoardObj.addParam('POST_CONTENT', POST_CONTENT);
			sopOpenApiAddBoardObj.addParam('POST_TITLE_EN', IMGURL);
			sopOpenApiAddBoardObj.addParam('LINK_URL', LINK_URL);
			sopOpenApiAddBoardObj.request({
				method : "POST",
				async : false,
				url : contextPath + "/ServiceAPI/QA/DevfaqManage/addBanner.json"
			});
		}
	};
	(function() {
