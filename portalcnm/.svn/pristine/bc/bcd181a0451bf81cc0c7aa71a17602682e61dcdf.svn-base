 ...
 *     
 */
public class CheckREL extends AbsAuth<Success> {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(CheckREL.class);
	@Resource
	...
	public Success executeAPI(HttpServletRequest req, HttpServletResponse res,
		...
		try {
			String SEARCH_WORD = req.getParameter(MustParam.SEARCH_WORD.name());
			if(SEARCH_WORD.getBytes().length > 50)
				throw new ApiException("입력값을 체크 해 주세요");
			return relManageDao.checkREL(SEARCH_WORD);
