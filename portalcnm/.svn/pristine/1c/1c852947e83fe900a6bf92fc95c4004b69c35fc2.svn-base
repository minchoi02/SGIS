...
(function(W, D) {
	...
	$(document).ready(function(){
		...
		$('#addButton').click(function(){
			srvLogWrite("L0", "05", "01", "23", "", "");
			location.href = './../QA/communityNoticeAdd.html?searchWordType=' + searchWordType_SEND + "&searchWord=" + searchWord_SEND + "&pageNumber=" + PAGENUMBER_SEND + "&sort=" + SORT_SEND + "&order=" + ORDER_SEND + "&PRIORITY_DISP_YN=" + PRIORITY_DISP_YN_SEND;
		});
		$(id_datagrid).datagrid({
			...
			url:contextPath +"/ServiceAPI/QA/CommunityNoticeManage/searchBoard.json"
		});
	...
	$boardManage = {
			delData : function(POST_NO_List) {
				...
				sopOpenApiDelDatatObj.request({
			        method : "POST",
			        async : false,
			        url : contextPath +"/ServiceAPI/QA/CommunityNoticeManage/deleteBoard.json"
			    });
			}
	};
	(function() {
