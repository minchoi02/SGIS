 ...
 * 
 */
public class DevUpdateBanner extends AbsAuth<Success> {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(DevUpdateBanner.class);
	@Resource
	...
	public Success executeAPI(HttpServletRequest req, HttpServletResponse res, String trId) throws AbsException {
		try {
			...
			String cleanXssTitle1 = "";
			cleanXssTitle1 = Security.cleanXss((String) paramMap.get("POST_TITLE_EN"));
			paramMap.put("POST_TITLE_EN", cleanXssTitle1);
			
			String cleanXssLinkUrl = "";
			cleanXssLinkUrl = (String) paramMap.get("LINK_URL");
			paramMap.put("LINK_URL", cleanXssLinkUrl);
			
			devFAQManageDao.updateBanner(paramMap);
