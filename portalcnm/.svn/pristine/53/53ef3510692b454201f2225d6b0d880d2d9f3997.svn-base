...
(function(W, D) {
	...
	$(document).ready(function(){
		...
		$('#addButton').click(function(){
			srvLogWrite("L0", "05", "02", "02", "", "");
			location.href = './../QA/DevboardAdd.html?searchWordType=' + searchWordType_SEND + "&searchWord=" + searchWord_SEND + "&pageNumber=" + PAGENUMBER_SEND + "&sort=" + SORT_SEND + "&order=" + ORDER_SEND + "&PRIORITY_DISP_YN=" + PRIORITY_DISP_YN_SEND;
		});
		$(id_datagrid).datagrid({
			...
			url:contextPath +"/ServiceAPI/QA/BoardManage/devsearchBoard.json"
		});
	...
	$boardManage = {
			delData : function(POST_NO_List) {
				...
				sopOpenApiDelDatatObj.request({
			        method : "POST",
			        async : false,
			        url : contextPath +"/ServiceAPI/QA/BoardManage/devdeleteBoard.json"
			    });
			}
	};
	(function() {
