...
<head>	
	...
	$(document).ready(function(){
		...
		function setGrid(){
			...
			if(regist.beingRegisteredStblList[tblId] == undefined) {
				if(!confirm("KOSIS에서 통계정보를 가져오지 못했습니다. 직접입력하시겠습니까?")) {					
					$("a.prev").click();
				} else {
					$("#pathNm").prop("disabled", true);
					regist.beingRegisteredStblList[tblId] = {};
					
					regist.beingRegisteredStblList[tblId].ORG_ID = $("#orgId").val()
					regist.beingRegisteredStblList[tblId].TBL_ID = $("#tblId").val()
					regist.beingRegisteredStblList[tblId].STAT_NM = $("#stattbDiv").val();
					regist.beingRegisteredStblList[tblId].STATTB_CLASS = "";
					regist.beingRegisteredStblList[tblId].TBL_NM = $(".registeredListStbl_step3 option:selected").text().substring($(".registeredListStbl_step3 option:selected").text().indexOf(")")+1);
					regist.beingRegisteredStblList[tblId].OBJ_NM = $("#tblNm").val();
					regist.beingRegisteredStblList[tblId].PATH_NM = $("#pathNm").val();
				...
				$("#stattbBeginYear").val(regist.beingRegisteredStblList[tblId].STRT_PRD_DE == undefined ? "" : regist.beingRegisteredStblList[tblId].STRT_PRD_DE);
				$("#stattbEndYear").val(regist.beingRegisteredStblList[tblId].END_PRD_DE == undefined ? "" : regist.beingRegisteredStblList[tblId].END_PRD_DE);
				$("#stattbYear").val(regist.beingRegisteredStblList[tblId].PRD_DE == undefined ? "" : regist.beingRegisteredStblList[tblId].PRD_DE);
			...
			if(regist.step == 1) {
				...
				setTimeout(function() {
					...
					$.ajax({
						method: "GET",
						async: false,	// 반드시 동기처리 해야 함
						url: proxyURL + "http://link.kostat.go.kr/view/kosisApi/getStblList.do?category=" + $("#selectListStblCategory option:selected").val(),
						dataType: "json",
						success: function(res) {
				...
				if($("#registeredListStbl option[data-origin!=Y]").length == 0) {
					alert("신규로 추가된 통계표가 존재하지 않습니다.");
					return;
				}
				$(".regProc.step2").hide();				
				$("a.next").hide();
		...
		$(".regProc.step3 .stblInfo .registeredListStbl_step3").on("change", function() {
			...
			$("#regionVarOrd").val(regist.beingRegisteredStblList[tblId].REGION_VAR_ORD == undefined ? "" : regist.beingRegisteredStblList[tblId].REGION_VAR_ORD);
			$("#stattbYear").val(regist.beingRegisteredStblList[tblId].STATTB_YEAR == undefined ? "" : regist.beingRegisteredStblList[tblId].STATTB_YEAR);
			$("#stattbOrder").val(regist.beingRegisteredStblList[tblId].STATTB_ORDER == undefined ? "" : regist.beingRegisteredStblList[tblId].STATTB_ORDER);
			$("#stattbBeginYear").val(regist.beingRegisteredStblList[tblId].STATTB_BEGIN_YEAR == undefined ? "" : regist.beingRegisteredStblList[tblId].STATTB_BEGIN_YEAR);
			$("#stattbEndYear").val(regist.beingRegisteredStblList[tblId].STATTB_END_YEAR == undefined ? "" : regist.beingRegisteredStblList[tblId].STATTB_END_YEAR);
		});
		...
		//통계표 정보 변경
		$("#regionBegin, #regionEnd").on("change", function() {
			let orgId = $("#orgId").val(); // 선택된 통계표 조직ID
		...
		//통계표 정보 변경
		$("#objNm, #stattbUrl, #stattbSourc, #regionVarOrd, #stattbYear, #stattbOrder, #stattbBeginYear, #stattbEndYear, #stattbDiv").on("keyup", function() {
			let orgId = $("#orgId").val(); // 선택된 통계표 조직ID
			let tblId = $("#tblId").val(); // 선택된 통계표 ID
	...
	<div class="wrapper">
		...
		<div class="contents">
			...
			<div class="acticle">
				...
				<div>
					<div class="regProc step1">
						<span class="title">1. 카테고리 선택</span>
						<select id="selectListStblCategory">
						</select>
					</div>
					<div class="regProc step2">
						<span class="title">2. 통계표 목록 선택</span>
						<div class="newDiv">
							<input type="text" id="newStblSearch" value="" placeholder="통계표 목록 검색(신규)"/>
							<select id="newListStbl" style="float: left; margin: 0 30px 20px 20px; height: 342px;" size="20" multiple>
							</select>
							<div class="regProc step2 loading">
								<img src="/s-portalcnm/img/loading.gif" />
								<span>불러오는 중입니다...</span>
							</div>
						</div>						
						<div class="sbxbtn">
							<img class="remove" src="/s-portalcnm/image/button/btn_remove_on.gif" />
							<img class="append" src="/s-portalcnm/image/button/btn_add_on.gif" />
						</div>
						<div class="registeredDiv">
						<input type="text" id="registeredStblSearch" value="" placeholder="통계표 목록 검색(기존)" />
							<div class="regProc step2 loading">
								<img src="/s-portalcnm/img/loading.gif" />
								<span>불러오는 중입니다...</span>
							</div>
							<select id="registeredListStbl" style="float: right; margin: 0 30px 20px 20px; height: 342px;" size="20" multiple>
							</select>
						</div>
					</div>
					<div class="regProc step3">
						...
						<div class="stblInfoBox">
							<div class="stblInfo">
								<table class="stblTable" summary="조회조건">
									...
									<tbody>
										<tr>
											<th class="right">통계표</th>
											<td colspan="3">
												<select class="registeredListStbl_step3">
												</select>
											</td>
										</tr>
										<tr class="step1">
										...
										<tr class="step2">
											...
											<td colspan="3">
												...
												<input type="text" class="input_use03 validatebox-text" id="tblNm" name="tblNm"/>
											</td>
										</tr>
										<tr class="step3">
