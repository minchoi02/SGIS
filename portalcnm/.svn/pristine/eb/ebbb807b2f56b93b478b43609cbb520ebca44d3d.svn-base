...
(function (W, D) {
	...
	$(document).ready(function () {
		...
		$(id_datagrid).datagrid({
			...
			columns : [ [ {
				...
				formatter : function (value, row, index) {
					if (value != null && value != '') {
						...
						var title = value;
						if (row.POST_DEPTH == 0) {
							returnStr += "boardGridDetail.html?POST_NO=" + row.POST_NO + "&BOARD_CD=" + row.BOARD_CD+"&POST_DEPTH=" + row.POST_DEPTH;
						}
			...
			url : contextPath + "/ServiceAPI/QA/BoardManage/searchGrid.json"
		});
	...
	$DevQASearch = {
		...
		reqType : function () {
			...
			var BOARD_CD = "BOARD_012";
			sopOpenApiReqTypeObj.addParam('BOARD_CD', "BOARD_012");
...
function getQueryParamsObj () {
	...
	if (firstFlag == true) {
		...
		if (BOARD_CD != null && BOARD_CD != false && BOARD_CD != '') {
			queryParamsObj['BOARD_CD'] = "BOARD_012";
		}
