 ...
 *     
 */
public class DeleteKosisCode extends AbsAuth<Success> {
	private static final Logger logger = Logger.getLogger(DeleteKosisCode.class);
	@Resource
	...
	public Success executeAPI(HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			String sopCode = (String) paramMap.get("sop_cd");
			...
			} else if(sopCode.length() == 5) {
				deleteParamMap.put("area_type", "sgg");
			} else if(sopCode.length() == 7) {
			...
			return kosisManageDao.deleteKosisCode(insertParamMap, deleteParamMap);
