 ...
 *     
 */
public class UpdateMAPJIT extends AbsAuth<Success> {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(UpdateMAPJIT.class);
	@Resource
	private JITManageDao jitManageDao;
	@Override
	public String getApiId() {
		return "jitmanage_updatemapjit";
	}
	...
	public Success executeAPI(HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			if(!(paramMap.get("NM").toString().getBytes().length < 100 && paramMap.get("PARAM_VALUE").toString().getBytes().length < 4000))
			...
			return jitManageDao.updateMAPJIT(paramMap);
	...
	private enum MustParam{
		NM,API_B_CLASS_CD,API_M_CLASS_CD,SEQ,PARAM_VALUE,
	}
	@Override
