...
(function(W, D) {
	...
	$(document).ready(function(){
		...
		srvLogWrite("L0", "05", "02", "18", "", "");
		
		//request Question
		$reqQuestion.requestData();
		//click the contant button
		$('#toContantButton').click(function(){
			location.href = './../QA/DevAPISearch.html?BOARD_CD=' + BOARD_CD_RETURN + '&keywordSelect=' + keywordSelect + '&keywordInput=' + keywordInput + '&pageNumber=' + pageNumber;
		});
	});
	//request Question
	$reqQuestion = {
		requestData : function() {
			...
			sopOpenApiReqQuestionObj.request({
		        method : "POST",
		        async : false,
		        url : contextPath +"/ServiceAPI/QA/QAManage/getAPIQuestion.json"
		    });
		},
	    	...
	    	onSuccess : function(status, res) {
	            ...
	            if(res.errCd == "0") { 
	            	if(result != null){
	            		if(result.info != null){
	            			//등록일시   new date
		            		$('#APIDetailTbody').find('td').eq(0).text(result.info.REG_TS);
		            		//카테고리   type
		            		/*$('#APIDetailTbody').find('td').eq(1).text(result.info.S_CLASS_CD_NM);*/
		            		//조회수  query count
		            		$('#APIDetailTbody').find('td').eq(2).text(result.info.POST_HITS);
		            		//이름  name
		            		$('#APIDetailTbody').find('td').eq(4).text(result.info.MEMBER_NM);
		            		//연락처  phone
		            		$('#APIDetailTbody').find('td').eq(5).text(result.info.CP_NO);
		            		//이메일  email
		            		$('#APIDetailTbody').find('td').eq(6).text(result.info.EMAIL);
		            		//제목  title
		            		$('#APIDetailTbody').find('td').eq(7).text(result.info.POST_TITLE);
		            		//내용  content
		            		$('#APIDetailTbody').find('td').eq(8).html(result.info.POST_CONTENT.replace(/\n/g,'<br/>'));
	            		...
	            		$('#APIDetailTbody').find('td').eq(9).html(fileLines);
