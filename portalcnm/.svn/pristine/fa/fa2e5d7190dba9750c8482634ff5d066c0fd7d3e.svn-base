...
var gpkiScriptBase   = '/s-portalcnm/client';     // * 스크립트에서 사용하는 클라이언트 모듈의 기본 경로 (var.js의 경로)
var clientModulePath = '/s-portalcnm/client/setup';	// * 클라이언트 설치 파일의 게시 경로. (cab, exe, dmg, rpm, deb)
var ServiceStartPageURL = '/s-portalcnm/html/CM/login.jsp';		// * 설치완료후 이동할 페이지
var clientInstallPath = '/s-portalcnm/newGPKI/install.html';  	// * 설치 페이지 경로 
...
var clientVersion     = '1.1.2.6';				// * 클라이언트 버전. 버전 정보는 ','로 자리 구분. ( 21.09.15 1.1.2.6_ver 클라이언트 변경  )
...
if (browserType == BROWSER_IE) {
	if (browserPlatform == PLATFORM_WIN_X64) {
		...
		SetupOffLineFilePath   = clientModulePath + '/GPKISecureWebSetup.exe';
	...
	if (browserPlatform == PLATFORM_MAC) {
		...
		SetupOffLineFilePath = clientModulePath + '/GPKISecureWebSetup.exe'; // for Non-IE browser on Windows.
	}
