...
function doClick(event) {                                                            // 날자를 선택하였을 경우
    ...
    if(cal_Day)
    {
        if (cal_Day.length > 7) {                                                    // 날자 값이있으면
            ...
            target.focus();
            if(target.name === "sgis_census_req_y_s_d"){
            	var sgis_census_req_y_s_d = document.getElementsByName("sgis_census_req_y_s_d");
            	var sgis_census_req_y_e_d = document.getElementsByName("sgis_census_req_y_e_d");
                for(var i=0;i<sgis_census_req_y_s_d.length;i++){
                	stday = el.title;
                	if(enday === undefined){
                		enday = sgis_census_req_y_e_d[0].value.replace(/-/gi, "")
                	}
                	if(Number(stday) > Number(enday)){
                		alert("게시시작일이 게시종료일보다 빠릅니다. 다시입력해주세요");
                		return false;
                		
                	}else{
                		sgis_census_req_y_s_d[i].value=conv_date(cal_Day);
                	}
                }
            }else{
            	var sgis_census_req_y_s_d = document.getElementsByName("sgis_census_req_y_s_d");
            	var sgis_census_req_y_e_d = document.getElementsByName("sgis_census_req_y_e_d");
                for(var i=0;i<sgis_census_req_y_e_d.length;i++){
                	enday=el.title;
                	if(stday === undefined){
                		stday = sgis_census_req_y_s_d[0].value.replace(/-/gi, "")
                	}
                	if(Number(stday) > Number(enday)){
                		alert("게시시작일이 게시종료일보다 빠릅니다. 다시입력해주세요");
                		return false;
                		...
                		sgis_census_req_y_e_d[i].value=conv_date(cal_Day);
                	}
                }
            }
        }
