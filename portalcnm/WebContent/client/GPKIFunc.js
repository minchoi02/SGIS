...
{	
	...
	if( nResult == 0 ) 
	{
		...
		var target = './makeDSKeySeucre';
		target += '.'+addPageExt();		
		target += '?encryptedData='+encodeURIComponent(strReturnData)+'&mkdummy='+challenge[2];
...
function initSecureSession(sessionid){
	...
	if( (nResult != -1) && (GPKISecureWeb.SetSessionID(sessionid)==0) && (GPKISecureWeb.Encrypt(SiteID+sessionid, '111')==30053) )
