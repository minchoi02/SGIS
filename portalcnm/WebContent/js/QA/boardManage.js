...
(function(W, D) {
	...
	$(document).ready(function(){
		...
		$('#addButton').click(function(){
			srvLogWrite("L0", "05", "01", "02", "", "");
			location.href = './../QA/boardAdd.html?BOARD_CD='+$boardManage.BOARD_CD+'&searchWordType=' + searchWordType_SEND + "&searchWord=" + searchWord_SEND + "&pageNumber=" + PAGENUMBER_SEND + "&sort=" + SORT_SEND + "&order=" + ORDER_SEND + "&PRIORITY_DISP_YN=" + PRIORITY_DISP_YN_SEND;
		});
		$(id_datagrid).datagrid({
		    ...
		    columns:[[ 
		        ...
		        {field:'POST_TITLE',title:'제목 ',align:'left',width:335,sortable:true,order:'desc',
		        	formatter: function(value, row, index){
		        		...
		        		if(value != null && value != ''){
		        			var btitle = value;
		        			value ="<a href='./boardDetail.html?BOARD_CD="+$boardManage.BOARD_CD;
		        			value += "&POSTNO="+ row.POST_NO + "&searchWordType=" + searchWordType_SEND + "&searchWord=" + searchWord_SEND + "&pageNumber=" + PAGENUMBER_SEND + "&sort=" + SORT_SEND + "&order=" + ORDER_SEND + "&PRIORITY_DISP_YN=" + PRIORITY_DISP_YN_SEND +"' style='color:#4a4a4a;' onmouseover='$(this).css(\"text-decoration\", \"underline\")' onmouseout='$(this).css(\"text-decoration\", \"none\")'>"+ btitle; 
		        			if(row.FILE_YN=="Y"){
			...
			url:contextPath +"/ServiceAPI/QA/BoardManage/searchBoard.json"
		});
	...
	$boardManage = {
			delData : function(POST_NO_List) {
				...
				if( $boardManage.BOARD_CD ){
					sopOpenApiDelDatatObj.addParam('BOARD_CD', $boardManage.BOARD_CD);
				}
				sopOpenApiDelDatatObj.addParam('POST_NO_List', POST_NO_List);
				sopOpenApiDelDatatObj.request({
			        method : "POST",
			        async : false,
			        url : contextPath +"/ServiceAPI/QA/BoardManage/deleteBoard.json"
			    });
			}
	};
	(function() {
...
function getQueryParamsObj(){
	...
	queryParamsObj['BOARD_CD'] = $boardManage.BOARD_CD;
