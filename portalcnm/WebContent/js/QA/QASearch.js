...
var firstFlag = false;
(function (W, D) {
	W.$QASearch = W.$QASearch || {};
	//send
	var BOARD_CD_SEND = $QASearch.BOARD_CD;
		...
		$('#delQAButton').click(function () {
			...
			if (row == null || row.length == 0) {
				...
				$('#ok_confirmPopup').click(function () {
					...
					$QASearch.delQA(BOARD_CD_List, PARENT_POST_ID_List, POST_DEPTH_List, POST_ORDER_List);
					confirmPopupRemove();
				});
			...
			columns : [ [ {
				...
				formatter : function (value, row, index) {
					if (value != null && value != '') {
						...
						var title = value;
						if (row.POST_DEPTH == 0) {
							returnStr += "QADetail.html?POST_NO=" + row.POST_NO + "&BOARD_CD=" + row.BOARD_CD+"&POST_DEPTH=" + row.POST_DEPTH;
						}
						else if (row.POST_DEPTH > 0) {
							returnStr += "QAReply.html?POST_NO=" + row.POST_NO + "&POST_DEPTH=" + row.POST_DEPTH + "&PARENT_POST_ID=" + row.PARENT_POST_ID + "&BOARD_CD=" + row.BOARD_CD + "&type=update";
	...
	$QASearch = {
		...
		reqType : function () {
			...
			var BOARD_CD = $QASearch.BOARD_CD;
			sopOpenApiReqTypeObj.addParam('BOARD_CD', $QASearch.BOARD_CD);
				...
				if (res.errCd == "0") {
					if (result != null) {
						...
						for ( var i = 0; i < result.length; i++) {
							if( $QASearch.BOARD_CD == 'BOARD_014' ){
								if( result[i].S_CLASS_CD != 'THEMRQ' ){
...
function getQueryParamsObj () {
	...
	if (firstFlag == true) {
		...
		if (BOARD_CD != null && BOARD_CD != false && BOARD_CD != '') {
			queryParamsObj['BOARD_CD'] = $QASearch.BOARD_CD;
		}
