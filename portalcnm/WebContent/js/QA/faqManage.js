...
(function(W, D) {
	...
	$(document).ready(function(){
		...
		$('#delButton').click(function(){
			...
			if (row == null || row.length == 0) {
				...
				$('#ok_confirmPopup').click(function(){
			    	...
			    	$faqdManage.delData(POST_NO_List);
					confirmPopupRemove();
				});
		...
		$('#addButton').click(function(){
			srvLogWrite("L0", "05", "01", "13", "", "");
			location.href = './../QA/faqAdd.html?BOARD_CD='+$faqdManage.BOARD_CD+'&searchWordType=' + searchWordType_SEND + "&searchWord=" + searchWord_SEND + "&pageNumber=" + PAGENUMBER_SEND + "&sort=" + SORT_SEND + "&order=" + ORDER_SEND + "&PRIORITY_DISP_YN=" + PRIORITY_DISP_YN_SEND;
		});
		$(id_datagrid).datagrid({
			...
			url:contextPath +"/ServiceAPI/QA/BoardManage/searchFAQ.json"
		});
	...
	$faqdManage = {
			delData : function(POST_NO_List) {
				...
				sopOpenApiDelDatatObj.addParam('BOARD_CD', $faqdManage.BOARD_CD);
				sopOpenApiDelDatatObj.addParam('POST_NO_List', POST_NO_List);
				sopOpenApiDelDatatObj.request({
			        method : "POST",
			        async : false,
			        url : contextPath +"/ServiceAPI/QA/BoardManage/deleteFAQ.json"
			    });
			}
	};
	(function() {
...
function getQueryParamsObj(){
	...
	queryParamsObj['BOARD_CD'] = $faqdManage.BOARD_CD;
