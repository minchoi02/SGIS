...
(function(W, D) {
	...
	$(document).ready(function(){
		...
		$('#replyQuestionButton').click(function(){
			srvLogWrite("L0", "05", "01", "10", "", "");
			location.href = './../QA/QAReply.html?POST_NO=' + POST_NO + '&POST_DEPTH=' + POST_DEPTH + '&PARENT_POST_ID=' + PARENT_POST_ID + '&BOARD_CD=' + BOARD_CD + '&type=reply' + '&BOARD_CD_SEND=' + BOARD_CD_RETURN + '&S_CLASS_CD_NM=' + S_CLASS_CD_NM + '&keywordSelect=' + keywordSelect + '&keywordInput=' + keywordInput + '&pageNumber=' + pageNumber;
		});
	});
	//request Question
	$reqQuestion = {
	            ...
	            if(res.errCd == "0") { 
	            	if(result != null){
	            		if(result.info != null){
		            		...
		            		//이름  name
		            		$('#QADetailTbody').find('td').eq(3).text(result.info.MEMBER_NM);
		            		//연락처  phone
		            		$('#QADetailTbody').find('td').eq(4).text(result.info.CP_NO);
		            		//이메일  email
		            		$('#QADetailTbody').find('td').eq(5).text(result.info.EMAIL);
		            		//제목  title
		            		$('#QADetailTbody').find('td').eq(6).text(result.info.POST_TITLE);
		            		//내용  content
		            		$('#QADetailTbody').find('td').eq(7).html(result.info.POST_CONTENT.replace(/\n/g,'<br/>'));
	            		...
	            		$('#QADetailTbody').find('td').eq(8).html(fileLines);
	            	}
	            } else {
