	...
	$(document)
			.ready(
					function() {
						...
						$('#cancelButton').click(
								function() {
									location.href = "./../QA/DevboardManage.html?searchWordType=" + searchWordType + "&searchWord=" + searchWord + "&pageNumber="
											+ pageNumber + "&sort=" + sort + "&order=" + order + "&PRIORITY_DISP_YN=" + PRIORITY_DISP_YN;

								});
						// plupload plugins
						pluploader = new plupload.Uploader({
							url : contextPath + '/ServiceAPI/QA/BoardManage/devaddBoardFile.json',
	...
	$boardUpdate = {
		...
		reqBoard : function() {
			...
			sopOpenApiReqBoardObj.request({
				method : "POST",
				async : false,
				url : contextPath + "/ServiceAPI/QA/BoardManage/devgetBoard.json"
			});
		},
		updateBoardAfterFile : function(POST_NO, POST_TITLE, PRIORITY_DISP_YN, POST_CONTENT, DELETE_FILE_YN, FILE_YN, FILE_EXTENSION, FILE_ID, FILE_NM,
			...
			sopOpenApiUpdateBoardAfterFileObj.request({
				method : "POST",
				async : false,
				url : contextPath + "/ServiceAPI/QA/BoardManage/devupdateBoard.json"
			});
		},
		...
		$class("sop.openApi.updateBoardAfterFile.api").extend(sop.cnm.absAPI).define({
			onSuccess : function(status, res) {
				if (res.errCd == "0") {
					...
					$('#ok_alertPopup').click(function() {
						location.href = './../QA/DevboardManage.html';
						confirmPopupRemove();
					});
					$('#close_confirmPopup').click(function() {
						location.href = './../QA/DevboardManage.html';
		...
		$class("sop.openApi.updateBoardInner.api").extend(sop.cnm.absAPI).define(
				{
					onSuccess : function(status, res) {
						if (res.errCd == "0") {
							...
							$('#ok_alertPopup').click(
									function() {
										location.href = "./../QA/DevboardManage.html?searchWordType=" + searchWordType + "&searchWord=" + searchWord
