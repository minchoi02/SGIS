...
//			}
		...
		$(id_datagrid).datagrid({	
		    ...
		    columns:[[ 
		        ...
		        {field:'PUB_DATA_ID',title:'공공DataID',align:'center',width:100}, 
		        {field:'PUB_DATA_NM',title:'공공Data명',align:'center',width:197,
					formatter: function(value,row,index){						
						var PUB_DATA_ID = encodeURIComponent((row.PUB_DATA_ID));
						var PUB_DATA_NM = encodeURIComponent((row.PUB_DATA_NM));
						var PUB_DATA_YEAR = encodeURIComponent((row.PUB_DATA_YEAR));
						var INTRACTVMAP_APPLY_YN = encodeURIComponent((row.INTRACTVMAP_APPLY_YN));
						var BIZSTATMAP_APPLY_YN = encodeURIComponent((row.BIZSTATMAP_APPLY_YN));
						if (value != null && value != ''){
							return "<a onclick='openModifyPopup(\""+ PUB_DATA_ID+"\",\""+PUB_DATA_NM+"\",\""+PUB_DATA_YEAR+"\",\""+INTRACTVMAP_APPLY_YN+"\",\""+BIZSTATMAP_APPLY_YN+"\")' style='color:#4a4a4a;cursor:pointer' onmouseover='$(this).css(\"text-decoration\", \"underline\")' onmouseout='$(this).css(\"text-decoration\", \"none\")'>"+ value + "</a>";
						} else {
							return value;
						}
					}
				},
				{field:'PUB_DATA_YEAR',title:'년도',align:'center',width:100}, 
				{field:'INTRACTVMAP_APPLY_YN',title:'대화형적용여부',align:'center',width:160},
				{field:'BIZSTATMAP_APPLY_YN',title:'생활업종적용여부',align:'center',width:160}				
		    ]],
		    queryParams: {
		    	SEARCH_TYPE_INTRACTV:$('#SEARCH_TYPE_INTRACTV').val(),
		    	SEARCH_TYPE_BIZSTAT: $('#SEARCH_TYPE_BIZSTAT').val()
			},
			...
			url:contextPath +"/ServiceAPI/DT/PubDataManage/searchPubData.json"
		});
