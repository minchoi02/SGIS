...
//			}
		...
		$(id_datagrid).datagrid({	
			...
			columns:[[ 
			          ...
			          {field:'CATEGORY_ICON_ID',hidden: true},
			          {field:'CATEGORY_ICON_URL',title:'아이콘', image:true, align:'center',width:100,
			        	  formatter: function(value,row,index){						
			        		  if (value != null && value != ''){
			        			  var CATEGORY_ICON_ID = encodeURIComponent((row.CATEGORY_ICON_ID)).trim();
			        			  return "<img style='margin-left:5px;' height='60' width='50' src='./../.."+value+"' onclick='clickPOP(\""+ CATEGORY_ICON_ID.trim()+"\")' />";
			        		  } else {
			        			  return value;
			        		  }
			        	  }
			          }, 
			          {field:'EXP',title:'설명',align:'center',width:70,
			        	  formatter: function(value,row,index){						
			        		  var CATEGORY_ICON_ID = encodeURIComponent((row.CATEGORY_ICON_ID)).trim();
			        		  if (value != null && value != ''){
			        			  return "<a onclick='clickPOP(\""+ CATEGORY_ICON_ID.trim()+"\")' style='color:#4a4a4a;cursor:pointer' onmouseover='$(this).css(\"text-decoration\", \"underline\")' onmouseout='$(this).css(\"text-decoration\", \"none\")'>"+ value + "</a>";
			        		  } else {
			        			  return value;
			        		  }
			        	  }
			          ...
			          url:contextPath +"/ServiceAPI/DT/ThemaMapManage/getThemaICON.json"
		});
...
function clickPOP(CATEGORY_ICON_ID){

	//opener.document.getElementById('data_test').value="에헴";

	$('#CATEGORY_ICON',opener.document).val(decodeURI(CATEGORY_ICON_ID));
	self.close();
