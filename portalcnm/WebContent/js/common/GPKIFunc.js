...
function Init()
{
	...
	//ubkey 사용시 세팅한다.
	//GPKISecureWeb.UbiKeyInit(UbikeyVersion,UbikeyPopupURL,UbikeyWParam,UbikeylParam);
...
// 브라우저에 embeded 된 인증서 사용자인터페이스를 이용한 로그인 
/*********************************************************************/
//                   LoginEmbedded	                //
/*********************************************************************/
...
{	
	...
	{
		document.gpkiForm.encryptedData.value = strReturnData;
	var strUrl = link.href;
	strUrl = strUrl.replace(/\/\//g,"/");
	link.href = strUrl;
		//document.gpkiForm.action = link;
		document.gpkiForm.action = "gpkiLoginForm.jsp?";
...
{	
	...
	link.href = strUrl;
		//document.gpkiForm.action = link;
		document.gpkiForm.action = "gpkiLogin.jsp?";
		if ( link.target == "" || link.target == null ) {
			document.gpkiForm.target="_self";
		}else{
			document.gpkiForm.target=link.target;
