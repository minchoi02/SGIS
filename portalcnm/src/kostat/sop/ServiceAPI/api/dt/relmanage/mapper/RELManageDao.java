package kostat.sop.ServiceAPI.api.dt.relmanage.mapper;
 ...
 */
@Component
public class RELManageDao extends SqlSessionDaoSupport {
	@Resource
	...
	public Map searchREL(Map paramMap) {	
		...
		resultMap.put("total", getSqlSession().selectList("RELManage.getSearchRELCount",paramMap));
		resultMap.put("rows", getSqlSession().selectList("RELManage.getSearchREL",paramMap));
		return resultMap;
	}
	...
	public Success addREL(Map paramMap) {
		Success success = new Success(false,Prompt.ADDFAIL);
		if((int) getSqlSession().insert("RELManage.addREL",paramMap) > 0){
			success.setSuccess(true);
	...
	public Success updateREL(Map paramMap){
		...
		if((int) getSqlSession().update("RELManage.updateREL",paramMap) > 0){
			success.setSuccess(true);
	...
	public Success deleteREL(String[] list) {
		...
		if((int)getSqlSession().delete("RELManage.deleteREL",list) > 0){
			success.setSuccess(true);
