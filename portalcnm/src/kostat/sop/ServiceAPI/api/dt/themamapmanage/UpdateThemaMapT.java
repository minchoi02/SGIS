 ...
 *    
 */
public class UpdateThemaMapT extends AbsAuth<Success> {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(UpdateThemaMapT.class);
	@Resource
	...
	public Success executeAPI(HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			/*201.03.21 수정*/
			Boolean flag = paramMap.get("TITLE").toString().getBytes().length < 100 && paramMap.get("EXP").toString().getBytes().length < 4000;
			if(!flag)
			...
			return themaMapManageDao.updateThemaMapt(paramMap);
	...
	private enum MustParam{
		STAT_THEMA_MAP_ID,CATEGORY,SRV_YN,TITLE,EXP
	}
	private enum OptionParam{
		ARTICLE_DIV,AREA_SET,STAT_THEMA_MAP_FILE_URL,STAT_DISP_LEVEL,METHOD,RELATEINFO
	}
