...
import kostat.sop.OpenAPI3.api.startupbiz.dto.CorpindecreaseMap;
...
public class Corpindecrease extends AbsAuthAPI< List >
{
	private static final Log logger = LogFactory.getLog( Corpindecrease.class );
	...
	public List executeAPI( HttpServletRequest req, HttpServletResponse res, String trId ) throws AbsException
	{
		...
		try
		{
			...
			List year_list = session.selectList( "startupbiz.getcorpindecreaseyear", mapParameter );

			for( int i = 0; i < year_list.size(); i++ )
			{
				mapParameter.put( "data_year", year_list.get( i ) );
				CorpindecreaseMap resultList = (CorpindecreaseMap) session.selectOne( getQueryStr(), mapParameter );
...
//			e.printStackTrace();
	...
	{
		return "startupbiz.corpindecrease";
	}
