package kostat.sop.OpenAPI3.view;
...
/**
 * XML 생성을 위한 최상위 추상클래스로 XML변환을 위한 기본적인 함수 구현
 */
@SuppressWarnings("rawtypes")
public abstract class AbsXmlFormat<T> extends AbsFormat<T> {
	...
	protected void _makeNFArray(NFArray array, Element element) throws AbsException {
		...
		for (int i = 0; i < arrSize; i++) {
			...
			if (data instanceof NFArray) {
				...
				newDataTag = _addElement(element, "data");
			}
			//newDataTag = _addElement(element, data.getClass().getSimpleName());
