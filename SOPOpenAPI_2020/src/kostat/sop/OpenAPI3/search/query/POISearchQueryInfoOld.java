...
public class POISearchQueryInfoOld extends AbsQueryInfo
{
	private static final Log logger = LogFactory.getLog(POISearchQueryInfoOld.class);
	private static Version version = Version.LUCENE_44;
	
	public POISearchQueryInfoOld(Map<String, String> qMap)
	{
...
//					tmpBbooQuery2.add(wildQuery, BooleanClause.Occur.MUST);
					logger.debug("arrAddrSyn : "+arrAddrSyn[ix]);
					addQeury( booQuery
							, enSearchIndex.addr_syn.name()
							, arrAddrSyn[ix]
							, BooleanClause.Occur.MUST
							, true
							, QueryParser.OR_OPERATOR);//.AND_OPERATOR);
...
//				booQuery.add(tmpBbooQuery2, BooleanClause.Occur.MUST);
			...
			if(!getQueryMap().get(enSearchIndex.adm_cd.name()).equals("")) {
				WildcardQuery wildQuery = new WildcardQuery(new Term( enSearchIndex.adm_cd.name(), getQueryMap().get(enSearchIndex.adm_cd.name())+"*"));
...
//						, QueryParser.AND_OPERATOR);//.AND_OPERATOR);
		...
		logger.debug("booQuery="+booQuery.toString());
