...
public class ByteUtils
{
        ...
        throws IllegalArgumentException, NumberFormatException
    {
        ...
        if(radix != 16 && radix != 10 && radix != 8)
            throw new IllegalArgumentException((new StringBuilder("For input radix: \"")).append(radix).append("\"").toString());
        int divLen = radix != 16 ? 3 : 2;
        int length = digits.length();
        if(length % divLen == 1)
            throw new IllegalArgumentException((new StringBuilder("For input string: \"")).append(digits).append("\"").toString());
        length /= divLen;
