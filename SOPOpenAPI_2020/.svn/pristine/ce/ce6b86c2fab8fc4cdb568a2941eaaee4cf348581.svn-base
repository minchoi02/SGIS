...
import kostat.sop.OpenAPI3.service.technicalbiz.mapper.SidoCompanyInfoMapper;
 ...
 */
@Service( "sidoCompanyInfoService" )
public class SidoCompanyInfoServiceImpl extends EgovAbstractServiceImpl
		implements SidoCompanyInfoService
{
	@Resource(name = "sidoCompanyInfoMapper")
	private SidoCompanyInfoMapper mapper;
...
//		List< Map< String, String > > sidoCdList = mapper.selectSidoInfo(parameter);
		...
		for(int i=0; i<sidoCdList.size(); i++){
			...
			String totalCnt = sidoCdList.get(i).get("techbiz_corp_total_cnt");
			
			Map tempParam = new HashMap();
			tempParam.put("sido_cd", sidoCd);
			
			listTemp = mapper.selectSidoCompanyInfo(tempParam);
			
			//2017.03.24 시도별 기술업종현황 비율정보 수정
			for (int k=0; k<listTemp.size(); k++) {
				int tmpCorpCnt = (Integer)listTemp.get(k).get("techbiz_corp_cnt");
				float tmpTotalCorpCnt = Float.parseFloat(totalCnt);
				float tmpCorpPer = (tmpCorpCnt / tmpTotalCorpCnt) * 100;
				listTemp.get(k).put("techbiz_corp_per", String.format("%.1f", tmpCorpPer));
			}
			
			mapTemp.put("sido_cd", sidoCd);
			mapTemp.put("sido_nm", sidoNm);
			mapTemp.put("techbiz_corp_total_cnt", totalCnt);
