(function(W,D){
	W.$collectCareer3 = W.$collectCareer3 || {};
	...
	$(document).ready(function(){	
		...
		$collectCareer3.event.setUIEvent();
	});
	...
	$collectCareer3.ui = {
			...
			collectCareerPaging : function(totalCount, totalPage, pageSize, data, pageIndex) {
				...
				$('.paging').paging({
					...
					onclick : function(e, page){
						$collectCareer3.ui.start = (page - 1) * pageSize;
						$collectCareer3.ui.currentPage = page;
						$collectCareer3.request.doReqcollectCareerList($collectCareer3.ui.start);
					}
				});
			},
			...
			setCareerListTable : function(data, pageSize, curPage) {
				...
				$("#collectCareer3 > tbody").empty();
				$("#collectCareer3 > tbody:last").append(html);
			}
	};
	
	//AJAX 내용 작성
	$collectCareer3.request = {
			//품질 검사 현황 정보 조회
			doReqcollectCareerList : function(start){
				
				var options = $collectCareer3.ui.setParams(start);
				$ajax.requestApi(contextPath + "/api/collect/getCollectCareerList3.do", options, function(res){
					
					switch(parseInt(res.errCd)){
						case 0:
							var result = res.collectCareer3;
							$("#collectCareer3 > tbody").empty();
							
							if (result.length > 0){
								var totalpage = Math.ceil(result[0].total / $collectCareer3.ui.maxCntPerPage);
								$collectCareer3.ui.collectCareerPaging(result[0].total, totalpage, $collectCareer3.ui.maxCntPerPage, result, $collectCareer3.ui.currentPage);
								$collectCareer3.ui.setCareerListTable(result, $collectCareer3.ui.maxCntPerPage, $collectCareer3.ui.currentPage);
							}
							else {
								$collectCareer3.ui.setEmptyListTable();		
								}
							break;
						default:
			...
			setUIEvent : function(){
				...
				$(document).on("click","#msgOkBtn", function(){
					...
					if(postCnt > 0){
						...
						$ajax.requestApi(contextPath + "/api/collect/deleteCollectCareer3.do", options, function(res){
							switch(parseInt(res.errCd)){
								case 0:
									$messageNew.open("알림", "삭제되었습니다");
									window.location.href = contextPath+"/view/collectData/collectCareer3";
									break;
								default:
				...
				$collectCareer3.ui.currentPage = 1;
