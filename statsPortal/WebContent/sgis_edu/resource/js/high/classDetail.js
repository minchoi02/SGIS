(function(W, D) {
	...
	$(document).ready(function() {
		srvLogWrite('T0','02','03','02','H','contents_id='+contents.contentsId);
	...
	contents = {
		...
		makeContents : function () {
			...
			$.ajax({
				...
				//콘텐츠 정보 
				$(".menuTi").html("<a href='/view/edu/high/classList?thema_id="+res.result.contents.thema_id+"'>"+res.result.contents.thema_nm+"</a>");
				...
				//콘텐츠 자세한 설명보기 > 기능 소거 2021.10
//				if("" != res.result.contents.link_detail && null != res.result.contents.link_detail){
//					$("#detailExp").css("display","inline-block");
//					$("#popupDetail").load(res.result.contents.link_detail);
//				}			
				//콘텐츠 sgis 활용하기 
				...
				//다음콘텐츠
				if("" != res.result.nextContents && null != res.result.nextContents){
					$(".nextContents").attr("onclick","javascript:logWriteAndMove('/view/edu/high/classDetail?contents_id="+res.result.nextContents.contents_id+"','T0','02','03','05','H','contents_id="+res.result.nextContents.contents_id+"')");
					...
					$(".nextContents").attr("onclick","location.href='/view/edu/high/classList?thema_id=H01'");
		...
		makeRecommend : function(){
			var html = '';
			//추천 콘텐츠 목록(해시태그 형태로 표시)
			$.each(contents.recommendList, function(i, val){
				$("#recommendTag"+ i).text(val.contents_title);
				$("#recommendTag"+ i).attr("href", "javascript:logWriteAndMove('/view/edu/high/classDetail?contents_id="+val.contents_id+"','T0','02','03','06','H','contents_id="+val.contents_id+"')");
			})
