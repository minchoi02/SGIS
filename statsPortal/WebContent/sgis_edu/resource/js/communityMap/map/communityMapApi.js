...
(function(W, D) {
	...
	$communityMapApi.request = {
		...
		getLbdmsData : function( seq ){
			var sopPortalGetLbdmsDataApi = new sop.portal.getLbdmsData.api();
			sopPortalGetLbdmsDataApi.addParam("seq", seq);
			sopPortalGetLbdmsDataApi.request({
				method : "POST",
				async : false,
				url : contextPath + "/ServiceAPI/community/communityLbdmsData.json"
			});
		},
		/**
		...
		getIndustrycode : function(class_deg,class_code,callback){
			...
			if(!$.isNumeric(class_deg)||!/^8|9|10$/.test(class_deg)){
				class_deg = 10;
			}
	        ...
	        onSuccess : function(status, res, options) {
	        	if(res.errCd == "0") {
	        		...
	        		for(var i=0; i< uploadData.length; i++){
						...
						var dataNameColumn = $communityMap.ui.mydata.dataNameColumn;
	        			dataNameColumn = ( ( dataNameColumn == 'null' || dataNameColumn == null ) ? '' : dataNameColumn );

	        			var html  = "<table style='margin:10px;'>";
						html += 	"<tr>";
						html += 		"<td style='font-size:14px;font-weight:bold;color:#3792de;'>"+$communityMap.ui.mydata.dataName +" : "+dataList[ dataNameColumn ]+"</td>";
						html +=		"</tr>";
						html +=		"<tr style='height:5px;'></tr>";
						for(var x = 0; x < $communityMap.ui.mydata.dataColumn.length; x ++){
							var dataColumnX = dataList[ $communityMap.ui.mydata.dataColumn[x] ];
							dataColumnX = ( ( dataColumnX === 'null' || dataColumnX == null ) ? '' : dataColumnX );

							html += 	"<tr>";
							html +=     	"<td style='font-size:12px;padding-left:5px;'>"+$communityMap.ui.mydata.data[x]+" : "+ dataColumnX +"</td>";
							html +=		"</tr>";
						}
						html += "</table>"; 
	...
	/**
	 * @name         : LBDMS 데이터 저장 목록 가져오기
	 * @description  : LBDMS 데이터목록을 지도에 마커생성 및 표출 
	 * @date         : 2018. 4. 20. 
	 * @author	     : 
	 * @history 	 :
	 * @param
	 */
	(function() {
	    $class("sop.portal.getLbdmsData.api").extend(sop.portal.absAPI).define({
	        onSuccess : function(status, res, options) {
	        	if(res.errCd == "0") {
	        		
	        		if($communityMapCommon.hasText($communityMap.ui.lbdmsMarkers)){
	        			$.each($communityMap.ui.lbdmsMarkers,function(){
	        				this.remove();
	        			});
	        		}
	        		
	        		var lbdmsData = [];
	        		lbdmsData = res.result;
	        		
	        		$communityMap.ui.lbdmsData = {
        				data : lbdmsData
	        		};
	        		...
	        		for(var i=0; i< lbdmsData.length; i++){
	        			var x_coord = lbdmsData[i].x;
	        			var y_coord = lbdmsData[i].y;
...
//						});
	        		...
	        		$communityMap.ui.lbdmsMarkers = markers;
    			};
	        },
	        onFail : function(status, options) {
