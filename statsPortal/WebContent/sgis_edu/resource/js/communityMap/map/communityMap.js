...
(function(W, D) {
	...
	$communityMap.ui = {
		...
		createMap: function(id, seq, view) {
			...
			if((!/^\/view\/edu\/.{3,4}\/community\/together_form/.test(location.pathname))&&(!$communityMapCommon.hasText(communityMapInfo.stats)||communityMapInfo.stats.length<=0)){
			...
			map.gMap.whenReady(function() {
				if(/^\/view\/edu\/.{3,4}\/community\/together_form/.test(location.pathname) ){
				...
				try{
					if(defaultCurCenter){
						...
						setTimeout(function(){
							if(defaultLbdms){
								$communityMapApi.request.getLbdmsData(defaultLbdms);
							}
						}, 500);
...
//				$("header").css({"height":"104px", "width":"970px"});
		...
		reportDataSet : function() {
			srvLogWrite( "H0", "03", "10", "00", $communityMapCommon.getParameter("cmmnty_map_id"), "" ); //jrj 로그
			
			var map = $communityMap.ui.mapList[$communityMap.ui.curMapId];
			$communityMap.ui.reportPopup = window.open(contextPath + "/sgis_edu/resource/js/communityMap/report/communityReportForm.html?cmmnty_map_id="+$communityMapCommon.getParameter("cmmnty_map_id"), "reportPrint","width=850, height=700, scrollbars=yes");
		},
		/**
		 * @name         : reportLoad
		...
		getSHPfile : function(){
			if($communityView.ui.markers.length>0){
				srvLogWrite( "H0", "03", "09", "00", $communityMapCommon.getParameter("cmmnty_map_id"), "" ); //jrj 로그
				var param = "cmmnty_map_id="+$communityMapCommon.getParameter("cmmnty_map_id");
				location.href=contextPath+$communityMapCommon.edu_url+"/fileDownLoadSHP?"+param;
			}else{
	...
	$communityMap.event = {
		...
		setUIEvent: function() {
			accessTokenInfo(function(){
				...
				if( $communityMap.ui.myDataVal ){
					$communityMapApi.request.getMyData( $communityMap.ui.myDataVal );
				}
				
				if( $communityMap.ui.lbdmsVal ){
					$communityMapApi.request.getLbdmsData( $communityMap.ui.lbdmsVal );
				}
