...
(function(W, D) {
	$(document).ready(function(){
		...
		$communityMapCommon.cmmnty_map_id = $communityMapCommon.getParameter("cmmnty_map_id");
		
		$("#wrap").prop("style","position:initial !important;");	
	});
		...
		setUIEvent: function() {
			...
			$("#mpsFile").change(function(){
				...
				initValid(function(){
					...
					formData.append("cmmnty_map_id",$communityMapCommon.cmmnty_map_id);
					$.ajax({
						url: contextPath+$communityMapCommon.edu_url+"/getBatchData",
						data: formData,
						processData: false,
						contentType: false,
						async: true,
			...
			$("#gioCoding").click(function(){
				...
				var msgTxt = "지오코딩은 지도에 표시할 X, Y 좌표을 찾는 과정입니다.<br>이미 X, Y 좌표가 있고, 지오코딩을 실행하지";
				msgTxt += " 않으시려면 “좌표로 지오코딩”을 클릭하세요.<br>X, Y 좌표값을 찾기 위해서는 “주소”정보가 필요합니다. ";
				msgTxt += "“주소로 지오코딩”을 클릭하시면 자동으로 지오코딩이 실행됩니다.";
					...
					"알림",msgTxt,[{
						...
						func : function(opt) {
							//init(function(){
								...
								$communityBatch.ui.sCount = 0;
								$communityBatch.ui.fCount = 0;
								
								var coordData = $("#basic_handson").handsontable("getData");

								$.each( coordData ,function(cnt,node){
									$("#basic_handson").handsontable("setDataAtCell", cnt, 0, "진행중");
						...
						func : function(opt) {
							//init(function(){
								...
								var addrData = $("#basic_handson").handsontable("getDataAtCol",3);

								console.log('======5');

								$.each( addrData ,function(cnt,node){
									$("#basic_handson").handsontable("setDataAtCell", cnt, 0, "진행중");
			...
			$("#save-button").click(function(){
				if(isValidPass()){
					...
					$.ajax({
						type: "POST",
						url : contextPath + $communityMapCommon.edu_url +"/poi/regist",
						data:{
							cmmnty_map_id:$communityMapCommon.cmmnty_map_id,
						...
						success: function(res) {
							abs.onBlockUIClose();
							if(res.success){
								srvLogWrite( "H0", "03", "07", "02", $communityMapCommon.cmmnty_map_id, "" ); //jrj 로그

								$communityMapCommon.alert("알림","등록되었습니다",function(){
									location.href = contextPath+$communityMapCommon.edu_url+"/together_view?cmmnty_map_id="+$communityMapCommon.cmmnty_map_id;
								});
							}else{
								if(res.message){
									$communityMapCommon.alert("알림",res.message);
	...
	function incrementCount(id){
		if( id == 'successCount' ){
			$communityBatch.ui.sCount ++;
			$("#successCount").html("").html( $communityBatch.ui.sCount );
		} else if( id == 'failCount' ){
			$communityBatch.ui.fCount ++;
			$("#failCount").html("").html( $communityBatch.ui.fCount );
		} else {
