...
(function(W, D) {
	...
	$communityLeftMenu.ui = {
		...
		//신고하기 버튼, 신고취소 버튼 클릭 이벤트
        reportBtnClick : function( cancel ){
        	if( !cancel ){
        		$communityLeftMenu.ui.leftLayerPopupAllClose();
        		$('#poi-detail-report-layer-popup input').val(null);
        		$('#poi-detail,#poi-detail-report-layer-popup').addClass('M_on');
        		$(".mCSB_container").css("height","500px");
        		$(".register_layer").css("left","1px");
        	} else {
        		$(".register_layer").css("left","");
		 ...
		 * @history 	 :
		 */
		defaultLbdms : function() {
			if($("#defaultLbdmsBtn").hasClass("on")) {
				$("#defaultLbdmsBtn").removeClass("on");
				$("#defaultLbdmsBtn").text("off");
				
				$.each($communityMap.ui.lbdmsMarkers,function(cnt,node){
					node.remove();
				});
			} else {
				$("#defaultLbdmsBtn").addClass("on");
				$("#defaultLbdmsBtn").text("on");
				$.each($communityMap.ui.lbdmsMarkers,function(cnt,node){
					node.addTo($communityMap.ui.mapList[$communityMap.ui.curMapId].gMap);
				});
			}
	...
	$communityLeftMenu.event = {
		...
		setUIEvent: function() {
			...
			function setStats(element){
				...
				var statsName = element.find("span").text();
				statsName = ( statsName ? statsName.replace(/\s/gi,"") : "" );
				
				if(element.data("type")=="history"){
					srvLogWrite( "H0", "03", "02", "01", statsName, "" ); //jrj 로그
					...
					srvLogWrite( "H0", "03", "02", "02", statsName, "" ); //jrj 로그
			...
			$("#share-facebook").click(function(){
				if($communityLeftMenu.ui.poiRegist.checkStartDate()){
					srvLogWrite( "H0", "03", "06", "01", $communityMapCommon.getParameter("cmmnty_map_id"), "" ); //jrj 로그
			...
			$("#share-twitter").click(function(){
				if($communityLeftMenu.ui.poiRegist.checkStartDate()){
					srvLogWrite( "H0", "03", "06", "02", $communityMapCommon.getParameter("cmmnty_map_id"), "" ); //jrj 로그
			...
			onSuccess : function(status, res, options) {
				if(res.errCd == "0") {
					if(res.result.success){
						srvLogWrite( "H0", "03", "07", "03", $communityMapCommon.getParameter("cmmnty_map_id"), "" ); //jrj 로그
