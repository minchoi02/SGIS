(function(W, D) {
	...
	$(document).ready(function() {
		...
		srvLogWrite( "H0", "01", "07", "02", "", "" ); //jrj 로그
	});
	W.$notice = W.$notice || {};
	$notice.ui = {
		search : function(){
			...
			srvLogWrite( "H0", "01", "07", "03", "", 
					$("#notice_search_select option:selected").text() + " : " + search_word ); //jrj 로그
			...
			success: function(res) {
				if(res.errCd=="0"){
					...
					$.each(res.result.summaryList,function(cnt,node){
						ul.append($("<li/>").append(
							$("<a/>",{"href":contextPath + $communityMapCommon.edu_url +"/notice/view?post_no=" + node.post_no+addQuery,"html":(page_number-cnt)+"&nbsp;&nbsp;&nbsp;"+node.post_title.replace(/\n/gim, "<br>")}).append(
