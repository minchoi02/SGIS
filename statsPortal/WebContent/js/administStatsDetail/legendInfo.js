			...
			this.initialize = function(delegate) {
				...
				if (this.delegate != undefined && this.delegate.namespace != undefined && this.delegate.namespace === "houseAnalysisMap") {
					...
					if($administStatsMain.ui.selectedThema == "신혼부부" || $administStatsMain.ui.selectedThema == "가구" || $administStatsMain.ui.selectedThema == "주택" || $administStatsMain.ui.selectedThema == "상세"){
						...
						this.legendColor1 = "#CB5B1B"; //2020.11.17[신예리] 컬러 변경
						this.legendColor2 = "#F5A020"; //2020.11.17[신예리] 컬러 변경
					} else if($administStatsMain.ui.selectedThema == "임업"){
						this.legendColor1 = "#4F7001"; //2020.11.17[신예리] 컬러 변경
						this.legendColor2 = "#D4D05D"; //2020.11.17[신예리] 컬러 변경
					} else if($administStatsMain.ui.selectedThema == "어업"){
						if($fisheryDash.fisheryType=="sea"){ // "#1A4351", "#60A8C2"
							//this.legendColor1 = "#0023AE";
							//this.legendColor2 = "#B7CFF3";
							this.legendColor1 = "#1A4351";
							this.legendColor2 = "#60A8C2";
						} else { // "#360D3E", "#BE86C9" (내수면)
							//this.legendColor1 = "#054FA4";
							//this.legendColor2 = "#74B8E7";
							this.legendColor1 = "#360D3E";
							this.legendColor2 = "#BE86C9";
						}
					} else {
						this.legendColor1 = "#cd1103";
						this.legendColor2 = "#ffd75d";
		...
		this.getColor = function(value, valPerSlice) {
			...
			if (valPerSlice.length < this.lv && that.legendType == "auto") {
				...
				for(var i = 1; i<valPerSlice.length;i++){ //2016.04.04 범례
					if(value <= valPerSlice[i]){
						// 2020-10-26 [곽제욱] 선택한 아이템이 존재할경우 범례 색 변경 START
						if($administStatsMap.ui.selectedPK != "" && $administStatsMap.ui.selectedPK != null){
							return [$.trim(that.selectedLegendColor[i-1]), i-1]; //2016.04.04 범례
						} else {
							return [$.trim(that.legendColor[i-1]), i-1]; //2016.04.04 범례
						}
						// 2020-10-26 [곽제욱] 선택한 아이템이 존재할경우 범례 색 변경 END
					}
				}
				// 2020-10-26 [곽제욱] 선택한 아이템이 존재할경우 범례 색 변경 START
				if($administStatsMap.ui.selectedObj != "" && $administStatsMap.ui.selectedObj != null){
					return [$.trim(that.selectedLegendColor[that.lv-1]), that.lv]; //2016.04.04 범례
				} else {
...
//					srvLogWrite("C0", "08", "05", "00", "", "");		//단계설정 3종
		...
		this.createLegend = function() {
			...
			infoLegend.update = function(props) {
				...
				this._div.innerHTML = '<div class="legendRing legendBox max" id="legendBox_'+that.id+'" data-ing="max">' // 2020-12-11 [곽제욱] data-html2canvas-ignore=true 제거
						...
						+'</ul>'
						+'<a href="javascript:void(0)" id="legendPopEvent00_'+that.id+'"><span>타입</span></a>'
						+'</li>'
						+'<li><a href="javascript:void(0)" id="legendPopEvent01_'+that.id+'"class="legendPopEvent" data-id="guganSettingLayer"><span>단계</span></a>'
						+'</li>'
