 ...
 */
(function(W, D) {
	W.$administStatsTmsMap = W.$administStatsTmsMap || {};
	$(document).ready(
		function() {});
...
//					}
	...
	$administStatsTmsMap.ui = {
			map : null,
			namespace : "administStatsTmsMap",
			mapList : [],
			mapInfoList : [],
			curMapId : 0,
			reportPopup : null,
			...
			getLogParams : function(){
				var params = "지역:"+$administStatsTmsMap.ui.settingInfo["adm_nm"];
				return params;
			},
			...
			drawMapData : function(p_map_region, p_map_type, p_map_gbn, year) {
				...
				var selMap;
				if(p_map_gbn == "left"){
					selMap = $administStatsTmsMap.ui.mapList[0];
				}
				else if(p_map_gbn == "right"){
					selMap = $administStatsTmsMap.ui.mapList[1];
				}
				
				var lv_map_region_before = $administStatsTmsMap.ui.mapRegion;
				var lv_map_type_before = $administStatsTmsMap.ui.mapType;
				var lv_surv_id = $administStatsTmsMap.ui.selectedSurvId;
				
				if(p_map_region == undefined || p_map_region == null) {
					p_map_region = $administStatsMain.ui.mapRegion;
				}
				else {
					$administStatsTmsMap.ui.mapRegion = p_map_region;
				}
				if(p_map_type == undefined || p_map_type == null) {
					p_map_type = $administStatsMain.ui.mapType;
				}
				else {
					$administStatsTmsMap.ui.mapType = p_map_type;
				}
				...
				// TODO :: clear 
				//지도 Clear
				$administStatsTmsMap.ui.clearMap(selMap);
				...
				if(p_map_type == "color" || p_map_type == "bubble") {
					...
					if(p_map_region == "sido") {
						//
						$administStatsTmsMap.ui.setAdministStatsData(selMap, "sido", "color", "", "", "",  lv_surv_id, year, function(p_list) {
							//리스트에서 unit 가져오기
							var lv_unit = "개";
							var lv_unit_nm = "수";
							...
							//경계 그리기
							$administStatsTmsMap.ui.setAdministStatsRegion(selMap, "sido", year, "", "", "", p_map_gbn, function() { // 2020-10-15 [곽제욱] 로딩바 완료처리를 위한 p_map_gbn 구분자 추가
								if($administStatsTmsMap.ui.mapToggleId != "" && $administStatsTmsMap.ui.mapToggleId != null){ // 2020-10-08 [곽제욱] tmsMapToggld 로 변경
									// 랭크이동시 mapToggleId를 초기화하고 하이라이트 처리
									var highLightAmdCd = $administStatsTmsMap.ui.mapToggleId; // 2020-10-08 [곽제욱] tmsMapToggld 로 변경
									// 맵토글ID 초기화
									$administStatsTmsMap.ui.mapToggleId = "";// 2020-10-08 [곽제욱] tmsMapToggld 로 변경
									selMap.setPolyLayerHighlight2(highLightAmdCd, p_map_gbn);
									//$administStatsTmsMap.ui.mapList[0].setPolyLayerHighlight2(highLightAmdCd, "left"); // 2020-10-08 [곽제욱] 시계열 맵 전용 하이라이트 처리함수로 변경
									//$administStatsTmsMap.ui.mapList[1].setPolyLayerHighlight2(highLightAmdCd, "right"); // 2020-10-08 [곽제욱] 시계열 맵 전용 하이라이트 처리함수로 변경
									// 하이라이트 처리 후 맵토글ID 세팅
									$administStatsTmsMap.ui.mapToggleId = highLightAmdCd; // 2020-10-08 [곽제욱] tmsMapToggld 로 변경
								}
							});
					...
					//색상/버블 (시군구)
						...
						$administStatsTmsMap.ui.setAdministStatsData(selMap, "sgg", "color", lv_sido_cd, lv_sgg_cd, "", lv_surv_id, year, function(p_list) {
							//리스트에서 unit 가져오기
							var lv_unit = "개";
							var lv_unit_nm = "수";
							if(p_list != null && p_list.length > 0) {
...
//							}
							...
							$administStatsTmsMap.ui.setAdministStatsRegion(selMap, lv_region, year, lv_sido_cd, "", "", p_map_gbn, function() { // 2020-10-15 [곽제욱] 로딩바 완료처리를 위한 p_map_gbn 구분자 추가
								
								if($administStatsMain.ui.selectedThema == "인구"){
									$administStatsMain.ui.selectedLevel = 2;
								}
								if($administStatsTmsMap.ui.mapToggleId != "" && $administStatsTmsMap.ui.mapToggleId != null){
									// 랭크이동시 mapToggleId를 초기화하고 하이라이트 처리
									var highLightAmdCd = $administStatsTmsMap.ui.mapToggleId;
									// 맵토글ID 초기화
									$administStatsTmsMap.ui.mapToggleId = "";
									selMap.setPolyLayerHighlight2(highLightAmdCd, p_map_gbn);
									//$administStatsTmsMap.ui.mapList[0].setPolyLayerHighlight2(highLightAmdCd, "left");
									//$administStatsTmsMap.ui.mapList[1].setPolyLayerHighlight2(highLightAmdCd, "right");
									// 하이라이트 처리 후 맵토글ID 세팅
									$administStatsTmsMap.ui.mapToggleId = highLightAmdCd;
								}
							});
						});
					...
					var zoomlevel = $administStatsTmsMap.ui.mapList[0].zoom;
					var coord_x;
					var coord_y;
			 ...
			 * 		p_callback : 콜백 함수
			 */
			setAdministStatsData : function(p_map, p_region, p_type, p_sido_cd, p_sgg_cd, p_emdong_cd, p_surv_id, p_year, p_callback) {
				//변수 선언
				...
				lv_params.itm_cd = $administStatsTmsMap.ui.selectedItmCd;
				//lv_params.isAtdrc = $administStatsMap.ui.isAtdrc; //비자치구 여부 체크
				lv_params.isAtdrc = false;
				
				var p_c1 = $administStatsTmsMap.ui.selectedC1;
				...
				//원 테이블로 조회
				var lv_url = contextPath+"/ServiceAPI/administStats/common/getAdministStatsData.json";
				// ajax 시작
				$.ajax({
					url: lv_url,
				    type: 'post',
					...
					//정보 저장
					$administStatsTmsMap.ui.mapStatsData[lv_tot_surv_id+"_"+p_type+"_"+p_region+"_"+lv_adm_cd] = res.result.mapData;
			 ...
			 * 		p_callback : 콜백 함수
			 */
			setAdministStatsRegion : function(p_map, p_region, p_base_year, p_sido_cd, p_sgg_cd, p_emdong_cd, p_map_gbn, p_callback) {
				/** 2020-10-27 [곽제욱] 로딩바를 위한 좌측, 우측맵 플래그 세팅 START */
				if(p_map_gbn == "left"){
					$administStatsTmsMap.ui.leftMapFlag = false;
				} else if(p_map_gbn == "right"){
					$administStatsTmsMap.ui.rightMapFlag = false;
				}
				...
				if(p_region == "sido") {
					...
					p_map.openApiBoundarySido(p_base_year, function(p_map2, p_res) {
						//정보 저장
						$administStatsTmsMap.ui.mapRegionData[p_region+"_"+lv_year+"_"+lv_adm_cd] = p_res;
				    	...
				    	/** 2020-10-27 [곽제욱] 로딩바를 위한 좌측, 우측맵 플래그 세팅 START */
						if(p_map_gbn == "left"){
							$administStatsTmsMap.ui.leftMapFlag = true;
						} else if(p_map_gbn == "right"){
							$administStatsTmsMap.ui.rightMapFlag = true;
						}
						/** 2020-10-27 [곽제욱] 로딩바를 위한 좌측, 우측맵 플래그 세팅 END */

						/** 2020-10-27 [곽제욱] 로딩바를 위한 좌측, 우측맵 플래그 세팅 START */
						if($administStatsTmsMap.ui.leftMapFlag == true && $administStatsTmsMap.ui.rightMapFlag == true){
							$administStatsMain.ui.loading(false); // 2020-10-14 [곽제욱] 맵 그리기 시작할때 loading바 생성		 
						} 
						/** 2020-10-27 [곽제욱] 로딩바를 위한 좌측, 우측맵 플래그 세팅 END */
					});
				}
				//시도 이외의 경계 데이터 조회
					...
					// ajax 시작
					$.ajax({
					    url: contextPath + "/ServiceAPI/administStats/common/getAdministStatsRegion.geojson",
					    type: 'get',
					    data: params
					}).done(function (res) { // 완료
						if(res.errCd == "0") {
							...
							//정보 저장
							$administStatsTmsMap.ui.mapRegionData[p_region+"_"+lv_year+"_"+lv_adm_cd] = res;
							//경계그리기
							p_map.setPolygonDataGeojson(res);
							var xcoor = 0;
							var ycoor = 0;
							//$administStatsMain.ui.getSidoSggPos(p_sido_cd+p_sgg_cd);
							...
							//$administStatsMap.ui.checkIsAtdrc(p_sido_cd+p_sgg_cd.substring(0,2)+"0")
							...
							if(adm_cd == '21'){
								...
								zoomLevel = zoomLevel + $administStatsMain.ui.zoomResize();
							}
			...
			getAdministStatsRegionYear : function() {
				if($administStatsTmsMap.ui.mapData == null) {
					return bndYear;
				}
				var lv_data = $administStatsTmsMap.ui.mapData.data;
				var lv_year = bndYear;
			...
			doCombineMap : function() {
				...
				//srvLogWrite('P0','03','03','02',$administStatsMain.ui.selectedThema,'leftYear='+$administStatsMain.ui.leftSelectedYear+',rightYear='+$administStatsMain.ui.rightSelectedYear);
				...
				$("#modal").attr("style", "width: 100%; height:"+($(window).outerHeight()+11)+"px; opacity:0.4; background-color:black; position: fixed; z-index: 9999; left:0; top:-11px");
					$administStatsTmsMap.loadingBar.show(1); // 2020-11-03 [곽제욱] 변수명 변경
					
					setTimeout(function() {
						$newlyTmsCombineMap.ui.initCombineMap();
						$administStatsTmsMap.loadingBar.close(); // 2020-11-03 [곽제욱] 변수명 변경
					}, 200);
			},
			...
			createMap : function(id, seq) {
				...
				map.createMap($administStatsTmsMap, id, {
				...
				legend.initialize($administStatsTmsMap.ui);
				map.legend = legend;
				legend.createLegend();
				...
				//사용자지정컨트롤설정
				$administStatsTmsMap.ui.mapList[seq] = map;
...
//					map.mapMove([map.gMap.getCenter().x,map.gMap.getCenter().y],8);
					...
					//지도이동 동기화 이벤트 설정
					if ($administStatsTmsMap.ui.mapList.length == 2) {
						$administStatsTmsMap.event.mapSyncEvent();
					}
					
				});
				
				map.gMap.on("moveend", function(e) {
					$administStatsTmsMap.callbackFunc.didMapMoveEnd(e, map);
				});
			...
			createInfoTooltip : function(event, data, type, map) {
				...
				if($administStatsMain.ui.selectedThema == "인구" && data.info.length>0){
			...
			setRegionCd : function(adm_cd) {
				...
				$administStatsTmsMapLeftmenu.ui.getSggList("current", sidoCd, "");		
				$administStatsTmsMapLeftmenu.ui.getCategoryCnt(sidoCd);
				
				setTimeout(function() {
					$administStatsTmsMap.ui.setMapPosition();
				}, 200);
			},
			...
			setMapPosition : function() {
				$administStatsTmsMap.noReverseGeoCode = true;
			...
			getCenterToAdmCd : function(center,callback) {
				...
				if(Object.prototype.toString.call(center)==="[object Array]"&&center.length==2){
					...
					center = $administStatsTmsMap.ui.mapList[0].gMap.getCenter();
					x = center.x;
					y = center.y;
				}
				...
				obj.request({
					...
					options : {
						callback : callback,
						center : center,
						target : $administStatsTmsMap.ui // 2020-10-08 [곽제욱] target : $administStatsTmsMap 으로 변경
					}
				});
			}
	};
	
	$administStatsTmsMap.request = {
			/**
			 * 
			 * @name         : reqGeocode
			...
			reqGeocode : function(x_coor, y_coor) {
				$.ajax({
						...
						switch(parseInt(res.errCd)) {
							case 0:
								...
								if (result != null) {
									...
									$administStatsTmsMap.ui.setRegionCd(adm_cd);
								}
								break;
							case -100:
								break;
							case -401:
								accessTokenInfo(function() {
									$administStatsTmsMap.request.reqGeocode(x_coor, y_coor);
								});
								break;
						}
					},
					...
					error: function(x,o,e) {
						$administStatsTmsMap.ui.setRegionCd($administStatsTmsMapLeftmenu.ui.defaultSidoCd);
					}
				});						
			},
	...
	$administStatsTmsMap.event = {
			...
			mapSyncEvent :  function() {
				var lMap = $administStatsTmsMap.ui.mapList[0].gMap;
				var rMap = $administStatsTmsMap.ui.mapList[1].gMap;
				lMap.sync(rMap);
				rMap.sync(lMap);
			},
			...
			setUIEvent : function() {
				...
				$("body").on("change", ".yearSelectBox", function() {
					...
					//srvLogWrite( "E0", "04", "01", "00", $administStatsTmsMap.ui.settingInfo.policy_idx_nm, $administStatsTmsMap.ui.getLogParams() ); // jrj 로그 > 조회년도 선택
					
					var id = $(this).attr("id");
					var idx = id.split("_")[1];
					var map = $administStatsTmsMap.ui.mapList[parseInt(idx)-1];
					var options = $administStatsTmsMapLeftmenu.ui.arrParamList[parseInt(idx)-1];
					var year = $(this).val();
					...
					switch(parseInt(options.data_div)) {
						case 3 :
							var result = $administStatsTmsMapApi.ui.userData[map.id][year];
							var res = {
									errCd : "0",
									id : "API_MYDATA",
									result : result,
							...
							var setData = $administStatsTmsMap.ui.setParams(options, map);
							setData.param["year"] = year;
							map.multiLayerControl.multiData = [];
							$administStatsTmsMapApi.ui.doUserBoundData(res, setData, map);
							break;
						default:
							sCall_param["year"] = year;
							options.call_param = sCall_param;
							$administStatsTmsMapLeftmenu.ui.arrParamList[parseInt(idx)-1]=options;  // 바뀐 apiparam을 저장.
							$administStatsTmsMap.ui.doReqStatsData(options, $administStatsTmsMap.ui.mapList[parseInt(idx)-1]);
							break;
					}
				});
	...
	// map event callback
	// ==============================//
	$administStatsTmsMap.callbackFunc = {
			// 맵 줌 종료 시, 콜백 호출
			...
			didMouseOverPolygon : function(event, data, type, map) {
				if (type != "polygon") {
					...
					$administStatsTmsMap.ui.createInfoTooltip(event, data, type, map);
				}else {
					$administStatsTmsMap.ui.createInfoTooltip(event, data, type, map);
				}
			}
			...
			didMapZoomEnd : function(event, map) {
			   ...
			   if(map.id == 0){
				   ...
				   var lv_zoom = $administStatsTmsMap.ui.mapList[0].zoom;
				   var beforeSido = $administStatsTmsMap.ui.prevSido;
				   
				   var center = $administStatsTmsMap.ui.mapList[0].gMap.getCenter();
				   $administStatsTmsMap.ui.getCenterToAdmCd(center, function(res) {
					   
						if(res == undefined || res.result == undefined || $administStatsMain.ui.selectedArea == "") { //20201013 박은식 and조건 or로 변경
							return;
						}
						...
						if(lv_zoom < sggZoom) {
							//이미 같은 전국이면 조회 안함
							if($administStatsTmsMap.ui.mapRegion != "sido" || $administStatsTmsMap.ui.mapList[0].zoom == "0"){ // 2020-10-15 [곽제욱] 같은시도레벨이면 조회하지 않도록 변경
								$("#dash_sido").val("99");
								$("#dash_sgg").val("999");
								$administStatsMain.ui.tmsPathChange("nationwide", "00");
								$administStatsMain.ui.selectedArea = "00";
								
								$administStatsMain.ui.leftSelectedYear = $('#selLeftYear option:selected').val();
								$administStatsTmsMap.ui.mapToggleId = ""; // 2020-10-08 [곽제욱] 시계열 맵토글 ID로 변경
								$newlyTms.ui.drawContent("PH0001", "T100", "", "left", $('#selLeftYear option:selected').val());
								
								$administStatsMain.ui.rightSelectedYear = $('#selRightYear option:selected').val();
								$newlyTms.ui.drawContent("PH0001", "T100", "", "right", $('#selRightYear option:selected').val());
								$("#leftMapDiv > h5").text('지도  - 전국')
					    		$("#rightMapDiv > h5").text('지도 - 전국')
					    		$administStatsTmsMap.ui.mapRegion = "sido";  // 2020-10-15 [곽제욱] 같은시도레벨이면 조회하지 않도록 변경
						...
						//시군구
							...
							if($administStatsTmsMap.ui.mapRegion!="sgg"){
								$("#dash_sido").val(lv_to_sido_cd);
								//시군구 조회
								
								$administStatsMain.ui.selectedArea = lv_to_sido_cd;
								$administStatsTmsMap.ui.mapToggleId = ""; // 2020-10-08 [곽제욱] 시계열 맵토글 ID로 변경
								$administStatsMain.ui.tmsPathChange("sgg", lv_to_sido_cd);
								$administStatsMain.ui.leftSelectedYear = $('#selLeftYear option:selected').val();
                      			$newlyTms.ui.drawContent("PH0001", "T100", "", "left", $('#selLeftYear option:selected').val());
						...
						$newlyTms.ui.searchData($("#selLeftYear").val(), $("#selRightYear").val());
						$newlyTms.ui.searchTimeGenderAgeData($("#selLeftYear").val(), $("#selRightYear").val(), $administStatsMain.ui.selectedArea);
						$newlyTms.ui.createAgeGenderTable($newlyTms.rightTimeGenderAgePopulation, "rightTable");
						$newlyTms.ui.createAgeGenderTable($newlyTms.leftTimeGenderAgePopulation, "leftTable");
						if($administStatsMain.ui.selectedArea == '00' || $administStatsMain.ui.selectedArea == '99'){
							...
							sidoHtml += "	<img src='/images/administStats/locationclick.png' alt='지역을 선택하세요'>";
							sidoHtml += "	<p>지역을 선택하시면 랭킹차트가 표출됩니다.</p>";
							sidoHtml += "</div>";
							$("#timeRankPopulationChart").html(sidoHtml)
					   }
					});
					$administStatsTmsMap.ui.prevZoom = lv_zoom;
			...
			didSelectedPolygon : function(event, data, type, map) {
				...
				if($administStatsTmsMap.ui.mapToggleId != data.properties.adm_cd) { // 2020-10-08 [곽제욱] 시계열 맵토글 ID로 변경				
					// 선택한 지역 하이라이트
					$administStatsTmsMap.ui.mapList[0].setPolyLayerHighlight2(data.properties.adm_cd, "left"); // 2020-10-08 [곽제욱] 시계열 맵 토글관련 수정
					$administStatsTmsMap.ui.mapList[1].setPolyLayerHighlight2(data.properties.adm_cd, "right"); // 2020-10-08 [곽제욱] 시계열 맵 토글관련 수정
		    		$administStatsTmsMap.ui.mapToggleId = data.properties.adm_cd; // 2020-10-08 [곽제욱] 시계열 맵토글 ID로 변경
		    		...
		    		$administStatsMain.ui.selectedArea = $administStatsTmsMap.ui.mapToggleId; // 2020-10-08 [곽제욱] 시계열 맵토글 ID로 변경
		    		if(data.properties.adm_cd.length == 5){
		    			var sgg_cd = data.properties.adm_cd.substring(2,5);
		    			$administStatsMain.ui.getSidoSggPos(sido_cd+sgg_cd);
		    		}  else {
		    			//$administStatsMain.ui.selectedArea = "00";
		    		}
					...
					$administStatsMain.ui.tmsPathChange(regionGb, data.properties.adm_cd);
				} else { 
					var regionGb = "";
					if(data.properties.adm_cd.length == 5){
						regionGb = "sgg";
						$administStatsMain.ui.selectedArea = data.properties.adm_cd.substring(0,2);
						$administStatsMain.ui.tmsPathChange(regionGb, $administStatsMain.ui.selectedArea);
					} else if(data.properties.adm_cd.length == 2){
						$administStatsMain.ui.selectedArea = "00";
						$administStatsMain.ui.tmsPathChange("nationwide", $administStatsMain.ui.selectedArea);
					}
					$administStatsTmsMap.ui.mapList[0].setPolyLayerHighlight2(data.properties.adm_cd, "left"); // 2020-10-08 [곽제욱] 시계열 맵 토글관련 수정
					$administStatsTmsMap.ui.mapList[1].setPolyLayerHighlight2(data.properties.adm_cd, "right"); // 2020-10-08 [곽제욱] 시계열 맵 토글관련 수정
					$administStatsTmsMap.ui.mapToggleId = ""; // 2020-10-08 [곽제욱] 시계열 맵토글 ID로 변경
