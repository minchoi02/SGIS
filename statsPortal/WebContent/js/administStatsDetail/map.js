...
(function (W, D) {
	...
	console.log("[administStats]  map.js");
	...
	sMap = {
			...
			map : function() {
				...
				this.createMap = function (delegate, target, opt) {
					...
					if (!opt.center) {
						this.center = [ 982682, 1744189 ]; // 2020-11-13 [곽제욱] 맵 첫 로딩시 center값 수정 [ 989674, 1818313 ] -> [ 982682, 1744189 ]
					}
					else {
					...
					this.markers3 = new sop.LayerGroup();
					...
					if(document.location.href.match("interactiveMap")){
						var osm2 = new sop.StatisticTileLayer('https://sgisapi.kostat.go.kr/tiles/bmap4/L{z}/{y}/{x}.png', {minZoom: 0, maxZoom: 13, attribution: '' }); //운영 // <!-- 2021-05-20 [hjh] 기존소스 주석해제 -->
						//var osm2 = new sop.StatisticTileLayer('http://10.134.2.100:20083/tiles/bmap4/L{z}/{y}/{x}.png', {minZoom: 0, maxZoom: 13, attribution: '' }); //로컬 // <!-- 2021-05-20 [hjh] 기존소스 주석 -->
						...
						//mng_s 20200420 이진호
						//header의 검색으로 대화형 통계지도 메뉴 들어올 시 ui 오류 해결
						if(document.location.href.match('interactiveMap/newly')){
							$interactiveLeftMenu.ui.setDetailStatsPanel('newlyHouse');
							$('#div_buttonBar').css('left', '360px');
							$('.stepClose.3depth_close').attr('class', 'stepClose 3depth_close on');
						}else if(document.location.href.match('interactiveMap/household')){
							$interactiveLeftMenu.ui.setDetailStatsPanel('newlyHouse');
							$interactiveLeftMenu.ui.newlyTab('household');
							$('#div_buttonBar').css('left', '360px');
							$('.stepClose.3depth_close').attr('class', 'stepClose 3depth_close on');
						}else if(document.location.href.match('interactiveMap/farmhousehold')){
							$interactiveLeftMenu.ui.setDetailStatsPanel('3f');
							$('#div_buttonBar').css('left', '360px');
							$('.stepClose.3depth_close').attr('class', 'stepClose 3depth_close on');
						}else if(document.location.href.match('interactiveMap/company')){
							$interactiveLeftMenu.ui.setDetailStatsPanel('company');
							$('#div_buttonBar').css('left', '300px');
							$('.stepClose.3depth_close').attr('class', 'stepClose 3depth_close on');
						}
						//mng_e 20200420 이진호
					}
					...
					if(location.href.match('interactiveMap/ecountryView')){
						...
						$('#mainIndexBtn, #newlyHouseBtn, #gridHideLeftBtn01, #companyBtn,  #publicDataBtn, #userDataBtn' ).click(function(){
					...
					if(location.href.match('interactiveMap/sharedata') || location.href.match('interactiveMap/newly')){//20200511 수정  (4번. 통계검색(중앙상단에 있는 위치/통계검색) 시 LNB메뉴 오동작)
						if(location.href.match('interactiveMap/newlyHouseView')) return;								//20200511 수정  (바로 위의 if문에서 interactiveMap/newlyHouseView를 잡아서 화면이 이상하게 보임, 이를 위한 if문)
						...
						$('#map_left_btn').click(function(){
							if(action){
								...
								var action2 = true;
								$('#mainIndexBtn, #newlyHouseBtn, #gridHideLeftBtn01, #companyBtn, #ecountryBtn, #publicDataBtn, #userDataBtn ').click(function(){
									if(action2){
										//alert('메뉴에서 행정통계 주요지표 클릭');
...
//					}
				...
				this.addPolygonGeoJson = function (obj, type, opt) {
					...
					if($administStatsMain.ui.selectedArea!="99"){
						if($administStatsMain.ui.tileChangeYn=="Y"){
							// 분기
							if($administStatsMain.ui.selectedThema == "신혼부부"){
								$administStatsMain.event.setTileMapChart("areaPopulation", "288", "250", "PH0001");
							} else if($administStatsMain.ui.selectedThema == "농업"){
								if($administStatsMain.ui.selectedYear == "2015"){
									$administStatsMain.event.setTileMapChart("areaFarm", "288", "250", "FS0013");
								} else if($administStatsMain.ui.selectedYear == "2010"){
									$administStatsMain.event.setTileMapChart("areaFarm", "288", "250", "FS0315");
								}
							} else if($administStatsMain.ui.selectedThema == "어업"){
								if($administStatsMain.ui.selectedYear == "2015"){
								//20201026 박은식 해수면/내수면 분기처리 START
									if($fisheryDash.fisheryType == "sea"){
										$administStatsMain.event.setTileMapChart("areaFishery", "288", "250", "FS0112");
									} else {
										$administStatsMain.event.setTileMapChart("areaFishery", "288", "250", "FS0171");
									}
								} else if($administStatsMain.ui.selectedYear == "2010"){
									if($fisheryDash.fisheryType == "sea"){
										$administStatsMain.event.setTileMapChart("areaFishery", "288", "250", "FS0413");
									} else {
										$administStatsMain.event.setTileMapChart("areaFishery", "288", "250", "FS0469");
									}
								//20201026 박은식 해수면/내수면 분기처리 END
								}
							} else if($administStatsMain.ui.selectedThema == "주택@@"){
								$administStatsMain.event.setTileMapChart("areaHouse", "288", "250", "PH0001");
							}
							// 가구
							else if($administStatsMain.ui.selectedThema == "가구"){
								$administStatsMain.event.setTileMapChart("areaHouseHold", "288", "250", "PH0001");
							}
							// 2020-10-13 임업 추가 START  jhs
							else if($administStatsMain.ui.selectedThema == "임업"){
								if($administStatsMain.ui.selectedYear == "2015"){
									$administStatsMain.event.setTileMapChart("areaForestry", "288", "250", "FS0235");
								} else if($administStatsMain.ui.selectedYear == "2010"){
									$administStatsMain.event.setTileMapChart("areaForestry", "288", "250", "FS0532");
								}
							}
							// 2020-10-13 임업 추가 END  jhs
						}
						//setPopulationTileMapChart("areaPopulation", "288", "250", "PH0001");
					}
                 ...
                 * 2020-09-08
				*/
				this.setPolyLayerHighlight = function (admCd) {
					if(this.dataGeojson != null) {
						...
						if($administStatsMap.ui.curMapId == 1){
							tempColor = $administStatsMap.ui.mapTempColor1;
							mapToggleId = $administStatsMap.ui.mapToggleId1;
							chartItmClick = $houseDash.chartItmClick1;
						}else if($administStatsMap.ui.curMapId == 0){
							tempColor = $administStatsMap.ui.mapTempColor;
							mapToggleId = $administStatsMap.ui.mapToggleId;
							chartItmClick = $houseDash.chartItmClick;
						}
						if(tempColor == "") tempColor = "#0086c6";
						...
						this.dataGeojson.eachLayer(function(layer) {
							...
							/*if(admCd.length == 2){
								mapToggleId = mapToggleId.substring(0, 2);
							}else if(admCd.length == 5){
								mapToggleId = mapToggleId.substring(2, 5);
							}*/
							
							//if($administStatsMap.ui.mapToggleId == admCd){
							if(mapToggleId == admCd){
								...
								if(layer.feature.properties.adm_cd == admCd){
									if($administStatsMap.ui.curMapId == 1){
										$administStatsMap.ui.mapTempColor1 = fillColor;
										if(mapToggleId == ""){
											tempColor = $administStatsMap.ui.mapTempColor1;
										}
									}else if($administStatsMap.ui.curMapId == 0){
										$administStatsMap.ui.mapTempColor = fillColor;
										if(mapToggleId == ""){
											tempColor = $administStatsMap.ui.mapTempColor;
										}
									}
									...
									//if(layer.feature.properties.adm_cd == $administStatsMap.ui.mapToggleId){
									if(layer.feature.properties.adm_cd == mapToggleId){
										layer.setStyle({
											weight : 1,
											color : "white",
											dashArray : dashArray,
								...
								$administStatsMap.ui.mapToggleId = admCd;
							}
						});
					}
				};
                ...
                /** 2020-10-08 [곽제욱] 시계열 맵 하이라이트처리 START */
				this.setPolyLayerHighlight2 = function (admCd, mapGbn) {
					if(this.dataGeojson != null) {
						...
						var tempColor = ""
						if(mapGbn == "left"){
							tempColor = $administStatsTmsMap.ui.leftMapTempColor;
						} else {
							tempColor = $administStatsTmsMap.ui.rightMapTempColor;
						}
						if(tempColor == "" && tempColor == null) tempColor = "#0086c6";

						this.dataGeojson.eachLayer(function(layer) {
							var fillColor = layer.options.fillColor;
							var dashArray = layer.options.dashArray;

							if($administStatsTmsMap.ui.mapToggleId == admCd){
								if(layer.feature.properties.adm_cd == admCd){
									layer.setStyle({
										weight : weight,
										color : color,
								...
								if(layer.feature.properties.adm_cd == admCd){
									if(mapGbn == "left"){
										$administStatsTmsMap.ui.leftMapTempColor = fillColor;
									} else {
										$administStatsTmsMap.ui.rightMapTempColor = fillColor;
									}
									//$administStatsTmsMap.ui.mapTempColor = fillColor;
									if($administStatsTmsMap.ui.mapToggleId == ""){
										if(mapGbn == "left"){
											tempColor = $administStatsTmsMap.ui.leftMapTempColor;
										} else {
											tempColor = $administStatsTmsMap.ui.rightMapTempColor;
									...
									if(layer.feature.properties.adm_cd == $administStatsTmsMap.ui.mapToggleId){
										layer.setStyle({
											weight : 1,
											color : "white",
											dashArray : dashArray,
				...
				/** 2020-10-08 [곽제욱] 시계열 맵 하이라이트처리 END */
				
				

				/** 2020.10.27[한광희] 상세페이지 맵 하이라이트처리 START */
				this.setDetailPolyLayerHighlight = function (admCd) {
					if(this.dataGeojson != null) {

						var color = "#0086c6";
						var weight = 5;
						var tempColor = $administStatsDetailMap.ui.mapTempColor;
						if(tempColor == "" && tempColor == null) tempColor = "#0086c6";
						...
						this.dataGeojson.eachLayer(function(layer) {
							...
							if($administStatsDetailMap.ui.mapToggleId == admCd){
								if(layer.feature.properties.adm_cd == admCd){
									layer.setStyle({
										weight : weight,
										color : color,
								...
								if(layer.feature.properties.adm_cd == admCd){
									$administStatsDetailMap.ui.mapTempColor = fillColor;
									if($administStatsDetailMap.ui.mapToggleId == ""){
										tempColor = $administStatsDetailMap.ui.mapTempColor;
									}
									layer.setStyle({
										weight : weight,
										color : color,
									...
									if(layer.feature.properties.adm_cd == $administStatsDetailMap.ui.mapToggleId){
										layer.setStyle({
											weight : 1,
											color : "white",
											dashArray : dashArray,
				...
				/** 2020.10.27[한광희] 상세페이지 맵 하이라이트처리 END */
				...
				this.setStatsData = function (type, data, showDataParamName, unit, length) {
					...
					if (data != null) {
						if (showDataParamName != null && showDataParamName.length > 0) {
							...
							console.log("[map.js] data[showData] = " + data["showData"]);
						}
				...
				this.openApiBoundarySgg = function (year, sidoCode, callback) {
					$.ajax({
						  type: "GET",
						  url: "/js/data/geo_sgg_" + year + "/geo_sgg_" + sidoCode + "_" + year + ".js",
						  success: function(res) {
							  res["pAdmCd"] = sidoCode;
							  var tmpOption = {
									  year : year,
				...
				this.setLayerColor = function (feature, layer) {
					...
					if (this.dataType == "normal") {
						...
						if (feature.info) {
							var name = "";
							if(feature.properties.adm_cd.length == 5 && feature.info.length > 1){
								for ( var x = 0; x < sidoValues.length; x++) {
									if(sidoValues[x].id == feature.properties.adm_cd.substring(0,2)){
										name = sidoValues[x].name;
										break;
									}
								}
								var index = 0;
								for ( var x = 0; x < feature.info.length; x++) {
									if(name + feature.info[x].region_nm.replace(/ /gi ,'') == feature.properties.adm_nm.replace(/ /gi ,'')){
										index = x;
									}
								}
								for ( var x = 0; x < feature.info.length; x++) {
									if(x != index){
										feature.info.splice(x,1);
									}
								}
							}
							for ( var x = 0; x < feature.info.length; x++) {
								if (feature.info[x].showData) {
									for (param in feature.info[x]) {
										if (param == feature.info[x].showData) {
											var tempColor =  that.legend.getColor(feature.info[x][param], that.legend.valPerSlice[x])[0];
											layer.setStyle({
											...
											var data = new Object();
											data.adm_cd = feature.info[x].adm_cd;
											data.color = tempColor;
											// 2020-11-12 [곽제욱] 선택한 항목이 없을경우에만 색상 추가 START
											if($administStatsMap.ui.selectedPK==""||$administStatsMap.ui.selectedPK==""){
												$administStatsMain.ui.tilePerColor.push(data);
											}
											// 2020-11-12 [곽제욱] 선택한 항목이 없을경우에만 색상 추가 END
											break;
										}
									}
								}
						...
						if (this.dataGeojson != null) {
							this.dataGeojson.eachLayer(function(layer) {
								...
								if (layer.feature.combine) {
									...
									/** 2020-09-07 kosis 데이터 보이도록 설정 추가 hs */
									if(layer.feature.info[2] != undefined && layer.feature.info[2] == "kosis"){
//										//alert("kosis 데이터");
									}
									else{
										...
										for (param in layer.feature.info[0]) {
											if (layer.feature.info[0].showData == param) {
												if (parseFloat(layer.feature.info[0][param]) < 5 &&
													...
													if(layer.feature.info[0][param] == "N/A"){
														layer.setCaption({title:layer.feature.info[0][param], color:"white", showAllZoomLevel: false}, [x,y]);
													}
													else{
						...
						if (this.multiLayerControl.dataGeojson != null) {
							for(var i=0; i<this.multiLayerControl.dataGeojson.length; i++) {
								...
								dataGeojson.eachLayer(function (layer) {
									if (layer.feature.info.length > 0) {
										/** 2020-09-07 kosis 데이터 보이도록 설정 추가 hs */
										if (layer.feature.info.length == 4) {
											/** 2020-09-07 kosis 데이터 보이도록 설정 추가 hs */
											if(layer.feature.info[2] != undefined && layer.feature.info[2] == "kosis"){
												var lv_value = ""+appendCommaToNumber(parseFloat(layer.feature.info[0]));
												if(lv_value.indexOf(".") == 0) lv_value = "0"+lv_value;
												// 소숫점 없이
												layer.setCaption({title:lv_value, color:"white", showAllZoomLevel: false});
												// 소수점 2자리
												//layer.setCaption({title:appendCommaToNumber(layer.feature.info[0]), color:"white", showAllZoomLevel: false});
												// 지도에 데이터 보이도록 설정
												legend.numberData = false;
//												this.checkShowCaption();
											}
											else if(layer.feature.info[2] != undefined && layer.feature.info[2] == "census"){
												var lv_value = ""+appendCommaToNumber(parseFloat(layer.feature.info[0]));
												if(lv_value.indexOf(".") == 0) lv_value = "0"+lv_value;
												// 소숫점 없이
												layer.setCaption({title:lv_value, color:"white", showAllZoomLevel: false});
...
//												this.checkShowCaption();
				...
				this.commonMoveEvent = function (mapId) {
					$administStatsMap.ui.mapList[mapId].gMap.on('drag', function (e) {
						var len = $administStatsMap.ui.mapList[mapId].gMap.dragging._positions.length;
						var positions = $administStatsMap.ui.mapList[mapId].gMap.dragging._positions;
						if(len < 2) {
							return;
						}
					});
					
					/*$administStatsMap.ui.mapList[mapId].gMap.on('dragend', function (e) {
						$administStatsMap.ui.mapList[mapId].gMap.fire("rgeoevent");
					});
					
					$administStatsMap.ui.mapList[mapId].gMap.on('zoomend', function (e) {
						$administStatsMap.ui.mapList[mapId].gMap.fire("rgeoevent");
					});*/
				};
				...
				this.combineStatsData = function (boundData, isPass) {//
					var idx = that.data[0].result.length;
					...
					for ( var k = 0; k < that.data.length; k++) {
						...
						for ( var i = 0; i < boundData.features.length; i++) {
									...
									for ( var x = 0; x < that.data[k].result.length; x++) {
										for (key in that.data[k].result[x]) {
											if (key == "adm_cd") {
												...
												if (code == that.data[k].result[x].adm_cd) {
							...
							if(isEmpty) {
								boundData.features[i].info.push({
									adm_cd: boundData.features[0].properties.adm_cd,
									region_nm: boundData.features[0].properties.adm_nm,
									dt: '-'
								});
								that.data[k].result.push({
									unit: that.data[k].unit,
									legendIndex: k
								})
								boundData.features[i]["dataIdx"] = idx;
								boundData.features[i]["_dataIdx"] = undefined
								boundData.features[i]["dataLength"] = that.data.length;
								idx ++;
							}
						}
									...
									for ( var i = 0; i < that.data[k].result.length; i++) {
										for (key in that.data[k].result[i]) {
											if (key == that.data[k].showData) {
												...
												if (key === "tot_ppltn" 	 ||	  //총신혼부부
												    key === "tot_ppltn_male" ||   //총신혼부부(남자)
												    key === "tot_ppltn_fem"  ||   //총신혼부부(여자)
												    key === "newly"     ||	  //신혼부부
												    ...
												    key === "nongga_ppltn"	 ||   //농가(신혼부부)
												    key === "imga_cnt"		 ||   //임가(가구)
												    key === "imga_ppltn"     ||   //임가신혼부부
												    key === "naesuoga_cnt"   ||   //내수면총어가
												    key === "naesuoga_ppltn" ||	  //해수면어가신혼부부
												    key === "haesuoga_cnt"   ||   //해수면총어가
												    key === "haesuoga_ppltn" ||   //해수면어가신혼부부
				...
				this.openApiReverseGeoCodeForBorough = function (center, callback) {
					...
					console.log("[map.js] openApiReverseGeoCodeForBorough 호출");
				...
				this.openApiBoundaryHadmareaForBorough = function (adm_cd, year, low_search, atdrc_yn, callback, bounds){
					...
					console.log("[map.js] sopOpenApiHadmareaForBoroughObj.request 1111 호출전...");
	...
	(function () {
		$class("sop.openApi.ReverseGeoCodeForBorough.api").extend(sop.portal.absAPI).define({
			...
			onSuccess : function (status, res, options) {
				...
				console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api 호출");
				console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api res.errCd [" + res.errCd);
				...
				if (res.errCd == "0") {
					...
					console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api that.isInnerMapShow2[" + that.isInnerMapShow2);
					console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api that.isInnerMapShow3[" + that.isInnerMapShow3);
			...
			onFail : function (status) {
				console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api onFail [" + status);
			}
		});
	}());
	...
	(function () {
		$class("sop.openApi.hadmareaForBorough.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				console.log("[map.js] sop.openApi.hadmareaForBorough.api res.errCd [" + res.errCd);
				var that = options.target;
				if (res.errCd == "0") {
					options["geojson"] = res;
			...
			onFail : function (status) {
				console.log("[map.js] sop.openApi.hadmareaForBorough.api onFail [" + status);
			}
		});
	}());
