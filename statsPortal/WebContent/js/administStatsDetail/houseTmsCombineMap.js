 ...
 */
(function(W, D) {
	W.$houseTmsCombineMap = W.$houseTmsCombineMap || {};
	$houseTmsCombineMap.noReverseGeoCode = true;
	$(document).ready(function() {
		$houseTmsCombineMap.event.setUIEvent();
	});
	
	$houseTmsCombineMap.ui = {
			combineDataSet : null,
			openerMapList : [],
			cdgLayer : null,
			openerArParamList : [],
			...
			initCombineMap : function(delegate) {
				...
				$houseTmsCombineMap.ui.par_width = $('#mapRgn_1').width();
				$houseTmsCombineMap.ui.par_height = $('#mapRgn_1').height();
				
				//this.delegate = delegate;
				$houseTmsCombineMap.ui.resizePopup();
				$houseTmsCombineMap.ui.createMap("mapCombine");
			
				if($administStatsTmsMap.ui.mapRegion =="sido" || $administStatsMain.ui.selectedArea == "00"){
					$houseTmsCombineMap.ui.drawTmsMapData("sido", "color", $houseTmsCombineMap.ui.map , $administStatsMain.ui.rightSelectedYear, $administStatsMain.ui.leftSelectedYear);
				}
				else if($administStatsTmsMap.ui.mapRegion =="sgg" && $administStatsMain.ui.selectedArea != "00"){
					$houseTmsCombineMap.ui.drawTmsMapData("sgg", "color", $houseTmsCombineMap.ui.map , $administStatsMain.ui.rightSelectedYear, $administStatsMain.ui.leftSelectedYear);
				}
				
				$('#mapRgn_1').width($houseTmsCombineMap.ui.par_width);
				$('#mapRgn_2').width($houseTmsCombineMap.ui.par_width);
			},
			...
			resizePopup : function() {
				...
				//팝업표출
				$houseTmsCombineMap.event.popupShow();
			...
			createMap : function(id) {
				...
				map.createMap($houseTmsCombineMap, id, {
				...
				legend.initialize($houseTmsCombineMap.ui);
				map.legend = legend;
				legend.createLegend();
				...
				map.gMap.whenReady(function() {
					map.createHeatMap();
					$houseTmsCombineMap.ui.clear(map); //2018.02.06 [개발팀]
					//$houseTmsCombineMap.ui.drawCombine();
				});
				
				map.gMap.on("moveend", function(e) {
					$houseTmsCombineMap.callbackFunc.didMapMoveEnd(e, map);
				});
			...
			createInfoTooltip : function(event, data, type, map) {
				...
				var html = "";
				if (type == "data") {
					var title = $houseTmsCombineMap.ui.dispTitle;
					//var unit = $houseTmsCombineMap.ui.dispUnit;
					var unit = "명";
					if (data.info != undefined && data.info.length > 0) {
			...
			drawTmsMapData : function(p_map_region, p_map_type, p_map, s_year, e_year ) {
				var selMap =  $houseTmsCombineMap.ui.map;
				
				var lv_map_region_before = $houseTmsCombineMap.ui.mapRegion;
				var lv_map_type_before = $houseTmsCombineMap.ui.mapType;
				var lv_surv_id = $administStatsMain.ui.selectedSurvId;
				
				if(p_map_region == undefined || p_map_region == null) {
					p_map_region = $administStatsMain.ui.mapRegion;
				}
				else {
					$houseTmsCombineMap.ui.mapRegion = p_map_region;
				}
				if(p_map_type == undefined || p_map_type == null) {
					p_map_type = $administStatsMain.ui.mapType;
				}
				else {
					$houseTmsCombineMap.ui.mapType = p_map_type;
				}
				...
				if($administStatsMain.ui.selectedArea.length == 5){
					...
					sido = $administStatsMain.ui.selectedArea;
				}
...
//				else if(lv_emdong_cd == "99") lv_adm_nm = lv_sido_nm + " " + lv_sgg_nm;
				...
				//지도 Clear
				$houseTmsCombineMap.ui.clearMap(selMap);
				...
				if(p_map_type == "color" || p_map_type == "bubble") {
					...
					if(p_map_region == "sido") {
						...
						$houseTmsCombineMap.ui.setTmsCombineData(selMap, "sido", "color", "", "", "",  "PH0001", s_year, e_year, function(p_list) {
							//리스트에서 unit 가져오기
							var lv_unit = "명";
							var lv_unit_nm = "수";
							...
							//경계 그리기
							$houseTmsCombineMap.ui.setAdministStatsRegion(selMap, "sido", s_year, "", "", "", function() {
								
							});
							
							// 2020-10-14 zoom level 변경
							$houseTmsCombineMap.ui.map.setZoom(1); //20210310 박은식 융합보기 줌레벨 변경처리 
					...
					//색상/버블 (시군구)
						...
						$houseTmsCombineMap.ui.setTmsCombineData(selMap, "atdrc", "color", lv_sido_cd, lv_sgg_cd, "", "PH0001", s_year, e_year, function(p_list) {  // 비자치구 경겨로 변경 [2020-10-08]
							//리스트에서 unit 가져오기
							var lv_unit = "개";
							var lv_unit_nm = "수";
							if(p_list != null && p_list.length > 0) {
							...
							$houseTmsCombineMap.ui.setAdministStatsRegion(selMap, lv_region, s_year, lv_sido_cd, "", "", function() {
								if($administStatsMap.ui.mapToggleId != "" && $administStatsMap.ui.mapToggleId != null){
								}
							});
							
							// zoom level
							$houseTmsCombineMap.ui.map.mapMove($administStatsTmsMap.ui.mapList[0].center, $administStatsTmsMap.ui.mapList[0].zoom);
						});
					...
					console.log("[houseTmsCombineMap] ###  adm_cd = " + adm_cd);
					...
					if($administStatsMain.ui.selectedArea == '00'){
						var lv_zoom = $houseTmsCombineMap.ui.map,zoom;
						$houseTmsCombineMap.ui.map.mapMove([990480.875, 1800313], lv_zoom,false)
					}
			...
			setTmsCombineData : function(p_map, p_region, p_type, p_sido_cd, p_sgg_cd, p_emdong_cd, p_surv_id, st_year, ed_year, p_callback) {
				...
				lv_params.itm_cd = $houseTmsCombineMap.ui.selectedItmCd;
				
				var p_c1 = $houseTmsCombineMap.ui.selectedC1;
				...
				var lv_url = contextPath+"/ServiceAPI/administStats/houseDash/getTmsCombineData.json";
				// ajax 시작
				$.ajax({
					url: lv_url,
				    type: 'post',
					...
					//정보 저장
					$houseTmsCombineMap.ui.mapStatsData[lv_tot_surv_id+"_"+p_type+"_"+p_region+"_"+lv_adm_cd] = res.result.mapData;
			...
			setAdministStatsRegion : function(p_map, p_region, p_base_year, p_sido_cd, p_sgg_cd, p_emdong_cd, p_callback) {
				...
				if(p_region == "sido") {
					...
					p_map.openApiBoundarySido(p_base_year, function(p_map2, p_res) {
						//정보 저장
						$houseTmsCombineMap.ui.mapRegionData[p_region+"_"+lv_year+"_"+lv_adm_cd] = p_res;
				...
				//시도 이외의 경계 데이터 조회
					...
					$.ajax({
						...
						if(res.errCd == "0") {
							...
							//정보 저장
							$houseTmsCombineMap.ui.mapRegionData[p_region+"_"+lv_year+"_"+lv_adm_cd] = res;
							//경계그리기
							...
							if($administStatsMap.ui.mapToggleId != "" && $administStatsMap.ui.mapToggleId.length == 5){
								var tempAdmCd = $administStatsMap.ui.mapToggleId;
								$administStatsMain.ui.getSidoSggPos(tempAdmCd);
			...
			getAdministStatsRegionYear : function() {
				if($houseTmsCombineMap.ui.mapData == null) {
					return bndYear;
				}
	...
	$houseTmsCombineMap.event = {
			 ...
			 */	
			popupClose : function() {
				var map = $houseTmsCombineMap.ui.map;
				...
				$houseTmsCombineMap.ui.par_width;
				$houseTmsCombineMap.ui.par_height;
		...
		$houseTmsCombineMap.callbackFunc = {
			// 맵 줌 종료 시, 콜백 호출
			...
			didMouseOverPolygon : function(event, data, type, map) {
				if (type != "polygon") {
					...
					$houseTmsCombineMap.ui.createInfoTooltip(event, data, type, map);
				}
			},
