	...
	$myGallery = {
	    	...
	    	$("body").on("click",".writeGalleryListItem li", function(){
	    		...
	    		if(!$(this).parents("ul").hasClass("openSelect")){
	    			if($(this).hasClass("statsGalleryDialog")){
	    				...
	    				$('#dialogDiv').load("/view/edu_gallery/myStatsGalleryDialog.jsp",function(){
	    					$("#addMyGallery").hide();
	    				});
	    				
	    			}else if($(this).hasClass("useGalleryDialog")){
	    				$('#dialogDiv').attr("class", "useGalleryDialog");
	    				$('#dialogDiv').load("/view/edu_gallery/myUseGalleryDialog.jsp");
	    				$("#addMyGallery").hide();
	    			}
		...
		selectGalleryList : function () {
			...
			sopPortalGalleryObj.request({
				method : "POST",
				async : false,
				url : contextPath + "/ServiceAPI/edu_gallery/galleryListAllMyGallery.json"
			});
		...
		shareInfoOpen : function(){
			...
			var linkUrl = domain + "/view/edu_gallery/resultGallery?shareDataId="+$myGallery.selectDataId+"&srvType="+srvType;
...
//		},
		...
		deleteGallery : function () {
			...
			if(null != $("#hidden_ss_page_info").val() && '' != $("#hidden_ss_page_info").val()){
				if($("#hidden_ss_page_info").val()=='T'){srvLogWrite('T0','02','07','05',$('#hidden_ss_school_grade').val(),'');}
			}
					...
					btns = [
							{
								...
								func : function(){
									...
									sopPortalDeleteGalleryObj.request({
										method : "POST",
										async : false,
										url : contextPath + "/ServiceAPI/edu_gallery/deleteGalleryMyGallery.json"
									});
								}
							},
							{
