	...
	$catchmentAreaObj = {
		...
		setGeometryInfo : function(pRst, pOpt) {
    		...
    		if(optData.rangeType === "T"){		//rangeType : T(주행시간), D(주행거리)
    			$catchmentAreaMain.ui.restoreRangeValue('T', geoData);
    		}else if(optData.rangeType === "D"){
    			$catchmentAreaMain.ui.restoreRangeValue('D', geoData);
    		}
			...
			$.each(features, function(idx1, feature){
				...
				if(!geoMap.hasOwnProperty(mKey)){
	    			...
	    			if(optData.areaMins instanceof Array){
	    				...
	    				if(idx != -1 && idx < $catchmentAreaMain.ui.saShpColor.length){
	    					geoColor = $catchmentAreaMain.ui.saShpColor[idx];
	    				}    				 
	    			}
		...
		setCircleInfo : function(pOpt) {
			...
			if(!this.geoCmap.hasOwnProperty(mKey)){
				...
				if(colorIdx >= 0 && colorIdx < $catchmentAreaMain.ui.saShpColor.length){
					geoColor = $catchmentAreaMain.ui.saShpColor[colorIdx];				
				}
		...
		setStatisticsInfo : function(pGb, pRst, pOpt) {
			...
			if(pGb == "S01"){
				...
				var rangeType = "rgChk";
				if(params["rangeType"] == "stats01"){
					rangeType = "T_" + params["rangeVal"];
				}else if(params["rangeType"] == "stats02"){
					rangeType = "D_" + params["rangeVal"];
				}else if(params["rangeType"] == "stats03"){
		...
		getShapeInfo : function(pGb, pRange, pMode) {
			...
			var rtnObj = {};
			var geoMap;			
			if(pGb == "stats01" || pGb == "T"){
				if(pMode == "T"){
					geoMap = this.geoTargetTmap;
				}else{
					geoMap = this.geoTmap;
				}				
			}else if(pGb == "stats02" || pGb == "D"){
				if(pMode == "T"){
					geoMap = this.geoTargetDmap;
				}else{
					geoMap = this.geoDmap;
				}
			}else if(pGb == "stats03" || pGb == "C"){
				if(pMode == "T"){
					geoMap = this.geoTargetCmap;
				}else{
					geoMap = this.geoCmap;
