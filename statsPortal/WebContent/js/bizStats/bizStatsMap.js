...
(function(W, D) {
		...
		function() {	
			...
			$(window).resize(function() {
				if($(window).height() <= 740){
					if($('.life2').hasClass("on")){
						$(".list_btn").css('bottom', '-232px');
						$(".nav-list").css('height', '175%');
						$(".menuAutoClose").css('bottom', '-252px');
						//$(".scrollBox").css('height', 'calc(100% - 107px)');
						$(".directions-details").css('top', '70%');
						$(".nav-sidebar").css('overflow', 'auto');
					}		
				}else if (iHeight == 750){
					$(".list_btn").css('bottom', '7px');
					$(".menuAutoClose").css('bottom', '-16px');
					$(".nav-list").css('height', '110%');
			...
			if (iHeight <= 740) {
				...
				$(".list_btn").css('bottom', '35px');
				$(".menuAutoClose").css('bottom', '17px');
				$(".nav-list").css('height', '105%');
				$(".directions-details").css('top', '70%');
				//$(".scrollBox").css('height', 'calc(100% - 107px)');
				//$(".directions-details").css('top', '70%');
			}
			...
			// mng_s 20210421 djlee 화면 크기 조정 - full 
			if(document.location.href.indexOf("type=full") > 1 ){
				console.log("Screen Type ::: full");
				$bizStatsMap.ui.doMaxSize(1);
			}
			// mng_e 20210430 djlee 화면 크기 조정 - full 
	...
	$bizStatsMap.ui = {
			lqMap : {tooltip:null,lqLayerInfo : null,adm_cd : null,theme_cd:null ,type : null,adm_nm : null,year:'2018',region:'country',yearTheme_cd:null,yearlq:null,event:null,map:null,censusCheck:"censusTen"},//year 정보는 최신으로 나중에 수정하기!!2019-04-11 박길섭
...
//////////////////////////////박길섭////////////////////////////////////////
			doUpdateDataboard : function(adm_cd, adm_nm, type, charttype) {
				srvLogWrite('G1', '03', '03', '00', '', '')
				...
				$bizStatsDataBoard.ui.updateDataBoard(options, type);
			},
			...
			doTradeMap : function(type, isShow) {
				...
				if (isShow) {
					...
					$(".sop-overlay-pane").show();//2019-04-04 박길섭
					map.mapBtnInfo.controlHide("poi");
					map.mapBtnInfo.controlHide("setting");
				}else {
					$("#btnList_"+type).find("ul").show();
			...
			lqInfoTooltip : function(event,data,map){
				...
				var showData="";
				var theme_cd="";
				var adm_nm=data.params.adm_nm;
				var adm_cd=data.params.adm_cd;
				$bizStatsMap.ui.lqMap.event=event;
				$bizStatsMap.ui.lqMap.map=map;
				showData=data.params.map.data[0].result[0].showData;
				
				for(var i=0;i<data.params.map.data[0].result.length;i++){
					if(data.params.adm_cd==data.params.map.data[0].result[i].adm_cd){
						corp_lq=data.params.map.data[0].result[i].corp_lq;
						worker_lq=data.params.map.data[0].result[i].worker_lq;
						corp_cnt=data.params.map.data[0].result[i].corp_cnt;
						worker_cnt=data.params.map.data[0].result[i].worker_cnt;
						corp_per=data.params.map.data[0].result[i].corp_per;
						worker_per=data.params.map.data[0].result[i].worker_per;
						theme_cd=data.params.map.data[0].result[i].theme_cd;
					}
				}
				
				
				var html =	'<div class="teclqTooltipBox">';
					html +=		'<div class="topbar">'+data.params.adm_nm+'</div>';
					html +=			'<div class="popContents">';
					html +=				'<div class="areaResultCompare">';
					html +=					'<ul class="f01">';
					html +=						'<li>';
					html +=							'<p class="t01 cRed">사업체 입지계수</p>';
					html +=						'<div class="wrapLqBox"><p class="t02">'+corp_lq+' (LQ)</p></div>';
					html +=							'<p class="t01 cRed">사업체수</p>';
					html +=							'<div class="wrapLqBox"><p class="t02">'+appendCommaToNumber(corp_cnt)+' (개)</p></div>';
					html +=							'<p class="t01 cRed">사업체 비율</p>';
					html +=							'<div class="wrapLqBox"><p class="t02">'+corp_per+' (%)</p></div>';
					//사업체수
					//사업체 비율
					...
					html +=						'<div class="wrapLqBox"><p class="t02">'+worker_lq+' (LQ)</p></div>';
					html +=							'<p class="t01 cBlue">종사자수</p>';
					html +=							'<div class="wrapLqBox"><p class="t02">'+appendCommaToNumber(worker_cnt)+' (명)</p></div>';
					html +=							'<p class="t01 cBlue">종사자 비율</p>';
					html +=							'<div class="wrapLqBox"><p class="t02">'+worker_per+' (%)</p></div>';
					//종사자수
					//종사자 비율
					...
					if($bizStatsMap.ui.lqMap.lqLayerInfo!="sgg"){
						...
						html +=				"<a href='javascript:srvLogWrite(\"G1\", \"06\", \"03\", \"02\", \"\", \"\");$bizStatsMap.ui.doGetSggLq(\""+adm_cd+"\",\""+adm_nm+"\",\""+theme_cd+"\",\""+showData+"\",\""+map.id+"\");' class='btnGtype t01'>해당지역 상세정보 보기</a>";
						
					}
					else{
						html +=				"<a href='javascript:srvLogWrite(\"G1\", \"06\", \"03\", \"02\", \"\", \"\");$bizStatsDataBoard.ui.categoryYearDetailTabClick(\""+$bizStatsMap.ui.lqMap.yearTheme_cd+"\",\""+$bizStatsMap.ui.lqMap.yearTheme_nm+"\",\""+adm_cd+"\",\""+adm_nm+"\");' class='btnGtype t01'>해당지역 상세정보 보기</a>";
					}
					/*html +=				"<a href='javascript:$bizStatsDataBoardApi.request.sggLqOfLqMap(\""+adm_cd+"\",\""+theme_cd+"\",\""+showData+"\");' class='btnGtype t01'>해당지역 상세정보 보기</a>";
					*/
					html +=			'</div>';
					html += '</div>';
					var coor_x = event.utmk.x;
					var coor_y = event.utmk.y;
			 ...
			 * @param 	     : 
			 */
			doGetSggLq : function(adm_cd,adm_nm,theme_cd,showData,mapId,theme_nm){
					...
					var category="sgg";
					$bizStatsMap.ui.lqMap.yearlq="first";
					$bizStatsMap.ui.lqMap.lqLayerInfo = category;
					$bizStatsMap.ui.lqMap.adm_nm =adm_nm;
					$bizStatsMap.ui.lqMap.adm_cd = adm_cd;
					$bizStatsMap.ui.lqMap.theme_cd = theme_cd;
					$bizStatsMap.ui.lqMap.type = showData;
					map.clearDataOverlay();
					...
					if(tmpLayer != null){
						...
						tmpLayer = $bizStatsMap.ui.lqLayerInfo;
					}
					...
					$("#lqIncreaseTab").show();//연도별 입지계수
					$("#categoryYearDetail_"+theme_cd).addClass("on");
					...
					$("#bizLqPrevButton").show();
					$("#countryOrSido").show();
					$("#sidoName").html(adm_nm+"대비");
					// mng_s 20190619 김건민
					$bizStatsDataBoard.ui.categoryYearChangeTabClick(theme_cd);//년도별 입지계수 지도
					$bizStatsDataBoard.ui.categoryYearDetailTabClick(theme_cd, theme_nm, adm_cd, adm_nm);
					// mng_e 20190619 김건민
					$bizStatsDataBoardApi.request.sggLqOfLqMap(adm_cd,theme_nm,theme_cd,showData,$bizStatsMap.ui.lqMap.region,$bizStatsMap.ui.lqMap.year);
					
					////console.log(map.dataGeojson);
					//alert($bizStatsMap.ui.lqMap.year);
					$bizStatsMapApi.request.openApiLqMapSgg(adm_cd,theme_cd,showData,map,$bizStatsMap.ui.lqMap.year,$bizStatsMap.ui.lqMap.region,adm_nm);
					setTimeout(function() {
						map.gMap.fitBounds($bizStatsMap.ui.lqMap.event.target ,{
							zoom : {animate : false}
						});
					},400);
				 ...
				 * 
				 * @name         : desggLqOfLqMap
				 * @description  : 다시 시군구 지역의 입지계수 정보 조회 
				 * @date         : 2018. 10. 02. 
				 * @author	     : 박길섭
				 * @history 	 :
				 * @param 	     : 
				 */
				deDoGetSggLq : function(adm_cd,adm_nm,theme_cd,showData,mapId,theme_nm,region){
						$bizStatsMap.noReverseGeoCode = true;
						...
						$("#lqIncreaseTab").show();//연도별 입지계수 
						$("#depth1Menu").find("li").removeClass("on");
						//$(".areaNameBox").html(adm_nm);
						$("#bizLqPrevButton").show();
						$("#countryOrSido").show();
						map.clearDataOverlay();
						
						$bizStatsMapApi.request.openApiLqMapSgg(adm_cd,theme_cd,showData,map,$bizStatsMap.ui.lqMap.year,region,adm_nm); 
			 ...
			 * @date         : 2015. 11. 3o. 2018. 09. 03., 2018. 10. 19.
			 * @author	     : 권차욱,박길섭 
			 */
			reportLoad : function() {
				var map = this.mapList[this.curMapId];
				...
				switch(menuType[$bizStatsLeftMenu.ui.curSelectedStatsType]) {
					case 0:
						...
						param = {
							...
							type : $bizStatsMap.ui.lqMap.type,//종사자 or 사업체(worker or null)
						};
						var type="";
						...
						if( !$("#sidoFeatureMajorBtn01").hasClass("on")){
							chart = [
								{
									title : adm_nm + " " + "업종별 입지계수 현황",
									data : $("#introSidoDiv").find(".sidoFeatureChart").clone(),
								},
								{
									title : "",
									data : "",
							...
							chart = [
							{
								title : adm_nm + " " + "생활편의업종(36종) 분류별 "+type+" 비율",
					...
					case 1:
						mapData = $bizStatsDataBoard.ui.chartDataList[map.id];
						adm_nm = $bizStatsDataBoard.ui.mapData[map.id].options.params.adm_nm;
						sido_nm=adm_nm.split(" ");
						//theme_nm = $bizStatsDataBoard.ui.theme_nm;
						var type;
						if($("#bizCateMenu0").hasClass("on")){
							type="all";
						}
						else{
							type="part"
						...
						var chart = [];
						if($("#sggFeatureMajorBtn01").hasClass("on")){
							for (var i=1; i<=4; i++) {
								var title="", data, tmpChart;
								
								switch(i) {
									case 1:
										title = adm_nm + " 생활업종 사업체 현황"; 
										break;
									case 2:
										title = adm_nm +  " 생활업종 종사자 현황";
										break;
									case 3:
										title = sido_nm[0] +" 생활업종 사업체 현황";
										break;
									case 4:
										title = sido_nm[0] +" 생활업종 종사자 현황";
										break;
									
								}
								$("#jobAreaDiv").find("#sigunguClassChartArea0"+i+">.hideable").show();
					            chart.push({title:title, data:$("#jobAreaDiv").find("#sigunguClassChartArea0"+i+":visible").clone()});
					            $("#jobAreaDiv").find("#sigunguClassChartArea0"+i+">.hideable").hide();
							}
							
						}
						else if($("#sggFeatureMajorBtn02").hasClass("on")&&$("#sigungu1").hasClass("on")){
							title = "전국 대비 "+adm_nm+" 생활업종 입지계수 현황";
							var doc = document.querySelector("#sigunguLctChart_1");
						...
						else if($("#sggFeatureMajorBtn02").hasClass("on")&&$("#sigungu2").hasClass("on")){
							if(sido_nm[2]!=undefined){
								title = sido_nm[0] +" 대비 "+sido_nm[1]+sido_nm[2]+" 생활업종 입지계수 현황";
							}
							else{
								title = sido_nm[0] +" 대비 "+sido_nm[1]+" 생활업종 입지계수 현황";
							}
							var doc = document.querySelector("#sigunguLctChart_1");
							var svg = doc.querySelector("svg");
							var xml  = new XMLSerializer().serializeToString(svg);
					...
					case 10:
						...
						param = {
							...
							year : $(".seoulBox>span").html(),
							type : $bizStatsMap.ui.lqMap.type,//종사자 or 사업체(worker or null)
							depth	: $bizStatsMap.ui.lqMap.yearlq
						};
						var chart = [];
						if(param.depth!=null){
							chart.push({title:"년도별 입지계수 현황", data:$("#lqMapDiv").find("#yearChart").clone()});
							
						}
						chart.push({title:"업종별 입지계수 현황", data:$("#lqMapDiv").find(".lqMapFeatureChart").clone()});
						origin = "통계청, 통계청전국사업체조사("+companyDataYear+")";
						break;	
					case 2:
						mapData = $bizStatsDataBoard.ui.chartDataList[map.id];
					...
					case 4:
						...
						if($(".dialogbox").css("left") == "0px"){
							...
							//mng_s 20190321_김건민
							//스파이더웹 차트
							var spiderCharts = $('#spiderChartArea [id^="dcharts"]').toArray();
							...
							// 바 차트
							var barCharts = $('#barChartArea [id$="Chart"]').toArray();
...
//				$bizStatsLeftMenu.ui.updateSearchBtnEffect("", this.curMapId);
			...
			doSidoIntro : function() {
				...
				setTimeout(function() {
					map.openApiBoundarySido(map.bnd_year, function() {
						if (map.geojson != null) {
							//////////////박길섭///////////////////////
							var sopPortalSidoLifeInfoObj = new sop.portal.sidoLifeCompanyInfo.api();
							sopPortalSidoLifeInfoObj.addParam("accessToken", accessToken);
							sopPortalSidoLifeInfoObj.request({
								method : "GET",
								async : false,
								//url : openApiPath + "/OpenAPI3/startupbiz/sidobusinessinfo.json",			 // 2020년 SGIS고도화 3차(테마코드) 기존 URL 주석처리 (pse)
								url : contextPath + "/ServiceAPI/OpenAPI3/startupbiz/sidobusinessinfo.json", // 2020년 SGIS고도화 3차(테마코드) 기존 API 대신 새로운 API 요청 (pse)
								options : {
									btntype : "chart",
									api_id : "API_0619",
									map : map,
			...
			doSidoWorkerIntro : function() {
				...
				setTimeout(function() {
					map.openApiBoundarySido(map.bnd_year, function() {
						if (map.geojson != null) {
							//////////////박길섭///////////////////////
							var sopPortalSidoLifeInfoObj = new sop.portal.sidoLifeWorkerInfo.api();
							sopPortalSidoLifeInfoObj.addParam("accessToken", accessToken);
							sopPortalSidoLifeInfoObj.request({
								method : "GET",
								async : false,
								//url : openApiPath + "/OpenAPI3/startupbiz/sidoworkerinfo.json",		  // 2020년 SGIS고도화 3차(테마코드) 기존 url 주석처리 (pse) 
								url : contextPath +"/ServiceAPI/OpenAPI3/startupbiz/sidoworkerinfo.json", // 2020년 SGIS고도화 3차(테마코드) 기존 API 대신 새로운 API 요청 (pse) 
								options : {
									btntype : "chart",
									api_id : "API_0620",
						        	map : map,
			 ...
			 * 
			 * @name         : doLqMap
			 * @description  : 업종별 입지계수 지도화면으로 전환한다.
			 * @date         : 2018. 09. 03. 
			 * @author	     : 박길섭 수정
			 * @history 	 :
			 */
			doLqMap : function(themeCd, themeNm,type,year) {
				...
				$bizStatsMapApi.request.openApiLqMap(themeCd, themeNm,type,map,$bizStatsMap.ui.lqMap.year);
				/*$bizStatsMapApi.request.openApiLqMap(themeCd, themeNm,map, function(res, options){
					//console.log("!!!!!!!!!!!!!!!");
					map.openApiBoundarySido(map.bnd_year, function() {	
						var geojson = map.addPolygonGeoJson(res, "polygon");
						  if (map.multiLayerControl.dataGeojson == null) {
			...
			doNormalMapInit : function() {
	    		...
	    		if ($bizStatsLeftMenu.ui.curSelectedStatsType == "userData") {
	    			...
	    			$("#view"+id).find(".tb_report").parent().show();
	    			$("#view1").find(".tb_inner").hide();
	    			$("#view1").find(".tb_trade").hide();
	    		}
	    		else {
			 ...
			 * @param themeCd : 테마코드
			 */
			doReqSggMap : function(themeCd, themeNm,type) {
				var map = this.mapList[this.curMapId];
				$bizStatsMapApi.request.openApiSggMap(themeCd, themeNm, map, type);
			},
					...
					if (params.adm_cd != undefined || params.adm_cd != null) {
						switch (params.adm_cd.length) {
							...
							case 7:
								...
								// mng_s 20200218 김건민 오류 수정
								if ($bizStatsDataBoard.ui.mapData[map.id].options.etc.theme_cd == themeCd && 
									$bizStatsDataBoard.ui.mapData[map.id].options.etc.year == year) {
									return;
								}else {
						...
						$(".jobChangeTitle").css("margin-left","30px");
					}else {
						map.gMap.setMaxZoom(3);
						$("#jobChangeDiv > .mapBefore").hide();
						$(".jobChangeTitle").css("margin-left","10px");
					}
				}
			...
			doReqCompanyOpen : function(themeCd, themeNm, year, params) {
				...
				if (params != null && params != undefined) {
					if (params.adm_cd != undefined || params.adm_cd != null) {
						...
						$(".jobOpenTitle").css("margin-left","30px");
					}else {
						map.gMap.setMaxZoom(3);
						$("#jobOpenDiv > .mapBefore").hide();
						$(".jobOpenTitle").css("margin-left","10px");
					}
				}
				...
				*/
				...
				if (params != null && params != undefined) {
					if (params.adm_cd != undefined || params.adm_cd != null) {
						...
						$(".jobBestTitle").css("margin-left","30px");
					}else {
						map.gMap.setMaxZoom(12);
						$("#jobBestDiv > .mapBefore").hide();
						$(".jobBestTitle").css("margin-left","10px");
					}
				}
			 ...
			 * @date         : 2016. 11. 26. ,2018. 10. 01
			 * @author	     : 권차욱, 박길섭 lq 추가
			 * @history 	 :
			 * @param type	 : 메뉴타입
			 */
			doPrevRegionInfo : function(type) {
				...
				switch (type) {
					case "lq":
						if($bizStatsMap.ui.lqMap.yearlq=="second"){
							$bizStatsMap.ui.lqMap.yearlq="first";
							$bizStatsDataBoard.ui.categoryYearDetailTabClick($bizStatsMap.ui.lqMap.yearTheme_cd,$bizStatsMap.ui.lqMap.yearTheme_nm);
						}
						else if($bizStatsMap.ui.lqMap.yearlq=="first"||$bizStatsMap.ui.lqMap.yearlq==null){
							$bizStatsMap.ui.lqMap.yearlq=null;
							var option = {
								params:{
									theme_cd : $bizStatsMap.ui.lqMap.theme_cd,
									year : $bizStatsMap.ui.lqMap.year,
									depth : null,
								},
							};
						$bizStatsMapApi.request.setStatsData("", option);
						/*$bizStatsMap.ui.lqMap.lqLayerInfo = category;
						$bizStatsMap.ui.lqMap.adm_nm =adm_nm;
						$bizStatsMap.ui.lqMap.adm_cd = adm_cd;
						$bizStatsMap.ui.lqMap.theme_cd = theme_cd;
						...
						$("#lqIncreaseTab").hide();
						$(".areaNameBox").html("전국"+" "+"<span style='font-size:13px;'>" +($bizStatsMap.ui.lqMap.year).replace("9016","2016")+"</span>");//2019-04-16 박길섭
						$(".areaNameBox").css("font-size","22px");
						$bizStatsMap.ui.lqMap.lqLayerInfo="";
						this.doLqMap($bizStatsMap.ui.lqMap.theme_cd,"",$bizStatsMap.ui.lqMap.type,$bizStatsMap.ui.lqMap.year);
						setTimeout(function() {
							$bizStatsDataBoardApi.request.sidoLqOfLqMap(option);
						//2019-04-15 박길섭 시작
							//$("#mCSB_6_container").css("top","0px");//데이터보드 화면 css 조정
							//$("#mCSB_6_container").css("left","0px");
						},200);
						setTimeout(function() {
							$(".dataSideContents").find(".mCSB_container").css("top","0px");
							$(".dataSideContents").find(".mCSB_container").css("left","0px");
						},300);
						//2019-04-15 박길섭 끝
						$("#countryOrSido").hide();
						}
			...
			doAnalysisShareInfo : function (type, data) {
				...
				if(dataArr.paramInfo.map_information_type!="worker"||curSelectedStatsType=="lqMap"){
				...
				setTimeout(function() {
					switch(typeList[curSelectedStatsType]) {
						case 0:
							...
							var adm_nm = dataArr.paramInfo.adm_nm;
							if(dataArr.paramInfo.map_information_type == undefined||dataArr.paramInfo.map_information_type=="corp"){
								$bizStatsMap.ui.doUpdateDataboard(adm_cd, adm_nm, "intro", "1");
							}else{
								...
								$("#standardButton").find(".ball").animate({"left":"2px"},200,'easeOutExpo');
								$("#standardButton").find(".txt").animate({"left":"34px"},200,'easeOutExpo');*/
								setTimeout(function() {
									$bizStatsMap.ui.doSidoWorkerIntro();
									$bizStatsMap.ui.doUpdateDataboard(adm_cd, adm_nm, "intro", "2");
								}, 200);
							}
							...
							break;
						case 10://업종별 입지계수 지도
							$("#lqInfoYearSettingList").find("li>a").removeClass("on");
							$("#lqInfoYear_"+dataArr.paramInfo.year).find("a").addClass("on");
							$bizStatsMap.ui.lqMap.year=dataArr.paramInfo.year;
							if(dataArr.paramInfo.depth==null||dataArr.paramInfo.depth==undefined){
								if(dataArr.paramInfo.map_information_type == "worker"||dataArr.paramInfo.map_information_type=="worker_lq"){
									$("#mapDataStandard").stop().animate({"right":"135px"},200);
									$("#mapDataStandard").show();
									$("#company").removeClass("on");
									$("#worker").addClass("on");
								...
								var theme_cd = dataArr.paramInfo.theme_cd;
								var theme_nm = dataArr.paramInfo.theme_nm;
								var map_information_type = dataArr.paramInfo.map_information_type;
								var options = {
									params:{
										adm_nm : dataArr.paramInfo.adm_nm,
										theme_cd : dataArr.paramInfo.theme_cd,
										theme_nm : dataArr.paramInfo.theme_nm,
										year : dataArr.paramInfo.year,
										map : map,
									},
									etc:{
										themeCd : dataArr.paramInfo.theme_cd,
										from : 'leftmenu',
									}
								};
								
								
								$bizStatsMap.ui.doLqMap(theme_cd,theme_nm,map_information_type,options.params.year);
								setTimeout(function() {
									$bizStatsDataBoard.ui.updateDataBoard(options, "lqMap");
								},1500);
							}
							else if(dataArr.paramInfo.depth=="first"){
								if(dataArr.paramInfo.map_information_type == "worker"||dataArr.paramInfo.map_information_type == "worker_lq"){
									$("#mapDataStandard").stop().animate({"right":"135px"},200);
									$("#mapDataStandard").show();
									$("#company").removeClass("on");
								...
								setTimeout(function() {
										var adm_cd = dataArr.paramInfo.adm_cd.slice(0,2);
										if (map.geojson != null) {
											map.geojson.eachLayer(function(layer) {
												if (layer.feature.properties.adm_cd == adm_cd) {
													var event = { target: layer };
													$bizStatsMap.ui.lqMap.event=event;//해당 도 줌을 하기위해 설정
												}
											});
										}
										$bizStatsMap.ui.doGetSggLq(adm_cd,adm_nm,theme_cd,map_information_type,"0");
										/*$bizStatsMap.ui.lqMap.yearlq=="second";
										
										$bizStatsMap.ui.lqMap.adm_nm=adm_nm;
										$bizStatsMap.ui.lqMap.adm_cd=adm_cd;
										$bizStatsDataBoard.ui.categoryYearDetailTabClick("1000","서비스","","");*/
...
//											}
								...
								if("color" == bizLegend_type||"bubble" == bizLegend_type){	//색상차트
									setTimeout(function() {
						...
						case 7:
							...
							*/
							...
							setTimeout(function() {
								...
								// mng_s 20200226 김건민
								if (type == "sharedata") {
									if(options.params.adm_cd != null && options.params.adm_cd != '00'){
										$(".MapBefore").show();
										...
										// 지자체 인허가  소분류 체크 
										var ck = $("label[for = 'jj_food_"+themeCd+"']").hasClass("on");
										if(!ck){
											$("label[for = 'jj_food_"+themeCd+"']").addClass("on");
										}else{
											$("label[for = 'jj_food_"+themeCd+"']").removeClass("on");
										}*/
										...
										switch(options.params.adm_cd.length) {
										case 2:
											$bizStatsMap.ui.mapList[$bizStatsMap.ui.curMapId].curPolygonCode = 2;
											layer_adm_cd = '00';
											break;
										case 5:
											$bizStatsMap.ui.mapList[$bizStatsMap.ui.curMapId].curPolygonCode = 3;
											layer_adm_cd = options.params.adm_cd.substring(0, 2);
											break;
										case 7:
											$bizStatsMap.ui.mapList[$bizStatsMap.ui.curMapId].curPolygonCode = 4;
											layer_adm_cd = options.params.adm_cd.substring(0, 5);
											break;
									
										}

										map.mapMove([centerArr[0], centerArr[1]], map.zoom);

										
										//$bizStatsDataBoard.ui.mapData[map.id].options;
										//map.openApiReverseGeoCode([map.center[0], map.center[1]]);
										//$bizStatsMap.ui.companyOpenClick(themeCd, dataArr.paramInfo.adm_cd, dataArr.paramInfo.adm_nm, map.center[0], map.center[1], "heatMap");
										$bizStatsMap.ui.doReqCompanyOpen(themeCd, themeNm, options.params.year, options.params);
										
										// mng_s 20200407 김건민(지자체 데이터 2020년 1분기 때문에 강제로 2019년으로 변경함)
										if(options.params.year == '2020'){
											options.params.year = '2019';
										}
										// mng_e 20200407 김건민
										map.openApiBoundaryHadmarea(layer_adm_cd, options.params.year, "0", null, function(map, res) { //2017.02.23
											options["preLayer"] = {
										...
										map.openApiBoundaryHadmarea(layer_adm_cd, options.params.year, "1", null, function(map, res) { //2017.02.23
											if (map.geojson) {
												map.geojson.remove();
												map.geojson = null;
											}
									...
									} else if(options.params.adm_cd != null && options.params.adm_cd == '00') {										
										map.openApiBoundaryHadmarea('00', options.params.year, "1", null, function(map, res) { //2017.02.23
											if (map.geojson) {
												map.geojson.remove();
												map.geojson = null;
											}
			...
			createInfoTooltip : function(event, data, type, map) {
				...
				if($bizStatsMap.ui.lqMap.tooltip=="lqMap"){//입지계수 일때 툴팁
					if (data.info.length > 0) {
						for (var i = 0; i < data.info.length; i++) {
							var tmpData = data.info[i];	
							if (i == 0) {
								if (tmpData.adm_nm != undefined) {
									html += "<div class='lqAdmName' style='font-size:14px;font-weight:bold;color:#3792de;'>"
											+ tmpData.adm_nm 
											+ "</div>";
								}
							}
							for (key in tmpData) {
								if (key == tmpData.showData) {
									var value;//종사자 or 사업체
										value = appendCommaToNumber(tmpData[tmpData.showData]);
										if($(".txt1").hasClass("on")){
											html += "<tr><td class='statsDatas' style='font-size:12px;padding-left:5px'>종사자 입지계수 : ";
										}
										else{
											html += "<tr><td class='statsDatas' style='font-size:12px;padding-left:5px'>사업체 입지계수 : ";
										}
			...
			setLqMapSidoChangeColorMode : function() {
				
				//console.log("setChangeColorMode2");
				...
				$bizStatsMap.ui.introJobAreaMarkers = [];
				//2012년부터 생긴 세종특별시에 대한 예외 처리 2019-04-15 박길섭 시작
				var k = 0;

				if (map.multiLayerControl.dataGeojson != null) {
					for (var i=0; i<map.multiLayerControl.dataGeojson.length; i++) {
						if(i==7&&map.multiLayerControl.dataGeojson.length!=result.length){//세종시 데이터가 없을시 배열길이가 틀림
							map.multiLayerControl.dataGeojson[7].eachLayer(function(layer) {//세종시 데이터 n/a 삽입
								layer.options.type = "data";
								layer.setStyle(map.setPolygonGeoJsonStyle("data"));
								layer.feature["info"] = []; //2017.02.23
							});
						...
						map.multiLayerControl.dataGeojson[k].eachLayer(function(layer) {
							layer.options.type = "data";
							layer.setStyle(map.setPolygonGeoJsonStyle("data"));
							layer.feature["info"] = []; //2017.02.23
							var tmpAdmCd = result[i].sido_cd;
							if (layer.feature.properties.adm_cd == tmpAdmCd) {
								result[i]["showData"]= showData;
								result[i]["unit"] = unit;
								result[i]["api_id"] = "";
								result[i]["adm_cd"] = tmpAdmCd;
								result[i]["adm_nm"] = layer.feature.properties.adm_nm;
								layer.feature.info.push(result[i]);
								layer.setStyle({
									weight : layer.options.weight,
									color : layer.options.color,
									dashArray : layer.options.dashArray,
									fillOpacity : layer.options.fillOpacity,
									fillColor : map.legend.getColor(parseFloat(result[i][showData]), map.legend.valPerSlice[0])[0]
								}); 
							}	
						});
						k++;
				...
				//2012년부터 생긴 세종시에 대한 예외 처리 2019-04-15 박길섭 끝
			},
			setLqMapSggChangeColorMode : function() {
				
				//console.log("setLqMapSggChangeColorMode");
				...
				$bizStatsMap.ui.introJobAreaMarkers = [];
//				//console.log("---------------------------------");
//				var color = new Color('#CC9933');
//				color.brightness(100);
//				color.alpha(0.5);
//				map.multiLayerControl.dataGeojson[0] = color;
				...
				for (var k=0; k<tmpSidoList.length; k++) {
					...
					for (var i=0; i<result.length; i++) {
						if (result[i].sido_cd == tmpSidoList[k]) {
							...
							var themeNm="";
							if(result[0].theme_cd=="0000"){
								themeNm="종합현황";
							}
							else if(result[0].theme_cd=="1000"){
								themeNm="서비스";
							}
							else if(result[0].theme_cd=="2000"){
								themeNm="도소매";
							}
							else if(result[0].theme_cd=="4000"){
								themeNm="숙박";
							}
							else if(result[0].theme_cd=="5000"){
								themeNm="음식점";
							}
							else if(result[0].theme_cd!="0000"||result[0].theme_cd!="1000"||result[0].theme_cd!="2000"||result[0].theme_cd!="4000"){
								themeNm=result[0].theme_cd_nm;
							}
							var summary =		"<div>";
								...
								summary += 				"<p style=''>"+themeNm+" : "+appendCommaToNumber(result[i][showData])+" ("+unit+")</p>";
	...
	$bizStatsMap.callbackFunc = {
			...
			didMapZoomEnd : function(event, map) {
				...
				if(map.zoom == 10){
					if($bizStatsLeftMenu.ui.srvLogLeftMenuNm == 'jobChange'){
						srvLogWrite('G1', '05', '03', '02', '', '');
					}else if ($bizStatsLeftMenu.ui.srvLogLeftMenuNm == 'jobOpen'){
						srvLogWrite('G1', '09', '04', '02', '', '');
					}
				}else if(map.zoom == 9){
					if($bizStatsLeftMenu.ui.srvLogLeftMenuNm == 'jobBest'){
						srvLogWrite('G1', '10', '04', '01', '', '');
					}
				}
				// mng_e 20190530 김건민
				...
				if (type == "data") {
								...
								case "lqMap":
									...
									var options = {
											params : {
												adm_cd : layer.feature.properties.adm_cd,
												adm_nm : layer.feature.properties.adm_nm,
												theme_cd : data.info[0].theme_cd,
												//theme_cd : data.info[0].theme_nm,
												map : map,
												
											},
									};
									
									$bizStatsMap.ui.lqMap.adm_nm=options.params.adm_nm;
									$bizStatsMap.ui.lqMap.adm_cd=options.params.adm_cd;//입지계수차트를 그리기위해 시도정보
									$bizStatsMap.ui.lqInfoTooltip(event,options,map);
					...
					switch($bizStatsLeftMenu.ui.curSelectedStatsType) {
						...
						case "jobOpen": //지자체 인허가
							...
							// mng_s 20200226 김건민
							var filterParam, fullName;
							
							if($bizStatsMap.ui.share_info_type == 'sharedata'){
								filterParam = $bizStatsDataBoard.ui.mapData[map.id].options.dataBoard.themeCd;
								fullName = $bizStatsDataBoard.ui.mapData[map.id].options.dataBoard.themeNm;
							}else{
			...
			introTooltipFunc : function(sido_cd, adm_cd, theme_cd, adm_nm, x_coord, y_coord, type) {
    			...
    			//업종밀집도변화 버튼 재설정
    			$("#density").empty();
    			//박길섭수정$("#density").html("<a onclick='javascript:$bizStatsMap.ui.companyDensityClick(\""+theme_cd+"\", \""+adm_cd+"\", \""+adm_nm+"\", \""+x_coord+"\", \""+y_coord+"\", \""+type+"\");'>업종 밀집도 변화</a>");
