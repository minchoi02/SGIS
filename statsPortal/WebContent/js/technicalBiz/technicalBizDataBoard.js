...
(function(W, D) {
	...
	$technicalBizDataBoard.ui = {
			...
			srvLogChartNm  : null, // mng_s 20190402 김건민
...
/*								html += '<li id="supply_'+marker_no+'" class="supplyList" style="cursor:pointer;">';
			...
			*/
			initSido : function(options){
				...
				//명칭
				$("#sidoDiv > .areaBox").html(options.params.adm_nm+" <span>"+companyDataYear+" (17개 주요 시도별 기술업종 현황)</span>");
			...
			initSigungu : function(options){
				// mng_s 20190404 김건민
				srvLogWrite('G2', '09', '02', '00', '', options.params.adm_nm);
				// mng_e 20190404 김건민
				$(".dataBoardDiv").hide();
				$("#sigunguDiv").show();
			...
			changeLqInfo : function(themeCd){
				...
				var srvLogYearDataBoardNm =$("#technicalDbTab_"+themeCd.toString().substring(0,2)).text();
				
				var srvLogYearListNm = $("#subTechnicalList li#subTechnicalList_" +themeCd).text();
				...
				if(themeCd.toString().length == 2){
					srvLogWrite('G2', '11', '08', '02', '', srvLogYearDataBoardNm);
					$technicalBizDataBoardApi.request.getInnerTechCd(themeCd.toString().substring(0,2),"subTechnicalList");
				}else{
					srvLogWrite('G2', '11', '08', '02', srvLogYearDataBoardNm + '/' +srvLogYearListNm, '');
			...
			lqInfoDataBoardOpen : function(options){
				...
				if(options.adm_cd == "00"){
					...
					$("#technicalLqDivAreaBox").html("전국 <span>" +(options.year+"").replace("9016","2016")+"</span>" );// 2019-04-14 djlee 수정
					...
					$("#technicalLqDivAreaBox").html(options.featureData[0].sido_nm +" <span>"+(options.year+"").replace("9016","2016")+"</span>" );// 2019-04-14 djlee 수정
					...
					$("#technicalLqDivAreaBox").html(options.featureData[0].sgg_nm + " <span>" +(options.year+"").replace("9016","2016")+"</span>" ); // 2019-04-17 djlee 수정
					
					$("#technicalLqDivAreaBox").html(options.featureData[0].sido_nm +" <span>"+(options.year+"").replace("9016","2016")+"</span>" ); // 2019-04-17 djlee 수정
			...
			changeLqInfoYearDataBoardGrid : function(techiBiz_class_cd){
				...
				var srvLogYearDataBoardNm = $("#technicalYearDbTab_"+techiBiz_class_cd.toString().substring(0,2)).text();
				
				var srvLogYearListNm = $("#subTechnicalYearList li#subTechnicalYearList_" +techiBiz_class_cd).text();
				...
				if(techiBiz_class_cd.toString().length == 2){
					...
					srvLogWrite('G1', '03', '06', '04', '', srvLogYearDataBoardNm + "/" +srvLogYearListNm);
			...
			changeLqInfoYearDataBoard : function(year){
				// mng_s 20190404 김건민
				srvLogWrite('G2', '11', '08', '06', '', year + '년');
				// mng_s 20190404 김건민
			...
			changeSigunguRankType: function(type){
				...
				switch(type) {
					case "corp_cnt":
						srvLogWrite('G2', '09', '06', '05', '', '사업체 수 차트');
						tmpType = "0";
						break;
					case "corp_per":
						srvLogWrite('G2', '09', '06', '06', '', '업종 비율 차트');
						tmpType = "0"; //업종비율도 사업체로 조회
						break;
					case "resid_ppltn_cnt":
						srvLogWrite('G2', '09', '06', '07', '', '거주인구 차트');
						tmpType = "2";
						break;
					case "worker_cnt":
						srvLogWrite('G2', '09', '06', '08', '', '업종 종사자 수 차트');
						tmpType = "3";
						break;
					case "avg_worker_cnt":
						srvLogWrite('G2', '09', '06', '09', '', '평균 종사자 수 차트');
						tmpType = "4";
						break;
				}
				// mng_e 20190402 김건민	
			...
			changeSigunguLctChart : function(region){
				...
				// mng_s 20190402 김건민
				$technicalBizDataBoard.ui.srvLogChartNm = region; 
				// mng_e 20190402 김건민
			...
			changeDensityTab : function(m_class_cd, s_class_cd){
				...
				if(m_class_cd == "0"){
					...
					$(".helperText").html(title);
					srvLogWrite('G2', '10', '09', '01', '', themeNm);
				}else{
			...
			changeDensitySubTab : function(s_class_cd){
				...
				// mng_s 20190404 김건민
				if(s_class_cd != 0 ){
					srvLogWrite('G2', '10', '09', '01', themeNm + '/' + themeNm_sub,'' );
				}
				// mng_e 20190404 김건민
				//this.options.params.base_year = undefined;
				// mng_s 20210517 김건민 (2018년에서 2019년 으로 변경)
				if (!$(".specializedInfo a").hasClass("on")) {
					if(!this.options.params.base_year){
						this.options.params.base_year = "2019";
						this.options.params.year = "2019";
					}
			...
			changeDensityBaseYear : function(base_year){
				...
				// mng_s 20190404 김건민
				srvLogWrite('G2', '10', '09', '02', '', base_year);
				// mng_e 20190404 김건민
			},
...
//////////////////////////////////////////////////////////////			
			...
			changeSupplyAreaSynthesizeStatsInfoTab02 : function(type02){
				...
				switch(type02){
					case 1 :
						srvLogWrite('G2', '13', '07', '03', '', '기술업종별 사업체 수(개)');
						break;
					case 2 :
						srvLogWrite('G2', '13', '07', '03', '', '기술업종별 사업체 비율(%)');
						break;
					case 3 :
						srvLogWrite('G2', '13', '07', '03', '', '기술업종별 사업체 증감(%)');
						break;
					default :
						srvLogWrite('G2', '13', '07', '03', '', '기술업종별 입지계수');
				}
				// mng_e 20190403 김건민
				...
				// 2019-04-14 djlee 수정 시작
				if(type02 != "3"){
					$(".supplyAreaSynthesizeStatsInfoChart .censusTabs").hide();
				}else{
					$(".supplyAreaSynthesizeStatsInfoChart .censusTabs").css("display" , "block");
				}
				// 2019-04-14 djlee 수정 끝
			...
			changeSupplyAreaSynthesizeStatsInfoTab03 : function(type03){
				...
				options.params["map"] = $technicalBizMap.ui.mapList[$technicalBizMap.ui.curMapId];
				// 2019-04-14 djlee 수정 시작
				$(".supplyAreaSynthesizeStatsInfoChart .censusTabs a").each(function(index,elem){
					if($(this).hasClass("on")){
						options.params["census"] =  $(this).data("value");
					}
				});
				// 2019-04-14 djlee 수정 끝
			...
			changeIndustryAreaSynthesizeStatsInfoTab02 : function(type02){
				...
				switch(type02){
					case 1 :
						srvLogWrite('G2', '14', '07', '02', '', '기술업종별 사업체 수(개)');
						break;
					case 2 :
						srvLogWrite('G2', '14', '07', '02', '', '기술업종별 사업체 비율(%)');
						break;
					case 3 :
						srvLogWrite('G2', '14', '07', '02', '', '기술업종별 사업체 증감(%)');
						break;
					default :
						srvLogWrite('G2', '14', '07', '02', '', '기술업종별 입지계수');
				}
				// mng_e 20190403 김건민
			...
			changeIndustryAreaSynthesizeStatsInfoTab03 : function(type03){
				...
				options.params["corp_worker_type"] = type01;
				options.params["map"] = $technicalBizMap.ui.mapList[$technicalBizMap.ui.curMapId];
				$(".industryAreaSynthesizeStatsInfoChart .censusTabs a").each(function(index,elem){
					if($(this).hasClass("on")){
						options.params["census"] =  $(this).data("value");
					}
				});
			...
			doComplexDetailInfo :function (options, data){
				...
				$("#industryVariationStateTab_Btn0"+options.params.type).removeClass("on");
				$("#industryVariationStateTab_Btn01").addClass("on");
			...
			changeSearchDetailAreaInfoTab02 : function(type01){
				...
				// mng_s 20190402 김건민
				if(type01 == "1"){
					srvLogWrite('G2', '12', '04', '01', '', '기술업종별 사업체 수');
					this.changeSearchDetailArea("0");
				}else if(type01 == "2"){
					srvLogWrite('G2', '12', '04', '01', '', '기술업종별 사업체 비율');
					this.changeSearchDetailAreaPieChart("0");
				}else if(type01 == "3"){
					srvLogWrite('G2', '12', '04', '01', '', '기술업종별 사업체 증감');
					this.changeSearchDetailAreaVariateLineChart("0");
				}else if(type01 == "4"){
					srvLogWrite('G2', '12', '04', '01', '', '기술업종별 사업체 입지계수');
			...
			changeSearchDetailAreaInfoTab03 : function(type03){
				...
				var srvLogYearDataBoardNm = $(".searchDetailStatsInfoTab02 .on").text();
			...
			searchDetailArea : function(adm_cd, adm_nm){
				// mng_s 20190402 김건민
				srvLogWrite('G2', '12', '03', '06', '', adm_nm);
				// mng_e 20190402 김건민
			...
			changeShowChartTable : function(id,idx){
				...
				var hideId = "";
				// mng_s 20190402 김건민
				var srvLogNm = $technicalBizDataBoard.ui.srvLogChartNm;
				...
				/*if(id == 'sigunguLctChart'){
					if(srvLogNm == '전국대비'){
						if(idx == "1"){
							srvLogWrite('G2', '09', '06', '04', '전국 대비/차트', '');
							showId = "#"+id+"_"+idx;
							hideId = "#"+id+"_2";
							$(hideId).hide();
							$(showId).show();
							...
							srvLogWrite('G2', '09', '06', '04', '전국 대비/표', '');
							showId = "#"+id+"_"+idx;
							hideId = "#"+id+"_1";
							$(showId).parent().find(".ar").css("margin-left", "-20px"); //2018.01.15 [개발팀]
							$(showId).parents(".compareBox").find(".combineGrid").show();
						...
						if(idx == "1"){
							srvLogWrite('G2', '09', '06', '04', '시도 대비/차트', '');
							showId = "#"+id+"_"+idx;
							hideId = "#"+id+"_2";
							$(hideId).hide();
							$(showId).show();
							...
							srvLogWrite('G2', '09', '06', '04', '시도 대비/표', '');
							showId = "#"+id+"_"+idx;
							hideId = "#"+id+"_1";
							$(showId).parent().find(".ar").css("margin-left", "-20px"); //2018.01.15 [개발팀]
							$(showId).parents(".compareBox").find(".combineGrid").show();
	...
	$technicalBizDataBoard.event = {
			...
			setUIEvent : function() {
				...
				body.on("click",".bizStatsDataBoard",function(){ 
					...
					//데이터 보드 오픈시 css 변경
					//2017.08.31 개발팀 추가 시작
					$("#mapDataStandard").stop().animate({"right" : "605px"}, 200);
					//2017.08.31 개발팀 추가 종료
					if(!ck){
						//일반 데이터보드
						$(".dataSideBox").stop().animate({"right":"0px"},200);//20년수정반영
						$(".sop-right").stop().animate({"right":"558px"},200);//20년수정반영
						$(this).addClass("on").stop().animate({"right":"430px"},200);//20년수정반영
					}else{
						//데이터보드 닫기
						$(".dataSideBox").stop().animate({"right":"-1500px"},200);
						$(".sop-right").stop().animate({"right":"0px"},200);//20년수정반영
				...
				body.on("click",".typeBox>a",function(){ 
					...
					if(ck%2){
						...
						$(this).parents(".compareBox").find(".combineGrid").attr('style','margin-top:-30px; display:block; overflow:visible;');
				...
				body.on("click",".btn_excelDownload", function(){
					...
					$("#excelDownForm").html("");
					if(!$("#sidoFeatureMajorBtn02").hasClass("on")){//시도에서 입지계수 엑셀 다운로드시 에러발생으로 조건 처리 업종별 입지계수에선 필요
						var titleData = [];
			...
			// 2019-04-09 djlee 추가 시작
			// 9차 10차 분기  추가 
			sensusCheck : function(checkVal , obj , type){
				...
				if(checkVal == 9){
					...
					$(".sensus10").hide();
					if(type == 1){
						$technicalBizDataBoard.ui.changeDensityBaseYear('2016');
					}else if(type == 2){
						$technicalBizDataBoard.ui.changeLqInfoYearDataBoard('2016');
					}else if(type == 3){
						setTimeout(function(){
							$("#supplyAreaInfoTab03 a").each(function(index,elem){
								if($(elem).hasClass("on")){
									eval($(elem).attr("href").replace("javascript:" , ""));
									return false;
								}
							});
						},400);
						...
						setTimeout(function(){
							$("#industryTab03 a").each(function(index,elem){
								if($(elem).hasClass("on")){
									eval($(elem).attr("href").replace("javascript:" , ""));
									return false;
								}
					...
					$(".sensus9").hide();
					if(type == 1){
						$technicalBizDataBoard.ui.changeDensityBaseYear('2019'); // 20210517 김건민 (2018년에서 2019년으로 변경)
					}else if(type == 2){
						$technicalBizDataBoard.ui.changeLqInfoYearDataBoard('2019'); // 20210517 김건민 (2018년에서 2019년으로 변경)
					}else if(type == 3){
						setTimeout(function(){
							$("#supplyAreaInfoTab03 a").each(function(index,elem){
								if($(elem).hasClass("on")){
