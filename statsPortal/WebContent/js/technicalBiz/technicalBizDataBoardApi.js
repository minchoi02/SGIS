	...
	$technicalBizDataBoardApi.request = {
			...
			*/
			sidoEconomyChart : function(options){
				...
				sopPortalSidoEconomyChart.addParam("base_year", ecoDataYear);
	...
	//			        url : contextPath + "/ServiceAPI/technicalBiz/supplyStartUpSupplyBarChart.json"
			...
			* @date         :  2016.10.17
			...
			technicalBizVariateLineChart : function(options, type01, type03){ 
				...
				sopPortalTechnicalBizVariateLineChartDrawObj.addParam("census", (options.params.census ? options.params.census : "10"));// 2019-04-14 djlee 수정
...
//				$("#mvnLimitTxt").html(resultData.mvn_limit);									//5. 입지조건 - 입주제한
			...
			lqInfoYearDataBoardGrid : function(options,lower_search,callback){
				...
				var census = null;
				$(".lqInfoYearCensusTabs a").each(function(index,elem){
					if($(this).hasClass("on")){
						census = $(this).data("year");
					}
				});
				if(census)sopPortalLqInfoYearDataBoardGridObj.addParam("census",census);
				...
				if(res.errCd == 0){ // 정상 수행
					...
					for(var i = 0; i < categories.length; i++){
						...
						if(i == 5 || i == 6){
							...
							tabHtml +="<a href='javascript:$technicalBizDataBoard.event.checkClass("+categoriesCd[i]+");' style='background:"+colorList[i]+";border:1px solid #fff; color:#fff;'>"+categories[i]+"</a>"; //2018.01.22 [개발팀]
						}else{
							checkHtml +="<a href='javascript:$technicalBizDataBoard.event.checkClass("+categoriesCd[i]+");' class='ckbtn on' id='check_"+categoriesCd[i]+"'><span class='ico'></span></a>"; //2018.01.22 [개발팀] 
		...
		if(result.length > 4){
			...
			if(type == "worker"){
				if(id == "#sigunguClassChartArea02"){
					//190219_s 주용민
					legend += '<p class="btn-more"><a onclick="javascript:$(\'' + id + ' .hideable\').slideToggle();$(\'#sigunguClassChartArea01' + ' .hideable\').slideToggle();">더보기..</a></p>';
//					legend += '<p class="btn-more"><p onclick="javascript:$(\'' + id + ' .hideable\').slideToggle();$(\'#sigunguClassChartArea01' + ' .hideable\').slideToggle();">더보기..</p></p>';
				}else{
					legend += '<p class="btn-more"><a onclick="javascript:$(\'' + id + ' .hideable\').slideToggle();$(\'#sigunguClassChartArea03' + ' .hideable\').slideToggle();">더보기..</a></p>';
//					legend += '<p class="btn-more"><p onclick="javascript:$(\'' + id + ' .hideable\').slideToggle();$(\'#sigunguClassChartArea03' + ' .hideable\').slideToggle();">더보기..</p></p>';
					//190219_e 주용민
				}
			}
			//2017.09.13 개발팀 수정 종료
			...
			onSuccess : function(status, res, options){
				if(res.errCd == 0){ // 정상 수행
					...
					for (var typeCd in showDataList) {
						...
						// mng_s 20201102 김건민
						if(typeCd == 'resid_ppltn_cnt'){
							$("#techRank_"+typeCd).find(".jnum").html(appendCommaToNumber(selectVal) + showDataList[typeCd][1] + "/1개" + "<br>" +"(" + sggNm.split(" ")[0] + " 평균 " + appendCommaToNumber(total_avg) + showDataList[typeCd][1] + "/1개" + ")"); //2018.01.15 [개발팀] 콤마
						}else{
		...
		$class("sop.portal.densityCombinedChart.api").extend(sop.portal.absAPI).define({
			onSuccess : function(status, res, options){
				if(res.errCd == 0){ // 정상 수행
					...
					var title =  options.params.adm_nm+"<span>("+mainTitle+"-"+options.dataBoard.themeNm+")"+((options.params.year).replace("9016","2016"))+"</span>"; //2019-04-21 djlee 수정 
					...
					for(var i=0; i<res.result.densityCombined.length; i++){
						...
						if(res.result.densityCombined[i].base_year == options.params.base_year.replace("9016","2016")){// 2019-04-14 djlee 수정
					...
					$("#densitySummaryTitle").text((options.params.base_year.replace("9016","2016") || companyDataYear) + " 총사업체 " + appendCommaToNumber(corpCnt) + "개" );// 2019-04-14 djlee 수정
					//================ 2018.01.08 [개발팀] 년도별 사업체수 표시 END =============//		
					
					// djlee 20190411 수정 시작
					var v = 1;
					if(options.params.base_year == '9016' || options.params.base_year == '2017')v =2.5;
		...
		$class("sop.portal.industryAreaStateBarChartDraw.api").extend(sop.portal.absAPI).define({
			onSuccess : function(status, res, options) {
				if(res.errCd == 0){ // 정상 수행
					...
					var type = "corp_cnt";
					if ($("#worker").hasClass("on")) {//2019-03-13 박길섭
						type = "worker_cnt";
						name1 = "종사자";
					}
			...
			onSuccess : function(status, res, options) {
				...
				switch(parseInt(res.errCd)) {
					case 0:
						...
						startUpSupplyListsTitleTxt = "선택한 창업시설 목록 개수";
							...
							for (var p in tmpData) {
								for(var i=0; i<tmpData[p].length; i++) {
									...
									if (tmpData[p][i].lct_type == "4") {//창업투자회사
										...
										// mng_s 20190419 김건민 (srvLog 하기 위해 id생성함)
										html +=	"<a href='"+tmpData[p][i].url+"'  target='_blank' id='tmpDataUrl' class='btnType02 w200' style='display:none;'>벤처창업입지114 이동</a>";
										// mng_e 20190419 김건민 
									}
			...
			onSuccess : function(status, res, options) {
				switch(parseInt(res.errCd)) {
					case 0:
						...
						if(options.census){
							var imsi = [];
							$(result.featureData).each(function(index,elem){
								// mng_s 20210518 김건민(2019년 추가해서 수정함.)
								if(options.census == '2017' || options.census == '9016' || options.census == '2018' || options.census == '2019' ){
									if(elem.real_base_year == '2017'|| elem.real_base_year == '9016' || elem.real_base_year == '2018' || elem.real_base_year == '2019' ){
										imsi.push(elem);
									}
								}else{
									if(elem.real_base_year != '2017' && elem.real_base_year != '9016' && elem.real_base_year != '2018' && elem.real_base_year != '2019' ){
...
//				legend += '			<p class="t02">' + appendCommaToNumber(legendArr.legendData[i].legendCnt) + '개</p>';
		...
		if(dataArr.length > 4){
			//190219_s 주용민
			hideable += '<br/><p class="btn-more"><a onclick="javascript:$(\'#' + id + 'Div .hideable\').slideToggle()">더보기..</a></p>';
						...
						switch(parseInt(res.errCd)) {
							case 0:
								...
								if(supply_cnt == 0 ){
									...
									corptxt = appendCommaToNumber(Math.floor(corp_cnt/supply_cnt)) + "개당  1개소" ;
									workertxt = appendCommaToNumber(Math.floor(worker_cnt/supply_cnt)) + "개당  1개소" ;
								}
								$("#searchTechbizAvgTxt").text(corptxt);
								$("#supplyWorkerAvgTxt").text(workertxt);
								
								$("#searchWorkerAvgTxt").text(workertxt);
								...
								switch(parseInt(options.type01)){
									...
									case 2 : 
										...
										for(var i = 0; i < chartData.length; i++){
											var tempData = [];
											tempData.push(chartData[i].techbiz_s_class_cd_nm);
											if ($("#company").hasClass("on")) {//2019-03-13 박길섭
													tempData.push(parseFloat(chartData[i].corp_per));
													legendArr.legendData.push({
														legendNm : chartData[i].techbiz_s_class_cd_nm,
														legendPer : chartData[i].corp_per,
