...
(function(W, D) {
	...
	$houseAnalysisMap.event = {
		...
		setUIEvent: function() {
			...
			// mng_s 2020.05.06 이금은 '비교지역 설정' 기능 추가
			$("input[name='stand_area_check']").click(function(){

				if($("input[name='stand_area_check']").is(":checked")) {
					$("dl.SetArea dt").eq(3).show();
					$("dl.SetArea dd").eq(3).show();
				} else {
					$("dl.SetArea dt").eq(3).hide();
					$("dl.SetArea dd").eq(3).hide();
					$("#stand-recommend-sido-select").val("00");
					$("#stand-recommend-sido-select").trigger("change");					
				}
			});
			// mng_e 2020.05.06 이금은 '비교지역 설정' 기능 추가
		},
		/**
		 * @name		 : leftMenu
		 * @description  : 메뉴 이벤트. 
		...
		leftMenu : function(){
			...
			$("body").on("click", ".nav-sidebar li a", function(){ 
				...
				if($(".nav-sidebar li").eq(0).find("a").hasClass("on")){
					$("#recommendAreaFind").show();
					$("#toSee").hide();
				}
				else if($(".nav-sidebar li").eq(1).find("a").hasClass("on")){
					$("#toSee").show();
					$("#recommendAreaFind").hide();
				}
				else{
					$("#recommendAreaFind").hide();
					$("#toSee").hide();
			...
			$("body").on("click", "#look-abode,#search-recommend,#search-poi, #search-recommend_le", function(){
				var id = $(this).attr("id");
				$('.thematic.nav-list.house-list').find('a').each(function(i, e) {
					if($(e).hasClass('on')) {
						id = $(e).parent('li').attr('id');
					}
				});
				if(id == "search-recommend_le"){
					id = "search-recommend";
				}
				$(".stepClose").data("id", id);
				$(".menu-box").hide();
				$("#"+id+"-box").show();
				console.log("id = " + id);
				//추천지역찾기, 주거현황보기 사이즈 달리하기 위하여 적용
				if($("#ideal-type>a").hasClass("on")){
					$(".nav-sidebar").css("left","448px");
					$(".nav-sidebar").css("z-index","10000");
					$(".step01").css("left","-450px");
				}else{
					$(".nav-sidebar").css("left","-80px");
					if(id=="look-abode"){
					...
					$(".quickBox").stop().animate({"left":"80px"},200);
				}
				//$houseAnalysisMap.leftmenu.setSubmitTooltipContent();
			});
			//닫기 버튼 클릭 시
			$("body").on("click",".stepClose",function(){
				...
				if(!on){
					...
					$(".quickBox.step01").stop().animate({"left":"82"},200);
				}else{
					$(".quickBox .bottom > a.stepClose").removeClass("on");
					$(".quickBox.step01").stop().animate({"left":"-430px"},200);
				} 
				//20년수정반영 끝
			});
			//20년수정반영 시작
			...
			//박길섭 추가
			$("body").on("click",".sideQuick.sq02,.stepClose",function(event,callback){//20년수정반영
				if($(".sideQuick.sq02").hasClass("on")){
					$houseAnalysisMap.leftmenu.closeAnimate($(this).data("id"));
					$(".quickBox .bottom > a.step01_stepClose").removeClass("on");//20년수정반영
				}
				else{
					$(".quickBox .bottom > a.step01_stepClose").addClass("on");//20년수정반영
			...
			$(".SetStepBar1>span").click(function(event,x){
				...
				event.pageX=event.pageX-80;/*우리동네수정*/ //왼쪽 사이드바가 추가 되어 좌표 변경
			...
			$("#search-recommend-box .IndexSelect>li").each(function(cnt,node){
				$(node).find(".SetStopPoint").draggable({
					...
					stop: function(e) {
						e.pageX=e.pageX-80;/*우리동네수정*/ //왼쪽 사이드바가 추가 되어 좌표 변경
						moveBar(e,$(this));
					}
				});
			});
			...
			$("#search-recommend-box .HouseMap .IndexSelect li[class^=index]>ul>li>a").click(function(e,isLimitMax){
				...
				if($(this).parent("li").hasClass("M_on")){
					...
					var standRegion = $("#stand-recommend-sido-select").find("option[value='"+$('#stand-recommend-sido-select').val()+"']").text()+" "+$("#stand-recommend-sgg-select").find("option[value='"+$('#stand-recommend-sgg-select').val()+"']").text();
					var interRegion = $("#inter-recommend-sido-select").find("option[value='"+$('#inter-recommend-sido-select').val()+"']").text()+" "+$("#inter-recommend-sgg-select").find("option[value='"+$('#inter-recommend-sgg-select').val()+"']").text();
					
					if($("#search-recommend-box .HouseMap .IndexSelect>li.index1").hasClass("M_on")){
						srvLogWrite('F0','02','01','01',$(this)[0].innerText,"비교지역:"+standRegion+",살고싶은 지역:"+interRegion);
					}else if($("#search-recommend-box .HouseMap .IndexSelect>li.index2").hasClass("M_on")){
						srvLogWrite('F0','02','01','02',$(this)[0].innerText,"비교지역:"+standRegion+",살고싶은 지역:"+interRegion);
					}else if($("#search-recommend-box .HouseMap .IndexSelect>li.index3").hasClass("M_on")){
						srvLogWrite('F0','02','01','03',$(this)[0].innerText,"비교지역:"+standRegion+",살고싶은 지역:"+interRegion);
					}else if($("#search-recommend-box .HouseMap .IndexSelect>li.index4").hasClass("M_on")){
						srvLogWrite('F0','02','01','04',$(this)[0].innerText,"비교지역:"+standRegion+",살고싶은 지역:"+interRegion);
					}else if($("#search-recommend-box .HouseMap .IndexSelect>li.index5").hasClass("M_on")){
						srvLogWrite('F0','02','01','05',$(this)[0].innerText,"비교지역:"+standRegion+",살고싶은 지역:"+interRegion);
					}else if($("#search-recommend-box .HouseMap .IndexSelect>li.index6").hasClass("M_on")){
						srvLogWrite('F0','02','01','06',$(this)[0].innerText,"비교지역:"+standRegion+",살고싶은 지역:"+interRegion);
					}else if($("#search-recommend-box .HouseMap .IndexSelect>li.index7").hasClass("M_on")){
						srvLogWrite('F0','02','01','07',$(this)[0].innerText,"비교지역:"+standRegion+",살고싶은 지역:"+interRegion);
					}
					return false;
				}
				$houseAnalysisMap.leftmenu.setSubmitTooltipContent();
		...
		databoard : function(){
			...
			$("body").on("click",".interactiveDataBoard",function(){
				if(!$(this).hasClass("disabled")){
					...
					if(!ck){
						...
						//Poi 아이콘 버튼 감추기
						var isOn = $(".info.sop-control").find(".rightQuick.rq01").hasClass('on');
						if(isOn) $(".info.sop-control").find(".rightQuick.rq01").trigger('click');
					}else{
						//데이터보드 닫기
						$houseAnalysisMap.databoard.closeDataBoard();
					}
			...
			//mng_s 20180508_김건민
			$("#indicator-navigator_1>li>a").mouseover(function(){
				$("#indicator-navigator_1>li").removeClass("M_on");
				$(this).parent().addClass("M_on");
			});
			//mng_e 20180508_김건민
			...
			//mng_s 20180508_김건민
			$("#indicator-navigator_1 ul a").tooltip("destroy");
			$("#indicator-navigator_1 ul a").tooltip({
				content: function () {
					return $(this).prop('title');
				},
				"position":{ my: "center top", at: "center top+20"}
			...
			//mng_s 20180508_김건민
			//설정된 지표별 현황 보기 지표 중분류 클릭시 이벤트
			$("#indicator-navigator_1 a[data-m-class]").click(function(){
				var b_class_idx_id = $(this).data("b-class");
				var m_class_idx_id = $(this).data("m-class");
				srvLogWrite('F0','03','02','00',bClassInfoList[b_class_idx_id].indicator[m_class_idx_id].b_class_idx_nm+"-"+bClassInfoList[b_class_idx_id].indicator[m_class_idx_id].m_class_idx_nm,'');
				...
				if($houseAnalysisMap.search.lastMapOption.center&&$houseAnalysisMap.search.lastMapOption.zoom){
					...
					$houseAnalysisMap.api.tmpOptions["params"] = $houseAnalysisMap.api.paramsArr;
					setTimeout(function(){
						$houseAnalysisMap.chart.indicatorChart1(mClassObject);
			...
			$("#indicator-navigator a[data-m-class]").click(function(){
				var b_class_idx_id = $(this).data("b-class");
				var m_class_idx_id = $(this).data("m-class");
				srvLogWrite('F0','03','04','00',bClassInfoList[b_class_idx_id].indicator[m_class_idx_id].b_class_idx_nm+"-"+bClassInfoList[b_class_idx_id].indicator[m_class_idx_id].m_class_idx_nm,'');
				$houseAnalysisMap.api.paramsArr["b_class_idx_id"]= b_class_idx_id;
				$houseAnalysisMap.api.paramsArr["m_class_idx_id"]= m_class_idx_id;
				$houseAnalysisMap.api.paramsArr["databoardChk"]= 0;
				$houseAnalysisMap.api.tmpOptions["params"] = $houseAnalysisMap.api.paramsArr;
			...
			$("#detailChart4 .ThisAreaInfo a.Title").click(function(){
				...
				if($(this).data("type")=="age")
					srvLogWrite("F0","03","05","01","","");
				else if($(this).data("type")=="house")
					srvLogWrite("F0","03","05","02","","");
				else if($(this).data("type")=="company")
					srvLogWrite("F0","03","05","03","","");
		...
		lifeStyle : function(){
			$("#LifeStyleSelect>li>a").click(function(){
				//mng_s 20190419 김건민
				var lifeStyleText = $(this).text();
				srvLogWrite("F0","02","03","00",$.trim(lifeStyleText),"");
				//mng_e 20190419 김건민
				if($(this).parent("li").hasClass("M_on")){
		...
		idealtype : function(){
			...
			$("#ideal-type").click(function(){
				$houseAnalysisMap.ui.resizeLayer();
				
				/////박길섭 추가
				$(".nav-sidebar li a").removeClass("on");
				$(".nav-sidebar li").eq(2).find("a").addClass("on");
				/////박길섭 추가
			...
			$("#ideal-type-close-button").click(function(){
				if($(".step01").css("left")!="-450px"){
					$("#ideal-type>a").removeClass("on");
				}
