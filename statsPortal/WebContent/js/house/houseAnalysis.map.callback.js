...
(function(W, D) {
	...
	$houseAnalysisMap.callbackFunc = {
		...
		didMapMoveEnd: function(event, map) {
			...
			//학구도poi조회
			if (poiHouseControl.isSchoolPOI && poiHouseControl.isSchoolShow && poiHouseControl.schoolType != undefined && poiHouseControl.schoolType != null) {
				if(map.zoom > 9){
					if (poiHouseControl.mapBounds == null) {
						poiHouseControl.clearSchool();
						poiHouseControl.schoolZone();
					}else {
						if (!poiHouseControl.mapBounds.contains(map.gMap.getCenter())) {
//						if(((poiHouseControl.mapBounds._northEast.y - poiHouseControl.mapBounds._southWest.y)*0.25 < Math.abs(map.center[1]-poiHouseControl.mapBounds.getCenter().y)) || ((poiHouseControl.mapBounds._northEast.x - poiHouseControl.mapBounds._southWest.x)*0.20 < Math.abs(map.center[0]-poiHouseControl.mapBounds.getCenter().x))){
							poiHouseControl.clearSchool();
							poiHouseControl.schoolZone();
						}
					}
			...
			//아파트poi조회			
			if (poiHouseControl.isAptPOI && poiHouseControl.isAptShow) {
				if(map.zoom > 9){
					if (poiHouseControl.mapBounds2 == null) {
						poiHouseControl.clearApt();
						poiHouseControl.aptZone();
					}else {
						if (!poiHouseControl.mapBounds2.contains(map.gMap.getCenter())) {
//						if(((poiHouseControl.mapBounds2._northEast.y - poiHouseControl.mapBounds2._southWest.y)*0.25 < Math.abs(map.center[1]-poiHouseControl.mapBounds2.getCenter().y)) || ((poiHouseControl.mapBounds2._northEast.x - poiHouseControl.mapBounds2._southWest.x)*0.20 < Math.abs(map.center[0]-poiHouseControl.mapBounds2.getCenter().x))){
							poiHouseControl.clearApt();
							poiHouseControl.aptZone();
						}
					}
