...
(function(W, D) {
	...
	srvLogWrite( "H0", "03", "01", "00", $communityMapCommon.cmmnty_map_id, "" ); //jrj 로그
	
	$(document).ready(function() {
		if( $("#is_new").val() == "Y" ){
			srvLogWrite( "H0", "01", "06", "00", $communityMapCommon.cmmnty_map_id, "" ); //jrj 로그
		}
		if( $("#is_hot").val() == "Y" ){
			srvLogWrite( "H0", "01", "05", "00", $communityMapCommon.cmmnty_map_id, "" ); //jrj 로그
		}
	});
		...
		ui:{
			...
			mapMarksLists: function(callback) {	
				if($communityMapCommon.hasText($communityView.ui.searchSymbol)||$communityView.ui.searchSymbol=="all"){
					...
					obj.addParam("cmmnty_map_id", $communityMapCommon.cmmnty_map_id);
					if($communityMapCommon.hasText($communityView.ui.searchSymbol)&&$communityView.ui.searchSymbol!="all"){
			 ...
			 */
			getPoiDetail : function(cmmnty_poi_id,poi_reply_id,poiReport){
				if($communityView.ui.markersObject[cmmnty_poi_id]){
					if( !poiReport ){
						srvLogWrite( "H0", "03", "03", "04", $communityMapCommon.cmmnty_map_id, "cmmnty_poi_id:"+cmmnty_poi_id ); //jrj 로그
					}
	...
	$communityView.event = {
		...
		setUIEvent: function() {
			...
			$("#"+popupId+"-reply-form").submit(function(){
				...
				srvLogWrite( "H0", "03", "03", "08", $communityMapCommon.cmmnty_map_id, "cmmnty_poi_id:"+$("#"+popupId).data("id") ); //jrj 로그
							...
							func : function(opt) {
								...
								*/
								...
								if(pw!=undefined&&pw.replace(/ /gi,'')!=''){
									srvLogWrite( "H0", "03", "03", "06", $communityMapCommon.cmmnty_map_id
											, "cmmnty_poi_id:"+$("#poi-detail").data("id") ); //jrj 로그
	...
	(function() {
		$class("sop.portal.mapMarks.api").extend(sop.portal.absAPI).define({
			onSuccess: function(status, res, options) {
				...
				$(".quickBox.step01 .stepClose").removeClass("on").click();
			...
			onSuccess: function(status, res, options) {
				...
				if (res.errCd == "0") {
					if(res.result.success){
						srvLogWrite( "H0", "03", "03", "07", $communityMapCommon.cmmnty_map_id, "cmmnty_poi_id:"+$("#poi-detail").data("id") ); //jrj 로그
						
						$communityView.ui.getPoiDetail(options.cmmnty_poi_id, true);
