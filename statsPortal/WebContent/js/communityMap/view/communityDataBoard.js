	...
	$communityDataBoard.ui = {
		...
		search : function(stat_type){
			...
			if( $(".interactiveDataBoard").hasClass("on") ){
				srvLogWrite( "H0", "03", "08", "01", $communityMapCommon.getParameter("cmmnty_map_id"), 
						"지역"+$("select[name=adm_cd] option:selected").text() + ",아이디:"+$("select[name=search_usr_id] option:selected").text() ); //jrj 로그
			}
		...
		setUIEvent : function() {
			...
			body.on("click",".interactiveDataBoard",function(){
				if(!$(this).hasClass("on")){
					...
					if( $(".interactiveDataBoard").hasClass("on") ){
						var type = $("#region-stat a.on").data("data-type");
						var apiCode = "02";
						if( type === "pie" ){
							apiCode = "03";
						} else if( type === "spider" ){
						...
						srvLogWrite( "H0", "03", "08", apiCode, $communityMapCommon.getParameter("cmmnty_map_id"), "" ); //jrj 로그
					}
			...
			$("#region-stat .chart-type>li>a").click(function(){
				...
				var apiCode = "02";
				if( $(this).data("value") === "pie" ){
					apiCode = "03";
				} else if( $(this).data("value") === "spider" ){
					apiCode = "04";
				} else if( $(this).data("value") === "all-data" ){
					apiCode = "05";
				}
