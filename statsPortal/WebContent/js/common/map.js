...
//		$(".toolBar").hide();
	...
	sMap = {
			...
			map : function() {
				...
				this.isMtsNoMapBorder = false; //SGIS_4 다중시계열 경계조회 no
				...
				this.createMap = function (delegate, target, opt) {
					...
					if(document.location.href.match("interactiveMap")){
						if(opt.isMiniMapShow == undefined || opt.isMiniMapShow == true){ //SGIS_4 다중시계열 미적용 추가(if문 추가)
...
0
				...
				//SGIS_4 다중시계열 관련 시작
				this.setmultiTimeStatsData = function(data, options, map){
					$interactiveMultiTimeSeries.ui.setmultiTimeStatsData(data, options, map);
					...
					if ( bounds != null && bounds != undefined && this.isInnerMapShow2) { //mng_s kimjoonha grid this.isInnerMapShow2 조건 추가 20180206
						...
						*/
						...
						sopOpenApiHadmareaObj.request({
							method : "GET",
							async : true,
							url : "/ServiceAPI/OpenAPI3/boundary/hadmarea.geojson", //SGIS4_1124_경계추가
							//url : openApiPath + "/OpenAPI3/boundary/hadmarea.geojson",//SGIS4_1124_경계추가
							options : {
								target : this,
								adm_cd : adm_cd,
								year : year,
				...
				this.openApiReverseGeoCode = function (center, callback) {
					//SGIS_4 다중시계열 경계조회 no 시작 (if문 추가)
					if(that.isMtsNoMapBorder){
						return;
					}else{
					//SGIS_4 다중시계열 경계조회 no 끝(if문 추가)
					...
					}//SGIS_4 다중시계열 경계조회 no 추가(if문 추가)
				};
...
//										fillColor : that.legend.legendColor[4]
				...
				this.setPolygonDataGeojson = function (geoData) {
					...
					if( that.mapBtnInfo && that.mapBtnInfo.delegate && 
						...
						if(that.legend != null && that.legend != undefined){ //SGIS_4 추가
				...
				this.setLegendForStatsData = function (isPass) {
					...
					if (isPass == undefined || !isPass) {
						...
						if(curPage.match(matchStr)){
							...
							//SGIS_4 추가 시작
							} else if( this.target == "mapRgn_4" ) {
								sLegendInfo.legendInfo.fixed_legend_val4 = arData;
							} else if(this.target == "mapRgn_5"){
								sLegendInfo.legendInfo.fixed_legend_val5 = arData;
							} else if(this.target == "mapRgn_6"){
								sLegendInfo.legendInfo.fixed_legend_val6 = arData;
							} else if(this.target == "mapRgn_7"){
								sLegendInfo.legendInfo.fixed_legend_val7 = arData;
							}
							//SGIS_4 추가 끝
						}
						//mng_e  20170802 정책통계지도 범례 좌측으로 고정
				...
				this.multiLayerControl = {
					...
					//SGIS4_1124_추가 시작
					reqBoundary2 : function(adm_cd, data, atdrc_yn, isZoomLvl4, callback) {
						switch(parseInt(that.boundLevel)) {
							case 0:	//하위경계 0레벨
								switch (that.curPolygonCode) {
									case 1:
									...
									case 4:
										if(isZoomLvl4){
											this.openApiBoundaryHadmareaForBorough(adm_cd, that.bnd_year, that.boundLevel, data, atdrc_yn, callback);
										}else{
											this.openApiBoundaryHadmarea(adm_cd, that.bnd_year, that.boundLevel, data, atdrc_yn, callback);
										}
										break;
					...
					//SGIS4_1124_다중시계열 4시군5시군 적용 시작
					openApiBoundaryHadmareaForBorough : function (adm_cd, year, low_search, data, atdrc_yn, callback) {
						var params = null;
						if (atdrc_yn != undefined && atdrc_yn == "1") {
							params = {
								"accessToken" : accessToken, 
						...
						$.ajax({
							  ...
							  success: function(res) {
								  switch(parseInt(res.errCd)) {
									  ...
									  case -401:
										  accessTokenInfo(function() {
											  that.multiLayerControl.openApiBoundaryHadmareaForBorough(adm_cd, year, low_search, data, atdrc_yn, callback);
										  });
										  break;
								  }
							  },
			...
			onSuccess : function (status, res, options) {
				...
				if (res.errCd == "0") {
					...
					if(!that.delegate.noReverseGeoCode){
						...
						if(document.location.href.match("interactive/mainIndexView")){ //SGIS_4 수정
		...
		$class("sop.openApi.ReverseGeoCodeForBorough.api").extend(sop.portal.absAPI).define({
			...
			onSuccess : function (status, res, options) {
				...
				if (res.errCd == "0") {
					...
					console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api curPolygonCode[" + curPolygonCode);
					
					if(!that.delegate.noReverseGeoCode){
						
						console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api that.delegate.noReverseGeoCode [" + that.delegate.noReverseGeoCode);
						console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api that.isNoReverseGeocode [" + that.isNoReverseGeocode);
						...
						//mng_s grid
							...
							case 3:
								...
								if (that.lastGeojsonInfo != null 
									...
									//console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api curSidoCd [" + curSidoCd + "] that.lastGeojsonInfo.adm_cd [" + that.lastGeojsonInfo.adm_cd);
									
									if (that.chkIfInteractive() && that.lastGeojsonInfo.zoom && that.lastGeojsonInfo.zoom == that.zoom){	//20200417 수정 (ggm)
										
										//console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api 1111");
										
										if (that.dataGeojson == null) {
											
											//console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api 2222");
											
											that.setPolygonDataGeojson(that.lastGeojsonInfo.geojson);
										}else {
											
											//console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api 3333");
											that.clearData();
											that.openApiBoundaryHadmareaForBorough(curSidoCd, that.bnd_year, "1", "0", that.boundaryCallback); //9월 서비스
										}									
									}else{
										
										//console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api 4444");
										
										//that.clearData(); //mng_s 20211208 
										//that.openApiBoundaryHadmareaForBorough(curSidoCd, that.bnd_year, "1", "0", that.boundaryCallback); //9월 서비스
									}
								}else {
									
									//console.log("[map.js] sop.openApi.ReverseGeoCodeForBorough.api 5555");
