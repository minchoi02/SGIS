...
(function(W, D) {
	...
	$(document).ready(function() {
		...
		//상단 대메뉴 마우스 이벤트 
		$("#gnb .clearFix li").on("mouseover" , function(){
			$(".inner .clearFix ul").removeClass("active");
			$(".inner .clearFix ul").eq($(this).index()).addClass("active");
			
			//select box 이벤트 비활성화 처리함
			$("select").blur();
			
		});
		
		if(window.location.href.indexOf("categoryListHuman") != -1){
			$('.board_tabs2 a').removeClass('active'); 
			$('#board_tabs_1').addClass('active'); 
			$('.board_area').hide(); 
			$('#board_area_1').show(); 
		}else if(window.location.href.indexOf("categoryListHouse") != -1){
			$('.board_tabs2 a').removeClass('active'); 
			$('#board_tabs_2').addClass('active'); 
			$('.board_area').hide(); 
			$('#board_area_2').show(); 
		}else if(window.location.href.indexOf("categoryListWelfare") != -1){
			$('.board_tabs2 a').removeClass('active'); 
			$('#board_tabs_3').addClass('active'); 
			$('.board_area').hide(); 
			$('#board_area_3').show(); 
		}else if(window.location.href.indexOf("categoryListWork") != -1){
			$('.board_tabs2 a').removeClass('active'); 
			$('#board_tabs_4').addClass('active'); 
			$('.board_area').hide(); 
			$('#board_area_4').show(); 
		}else if(window.location.href.indexOf("categoryListFitness") != -1){
			$('.board_tabs2 a').removeClass('active'); 
			$('#board_tabs_5').addClass('active'); 
			$('.board_area').hide(); 
			$('#board_area_5').show(); 
		}else if(window.location.href.indexOf("categoryListEnvironment") != -1){
			$('.board_tabs2 a').removeClass('active'); 
			$('#board_tabs_6').addClass('active'); 
			$('.board_area').hide(); 
			$('#board_area_6').show(); 
		}
