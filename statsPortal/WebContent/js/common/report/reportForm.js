				...
				setMainReportInfo : function(data) {
					...
					if (this.mapType == "interactiveMap") {
						if ( data.param.isKosis != undefined && data.param.isKosis ) {
							mainTitle = data.param.title + ( data.data[0].UNIT ? "("+data.data[0].UNIT+")" : "" );
						}else if( data.param.isEcountry != undefined && data.param.isEcountry ){
							mainTitle = data.param.title + ( data.data.UNIT ? "("+data.data.UNIT+")" : "" );
						}else {
					...
					if (this.mapType == "interactiveMap") {
						...
						if (Object.prototype.toString.call(data.geojson) === "[object Array]") {
							...
							for(var i=0; i<data.geojson.features.length; i++) {
								...
								if (layer.info.length > 0) {
									var info = null;
									if (layer.isKosis || layer.isEcountry) {
										info = layer.info;
...
//								adm_cd = info.adm_cd;
							...
							//mng_s 20210715 이진호
							//option.total 값에 따른 분기처리
							//option.total 이 0일 때 infinity 출력됨
							if(options.total == 0){
								rate = "-";
							}else{
				...
				drawGrid : function(data) {
					...
					for (var i=0; i<gridList.length; i++) {
						...
						if( adm_cd.length > 7 ){
		 				...
		 				//mng_s 20210716 이진호 보고서 표 비율 값 infinity 수정 (주석걸린 소스가 원본 소스) 
		 				//html += "<td class='ar'><div class='valueBox'><span class='txt'>"+gridList[i].rate.toFixed(2)+"</span></div></td>";
		 				if(gridList[i].rate == "-"){
	 						html += "<td class='ar'><div class='valueBox'><span class='txt'>"+gridList[i].rate+"</span></div></td>";
	 					}else{
	 						html += "<td class='ar'><div class='valueBox'><span class='txt'>"+gridList[i].rate.toFixed(2)+"</span></div></td>";
	 					}
		 				//mng_e 20210716 이진호
