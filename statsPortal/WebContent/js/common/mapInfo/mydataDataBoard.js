...
function getSession() {
	...
	if (AuthInfo.authStatus) {
		$("#myDataSample").css("display","none");
		$(".myDataListBtn").css("display","");
		var dttxt ="<div style='text-align: center; margin-left:0px !important; margin-bottom:10px;' class='roundTextBox'>"; 
			dttxt +="<a href='/view/mypage/myData/dataList' class='btnStyle01 myDataListBtn' style='background:#00ff0000 !important; color:white;'>나의 데이터 이동</a></div>";
			//mng_s 20211118 이진호, 들여쓰기 추가
			dttxt += "&nbsp;&nbsp;&nbsp;※ 사용자가 보유하고 있는 txt, csv, Excel, KML 등의 <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 포맷파일을 업로드하여 지도 위에 매핑할 수 있습니다.";
			//mng_e 20211118 이진호
			//dttxt +="<div style='text-align: center;'>"; 
			//dttxt +="<a href='/view/mypage/myData/dataList' class='btnStyle01' style='background:#0d66ac !important; border-radius:5px; color:white;'>나의 데이터 이동</a></div>";
			$(".myDataTxt").html(dttxt);
		$mydataDataBoard.ui.myDataList(1); // 나의데이터 목록
	} else {
		$("#myDataSample").css("display","");
		$(".myDataTxt").css("display","none");
		$(".myDataList").css("display","none");
		$(".myDataListBtn").css("display","none");
		var txt ="<div style='text-align: center; margin-left:0px !important;' class='roundTextBox'>"; 
			txt +="<a href='/view/mypage/myData/dataList' class='btnStyle01 myDataListBtn' style='background:#00ff0000 !important; color:white;'>나의 데이터 이동</a></div>";
			
			//mng_s 20211118 이진호, 문구 수정 
			//txt +="<li style='margin-top:10px; font-family:inherit; color:#666;' class='myDataTxt'>'나의 데이터'를 이용하기 위해서는 '나의 데이터'<br>이동을 클릭하여 회원가입 및 로그인을 하셔야 합니다.</li>";
			txt +="<li style='margin-top:10px; font-family:inherit; color:#666; word-spacing:1px;' class='myDataTxt'>&nbsp;&nbsp;&nbsp;※ '나의 데이터'를 이용하기 위해서는 먼저<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;회원가입 및 로그인을 하셔야 합니다.</li>";
			//mng_e 20211118 이진호
			txt +="<div style='text-align: center; margin-left:0px !important;' class='roundTextBox'>"; 
			txt +="<a id='myDataSample' onclick='javascript:window.open(\"/jsp/sample/dataUpload.jsp\",\"_blank\");' class='btnStyle01 myDataListBtn' style='background:#00ff0000 !important; color:white;'>나의 데이터 체험하기</a></div>";
			txt +="<li style='margin-top:10px; font-family:inherit; color:#666;  word-spacing:1px;' class='myDataTxt'>";
			//mng_s 20211118 이진호, 문구수정
			//txt +="'공개된 사용자 데이터 목록'을 이용하여 '나의 데이터'를<br>체험해 볼 수 있습니다.<br><br><p style='font-size:13px;display:inline-block;'>※</p> 단, '나의 데이터 체험하기'는 저장하기 기능이 없다는<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;점을 유의하여 주시기 바랍니다.</li>";
			txt +="<p style='font-size:13px;display:inline-block;'>&nbsp;&nbsp;&nbsp;※</p> '나의 데이터 체험하기'는 저장하기 기능이<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;없다는 점을 유의하여 주시기 바랍니다.</li>";
			//mng_e 20211118 이진호
			
		/*var txt  = "비회원에게 '나의 데이터'를 이용해 볼 수 있는 기회를 제공하기 위하여 '나의 데이터 체험하기' 메뉴를 제공하고 있습니다.<br/>";
			txt += "공개된 사용자 데이터 목록'을 이용하여 '나의 데이터'를 체험해 볼 수 있습니다.<br/>";
			txt += "단 '나의 데이터 체험가기'는 저장하기 기능이 없다는 점을 유의하여 주시기 바랍니다. 저장기능을 이용하기 위해서는 회원 가입 후<br/>";
			txt += "'나의 데이터'를 이용하여 주시기 바랍니다.<br/>"; */
		$("#myDataLoadList").html(txt).css("margin-left", "10px");
	}
	
	$(".myDataTxt").css("letter-spacing","-0.045em !important").css("line-height","1.5em").css("font-size","11px");
...
(function(W, D) {
	...
	$mydataDataBoard.ui = {
		 ...
		 * @history :
		 */
		updateMyData : function(data_id, title, type, isClear, gb) {
			// 초기화 //2016.03.18 수정
			// this.reset(this.map_id);
			
			
			if(document.location.href.match("interactiveMap")){	
				if(gb == 1 || gb == undefined){
					srvLogWrite("C0", "02", "07", "01", "", "");
				}else{
					srvLogWrite("C0", "02", "07", "02", "", "");
				}
			}
		...
		drawUserData : function(res) {
			...
			headerArray.push("선택");
			headerArray.push("No");
			$("#groupCell,#weightSel").find("option").remove();
			$("#weightSel").append('<option value="">선택안함</option>');
			...
			for (var i = 0; i < col_nm.length; i++) {
				...
				$("#groupCell,#weightSel").append(
						"<option value='" + col_id[i] + "'>" + col_nm[i]
								+ "</option>");

				var ratiofield = $mydataDataBoard.ui.mapData[ $mydataDataBoard.ui.map_id ].options['ratiofield'];
				if( ratiofield ){
					$("#weightSel").val( ratiofield.toUpperCase() );
				}
			}
			...
			if ( mainData.MAP_DISP_TYPE == "location"
					|| mainData.MAP_DISP_TYPE == "ratio" || mainData.MAP_DISP_TYPE == "pbubble" ) {
				$("#showInfoWindowPopup").show();
				$("#showInfoWindowPopup").next().show();
				$("#groupingInfo").parent().show();
				$("#groupingInfo").show();

				//id dupl 오류로 class로변경
				//$("#searchMethod01").show(); 
				//$("#searchMethod02").show();
				
				$(".searchMethod01,.searchMethod02,.searchMethod05").show(); 
				$("#searchMethod03,#searchMethod04").hide();
				
				if (mainData.MAP_DISP_TYPE == "location") {
					$("#dbTypeCk01").next().click();
				} else if( mainData.MAP_DISP_TYPE == "ratio" ){
					$("#dbTypeCk02").next().click();
				} else if( mainData.MAP_DISP_TYPE == "pbubble" ){
					$("#dbTypeCk05").next().click();
				}
				...
				if( mainData.MAP_DISP_TYPE == "ratio" || mainData.MAP_DISP_TYPE == "pbubble" ){
					$("#weightDiv,.weightSelDiv").css("display","").show();
				} else {
					$("#weightDiv,.weightSelDiv").css("display","none").hide();
				}
				...
				$("#groupingInfo").hide();
				$(".searchMethod01,.searchMethod02,.searchMethod05").hide();
				$("#searchMethod03,#searchMethod04").show();
				
				if (mainData.MAP_DISP_TYPE == "bubble") {
					$("#dbTypeCk03").next().click();
				} else {
					$("#dbTypeCk04").next().click();
				}
		...
		drawMapData : function() {
			...
			if (mapData.searchMethod == "location") {
				...
				for (var i = 0; i < dataList.length; i++) {
					...
					var html  = "<div>";
				 	 ...
				 	 html += 	"<table style='width:300px;height:100%;margin-left:-135px;margin-top:-25px; text-align:center;'>";
			...
			// 열지도
				...
				var ratioField = mapData.ratiofield;
				var ratioNum;
				
				if( ratioField != "" && typeof ratioField != 'undefined' && ratioField != null ){
					ratioNum = $mydataDataBoard.Util.changeWord( ratioField ) - 2 ;
					ratioField = ratioField.toUpperCase();
					
					var maxVal = 0;
					
					for( var i=1; i<dataList.length; i++ ){
						var num = Number( dataList[i].USR_DATA[ ratioNum ][ ratioField ] );
						if( isNaN( num ) ){
							num = 0;
						} else {
							if( maxVal < num ){
					...
					if( maxVal > 0 ){
						this.map.zoomLevelHeat = false;
						//나의데이터 radius, blur 값 동일하게
						this.map.setHeatMapOptions( 20, 30, maxVal );
					}
					
				} else {
					this.map.zoomLevelHeat = true;
					this.map.setHeatMapOptions( 20, 30, 1 );
				}
				...
				for( var i=0; i<dataList.length; i++ ){
					...
					if( x_coord && y_coord ){
						...
						if( ratioField != "" && typeof ratioField != 'undefined' ){
							num = Number( dataList[i].USR_DATA[ ratioNum ][ ratioField ] );
						...
						this.map.addHeatMap( x_coord, y_coord, num );
					}
				}

			}
			else if( mapData.searchMethod == "pbubble" ){
				zoomLevel = 10;
				
				//$interactiveMap.ui.mapList[0].markers.remove();
				this.map.clearDataOverlay();
				...
				for( var i=0; i<dataList.length; i++ ){
					...
					var admCd = dataList[i].TOT_REG_CD;
					var data = 0;
					...
					dispData =  mapData.rowHeaderArray[radio1Index].COL_NM + 
							":" + $mydataDataBoard.Util.getFirstKeyValue(dataList[i].USR_DATA[radio1Index] );
					...
					var data = 0;
					if( mapData.ratiofield != "" && typeof mapData.ratiofield != 'undefined' ){
						data = Number( dataList[i].USR_DATA[ $mydataDataBoard.Util.changeWord( mapData.ratiofield ) - 2 ][ mapData.ratiofield.toUpperCase() ] );
					...
					if( x_coord && y_coord ){
						...
						if( !resultList[ admCd.substring(0,7) ] ){
							var resultObject = {
									id : "API_MYDATA",
									result : [ resultRow ],
									pAdmCd : admCd.substring(0,7),
							};
							resultList[ admCd.substring(0,7) ] = resultObject;
							this.map.selectedBoundList.push( resultObject );
						} else {
							resultList[ admCd.substring(0,7) ].result.push( resultRow );
						}
					}
				}
				...
				for( var i in resultList ){
					if( i != "isEmpty" ){
						...
						this.map.multiLayerControl.setStatsData("normal", resultList[i], options, false);
					}
				}
			}
			...
			if( mapData.searchMethod == "ratio" || mapData.searchMethod == "pbubble" ){
				$("#weightTxt").html( ( mapData.searchMethod == "pbubble" ? "버블지도" : "열지도" ) );
				$("#weightDiv,.weightSelDiv").css("display","").show();
			} else {
				$("#weightDiv,.weightSelDiv").css("display","none").hide();
			}
		...
		clearGroupingCell : function() {
			...
			if(document.location.href.match("interactiveMap")){
				srvLogWrite("C0", "02", "07", "05", "", "해제");		//데이터 표출 정보 설정
		...
		reset : function(map_id) {
			...
			if (this.map.markers&&this.map.mapMode!="white") {//20년수정반영
				this.map.markers.clearLayers();
			}
		...
		toolTipChange : function() {
			...
			if(document.location.href.match("interactiveMap")){
				srvLogWrite("C0", "02", "07", "04", "", "");		//데이터 표출정보 설정

			}

			var dispDataArray = new Array();
			var toolTipArray = new Array();
			var ratiofield = $("#weightSel").val();
			// toolTip
			...
			this.mapData[this.map_id].options.ratiofield = ratiofield;
	...
	$mydataDataBoard.Util = {
		...
		groupingCell : function(col_id) {
			if(document.location.href.match("interactiveMap")){
				srvLogWrite("C0", "02", "07", "05", "", "그룹");		//데이터 표출 정보 설정
			...
			body
					.on(
							...
							function() {
								...
								if(document.location.href.match("interactiveMap")){
									srvLogWrite("C0", "02", "07", "03", "", "");		//나의데이터 조회방식 설정
							...
							if (res.errCd == "0") {
								...
								for (var i = 0; i < result.list.length; i++) {
									...
									html += "	<a href=\"javascript:$mydataDataBoard.ui.updateMyData('" 
											+ elem.DATA_ID
											+ "','"
											+ elem.DATA_TITLE + "', '"+elem.MAP_DISP_TYPE+"', '1');\">";
								...
								if (result.totalcount > 5) {
									var htmlPage = "<br><div id='myDataPaging' class='pagenation' align='center' style='width: 100%;'><span class='pages'></span></div>";
							...
							if (res.errCd == "0") {
								...
								for (var i = 0; i < result.list.length; i++) {
									...
									html += "	<a href=\"javascript:$mydataDataBoard.ui.updateMyData('"
											+ elem.DATA_ID
											+ "','"
											+ elem.DATA_TITLE + "', '"+elem.MAP_DISP_TYPE+"', '2');\">";
									html += "		<div class='mydata_title'>"
											+ elem.DATA_TITLE + "</div>";
									html += "		<div class='mydata_info'>"
											+ elem.USR_ID.substring(0, 3)
						...
						onSuccess : function(status, res, options) {
							...
							if (res.errCd == "0") {
								...
								if (extName[extName.length - 1] == "kml") {
									...
									mapData.ratiofield = res.result[0].mainData.RATIOFIELD;
