...
(function(W, D) {
	...
	share = {
		shareInfo : function(map, delegate) {
				...
				if (options != null) {
					if(options.url != null || options.api_call_url != null) {
						...
						if (type == "normal") {
							...
							if (options.params.isEcountry !== undefined && options.params.isEcountry == true) {
								isEcountry = true; 
								dist_level = options.dist_level;
							}
							...
							if (options.param_info.isEcountry !== undefined && options.param_info.isEcountry == true) {
								params.year_nm = $interactiveMapEcountry.ecountry_data_year_nm;
								params.base_item_nm = $interactiveMapEcountry.ecountry_base_item_nm;
								isEcountry = true; 
							}
						}
						...
						if( isEcountry ){
							shareInfo.params.paramInfo.adm_cd = $interactiveMapEcountry.pAdmCd;
						}
			...
			/**
			 * 
			 * @name         : doShareToKakaoTalk
			 * @description  : 카카오talk에 공유한다.
			 * @date         : 2021. 12. 3. 
			 * @author	     : 김준하
			 */
			this.doShareToKakaoTalk = function(linkUrl) {
				Kakao.Auth.login({
					...
					success : function(authObj) {
						...
						Kakao.API.request({
							...
							success: function(res) {
								messageAlert.open("알림", "카카오톡에 정상적으로 공유하였습니다.");
							},
							fail: function(error) {
								messageAlert.open("알림", "카카오톡에 공유를 실패하였습니다.<br>("+error+")");
							},
						})
					},
					
					fail : function(error) {
						messageAlert.open("알림", "카카오톡에 공유를 실패하였습니다.<br>("+error.error_description+")");
					}
				})
			};
			...
			this.openApiRegBookmark = function(params, hist_type, title, srvType, shareType) {
				...
				sopOpenApiRegBookmarkObj.addParam("params", encodeURIComponent(tmpParams) );
