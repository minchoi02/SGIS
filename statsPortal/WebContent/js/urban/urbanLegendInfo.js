/**
 * 도시화 분석 지도 범례에 관한 클래스
 *
 * history : 2021/11/02 초기 작성 version : 1.0 see : 원형(/js/common/mapInfo/legendInfo.js)
 *
 */

(function(W, D) {
	W.$urbanLegendInfo = W.$urbanLegendInfo || {};
	    ...
	    this.drawBubbleMap = function(geojson) {
		    ...
		    geojson.eachLayer(function(layer) {
		    	...
		    	if (layer.feature.info.length > 0) {
		    		if (layer.feature.isEcountry) {
				    	...
				    	var toolTip  = "<div style='margin:10px;'>";
			    			...
			    			if(window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw"){
			    				...
			    				toolTip += "<div style='font-size:12px;padding-left:5px;'>"+title+covidDt+" : "+appendCommaToNumber(data)+unit+"</div>";
			    			} else {
				    	...
				    	var toolTip  = "<div style='margin:10px;'>";
				    		toolTip += 		"<div style='font-size:14px;font-weight:bold;color:#3792de;'>";

				    		if( info.api_id == "API_MYDATA" ){
				    			toolTip += info.dispData;
				    		} else {
				    			toolTip += adm_nm;
				    		}
			    	...
			    	//코로나추가 20200724 주용민
			    	if(window.parent.$thematicMapMain != undefined && window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw"){
				    	marker.on({
				    		click : function(e){
								$(".covidDbTabs").hide();
								$(".dataSideBox").css("height","500px");
								$thematicMapFrame05.ui.mapList[0].adm_nm = layer.feature.properties.adm_nm;;
								$thematicMapFrame05.ui.mapList[0].tmpAdmCd = layer.feature.properties.adm_cd;
								$thematicMapFrame05.ui.mapList[0].getThemaMapDataboardData(layer.feature.properties.adm_cd);
								thematicCharts(layer.feature.properties.adm_cd);

								if(window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw"){
									$thematicMapFrame05.ui.mapList[0].isFirstDraw = true;
									$thematicMapFrame05.ui.mapList[0].selectedAdmCd = layer.feature.properties.adm_cd;
			...
			for (var i=0; i<arData.length; i++) {
				for (var k=0; k<arData[i].length; k++) {
					if (parseFloat(arData[i][k]) <= 0) {
						//코로나추가 20200722 주용민
						if(window.parent.$thematicMapMain != undefined){

							// mng_s 2020. 12. 01 j.h.Seok 통계주제도 통합
//							if((window.parent.$thematicMapMain.param.mapType == "03" || window.parent.$thematicMapMain.param.mapType == "05") && window.parent.$thematicMapMain.param.stat_thema_map_id != "tDpFJzEJKt20161109104000137DvvIDyvrpv" && window.parent.$thematicMapMain.param.stat_thema_map_id != "uwGrLn6xJp20160202203129219tIGyEvtDpH"){
							if(
								(
									window.parent.$thematicMapMain.param.mapType == "03"
									|| window.parent.$thematicMapMain.param.mapType == "05"
									|| window.parent.$thematicMapMain.param.mapType == "13"
									|| window.parent.$thematicMapMain.param.mapType == "15"
								)
								&& window.parent.$thematicMapMain.param.stat_thema_map_id != "tDpFJzEJKt20161109104000137DvvIDyvrpv"
								&& window.parent.$thematicMapMain.param.stat_thema_map_id != "uwGrLn6xJp20160202203129219tIGyEvtDpH"
								&& window.parent.$thematicMapMain.param.stat_thema_map_id != "szJnFHvnFv20160202203129221JKtxLLtCLz"){
							...
							if( !window.parent.$policyStaticSocMap ){			//20210729 정책통계지도 생활 SOC
								this.isNegative = true;
								if (negativeData == null) {negativeData = [];}
								negativeData.push(arData[i][k]);
							}
			...
			var negativeDataTmp = negativeData;
			var tmpArray = [];
			if(negativeDataTmp != null && negativeDataTmp.length >0){
				for(var i=0; i<negativeDataTmp.length; i++){
					if(negativeDataTmp[i] >= 0){
						tmpArray.push(negativeDataTmp[i]);
						negativeDataTmp.splice(i,1);
					}
				}
				if(negativeDataTmp.length == 0){
					this.isNegative = false;
					negativeData = [];
					nagativeLv = 0;
					if(positiveData != null && positiveData == 'undefined'){
						positiveData = positiveData.concat(tmpArray);
					}else{
						positiveData = tmpArray;
					}
				}
			}
			//양수만 존재할 경우
...
//					var colorList2 = getCalculColor("#663240", "#cc0000", positiveLv);
					...
					setLegendColor(colorList,"#colorStatus_"+this.id, this.id,that);
				}
				//음수만 존재할 경우
				else {
					//데이터 중복제거
...
//					var colorList2 = getCalculColor("#a501b7", "#7c02cd", that.lv);
			...
			for ( var k = 0; k < arData.length; k++) {
				if (arData[k].length == 1) {
					...
					for ( var y=0; y <that.lv; y++) { //2016.04.04 범례
						if (result == 1 && min != max) {
							//tmpResult.push(result); //범례 레벨 전부 1로 나오는 현상으로 주석처리한 후 밑에 식으로 갖다 씀 2020.07.17
		...
		this.setUserLegend = function(type, lv) {
			...
			console.log("[urbanLegendInfo.js] setUserLegend() 호출");
			console.log("[urbanLegendInfo.js] setUserLegend() this.map.isInnerMapShow2 [" + this.map.isInnerMapShow2);
			console.log("[urbanLegendInfo.js] setUserLegend() this.map.isInnerMapShow3 [" + this.map.isInnerMapShow3);
		...
		this.colorck = function(){
			...
			$("body").on("click",".colorck li>a",function(){
				...
				console.log("[urbanLegendInfo.js] colorck() color [" + color);
				...
				if(dataid =="colorSetting_"+that.id){
					...
					//mng_s 20210308 이진호
					//selectTyp 에 따른 분기 처리
					if (that.selectType == "dot") {
						that.changeDotColor(color);
					}else{
						console.log("[urbanLegendInfo.js ] changeBubbleColor 호출 ");
						that.changeBubbleColor(color);
					}
					//mng_e 20210308 이진호
		...
		this.goganConfirm = function(){  // 범례구간설정 적용버튼
			$("body").on("click","#goganEvent_"+that.id, function(e){
				...
				//mng_s 20210308 이진호
				//타입이 점(dot)일 때 초기화가 되지 않는 현상 수정
				//that.selectType에 따른 분기처리
				if(that.selectType == "dot"){
					console.log("that type == dot");
					that.changeDotColor();
				}else{
					console.log("that type == bubble or heat");
		...
		this.legendInit = function(isClear){
			...
			//mng_s 20210308 이진호
			//주석 처리 한 소스는 that.changeDataMode("color"); 다음 부분 으로 이동
		...
		this.legendCopy = function(targetId) {
			...
			if(curPage.match(matchStr)){ //mng_s 20180105 정책맵일경우
				...
				if (targetId==0) {
					this.data = uLegendInfo.legendInfo.fixed_legend_val1;
				} else if (targetId==1) {
					this.data = uLegendInfo.legendInfo.fixed_legend_val2;
				} else if (targetId==2) {
					this.data = uLegendInfo.legendInfo.fixed_legend_val3;
				}
			}
			...
			$("#legendDataSlider").slider({
		        ...
		        slide: function( event, ui ) {  //ui.value
		        	...
		        	if(map.isInnerMapShow2){
						...
						$('path[fill=#F0FFF0]').css('fill-opacity', '0.1');
						$urbanLegendInfo.ui.slideInfo.slideValue2 = ui.value;
		        		...
		        		$("path").css('fill-opacity', ui.value);
		        		$urbanLegendInfo.ui.slideInfo.slideValue1 = ui.value;
		        	}
			    }
		    });
			//mng_e
...
function legendColor(c01, c02, cls, max,id,obj){
	...
	console.log("[urbanLegendInfo.js] legendColor() 호출");
	console.log("[urbanLegendInfo.js] legendColor() obj [" + obj);
