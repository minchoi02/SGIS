...
//							//params.region = "sgg";
			...
			setTotSurvData : function(p_map, p_region, p_type, p_sido_cd, p_sgg_cd, p_emdong_cd, p_surv_id, p_callback) {
				if(p_surv_id=='FS0600'&&$totSurvMain.ui.selectedYear == 2020){
					p_surv_id = 'DT_1AG20104';
				}else if(p_surv_id=='FS0013'&&$totSurvMain.ui.selectedYear == 2015){
				    p_surv_id = 'DT_1AG15103'
				}else if(p_surv_id=='FS0315'&&$totSurvMain.ui.selectedYear == 2010){
				    p_surv_id = 'DT_1AG104'
				}else{
				...
				$.ajax({
					...
					for(let i=0; i<copyRes.length; i++) {
						...
						for(let j=0; j<res.length; j++) {
							if(copyRes[i].OV_L1_KOR == res[j].OV_L1_KOR) {
								isAlready = true;
							}
						}
						if(!isAlready) {
							...
							for(let j=0; j<res.length; j++) {
								if(copyRes[i].OV_L1_KOR == res[j].OV_L1_KOR) {
									res[j].DTVAL_CO = parseInt(res[j].DTVAL_CO) + parseInt(copyRes[i].DTVAL_CO);
								}
							}
						}
					}
					...
					if(res[0].OV_L1_ID == "00") {
						...
						for(var i=0; i<res.length; i++) {
							if(res[i].OV_L1_KOR != "읍부" && res[i].OV_L1_KOR != "동부" && res[i].OV_L1_KOR != "면부") {
								if($totSurvMain.ui.selectedArea.length == 2 && $totSurvMain.ui.selectedArea == "00") {
					...
					for(var i =0; i<res.length; i++){
						...
						res.forEach((d)=>{
							d.adm_cd=d.OV_L1_ID;
							d.dt=d.DTVAL_CO;
							d.itm_nm=d.CHAR_ITM_NM;
							d.region_nm=d.OV_L1_KOR;
							d.surv_id=d.surv_id;
					});
						var tmp= {adm_cd: res[i].OV_L1_ID, dt: res[i].DTVAL_CO, itm_nm: res[i].CHAR_ITM_NM, region_nm: res[i].OV_L1_KOR , surv_id: res[i].surv_id};
						tmpdata.push(tmp);
						}
					}
					res=tmpdata;
					...
					/*for(var i=0; i<totalArray.length; i++){
						if(totalArray[i].adm_cd != "00" && totalArray[i].adm_cd != "03" && totalArray[i].adm_cd != "04" && totalArray[i].adm_cd != "05"){
							$totSurvMap.ui.mapTotalVal += parseFloat(totalArray[i].dt);
						}
					}*/
					// 2020-11-18 [곽제욱] 비율계산을 위한 맵데이터값 계산로직 START
					//정보 저장
					$totSurvMap.ui.mapStatsData[lv_tot_surv_id+"_"+p_type+"_"+p_region+"_"+lv_adm_cd] = res;
			...
			setTotSurvData2 : function(p_map, p_region, p_type, p_sido_cd, p_sgg_cd, p_emdong_cd, p_surv_id, p_callback) {
				...
				var api_params = {
					surv_year_list: $totSurvMain.ui.selectedYear							// 수록시점
						, org_id_list: "101"							// 조직번호
							, tbl_id_list: typeof ($farmDash.servMappring[lv_params.surv_id])=='object'?$farmDash.servMappring[lv_params.surv_id][$totSurvMain.ui.selectedYear]:$farmDash.servMappring[lv_params.surv_id]				// 통계표 ID
				...
				//원 테이블로 조회
				//var lv_url = contextPath+"/ServiceAPI/totSurv/common/getTotSurvData.json";
				var lv_url = "/view/totSurv/proxy?" + "http://link.kostat.go.kr/view/kosisApi/TotsurvStatData.do?";
				// ajax 시작
				$.ajax({
					url: lv_url,
				    //type: 'post',
	...
	$totSurvMap.callbackFunc = {
			...
			didSelectedPolygon : function(event, data, type, map) {
				//20210222 박은식 총조사인구 차트선택 이후 맵 조작 시 초기화 START
				if($totSurvMap.ui.selectedObj[0] != undefined &&
				   $totSurvMap.ui.selectedObj != undefined && 
		    	   $totSurvMap.ui.selectedObj != null &&
		    	   $totSurvMap.ui.selectedObj != "" &&
		    	   $totSurvMap.ui.selectedObj.attr("id") == "totalPopulation") {
		    		for(var i=0; i < $(".yearBtn").length; i++){
