 ...
 */
(function(W, D) {
	W.$totSurvMapApi = W.$totSurvMapApi || {};
		
	$totSurvMapApi.census_result_data = [];
	$totSurvMapApi.region_cd = "";
...
//					}
	...
	$totSurvMapApi.request = {
			...
			openApiTotalPopulation : function(params) {
				...
				// 선택된 년도 설정
				sopOpenApiTotalPopulationObj.addParam("year", $totSurvMain.ui.selectedYear);
				// 년도 TODO
			...
			*/  /*2020.11.02 미사용 주석 */
			...
			openApiSearchPopulation : function(region_cd) {
				...
				$totSurvMapApi.region_cd = region_cd;
				// 시도 
				sopOpenApiSearchPopulationObj.addParam("adm_cd", region_cd);
				// 선택된 년도 설정
				sopOpenApiSearchPopulationObj.addParam("year", $totSurvMain.ui.selectedYear);
				// 년도 TODO
			...
			openApiHouseHold : function(region_cd) {
				...
				// 선택된 년도 설정
				sopOpenApiHouseHoldObj.addParam("year", $totSurvMain.ui.selectedYear);
				// 년도 TODO
			...
			openApiSearchHouse : function(region_cd) {
				...
				// 선택된 년도 설정
				sopOpenApiSearchHouseObj.addParam("year", $totSurvMain.ui.selectedYear);
				// 년도 TODO
			...
			openApiFarmHouseHold : function(region_cd) {
				...
				// 선택된 년도 설정
				sopOpenApiFarmHouseHoldObj.addParam("year", $totSurvMain.ui.selectedYear);
				// 년도 TODO
...
//						params : params,
			...
			openApiForestryHouseHold : function(region_cd) {
				...
				// 선택된 년도 설정
				sopOpenApiForestryHouseHoldObj.addParam("year", $totSurvMain.ui.selectedYear);
				// 년도 TODO
			...
			openApiFisheryHouseHold : function(region_cd) {
				...
				// 선택된 년도 설정
				sopOpenApiFisheryHouseHoldObj.addParam("year", $totSurvMain.ui.selectedYear);
				// 년도 TODO
			...
			*/  /*2020.11.02 미사용 주석 */
			...
			openApiHouse : function(region_cd) {
				...
				// 선택된 년도 설정
				sopOpenApiHouseObj.addParam("year", $totSurvMain.ui.selectedYear);
			...
			*/  /*2020.11.02 미사용 주석 */			
			...
			openApiItemCombine : function(params) {
				...
				var api_url = contextPath + $totSurvMapApi.request.API_FUSION_URL;		//초기값은 기본 결합조건api주소
				var methodType = "POST";
					...
					if(params.noneParams[i].key == "fusion_query_type") {	
						if(params.noneParams[i].value == "population" || params.noneParams[i].value == "household" || params.noneParams[i].value == "house") {		//인구, 가구, 주택 조건
							if(params.noneParams[i].value == "population") {
								api_url = openApiPath + $totSurvMapApi.request.API_0302_URL;		//인구 OPENAPI 호출
							} else if(params.noneParams[i].value == "household") {
								api_url = openApiPath + $totSurvMapApi.request.API_0305_URL;		//가구 OPENAPI 호출
							} else if(params.noneParams[i].value == "house") {
								api_url = openApiPath + $totSurvMapApi.request.API_0306_URL;		//주택 OPENAPI 호출
							}
							methodType = "GET";
				...
				if($interactiveMap.ui.chkIfInteractive() && !params.map.isInnerMapShow2 && !params.map.isInnerMapShow3 && (params.is_zoom_lvl4 == "Y" || params.is_non_self == "Y")){	//20200417 수정 (ggm)
					if(api_url == openApiPath + $totSurvMapApi.request.API_0302_URL){
						api_url = contextPath + this.API_0302_1_URL;		//20200427 수정 (ggm)
					}else if(api_url == openApiPath + $totSurvMapApi.request.API_0305_URL){
						api_url = contextPath + this.API_0305_1_URL;		//20200427 수정 (ggm)
					}else if(api_url == openApiPath + $totSurvMapApi.request.API_0306_URL){
			...
			*/  /*2020.11.02 미사용 주석 */
			...
			setStatsDataPopulation : function (res, options, isContry) {
				...
				console.log("----------------------------------------------------------");
				console.log("region_cd  = " + $totSurvMapApi.region_cd);
				console.log("selectArea = " + $totSurvMain.ui.selectedArea);
				console.log("----------------------------------------------------------");
				...
				var lv_unit = "명";
				$totSurvMap.ui.map.setStatsData("normal", {"pAdmCd": $totSurvMapApi.region_cd, "result" : result}, "population", lv_unit);
				
				//경계 그리기
				var lv_region = "emdong"; //비자치구 여부 체크
				// 선택된 년도 설정
				var year = $totSurvMain.ui.selectedYear;
				$totSurvMap.ui.setTotSurvRegion($totSurvMap.ui.map, lv_region, year, $totSurvMapApi.region_cd, "", "", function() {
					// 지도 센터 이동
					$("#dash_sido").val($totSurvMapApi.region_cd.substring(0,2));
					$totSurvMap.ui.mapToggleId = $totSurvMapApi.region_cd;
				});
				$totSurvMap.ui.prevZoom = "5";
				$totSurvMap.ui.map.mapMove($totSurvMain.ui.getSidoSggPos($totSurvMapApi.region_cd), '5', false);
			...
			setStatsDataHouseHold : function (res, options, isContry) {
				...
				// hshs TODO : 선택된 코드로 변경
				var lv_unit = "가구";
				$totSurvMap.ui.map.setStatsData("normal", {"pAdmCd": $totSurvMapApi.region_cd, "result" : result}, "household_cnt", lv_unit);
			...
			setStatsDataHouse : function (res, options, isContry) {
				...
				// hshs TODO : 선택된 코드로 변경
				var lv_unit = "호";
				$totSurvMap.ui.map.setStatsData("normal", {"pAdmCd": $totSurvMapApi.region_cd, "result" : result}, "house_cnt", lv_unit);
			...
			setStatsDataFarmHouseHold : function (res, options, isContry) {
				...
				// hshs TODO : 선택된 코드로 변경
				var lv_unit = "가구";
				$totSurvMap.ui.map.setStatsData("normal", {"pAdmCd": $totSurvMapApi.region_cd, "result" : result}, "farm_cnt", lv_unit);
			...
			setStatsDataForestry : function (res, options, isContry) {
				...
				// hshs TODO : 선택된 코드로 변경
				var lv_unit = "가구";
				$totSurvMap.ui.map.setStatsData("normal", {"pAdmCd": $totSurvMapApi.region_cd, "result" : result}, "forestry_cnt", lv_unit);
			...
			setStatsFishery : function (res, options, isContry) {
				...
				// hshs TODO : 선택된 코드로 변경
				var lv_unit = "명";
				$totSurvMap.ui.map.setStatsData("normal", {"pAdmCd": $totSurvMapApi.region_cd, "result" : result}, "fishery_cnt", lv_unit);
		...
		.define(
			{
				onSuccess : function(status, res, options) {
					...
					switch (parseInt(res.errCd)) {
						case 0:
							var isContry = $totSurvMapApi.request.checkContryData(res, options, map);
								$totSurvMain.ui.selectedLevel = "3"; // 2020-10-13 [곽제욱] 읍면동 조회 성공시 selectedLevel 세팅
								// 인구 읍면동 경계 조회 함수 
								$totSurvMapApi.request.setStatsDataPopulation(res, options, isContry);
						...
						case -401:
							accessTokenInfo(function() {
								$totSurvMapApi.request.openApiSearchPopulation(options.params);
							});
							break;
						case -100:
						...
						case -200:
						    /* 2020-10-06  예외로직 추가 */
							commonTotSurv_alert("더 이상 상세정보가 없습니다.");  // 2020-10-15 박은식 년도 정보를 확인해주세요->더이상 상세정보가 없습니다 문구 수정
							...
							break;
						default:
							$totSurvMapApi.request.combineFailCnt++;
		...
		.define(
				{
					onSuccess : function(status, res, options) {
						...
						switch (parseInt(res.errCd)) {
							case 0:
								var isContry = $totSurvMapApi.request.checkContryData(res, options, map);
								$totSurvMapApi.request.setStatsDataHouseHold(res, options, isContry);		
								break;
							case -401:
								accessTokenInfo(function() {
									$totSurvMapApi.request.openApiHouseHold(options.params);
								});
								break;
							case -100:
								commonTotSurv_alert("검색결과가 존재하지 않습니다.");
					...
					onSuccess : function(status, res, options) {
						...
						switch (parseInt(res.errCd)) {
							case 0:
								var isContry = $totSurvMapApi.request.checkContryData(res, options, map);
								if (map.selectedBoundMode == "multi") {
									$totSurvMapApi.request.setMultiStatsData(res, options, isContry);		
								}else {
									$totSurvMapApi.request.setStatsData(res, options, isContry);		
								}
								break;
							case -401:
								accessTokenInfo(function() {
									$totSurvMapApi.request.openApiHouseHoldMember(options.params);
								});
								break;
							case -100:
								commonTotSurv_alert("검색결과가 존재하지 않습니다.");
		...
		.define(
				{
					onSuccess : function(status, res, options) {
						...
						switch (parseInt(res.errCd)) {
							case 0:
								var isContry = $totSurvMapApi.request.checkContryData(res, options, map);
								$totSurvMapApi.request.setStatsDataFarmHouseHold(res, options, isContry);		
								break;
							case -401:
								accessTokenInfo(function() {
									$totSurvMapApi.request.openApiFarmHouseHold(options.params);
								});
								break;
							case -100:
								commonTotSurv_alert("검색결과가 존재하지 않습니다.");
		...
		.define(
				{
					onSuccess : function(status, res, options) {
						...
						switch (parseInt(res.errCd)) {
							case 0:
								var isContry = $totSurvMapApi.request.checkContryData(res, options, map);
									$totSurvMapApi.request.setStatsDataForestry(res, options, isContry);		
								break;
							case -401:
								accessTokenInfo(function() {
									$totSurvMapApi.request.openApiForestryHouseHold(options.params);
								});
								break;
							case -100:
								commonTotSurv_alert("검색결과가 존재하지 않습니다.");
		...
		.define(
				{
					onSuccess : function(status, res, options) {
						...
						switch (parseInt(res.errCd)) {
							case 0:
								var isContry = $totSurvMapApi.request.checkContryData(res, options, map);
								$totSurvMapApi.request.setStatsFishery(res, options, isContry);		
								break;
							case -401:
								accessTokenInfo(function() {
									$totSurvMapApi.request.openApiFisheryHouseHold(options.params);
								});
								break;
							case -100:
								commonTotSurv_alert("검색결과가 존재하지 않습니다.");
											...
											if (node.childCount == null) {
												if (node.children.length > 0 ) {
													if(node.children[0].id == node.id + "_progress") {
														if (node.isExpanded) {
															$totSurvMapApi.request.openApiInterstryCode(
																	node.depth,
																	options.class_deg,
																	node.cd);
														}
		...
		$class("sop.openApi.Company.api").extend(sop.portal.absAPI).define(
				{
					onSuccess : function(status, res, options) {
						...
						switch (parseInt(res.errCd)) {
							...
							case -401:
								accessTokenInfo(function() {
									$totSurvMapApi.request.openApiCompany(options.params);
								});
								break;
							case -100:
								if (map.selectedBoundMode == "multi") {
									res["result"] = [];
									$totSurvMapApi.request.setMultiStatsData(res, options);	
								}else {
									messageAlert.open(
											"알림", 
											"검색결과가 존재하지 않습니다.",
		...
		$class("sop.openApi.House.api").extend(sop.portal.absAPI).define(
				{
					onSuccess : function(status, res, options) {	
						...
						switch (parseInt(res.errCd)) {
							case 0:
								var isContry = $totSurvMapApi.request.checkContryData(res, options, map);
									$totSurvMapApi.request.setStatsDataHouse(res, options, isContry);		
								break;
							case -401:
								accessTokenInfo(function() {
									$totSurvMapApi.request.openApiHouse(options.params);
								});
								break;
							case -100:
								if (map.selectedBoundMode == "multi") {
						...
						switch (parseInt(res.errCd)) {
							case 0:
								...
								for (var i=0; i<infoList.length; i++) {
									...
									$totSurvMapApi.request.openApiShareForStats(infoList[i]);
								}	
								break;
							default:
								messageAlert.open("알림", res.errMsg);
		...
		$class("sop.openApi.shareForStats.api").extend(sop.portal.absAPI).define(
				{
					onSuccess : function(status, res, options) {
						...
						if (res.errCd == "0") {
							...
							accessTokenInfo(function() {
								$totSurvMapApi.request.openApiShareForStats(options.params, options.adm_cd);
							});
		...
		$class("sop.portal.login.api").extend(sop.portal.absAPI).define(
				{
					onSuccess : function(status, res, options) {
						if (res.errCd == "0") {
							...
							$totSurvMapApi.request.openApiRegBookmark(
							...
							//개발자 로그인
							$totSurvMapApi.request.openApiDeveloperLoginProcess(options.id, options.pwd);
	    ...
	    $class("sop.openApi.geocode.api").extend(sop.portal.absAPI).define({
	        onSuccess : function(status, res, options) {	        
	            if(res.errCd == "0") {
	            	...
	            	accessTokenInfo(function() {
	            		$totSurvMapApi.request.openApiGeocode(options.address);
	            	});
	            }  else if (res.errCd == "-100") {
	            	$interactiveMap.ui.addressAdmCd = "";
        			$interactiveMap.ui.x = "962202";
	            ...
	            var arrayKey = $interactiveMap.ui.sKeyword.split(" ");
	            if(arrayKey.length < 2) {
					$totSurvMapApi.request.openApiSOP($interactiveMap.ui.sKeyword, $interactiveMap.ui.sopCurrentPageIndex);
	            } else {
	            	$totSurvMapApi.request.openApiSOP($interactiveMap.ui.searchKeyword, $interactiveMap.ui.sopCurrentPageIndex);
	            }
	        },
	        onFail : function(status) {
	        	//SOP 검색
	        ...
	        onSuccess : function(status, res, options) {
	            ...
	            if(res.errCd == "0") {
	            	...
	            	for(var i = 0; i < result.resultdata.length; i++) {
	            		...
	            		html += "	<div style='cursor: pointer; margin-left: 20px; font-size: 11px;' onclick=\"javascript:$totSurvMapApi.request.searchReverseGeoCode('sop', '"+elem.url + "','"+elem.nm+"')\">" + elem.nm + "</div>";
	            		html += "</li>";
	            	}
	            	html += "</ul>";
	            	html += "</div>";	            		            
	            	...
	            	accessTokenInfo(function() {
	            		$totSurvMapApi.request.openApiSOP(options.searchword, options.pagenum);
	            	});
	            }
	        },
	        onFail : function(status) {
		...
		$class("sop.openApi.itemcombine.api").extend(sop.portal.absAPI).define(
				{
					onSuccess : function(status, res, options) {	
						if (res.errCd == "0") {
							...
							accessTokenInfo(function() {
								$totSurvMapApi.request.openApiItemCombine(options.params);
							});
						} else {
							var map = options.params.map;
							map.clearDataOverlay();
