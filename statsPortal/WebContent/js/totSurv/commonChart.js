...
function highChartModal(data, chartType) { // 2021-08-25 [이영호] 공통차트 변수 추가
	...
	$commonChart.maxTotVal = 0;
	let chartObjData = {};
	for(var i=0; i<data.length; i++) {
		for(let k=0; k<chartType.length; k++) {
			if(data[i].CHAR_ITM_ID == chartType[k].itmId) {
				if(chartType[k].subsumYn != "Y") {
					for(let l=0; l<chartType.length; l++) {
						if(data[i]["OV_L" + chartType[l].dispVarOrd + "_ID"] == chartType[l].itmId) {
							if(chartType[l].subsumYn != "Y") {
					...
					if($commonChart.maxTotVal < parseInt(data[i].DTVAL_CO)) {
						$commonChart.maxTotVal = parseInt(data[i].DTVAL_CO);
					}
				}
			}
		}
...
function verticalBarHighChart(data, type) {
		...
		for(var j=0; j<ds.length; j++) {
			...
			if(ds[j].CHAR_ITM_ID != "13999001") {
				...
				if(type[0].dispUnitNm != "%") {
					...
					for(var k=0; k<$ecnmyDash.currentData.T30.length; k++) {
						if(ds[j]["OV_" + $ecnmyDash.itmLv.split("_")[1].toUpperCase() + "_ID"] == $ecnmyDash.currentData.T30[k]["OV_L" + $ecnmyDash.ui.dispOptions[4][0].dispVarOrd + "_ID"]) {
							let dtval = (ds[j].DTVAL_CO != undefined ? ds[j].DTVAL_CO : 0);
							dataArr.push({y:parseFloat((parseInt(dtval)/parseInt($ecnmyDash.currentData.T30[k].DTVAL_CO)*100).toFixed(1)), color: $ecnmyDash.chartsOption.color[0]});
							
							if(maxVal < parseInt(dtval)) {
								maxVal = parseFloat((parseInt(dtval)/parseInt($ecnmyDash.currentData.T30[k].DTVAL_CO)*100).toFixed(1));
							}	
						}
					}
				}				
	...
	if($commonChart.maxTotVal != 0) {
		maxVal = $commonChart.maxTotVal;
	}
		...
		for(var j=0; j<ds.length; j++) {
			...
			if(ds[j].CHAR_ITM_ID != "13999001") {
				...
				if(type[0].dispUnitNm != "%") {
					...
					for(var k=0; k<$ecnmyDash.currentData.T30.length; k++) {
						if(ds[j]["OV_" + $ecnmyDash.itmLv.split("_")[1].toUpperCase() + "_ID"] == $ecnmyDash.currentData.T30[k]["OV_L" + $ecnmyDash.ui.dispOptions[4][0].varOrd + "_ID"]) {
							let dtval = (ds[j].DTVAL_CO != undefined ? ds[j].DTVAL_CO : 0);
							dataArr.push({y:parseFloat((parseInt(dtval)/parseInt($ecnmyDash.currentData.T30[k].DTVAL_CO)*100).toFixed(1)), color: $ecnmyDash.chartsOption.color[0]});
...
function pieHighChart(data, type) {  
	...
	if(Object.keys(data).length > 1) { //2레벨
		for(var i=Object.keys(data).length-1; i>=0; i--) {
			var ds = data[Object.keys(data)[i]];
					
			for(var j=0; j<ds.length; j++) {
				var isAlready = false;
				for(var k=0; k<chartData.length; k++) {
					if(chartData[k].id == ds[j]["OV_L" + ovLv + "_ID"]) {
						isAlready = true;	
					}
				}			
				for(var k=0; k<categories.length; k++) {
					if(categories[k] == ds[j]["OV_L" + ovLv + "_KOR"]) {
						isAlready = true;
					}
				}
		...
		for(var i=0; i<categories.length; i++) {
			var subChartData = [], subCategories = [], sum = 0;		
			for(var j=0; j<Object.keys(data).length; j++) {
				var ds = data[Object.keys(data)[j]];
				for(var k=0; k<ds.length; k++) {
					if(categories[i] == ds[k]["OV_L" + ovLv + "_KOR"]) {
						sum = sum + parseInt(ds[k].DTVAL_CO);
						subChartData.push(parseInt(ds[k].DTVAL_CO));
						subCategories.push(ds[k]["CHAR_ITM_NM"]);
					}
				}			
			}
		...
		let industryData = [],
	  		genderData = [],
	  		dataLen = categoryData.length,
	  		drillDataLen;
	
		categoryData.sort(function(a, b) { b.y - a.y });
		for (var i=0; i<dataLen; i++) {
	  		...
	  		drillDataLen = categoryData[i].drilldown.data.length;
	  		for (var j=0; j<drillDataLen; j++) {
				var setColor = "";
	    		if(categoryData[i].drilldown.categories[j] == "남") {
					setColor = "#009589";
				} else {
					setColor = "#f08246";
				}
	    		genderData.push({
	      			name: categoryData[i].drilldown.categories[j],
	      			y: categoryData[i].drilldown.data[j],
	      			color: setColor
	    		});
	  		}
		}
		...
		var chartDiv = $('#chartDiv').highcharts({
			...
			series: [{
				...
				dataLabels: {
					formatter: function() {
						return numberFormat(this.y) + type[0].kosisUnitNm;
					}
				}
			}],
			responsive: {
		...
		for(var i=0; i<data[Object.keys(data)[0]].length; i++) {
			var objData = data[Object.keys(data)[0]];
			chartData.push({
				name: objData[i]["OV_L" + ovLv + "_KOR"],
				y: parseInt(objData[i]["DTVAL_CO"])
			});
		}
		...
		var chartDiv = $('#chartDiv').highcharts({
    		...
    		tooltip: {
      			formatter: function () {
					return "<b>" + this.key + ": <br/>" + numberFormat(parseInt(data[Object.keys(data)[0]][this.point.index].DTVAL_CO)) + type[0].kosisUnitNm + "</b>" ;
				},
				useHTML: true,
				shared: false,
			    shadow: false,
