...
//					}
				...
				this.addPolygonGeoJson = function (obj, type, opt) {
					...
					if($totSurvMain.ui.selectedArea!="99"){
						if($totSurvMain.ui.tileChangeYn=="Y"){
							// 분기
							if($totSurvMain.ui.selectedThema == "인구"){
								$totSurvMain.event.setTileMapChart("areaPopulation", "288", "250", "PH0001");								
							} else if($totSurvMain.ui.selectedThema == "농업"){
								if($totSurvMain.ui.selectedYear == "2020"){
									$totSurvMain.event.setTileMapChart("areaFarm", "288", "250", "FS0600");									
								} else if($totSurvMain.ui.selectedYear == "2015"){
									$totSurvMain.event.setTileMapChart("areaFarm", "288", "250", "FS0013");									
								} else if($totSurvMain.ui.selectedYear == "2010"){
									$totSurvMain.event.setTileMapChart("areaFarm", "288", "250", "FS0315");
								}
							} else if($totSurvMain.ui.selectedThema == "어업"){
								if($totSurvMain.ui.selectedYear == "2020"){
									if($fisheryDash.fisheryType =="sea"){
										$totSurvMain.event.setTileMapChart("areaFishery", "288", "250", "FS0614");	
									} else {
										$totSurvMain.event.setTileMapChart("areaFishery", "288", "250", "FS0621");
									}
								} else if($totSurvMain.ui.selectedYear == "2015"){
								//20201026 박은식 해수면/내수면 분기처리 START
									if($fisheryDash.fisheryType == "sea"){
										$totSurvMain.event.setTileMapChart("areaFishery", "288", "250", "FS0112");	
									} else {
										$totSurvMain.event.setTileMapChart("areaFishery", "288", "250", "FS0171");
									}
								} else if($totSurvMain.ui.selectedYear == "2010"){
									if($fisheryDash.fisheryType == "sea"){
										$totSurvMain.event.setTileMapChart("areaFishery", "288", "250", "FS0413");
									} else {
										$totSurvMain.event.setTileMapChart("areaFishery", "288", "250", "FS0469");
									}
								//20201026 박은식 해수면/내수면 분기처리 END
								}
							} else if($totSurvMain.ui.selectedThema == "주택"){
							//	$totSurvMain.event.setTileMapChart("areaHouse", "288", "250", "PH0001");
								$houseDash.event.setTileMapChart("areaHouse", "288", "250", "PH0001");
							}
							// 가구
							else if($totSurvMain.ui.selectedThema == "가구"){
								//$totSurvMain.event.setTileMapChart("areaHouseHold", "288", "250", "PH0001"
								$houseHoldDash.event.setTileMapChart("areaHouseHold", "288", "250", "PH0001");
							}
							// 2020-10-13 임업 추가 START  jhs
							else if($totSurvMain.ui.selectedThema == "임업"){
								if($totSurvMain.ui.selectedYear == "2015"){
									$totSurvMain.event.setTileMapChart("areaForestry", "288", "250", "FS0235");									
								} else if($totSurvMain.ui.selectedYear == "2010"){
									$totSurvMain.event.setTileMapChart("areaForestry", "288", "250", "FS0532");
								}
							}
							// 2020-10-13 임업 추가 END  jhs
							// 2022-02-11 임업 추가 START [이영호]
				...
				*/
				...
				/*this.setPolyLayerHighlight = function (admCd) {
					if($totSurvMain.ui.selectedThema == "가구"){
						var mapArea = "#areaHouseHold";
					}else if ($totSurvMain.ui.selectedThema == "주택"){
						var mapArea = "#areaHouse";
					}else if ($totSurvMain.ui.selectedThema == "농업"){
						var mapArea = "#areaFarm";
					}else if ($totSurvMain.ui.selectedThema == "임업"){
						var mapArea = "#areaForestry";
					}else if ($totSurvMain.ui.selectedThema == "어업"){
						var mapArea = "#areaFishery";
					}
					if($totSurvMain.ui.selectedThema == "가구" || "주택"){
						if(this.dataGeojson != null) {
							let chartDt = $(mapArea).highcharts().xAxis[0].series[0].data; // 타일(트리)맵 차트 데이터
							var color = "#0086c6";
							var weight = 5;
							var tempColor = $totSurvMap.ui.mapTempColor;
							if(tempColor == "" && tempColor == null) tempColor = "#0086c6";
							
							if($totSurvMap.ui.mapToggleId !== ''){
								let c = $totSurvMain.ui.tilePerColor.filter(function(c){if(c.adm_cd===$totSurvMap.ui.mapToggleId) return c})[0].color;
								for(var i=0; i<chartDt.length; i++) {
									if($totSurvMap.ui.mapToggleId == chartDt[i].id){
										chartDt[i].update({ color: c});
							...
							this.dataGeojson.eachLayer(function(layer) {
								...
								if($totSurvMap.ui.mapToggleId == admCd){
									...
									if(layer.feature.properties.adm_cd == admCd){
										$totSurvMap.ui.mapTempColor = fillColor;
										if($totSurvMap.ui.mapToggleId == ""){
											tempColor = $totSurvMap.ui.mapTempColor; 
										}
										layer.setStyle({
											weight : weight,
											color : color,
										...
										for(var i=0; i<chartDt.length; i++) {
											if(layer.feature.properties.adm_cd == chartDt[i].id) {
												chartDt[i].update({ color: "#0086c6"});
												chartDt[i].select(true);
											}
										}
									}
									else{
										if(layer.feature.properties.adm_cd == $totSurvMap.ui.mapToggleId){
											layer.setStyle({
												weight : 1,
												color : "white",
												dashArray : dashArray,
				...
				*/
				...
				this.setPolyLayerHighlight2 = function (admCd, mapGbn) {
					if(this.dataGeojson != null) {
						...
						var tempColor = ""
						if(mapGbn == "left"){
							tempColor = $totSurvTmsMap.ui.leftMapTempColor;
						} else {
							tempColor = $totSurvTmsMap.ui.rightMapTempColor;
						}
						if(tempColor == "" && tempColor == null) tempColor = "#0086c6";
						...
						this.dataGeojson.eachLayer(function(layer) {
							...
							if($totSurvTmsMap.ui.mapToggleId == admCd){
								if(layer.feature.properties.adm_cd == admCd){
									layer.setStyle({
										weight : weight,
										color : color,
								...
								if(layer.feature.properties.adm_cd == admCd){
									if(mapGbn == "left"){
										$totSurvTmsMap.ui.leftMapTempColor = fillColor;
									} else {
										$totSurvTmsMap.ui.rightMapTempColor = fillColor;
									}
									//$totSurvTmsMap.ui.mapTempColor = fillColor;
									if($totSurvTmsMap.ui.mapToggleId == ""){
										if(mapGbn == "left"){
											tempColor = $totSurvTmsMap.ui.leftMapTempColor;
										} else {
											tempColor = $totSurvTmsMap.ui.rightMapTempColor;
									...
									if(layer.feature.properties.adm_cd == $totSurvTmsMap.ui.mapToggleId){
										layer.setStyle({
											weight : 1,
											color : "white",
											dashArray : dashArray,
				...
				this.setDetailPolyLayerHighlight = function (admCd) {
					if(this.dataGeojson != null) {
						...
						var color = "#0086c6";
						var weight = 5;
						var tempColor = $totSurvDetailMap.ui.mapTempColor;
						if(tempColor == "" && tempColor == null) tempColor = "#0086c6";
						...
						this.dataGeojson.eachLayer(function(layer) {
							...
							if($totSurvDetailMap.ui.mapToggleId == admCd){
								if(layer.feature.properties.adm_cd == admCd){
									layer.setStyle({
										weight : weight,
										color : color,
								...
								if(layer.feature.properties.adm_cd == admCd){
									$totSurvDetailMap.ui.mapTempColor = fillColor;
									if($totSurvDetailMap.ui.mapToggleId == ""){
										tempColor = $totSurvDetailMap.ui.mapTempColor; 
									}
									layer.setStyle({
										weight : weight,
										color : color,
									...
									if(layer.feature.properties.adm_cd == $totSurvDetailMap.ui.mapToggleId){
										layer.setStyle({
											weight : 1,
											color : "white",
											dashArray : dashArray,
								...
								if (feature.info[x].showData) {
									for (param in feature.info[x]) {
										if (param == feature.info[x].showData) {
											...
											if($totSurvMap.ui.selectedObj==""||$totSurvMap.ui.selectedObj[0]==""){
												$totSurvMain.ui.tilePerColor.push(data);
											}
