...
(function(W, D) {
	...
	$(document).ready(function() {
		...
		// 임업 대쉬보드 추가 2020-10-13 jhs END
			...
			$totSurvMain.ui.changeMenu(7);
		} else if(gv_url == "detail2") { // 2020-10-29 [곽제욱] 인구주택총조사 case 추가
...
//			$totSurvMain.ui.ready();
		...
		changeMenu : function (pIndex, pCallback) {
			...
			if ($totSurvMain.ui.loadByPage) {
				...
				if(pIndex=="0"){
					...
					id="ecnmyMenu";
				} else if(pIndex=="8"){ // 2021-08-04 [이영호] 경제총조사 추가
					id="detail2";
				}
				$("#"+id).addClass("on");
			...
			if($totSurvMain.ui.selectedArea != null && $totSurvMain.ui.selectedArea.length == 5){
				if($ecnmyMap.ui.mapToggleId != null && $ecnmyMap.ui.mapToggleId.length == 5){
			...
			if(menuChangeFlag){
				...
				case 1: // 중메뉴: 인구총조사
					if ($totSurvMain.ui.prevIndex == pIndex) { // 주제 변경이 없을경우(년도변화만 존재할경우)
						if($totSurvMain.ui.yearChangeYn == "Y"){
							...
							$ecnmyMap.ui.selectedItmCd = "T100";
							$ecnmyMap.ui.selectedC1 = "";
							$ecnmyMap.ui.selectedC2 = "";
						...
						// 년도변경이 아닌경우
							...
							$ecnmyMap.ui.selectedObj[0] = ""; // 2020-11-19 [곽제욱] 선택한 object 초기화
				...
				case 2: // 중메뉴: 가구총조사
					...
					/** 2020-10-20 [곽제욱] 가구총조사 changeMenu 수정 START */
						if($totSurvMain.ui.yearChangeYn == "Y"){
							$populationDash.ui.clear(); // 인구대쉬보드 차트 삭제
							$ecnmyMap.ui.selectedItmCd = "T200";
							$ecnmyMap.ui.selectedC1 = "";
							$ecnmyMap.ui.selectedC2 = "";
				...
				case 4: // 중메뉴: 농업총조사
					if ($totSurvMain.ui.prevIndex == pIndex) { // 주제 변경이 없을경우(년도변화만 존재할경우)
						if($totSurvMain.ui.yearChangeYn == "Y"){
							...
							$ecnmyMap.ui.selectedItmCd = "T00"; // 2020-11-19 [곽제욱] 농업 아이템코드 가구원->가구로 변경
							$ecnmyMap.ui.selectedC1 = "000";
							$ecnmyMap.ui.selectedC2 = "";
							$totSurvMain.ui.chartSaveClear(); // 2020-10-15 [곽제욱] 선택된 차트항목 초기화
				...
				case 6: // 중메뉴: 어업총조사
					...
					if ($totSurvMain.ui.prevIndex == pIndex) { // 주제 변경이 없을경우(년도변화만 존재할경우)
						if($totSurvMain.ui.yearChangeYn == "Y"){
							...
							$ecnmyMap.ui.selectedItmCd = "T00"; //20201110 박은식 어업 인구에서 가구로 변경
							$ecnmyMap.ui.selectedC1 = "000";
							$ecnmyMap.ui.selectedC2 = "";
							$totSurvMain.ui.chartSaveClear(); // 2020-10-15 [곽제욱] 선택된 차트항목 초기화
							$ecnmyMap.ui.selectedSurvId = "";
				...
				case 12: // 중메뉴: 인구 시계열 // 2020-10-07 [곽제욱] 시계열 index 변경
					if ($totSurvMain.ui.prevIndex == pIndex) { // 주제 변경이 없을경우(년도변화만 존재할경우)
						...
						$ecnmyMap.ui.chartToggleYn = "N" // 2020-12-07 [곽제욱] 선택된 차트항목 초기화
				...
				case 7: // 중메뉴: 경제총조사 // 2021-08-04 [이영호] 
					if ($totSurvMain.ui.prevIndex == pIndex) { // 주제 변경이 없을경우(년도변화만 존재할경우)
						if($totSurvMain.ui.yearChangeYn == "Y") {
							...
							$totSurvMain.ui.refresh("sido", "00");							
							$totSurvMain.ui.selectedArea = "00";
						...
						$ecnmyMap.ui.chartToggleYn = "N" // 2021-08-04 [이영호] 선택된 차트항목 초기화
						...
						srvLogWrite('P0','10','01','00',$totSurvMain.ui.selectedThema+'|'+$ecnmyDash.ecnmyType);
					}					
					isHeaderCommBtn(true);  // 2020.10.23[주형식] 헤더 공통버튼 show/hide 추가
					break;
				case 8: // 중메뉴: 2021-09-14 [이영호] 상세지표 조회
					if ($totSurvMain.ui.prevIndex == pIndex) {
						/** 2020.10.28[한광희] 상세페이지 화면에서 상세페이지 메뉴 클릭시 초기화 하도록 수정 START */
						$totSurvMain.ui.removeMap();
		...
		ChangeMenuCallback: function(pIndex) {
			...
			if($totSurvMain.ui.yearChangeYn=="N") {
				...
				case 7: if (typeof $ecnmyDash.ui.ready != 'undefined') { $ecnmyDash.ui.ready(); } break; // 2020-10-07 [곽제욱] 시계열 index 변경
				case 8: if (typeof $ecnmyDash.ui.ready != 'undefined') { $totSurvDetail.ui.ready(); } break; // 2020-10-07 [곽제욱] 시계열 index 변경
				default: return;
				}
			}
			// event 바인딩
			...
			if($ecnmyMap.ui.mapToggleId!=""){
				$ecnmyMap.ui.tileTempColor = $("rect[value='"+$ecnmyMap.ui.mapToggleId+"']").attr("fill");
				$("rect[value='"+$ecnmyMap.ui.mapToggleId+"']").attr("fill", "#0086c6"); // 2020-11-02 [곽제욱] 타일차트 색변경 추가
			}
...
//			}
		...
		titleChange : function(adm_cd){
			...
			if($ecnmyMap.ui.mapToggleId != adm_cd) {
				...
				// 2020-10-27 [곽제욱] title 문구변경 로직 추가 START
					...
					title = $("#dash_sgg option[value='"+$ecnmyMap.ui.mapToggleId.substring(2,5)+"']").html();
		 ...
		 getSidoSggPos : function(region_cd){
			...
			if(region_cd.length == 2){
				...
				if(sido != undefined && sgg != null){
					...
					if($ecnmyMap.ui.mapToggleId == "" && $totSurvMain.ui.selectedArea.length ==5 && ($totSurvMain.ui.selectedArea != region_cd)){
		...
		setHelpTooltip : function(target){
			...
			case "totalIMF":
				...
				text="해당 연도 아시아, 아메리카, 유럽, 아프리카,<br/> 오세아니아에 속하는 210개 국가 중 사업체 순위";
				authorship = "IMF";
				break;
			case "totalSido":
				title = "광역시도 중";
				text="대한민국 1개의 특별시, 6개의 광역시, 8개의 도,<br/> 1개의 특별자치도, 1개의 특별자치시 총 17개 <br/>광역시도 중 사업체 순위<br/>※세종특별자치시는 2012년도 부터 적용";
				authorship = "통계청, 『경제총조사』";
			...
			case "totalSgg":
				title = "시군구 중";
				text="대한민국 시군·구 중 사업체 순위<br/>※행정시·자치구가 아닌 시는 순위에 포함하고,<br/>행정시·자치구가 속한 구는 순위에서 제외함";
				authorship = "통계청, 『경제총조사』";
				linkUrl = "http://kosis.kr/statHtml/statHtml.do?orgId=101&tblId=DT_1IN1502&lang_mode=ko&vw_cd=MT_ZTITLE&list_id=A11_2015_1_10_10&conn_path=I4"; // 2020-10-29 [곽제욱] url 추가
				break;
			/*공통 end*/
			...
			case "fisheryRatioHelp": // 20201023 박은식 임가 도움말 id값 추가
				...
				// 2020-10-29 [곽제욱] url 추가 END
				break;
			// 2021-12-22 [이영호] url 추가 START
			case "ecnmyHelp": // 20211222 이영호 경총 도움말 id값 추가
				title = "사업체 수";
				text="전주기 대비 사업체수 증감률</br>(비교대상 – 기준 / 기준) * 100";
				authorship = "통계청, 『경제총조사』";
				if($ecnmyDash.ecnmyType == "ecnmy9th"){
					if($totSurvMain.ui.selectedYear == "2010"){
						linkUrl = "https://kosis.kr/statHtml/statHtml.do?orgId=101&tblId=DT_1KI2002&conn_path=I2";
					} else {
						linkUrl = "https://kosis.kr/statHtml/statHtml.do?orgId=101&tblId=DT_1KI1510&conn_path=I2";
					}
				} else {
					if($totSurvMain.ui.selectedYear == "2015"){
						linkUrl = "https://kosis.kr/statHtml/statHtml.do?orgId=101&tblId=DT_1KI1510_10&conn_path=I2";
					}
				}
				break;
			case "ecnmyWorkerHelp": // 20211222 이영호 경총 도움말 id값 추가
				title = "종사자 수";
				text="전주기 대비 종사자수 증감률</br>(비교대상 – 기준 / 기준) * 100";
				authorship = "통계청, 『경제총조사』";
				if($ecnmyDash.ecnmyType == "ecnmy9th"){
					if($totSurvMain.ui.selectedYear == "2010"){
						linkUrl = "https://kosis.kr/statHtml/statHtml.do?orgId=101&tblId=DT_1KI2002&conn_path=I2";
				...
				break;
			case "ecnmySalesHelp": // 20211222 이영호 경총 도움말 id값 추가
				title = "매출액";
				text="전주기 대비 매출액 증감률</br>(비교대상 – 기준 / 기준) * 100</br></br>";
				text+="※ 매출액 및 영업(사업)수익: 지난 1년간 영업활동으로 벌어들인 영업수입액으로써 매출 부가가치세는 제외함";
				authorship = "통계청, 『경제총조사』";
				if($ecnmyDash.ecnmyType == "ecnmy9th"){
					if($totSurvMain.ui.selectedYear == "2010"){
						linkUrl = "https://kosis.kr/statHtml/statHtml.do?orgId=101&tblId=DT_1KI2002&conn_path=I2";
				...
				break;
			case "ecnmyMainHelp": // 2022-01-06 이영호 경총 10차 추가
				title = "사업체 현황";
				text="‘15년 기준 경제총조사 산업분류 10차 개정 자료는 9차 산업분류를 10차로 연계하여 서비스함</br>" +
					" 9차 산업분류에서 10차 산업분류로 개정되면서" +
					" 산업대분류별 사업체 수 등이 변경되었으므로" +
					" 산업분류 차수 간 비교보다는 통계활용도 제고를" +
					" 위한 참고용으로 활용하도록 권고함";
				authorship = "통계청, 『경제총조사』";
				if($ecnmyDash.ecnmyType == "ecnmy9th"){
					if($totSurvMain.ui.selectedYear == "2010"){
		...
		chartSelectedSave : function(target, data, color, index, resizeYn, title){
			...
			//20201102 박은식 특수한 차트 처리를 위한 분기처리 START
				...
				//20201126 박은식 차트 유지 로직 추가 END
					...
					let copyObj = $("#"+target).find(".eventGroup"+data.idx).find("rect").eq(index).slice();
					$ecnmyMap.ui.selectedObj[0] = copyObj[0];
				...
				//20201126 박은식 차트 유지 로직 추가 END
					...
					copyObj = $("#"+target).find(".eventGroup").eq(index).slice();
					$ecnmyMap.ui.selectedObj[0] = copyObj[0];
					if(title != "" && title != undefined){
						$("#itmDiv").html(title);
					}
		...
		chartSaveClear : function(){
    		...
    		//$ecnmyMap.ui.selectedObj = ""; // 2020-11-12 [곽제욱] 선택항목 초기화 수정
    		$ecnmyMap.ui.selectedItmCd = $ecnmyMap.ui.selectedItmCd;
    		$ecnmyMap.ui.selectedSurvId = $totSurvMain.ui.selectedSurvId; // 2020-10-20 [박은식] survId 추가
    		//20201126 박은식 초기화 값 변경 START
    		$ecnmyMap.ui.selectedC1 = "";
    		$ecnmyMap.ui.selectedC2 = "";
    		$ecnmyMap.ui.selectedC3 = "";
...
//					console.log("########## kakao linkUrl = " + linkUrl);
			...
			setTileMapChart : function(target, width, height, surv_id){
				...
				if($totSurvMain.ui.selectedLevel != "3" || $ecnmyMap.ui.isAtdrc){
					...
					// 선택한 itmCd
					var itm_cd = $ecnmyMap.ui.selectedItmCd;
					...
					if(target == "areaFarm" ){
						...
						$ecnmyMap.ui.selectedC1 = c1;
						$ecnmyMap.ui.selectedItmCd = itm_cd;
					}
					...
					$.ajax({
						...
						data: { year: year, region_cd : regionCd, surv_id : surv_id, itm_cd : itm_cd, isAtdrc:$ecnmyMap.ui.isAtdrc, c1:c1}, // 
						dataType: "json",
						success: function(res) {
							if (res.errCd == "0") {
								...
								svg
								    ...
								    .on("click", function(){ // click 이벤트
								    	...
								    	if($totSurvMain.ui.selectedThema == "어업"){
								    		if($totSurvMain.ui.selectedYear == "2015"){
												if($fisheryDash.fisheryType =="sea"){
													$ecnmyMap.ui.selectedSurvId = "FS0112";
												} else {
													$ecnmyMap.ui.selectedSurvId = "FS0171";
												}
																
											} else if($totSurvMain.ui.selectedYear == "2010"){
												if($fisheryDash.fisheryType =="sea"){
													$ecnmyMap.ui.selectedSurvId = "FS0413";
												} else {
													$ecnmyMap.ui.selectedSurvId = "FS0469";
												}
											}
								    	}
								    	...
								    	if(region_cd.length == 2){
								    		...
								    		if($totSurvMain.ui.selectedThema == "인구"){
								    			...
								    			$ecnmyMap.ui.selectedC1 = "000";
								    			//$ecnmyMap.ui.selectedItmCd = "T01";
								    			$ecnmyMap.ui.selectedItmCd = "T00";    // 2020.11.10 주형식 가구수로 변경
									    		if($totSurvMain.ui.selectedYear == "2015"){
								    				$ecnmyMap.ui.selectedSurvId = "FS0013";								    				
								    			} else if($totSurvMain.ui.selectedYear == "2010"){
								    				$ecnmyMap.ui.selectedSurvId = "FS0315";
								    			}
								    		...
								    		// 2020-10-13 임업대쉬보드 추가 START jhs
								    			...
								    			$ecnmyMap.ui.selectedC1 = "000";
								    			$ecnmyMap.ui.selectedItmCd = "T00"; // 2020-11-17 [곽제욱] 임업 itm_cd 수정 T01->T00
									    		if($totSurvMain.ui.selectedYear == "2015"){
								    				$ecnmyMap.ui.selectedSurvId = "FS0235";								    				
								    			} else if($totSurvMain.ui.selectedYear == "2010"){
								    				$ecnmyMap.ui.selectedSurvId = "FS0532";
								    			}
								    		...
								    		// 2020-10-13 임업대쉬보드 추가 END jhs
								    			...
								    			$ecnmyMap.ui.selectedC1 = "000";
								    			$ecnmyMap.ui.selectedItmCd = "T00"; //20201110 박은식 어업 itm_cd 변경
//								    			/** 2020-10-26 [곽제욱] 해수면/내수면 분기로 인한 주석처리 START */
								    	...
								    	// 시군구 데이터 일 경우 kosis정보 호출
								    		...
								    		// 선택한 지역 하이라이트
								    		//$ecnmyMap.ui.map.setPolyLayerHighlight(region_cd);
								    		// 인구.... 
								    		// 비자치 구인지 체크한다.
								    		...
								    		if($ecnmyMap.ui.isAtdrc != true){
								    			...
								    			if($totSurvMain.ui.selectedThema == "인구"){
								    				...
								    				//$ecnmyMapApi.request.openApiSearchPopulation(region_cd); // 2020-10-15 [곽제욱] 공간정보서비스과 요청으로 읍면동 조회 주석처리 
...
//								    				alert(">>>>>> 어업.... 비자치구..........");
								    		...
								    		//$ecnmyMap.ui.map.setPolyLayerHighlight($totSurvMain.ui.selectedArea); // 2020-10-06 [곽제욱] 타일차트 클릭시 하이라이트 설정 제거
								    		if($totSurvMain.ui.selectedThema == "인구"){
								    			$populationDash.ui.getRankSet("", "",region_cd)								    			
								    		}
								    		//20201019 박은식 주택 추가 START
 ...
 */
function getKosisMataDataUrl(url){
