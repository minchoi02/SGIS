...
(function(W, D) {
	...
	$ecnmyMap.ui = {
			...
			clearLayer: function() {// used
				$totSurvMain.ui.log("$ecnmyMap.ui.clearLayer - begin");
			...
			createMap : function(id, seq) {
				...
				this.map.createMap($ecnmyMap, id, {
				...
				//지도 범례 등록
				var legend = new sLegendInfo.legendInfo($ecnmyMap.ui.map);
				legend.linkTooltip = function() {}; //툴팁오류 방지
				legend.drawBubbleMap = $ecnmyMap.ui.drawBubbleMap; // 버블 지도 Override
				legend.initialize($ecnmyMap.ui);
				this.map.legend = legend;
				legend.createLegend();
				
				if($totSurvMain.ui.selectedThema != "경제") {
					this.map.gMap.whenReady(function() {
						$ecnmyMap.ui.map.createHeatMap();
					});
				}				
			...
			createInfoTooltip : function(event, data, type, map) {
				...
				var total = $ecnmyMap.ui.mapTotalVal;
				var ratio = 0;
		        if(total != 0 && total !=""){
		        	if(data.info.length>0){
		        		ratio = (Math.round(data.info[0][data.info[0].showData] / total * 100 * 100) / 100);
		        	} else {
		        		ratio = 0;
		        	}
		        } else {
...
//					if($ecnmyMap.ui.selectedSurvId == "FS0013" || $ecnmyMap.ui.selectedSurvId == "FS0315") {
//						lv_html += " (명)";
//					} else {
						lv_html += " (가구)";
//					}
					...
					/*if($ecnmyMap.ui.selectedSurvId == "FS0235" || $ecnmyMap.ui.selectedSurvId == "FS0532"){
						lv_html += " (명)";
					} else {*/
					lv_html += " (가구)";
					/*}*/
...
//					if($ecnmyMap.ui.selectedSurvId == "FS0112" || $ecnmyMap.ui.selectedSurvId == "FS0171" || $ecnmyMap.ui.selectedSurvId == "FS0413" || $ecnmyMap.ui.selectedSurvId == "FS0469"){
						lv_html += " (가구)"; 
//					} else {
//						lv_html += " (가구)";
//					}
				//20201110 박은식 어업 인구에서 가구로 변경 END
					...
					if($($("div[id^=highChartDiv]div[id$=Wrap]")[0]).css("border-width") == "3px") {
						lv_html += "(" + $ecnmyDash.ui.dispOptions[1][0].kosisUnitNm + ")";
					} else if($($("div[id^=highChartDiv]div[id$=Wrap]")[1]).css("border-width") == "3px") {
						lv_html += "(" + $ecnmyDash.ui.dispOptions[2][0].kosisUnitNm + ")";
					} else if($($("div[id^=highChartDiv]div[id$=Wrap]")[2]).css("border-width") == "3px") {
						lv_html += "(" + $ecnmyDash.ui.dispOptions[3][0].kosisUnitNm + ")";
					} else if($($("div[id^=highChartDiv]div[id$=Wrap]")[3]).css("border-width") == "3px") {
						lv_html += "(" + $ecnmyDash.ui.dispOptions[4][0].kosisUnitNm + ")";
					}
				}
			...
			drawMapData : function(p_map_region, p_map_type) {
				...
				var lv_surv_id = $ecnmyMap.ui.selectedSurvId;
				...
				if(p_map_type == "color" || p_map_type == "bubble") {
					...
					if(p_map_region == "sido") {
						//
						$ecnmyMap.ui.setTotSurvData($ecnmyMap.ui.map, "sido", "color", "", "", "",  lv_surv_id, function(p_list) {
							//리스트에서 unit 가져오기
							var lv_unit = "개";
							var lv_unit_nm = "수";
							...
							//데이터 넣기
							$ecnmyMap.ui.map.setStatsData("normal", {"pAdmCd": "00", "result" : p_list}, "dt", lv_unit);
					...
					//색상/버블 (시군구)
						...
						console.log("================  sgg  =================");
						// 비자치구 일경우 
						if($ecnmyMap.ui.isAtdrc == true && ($totSurvMain.ui.selectedArea).length == 5 && ($ecnmyMap.ui.mapToggleId == "" || $totSurvMain.ui.yearChangeYn == "Y" || gv_type=="locgov" || gv_type=="totFarmLocgov" || gv_type=="totPeopleLocgov" ) ){ // 2020-10-07 [곽제욱] 순위검색 case 예외처리 // 2020-11-05 [곽제욱] 조건 추가
//							//params.region = "sgg";
							var s_sido_cd = "";
							var s_sgg_cd = "";
							...
							$ecnmyMap.ui.setTotSurvData($ecnmyMap.ui.map, "sgg", "color", s_sido_cd, s_sgg_cd, "", lv_surv_id, function(p_list) {
								//리스트에서 unit 가져오기
								var lv_unit = "개";
								var lv_unit_nm = "수";
								if(p_list != null && p_list.length > 0) {
								...
								//데이터 넣기
								$ecnmyMap.ui.map.setStatsData("normal", {"pAdmCd": s_sido_cd+s_sgg_cd, "result" : p_list}, "dt", lv_unit);
							...
							$ecnmyMap.ui.setTotSurvData($ecnmyMap.ui.map, "sgg", "color", lv_sido_cd, lv_sgg_cd, "", lv_surv_id, function(p_list) {
								//리스트에서 unit 가져오기
								var lv_unit = "개";
								var lv_unit_nm = "수";
								if(p_list != null && p_list.length > 0) {
								...
								//데이터 넣기
								$ecnmyMap.ui.map.setStatsData("normal", {"pAdmCd": lv_sido_cd, "result" : p_list}, "dt", lv_unit);
								...
								$ecnmyMap.ui.setTotSurvRegion($ecnmyMap.ui.map, lv_region, $ecnmyMap.ui.getTotSurvRegionYear(), lv_sido_cd, "", "", function() {
									...
									$totSurvMain.ui.pathChange(lv_region, tempRegionCd.substring(4,5)+"0");
				...
				$.ajax({
					...
					if(totalArray[i].adm_cd != "00"){
						for(var i=0; i<totalArray.length; i++){
							$ecnmyMap.ui.mapTotalVal += parseFloat(totalArray[i].dt);
						}
					} else {
						$ecnmyMap.ui.mapTotalVal += parseFloat(totalArray[0].dt);
					}
			...
			setTotSurvRegion : function(p_map, p_region, p_base_year, p_sido_cd, p_sgg_cd, p_emdong_cd, p_callback) {
				...
				if(p_region == "sido") {
					...
					p_map.openApiBoundarySido(p_base_year, function(p_map2, p_res) {
						//정보 저장
						$ecnmyMap.ui.mapRegionData[p_region+"_"+lv_year+"_"+lv_adm_cd] = p_res;
						
						//로딩바 숨김
						if($totSurvMain.ui.currentPageName == "ecnmyMap" && p_map.id == "ecnmyMapMap") {
							//$totSurvMain.ui.loading(false);
						}
						if($("#"+p_map.id+"_loading").length) {
							$("#"+p_map.id+"_loading").hide();
						...
						if($totSurvMain.ui.selectedArea == "00"){
							...
							zoomLevel = $ecnmyMap.ui.map.zoom;
						}
				...
				//시도 이외의 경계 데이터 조회
					...
					if(p_sido_cd.length==5){
						if($ecnmyMap.ui.mapToggleId != null && $ecnmyMap.ui.mapToggleId != ""){
					...
					$.ajax({
						...
						if(res.errCd == "0") {
							...
							$ecnmyMap.ui.isAtdrc = res.result.rslt;
						}
					});
					...
					$.ajax({
						...
						if(res.errCd == "0") {
							...
							//정보 저장
							$ecnmyMap.ui.mapRegionData[p_region+"_"+lv_year+"_"+p_sido_cd+p_sgg_cd] = res;
							//경계그리기
							...
							$ecnmyMap.ui.checkIsAtdrc(p_sido_cd+p_sgg_cd.substring(0,2)+"0")
							// 시군구에서 슬라이드 이동(랭킹이동)시 x좌표 y좌표를 선택한 곳으로 세팅
							if($ecnmyMap.ui.mapToggleId != "" && $ecnmyMap.ui.mapToggleId.length == 5 && !$ecnmyMap.ui.isAtdrc){
								var tempAdmCd = $ecnmyMap.ui.mapToggleId;
								$totSurvMain.ui.getSidoSggPos(tempAdmCd);
								$("#dash_sido").val(tempAdmCd.substring(0,2));
								xcoor = $("#dash_sgg option:selected").attr("data-coor-x");
								ycoor = $("#dash_sgg option:selected").attr("data-coor-y");
							} else if($ecnmyMap.ui.mapToggleId != "" && $ecnmyMap.ui.mapToggleId.length == 5 && $ecnmyMap.ui.isAtdrc){
								var tempAdmCd = $ecnmyMap.ui.mapToggleId.substring(0,4)+"0";
								$totSurvMain.ui.getSidoSggPos(tempAdmCd);
								$("#dash_sido").val(tempAdmCd.substring(0,2));
								xcoor = $("#dash_sgg option:selected").attr("data-coor-x");
								ycoor = $("#dash_sgg option:selected").attr("data-coor-y");
							...
							$ecnmyMap.ui.prevZoom = zoomLevel;
			...
			openApiBoundarySido : function (year, callback) {// used
				$totSurvMain.ui.log("$ecnmyMap.ui.openApiBoundarySido - begin");
				...
				$.ajax({
					  ...
					  success: function(res) {
						  ...
						  $ecnmyMap.ui.map = $ecnmyMap.ui.mapList[$ecnmyMap.ui.curMapId];
						  
						  $ecnmyMap.ui.map.mapMove([989674, 1818313], 2);
						  
						  $ecnmyMap.ui.lastGeojsonInfo = tmpOption ;
						  $ecnmyMap.ui.setPolygonDataGeojson(res);
			...
			setPolygonDataGeojson : function (geoData) {// used
				$totSurvMain.ui.log("$ecnmyMap.ui.setPolygonDataGeojson - begin");
				
				var that = $ecnmyMap.ui;
			...
			combineTotSurvData : function (boundData, isPass) {// used
				$totSurvMain.ui.log("$ecnmyMap.ui.combineTotSurvData - begin");
				
				this.data = $ecnmyMap.ui.MapData;
			...
			getCenterToAdmCd : function(center,callback) {
				...
				if(Object.prototype.toString.call(center)==="[object Array]"&&center.length==2){
					...
					center = $ecnmyMap.ui.map.gMap.getCenter();
					x = center.x;
					y = center.y;
				}
	...
	// map event callback
	// ==============================//
	$ecnmyMap.callbackFunc = {

			// 맵이동 시작시, 콜백 호출
			didMapMoveStart : function(event, map) {
				console.log("[ecnmyMap.js] didMapMoveStart 호출");
			},

			// 맵이동 종료시, 콜백 호출
			didMapMoveEnd : function(event, map) {
				
				console.log("[ecnmyMap.js] didMapMoveEnd 호출");
				console.log("[ecnmyMap.js] didMapMoveEnd() $ecnmyMap.ui.dropBtnInfo[map.id] [" + $ecnmyMap.ui.dropBtnInfo[map.id]);
			...
			didMapZoomEnd : function(event, map) {
				...
				var prevZoom = $ecnmyMap.ui.prevZoom;
				...
				if(prevZoom != lv_zoom){
					...
					if($(".mapInfo").css('display') == "none" || $(".mapInfo").css('display') == undefined){ // 2020-10-13 [곽제욱] 개방형지도 로직 변경으로 인한 조건문 처리
						var center = $ecnmyMap.ui.map.gMap.getCenter();
						$ecnmyMap.ui.getCenterToAdmCd(center, function(res) {
							...
							switch (adm_cd.length) {
								case 2:
									...
									if($ecnmyMap.ui.selectedObj != undefined && 
							    	   $ecnmyMap.ui.selectedObj != null &&
							...
							if(lv_zoom < sggZoom) {
								...
								if($ecnmyMap.ui.mapRegion != "sido") {
									...
									else if(selectedThema == "경제"){ //2021-09-06 [이영호] 경제 START
										$ecnmyDash.event.allChange("00", "1");
										//$("#oldEcnmyRanking").html("전국의 어가 인구 중 고령인구");
										//$("#fisheryHouseRanking").html("전국의 어가(가구) 수");
									} //2021-09-06 [이영호] 경제 END
								}
							...
							// 경기도 -> 전국 이동
								...
								switch (adm_cd2.length) {
						            case 5:
						            	$ecnmyMap.ui.checkIsAtdrc(adm_cd2.substring(0,4)+"0");
						            	if($ecnmyMap.ui.isAtdrc==true){
						            		emdongZoom = 6;
						            	} else {
						            		emdongZoom = 6;
								...
								if(lv_zoom >= emdongZoom){
									if(!($ecnmyMap.ui.mapRegion == "sgg" && lv_sido_change_yn == "N" && !$ecnmyMap.ui.isAtdrc)){
										if($ecnmyMap.ui.mapRegion!="emdong" && lv_atdrc_change_yn == "Y"){ // 2020-11-11 [곽제욱] 비자치구 경계 체크로직 추가
											...
											$ecnmyMap.ui.drawMapData("sgg", "color");
											...
											//20201022 박은식 임업, 어업 분기처리 추가 END
											else if($totSurvMain.ui.selectedThema == "경제"){ //2021-09-06 [이영호] 경제 START
												$ecnmyDash.event.allChange($totSurvMain.ui.selectedArea, "2");
											}
											//2021-09-06 [이영호] 경제 END
										}
									} 
									...
									//이미 같은 시군구이면 조회 안함
									if(!($ecnmyMap.ui.mapRegion == "sgg" && lv_sido_change_yn == "N")) {
										...
										// 각 주제별 차트 변화함수 호출
										$totSurvMain.ui.selectedLevel = "2";
										$ecnmyMap.ui.mapToggleId = ""; // 2020-10-15 [곽제욱] 맵토글ID 초기화
										if(selectedThema == "인구"){
										...
										//20201022 박은식 임업, 어업 분기처리 추가 END										
											...
											$ecnmyDash.event.allChange(lv_to_sido_cd, "1");
											$ecnmyDash.ui.getRankSet("", "", lv_to_sido_cd);
										}	//2021-09-06 [이영호] 경제 END
									} else {
							...
							//읍면동
								...
								//이미 같은 읍면동이면 조회 안함
								if(!($ecnmyMap.ui.mapRegion == "emdong" && lv_sgg_change_yn == "N")) {
				...
				if (type != "polygon") {
					if( map.isInnerMapShow2!=undefined && map.isInnerMapShow2 ) { //mng_s 그리드일 경우 
						...
						if (data.info.length > 0) { //데이터가 있을 경우만 툴팁을 보여준다.
							$ecnmyMap.ui.createInfoTooltip(event, data, type, map);
						}
			...
			didSelectedPolygon : function(event, data, type, map) {
				...
				if($ecnmyMap.ui.selectedObj != undefined && 
		    	   $ecnmyMap.ui.selectedObj != null &&
		    	   $ecnmyMap.ui.selectedObj != "") {
					if($ecnmyMap.ui.selectedObj.attr("id") == "totalPopulation") {
						for(var i=0; i < $(".yearBtn").length; i++){
						    if($(".yearBtn").eq(i).hasClass("on")){
						    	$totSurvMain.ui.selectedYear = $(".yearBtn").eq(i).text();
						    }
						}
			    		$populationDash.ui.chartItmClick($ecnmyMap.ui.selectedObj, "#576574", $totSurvMain.ui.selectedYear);
					}
		    	}
		    	//20210222 박은식 총조사인구 차트선택 이후 맵 조작 시 초기화 END
				...
				if (type == "data") {
					if(data.info[0].adm_cd.length == 7 ){
						...
						var title = "";
						if($ecnmyMap.ui.mapToggleId != data.info[0].adm_cd) {
							//20201019 박은식 각 태마별 분기처리 추가 START
							// 전체 차트 변경처리
							var selectedThema = $totSurvMain.ui.selectedThema;
							$ecnmyDash.polygonSelectArea = data.properties.adm_cd; //총조사인구 연령별 분포 차트 단위 변경 기준 변수(하이라이트 적용 시 지역 코드 적용)
							...
							if(adm_cd.length == 2) {
								$ecnmyDash.event.pathChange("sgg", adm_cd);
							} else if(adm_cd.length == 5 && adm_cd.substring(4,5) == "0") {
								...
								$totSurvMain.ui.selectedLevel = "2";
								$totSurvMain.ui.getSidoSggPos(adm_cd);
								$ecnmyDash.event.pathChange("atdrc", adm_cd);
								...
								$totSurvMain.ui.selectedLevel = "2";
								$totSurvMain.ui.getSidoSggPos(adm_cd);
								$ecnmyDash.event.pathChange("emdong", adm_cd);
							}
							...
							$ecnmyDash.event.allChange(adm_cd, "2");
							
							//20201022 박은식 임업, 어업 분기처리 추가 END
							$ecnmyDash.event.titleChange(adm_cd);
							
							// 선택한 지역 하이라이트
							$ecnmyMap.ui.map.setPolyLayerHighlight(adm_cd);
							$ecnmyMap.ui.mapToggleId = data.info[0].adm_cd;
							$ecnmyDash.ui.getRankSet("","", data.info[0].adm_cd);
						} else { 
							$ecnmyDash.polygonSelectArea = ""; //총조사인구 연령별 분포 차트 단위 변경 기준 변수(하이라이트 토글 시 지역 코드 초기화)
							var adm_cd = data.info[0].adm_cd;
							var sidoCd = adm_cd.substring(0,2);
							var sggCd = "999";
							var selectedThema = $totSurvMain.ui.selectedThema;
							if(adm_cd.length == 5){
								$ecnmyMap.ui.checkIsAtdrc(adm_cd.substring(0,4)+"0");
								if($ecnmyMap.ui.isAtdrc){
									...
									$ecnmyDash.event.allChange($totSurvMain.ui.selectedArea, "2");
									//20201022 박은식 임업,어업 분기처리 추가 END
									//$ecnmyDash.event.pathChange("atdrc", adm_cd.substring(0,4)+"0"); // 2020-11-30 [곽제욱] 선택시 navigation 해제 추가
									$ecnmyDash.event.pathChange("atdrc", adm_cd.substring(0,2));
								} else {
									$totSurvMain.ui.selectedArea = adm_cd.substring(0,2);
										$ecnmyDash.event.allChange($totSurvMain.ui.selectedArea, "2");
									//$ecnmyDash.event.pathChange("sgg", adm_cd.substring(0,2)); // 2020-11-30 [곽제욱] 선택시 navigation 해제 추가
									$ecnmyDash.event.pathChange("sgg", adm_cd.substring(0,4)+"0");
								}
							} else if(adm_cd.length == 2){
								sidoCd = "99";
								$totSurvMain.ui.selectedLevel = "0"
								$totSurvMain.ui.selectedArea = "00";
								$ecnmyDash.event.allChange("00", "2");
							...
							let chartDt = $("#areaEcnmyY" + $totSurvMain.ui.selectedYear).highcharts().xAxis[0].series[0].data; // 타일(트리)맵 차트 데이터
							if($ecnmyDash.selectedItmId == "0") {
								for(var i=0; i<chartDt.length; i++) {
									for(var j=0; j<$totSurvMain.ui.tilePerColor.length; j++){
										let fillColor = $totSurvMain.ui.tilePerColor[j].color;
										if($totSurvMain.ui.tilePerColor[j].adm_cd == chartDt[i].id) {
											chartDt[i].update({ color: fillColor});
											chartDt[i].select(false);
										}
									}
								}
							}
			...
			didDrawCreate : function(event, type, map) {
				...
				if (map.curPolygonCode == "1") {
					...
					$ecnmyMapApi.request.userAreaBoundInfo(area, type, map.curPolygonCode, event, map);	
				}
