				...
				if(p_map_type == "color" || p_map_type == "bubble") {
					...
					if(p_map_region == "sido") {
						...
						$totSurvMap.ui.setTotSurvData($totSurvMap.ui.map, "sido", "color", "", "", "",  lv_surv_id, function(p_list) {
							p_list = p_list.filter((f)=>(f.adm_cd=='00'||Number(f.adm_cd)>10));
							//리스트에서 unit 가져오기
							var lv_unit = "개";
							var lv_unit_nm = "수";
...
//							//params.region = "sgg";
			...
			setTotSurvData : function(p_map, p_region, p_type, p_sido_cd, p_sgg_cd, p_emdong_cd, p_surv_id, p_callback) {
				...
				//파라
				var l2=""; var l3="";
				if($totSurvMap.ui.selectedItmCd=='T100'){
					$totSurvMap.ui.selectedItmCd='T00';
				}
				if(p_surv_id == 'DT_1IN1503'){l2="000"; l3="";}
				if(p_surv_id == 'DT_1PB1607'){ l2="0"; l3="000"; }
				if(p_surv_id == 'DT_1IN0001_05'){l2="00"; l3="";}
				if(p_surv_id == 'DT_1JD1505'){l2="10,11,12,13,14,15"; l3="";}
				...
				surv_year_list: $totSurvMain.ui.selectedYear							// 수록시점
					, org_id_list: "101"							// 조직번호
						...
						, char_itm_id_list: $totSurvMap.ui.selectedItmCd						// 표특성항목
						, adm_cd: ""								// 지역코드
						, adm_unit: ""								// 지역단위
						, ov_l1_list: lv_adm_cd								// 항목 1
						, ov_l2_list: l2								// 항목 2							// 항목 2
						, ov_l3_list: l3								// 항목 3
						, ov_l4_list: ""								// 항목 4
						, ov_l5_list: ""								// 항목 5
			...
			$.ajax({
				...
				for(var i =0; i<res.length; i++){
					...
					if($totSurvMain.ui.selectedNm){
						if($totSurvMain.ui.selectedNm==res[i].OV_L2_KOR)
							tmpdata.push(tmp);
					}else 
						tmpdata.push(tmp);
				}
				...
				if(res[0].adm_cd == "00") {
					$totSurvMap.ui.mapTotalVal = parseFloat(res[0].dt);
				} else {
					for(var i=0; i<res.length; i++) {
						if(res[i].region_nm != "읍부" && res[i].region_nm != "동부" && res[i].region_nm != "면부") {
							if($totSurvMain.ui.selectedArea.length == 2) {
								$totSurvMap.ui.checkIsAtdrc(res[i].adm_cd);
								if(!$totSurvMap.ui.isAtdrc){
									$totSurvMap.ui.mapTotalVal += parseFloat(res[i].dt);
								}
							} else if($totSurvMain.ui.selectedArea.length == 5) {
								if($totSurvMain.ui.selectedArea.substring(0,4) == res[i].adm_cd.substring(0,4)) {
									if(res[i].adm_cd.substring(4,5) != "0") {
										$totSurvMap.ui.checkIsAtdrc(res[i].adm_cd);
										if(!$totSurvMap.ui.isAtdrc){
											$totSurvMap.ui.mapTotalVal += parseFloat(res[i].dt);
										}
