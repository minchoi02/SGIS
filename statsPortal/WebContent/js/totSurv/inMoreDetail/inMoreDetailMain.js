...
(function(W, D) {
	...
	$(document).ready(function() {
		...
		} else if(gv_url == "ecnmy" || gv_url == "ecnmyDashLoc") { // 2021-08-04 [이영호] 경제총조사 추가 
			$totSurvMain.ui.changeMenu(7);
		...
		} else if(gv_url == "ecnmy" || gv_url == "ecnmyDashLoc"){ // 2021-08-11 [이영호] 경제총조사
			$totSurvMain.ui.changeMenu(7);
		} else if(gv_url == "inMoreDetail"){ // 2021-08-11 [이영호] 경제총조사
			$totSurvMain.ui.changeMenu(8);
		}
	});
...
//			$totSurvMain.ui.ready();
		...
		changeMenu : function (pIndex, pCallback) {
			...
			if ($totSurvMain.ui.loadByPage) {
				...
				if(pIndex=="0"){
					...
					id="fisheryMenu";
				} else if(pIndex=="7"){ // 2021-08-04 [이영호] 경제총조사 추가
					id="ecnmyMenu";
				} 
				$("#"+id).addClass("on");
			...
			if($totSurvMain.ui.selectedArea != null && $totSurvMain.ui.selectedArea.length == 5){
				if($inMoreDetailMap.ui.mapToggleId != null && $inMoreDetailMap.ui.mapToggleId.length == 5){
			...
			if(menuChangeFlag){
				...
				case 1: // 중메뉴: 인구총조사
					if ($totSurvMain.ui.prevIndex == pIndex) { // 주제 변경이 없을경우(년도변화만 존재할경우)
						if($totSurvMain.ui.yearChangeYn == "Y"){
							...
							$inMoreDetailMap.ui.selectedItmCd = "T100";
							$inMoreDetailMap.ui.selectedC1 = "";
							$inMoreDetailMap.ui.selectedC2 = "";
							$totSurvMain.ui.chartSaveClear(); // 2020-10-15 [곽제욱] 선택된 차트항목 초기화
							$inMoreDetailMap.ui.selectedSurvId = "PH0001";
						...
						// 년도변경이 아닌경우
							...
							$inMoreDetailMap.ui.selectedObj[0] = ""; // 2020-11-19 [곽제욱] 선택한 object 초기화
				...
				case 2: // 중메뉴: 가구총조사
					...
					/** 2020-10-20 [곽제욱] 가구총조사 changeMenu 수정 START */
						if($totSurvMain.ui.yearChangeYn == "Y"){
							$populationDash.ui.clear(); // 인구대쉬보드 차트 삭제
							$inMoreDetailMap.ui.selectedItmCd = "T200";
							$inMoreDetailMap.ui.selectedC1 = "";
							$inMoreDetailMap.ui.selectedC2 = "";
				...
				case 4: // 중메뉴: 농업총조사
					if ($totSurvMain.ui.prevIndex == pIndex) { // 주제 변경이 없을경우(년도변화만 존재할경우)
						if($totSurvMain.ui.yearChangeYn == "Y"){
							...
							$inMoreDetailMap.ui.selectedItmCd = "T00"; // 2020-11-19 [곽제욱] 농업 아이템코드 가구원->가구로 변경
							$inMoreDetailMap.ui.selectedC1 = "000";
							$inMoreDetailMap.ui.selectedC2 = "";
							$totSurvMain.ui.chartSaveClear(); // 2020-10-15 [곽제욱] 선택된 차트항목 초기화
							$inMoreDetailMap.ui.selectedSurvId = "";
				...
				case 6: // 중메뉴: 어업총조사
					...
					if ($totSurvMain.ui.prevIndex == pIndex) { // 주제 변경이 없을경우(년도변화만 존재할경우)
						if($totSurvMain.ui.yearChangeYn == "Y"){
							...
							$inMoreDetailMap.ui.selectedItmCd = "T00"; //20201110 박은식 어업 인구에서 가구로 변경
							$inMoreDetailMap.ui.selectedC1 = "000";
							$inMoreDetailMap.ui.selectedC2 = "";
							$totSurvMain.ui.chartSaveClear(); // 2020-10-15 [곽제욱] 선택된 차트항목 초기화
							$inMoreDetailMap.ui.selectedSurvId = "";
				...
				case 12: // 중메뉴: 인구 시계열 // 2020-10-07 [곽제욱] 시계열 index 변경
					if ($totSurvMain.ui.prevIndex == pIndex) { // 주제 변경이 없을경우(년도변화만 존재할경우)
						...
						$inMoreDetailMap.ui.chartToggleYn = "N" // 2020-12-07 [곽제욱] 선택된 차트항목 초기화
					...
					isHeaderCommBtn(true);  // 2020.10.23[주형식] 헤더 공통버튼 show/hide 추가
				case 7: // 중메뉴: 경제총조사 // 2021-08-04 [이영호] 
					if ($totSurvMain.ui.prevIndex == pIndex) { // 주제 변경이 없을경우(년도변화만 존재할경우)
						if($totSurvMain.ui.yearChangeYn == "Y") {
							$ecnmyDash.ui.clear(); // 경제대쉬보드 차트 삭제
							$inMoreDetailMap.ui.selectedItmCd = "T10";
							if($totSurvMain.ui.selectedYear == "2015") {
								$inMoreDetailMap.ui.selectedTblId = "DT_1KI1510";
							} else {
								$inMoreDetailMap.ui.selectedTblId = "DT_1KI2002";
							}
							$totSurvMain.ui.chartSaveClear(); // 2021-08-11 [이영호] 선택된 차트항목 초기화
							$inMoreDetailMap.ui.selectedSurvId = "";
							$ecnmyDash.event.setDefaultParams($totSurvMain.ui.selectedYear);
							$ecnmyDash.ui.getRankSet($totSurvMain.ui.selectedArea, "", "00");
							...
							$totSurvMain.ui.tileChangeYn="Y" // 2021-08-11 [이영호] 타일맵변경여부 Y
							$totSurvMain.ui.selectedThema = '경제'; // 주제 세팅
							$ecnmyDash.ui.init(); // 경제대쉬보드 초기화
						}
					} else {
						$inMoreDetailMap.ui.chartToggleYn = "N" // 2021-08-04 [이영호] 선택된 차트항목 초기화
						$totSurvMain.ui.chartSaveClear(); // 2021-08-04 [이영호] 선택된 차트항목 초기화
						$totSurvTmsMap.ui.mapToggleId = ""; // 2021-08-04 [이영호] 시계열 진입시 mapToggleId 초기화
						$totSurvMain.ui.tmsPathChange("nationwide", "");
						$totSurvMain.ui.removeMap();
						$totSurvMain.ui.removeContent; // 주제 변경이 있을경우 divContent 영역 전체 초기화
						$totSurvMain.ui.selectedThema = '경제'; // 주제 세팅
				...
				case 8: // 중메뉴: 더보기 조회
					if ($totSurvMain.ui.prevIndex == pIndex) {
						/** 2020.10.28[한광희] 상세페이지 화면에서 상세페이지 메뉴 클릭시 초기화 하도록 수정 START */
						$totSurvMain.ui.selectedThema = '더보기';
						$totSurvMain.ui.selectedArea = '';
						$inMoreDetail.ui.pageReadyYnFlag = "Y";
						$("#sideMenuArea").show();
						$("#inMoreDetailDataDiv").css("width","calc(100% - 500px)");
						$(".sideMenuToggleBtn").removeClass("on");
						/** 2020.10.28[한광희] 상세페이지 화면에서 상세페이지 메뉴 클릭시 초기화 하도록 수정 END */
					} else {
						$totSurvMain.ui.selectedThema = '더보기';
						$totSurvMain.ui.selectedArea = '';
						$inMoreDetail.ui.init();
						//$inMoreDetail.ui.ready();
						$("#sideMenuArea").show();

						srvLogWrite('P0','09','06','01',$totSurvMain.ui.selectedThema);
					} 
					$inMoreDetail.ui.viewTypeFalg = true;	// 2020.10.28[한광희] 상세페이지 첫진입/좌축메뉴 초기화 구분값
					isHeaderCommBtn(false);  // 2020.10.23[주형식] 헤더 공통버튼 show/hide 추가
					break;
		...
		ChangeMenuCallback: function(pIndex) {
			...
			if($totSurvMain.ui.yearChangeYn=="N") {
				switch (pIndex) {
				case 0: if ($inMoreDetail.ui.pageReadyYnFlag == 'Y') { $inMoreDetail.ui.ready(); } break;
			...
			case 12: //중메뉴 : 인구시계열
				...
				break;
			// 2021-08-11 [이영호] 경제총조사 추가
			case 7: //중메뉴 : 경제총조사 대시보드
				if ($totSurvMain.ui.isSetUIEventArr[pIndex] == false) {
					$ecnmyDash.event.setUIEvent();
				}
				break;
			default:
				return;
			...
			if($inMoreDetailMap.ui.mapToggleId!=""){
				$inMoreDetailMap.ui.tileTempColor = $("rect[value='"+$inMoreDetailMap.ui.mapToggleId+"']").attr("fill");
				$("rect[value='"+$inMoreDetailMap.ui.mapToggleId+"']").attr("fill", "#0086c6"); // 2020-11-02 [곽제욱] 타일차트 색변경 추가
			}
...
//			}
		...
		titleChange : function(adm_cd){
			...
			if($inMoreDetailMap.ui.mapToggleId != adm_cd) {
				...
				// 2020-10-27 [곽제욱] title 문구변경 로직 추가 START
					...
					title = $("#dash_sgg option[value='"+$inMoreDetailMap.ui.mapToggleId.substring(2,5)+"']").html();
		...
		refresh : function(region, lv_adm_cd){
			...
			if(region == "sgg"){
				...
				/** 2021-09-01 [이영호] 경제 추가 START */
				else if(thema == "경제"){
					$ecnmyDash.ui.rankSlideInit();//20200908 박은식 슬라이드 초기화
				}
				/** 2021-09-01 [이영호] 경제 추가 END */
				...
				if(upperRegCd != ''){ //비자치구의 상위 행정시 이동 지역코드 담는 부분
					...
					//20201022 박은식 임업 분기 추기 END
					//2021-09-01 [이영호] 경제  START
					else if(thema == "경제"){
						$ecnmyDash.upperBack = true;
					}
					//2021-09-01 [이영호] 경제  END
				...
				if(upperRegCd != ''){ //비자치구의 상위 행정시 이동 지역코드 담는 부분
					...
					//20201022 박은식 임업 분기 추기 START
						...
						$forestryDash.upperBack = true;
					//20201022 박은식 임업 분기 추기 END					
					} else if(thema == "경제"){ //2021-09-01 이영호 경제 추기 START
						$ecnmyDash.upperBack = true;					
					}	//2021-09-01 이영호 경제 추기 END
					admCd = upperRegCd; // 읍면동에서 삭제버튼 클릭시 시도 조회를 위하여 지역코드에 시도데이터 세팅
				} else {
					$("#dash_sido").val(lv_adm_cd.substring(0,2));
					admCd = lv_adm_cd; // 읍면동에서 삭제버튼 클릭시 시도 조회를 위하여 지역코드에 시도데이터 세팅
			...
			//20201022 박은식 임업 분기 추기 END
			}	//2021-09-01 [이영호] 경총 추가 START			
			else if(thema == "경제"){				
				//20201019 박은식 주택 추가 START				
				$ecnmyDash.event.allClear(); //2020-11-11 [박은식] 임업 surv_id, itm_cd, c1, c2값 초기화 진행
				$ecnmyDash.event.allChange($totSurvMain.ui.selectedArea, "1");
				$ecnmyDash.ui.getRankSet("", "", $totSurvMain.ui.selectedArea);
				if(upperRegCd.length == 2) {
					$ecnmyDash.event.pathChange("sido", $totSurvMain.ui.selectedArea);
				} else if(upperRegCd.length == 5 && upperRegCd.substring(4,5) == "0") {
					$ecnmyDash.event.pathChange("sgg", $totSurvMain.ui.selectedArea);
				} else if(upperRegCd.length == 5 && upperRegCd.substring(4,5) != "0") {
					$ecnmyDash.event.pathChange("atdrc", $totSurvMain.ui.selectedArea);
				}
			} //2021-09-01 [이영호] 경총 추가 END
		},
		 ...
		 getSidoSggPos : function(region_cd){
			...
			if(region_cd.length == 2){
				...
				if(sido != undefined && sgg != null){
					...
					if($inMoreDetailMap.ui.mapToggleId == "" && $totSurvMain.ui.selectedArea.length ==5 && ($totSurvMain.ui.selectedArea != region_cd)){
		...
		chartSelectedSave : function(target, data, color, index, resizeYn, title){
			...
			//20201102 박은식 특수한 차트 처리를 위한 분기처리 START
				...
				//20201126 박은식 차트 유지 로직 추가 END
					...
					$inMoreDetailMap.ui.selectedObj[0] = $("#"+target).find(".eventGroup"+data.idx).find("rect").eq(index)[0];
		...
		chartSaveClear : function(){
    		...
    		$inMoreDetailMap.ui.selectedObj = ""; // 2020-11-12 [곽제욱] 선택항목 초기화 수정
    		$inMoreDetailMap.ui.selectedItmCd = $inMoreDetailMap.ui.selectedItmCd;
    		$inMoreDetailMap.ui.selectedSurvId = $totSurvMain.ui.selectedSurvId; // 2020-10-20 [박은식] survId 추가
    		//20201126 박은식 초기화 값 변경 START
    		$inMoreDetailMap.ui.selectedC1 = "";
    		$inMoreDetailMap.ui.selectedC2 = "";
    		$inMoreDetailMap.ui.selectedC3 = "";
			...
			if ( (navigator.appName == 'Netscape' && navigator.userAgent.search('Trident') != -1) || (agent.indexOf("msie") != -1) ){
				...
				commonTotSurvTotSurv_confirm("해당 지도 이미지를 저장하시겠습니까?",  
						function okFn(opt){
							...
							if($inMoreDetailMap.ui.selectedObj != ""){
...
//				});
				...
				body.on("click", ".reportPrintBtn", function(){		// 2020.10.29 버튼 변경
					$totSurvMain.ui.isDashPopup = window.open("/js/totSurv/report/inMoreDetail/totSurvDashPrint.html", "dashPrint","width=1600, height=850, scrollbars=yes");
				});
...
//					console.log("########## kakao linkUrl = " + linkUrl);
			...
			setTileMapChart : function(target, width, height, surv_id){
				...
				if($totSurvMain.ui.selectedLevel != "3" || $inMoreDetailMap.ui.isAtdrc){
					...
					if(regionCd==null || regionCd==undefined){
						regionCd = '99';
					} else if(regionCd.length==2 && $inMoreDetailMap.ui.mapToggleId!=null && $inMoreDetailMap.ui.mapToggleId!=""){
						regionCd = '00';
					} else if(regionCd.length==5){
						// 행정시도 인지 비교
						$inMoreDetailMap.ui.checkIsAtdrc(regionCd);
						if($inMoreDetailMap.ui.isAtdrc){
							// 행정시도 이면서 지도 선택인 경우는 상위지역(시도레벨) 조회(타일맵 변경 없음)  
							if($inMoreDetailMap.ui.mapToggleId!=null && $inMoreDetailMap.ui.mapToggleId!="" && gv_type != "locgov" && gv_type!="totFarmLocgov" && gv_type!="totPeopleLocgov"){ // 2020-11-09 [곽제욱] parameter 분기처리 추가
							...
							// 행정시도가 아닐경우 상위지역 체크
							$inMoreDetailMap.ui.checkIsAtdrc(regionCd.substring(0,4)+"0");
					...
					// 선택한 itmCd
					var itm_cd = $inMoreDetailMap.ui.selectedItmCd;
					...
					if(target == "areaFishery" ){
						var c1 = $inMoreDetailMap.ui.selectedC1;
					...
					if(target == "areaFarm" ){
						...
						$inMoreDetailMap.ui.selectedC1 = c1;
						$inMoreDetailMap.ui.selectedItmCd = itm_cd;
					}
					...
					$.ajax({
						...
						data: { year: year, region_cd : regionCd, surv_id : surv_id, itm_cd : itm_cd, isAtdrc:$inMoreDetailMap.ui.isAtdrc, c1:c1}, // 
						dataType: "json",
						success: function(res) {
							if (res.errCd == "0") {
								...
								svg
								    ...
								    .attr('fill', function(d) {
								    	for(var i=0; i<$totSurvMain.ui.tilePerColor.length; i++){
								    		if(d.data.region_cd == $totSurvMain.ui.tilePerColor[i].adm_cd){
								    			return $totSurvMain.ui.tilePerColor[i].color; 
								    		}
								    	}
								    })
								    .on("click", function(){ // click 이벤트
									    //20210222 박은식 총조사인구 차트선택 이후 타일맵 클릭 시 초기화 START
								    	if($inMoreDetailMap.ui.selectedObj != undefined && 
								    	   $inMoreDetailMap.ui.selectedObj != null &&
								    	   $inMoreDetailMap.ui.selectedObj != "" &&
								    	   $inMoreDetailMap.ui.selectedObj.attr("id") == "totalPopulation") {
								    	...
								    	$inMoreDetailMap.ui.map.setPolyLayerHighlight(""); // 2020-10-12 [곽제욱] 하이라이트 초기화
								    	$inMoreDetailMap.ui.mapToggleId = ""; // 맵토글 ID 초기화
								    	...
								    	if($totSurvMain.ui.selectedThema == "어업"){
								    		if($totSurvMain.ui.selectedYear == "2015"){
												if($fisheryDash.fisheryType =="sea"){
													$inMoreDetailMap.ui.selectedSurvId = "FS0112";
												} else {
													$inMoreDetailMap.ui.selectedSurvId = "FS0171";
												}
																
											} else if($totSurvMain.ui.selectedYear == "2010"){
												if($fisheryDash.fisheryType =="sea"){
													$inMoreDetailMap.ui.selectedSurvId = "FS0413";
												} else {
													$inMoreDetailMap.ui.selectedSurvId = "FS0469";
												}
											}
								    	}
								    	...
								    	if(region_cd.length == 2){
								    		...
								    		if($totSurvMain.ui.selectedThema == "인구"){
								    			...
								    			$inMoreDetailMap.ui.selectedItmCd = "T310";
									    		$inMoreDetailMap.ui.selectedSurvId = "PH0001";
								    			...
								    			$inMoreDetailMap.ui.selectedC1 = "000";
								    			//$inMoreDetailMap.ui.selectedItmCd = "T01";
								    			$inMoreDetailMap.ui.selectedItmCd = "T00";    // 2020.11.10 주형식 가구수로 변경
									    		if($totSurvMain.ui.selectedYear == "2015"){
								    				$inMoreDetailMap.ui.selectedSurvId = "FS0013";								    				
								    			} else if($totSurvMain.ui.selectedYear == "2010"){
								    				$inMoreDetailMap.ui.selectedSurvId = "FS0315";
								    			}
								    		...
								    		// 2020-10-13 임업대쉬보드 추가 START jhs
								    			...
								    			$inMoreDetailMap.ui.selectedC1 = "000";
								    			$inMoreDetailMap.ui.selectedItmCd = "T00"; // 2020-11-17 [곽제욱] 임업 itm_cd 수정 T01->T00
									    		if($totSurvMain.ui.selectedYear == "2015"){
								    				$inMoreDetailMap.ui.selectedSurvId = "FS0235";								    				
								    			} else if($totSurvMain.ui.selectedYear == "2010"){
								    				$inMoreDetailMap.ui.selectedSurvId = "FS0532";
								    			}
								    		...
								    		// 2020-10-13 임업대쉬보드 추가 END jhs
								    			...
								    			$inMoreDetailMap.ui.selectedC1 = "000";
								    			$inMoreDetailMap.ui.selectedItmCd = "T00"; //20201110 박은식 어업 itm_cd 변경
//								    			/** 2020-10-26 [곽제욱] 해수면/내수면 분기로 인한 주석처리 START */
								    	...
								    	// 시군구 데이터 일 경우 kosis정보 호출
								    		...
								    		// 선택한 지역 하이라이트
								    		//$inMoreDetailMap.ui.map.setPolyLayerHighlight(region_cd);
								    		// 인구.... 
								    		// 비자치 구인지 체크한다.
								    		$inMoreDetailMap.ui.checkIsAtdrc(region_cd);	
								    		if($inMoreDetailMap.ui.isAtdrc != true){
								    			// 2020-11-12 [곽제욱] 최하레벨 tileMap 선택시 색상변경 로직 START
								    			if($inMoreDetailMap.ui.mapToggleId != $totSurvMain.ui.selectedArea){
								    				for(var i=0; i<$totSurvMain.ui.tilePerColor.length; i++){
								    				...
								    				var tempColor = '';
								    				for(var i=0;i < $totSurvMain.ui.tilePerColor.length; i++){
								    					if($totSurvMain.ui.tilePerColor[i].adm_cd == $inMoreDetailMap.ui.mapToggleId){
								    						tempColor = $totSurvMain.ui.tilePerColor[i].color
								    					}
								    				}
								    				//var tempColor = $totSurvMain.ui.tilePerColor.find(function(x) {return x.adm_cd == $inMoreDetailMap.ui.mapToggleId}).color;
								    				//20201118 박은식 IE에서 find() 미지원으로 변경 처리 END
								    				$("rect[value='"+$inMoreDetailMap.ui.mapToggleId+"']").attr("fill", tempColor);
								    			}
								    			...
								    			if($totSurvMain.ui.selectedThema == "인구"){
								    				...
								    				//$inMoreDetailMapApi.request.openApiSearchPopulation(region_cd); // 2020-10-15 [곽제욱] 공간정보서비스과 요청으로 읍면동 조회 주석처리 
								    				...
								    				if($inMoreDetailMap.ui.mapToggleId != $totSurvMain.ui.selectedArea) {
														// 선택한 지역 하이라이트
											    		$inMoreDetailMap.ui.map.setPolyLayerHighlight($totSurvMain.ui.selectedArea);
											    		$inMoreDetailMap.ui.mapToggleId = $totSurvMain.ui.selectedArea;
													} else {
														$inMoreDetailMap.ui.map.setPolyLayerHighlight($totSurvMain.ui.selectedArea);
														$inMoreDetailMap.ui.mapToggleId = "";
													}  
													$inMoreDetailMap.ui.checkIsAtdrc($totSurvMain.ui.selectedArea.substring(0,4)+"0");
...
//								    				alert(">>>>>> 어업.... 비자치구..........");
								    		...
								    		//$inMoreDetailMap.ui.map.setPolyLayerHighlight($totSurvMain.ui.selectedArea); // 2020-10-06 [곽제욱] 타일차트 클릭시 하이라이트 설정 제거
								    		if($totSurvMain.ui.selectedThema == "인구"){
								    			$populationDash.ui.getRankSet("", "",region_cd)								    			
								    		}
								    		//20201019 박은식 주택 추가 START
								    ...
								    .on("mouseover", function(d) { // 마우스 over 이벤트 
								        ...
								        var unit = "";
								        if($totSurvMain.ui.selectedThema == "인구"){ //20201022 어업, 농업 조건문 추가 //20201111 박은식 농업 어업 명->가구 변경
								        	unit = "명"
								        } else if($totSurvMain.ui.selectedThema == "주택"){
								        	unit = "호"// 20201028 박은식 단위변경
								        } else if($totSurvMain.ui.selectedThema == "가구" || $totSurvMain.ui.selectedThema == "임업" || $totSurvMain.ui.selectedThema == "어업" || $totSurvMain.ui.selectedThema == "농업"){	// 2020.11.09[한광희] 임업 가구로 단위 변경  //20201111 박은식 농업 어업 명->가구 변경
								        	unit = "가구"
								        }
								    ...
								    .on("mouseout", function(d) { // 마우스 out 이벤트
								    	...
								    	if(d3.select(this).attr("fill")!="#0086c6"){
									    	d3.select(this)
									    	.attr("fill", $totSurvMain.ui.tempColor) // 2020-09-04 [신예리] 타일맵 마우스 out 이벤트 컬러 변경
...
function showTuto(){
	...
	}else if($totSurvMain.ui.selectedThema == '더보기'){
		$('.totSurvDetailTuto').removeClass("none");
	}else if($totSurvMain.ui.selectedThema == '경제'){
		$('.ecnmyDashTuto').removeClass("none");
	}
}
