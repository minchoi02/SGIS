...
W.$myDataModify = W.$myDataModify || {};
	...
	$myDataModify = {
			...
			dispPop : function(idx){
				...
				if( idx == "1"){
					srvLogWrite('A0', '11', '03', '07', '', '집계');
				}else if( idx == "2"){
					srvLogWrite('A0', '11', '03', '07', '', '연산');					
				}
			...
			updateMyData : function(usr_data,meta_data){
				...
				if( $("#ratioWeightSel").val() != "" && $("#ratioWeightSel").val() != "undefined" ){
					updateMyData.addParam("ratiofield", $("#ratioWeightSel").val());
				}
				...
				if($myDataModify.rdOption && $myDataModify.rdOption.length == 0){
			...
			setGioCalc : function(){
				...
				$myDataModify.orgnVisualSetting = "colorFull";
			...
			ptype : function( val ){
				...
				//라디오 체크, 라벨 on
				$("input[name=rd_ptype][value='"+ val +"']").attr("checked","checked");
				$("input[name=rd_ptype][value='"+ val +"']").next('label').addClass('on');
				
				$myDataModify.dataVisualSetting = val;
				
				if( $("#rd_ptype2").attr("checked") != 'checked' && $("#rd_ptype5").attr("checked") != 'checked' ){
					$("#ratioWeightDiv").css("display","none").hide();
				} else {
					$("#weightTxt").text( ( $myDataModify.dataVisualSetting == 'pbubble' ? "버블지도" : "열지도" ) );
			...
			popMapGisSetting : function(){
				if($myDataModify.fileType == "kml"){
					...
					$myDataModify.map.gMap.whenReady(function() {
						...
						$myDataModify.ptype( $myDataModify.orgnVisualSetting );
						...
						if($myDataModify.rowDataArray.length >= 1){
							...
							if($myDataModify.dataVisualSetting=="location"){
								...
								var ratioField = $("#ratioWeightSel").val(); //c
								...
								if( ratioField != "" && typeof ratioField != 'undefined' ){
									...
									for( var i=0; i<$myDataModify.rowDataArray.length; i++ ){
										var num = Number( $myDataModify.rowDataArray[i].USR_DATA[ ratioNum-2 ][ changeNumber( ratioNum ) ] );
										if( isNaN( num ) ){
											num = 0;
										} else {
											if( maxVal < num ){
									...
									if( maxVal > 0 ){
										$myDataModify.map.zoomLevelHeat = false;
										$myDataModify.map.setHeatMapOptions( 20, 30, maxVal );
									}
									
								} else {
									$myDataModify.map.zoomLevelHeat = true;
									$myDataModify.map.setHeatMapOptions( 20, 30, 1 );
								}
								
								if($myDataModify.rowDataArray.length >=1){
									for(var i=0;i<$myDataModify.rowDataArray.length;i++){
										if( $myDataModify.rowDataArray[i].GEO_X && $myDataModify.rowDataArray[i].GEO_Y ){
											var num = 1000;
											if( ratioField != "" && typeof ratioField != 'undefined' ){
												num = Number( $myDataModify.rowDataArray[i].USR_DATA[ ratioNum-2 ][ changeNumber( ratioNum ) ] );
												if( isNaN( num ) ){
													num = 0;
												}
											}
											
											$myDataModify.map.addHeatMap(
													$myDataModify.rowDataArray[i].GEO_X,
													$myDataModify.rowDataArray[i].GEO_Y, 
													num );
										}
									}
								}
								
							} else if( $myDataModify.dataVisualSetting == "pbubble" ){
								var resultList = new Array();
								var resultArray = new Array();
								
								$myDataModify.map.selectedBoundList = [];
								$myDataModify.tot_type = "4";
								
								for( var i=0; i<$myDataModify.hot.countRows()-1; i++ ){
									var resultRow = new Array();
									
									var admCd = $myDataModify.hot.getDataAtCell(i+1,$myDataModify.hot.countCols()-2);
									var data = Number( $myDataModify.hot.getDataAtCell(i+1, changeWord( $("#ratioWeightSel").val() ) ) );
									
									var geo_x =  $myDataModify.hot.getDataAtCell(i+1, $myDataModify.hot.countCols()-4 );
									var geo_y =  $myDataModify.hot.getDataAtCell(i+1, $myDataModify.hot.countCols()-3 );
			...
			popMapMenuSetting : function(){
				...
				var dispHtml = "";
				var ttHtml = "";
				var selHtml = '<option value="">선택안함</option>';
				//표출데이터 설정
				...
				$.each($myDataModify.rowHeaderArray,function(key,value){					
					...
					var selected = ( $myDataModify.ratiofield == (i+1) ? 'selected="selected"' : '' );
					var checked = ( $myDataModify.dispData[i-1] == true ? 'checked="checked"' : '' );
					var checked2 = ( $myDataModify.tooltipSetting[i-1] == true ? 'checked="checked"' : '' );

					dispHtml +=	'<input type="radio" value="'+value.COL_NM+'" name="rd_pselect" id="rd_pselect'+i+'" '+checked+'/>';
					dispHtml +='<label for="rd_pselect'+i+'" class="'+( checked != "" ? "on" : "" )+'"><a href="javascript:$myDataModify.pselect('+i+')">'+value.COL_NM+'</a></label>';
					ttHtml +=	'<input type="checkbox" value="'+value.COL_NM+'" name="rd_pck" id="rd_pck'+i+'" '+checked2+'/>';
					ttHtml +='<label for="rd_pck'+i+'" class="'+( checked2 != "" ? "on" : "" )+'" onclick=""><a >'+value.COL_NM+'</a></label>';
					selHtml += '<option value="'+changeNumber(i+1)+'" '+selected+'>'+value.COL_NM+'</option>';
				...
				var dvs = $myDataModify.dataVisualSetting;
				
				if($myDataModify.gioCalc != 1 | $myDataModify.wasGioCalc == true){
					html += '<input type="radio" value="colorFull" name="rd_ptype" id="rd_ptype3" checked="'+( dvs == "colorFull" ? "checked" : "" )+'" data-idx="3">';
					html += '<label for="rd_ptype3" class="'+( dvs == "colorFull" ? "on" : "" )+'" ><a >색상지도</a></label>';
					html += '<input type="radio" value="bubble" name="rd_ptype" id="rd_ptype4" checked="'+( dvs == "bubble" ? "checked" : "" )+'" data-idx="4">';
					html += '<label for="rd_ptype4" class="'+( dvs == "bubble" ? "on" : "" )+'"><a >버블지도</a></label>';
				}else{
					html += '<input type="radio" value="location" name="rd_ptype" id="rd_ptype1" checked="'+( dvs == "location" ? "checked" : "" )+'" data-idx="1">';
					html += '<label for="rd_ptype1" class="'+( dvs == "location" ? "on" : "" )+'" ><a >위치표시</a></label>';
					html += '<input type="radio" value="ratio" name="rd_ptype" id="rd_ptype2" checked="'+( dvs == "ratio" ? "checked" : "" )+'" data-idx="2">';
					html += '<label for="rd_ptype2" class="'+( dvs == "ratio" ? "on" : "" )+'"><a >열지도</a></label>';
					html += '<input type="radio" value="pbubble" name="rd_ptype" id="rd_ptype5" checked="'+( dvs == "pbubble" ? "checked" : "" )+'" data-idx="5">';
					html += '<label for="rd_ptype5" class="'+( dvs == "pbubble" ? "on" : "" )+'"><a >버블지도</a></label>';
				}
			...
			createMap : function(id, seq) {
				...
				$myDataModify.map.legend.legendColor = ["#ffd75d", "#f6b64e", "#ee953f", "#e67430", "#dd5321", "#d53212", "#cd1103"];
				...
				$myDataModify.map.legend.drawBubbleMap = function(geojson){
		    		...
		    		geojson.eachLayer(function(layer) {
			    		...
			    		if (layer.feature.info.length > 0) {
			    			var forlen = ( ( $myDataModify.wasGioCalc == true || $myDataModify.gioCalc != 1 ) ? 1 : layer.feature.info.length );
			    			
				    		for( var k=0; k<forlen; k++ ){
				    			
				    			if( !( $myDataModify.wasGioCalc == true || $myDataModify.gioCalc != 1 ) ){
				    			...
				    			for(var i = 0; i<$myDataModify.rowDataArray.length;i++){
			    					...
			    					if( $myDataModify.dataVisualSetting == 'pbubble' ){
			    						if( ( $myDataModify.rowDataArray[i].GEO_X == layer.feature.info[k].geo_x ) 
			    								&& ( $myDataModify.rowDataArray[i].GEO_Y == layer.feature.info[k].geo_y ) ){
			    							for(var j = 0; j<checkList.length; j++){
			    								sMessage +="<tr style='font-size:12px;padding-left:5px;'><td>";
			    								sMessage +=$myDataModify.hot.getDataAtCell(0,checkList[j]+2) + ":";
			    								sMessage +=$myDataModify.hot.getDataAtCell(i+1,checkList[j]+2);
			    								sMessage +="</td></tr>";
			    							}
			    						}
			    					} else {
				...
				$myDataModify.map.setLayerColor = function (feature, layer) {
					var that = $myDataModify.map;
			...
			reMakeMap : function(){
				...
				if($myDataModify.dataVisualSetting =="location"){
					...
					if( ratioField != "" && typeof ratioField != 'undefined' ){
						...
						if( maxVal > 0 ){
							$myDataModify.map.zoomLevelHeat = false;
							$myDataModify.map.setHeatMapOptions( $myDataModify.map.heatRadius, $myDataModify.map.heatBlur, maxVal );
						}
			...
			makeUsrData : function(myData){
				...
				for(var i = 0; i<myData.length; i ++){
					...
					for(var j = 0; j < myData[i].USR_DATA.length; j++){
						...
						if( $myDataModify.dataVisualSetting == "colorFull" || $myDataModify.dataVisualSetting == "bubble" ){
							rowData[$myDataModify.hot.getColHeader(j+1)] = value;
						} else {
			...
			saveConfirm : function( newSave ){
				if( newSave && $("#newSubject").val() == "" ){
			...
			saveMyData : function(){
				if($myDataModify.fileType == "kml"){
					...
					if( $myDataModify.gioCalc == 1 && !$myDataModify.wasGioCalc ){
						$myDataModify.saveConfirm();
					}else{
						$("#gioPop3").show();
						$("#newSubject").focus();
					}
			...
			removeRow : function(){
				srvLogWrite('A0', '11', '03', '07', '', '삭제(행)');
				$myDataModify.myData.pop();
			},
			createCol : function(){
				srvLogWrite('A0', '11', '03', '07', '', '추가(열)');
			...
			gioCalcMakeRowData : function(sumArray,sumType){
				...
				for(var key in sumArray){
					if(key !="isEmpty"){
						...
						if( $myDataModify.isDecimal( sumArray[key] ) ){
							rowArray.push( Number( sumArray[key].toFixed(2) ));
						} else {
							rowArray.push( Number( sumArray[key] ));
						}
	        	...
	        	if(res.errCd == "0") {
	        		...
	        		var extName = str.split(".");
			  		...
			  		if( res.result && res.result.length > 0 ){
	        			for( var i=0; i<res.result[0].metaData.length; i++ ){
	        				if( typeof res.result[0].metaData[i].COL_NM == 'undefined' ){
	        					res.result[0].metaData[i].COL_NM = 'noname';
	        				}
	        			}
	        		}
	        		...
	        		if(extName[extName.length-1] == "kml"){
						...
						$myDataModify.orgnVisualSetting = mainData.MAP_DISP_TYPE;
		        		...
		        		if( mainData.MAP_DISP_TYPE == "ratio" || mainData.MAP_DISP_TYPE == "pbubble" ){
		        			...
		        			if( mainData.RATIOFIELD ){ //>>A,B,C...
		        				ratiofield = changeWord( mainData.RATIOFIELD.toLowerCase() );
		        			}
		        			
		        			$myDataModify.ratiofield = ratiofield;
		        		}
