 ...
 */
(function(W, D) {
	W.$tsMapFrame = W.$tsMapFrame || {};
	
	$(document).ready(function() {// used
		$workRoad.ui.log("$tsMapFrame.ready - begin");

		$tsMapFrame.ui.bndYearSelectbox();		//조사년도 설정
		$tsMapFrame.ui.commonDataList();		//공공데이터, 나의데이터 목록
		$tsMapFrame.event.setUIEvent();	//UI에 사용되는 이벤트를 설정한다.
		...
		if(parseInt($("#mainIndex_year").val()) == 2016){
			$("#mainIndex_box2").hide();
			$tsMapFrame.ui.companyClassView();
		}
		...
		$.cssHooks.backgroundColor = {
			    get: function(elem) {
					$workRoad.ui.addFnc01List("$tsMapFrame.[$.cssHooks.backgroundColor.get]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
		...
		$.cssHooks.color = {
			    get: function(elem) {
					$workRoad.ui.addFnc01List("$tsMapFrame.[$.cssHooks.color.get]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			        if (elem.currentStyle)
		...
		$("[name='mainIndex_radio']").on('click',(function(e){
			$workRoad.ui.addFnc01List("$tsMapFrame.event.[[name='mainIndex_radio']]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도 						
			...
			$tsMapFrame.ui.subConditionNoneAll();
	...
	$tsMapFrame.ui = {
			...
			subConditionNoneAll : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.subConditionNoneAll");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			addSearchBtn : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.addSearchBtn");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도				
				//조회버튼은 최대 10개만 가능
				...
				if(this.btnLimitCnt()) {
					...
					//일반 버튼일 경우 파라미터 유효성 검사					
						...
						this.searchbtnCnt++;
						$tsMapApi.request.openApiStatBaseYearProcess();	//통계별 최신년도 가져오기
						$tsMapFrame.event.stepCloseAnimate(1, "check");	
			...
			btnLimitCnt : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.btnLimitCnt");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			 ...
			 */
			getBtnCnt : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.getBtnCnt");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var cnt = $("#searchBtnResultRgn").find("li:visible").length;
			},
			...
			createSearchBtn : function(curSelectedStatsType, atdrc_yn) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.createSearchBtn");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				var html = "<li class='dragItem' id='dragItem_"+this.searchbtnCnt+"'>" +
								...
								"</a>" +
								"<a href='javascript:$tsMapFrame.ui.deleteSearchBtn("+this.searchbtnCnt+");' class='sqdel'><img src='/img/um/btn_closel01.png' alt='삭제' /></a>" +
						   "</li>";
				...
				"</a>"+
				"<a href='javascript:$tsMapFrame.ui.deleteSearchBtn(0);' class='sqdel'><img src='/img/um/btn_closel01.png' alt='삭제' class='mCS_img_loaded'></a>"+
				"</li>";*/
...
//				});
				...
				$(".dragItem").draggable({ 
					...
					helper : function() {	//드래그시 폰트 색상 변경
						...
						//아직 조회가 안된 버튼은 검정글씨
						if($tsMapFrame.ui.curSearchBtnArray["one"] != btnId &&
							$tsMapFrame.ui.curSearchBtnArray["two"] != btnId &&
							$tsMapFrame.ui.curSearchBtnArray["three"] != btnId) {
			...
			searchModeSetting : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.searchModeSetting");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				
				console.log("[tsMapFrame.js] searchModeSetting 호출");

				//mng_s
				if($tsMapFrame.ui.isInnerMapShow2!=undefined && $tsMapFrame.ui.isInnerMapShow2) {
				...
				$(".dragItem").off("dblclick").dblclick(function(event) {
					$workRoad.ui.addFnc01List("$tsMapFrame.ui.searchModeSetting.[.dragItem.dblclick]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					var id = $("#"+event.currentTarget.id).find("a").attr("id");
					var index = id.split("-")[1];
					var tmpParam = "";
					for(var i = 0; i < $tsMapFrame.ui.arParamList.length; i ++) {
						if($tsMapFrame.ui.arParamList[i].idx == index) {
							tmpParam = $tsMapFrame.ui.arParamList[i];
						}
					}
					// 더블클릭 시, 콜백 호출
					$tsMap.callbackFunc.didMapDoubleClick(id, tmpParam);
				});
			...
			updateSearchBtnEffect : function(btn_id, map_id) {// used
				$workRoad.ui.log("$tsMapFrame.ui.updateSearchBtnEffect - begin");
				...
				if(this.curSearchBtnArray["one"] != "") {
					$tsMapFrame.event.dragAnimate(this.curSearchBtnArray["one"], this.mapColor[0]);
				}
				if(this.curSearchBtnArray["two"] != "") {
					$tsMapFrame.event.dragAnimate(this.curSearchBtnArray["two"], this.mapColor[1]);
				}
				if(this.curSearchBtnArray["three"] != "") {
					$tsMapFrame.event.dragAnimate(this.curSearchBtnArray["three"], this.mapColor[2]);
				}
				
				//분할 화면에 같은 버튼을 조회했을 경우
				if(this.curSearchBtnArray["one"] != "" && (this.curSearchBtnArray["one"] == this.curSearchBtnArray["two"])) {
					$tsMapFrame.event.dragAnimate(this.curSearchBtnArray["one"], "#394955");
				}
				if(this.curSearchBtnArray["two"] != "" && (this.curSearchBtnArray["two"] == this.curSearchBtnArray["three"])) {
					$tsMapFrame.event.dragAnimate(this.curSearchBtnArray["two"], "#394955");
				}
				if(this.curSearchBtnArray["three"] != "" && (this.curSearchBtnArray["three"] == this.curSearchBtnArray["one"])) {
					$tsMapFrame.event.dragAnimate(this.curSearchBtnArray["three"], "#394955");
				}
			},
			...
			setParams : function(curSelectedStatsType, api_id) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.setParams");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				console.log("[$tsMapFrame.js] setParams 조건버튼으로 만들어진 통계정보에 대한 파라미터정보를 설정");
				...
				// 전국사업체조사
					...
					if (api_id == "API_0304-b") {
						...
						if($("#company_TreeBox").is(":visible")) {
							// 산업체유형 선택
							if ($tsMapFrame.ui.curSelectedCompanyNode != null && $tsMapFrame.ui.curSelectedCompanyNode.cd.length > 0) {
								tmpArParams.push({
									key : "class_code",
									value : $tsMapFrame.ui.curSelectedCompanyNode.cd
								});
								tmpArParamName.push($tsMapFrame.ui.curSelectedCompanyNode.text);
							}
						}
						...
						*/
						...
						if(($tsMapFrame.ui.curSelectedCompanyNode == null || $tsMapFrame.ui.curSelectedCompanyNode.cd == "" )&& !$("#company_SearchBox").is(":visible")) {
			...
			setRevertParams : function(data, type) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.setRevertParams");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			setDetailStatsPanel : function(type) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.setDetailStatsPanel");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				console.log("[tsMapFrame.js] setDetailStatsPanel 호출 : "+type);
...
//				$(".sideQuick.sq02").stop().animate({"left":"560px"},200);
				...
				switch(menuType[type]) {
					...
					case 3:
						...
						$tsMap.ui.companyTree = null;
						$tsMapFrame.ui.curSelectedCompanyNode = null;
						$tsMapApi.request.openApiInterstryCode(0, class_deg, null);
						break;
...
//						$("#buttonMakeBtn").html("세부 조건창 열기");
						...
						$tsMapFrame.event.kosisTreeWidth();
						//9월 서비스
					...
					case 7:
						...
						break;
					case 8:
						$tsMapFrame.ui.companyTree = null;
						inx = 3;
			...
			populationTab : function(type) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.populationTab");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var tabType = {
					"population" : 0,				//인구조건
					"household"  : 1,				//가구조건
					"house" 	 : 2,					//주택조건
			...
			tripleFTab : function(type) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.tripleFTab");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var tabType = {
					"farm" : 0,				//농가
					"forest"  : 1,				//임가
					"fish" 	 : 2			//어가
			...
			companyTab : function(type) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.companyTab");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				if(tabType[type] == 0) {		//산업분류
					...
					if($tsMapFrame.ui.isInnerMapShow2!=undefined && $tsMapFrame.ui.isInnerMapShow2) {
						...
						$("#buttonMakeBtn").show();
						$("#menuAutoClose2Lev").show();
					} else if($tsMapFrame.ui.isInnerMapShow3!=undefined && $tsMapFrame.ui.isInnerMapShow3) {
						//버튼생성 보이기
						//기존 산업분류에서는 3뎁스에서 버튼이 존재하고 그리드에서는 2뎁스에 존재해야하므로 여기다 세팅한다.
						$("#buttonMakeBtn").text("사업체조사 버튼생성");
						$("#buttonMakeBtn").show();
			...
			companyClassView : function(type) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.companyClassView");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			companySearch : function(pageNum) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.companySearch");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				if($("#companySearchText").val() == "") {
					...
					var sopPortalCorpClassSearchObj = new sop.portal.ts.corpClassSearch.api.ts();
			...
			companyPaging : function(totalCount, currentIndex) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.companyPaging");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var pageSize = 5;
				...
				$('#corpClassPaging .pages').paging({
					...
					onclick:function(e,page){
						$tsMapFrame.ui.corpClassNum = page-1;
						$tsMapFrame.ui.companySearch(page-1);
					}
				});
			},
			...
			companyTreeShow : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.companyTreeShow");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			deleteSearchBtn : function(value) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.deleteSearchBtn");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			bndYearSelectbox : function() {// used
				$workRoad.ui.log("$tsMapFrame.ui.bndYearSelectbox - begin");
...
//					$("#company_year_theme").append("<option value="+year+">"+year+"년도</option>");
			...
			btnValidationCheck : function(curSelectedStatsType, api_id) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.btnValidationCheck");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				//총조사 주요지표
				if(curSelectedStatsType == "mainIndex") {
					var str = "";
...
//					}
			...
			commonDataList : function() {// used
				$workRoad.ui.log("$tsMapFrame.ui.commonDataList - begin");
				
				// 임시 - 2018.10.31	ywKim	주석
//				$publicDataBoard.ui.leftMenuList($tsMap.ui);		//공공데이터 목록
...
//				$publicDataBoard.ui.delegateSetting($tsMap.ui);			//공공데이터 세팅
			 ...
			 */
			doMaxSize : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.doMaxSize");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				$tsMap.ui.doMaxSize();
			},
			 ...
			 */
			doAddMap : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.doAddMap");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				$tsMap.ui.doAddMap();
			},
			...
			doCombineMap : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.doCombineMap");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var mapId = parseInt($tsMap.ui.curMapId) + 1;
				$tsMap.ui.doCombineMap(mapId);
			},
			 ...
			 */
			doBookMark : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.doBookMark");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var mapId = parseInt($tsMap.ui.curMapId) + 1;
				$tsMap.ui.doBookMark(mapId);
			},
			 ...
			 */
			doShare : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.doShare");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var mapId = parseInt($tsMap.ui.curMapId) + 1;
				$tsMap.ui.doShare(mapId);
			},
			 ...
			 */
			doReport : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.doReport");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var mapId = parseInt($tsMap.ui.curMapId) + 1;
				$tsMap.ui.reportDataSet(mapId);
			},
			 ...
			 */
			doHelp : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.doHelp");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			updateToolTip : function(id, msg) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.updateToolTip");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					$("#"+id).tooltip({
				    content: msg
				});
			},
			...
			showNumberClick : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.showNumberClick");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			showNumberSetting : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.showNumberSetting");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var map_id = $tsMap.ui.curMapId;
				//9월 서비스
				if ($tsMap.ui.mapList[map_id] == null) {
					return;
				}
				var legend = $tsMap.ui.mapList[map_id].legend;
				//통계표출 on일 경우
			...
			checkFusionItem : function(e, type) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.checkFusionItem");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			checkMustItem : function(e, el) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.checkMustItem");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				//체크된 라이오 버튼이 하나밖에 없을 때, 같은 라이오버튼을 누를때 항상 체크되도록 변경
			...
			sqlListBoxPosition : function(left) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.sqlListBoxPosition");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			sqlListBoxPosition2 : function(left) {
				$workRoad.ui.addFnc01List("$tsMapFrame.ui.sqlListBoxPosition2");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				if($("#menuAutoClose_radio").attr("checked") != "checked") {
					$tsMapFrame.ui.sqlListBoxPosition(left);
					$tsMapFrame.ui.sqlListBoxLeft = left;
				}else{
					$tsMapFrame.ui.sqlListBoxPosition("0px");
					$tsMapFrame.ui.sqlListBoxLeft = "0px";
				}
			}
	};
	...
	$tsMapFrame.event = {
			...
			setUIEvent : function() {// used
				$workRoad.ui.log("$tsMapFrame.event.setUIEvent - begin");
...
//				this.slideValue("houseBdspaceFrom_combine", "houseBdspaceTo_combine", "#slider-range3_combine", "㎡", 9); //2016.09.08 9월 추가분 
		        ...
		        body.on("click", ".themul > li", function() {
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[1]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//						$(".sideQuick.sq03").addClass("on");
////					}else{ 
////						$tsMapFrame.event.stepCloseAnimate(1, "pass"); 
////						$(this).find("span").show();
...
//				
				...
				body.on("click",".sideQuick.sq03", function(){// used
					$workRoad.ui.log('$tsMapFrame.event.setUIEvent.["click",".sideQuick.sq03"] - begin');
					...
					if(!on){
						...
						if (isDrag) {
							...
							$(".sqListBox.sq03").show();
							$(this).next(".sqListBox").stop().animate({"left":$tsMapFrame.ui.sqlListBoxLeft},200);
						}
					}else{
						$(this).removeClass("on");
						if (isDrag) {
...
//						idx = 3;
//					}
//					$tsMapFrame.event.stepCloseAnimate(idx, "pass"); 
...
//			    }); 
				...
				body.on("click","a.roundTextBox",function(e){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[3]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					var ck = $(this).hasClass("on"); 
					if(!ck){
						$(this).addClass("on");
					...
					if(fusionCk) {
						$tsMapFrame.ui.checkFusionItem(e, "tab");
					}
			    });
				...
				body.on("click","div.roundTextBox",function(e){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[4]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					var ck = $(this).hasClass("on"); 
					if(!ck){
						$(this).addClass("on");
						$(this).children("input").prop("checked", true);
				...
				body.on("click","a.subRoundTextBox",function(){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[5]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				body.on("click",".mainIndex_stepBox label",function(){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[6]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				body.on("click",".publicData_stepBox label",function(){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[7]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//					$publicDataBoard.ui.updatePublicData(type);
				...
				body.on("click",".radioStepBox label",function(e){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[8]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					//2016.08.23 권차욱 9월 서비스
					if ($(this).prev().attr("name") == "rd_combine_base") {
						if (!$tsMapFrame.ui.checkFusionItem(e)) {
							return;
						}
						...
						if (!$tsMapFrame.ui.checkMustItem(e, el)) {
							return;
						}
					}
					var ck = $(this).hasClass("on");
...
//					}
				...
				body.on("click",".radioStepOneBox label",function(){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[9]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					var ck = $(this).hasClass("on");
				...
				body.on("click",".radioStepOneBox2 label",function(){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[10]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					var ck = $(this).hasClass("on");
					$(this).parent().parent().find("label").removeClass("on");
					$(this).parent().parent().find("input").removeAttr("checked");
					if(!ck){
...
//					console.log("tsMapFrame.js multiCheckBox clicked.");
//					//mng_s
//					if($tsMapFrame.ui.isInnerMapShow2!=undefined && $tsMapFrame.ui.isInnerMapShow2) {
//						//주택조건 탭이고 그리드일 경우 단일 선택한다. 가구조건에서는 다중선택임
//						if( $tsMapFrame.ui.curSelectedDetailStatsType == "API_0306" ) {
//							$(this).parent().parent().find("label").removeClass("on");
//							$(this).parent().parent().find("input").removeAttr("checked");
//							$(this).addClass("on");
//							$(this).prev().attr("checked", "checked");
...
//			    });
				...
				body.on("click","#bottomService",function(){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[11]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					var ck = $(this).hasClass("on");
					if(!ck){
						$(this).addClass("on");
				...
				body.on("click",".hangjungArea .resizeIcon",function(){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[12]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				body.on("click",".subRadioStepBox label",function(){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[13]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				$('#companySearchText').keydown(function(e){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[14]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					if (e.keyCode == 13){	// when press ENTER key, accept the inputed value.
						$tsMapFrame.ui.companySearch(0);
					}
				});
...
//				});
				$(".sqListBox > .sqTabs").dblclick(function() {
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[15]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				$("#mainIndex_year").change(function() {
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[16]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				$("#population_year").change(function() {
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[17]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					if (parseInt(this.value) > 2010) {
						...
						//mng_s
						if( $tsMapFrame.ui.isInnerMapShow2!=undefined && $tsMapFrame.ui.isInnerMapShow2 ) { //mng_s 그리드일 경우
							//교육정도별 및 혼인정도별 메뉴는 하이드 상태그래도 유지한다.
							$("#populationEduTab").hide();
							$("#populationMarryTab").hide();
						} else if( $tsMapFrame.ui.isInnerMapShow3!=undefined && $tsMapFrame.ui.isInnerMapShow3 ) { //mng_s 그리드일 경우
							//교육정도별 및 혼인정도별 메뉴는 하이드 상태그래도 유지한다.
							$("#populationEduTab").hide();
							$("#populationMarryTab").hide();
						} else {
				...
				$("#household_year").change(function() {
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[18]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					if (parseInt(this.value) > 2010) {
				...
				$("#house_year").change(function() {
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[19]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					if (parseInt(this.value) > 2010) {
						$("#householdOcptnTab").hide();
				...
				$("#population_year_combine").change(function() {
					$workRoad.ui.addFnc01List("$tsMapFrame.event.setUIEvent.[20]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					if (parseInt(this.value) > 2010) {
...
//			},
			...
			slideValue : function(from, to, slider, etc, limit) {// used
				$workRoad.ui.log("$tsMapFrame.event.slideValue - begin");
			    ...
			    $(domFrom).change(function(){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.slideValue.[1]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			    ...
			    $(domTo).change(function(){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.slideValue.[2]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			    ...
			    $(domTo).click(function(){
					$workRoad.ui.addFnc01List("$tsMapFrame.event.slideValue.[3]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			    ...
			    $(domSlider).slider({
			        ...
			        //9월서비스 20160729 권차욱 수정
			        start : function(e, ui) {
						$workRoad.ui.addFnc01List("$tsMapFrame.event.slideValue.[4]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			        	if (from == "populationAgeFrom" 		|| 
				        	from == "populationAgeFrom_combine" || 
				        	from == "3fAgeFrom"         		|| 
			        ...
			        stop : function(e, ui) {
						$workRoad.ui.addFnc01List("$tsMapFrame.event.slideValue.[5]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			        	if (from == "populationAgeFrom" || from == "populationAgeFrom_combine" || from == "3fAgeFrom") {
			...
			dragAppend : function(selector, bgColor) {
				$workRoad.ui.addFnc01List("$tsMapFrame.event.dragAppend");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				$(selector).css("background-color", bgColor);
				$(selector).addClass("M_on");
			},
			dragAnimate : function(btn_id, bgColor) {
				$workRoad.ui.addFnc01List("$tsMapFrame.event.dragAnimate");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var selector = $("#"+btn_id);
			...
			companyListTreeWidth : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.event.companyListTreeWidth");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			companyThemeTreeWidth : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.event.companyThemeTreeWidth");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			kosisTreeWidth : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.event.kosisTreeWidth");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			outputAreaSearch : function() {
				$workRoad.ui.addFnc01List("$tsMapFrame.event.outputAreaSearch");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				if($tsMapFrame.event.marker != null){
					$tsMapFrame.event.marker.remove();
					$tsMapFrame.event.marker = null;
				}
...
//			 			},500);
						...
						setTimeout(function(){
							map.geojson.eachLayer(function(layer) {
								if(layer.feature.properties.adm_cd == tot_reg_cd){
									$tsMapFrame.event.marker = sop.marker([ result.x_coor, result.y_coor ]);		
									$tsMapFrame.event.marker.addTo(map.gMap);
									...
									toolTip +=		"</table>";
									$tsMapFrame.event.marker.bindToolTip(toolTip,{
										minWidth:200,
										maxWidth:200,
										maxHeight:500
									});
	...
	if ($workRoad.ui.isSetCallback('tsMapFrame')) return;
	
	/** ********* 산업분류 검색 목록 Start ********* */
	$class("sop.portal.ts.corpClassSearch.api.ts").extend(sop.portal.absAPI).define({
		onSuccess : function (status, res, options) {
			...
			if(res.errCd == "0") {
	    		...
	    		$tsMapFrame.ui.companyPaging(result.totalcount, $tsMapFrame.ui.corpClassNum);
			} else {
				//총 개수
