...
//					$("#viewTimeSeriesData_dt_area a").removeClass("on");
				...
				if ((res.id == "API_0301" || res.id == "4011" || res.pAdmCd == "00") || this.map.boundLevel != "1") { //9월 서비스
					...
					$("#saDataBoard #saStats").closest('dl').hide();
					$("#fullDataBoardTitle").find("#saDataBoard #saStats").show();	//2016.03.18 수정
				}else {
					...
					$("#saDataBoard #saStats").closest('dl').hide();
					$("#fullDataBoardTitle").find("#saDataBoard #saStats").hide();
				}
			},
			...
			updateDataBoardMulti : function(res, options) {
				...
				$("#saDataBoard #saTitle").html(options.params.title);	// 제목
				$("#fullDataBoardTitle").find("#saDataBoard #saTitle").html("제목 : " + options.params.title);
...
//				var year = "";
				var title = "";
				var regDtMonth = "";	// 2020.04.23[한광희] 월평균자료 조회 기능 추가
				for(var i = 0; i < options.params.param.length; i ++) {
					if(options.params.param[i].key == "today") {
						today = options.params.param[i].value;
					}
					//2019-01-15 선택항목 및 지도에 년도 나오는거 날짜로 표시되게 변경.
					else if(options.params.param[i].key == "term") {
						term = options.params.param[i].value;
...
//					}
					...
					else if(options.params.param[i].key == "regDtMonth") {
						regDtMonth = options.params.param[i].value;
					}
				...
				if (today.length > 0) {
					...
					if (termName.length > 0) {
						/** 2020.04.23[한광희] 월평균자료 조회 기능 추가 START */
						if(regDtMonth.length > 0 && regDtMonth != "999999") {
							/** 2021.07.07[hjh] 월평균자료 조회 기능 추가 START */
							title = "월별 " + options.params.title + " (" + $workRoad.util.dateWithSign(regDtMonth, ".") + ") (" + termName + ") ";
							/** 2021.07.07[hjh] 월평균자료 조회 기능 추가 END */
						} else {
							title = options.params.title + " (" + $workRoad.util.dateWithSign(today, ".") + ") (" + termName + ") ";
						}
						/** 2020.04.23[한광희] 월평균자료 조회 기능 추가 END */
					} else {
						/** 2020.04.23[한광희] 월평균자료 조회 기능 추가 START */
						if(regDtMonth.length > 0 && regDtMonth != "999999") {
							/** 2021.07.07[hjh] 월평균자료 조회 기능 추가 START */
							title = "월별 " + options.params.title + " (" + $workRoad.util.dateWithSign(regDtMonth, ".") + ")";
							/** 2021.07.07[hjh] 월평균자료 조회 기능 추가 END */
						} else {
							title = options.params.title + " (" + $workRoad.util.dateWithSign(today, ".") + ")";
						}
						/** 2020.04.23[한광희] 월평균자료 조회 기능 추가 END */
					}
//				if (year.length > 0) {
//					title = options.params.title + " ("+year+")";
				...
				var adm_nm = ($wrmTodayStatus.ui.adm_nm.length > 0) ? $wrmTodayStatus.ui.adm_nm : "전국";	// 2018.11.30	ywKim	추가: 데이터보드 정보 테이블의 지역 데이터 관련

				$(".dataBoardNotice").hide();
				$("#saDataBoard #saTitle").html(title);						// 제목
				$("#saDataBoard #saOrigin").html(options.params.origin);	// 출처
				$("#saDataBoard #saArea").html(adm_nm);						// 지역
//				$("#saDataBoard #saArea").html(options.params.adm_nm);		// 지역 / ※ options.params.adm_nm : 지도 중심위치
//				$("#saDataBoard #saStats").html("통계 :");													//통계는 Pie차트를 통해서 입력
				$("#fullDataBoardTitle").find("#saDataBoard #saTitle").html("제목 : " + title);
				$("#fullDataBoardTitle").find("#saDataBoard #saArea").html("지역 : " + options.params.adm_nm);
//				$("#fullDataBoardTitle").find("#saDataBoard #saStats").html("통계 :");						//통계는 Pie차트를 통해서 입력
				...
				if (origin != undefined && origin.length != null) {
					$("#saDataBoard #saOrigin").show();
					$("#saDataBoard #saOrigin").html(origin);	// 출처
					$("#fullDataBoardTitle").find("#saDataBoard #saOrigin").show();
					$("#fullDataBoardTitle").find("#saDataBoard #saOrigin").html("출처 : " + origin);
				}
			...
			updateInfoKosis : function(title, value) {
				$("#fullDataBoardTitle").find("#saDataBoard #saTitle").html("제목 : " + title);
				$("#saDataBoard #saTitle").html(title);	// 제목
				$("#saDataBoard #saOrigin").html("");
				$("#saDataBoard #saArea").html("");
				$("#saDataBoard #saSum").html("");
				$("#fullPieChart").hide();
				$(".fullPieLegend").hide();
				$("#fullTableTimeSeries").hide();
				$("#fullDataBoardTitle").find("#saDataBoard #saOrigin").show();
				$("#fullDataBoardTitle").find("#saDataBoard #saArea").hide();
				$("#fullDataBoardTitle").find("#saDataBoard #saOrigin").html("출처 : KOSIS(<a href='javascript:saMapEtc.kosisLinked();'>통계표보기</a>)");
				$("#saDataBoard #saStats").html("출처 : KOSIS(<a href='javascript:saMapEtc.kosisLinked();'>통계표보기</a>)");
			},
			...
			updateTargetAreaChart : function(res, options) {
				...
				if(res.result != undefined) {
					for(var i = 0; i < res.result.length; i ++) {
						...
						xAxisCat.push(elem.adm_nm);
						//============================END================================//
						console.log("res.id / res.showData : " + res.id + " / " + res.showData);
						if(res.id == "API_0301") {	//인구통계총괄
							...
							titleText = "가구원 수";
							retDataList.push(parseFloat(elem.population));
						}else if(res.id == "113001") {	//구인현황 - 산업분류 조회 - return class define
							titleText = "업종별 구인현황";
							retDataList.push(parseFloat(elem.indust_class));	//구인현황추가 - 산업분류별
						/** 2021.07.07[hjh] 월평균자료 조회 기능 추가 START */
						}else if(res.id == "113009") {	//구인현황 - 산업분류 조회 - return class define
							titleText = "업종별 구인현황";
							retDataList.push(parseFloat(elem.indust_class));	//구인현황추가 - 산업분류별
						/** 2021.07.07[hjh] 월평균자료 조회 기능 추가 END */
						}else if(res.id == "113002") {	//구인현황 - 직종분류 조회 - return class define
							titleText = "직종별 구인현황";
							retDataList.push(parseFloat(elem.rcrit_jssfc));	//구인현황추가 - 직종분류
						}else if(res.id == "113003") {	//구인현황 - 기업형태 조회 - return class define
							titleText = "기업형태별 구인현황";
							retDataList.push(parseFloat(elem.entrprs_type));	//구인현황추가 - 직종분류
						}else if(res.id == "113004") {	//구인현황 - 고용형태 조회 - return class define
							titleText = "고용형태별 구인현황";
							retDataList.push(parseFloat(elem.emplym_type));	//구인현황추가 - 고용형태별
						}else if(res.id == "113005") {	//구인현황 - 임금형태 조회 - return class define
							titleText = "임금형태별 구인현황";
							retDataList.push(parseFloat(elem.wage_type));	//구인현황추가 - 임금형태
						}else if(res.id == "113006") {	//구인현황 - 학력 조회 - return class define
							titleText = "학력별 구인현황";
							retDataList.push(parseFloat(elem.acdmcr));	//구인현황추가 - 학력
						}else if(res.id == "113007") {	//구인현황 - 경력 조회 - return class define
							titleText = "경력별 구인현황";
							retDataList.push(parseFloat(elem.career));	//구인현황추가 - 경력
						//2019-05-29 [김남민] 구인현황 분석 > 업종별 직종별 등 분석조건을 AND 조건이 가능하게 개선(교차분석) START
						}else if(res.id == "113008") {	//구인현황 - 전체 조회 - return class define
							titleText = "구인현황";
							retDataList.push(parseFloat(elem.status_anls_all));	//구인현황추가 - 전체
						}
				...
				$("#saDataBoard #targetCharts").css("height", height+"px");
				...
				//해당지역 데이터 보기 차트 (일반)
				$('#saDataBoard #targetCharts').highcharts(
				{
					chart : {
											...
											map.dataGeojson.eachLayer(function(layer) {
												if (layer.feature) {
													...
													if (layer.feature.dataIdx == idx-1) {
														...
														$saMap.callbackFunc.didMouseOverPolygon(e, layer.feature, layer.options.type, map);
													}
												}
											});
										}
				...
				//9월 서비스
				$("#saDataBoard #targetCharts").find('.highcharts-axis-labels text').click(function(){
					//집계구
				...
				for (var i=0; i<res.length; i++) {
					...
					if (data.result != undefined && data.result.length > 0) {
						for (var k=0; k<data.result.length; k++) {
							...
							console.log("data.id / data.showData : " + data.id + " / " + data.showData);
...
//				$("#saDataBoard #wrapperChartScroll").css("height", (height+50)+"px");
				$("#saDataBoard #targetCharts").css("height", height+"px");
			...
			updateTargetAreaTable : function(res, options) {
				...
				var retDataList = [];	//수치 데이터
				$saDataBoard.ui.dataSumValue = 0;
				//mng_s 20180109 주용민
				if(res.result2 == undefined){
					res.result2 = [];
				}
				//2019-05-29 [김남민] 구인현황 분석 > 업종별 직종별 등 분석조건을 AND 조건이 가능하게 개선(교차분석) START
				$("#saDataBoard #saSum").html(""); //초기화
				//2019-05-29 [김남민] 구인현황 분석 > 업종별 직종별 등 분석조건을 AND 조건이 가능하게 개선(교차분석) END
				for(var i = 0; i < res.result.length + res.result2.length; i ++) {
					...
					if(res.id == "API_0301") {	//인구통계총괄
						if (res.showData == "tot_ppltn") {
							...
							$("#saDataBoard #saSum").html(appendCommaToNumber($saDataBoard.ui.dataSumValue));
						...
						$("#saDataBoard #saSum").html(appendCommaToNumber($saDataBoard.ui.dataSumValue));
						retDataList.push(parseFloat(elem.population));
					}else if(res.id == "113001") {	//구인현황 - 산업분류 조회
						this.dataBoardSum(elem.indust_class);	// 구인현황추가
						$("#saDataBoard #saSum").html(appendCommaToNumber($saDataBoard.ui.dataSumValue));
						retDataList.push(parseFloat(elem.indust_class));
						/** 2021.07.07[hjh] 월평균자료 조회 기능 추가 START */
					}else if(res.id == "113009") {	//구인현황 - 산업분류 조회 - return class define
						this.dataBoardSum(elem.indust_class);	// 구인현황추가
						$("#saDataBoard #saSum").html(appendCommaToNumber($saDataBoard.ui.dataSumValue));
					...
					/** 2021.07.07[hjh] 월평균자료 조회 기능 추가 END */
						...
						this.dataBoardSum(elem.rcrit_jssfc);	// 구인현황추가
						$("#saDataBoard #saSum").html(appendCommaToNumber($saDataBoard.ui.dataSumValue));
						retDataList.push(parseFloat(elem.rcrit_jssfc));
					}else if(res.id == "113003") {	//구인현황 - 기업형태 조회 - return class define
						this.dataBoardSum(elem.entrprs_type);	// 구인현황추가
						$("#saDataBoard #saSum").html(appendCommaToNumber($saDataBoard.ui.dataSumValue));
						retDataList.push(parseFloat(elem.entrprs_type));	//구인현황추가 - 기업형태
					}else if(res.id == "113004") {	//구인현황 - 고용형태 조회 - return class define
						this.dataBoardSum(elem.entrprs_type);	// 구인현황추가
						$("#saDataBoard #saSum").html(appendCommaToNumber($saDataBoard.ui.dataSumValue));
						retDataList.push(parseFloat(elem.emplym_type));	//구인현황추가 - 고용형태별
					}else if(res.id == "113005") {	//구인현황 - 임금형태 조회 - return class define
						this.dataBoardSum(elem.wage_type);	// 구인현황추가
						$("#saDataBoard #saSum").html(appendCommaToNumber($saDataBoard.ui.dataSumValue));
						retDataList.push(parseFloat(elem.wage_type));	//구인현황추가 - 임금형태
					}else if(res.id == "113006") {	//구인현황 - 학력 조회 - return class define
						this.dataBoardSum(elem.acdmcr);	// 구인현황추가
						$("#saDataBoard #saSum").html(appendCommaToNumber($saDataBoard.ui.dataSumValue));
						retDataList.push(parseFloat(elem.acdmcr));	//구인현황추가 - 학력
					}else if(res.id == "113007") {	//구인현황 - 경력 조회 - return class define
						this.dataBoardSum(elem.career);	// 구인현황추가
						$("#saDataBoard #saSum").html(appendCommaToNumber($saDataBoard.ui.dataSumValue));
						retDataList.push(parseFloat(elem.career));	//구인현황추가 - 경력
					//2019-05-29 [김남민] 구인현황 분석 > 업종별 직종별 등 분석조건을 AND 조건이 가능하게 개선(교차분석) START
					}else if(res.id == "113008") {	//구인현황 - 전체 조회 - return class define
						this.dataBoardSum(elem.status_anls_all);	// 구인현황추가
						$("#saDataBoard #saSum").html(appendCommaToNumber($saDataBoard.ui.dataSumValue));
						retDataList.push(parseFloat(elem.status_anls_all));	//구인현황추가 - 전체
					}
					//2019-05-29 [김남민] 구인현황 분석 > 업종별 직종별 등 분석조건을 AND 조건이 가능하게 개선(교차분석) END
				...
				//정렬
				var dataList = $saDataBoard.Util.tableDataSort(dataList);
			...
			dataBoardSum : function(val) {
				...
				$saDataBoard.ui.dataSumValue += parseFloat(val);
			},
					...
					if (data.result != undefined && data.result.length > 0) {
						for (var k=0; k<data.result.length; k++) {
							...
							if (data.id == "API_0301") {	//인구통계총괄
								...
								retDataList.push(parseFloat(elem.indust_class));	// 구인현황 추가
							/** 2021.07.07[hjh] 월평균자료 조회 기능 추가 START */
							}else if(res.id == "113009") {	//구인현황 - 산업분류 조회
								retDataList.push(parseFloat(elem.indust_class));	// 구인현황 추가
							/** 2021.07.07[hjh] 월평균자료 조회 기능 추가 END */
							}else if(res.id == "113002") {	//구인현황 - 직종분류 조회
								retDataList.push(parseFloat(elem.rcrit_jssfc));	// 구인현황 추가
							}else if(res.id == "113003") {	//구인현황 - 기업형태 조회 - return class define
								retDataList.push(parseFloat(elem.entrprs_type));	//구인현황추가 - 기업형태
							}else if(res.id == "113004") {	//구인현황 - 고용형태 조회 - return class define
								retDataList.push(parseFloat(elem.emplym_type));	//구인현황추가 - 고용형태별
			...
			timeSeriesInit : function(params) {
				...
				console.log("[saDataBoard.js] timeSeriesInit() 호출");

//				var dataBoard = this.mapData[this.map_id].dataBoard;
				var dataBoard = this.mapData[$saMap.ui.curMapId].dataBoard;
				var date = new Date();
...
//				}
			...
			updateTimeSeries : function(options) {
				...
				console.log("saDataBoard.js @@@@@@@@@@@@@@@@@@@@@@@@@@@@ maxYear1 : " + maxYear);
				...
				console.log("saDataBoard.js @@@@@@@@@@@@@@@@@@@@@@@@@@@@ maxYear2 : " + maxYear);
				//총조사 사업체일 경우 기본 최소년도는 2000년, 재작년도를 maxYear로 잡는다.
				...
				console.log("saDataBoard.js @@@@@@@@@@@@@@@@@@@@@@@@@@@@ maxYear3: " + maxYear);

				if(options.params.api_id == "API_0307"){
					maxYear= date.getFullYear()-2;
				}

				console.log("saDataBoard.js @@@@@@@@@@@@@@@@@@@@@@@@@@@@ maxYear4 : " + maxYear);
				...
				if (options.params.param != undefined && options.params.param.length > 0) {
					for (var i=0; i<options.params.param.length; i++) {
						if (options.params.param[i].key == "year") {
							...
							$saMap.ui.curDropParams.time_year = base_year;
				...
				console.log("saDataBoard.js @@@@@@@@@@@@@@@@@@@@@@@@@@@@ maxYear5 : " + maxYear);
				...
				for(var i = 0; i < dataBoard.yearArray.length; i ++) {
					$("#li_y"+dataBoard.yearArray[i]).on('click', function() {
						...
						if(!$(".btn_clockTypeSetting").hasClass("on")) {
							$saDataBoard.ui.selectTimeSeries(options, $(this).find("input"));
						}
					});
				}
...
//					this.delegate.shareUrlInfo = [];
					...
					for(var i = 0; i < options.params.param.length; i ++) {
						if(options.params.param[i].key == "year") {
							...
							$saMap.ui.curDropParams.time_year = $(yearObj).val();
						}
					}
					...
					$saMap.ui.requestOpenApi(options.params);
				}
			},
			...
			doReqTimeSeries : function() {
				...
				if (dataBoard.selectedYearArray.length > 0) {
					if (dataBoard.timeSeriesCnt == dataBoard.selectedYearArray.length) {
						$saMapApi.request.mask.close(); //9월 서비스
			...
			timerSeriesPlay : function() {
				//9월 서비스
				var dataBoard = $saDataBoard.ui.mapData[$saDataBoard.ui.map_id].dataBoard;
				var map = $saDataBoard.ui.map;
				...
				if (dataBoard.timeSeriesPushData[dataBoard.timeSeriesCnt].res.errCd == "0") {
					...
					//범례고정일 경우
					if ($saDataBoard.ui.isFixedLegend) {
						if (map.dataGeojson) {
							map.dataGeojson.remove();
							map.removeCaption();
							map.dataGeojson = null;
						}
						map.legend.valPerSlice = $saDataBoard.ui.fixedLegend;
					...
					$saMap.ui.curDropParams[map.id] = options.params;
					dataBoard.timeSeriesCnt++;
			...
			selectChartData : function(properties, index, map_id) {
				var charts = $("#saDataBoard #targetCharts").highcharts();

				if($("#saDataBoard #targetCharts").is(":visible")) {
...
//							$("#saDataBoard #wrapperChartScroll").mCustomScrollbar("scrollTo", $("#saDataBoard #targetCharts").find('.highcharts-axis-labels text').eq(scrollIdx));
...
//							charts.series[0].data[this.lastChatIndex].update({ color: '#2951f2' });
//							$("#saDataBoard #wrapperChartScroll").mCustomScrollbar("scrollTo", $("#saDataBoard #targetCharts").find('.highcharts-axis-labels text').eq(lastIndx-1));
						}
					}
				}
			},
...
//				dataBoard.timer = 0;
			...
			reset : function(map_id) {
				...
				$("#saDataBoard #saTitle").html("");		//제목
				$("#saDataBoard #saArea").html("");		//지역
				$("#saDataBoard #saStats").html("");		//통계
				$("#saDataBoard #targetCharts").empty();		//표 차트
				...
				$("#fullDataBoardTitle").find("#saDataBoard #saTitle").html("");
				$("#fullDataBoardTitle").find("#saDataBoard #saOrigin").html("");
				$("#fullDataBoardTitle").find("#saDataBoard #saArea").html("");
				$("#fullDataBoardTitle").find("#saDataBoard #saStats").html("");
	...
	$saDataBoard.event = {
			 ...
			 */
			setUIEvent : function() {
				console.log("$saDataBoard.event.setUIEvent() called.");
//				var body = $("body");
				...
				$("#dataSlider").slider({
			        ...
			        slide: function( event, ui ) {  //ui.value
			        	$("#saDataSideBox, #saDataBoardToggle").css("opacity", ui.value*0.1);
				    }
			    });
				$("#saDataSideBox, #saDataBoardToggle").css( "opacity", $("#dataSlider").slider( "value" ) );

				//닫기 버튼
				$workRoad.event.set("click","#saDataSideBox .bar>a",function(){
					$("#saDataSideBox").removeClass("full");
					$("#saDataSideBox").stop().animate({"right":"-1500px"},200);
					$("#saDataBoardToggle").removeClass("on").stop().animate({"right":"0"},200);
					$(".sop-right").stop().animate({"right":"0px"},200);//2019-06-12 박길섭
				});

				//4개 탭 열고 닫기
				$workRoad.event.set("click","#saDataBoard .dscList dt>a",function(){
				...
				//데이터보드 열고 닫기
				$workRoad.event.set("click","#saDataBoardToggle",function(){

					var ck = $(this).hasClass("on");
					if(!ck){
						//일반 데이터보드
						$(".sop-right").stop().animate({"right":"558px"},200);//2019-06-12 박길섭
						$("#saDataSideBox").stop().animate({"right":"0px"},200);
//						$("#saDataSideBox").stop().animate({"right":"50px"},200);
...
//						$(this).addClass("on").stop().animate({"right":"476px"},200);
						...
						// 2019.03.13 접근log 생성
						srvLogWrite('D0', '04', '06', '01', '', '');
					}else{
//						var full = $("#saDataSideBox").hasClass("full");
//						if(!full){
//							//KOSIS, 공공데이터, 나의데이터일 경우 확장 데이터보드를 열지 않는다.
//							if($saSubMenu.ui.curSelectedStatsType == "kosis"
...
//						}
				...
				//표보기, 차트보기
				$workRoad.event.set("click","#saDataBoard .typeBox>a",function(){
					...
					if(ck%2){
						...
						$('#saDataBoard #wrapperChartScroll').show();
						...
						$('#saDataBoard #wrapperChartScroll').hide();
						...
						// 2019.03.13 접근log 생성
						srvLogWrite('D0', '04', '06', '02', '', '');
					}
			    });
				//데이터보기 엑셀 다운로드
				$workRoad.event.set("click","#saDataBoard .btn_excelDownload", function(){
					...
					$(this).parents(".compareBox").eq(0).find(".tables").eq(0).find("div").find("table tr").each(function(){
					 ...
					 // 2019.03.13 접근log 생성
					 srvLogWrite('D0', '04', '06', '03', '', '');
				});

				//시계열 세팅 버튼
				$workRoad.event.set("click","#saDataBoard .btn_clockTypeSetting",function(){
					$(this).addClass("on");
					$(".yearList li").show();
					...
					if ($(".btn_clockTypeLegend").text() == "on") {
						$(".year_input").hide();
						$(".year_radio").show();
						if ($saDataBoard.ui.fixedLegendYear != null) {
							$(".year_radio").each(function() {
								if ($(this).val() == $saDataBoard.ui.fixedLegendYear) {
									$(this).prop("checked");
								}
							});
						}
				...
				//시계열 확인
				$workRoad.event.set("click","#saDataBoard .btn_clockTypeOk",function(){
					var dataBoard = $saDataBoard.ui.mapData[$saDataBoard.ui.map_id].dataBoard;
					...
					$(".yearList li").each(function(i){
						...
						if(!ck){
							...
							console.log("[saDataBoard.js] $(this).find(\"input\").val() [" + $(this).find("input").val());
						}
					});
				...
				//시계열 플레이/스탑 버튼
				$workRoad.event.set("click", "#saDataBoard .btn_clockTypePlay", function() {
					...
					if ( sop.isInnerMapShow2 || sop.isInnerMapShow3 ) { //그리드이면
						...
						if($(this).text() == "play") {
							...
							dataBoard.isTimeSeriesPlay = true;
							//9월 서비스
							$saDataBoard.ui.map.isDrop = true;
							$saDataBoard.ui.map.undoDropLayerBounds();
							$saDataBoard.ui.map.setFixedBoundLevel(true);
							...
							clearInterval(dataBoard.timer);
							$saDataBoard.ui.map.setFixedBoundLevel(false); //9월 서비스
						}
					}
				...
				//시계얄-범례고정 on/off
				$workRoad.event.set("click", "#saDataBoard .btn_clockTypeLegend", function() {
					if($(this).text() == "off") {
						$(this).text("on");
						...
						$(".year_radio").each(function() {
							...
							if (radio) {
								$saDataBoard.ui.fixedLegendYear = $(this).val();
							}
						})
						$saDataBoard.ui.isFixedLegend = true;
						...
						$(".year_input").show();
						$(".year_radio").hide();
						$saDataBoard.ui.isFixedLegend = false;
					}
				});

				//시계열-범례고정 버튼 선택
				$workRoad.event.set("click", "#saDataBoard .year_radio", function() {
					$saDataBoard.ui.fixedLegendYear = $(this).val();
				});


				//시계열 년도 div 클릭
				$workRoad.event.set("click", "#saDataBoard #tableTimeSeries div", function() {
...
//				$("#saDataBoard #wrapperChartScroll").mCustomScrollbar({axis:"xy",advanced: { autoExpandHorizontalScroll: true }});
				$("#saDataBoard #wrapperFullChartScroll").mCustomScrollbar({axis:"x",advanced: { autoExpandHorizontalScroll: true }});
			},
			...
			dataBoardOpen : function() {
				...
				if(!$("#saDataBoardToggle").hasClass("on")) {
					$("#saDataBoardToggle").click();
					$("#dataBoard").show();
				}
			}
	};
...
//					return b.value - a.value;
	...
	if ($workRoad.ui.isSetCallback('saDataBoard')) return;
	    ...
	    $class("sop.portal.pieChartDraw.api").extend(sop.portal.absAPI).define({
	        onSuccess : function(status, res, options) {
	            ...
	            if(res.errCd == "0") {
		        	...
		        	//통계
//		        	$("#saDataBoard #saStats").html("통계 : " + pieCount);
//		        	$("#fullDataBoardTitle").find("#saDataBoard #saStats").html("통계 : " + pieCount);
