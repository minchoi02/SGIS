...
(function (W, D) {
	...
	sMap = {
			...
			map : function() {
				...
				this.bounds = null;
				this.curPolygonCode = 2;	//20181011 손원웅_변경 전 값은 4
				this.dataPolygonCod = 4;	// 조회했던 경계레벨
				this.SidoComboVal = 0;		//20181108 손원웅_콤보박스 선택 값
				this.xComboVal = 0;		//20181108 손원웅_콤보박스 선택 값
				this.yComboVal = 0;		//20181108 손원웅_콤보박스 선택 값
				this.clearDataCnt = 0;	//20181123 손원웅 지도 경계 count
				this.data = [];
				this.curSidoCd = null;
				this.curSiggCd = null;
				this.curDongCd = null;
...
//				this.isZoomSliderControl = true;
				...
				this.dataPolygonFillOpacity = 0.8;					// 2018.12.27	ywKim	변경: 0.7 -> 0.8
				this.boundaryCallback = null; //9월 서비스
				this.atdrc_yn = null;//9월 서비스 자치구 유무(자치구 : 1)
				this.combineCallback = null;
				this.usingZoomLevelBySidoArea = true;				// 지도 표시할때 시도의 면적에 따라 ZoomLevel 다르게 적용할지 여부 - 2019.01.24	ywKim	추가

				// 맵을 생성한다.
				this.createMap = function (delegate, target, opt) {
					$workRoad.ui.log("sMap.map.createMap - begin");
				...
				this.createInfoControl = function() {
					$workRoad.ui.log("sMap.map.createInfoControl - begin");
				...
				this.infoControlShowHide = function(show) {
					$workRoad.ui.log("sMap.map.infoControlShowHide - begin");
				...
				this.createDrawControl = function(type) {
					$workRoad.ui.log("sMap.map.createDrawControl - begin");
				...
				this.update = function () {
					$workRoad.ui.log("sMap.map.update - begin");
					
					this.gMap._onResize();
				},
				//POI클릭시 현재 map Zoomlevel값 가지고온다
				this.poiZoomLevel = function(){
					$workRoad.ui.log("sMap.map.poiZoomLevel - begin");
				...
				this.mapMove = function (center, zoom, animate) {
					$workRoad.ui.log("sMap.map.mapMove - begin");
				...
				this.setZoom = function (zoom) {
					$workRoad.ui.log("sMap.map.setZoom - begin");
				...
				this.setZoomCalibrate = function(zoom, value) {
					$workRoad.ui.log("sMap.map.setZoomCalibrate - begin");
				...
				this.setFixedBoundLevel = function(bool) {
					$workRoad.ui.log("sMap.map.setFixedBoundLevel - begin");
				...
				this.setBoundSelectedMoode = function(mode) {
					$workRoad.ui.log("sMap.map.setBoundSelectedMoode - begin");
				...
				this.setBoundSelectedLayer = function() {
					$workRoad.ui.log("sMap.map.setBoundSelectedLayer - begin");
				...
				this.setBoundLevel = function(level) {
					$workRoad.ui.log("sMap.map.setBoundLevel - begin");
					
					this.boundLevel = level;
				};
				
				this.clearLayerStyle = function(layer) {
					$workRoad.ui.log("sMap.map.clearLayerStyle - begin");
				...
				this.contains = function (bounds, point) {
					$workRoad.ui.log("sMap.map.contains - begin");
				...
				this.addMarker = function (x, y, options) {
					$workRoad.ui.log("sMap.map.addMarker - begin");
				...
				this.addHoverMarker = function (x, y, options,idx) {
					$workRoad.ui.log("sMap.map.addHoverMarker - begin");
				...
				this.mapReload = function (center, zoom) {
					$workRoad.ui.log("sMap.map.mapReload - begin");					
				...
				this.addCircleMarker = function(x, y, options) {
					$workRoad.ui.log("sMap.map.addCircleMarker - begin");
				...
				this.removeMarker = function (marker) {
					$workRoad.ui.log("sMap.map.removeMarker - begin");
				...
				this.clearDataOverlay = function () {
					$workRoad.ui.log("sMap.map.clearDataOverlay - begin");
...
//					}
				...
				this.clearLayer = function() {
					$workRoad.ui.log("sMap.map.clearLayer - begin");
				...
				this.clearData = function () {
					$workRoad.ui.log("sMap.map.clearData - begin");
				...
				this.clearToolTip = function() {
					$workRoad.ui.log("sMap.map.clearToolTip - begin");
				...
				this.setInnerMap = function(isShow) {
					$workRoad.ui.log("sMap.map.setInnerMap - begin");
				...
				this.setInnerMap2 = function(isShow) {
					$workRoad.ui.log("sMap.map.setInnerMap2 - begin");
					...
					if (isShow) {
						...
						$todayStatusMap.ui.doClearMap(1);
						this.openApiReverseGeoCode(that.center);
				...
				this.setInnerMap3 = function(isShow) {
					$workRoad.ui.log("sMap.map.setInnerMap3 - begin");
				...
				this.setTradeMap = function(isShow) {
					$workRoad.ui.log("sMap.map.setTradeMap - begin");
				...
				this.setLegendColor = function () {
					$workRoad.ui.log("sMap.map.setLegendColor - begin");
				...
				this.updatePolygonGeoJson = function () {
					$workRoad.ui.log("sMap.map.updatePolygonGeoJson - begin");
				...
				this.setPolygonGeoJsonStyle = function (type) {
					$workRoad.ui.log("sMap.map.setPolygonGeoJsonStyle - begin");
						...
						case "data":
							...
							if(sop.isInnerMapShow2) {
								if($legendInfo.ui.slideInfo.slideValue2 == undefined){
									$legendInfo.ui.slideInfo.slideValue2 = 0.8;					// 2018.12.27	ywKim	변경: 0.7 -> 0.8
								};
								...
								if($legendInfo.ui.slideInfo.slideValue1 == undefined){
									$legendInfo.ui.slideInfo.slideValue1 = 0.8;					// 2018.12.27	ywKim	변경: 0.7 -> 0.8
								};
				...
				this.addPolygonGeoJson = function (obj, type, opt) {
					$workRoad.ui.log("sMap.map.addPolygonGeoJson - begin");
				...
				this.setPolyLayerMouseover = function (e) {
					$workRoad.ui.log("sMap.map.setPolyLayerMouseover - begin");
				...
				this.setPolyLayerMouseout = function (e) {
					$workRoad.ui.log("sMap.map.setPolyLayerMouseout - begin");
				...
				this.addControlEvent = function (type, opt) {
					$workRoad.ui.log("sMap.map.addControlEvent - begin");
					...
					if (type == "movestart") {
						...
						this.gMap.on("moveend", function (e) {	
							...
							//2019-01-11 IE에서 스크립트 오류 계속 발생, 화면을 건드릴때마다 reverseGeocode 호출 및 시도경계를 계속 가져옴
				...
				this.autoDownBoundary = function () {
					$workRoad.ui.log("sMap.map.autoDownBoundary - begin");
					...
					switch(parseInt(this.boundLevel)) {
						...
						case 1: //하위경계 1레벨
							...
							switch (this.curPolygonCode) {
								case 1:
									//this.setZoom(2);	20181011 손원웅_줌레벨 변경
									this.setZoom(1);
									break;
								case 2:
									this.setZoom(4); //6->4
				...
				this.autoUpBoundary = function () {
					$workRoad.ui.log("sMap.map.autoUpBoundary - begin");
				...
				this.setDroppedInfo = function() {
					$workRoad.ui.log("sMap.map.setDroppedInfo - begin");
				...
				this.undoDropLayerBounds = function() {
					$workRoad.ui.log("sMap.map.undoDropLayerBounds - begin");
				...
				this.isContainBound = function() {
					$workRoad.ui.log("sMap.map.isContainBound - begin");
				...
				this.getLocation = function () {
					$workRoad.ui.log("sMap.map.getLocation - begin");
				...
				this.setStatsData = function (type, data, showDataParamName, unit, length) {
					$workRoad.ui.log("sMap.map.setStatsData - begin");
				...
				this.openApiBoundaryContry = function (callback) { //9월 서비스
					$workRoad.ui.log("sMap.map.openApiBoundaryContry - begin");
				...
				this.openApiBoundarySido = function (year, callback) {
					$workRoad.ui.log("sMap.map.openApiBoundarySido - begin");
					
					//20181011 손원웅_adm_cd 값이 "00"(전국??)일 경우 종료라서 주석 처리 함	*기존 adm_cd 값은 "00" 아님 
					//-시작
					// 2016. 04. 26 j.h.Seok
					/*if (that.lastGeojsonInfo != null) {
				...
				this.openApiBoundaryHadmarea = function (adm_cd, year, low_search, atdrc_yn, callback, bounds) {
					$workRoad.ui.log("sMap.map.openApiBoundaryHadmarea - begin");
					...
					if ( bounds != null && bounds != undefined && this.isInnerMapShow2) { //mng_s kimjoonha grid this.isInnerMapShow2 조건 추가 20180206
						...
						var sopOpenApiHadmareaObj = new sop.openApi.wrm.hadmarea.api();
				...
				this.openApiBoundaryStatsarea = function (adm_cd, year, callback) { //9월 서비스
					$workRoad.ui.log("sMap.map.openApiBoundaryStatsarea - begin");
					
					var sopOpenApiStatsareaObj = new sop.openApi.wrm.statsarea.api();
				...
				this.openApiReverseGeoCode = function (center, callback) {
					$workRoad.ui.log("sMap.map.openApiReverseGeoCode - begin");					
					
					console.log("[map.js] openApiReverseGeoCode 호출");
					
					var sopOpenApiReverseGeoCodeObj = new sop.openApi.wrm.ReverseGeoCode.api();
				...
				this.openApiReverseGeoCodeGrid = function (center) {
					$workRoad.ui.log("sMap.map.openApiReverseGeoCodeGrid - begin");
					
					console.log("[map.js] openApiReverseGeoCodeGrid 호출");
					
					var sopOpenApiReverseGeoCodeGridObj = new sop.openApi.wrm.ReverseGeoCodeGrid.api();
				...
				this.openApiBuildArea = function(bounds) {
					$workRoad.ui.log("sMap.map.openApiBuildArea - begin");
					
					var sopOpenApibdAreaObj = new sop.openApi.wrm.bdArea.api();
				...
				this.openApiGridArea = function(bounds) {
					$workRoad.ui.log("sMap.map.openApiGridArea - begin");
					...
					var sopOpenApiGridAreaObj = new sop.openApi.wrm.gridarea.api();
				...
				this.openApiTradeArea = function(bounds) {
					$workRoad.ui.log("sMap.map.openApiTradeArea - begin");
					
					var sopOpenApitradeAreaObj = new sop.openApi.wrm.tradeArea.api();
				...
				this.setLayerColor = function (feature, layer) {
					$workRoad.ui.log("sMap.map.setLayerColor - begin");
...
//										fillColor : that.legend.legendColor[4]
				...
				this.checkShowCaption = function () {
					$workRoad.ui.log("sMap.map.checkShowCaption - begin");
				...
				this.setCaption = function () {
					$workRoad.ui.log("sMap.map.setCaption - begin");
							...
							this.dataGeojson.eachLayer(function(layer) {
								...
								if (layer.feature.combine) {
									if (layer.feature.combineData != undefined) {
										...
										layer.setCaption({title:"0", color:"white", showAllZoomLevel: false}, [x,y]);
									}
										...
										for (param in layer.feature.info[0]) {
											if (layer.feature.info[0].showData == param) {
												if (parseFloat(layer.feature.info[0][param]) < 5 && 
														...
														param != "acdmcr" &&
														param != "career" &&
														param != "right_data_val" && //오늘의 구인현황
														//mng_e 20200327 이금은
														param != "employee_cnt") {
													layer.setCaption({title:"0", color:"white"}, [x,y]);
												}else {
							...
							for(var i=0; i<this.multiLayerControl.dataGeojson.length; i++) {
								...
								dataGeojson.eachLayer(function (layer) {
									if (layer.feature.info.length > 0) {
										...
										layer.setCaption({title:"0", color:"white"});
//										layer.setCaption({title:"N/A", color:"#808080"});
				...
				this.removeCaption = function () {
					$workRoad.ui.log("sMap.map.removeCaption - begin");
				...
				this.setPolygonDataGeojson = function (geoData) {
					$workRoad.ui.log("sMap.map.setPolygonDataGeojson - begin");
					
					that.clearDataCnt++;	//20181123 손원웅 지도 경계 count
					...
					//20181011 20181011 손원웅_boundLevel이란?
					//if (this.geojson || this.boundLevel == "2") {		*기존 or 조건일 경우 경계 표출 안됨, 아래 조건 내 로직 수정 요망.
				...
				this.combineStatsData = function (boundData, isPass) {
					$workRoad.ui.log("sMap.map.combineStatsData - begin");
					...
					if(document.location.href.match("interactiveMap")){
						$todayStatusMap.ui.toolTipTitle = that.data[0].showData;
						$todayStatusMap.ui.toolTipUnit = that.data[0].unit;	
				...
				this.setLegendForStatsData = function (isPass) {
					$workRoad.ui.log("sMap.map.setLegendForStatsData - begin");
								...
								if(that.data[k].result != null) {
									for ( var i = 0; i < that.data[k].result.length; i++) {
										for (key in that.data[k].result[i]) {
											console.log("2617:that.data[k].showData : " + that.data[k].showData);
											if (key == that.data[k].showData) {
												
												//개인정보보호 때문에 특정 센서스 정보의 경우, 5미만은 4로 치환한다.
												if (
													/* mng_s 20200327 이금은 주석처리
												    key === "indust_class" 	 ||	  //구인현황 추가 - 산업분류 조회
													key === "rcrit_jssfc" 	 ||	  //구인현황 추가 - 직종분류 조회
													key === "entrprs_type" 	 ||	  //구인현황 추가 - 기업형태 조회
													key === "emplym_type" 	 ||	  //구인현황 추가 - 고용형태 조회
													key === "wage_type" 	 ||	  //구인현황 추가 - 임금형태 조회
													key === "acdmcr" 	     ||	  //구인현황 추가 - 학력 조회
													key === "career" 	     ||	  //구인현황 추가 - 경력 조회
													mng_e 20200327 이금은 주석처리 */
													//2019-05-29 [김남민] 구인현황 분석 > 업종별 직종별 등 분석조건을 AND 조건이 가능하게 개선(교차분석) START
													//key === "status_anls_all" 	 ||	  //구인현황 추가 - 구인현황분석 전체 조회
													//2019-05-29 [김남민] 구인현황 분석 > 업종별 직종별 등 분석조건을 AND 조건이 가능하게 개선(교차분석) END
													//key === "right_data_val" 	 ||	  //구인현황 추가	// 2019.06.17[한광희] 오늘의 구인현황 기간조회 기능 활성화에 따른 주석 처리
													key === "tot_ppltn" 	 ||	  //총인구
				...
				this.multiLayerControl = {
					...
					dataGeojson : null,
					clear : function () {
						$workRoad.ui.log("sMap.map.multiLayerControl.clear - begin");
					...
					setStatsData : function (type, data, options, isShare) {
						$workRoad.ui.log("sMap.map.multiLayerControl.setStatsData - begin");
					...
					setStatsData2 : function (type, data, options, isShare) {
						$workRoad.ui.log("sMap.map.multiLayerControl.setStatsData2 - begin");
					...
					reqBoundary : function(adm_cd, data, atdrc_yn, callback) {
						$workRoad.ui.log("sMap.map.multiLayerControl.reqBoundary - begin");
					...
					autoDownBoundary : function () {
						$workRoad.ui.log("sMap.map.multiLayerControl.autoDownBoundary - begin");
					...
					setPolygonDataGeojson : function (Ldata) {
						$workRoad.ui.log("sMap.map.multiLayerControl.setPolygonDataGeojson - begin");
					...
					combineStatsData : function (Ldata) {
						$workRoad.ui.log("sMap.map.multiLayerControl.combineStatsData - begin");
					...
					openApiBoundaryContry : function (data, callback) {
						$workRoad.ui.log("sMap.map.multiLayerControl.openApiBoundaryContry - begin");
					...
					openApiBoundarySido : function (year, data, callback) {
						$workRoad.ui.log("sMap.map.multiLayerControl.openApiBoundarySido - begin");
					...
					openApiBoundaryHadmarea : function (adm_cd, year, low_search, data, atdrc_yn, callback) {
						$workRoad.ui.log("sMap.map.multiLayerControl.openApiBoundaryHadmarea - begin");
					...
					openApiBoundaryStatsarea : function (adm_cd, year, data, callback) {
						$workRoad.ui.log("sMap.map.multiLayerControl.openApiBoundaryStatsarea - begin");
				...
				this.createHeatMap = function(options) {
					$workRoad.ui.log("sMap.map.createHeatMap - begin");
				...
				this.addHeatMap = function(x, y, data) {
					$workRoad.ui.log("sMap.map.addHeatMap - begin");
				...
				this.setHeatMapOptions = function(radius, blur, max) { //9월 서비스
					$workRoad.ui.log("sMap.map.setHeatMapOptions - begin");
	...
	(function () {
		$class("sop.openApi.wrm.hadmarea.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				$workRoad.ui.log("sMap.sop.openApi.wrm.hadmarea.api - begin");
				
				console.log("[map.js] sop.openApi.wrm.hadmarea.api res.errCd [" + res.errCd);
			...
			onFail : function (status) {
				console.log("[map.js] sop.openApi.wrm.hadmarea.api onFail [" + status);
			}
		});
	}());
	...
	(function () {
		$class("sop.openApi.wrm.statsarea.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				$workRoad.ui.log("sMap.sop.openApi.wrm.statsarea.api - begin");
	...
	(function () {
		$class("sop.openApi.wrm.gridarea.api").extend(sop.portal.absAPI).define({
			
			onSuccess : function (status, res, options) {
				$workRoad.ui.log("sMap.sop.openApi.wrm.gridarea.api - begin");
				
				console.log("[map.js] sop.openApi.wrm.gridarea.api 호출");
				console.log("[map.js] sop.openApi.wrm.gridarea.api res.errCd [" + res.errCd);
			...
			onFail : function (status) {
				console.log("[map.js] sop.openApi.wrm.gridarea.api onFail status [" + status);				
			}
		});
	}());
	...
	(function () {
		$class("sop.openApi.wrm.ReverseGeoCode.api").extend(sop.portal.absAPI).define({
			
			onSuccess : function (status, res, options) {
				$workRoad.ui.log("sMap.sop.openApi.wrm.ReverseGeoCode.api - begin");
				
				console.log("[commonmap.js] sop.openApi.wrm.ReverseGeoCode.api 호출");
				console.log("[commonMap.js] sop.openApi.wrm.ReverseGeoCode.api res.errCd [" + res.errCd);
				...
				if (res.errCd == "0") {
					...
					//경계 레벨에 따른 코드 값 설정_20181108 손원웅
					if(that.SidoComboVal != 0){
						//that.mapMove([989674,1818313],2);
						if(that.SidoComboVal == "99"){
							curPolygonCode = 2;
							//$tsMap.ui.mapList[0].setZoom(1);
							that.setZoom(2);
						}else{
							//that.data 0일 때 스크립트 오류 제거
							if(that.data.length > 0){
								that.data[0].pAdmCd = that.SidoComboVal;
								curSidoCd = that.SidoComboVal;
								curPolygonCode = 3;
								//$tsMap.ui.mapList[0].mapMove([$tsMain.ui.tsMain_coor_x,$tsMain.ui.tsMain_coor_y],5);	//지도 center 및 줌 레벨 변경
								
								// 시는 확대, 도는 축소 - 2019.01.24	ywKim	변경
								if (that.usingZoomLevelBySidoArea) {
									var doAreaCode = ["31", "32", "33", "34", "35", "36", "37", "38"];// 경기도,강원도,충청남북도,전라남북도,경상남북도
									if (doAreaCode.indexOf(curSidoCd) >= 0) {
										that.mapMove([that.xComboVal,that.yComboVal],3);
									} else {
										//2019-12-26 [김남민] 일자리 맵 > 구인현황 분석 > 지역조건 오류수정 및 조회범위 읍면동 확대 (검색조건이 변하지는 않음) START
										//읍면동 줌 레벨 추가
										if((""+curSidoCd).length == 5) {
											that.mapMove([that.xComboVal,that.yComboVal],6);	//지도 center 및 줌 레벨 변경
										}
										else {
											that.mapMove([that.xComboVal,that.yComboVal],5);	//지도 center 및 줌 레벨 변경
										}
								...
								//위 스크립트 오류를 제거 및 mapMove 함수를 사용하면 무한루프를 돌기 때문에 return 처리
								return;
							}
						}
					}
					
					console.log("[map.js] sop.openApi.wrm.ReverseGeoCode.api that.isInnerMapShow2[" + that.isInnerMapShow2);
					console.log("[map.js] sop.openApi.wrm.ReverseGeoCode.api that.isInnerMapShow3[" + that.isInnerMapShow3);
					
					if(!that.delegate.noReverseGeoCode){
						//공유된 정보일 경우, 조회했던 경계레벨로 설정
						if(document.location.href.match("interactive")){
							if($todayStatusMap.ui.isShare){
								var data = $todayStatusMap.ui.shareData["data"];
								...
								if(data.curPolygonCode != undefined){
									...
									$todayStatusMap.ui.isShare = false;
								}
							}
						}
							...
							case 3:
								...
								if (that.lastGeojsonInfo != null 
									...
									if (that.dataGeojson == null) {
										...
										if(that.clearDataCnt > 0){	//20181123 손원웅 지도 경계 그리고 난 후 $tsMap.ui.mapList[0].data clear
											that.clearData();
										}
										that.openApiBoundaryHadmarea(curSidoCd, that.bnd_year, "1", "0", that.boundaryCallback); //9월 서비스
									}
								}else {
									if(that.clearDataCnt == 0){
										that.openApiBoundaryHadmarea(curSidoCd, that.bnd_year,  "1", "0", that.boundaryCallback); //9월 서비스
									}
								}
								break;
			...
			onFail : function (status) {
				console.log("[map.js] sop.openApi.wrm.ReverseGeoCode.api onFail [" + status);
			}
		});
	}());
	/** ********* OpenAPI 리버스지오코딩. End ********* */
	...
	(function () {
		$class("sop.openApi.wrm.ReverseGeoCodeGrid.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				$workRoad.ui.log("sMap.sop.openApi.wrm.ReverseGeoCodeGrid.api - begin");
	...
	(function () {
		$class("sop.openApi.wrm.bdArea.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				$workRoad.ui.log("sMap.sop.openApi.wrm.bdArea.api - begin");
	...
	/** ********* OpenAPI 상권조회 Start ********* */
	(function () {
		$class("sop.openApi.wrm.tradeArea.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				$workRoad.ui.log("sMap.sop.openApi.wrm.tradeArea.api - begin");
