 ...
 */
(function(W, D) {
	W.$ssaLifeQualityDetailPopup = W.$ssaLifeQualityDetailPopup || {};
	
	$ssaLifeQualityDetailPopup.ui = {
		pSelector	: "#ssaLifeQualityDetailPopup",
		DetailData : [],
		...
		show : function() {
			...
			if ($ssaMap.ui.curMapId == "0") {
				...
				listElement += '<input type="hidden" id="wrmSubTitle1DetailPopup" value="ssaLifeQualityDetailPopup">';
				...
				listElement += '<input type="hidden" id="wrmSubTitle2DetailPopup" value="ssaLifeQualityDetailPopup">';
				...
				listElement += '<input type="hidden" id="wrmSubTitle3DetailPopup" value="ssaLifeQualityDetailPopup">';
		...
		selectDataBoardMap : function(param) {
			...
			$ssaLifeQualityDetailPopup.ui.MapData[0] = Tmpdata;
			$ssaLifeQualityDetailPopup.ui.MapType = $ssaLifeQuality.ui.MapType;
			$ssaMap.ui.ssaLifeQualityOpenApiBoundarySido("2016");	//최신년도 데이터가 2016...
		},
		...
		SearchPopupData : function(){
			...
			srvLogWrite('D0', '05', '05', '03', '', slwParams);

			console.log("SearchPopupData - dataParams : " + JSON.stringify(dataParams));
			
			$.ajax({
				type: "POST",
				url : contextPath + "/ServiceAPI/workRoad/statsAnls/getStatsAnlsLifeQualityDetail.json",
				async: false,
				dataType: "json",
				data: dataParams,
				success: function(res) {
					if(res.errCd == 0){
						...
						//ui에서 사용
						$ssaLifeQualityDetailPopup.ui.DetailData = this.DetailData;
						$ssaLifeQualityDetailPopup.ui.Category = this.Category;
						...
						console.log("SearchPopupData - This_Mon : " + JSON.stringify(This_Mon));
						console.log("SearchPopupData - Last_Mon : " + JSON.stringify(Last_Mon));
						...
						if(Box_DetailData.length == 0){
							$('div[id="ssaLifeQualityDetail_popup"] *').remove();
							...
							listElement += '</table>';
							listElement += '</div>';
							$('div[id="ssaLifeQualityDetail_popup"]').append(listElement);
...
//							unitObj.compareOperation = "-";
							...
							listElement += '<dd title="' + result.value + '">' + result.text + '</dd> <span class="ssaLifeQualityDetailPopup job-arrow '+ result.iconClass +'" id='+ link_ID+"_apc" +'></span>';
							listElement += '</dl>';
							listElement += '<br>';
							...
							$('#ssaLifeQualityDetailPopup #'+link_ID+'_apc').attr('data-val', result.iconText);
							$('#ssaLifeQualityDetailPopup #'+link_ID+'_apc').attr('data-subj', idaSubj);
							$('#ssaLifeQualityDetailPopup #'+link_ID+'_apc').attr('title', idaTitle);
		...
		SearchPopupChart : function(){
			...
			$.ajax({
				type: "POST",
				url : contextPath + "/ServiceAPI/workRoad/statsAnls/getStatsAnlsLifeQualityDetailChart.json",
				async: false,
				dataType: "json",
				data: dataParams,
				success: function(res) {
					if(res.errCd == 0){
						...
						console.log("Chart_DetailData : " + JSON.stringify(Chart_DetailData));
...
//								20190319 손원웅 추가
						...
						//Y축 단위 명칭 변경
						if(this.DetailData[0].link_id == "E3506"){
							yLabel = '점';
						}else if(this.DetailData[0].link_id == "E3505" || this.DetailData[0].link_id == "E3503" || this.DetailData[0].link_id == "E3504" || this.DetailData[0].link_id == "E3502"){
							yLabel = '%';
						}else if(this.DetailData[0].link_id == "E3501"){
							yLabel = '계수';
						}
						
						console.log("PC_Series : " + JSON.stringify(PC_Series));
						
						$('#ssaLifeQualityDetailChart').highcharts({
							chart: {
							...
							plotOptions: {								
								series: {
						            ...
						            point: {
						                events: {						                	
						                    click: function (e) {
						                    	//2020-04-20 [곽제욱] 삶의질 지표 Map에 표출하기 위한 변경 START
						                    	...
						                    	if(dataParams.Link_id == "E3504" || dataParams.Link_id == "E3505" || dataParams.Link_id == "E3503") {
						                    	...
						                    	$ssaDataBoard.ui.updateTargetAreaLifeQualityChart(this.category);	//데이터보드 차트
						                    	$ssaDataBoard.ui.updateTargetAreaLifeQualityTable(this.category);	//데이터보드 표
						                    	var lvItem = "";
...
//															console.log("SearchPopupMap - DataInfo data : " + JSON.stringify(data));
						                    	...
						                    	//2020-04-20 [곽제욱] 삶의질 지표 Map에 표출하기 위한 변경 END
...
//						        verticalAlign: 'middle'
		...
		SearchPopupMap : function(){
			...
			$.ajax({
				type: "POST",
				url : contextPath + "/ServiceAPI/workRoad/statsAnls/getStatsAnlsLifeQualityMainDataBoard.json",
				async: false,
				dataType: "json",
				data: dataParams,
				success: function(res) {
					if(res.errCd == 0){
						...
						//ui에서 사용
						$ssaLifeQualityDetailPopup.ui.DetailDataChart = this.DetailDataChart;
						$ssaLifeQualityDetailPopup.ui.DetailData = this.DetailData;
						$ssaLifeQualityDetailPopup.ui.Category = this.Category;
						$ssaLifeQualityDetailPopup.ui.CurrentDate = this.CurrentDate;
...
//								console.log("SearchPopupMap - DataInfo data : " + JSON.stringify(data));
								...
								var this_Mon = $workRoad.util.dateWithSign($ssaLifeQualityDetailPopup.ui.CurrentDate[0].max_prd_de, "."); //max데이터 월 또는 현재 월
								var title    = data.stat_nm + " (" + this_Mon + ")";
								$("#ssaDataBoard #ssaStatPath").html(data.stat_path);
								$("#ssaDataBoard #ssaTitle").html(title);					// 제목
								$("#ssaDataBoard #ssaOrigin").html(data.colct_source);		// 출처
						...
						var This_Mon = this.CurrentDate[0].max_prd_de;	//max데이터 월 또는 현재 월
						//2020-04-24 [곽제욱] 삶의 질 MAP 표출 START
						var Last_Mon = parseFloat(This_Mon) - 1; //This_Mon 이전 월
						//지도 그리기_시작
						//지도 데이터 세팅
						var result = [];
						...
						console.log("This_Mon : " + This_Mon);
						
						if(dataParams.Link_id == "E3504" || dataParams.Link_id == "E3505" || dataParams.Link_id == "E3503"){
							$ssaMap.ui.getTodayStatusMapData("","","",This_Mon);	//지도 연도 세팅
							$ssaMap.ui.ssaLifeQualityOpenApiBoundarySido("2017");	//최신년도 데이터가 2016...
						} else {
						...
						//2020-04-24 [곽제욱] 삶의 질 MAP 표출 END
						
						//데이터보드 차트 카테고리 세팅
						$ssaLifeQualityDetailPopup.ui.DB_Chart_Category = [];
						var Chk_Category = [];
						...
						for(var i=0; i<this.DetailData.length; i++){
							if(This_Mon == this.DetailData[i].prd_de){
								$ssaLifeQualityDetailPopup.ui.DB_Chart_Category.push(this.DetailData[i].c1_nm);
								var TmpResult = {};
								TmpResult.adm_cd = this.DetailData[i].c1;
								TmpResult.adm_nm = this.DetailData[i].c1_nm;
						...
						console.log("SearchPopupMap - DB_Chart_Category : " + JSON.stringify($ssaLifeQualityDetailPopup.ui.DB_Chart_Category));
						...
						$ssaLifeQualityDetailPopup.ui.DataBoardChart = [];	//초기화
						var Tmpdata2 = [];
						var TmpCnt = 0;
...
//								Tmpdata2 = Tmpdata;
						...
						$ssaLifeQualityDetailPopup.ui.DataBoardChart = Tmpdata2;
						...
						if(dataParams.Link_id == "E3504" || dataParams.Link_id == "E3505" || dataParams.Link_id == "E3503"){
							console.log("차트, 테이블 입력 진입");
							$ssaDataBoard.ui.updateTargetAreaLifeQualityChartInvert(This_Mon);	//차트
							$ssaDataBoard.ui.updateTargetAreaLifeQualityTableInvert(This_Mon);	//테이블
							console.log("차트, 테이블 입력 완료");
						}else{
							...
							for(var i=0; i<this.DetailData.length; i++){
								if(This_Mon == this.DetailData[i].prd_de){
									$ssaLifeQualityDetailPopup.ui.DB_Chart_Category.push(this.DetailData[i].itm_nm);
								}
							...
							console.log("DB_Chart_Category : " + JSON.stringify($ssaLifeQualityDetailPopup.ui.DB_Chart_Category));
							...
							console.log("Y축 첫번째 : " + this.Category[0].mon0);
							$ssaLifeQualityDetailPopup.ui.DataBoardCategory = arry_Category;
							...
							$ssaLifeQualityDetailPopup.ui.DataBoardChart = data;
							$ssaLifeQualityDetailPopup.ui.DataBoardTable = TmpData;
							
							console.log("else 진입");
							
							$ssaDataBoard.ui.updateTargetAreaLifeQualityChart(This_Mon); //차트
							$ssaDataBoard.ui.updateTargetAreaLifeQualityTable(This_Mon); //테이블
	...
	$ssaLifeQualityDetailPopup.event = {
			/**
			 * @name		 : 이벤트 바인딩 
			...
			setUIEvent: function() {
				// 닫기 버튼
				$workRoad.event.set("click", "#ssaLifeQualityDetailPopup .topbar a", function() {
					$ssaLifeQuality.ui.show();
				});			
			},
	}
}(window, document)); 