 ...
 * ************************/		
		...
		/**
		 * @name         : DetailInfo
		 * @description  : 일자리통계분석 > 일자리 증감 > 지표 설명 정보 팝업 호출로 인한 추가
		 * @date         : 2019.05.17
		 * @author	     : 한광희
		 * @history 	 : 
		 */
		DetailInfo : function(item, statNm, stat_exp, colct_source) {				
			var left = parseInt($("#ssaJobGrowth").css("left"));
			var top = parseInt($("#ssaJobGrowth").css("top"));
			var width = parseInt($("#ssaJobGrowth").css("width"));
			...
			if (item == 'E3224') {	// 피보험자 증감
				left = left + 160 - $workRoad.ui.coordX;
				top = top + 58 - $workRoad.ui.coordY;
			} else if (item == 'E3218') {	// 일자리 > 구직건수
				left = left + 200 - $workRoad.ui.coordX;
				top = top + 100 - $workRoad.ui.coordY;
			} else if (item == 'E3208') {	// 일자리 > 구인 인원
				left = left + 200 - $workRoad.ui.coordX;
				top = top + 143 - $workRoad.ui.coordY;
			} else if (item == 'I3207') {	// 사업체 > 폐업 수
				left = left + 200 - $workRoad.ui.coordX;
				top = top + 186 - $workRoad.ui.coordY;
			} else if (item == 'I3206') {	// 사업체 > 신설 수
				left = left + 200 - $workRoad.ui.coordX;
				top = top + 229 - $workRoad.ui.coordY;
			}
		...
		ChangeCondition : function(item) {
			...
			// 2019.03.13 접근log 생성
			srvLogWrite('D0', '05', '02', '02', '', 'Link_id='+item);
...
//							var result = $workRoad.util.toNumberString(Val, {points: [0.1,3.1,6.1], units: [unit_Nm,"천호","백만호"]});
							...
							if(link_ID == "E3218" || link_ID == "E3208" || link_ID == "I3207" || link_ID == "I3206"){	//일자리 - 구직자, 일자리 - 구인인원, 사업체 - 폐업수, 사업체 - 창업수 표출부분	
								...
								listElement += '<dl> <dt>' + link_Nm + updt_cycle_html + '&nbsp;&nbsp;<img src="/images/workRoad/ico/ico_tooltip01.png" alt="물음표" onmouseover="javascript:$ssaJobGrowth.ui.DetailInfo(\'' + link_ID + '\', \''+ statNm +'\', \''+ tooltipVal +'\', \'' + colct_source + '\');" onmouseout="javascript:$ssaDetailInfoPopup.ui.hide();"/></dt>';
				...
				success: function(res) {
					if(res.errCd == 0){
						...
						*/
						...
						//if(item == undefined){
//						20190313 손원웅 추가_피보험자 증감으로 바뀌면서 단위(%) 추가
						var arr_yAxis = [{
										title: { 
											text: '명', // 2020-05-18 [곽제욱] Kosis 데이터 연계로 인하여 통계 단위 변경(% -> 명)
											enabled: false,
											style: {}
										},
										labels: {
											enabled: (item == "E3224") ? true : false,
											formatter: function () {
												return $workRoad.util.addComma(this.value)+" 명" // 2020-05-18 [곽제욱] Kosis 데이터 연계로 인하여 통계 단위 변경(% -> 명)
											},
											style: {}
										},
										lineWidth: 1,
...
//										enabled: (item == "E3224" || item == "E3208" || item == "I3207") ? true : false,
										enabled: (item == "E3208" || item == "I3207") ? true : false,
										formatter: function () {
...
//									visible : (item == "E3224" || item == "E3208" || item == "I3207") ? true : false
									visible : (item == "E3208" || item == "I3207") ? true : false
								},
								{	title: {
										text: '천명',
										enabled: false,
		...
		addSearchBtn : function() {
			...
			*/
			...
			//if(this.btnLimitCnt()) {
				...
				if(ParamLink_id =="E3224") {		// 조회조건 팝업 호출
					$("#ssaJobGrowthSearchPopup #ssaJobGrowthSearchTitle").html(Var_Title);	
					$ssaJobGrowthSearchPopup.ui.show(ParamLink_id);
				} else {	// 그 외 통계는 바로 호출
					$("#ssaJobGrowth").hide();
				...
				/** 2019.05.14 [곽제욱] 일자리증감 조회조건을 팝업 변경으로 인한 수정 END */
