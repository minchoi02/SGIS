 ...
 */
(function(W, D) {
	W.$ssaMapFrame = W.$ssaMapFrame || {};
	
	$(document).ready(function() {// used
		$workRoad.ui.log("$ssaMapFrame.ready - begin");

		$ssaMapFrame.ui.bndYearSelectbox();		//조사년도 설정
		$ssaMapFrame.ui.commonDataList();		//공공데이터, 나의데이터 목록
		$ssaMapFrame.event.setUIEvent();	//UI에 사용되는 이벤트를 설정한다.
		...
		if(parseInt($("#mainIndex_year").val()) == 2016){
			$("#mainIndex_box2").hide();
			$ssaMapFrame.ui.companyClassView();
		}
		...
		$.cssHooks.backgroundColor = {
			    get: function(elem) {
					$workRoad.ui.addFnc04List("$ssaMapFrame.[$.cssHooks.backgroundColor.get]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
		...
		$.cssHooks.color = {
			    get: function(elem) {
					$workRoad.ui.addFnc04List("$ssaMapFrame.[$.cssHooks.color.get]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			        if (elem.currentStyle)
		...
		$("[name='mainIndex_radio']").on('click',(function(e){
			$workRoad.ui.addFnc04List("$ssaMapFrame.event.[[name='mainIndex_radio']]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도 						
			...
			$ssaMapFrame.ui.subConditionNoneAll();
	...
	$ssaMapFrame.ui = {
			...
			subConditionNoneAll : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.subConditionNoneAll");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			addSearchBtn : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.addSearchBtn");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도				
				//조회버튼은 최대 10개만 가능
				//alert("else if" + this.curSelectedStatsType + "/" + this.curSelectedDetailStatsType);
				if(this.btnLimitCnt()) {
					//KOSIS일 경우
					if (this.curSelectedStatsType == "kosis") {
						if(todayStatusMapEtc.curSelectNodeCheck == false) {
							messageAlert.open("알림", "항목을 선택하여 주세요.");
							return false;
						} else {
							
							todayStatusMapEtc.checkSubMenu(todayStatusMapEtc.curSelectNode);
						}
					}
					...
					//일반 버튼일 경우 파라미터 유효성 검사					
						...
						this.searchbtnCnt++;
						$ssaMapApi.request.openApiStatBaseYearProcess();	//통계별 최신년도 가져오기
						$ssaMapFrame.event.stepCloseAnimate(1, "check");	
			...
			btnLimitCnt : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.btnLimitCnt");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			 ...
			 */
			getBtnCnt : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.getBtnCnt");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var cnt = $("#searchBtnResultRgn").find("li:visible").length;
			},
			...
			createSearchBtn : function(curSelectedStatsType, atdrc_yn) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.createSearchBtn");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				if(curSelectedStatsType == "kosis") {
					tmpTitle = todayStatusMapEtc.curSelectedTitle;
					for (var i = 0; i < this.arParamList.length; i++) {
						if (this.arParamList[i].idx == this.searchbtnCnt) {
				...
				var html = "<li class='dragItem' id='dragItem_"+this.searchbtnCnt+"'>" +
								...
								"</a>" +
								"<a href='javascript:$ssaMapFrame.ui.deleteSearchBtn("+this.searchbtnCnt+");' class='sqdel'><img src='/img/um/btn_closel01.png' alt='삭제' /></a>" +
						   "</li>";
				...
				"</a>"+
				"<a href='javascript:$ssaMapFrame.ui.deleteSearchBtn(0);' class='sqdel'><img src='/img/um/btn_closel01.png' alt='삭제' class='mCS_img_loaded'></a>"+
				"</li>";*/
...
//				});
				...
				$(".dragItem").draggable({ 
					...
					helper : function() {	//드래그시 폰트 색상 변경
						...
						//아직 조회가 안된 버튼은 검정글씨
						if($ssaMapFrame.ui.curSearchBtnArray["one"] != btnId &&
							$ssaMapFrame.ui.curSearchBtnArray["two"] != btnId &&
							$ssaMapFrame.ui.curSearchBtnArray["three"] != btnId) {
			...
			searchModeSetting : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.searchModeSetting");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				
				console.log("[ssaMapFrame.js] searchModeSetting 호출");

				//mng_s
				if($ssaMapFrame.ui.isInnerMapShow2!=undefined && $ssaMapFrame.ui.isInnerMapShow2) {
				...
				$(".dragItem").off("dblclick").dblclick(function(event) {
					$workRoad.ui.addFnc04List("$ssaMapFrame.ui.searchModeSetting.[.dragItem.dblclick]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					var id = $("#"+event.currentTarget.id).find("a").attr("id");
					var index = id.split("-")[1];
					var tmpParam = "";
					for(var i = 0; i < $ssaMapFrame.ui.arParamList.length; i ++) {
						if($ssaMapFrame.ui.arParamList[i].idx == index) {
							tmpParam = $ssaMapFrame.ui.arParamList[i];
						}
					}
					// 더블클릭 시, 콜백 호출
					$ssaMap.callbackFunc.didMapDoubleClick(id, tmpParam);
				});
			 ...
			 */
			updateSearchBtnEffect : function(btn_id, map_id) {// used
				$workRoad.ui.log("$ssaMapFrame.ui.updateSearchBtnEffect - begin");
				...
				if(this.curSearchBtnArray["one"] != "") {
					$ssaMapFrame.event.dragAnimate(this.curSearchBtnArray["one"], this.mapColor[0]);
				}
				if(this.curSearchBtnArray["two"] != "") {
					$ssaMapFrame.event.dragAnimate(this.curSearchBtnArray["two"], this.mapColor[1]);
				}
				if(this.curSearchBtnArray["three"] != "") {
					$ssaMapFrame.event.dragAnimate(this.curSearchBtnArray["three"], this.mapColor[2]);
				}
				
				//분할 화면에 같은 버튼을 조회했을 경우
				if(this.curSearchBtnArray["one"] != "" && (this.curSearchBtnArray["one"] == this.curSearchBtnArray["two"])) {
					$ssaMapFrame.event.dragAnimate(this.curSearchBtnArray["one"], "#394955");
				}
				if(this.curSearchBtnArray["two"] != "" && (this.curSearchBtnArray["two"] == this.curSearchBtnArray["three"])) {
					$ssaMapFrame.event.dragAnimate(this.curSearchBtnArray["two"], "#394955");
				}
				if(this.curSearchBtnArray["three"] != "" && (this.curSearchBtnArray["three"] == this.curSearchBtnArray["one"])) {
					$ssaMapFrame.event.dragAnimate(this.curSearchBtnArray["three"], "#394955");
				}
			},
			...
			setParams : function(curSelectedStatsType, api_id) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.setParams");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				console.log("[$ssaMapFrame.js] setParams 조건버튼으로 만들어진 통계정보에 대한 파라미터정보를 설정");
				...
				if (curSelectedStatsType == "mainIndex") {
					...
					if (api_id == "API_WORK_URL") {
						...
						//filterParam = $("#API_0301").find(".dbTypeCk label.on").prev("input").val();
						filterParam = "right_data_val";
						
						fullName = $("#API_0301").find(".dbTypeCk label.on").text();
						
						if($("#API_0301").find(".dbTypeCk label.on").text().indexOf("(") > -1) {
						...
						else if(filterParam == "tot_work") {	// 구인현황 조회
							unit = "구인명";
						}
						//20181011 손원웅_컬럼명 추가 _시작
						else if(filterParam == "right_data_val") {	// 구인현황 조회
							unit = "구인명";
						}
						//_끝
					...
					if (api_id == "API_0301") {
						...
						else if(filterParam == "tot_test") {	// 구인현황 조회
							unit = "구인명";
						}
				...
				// 전국사업체조사
					...
					if (api_id == "API_0304-b") {
						...
						if($("#company_TreeBox").is(":visible")) {
							// 산업체유형 선택
							if ($ssaMapFrame.ui.curSelectedCompanyNode != null && $ssaMapFrame.ui.curSelectedCompanyNode.cd.length > 0) {
								tmpArParams.push({
									key : "class_code",
									value : $ssaMapFrame.ui.curSelectedCompanyNode.cd
								});
								tmpArParamName.push($ssaMapFrame.ui.curSelectedCompanyNode.text);
							}
						}
						...
						*/
						...
						if(($ssaMapFrame.ui.curSelectedCompanyNode == null || $ssaMapFrame.ui.curSelectedCompanyNode.cd == "" )&& !$("#company_SearchBox").is(":visible")) {
				...
				// KOSIS
					...
					tmpArParams.push({
						key : "org_id",
						value : todayStatusMapEtc.org_id
					});
					tmpArParams.push({
						key : "tbl_id",
						value : todayStatusMapEtc.tbl_id
					});
					
					tmpArParams.push({
						key : "obj_var_id",
						value : todayStatusMapEtc.kosis_obj_var_id
					});
					
					tmpArParams.push({
						key : "field_id",
						value : todayStatusMapEtc.kosis_field_id
					});
					
					if (todayStatusMapEtc.kosis_data_item_detail != null
							&& todayStatusMapEtc.kosis_data_item_detail.length > 2) {
						tmpArParams.push({
							key : "kosis_data_item_detail",
							value : todayStatusMapEtc.kosis_data_item_detail
						});
					} else {
						tmpArParams.push({
							key : "kosis_data_item_detail",
					...
					tmpArParams.push({
						key : "kosis_data_item",
						value : todayStatusMapEtc.kosis_data_item
					});
					tmpArParams.push({
						key : "kosis_data_period",
						value : todayStatusMapEtc.kosis_data_period
					});
					tmpArParams.push({
						key : "kosis_data_year",
						value : todayStatusMapEtc.kosis_data_year
					});
					tmpArParams.push({
						key : "gis_se",
						value : todayStatusMapEtc.gis_se
					});
				}
			...
			setRevertParams : function(data, type) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.setRevertParams");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			setDetailStatsPanel : function(type) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.setDetailStatsPanel");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				console.log("[ssaMapFrame.js] setDetailStatsPanel 호출 : "+type);
...
//				$(".sideQuick.sq02").stop().animate({"left":"560px"},200);
				...
				switch(menuType[type]) {
					...
					case 3:
						...
						$ssaMap.ui.companyTree = null;
						$ssaMapFrame.ui.curSelectedCompanyNode = null;
						$ssaMapApi.request.openApiInterstryCode(0, class_deg, null);
						break;
...
//						$("#buttonMakeBtn").html("세부 조건창 열기");
						...
						$ssaMapFrame.event.kosisTreeWidth();
						//9월 서비스
						if (todayStatusMapEtc.checkKosisBtn()) {
					...
					case 7:
						...
						break;
					case 8:
						$ssaMapFrame.ui.companyTree = null;
						inx = 3;
			...
			populationTab : function(type) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.populationTab");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var tabType = {
					"population" : 0,				//인구조건
					"household"  : 1,				//가구조건
					"house" 	 : 2,					//주택조건
			...
			tripleFTab : function(type) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.tripleFTab");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var tabType = {
					"farm" : 0,				//농가
					"forest"  : 1,				//임가
					"fish" 	 : 2			//어가
			...
			companyTab : function(type) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.companyTab");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				if(tabType[type] == 0) {		//산업분류
					...
					if($ssaMapFrame.ui.isInnerMapShow2!=undefined && $ssaMapFrame.ui.isInnerMapShow2) {
						...
						$("#buttonMakeBtn").show();
						$("#menuAutoClose2Lev").show();
					} else if($ssaMapFrame.ui.isInnerMapShow3!=undefined && $ssaMapFrame.ui.isInnerMapShow3) {
						//버튼생성 보이기
						//기존 산업분류에서는 3뎁스에서 버튼이 존재하고 그리드에서는 2뎁스에 존재해야하므로 여기다 세팅한다.
						$("#buttonMakeBtn").text("사업체조사 버튼생성");
						$("#buttonMakeBtn").show();
			...
			companyClassView : function(type) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.companyClassView");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			companySearch : function(pageNum) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.companySearch");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				if($("#companySearchText").val() == "") {
					...
					var sopPortalCorpClassSearchObj = new sop.portal.ssa.corpClassSearch.api.ssa();
			...
			companyPaging : function(totalCount, currentIndex) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.companyPaging");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var pageSize = 5;
				...
				$('#corpClassPaging .pages').paging({
					...
					onclick:function(e,page){
						$ssaMapFrame.ui.corpClassNum = page-1;
						$ssaMapFrame.ui.companySearch(page-1);
					}
				});
			},
			...
			companyTreeShow : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.companyTreeShow");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			deleteSearchBtn : function(value) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.deleteSearchBtn");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			 ...
			 * @history 	 :
			 */
			bndYearSelectbox : function() {// used
				$workRoad.ui.log("$ssaMapFrame.ui.bndYearSelectbox - begin");
...
//					$("#company_year_theme").append("<option value="+year+">"+year+"년도</option>");
			...
			btnValidationCheck : function(curSelectedStatsType, api_id) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.btnValidationCheck");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				//총조사 주요지표
				if(curSelectedStatsType == "mainIndex") {
					var str = "";
...
//					}
			 ...
			 * @history 	 :
			 */
			commonDataList : function() {// used
				$workRoad.ui.log("$ssaMapFrame.ui.commonDataList - begin");
				
				// 임시 - 2018.10.31	ywKim	주석
//				$publicDataBoard.ui.leftMenuList($ssaMap.ui);		//공공데이터 목록
...
//				$publicDataBoard.ui.delegateSetting($ssaMap.ui);			//공공데이터 세팅
			 ...
			 */
			doMaxSize : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.doMaxSize");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				$ssaMap.ui.doMaxSize();
			},
			 ...
			 */
			doAddMap : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.doAddMap");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				$ssaMap.ui.doAddMap();
			},
			...
			doCombineMap : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.doCombineMap");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var mapId = parseInt($ssaMap.ui.curMapId) + 1;
				$ssaMap.ui.doCombineMap(mapId);
			},
			 ...
			 */
			doBookMark : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.doBookMark");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var mapId = parseInt($ssaMap.ui.curMapId) + 1;
				$ssaMap.ui.doBookMark(mapId);
			},
			 ...
			 */
			doShare : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.doShare");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var mapId = parseInt($ssaMap.ui.curMapId) + 1;
				$ssaMap.ui.doShare(mapId);
			},
			 ...
			 */
			doReport : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.doReport");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var mapId = parseInt($ssaMap.ui.curMapId) + 1;
				$ssaMap.ui.reportDataSet(mapId);
			},
			 ...
			 */
			doHelp : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.doHelp");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			updateToolTip : function(id, msg) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.updateToolTip");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					$("#"+id).tooltip({
				    content: msg
				});
			},
			...
			showNumberClick : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.showNumberClick");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var map_id = $ssaMap.ui.curMapId;
				var legend = $ssaMap.ui.mapList[map_id].legend;
				//통계표출 on일 경우 off로 변경
			...
			showNumberSetting : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.showNumberSetting");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var map_id = $ssaMap.ui.curMapId;
				//9월 서비스
				if ($ssaMap.ui.mapList[map_id] == null) {
					return;
				}
				var legend = $ssaMap.ui.mapList[map_id].legend;
				//통계표출 on일 경우
			...
			checkFusionItem : function(e, type) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.checkFusionItem");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			checkMustItem : function(e, el) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.checkMustItem");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				//체크된 라이오 버튼이 하나밖에 없을 때, 같은 라이오버튼을 누를때 항상 체크되도록 변경
			...
			sqlListBoxPosition : function(left) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.sqlListBoxPosition");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			sqlListBoxPosition2 : function(left) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.ui.sqlListBoxPosition2");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				if($("#menuAutoClose_radio").attr("checked") != "checked") {
					$ssaMapFrame.ui.sqlListBoxPosition(left);
					$ssaMapFrame.ui.sqlListBoxLeft = left;
				}else{
					$ssaMapFrame.ui.sqlListBoxPosition("0px");
					$ssaMapFrame.ui.sqlListBoxLeft = "0px";
				}
			}
	};
	...
	$ssaMapFrame.event = {
			...
			setUIEvent : function() {// used
				$workRoad.ui.log("$ssaMapFrame.event.setUIEvent - begin");
...
//				this.slideValue("houseBdspaceFrom_combine", "houseBdspaceTo_combine", "#slider-range3_combine", "㎡", 9); //2016.09.08 9월 추가분 
		        ...
		        body.on("click", ".themul > li", function() {
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[1]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//						$(".sideQuick.sq03").addClass("on");
////					}else{ 
////						$ssaMapFrame.event.stepCloseAnimate(1, "pass"); 
////						$(this).find("span").show();
...
//				}); 
				...
				//선택항목 클릭 시
				body.on("click",".sideQuick.sq03", function(){// used
					$workRoad.ui.log('$ssaMapFrame.event.setUIEvent.["click",".sideQuick.sq03"] - begin');
					...
					if(!on){
						...
						if (isDrag) {
							...
							$(".sqListBox.sq03").show();
							$(this).next(".sqListBox").stop().animate({"left":$ssaMapFrame.ui.sqlListBoxLeft},200);
						}
					}else{
						$(this).removeClass("on");
						if (isDrag) {
...
//						idx = 3;
//					}
//					$ssaMapFrame.event.stepCloseAnimate(idx, "pass"); 
...
//			    }); 
				...
				body.on("click","a.roundTextBox",function(e){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[3]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					var ck = $(this).hasClass("on"); 
					if(!ck){
						$(this).addClass("on");
					...
					if(fusionCk) {
						$ssaMapFrame.ui.checkFusionItem(e, "tab");
					}
			    });
				...
				body.on("click","div.roundTextBox",function(e){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[4]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					var ck = $(this).hasClass("on"); 
					if(!ck){
						$(this).addClass("on");
						$(this).children("input").prop("checked", true);
				...
				body.on("click","a.subRoundTextBox",function(){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[5]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				//주요지표 목록보기 - 해당분류 세부항목 선택하기하기
				body.on("click",".mainIndex_stepBox label",function(){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[6]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				body.on("click",".publicData_stepBox label",function(){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[7]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//					$publicDataBoard.ui.updatePublicData(type);
				...
				body.on("click",".radioStepBox label",function(e){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[8]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					//2016.08.23 권차욱 9월 서비스
					if ($(this).prev().attr("name") == "rd_combine_base") {
						if (!$ssaMapFrame.ui.checkFusionItem(e)) {
							return;
						}
						...
						if (!$ssaMapFrame.ui.checkMustItem(e, el)) {
							return;
						}
					}
					var ck = $(this).hasClass("on");
...
//					}
				...
				body.on("click",".radioStepOneBox label",function(){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[9]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					var ck = $(this).hasClass("on");
				...
				body.on("click",".radioStepOneBox2 label",function(){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[10]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					var ck = $(this).hasClass("on");
					$(this).parent().parent().find("label").removeClass("on");
					$(this).parent().parent().find("input").removeAttr("checked");
					if(!ck){
				...
				// 주석처리 - 2018.01.02	ywKim	변경: 멀티 체크박스 사용 안함
//				//다중 라디오버튼 선택
//				body.on("click",".multiCheckBox label",function(){
//					console.log("ssaMapFrame.js multiCheckBox clicked.");
//					//mng_s
//					if($ssaMapFrame.ui.isInnerMapShow2!=undefined && $ssaMapFrame.ui.isInnerMapShow2) {
//						//주택조건 탭이고 그리드일 경우 단일 선택한다. 가구조건에서는 다중선택임
//						if( $ssaMapFrame.ui.curSelectedDetailStatsType == "API_0306" ) {
//							$(this).parent().parent().find("label").removeClass("on");
//							$(this).parent().parent().find("input").removeAttr("checked");
//							$(this).addClass("on");
//							$(this).prev().attr("checked", "checked");
...
//			    });
				...
				body.on("click","#bottomService",function(){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[11]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					var ck = $(this).hasClass("on");
					if(!ck){
						$(this).addClass("on");
				...
				body.on("click",".hangjungArea .resizeIcon",function(){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[12]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				body.on("click",".subRadioStepBox label",function(){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[13]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				$('#companySearchText').keydown(function(e){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[14]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					if (e.keyCode == 13){	// when press ENTER key, accept the inputed value.
						$ssaMapFrame.ui.companySearch(0);
					}
				});
...
//				});
				$(".sqListBox > .sqTabs").dblclick(function() {
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[15]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				$("#mainIndex_year").change(function() {
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[16]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				$("#population_year").change(function() {
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[17]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					if (parseInt(this.value) > 2010) {
						...
						//mng_s
						if( $ssaMapFrame.ui.isInnerMapShow2!=undefined && $ssaMapFrame.ui.isInnerMapShow2 ) { //mng_s 그리드일 경우
							//교육정도별 및 혼인정도별 메뉴는 하이드 상태그래도 유지한다.
							$("#populationEduTab").hide();
							$("#populationMarryTab").hide();
						} else if( $ssaMapFrame.ui.isInnerMapShow3!=undefined && $ssaMapFrame.ui.isInnerMapShow3 ) { //mng_s 그리드일 경우
							//교육정도별 및 혼인정도별 메뉴는 하이드 상태그래도 유지한다.
							$("#populationEduTab").hide();
							$("#populationMarryTab").hide();
						} else {
				...
				$("#household_year").change(function() {
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[18]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					if (parseInt(this.value) > 2010) {
				...
				$("#house_year").change(function() {
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[19]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					if (parseInt(this.value) > 2010) {
						$("#householdOcptnTab").hide();
				...
				$("#population_year_combine").change(function() {
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.setUIEvent.[20]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					if (parseInt(this.value) > 2010) {
...
//			},
			 ...
			 */
			//2016.09.08 9월 추가분 
			slideValue : function(from, to, slider, etc, limit) {// used
				$workRoad.ui.log("$ssaMapFrame.event.slideValue - begin");
			    ...
			    $(domFrom).change(function(){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.slideValue.[1]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			    ...
			    $(domTo).change(function(){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.slideValue.[2]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			    ...
			    $(domTo).click(function(){
					$workRoad.ui.addFnc04List("$ssaMapFrame.event.slideValue.[3]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			    ...
			    $(domSlider).slider({
			        ...
			        //9월서비스 20160729 권차욱 수정
			        start : function(e, ui) {
						$workRoad.ui.addFnc04List("$ssaMapFrame.event.slideValue.[4]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			        	if (from == "populationAgeFrom" 		|| 
				        	from == "populationAgeFrom_combine" || 
				        	from == "3fAgeFrom"         		|| 
			        ...
			        stop : function(e, ui) {
						$workRoad.ui.addFnc04List("$ssaMapFrame.event.slideValue.[5]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			        	if (from == "populationAgeFrom" || from == "populationAgeFrom_combine" || from == "3fAgeFrom") {
			...
			dragAppend : function(selector, bgColor) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.event.dragAppend");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				$(selector).css("background-color", bgColor);
				$(selector).addClass("M_on");
			},
			dragAnimate : function(btn_id, bgColor) {
				$workRoad.ui.addFnc04List("$ssaMapFrame.event.dragAnimate");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var selector = $("#"+btn_id);
			...
			companyListTreeWidth : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.event.companyListTreeWidth");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			companyThemeTreeWidth : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.event.companyThemeTreeWidth");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			kosisTreeWidth : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.event.kosisTreeWidth");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			outputAreaSearch : function() {
				$workRoad.ui.addFnc04List("$ssaMapFrame.event.outputAreaSearch");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				if($ssaMapFrame.event.marker != null){
					$ssaMapFrame.event.marker.remove();
					$ssaMapFrame.event.marker = null;
				}
...
//			 			},500);
						...
						setTimeout(function(){
							map.geojson.eachLayer(function(layer) {
								if(layer.feature.properties.adm_cd == tot_reg_cd){
									$ssaMapFrame.event.marker = sop.marker([ result.x_coor, result.y_coor ]);		
									$ssaMapFrame.event.marker.addTo(map.gMap);
									...
									toolTip +=		"</table>";
									$ssaMapFrame.event.marker.bindToolTip(toolTip,{
										minWidth:200,
										maxWidth:200,
										maxHeight:500
									});
	...
	if ($workRoad.ui.isSetCallback('ssaMapFrame')) return;
	
	/** ********* 산업분류 검색 목록 Start ********* */
	$class("sop.portal.ssa.corpClassSearch.api.ssa").extend(sop.portal.absAPI).define({
		onSuccess : function (status, res, options) {
			...
			if(res.errCd == "0") {
	    		...
	    		$ssaMapFrame.ui.companyPaging(result.totalcount, $ssaMapFrame.ui.corpClassNum);
			} else {
				//총 개수
