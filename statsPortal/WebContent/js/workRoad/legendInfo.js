 ...
 * 범례에 관한 클래스
 * 
 * history : (주)웨이버스, 1.0, 2018/07/30 초기 작성 author : 손광련 version : 1.0 see :
 * 
 */
	...
	$legendInfo.ui = {
		slideInfo : function(){
			if(map.isInnerMapShow2 || map.isInnerMapShow3){ //mng_s map.isInnerMapShow2추가 20180219
				var slideValue2 = 0.8;			// 2018.12.27	ywKim	변경: 0.7 -> 0.8	
			}else{
				var slideValue1 = 0.8;			// 2018.12.27	ywKim	변경: 0.7 -> 0.8	
			}
		},
		/**
...
//					var colorList2 = getCalculColor("#a501b7", "#7c02cd", that.lv);
		...
		this.calEqualLegend = function(arData) {
			...
			for ( var k = 0; k < arData.length; k++) {
				if (arData[k].length == 1) {
					...
					if (result == 0 && min != max) {
						result = 'a';				//leekh a로 세팅하면 됨 범례가 1단위로 생성될 경우 오류를 수정함
					}
							
					var tmpResult = new Array();
					for ( var y=0; y <that.lv; y++) { //2016.04.04 범례
						if (result == 'a' && min != max) {	//leekh 범례오류수정 범례가 1단위로 생성될 경우 오류를 수정함
							tmpResult.push(1);				//leekh 범례오류수정 범례가 1단위로 생성될 경우 오류를 수정함
						}else {
							// 2016. 09. 23 SseOk 수정
		...
		this.getColor = function(value, valPerSlice) {
			...
			if (valPerSlice != undefined) {// 예외처리 추가 - 2019.01.16	ywKim	변경
		...
		this.userColorSetting = function(){
			...
			if (typeof $('.colorbarBox>a').wheelColorPicker != 'undefined') {// Validation 추가 - 2018.10.22	ywKim	변경
			...
			if (typeof $('.jumColorLink').wheelColorPicker != 'undefined') {// Validation 추가 - 2018.10.22	ywKim	변경
		...
		this.popClose = function(){
			$("body").on("click",".topbar>a, .hanClose",function(){
				...
				if(id=="guganSettingLayer"){
					...
					$("#legendDataSlider").slider("option","value",0.8);		// 2018.12.27	ywKim	변경: 0.7 -> 0.8
				}
			});
		}
		...
		this.createLegend = function() {
			...
			infoLegend.update = function(props) {
				...
				this._div.innerHTML = '<div data-html2canvas-ignore=true class="legendRing legendBox max" id="legendBox_'+that.id+'" data-ing="max">'
						...
						+colorList
						+'</ul>'
						+'<div class="heatMin" id="heatMinLegend_'+that.id +'" style="display:none;"><img src="/images/workRoad/ico/wheel.500.png" style="width:80px;height:80px;margin:14px 10px 10px 10px;"/></div>'
						...
						+'<img src="/images/workRoad/ico/ico_dbPlay.png" style="float:left;margin:4px 10px 0 10px;width:10px;height:10px;"/><div style="height:20px;font-size:11px;">반지름 조절 (Radius)</div>'
						...
						+'</ul>'
						+'</div>'
						+'<img src="/images/workRoad/ico/ico_dbPlay.png" width=10 height=10 style="float:left;margin:4px 10px 0 10px;width:10px;height:10px;"/><div style="height:20px;font-size:11px;">흐림도 조절 (Blur)</div>'
						+'<div style="height:50px;">'
			...
			$("#legendDataSlider").slider({
		        ...
		        min: 0.1,
		        max: 1,
		        value: 0.8,								// 2018.12.27	ywKim	변경: 0.7 -> 0.8
		        step : 0.1,
			...
			$("#initButton_"+that.id).click(function(){
				...
				$("#legendDataSlider").slider("option","value",0.8);			// 2018.12.27	ywKim	변경: 0.7 -> 0.8
				$(".sop-interactive").css('fill-opacity', 0.8);					// 2018.12.27	ywKim	변경: 0.7 -> 0.8
			});
...
function resizeColor(c01, c02, cls, max){  // 범례구간설정 색지정
	...
	if (typeof $.xcolor != 'undefined' && typeof $.xcolor.gradientlevel != 'undefined') {// Validation 추가 - 2018.10.22	ywKim	변경
