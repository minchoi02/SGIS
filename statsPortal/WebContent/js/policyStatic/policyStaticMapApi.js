...
//			local_URL : "/ServiceAPI/policyWrite/getLocalGovernmentPolygonData.json",
					...
					success : function(res) {
						...
						*/
						if(
							...
							$policyStaticMap.ui.setDemandYearListShowHide();
						}
			...
			reqSocData : function(parameter, options, map) { //생활SOC
				$.ajax({
					method: "POST",
					async: true,
					url: contextPath + "/view/soc/getDataList",
					data: {
						idx_id : $policyStaticMapLeftmenu.ui.idx_id,
						adm_cd : parameter.params.adm_cd,
						fac_cd: $("#fac-select-2 option:selected").val(),
						region_div : $("#boundLevelTitle option:selected").val(),
						dstnc_ctgry : $("#ctgry-select-2 option:selected").val(),
						map_id : map.id
					},
					dataType: "json",
					success: function(res) {
						if( res && res.result ){
							...
							if (result.length > 0) {
								...
								$policyStaticSocMap.ui.cur_fac_cl_nm = $("#fac-select-2 option:selected").text();
								
								if( $policyStaticMapLeftmenu.ui.idx_id == 'soc_ppltn' ){ //한번 호출로 왼쪽,오른쪽 지도 데이터 다 불러옴
									var sMap_param = eval("("+ $policyStaticMapLeftmenu.ui.arrParamList[map.id].map_param +")");
									
									if( map.id == 0 ){
										$policyStaticSocMap.ui.getPpltnName( result[0].ppltn_div );
										sMap_param.showTitle = $policyStaticSocMap.ui.cur_ppltn_div_nm;
									} else {
										sMap_param.showTitle = $policyStaticSocMap.ui.cur_fac_cl_nm;
									}
									
									$policyStaticMapLeftmenu.ui.arrParamList[map.id].map_param = JSON.stringify( sMap_param );
									$policyStaticMapApi.ui.setStatsData(res, parameter, options, map, $policyStaticSocMap.ui.boundaryData(res,parameter,options,map));
									
								} else {
									$policyStaticMapApi.ui.setStatsData(res, parameter, options, map, $policyStaticSocMap.ui.boundaryData(res,parameter,options,map));
								}
							} else {
								$policyStaticSocMap.ui.boundaryData(res,parameter,options,map);
							}
			...
			reqLocalStatsData : function(parameter, options, map, callback) {
				...
				//20201028
				if( $policyStaticMapLeftmenu.ui.idx_id == '3724842547201711201014253871792207583' ){
					parameter.params["bnd_year"] = dataYear;
				} else {
					parameter.params["bnd_year"] = bndYear;
				}
			...
			reqSocPoiData : function(parameter, options, map, callback) { //생활SOC
				$.ajax({
					url : contextPath + "/view/soc/getPoiList",
					type : "POST",
					data: {
						idx_id : $policyStaticMapLeftmenu.ui.idx_id,
						adm_cd : $policyStaticMap.ui.settingInfo['adm_cd'],
						fac_cd: $("#fac-select-2 option:selected").val(),
						region_div : $("#boundLevelTitle option:selected").val()
					},
					async : true,
					...
					success : function(res) {
						...
						parameter.adm_cd = $policyStaticMap.ui.settingInfo['adm_cd'];
						
						$policyStaticMapApi.ui.clear(map);
						$policyStaticMapApi.ui.setSocPoiData(res, parameter, options, map, callback, 'soc');
						$policyStaticMap.ui.doSetPoiSetting(parseInt(options.call_info_serial)-2, options.map_param);
						$policyStaticMapApi.loadingBar.close();
					},
					error : function(res) {
	...
	$policyStaticMapApi.ui = {
			...
			doSocData : function(options, map) { //생활SOC
				$policyStaticMapApi.request.reqSocData(
						deepCopy(options.param), //호출 파라미터
						options, 						//옵션정보
						map);
			},
			...
			doSocPoiData : function(options, map) { //생활SOC
				$policyStaticMapApi.request.reqSocPoiData(
						deepCopy(options.params), //호출 파라미터
						options, 						//옵션정보
						map, 							//맵정보
						function(res, options, map) { //callback
				...
				for (var i=0; i<result.length; i++) {
					if (!(result[i][parameter.filter] == "N/A"   ||	
					     (parameter.filter == "ppltn_dnsty"      ||	//인구밀도		
					      ...
					      parameter.filter == "val"   			 || //soc
					      parameter.filter == "avg_fmember_cnt"     //평균가구원
					     ) && ( result[i][parameter.filter] == "0" || typeof result[i][parameter.filter] == 'undefined') ) ) {
					...
					switch(parseInt($policyStaticMap.ui.settingInfo["idx_type"])) {
						...
						case 2: //통계연산형지표
							...
							for (var i=0; i<metaData.length; i++) {
								if ( (metaData[i].CHECK_TYPE == "3" ) || ( metaData[i].CHECK_TYPE == "1" ) ) {
			 ...
			 * @param callback : 콜백
			 */
			setCensusPoiData : function(res, parameter, options, map, callback, type) {
				var result = res.result;
			...
			setSocPoiData : function(res, parameter, options, map, callback, type) {
				var result = res.result;
				var featureLayer = null;
				var clusterLayer = null;
				var adm_cd = parameter.adm_cd;
			...
			setUserPoiData : function(res, parameter, options, map, callback) {
				...
				*/
				...
				for (var i=0; i<metaData.length; i++) {
					if (typeof metaData[i].CHECK_TYPE != "undefined" && Number( metaData[i].CHECK_TYPE ) > 0) {
						columns.push({ COLUMN : metaData[i].COL_ID, COLUMN_NM : metaData[i].COL_NM });
						//break;
					}
				}
				...
				for (var i=0; i<uploadData.length; i++) {
					...
					for( var j=0; j<columns.length; j++ ){
						title += ( columns[j].COLUMN_NM +" : "+ userData[ columns[j].COLUMN ] );
