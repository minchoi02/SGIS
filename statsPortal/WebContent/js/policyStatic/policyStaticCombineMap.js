	...
	$policyStaticCombineMap.ui = {
			...
			initCombineMap : function(delegate) {
				...
				if( $policyStaticMap.ui.soc_yn ){
					$policyStaticSocMap.ui.centerMove( $policyStaticCombineMap.ui.mapList[0] );
				}
			...
			getDemandIndexCombineData : function(mapList, type) {
				...
				for (var i=0; i<multiDataA.length; i++) {
					//2019-04-23 [김남민] 정책통계지도 > 좌측을 Tobe 우측을 Asis로 변경. (try 추가)
					try {
						//2019-04-23 [김남민] 정책통계지도 > 좌측을 Tobe 우측을 Asis로 변경. START
						var featuresB = multiDataA[i].layer.features;
						var featuresA = multiDataB[i].layer.features;
			...
			/**
			 * @name         : setGridHeaderForSort
			 * @description  : 표 헤더에 정렬 관련 설정을 한다.
			 * 				   이 설정으로 표 헤더를 클릭하면 정렬 기능 (오름차순, 내림차순)이 동작한다.
			 * @date         : 2018.06.13 
			 * @author	     : ywKim
			 * @history 	 :
			 * 			 2018.06.13	ywKim	신규 [v180611] - 엑셀다운/정렬 기능 추가
 			 * @param pHeaderType : 그리드 헤더 종류 : seq, adm_nm, adm_cd, data
			 *						기본값 : undefined / data
			 * @param pSortType : 정렬 종류 / 그리드 헤더 클릭시 사용
			 * 					  기본값 : undefined / desc
			 */
			setGridHeaderForSort : function (pHeaderType, pSortType) {
				
				var tag = ["seq", "adm_nm", "adm_cd", "data"];
				var subHeader = ["", "", "", ""]; // 헤더명 우측에 추가될 텍스트 (
				var headerType = (typeof pHeaderType !== 'undefined') ? pHeaderType : "data";
				var sortType = (typeof pSortType !== 'undefined') ? pSortType : "";
				var sortIcon = "";		// 헤더명에 추가될 텍스트 (현재 정렬을 알리는 표시 (▲ or ▼) (기본값: '')  
				var curIdx = 3;			// clicked column index
				
				// sortIcon 선택
				if (sortType == "asc") sortIcon = " ▲";
				else if (sortType == "desc") sortIcon = " ▼";
				
				// 클릭된 컬럼 인덱스 구하기
				if (headerType == tag[0]) {
					curIdx = 0; 
				} else if (headerType == tag[1]) {
					curIdx = 1; 
				} else if (headerType == tag[2]) {
					curIdx = 2; 
				} else {
					curIdx = 3;
				}
				
				// 클릭된 컬럼에 추가할 텍스트에 sortIcon 설정
				subHeader[curIdx] = sortIcon;
				
				// 루프 - 대상 테이블에 헤더 컬럼들
				$("tr.table_header").eq(0).find(".addSideLine").each(function(i){
					
					$(this).addClass("table_header");
					$(this).css("cursor", "pointer");
					$(this).attr("data-th", tag[i]);
					
					if (curIdx == i)
						$(this).attr("data-st", sortType);
					
					var header = $(this).text();
					$(this).text(header + subHeader[i]);
				});
			},
			...
			drawGrid : function(id, data, type) {
				...
				switch(type) {
					...
					case 2: //증감/증감률
						...
						html +=	"<tr class='table_header'>";// 2018.06.12	ywKim	 변경 - table_header 클래스는 [엑셀 내보내기]와 [정렬] 기능을 위해 사용됨 [v180611] - 엑셀다운/정렬 기능 추가					
//						html +=		"<tr>";
						...
						for (var i=0; i<data.length; i++) {
							html += "<tr class='combine_tr'>";
							html +=		"<td class='addSideLine'>"+ ((data[i].seq) ? data[i].seq : (i + 1) ) +"</td>";// 2018.06.14	ywKim	변경 - 정렬 기능을 위해 순번 기록 [v180611] - 엑셀다운/정렬 기능 추가
//							html +=		"<td class='addSideLine'>"+(i+1)+"</td>";
					...
					case 3:
						...
						html += 	"<col width='' />";
						html +=	"</colgroup>";
						html +=	"<tr class='table_header'>";// 2018.06.12	ywKim	 변경 - 엑셀 기능을 위함 [v180611] - 엑셀다운/정렬 기능 추가
//						html +=	"<tr>";
						html +=		"<th class='addSideLine'>순위</th>";
						html +=		"<th class='addSideLine'>지역명</th>";
						html +=		"<th class='addSideLine'>행정동코드</th>";
						...
						for (var k=0; k<data.length; k++) {
							...
							html += "<tr class='combine_tr'>";
							html +=		"<td class='addSideLine' rowspan='"+length+"'>"+ ((data[k].seq) ? data[k].seq : (k + 1) ) +"</td>";// 2018.06.14	ywKim	변경 - 정렬 기능을 위해 순번 기록 [v180611] - 엑셀다운/정렬 기능 추가
...
//				$("#dataA_origin").html(data1.source");		//원본소스
				if( data1.data_div != '6' ){
					...
					if (sCall_param2.year != null){
						...
						strCallParamYear2 = "(" + (sCall_param2.year.replace(/9016/g,"2016"))+ ")"
						//2019-04-22 [김남민] 정책통계지도 > 9차(8차), 10차 분리 적용. END
					}
					
					$("#dataB_origin").html(data2.source + strCallParamYear2);
				} else {
					$("#dataA_origin,#dataB_origin").html(data1.source);
				}
				...
				switch (parseInt(grade)) {
					...
					case 3:
						...
						setTimeout(function() {
							$("#gongDataArea").show();
							if(data2.idx_id == "9221137705201710301546072662443762660"){	//어린이집 분포 현황
								$("#gongDataArea").hide();
								$("#inst_title").html("보건복지부");
							}else if(data2.idx_id == "4567814844201710301633013080966785401"){	//어린이보호구역 분포 현황
								$("#gongDataAreaText").html("공공데이터포털 -> 본 자료는 경찰청의 자료를 활용하였으며, 누락된 지역이 있을 수 있습니다. 이점 활용에 유의하시기 바랍니다.");
								$("#inst_title").html("경찰청");
							}else if(data2.idx_id == "5075653758201711211705104277120963624"){	//재해위험지구 분포 현황
								$("#gongDataAreaText").html("본 자료는 행정안전부의 자료를 활용하였으며, 누락된 지역이 있을 수 있습니다. 이점 활용에 유의하시기 바랍니다..");
								$("#inst_title").html("행정안전부");
							}else if(data2.idx_id == "3074823551201711211711119354566751530"){	//민방위대피시설 분포 현황
								$("#gongDataArea").hide();
								$("#inst_title").html("행정안전부");
							}
						},1000);
				...
				switch (parseInt(settingInfo["idx_type"])) {
					...
					case 3: //시설분석형지표
						...
						for (var k=0; k<poiList.length; k++) {
							...
							//20200716 적은지역 순위에 poiCnt 0인 데이터 안나오게 수정
							var tmpRankData = {
								data : [],
								length : 0
							};
							...
							for (var i=0; i<combineData.combineDataInfo.length; i++) {
								...
								for (var v=0; v<poiList[k].data.length; v++) {
									//20200716 indexof로 하면 중간에 끼어있는 코드도 증가처리해서 수정
									//if (poiList[k].data[v].tot_reg_cd.indexOf(adm_cd) != -1) {
									if( poiList[k].data[v].tot_reg_cd.startsWith( adm_cd ) ) {
										poiCnt++;
									}
								}
								...
								if( tmpRankInfo.poiCnt > 0 ){
									tmpRankData['data'].push(tmpRankInfo);
								}
							}
							tmpRankData['length'] = tmpRankData['data'].length;
							arRanInfo.push(tmpRankData);
						}
						
						//데이터 정렬
						for (var i=0; i<arRanInfo.length; i++) {
							arRanInfo[i]['data'] = arRanInfo[i]['data'].sort(function(a, b) {
						...
						for (var k=0; k<arRanInfo.length; k++) {
							var arRankData = [];
							for (var i=0; i<listCnt; i++) {
								var maxRankData = arRanInfo[k]['data'][i];
								var minRankData = arRanInfo[k]['data'][arRanInfo[k]['length']-(1+i)];
							...
							//표 타이틀 설정
							var title = arRanInfo[k]['data'][0].title + " 대비 통계정보";
					...
					for (var k=0; k<featuresA.length; k++) {
						...
						if (dataA != null && dataB != null) {
							...
							switch(parseInt($policyStaticMap.ui.settingInfo["nomfrm_cd"])) {
								...
								case 4: //나누기(/)
									if( $policyStaticMap.ui.soc_yn ){
										combineData = Math.round((tmpDataA / tmpDataB));
									} else {
										combineData = parseFloat((tmpDataA / tmpDataB).toFixed(1));
									}
									break;
...
//				this.dispUnit = "";
			...
			changeChartAndGrid : function(idx) {
				switch(idx) {
					case 1: //차트영역
						// 2018.06.12	ywKim	추가 [v180611] - 엑셀다운/정렬 기능 추가
						// 엑셀 버튼 숨기기
						$("#btn_excelDownload").closest('div').css("margin-top", "");
						document.getElementById("btn_excelDownload").style.display = 'none';
					...
					case 2: //표영역
						// 2018.06.12	ywKim	추가 [v180611] - 엑셀다운/정렬 기능 추가
						// 엑셀 버튼 보이기
						document.getElementById("btn_excelDownload").style.display = 'block';// 제이쿼리 사용하면 오류 발생
						$("#btn_excelDownload").closest('div').css("margin-top", "-50px");
			 ...
			 * @param pHeaderType : 그리드 헤더 종류 : seq, adm_nm, adm_cd, data
			 *						기본값 : undefined / data
			 *						그리드 헤더 클릭시 사용 // 2018.06.13	ywKim	추가 [v180611] - 엑셀다운/정렬 기능 추가
			 * @param pSortType : 정렬 종류 / 그리드 헤더 클릭시 사용
			 * 					  기본값 : undefined / desc
			 * 					  그리드 헤더 클릭시 사용 // 2018.06.13	ywKim	추가 [v180611] - 엑셀다운/정렬 기능 추가
			 */
			setChart : function(isRefesh, pHeaderType, pSortType) {

				// 2018.06.13	ywKim	추가 - 정렬기능을 위함 [v180611] - 엑셀다운/정렬 기능 추가
				headerType = (typeof pHeaderType !== 'undefined') ? pHeaderType : 'data';
				sortType = (typeof pSortType !== 'undefined') ? pSortType : 'data';
				...
				switch (parseInt(settingInfo["idx_type"])) {
					case 1: //수요변화지표
						...
						// 2018.06.13	ywKim	추가 [v180611] - 엑셀다운/정렬 기능 추가
						if (typeof pHeaderType !== 'undefined') {// 최초 한번은 정렬하면 안됨
							this.sortChartByUser(combineData.combineDataInfo, pHeaderType, pSortType);
						}
						...
						this.setGridHeaderForSort(pHeaderType, pSortType);// 2018.06.13	ywKim	추가 : 표 헤더에 정렬 관련 설정을 한다. [v180611] - 엑셀다운/정렬 기능 추가
					...
					case 2: //통계연산형
						...
						// 2018.06.14	ywKim	추가 [v180611] - 엑셀다운/정렬 기능 추가
						if (typeof pHeaderType !== 'undefined') {// 최초 한번은 정렬하면 안됨
							this.sortChartByUser(combineData.combineDataInfo, pHeaderType, pSortType);
						}
						...
						this.setGridHeaderForSort(pHeaderType, pSortType);// 2018.06.14	ywKim	추가: 표 헤더에 정렬 관련 설정을 한다.  [v180611] - 엑셀다운/정렬 기능 추가
					...
					case 3: //시설분석형
						...
						// 2018.06.14	ywKim	추가 [v180611] - 엑셀다운/정렬 기능 추가
						if (typeof pHeaderType !== 'undefined') {// 최초 한번은 정렬하면 안됨
							this.sortChartByUser(chartList, pHeaderType, pSortType);
						}
			...
			/**
			 * 
			 * @name         : sortChartByUser
			 * @description  : 그리드 헤더를 클릭했을때 정렬하기
			 * @date         : 2018.06.13 
			 * @author	     : ywKim
			 * @history 	 :
			 * 			2018.06.13	ywKim	신규 [v180611] - 엑셀다운/정렬 기능 추가
			 * @param data : 정렬데이터 소스 
			 * @param pHeaderType : 그리드 헤더 종류
			 * @param pSortType : 정렬 종류 / 그리드 헤더 클릭시 사용
			 * 					  기본값 : undefined / desc
			 * 					  그리드 헤더 클릭시 사용
			 */
			sortChartByUser : function(pData, pHeaderType, pSortType) {
				// 순위 설정
				// 정렬을 하게 되면 순위가 달라지므로 이곳에서 순위 설정하는 것이 옳다.
				...
				if (pSortType == "asc") {
					...
					case 'adm_nm':
						pData = pData.sort(function(a, b) {
							if (a.adm_nm.toString().toUpperCase() > b.adm_nm.toString().toUpperCase()) return 1;
							if (b.adm_nm.toString().toUpperCase() > a.adm_nm.toString().toUpperCase()) return -1;
							return 0;
						});
						break;
					case 'adm_cd':							
						pData = pData.sort(function(a, b) {
							return parseFloat(a.adm_cd) - parseFloat(b.adm_cd);
						});
						break;
					default:
						if (pData && pData.length > 0 && pData[0].poiData && pData[0].poiData.length > 0) {//시설분석형 그리드 대응
							pData = pData.sort(function(a, b) {
								return parseFloat(a.poiData[0].data) - parseFloat(b.poiData[0].data);
							});
						} else {
							pData = pData.sort(function(a, b) {
					...
					case 'adm_cd':							
						pData = pData.sort(function(a, b) {
							return parseFloat(b.adm_cd) - parseFloat(a.adm_cd);
						});
						break;
					default:
						if (pData && pData.length > 0 && pData[0].poiData && pData[0].poiData.length > 0) {//시설분석형 그리드 대응
							pData = pData.sort(function(a, b) {
								return parseFloat(b.poiData[0].data) - parseFloat(a.poiData[0].data);
							});
						} else {
							pData = pData.sort(function(a, b) {
								return parseFloat(b.data) - parseFloat(a.data); //2018.01.09 [개발팀] sort변경
			...
			censusDataTooltip : function(data, title, unit) {
				var html = "<table style='margin:10px;"+($policyStaticMap.ui.soc_yn?"min-width:150px;":"")+"'>";
			...
			doSavePolicyStatic : function() {
				...
				srvLogWrite( "E0", "05", "16", "03", $("#pCtgrSelectBox option:selected").text(), $policyStaticMap.ui.getLogParams() ); // jrj 로그 > 수정
...
//				$(".DataBoxScroll").mCustomScrollbar({axis:"y"});
				...
				$("body").on("click", ".pcTabs li>a", function() {
					...
					switch (id) {
						...
						case "combineDataTab": //융합결과탭
							...
							srvLogWrite( "E0", "04", "05", ( $( "#combineTypeBox a.on" ).index() == 0 ? "02" : "03" )
									, $policyStaticMap.ui.settingInfo.policy_idx_nm, $policyStaticMap.ui.getLogParams() ); // jrj 로그 > 융합하기 융합결과 데이터 차트,표
				...
				$("body").on("click","#combineTypeBox>a",function(){ 
					...
					srvLogWrite( "E0", "04", "05", ( $( this ).index() == 0 ? "02" : "03" )
							, $policyStaticMap.ui.settingInfo.policy_idx_nm, $policyStaticMap.ui.getLogParams() ); // jrj 로그 > 융합하기 융합결과 데이터 차트,표
			    });
							...
							btns = [
								{
									...
									disable : false,
									func : function(opt) {
										srvLogWrite( "E0", "05", "16", "04", $("#pCtgrSelectBox option:selected").text(), $policyStaticMap.ui.getLogParams() ); // jrj 로그 > 삭제
				...
				// 데이터보기 엑셀 다운로드 - 엑셀버튼 클릭
				// 2018.06.12	ywKim 추가 [v180611] - 엑셀다운/정렬 기능 추가
				$("body").on("click", ".btn_excelDownload", function() {

					// 동적 Form 생성 (폼을 미리 디자인해 놓은 오류 발생)
				    var myForm = document.getElementById("excelDownForm");				    
				    if (myForm == null) {
				    	myForm = document.createElement("form");
				    	document.body.appendChild(myForm);
				    }				    
				    myForm.setAttribute("method", "post");
				    myForm.setAttribute("id", "excelDownForm");
				    myForm.setAttribute("name", "excelDownForm");				    
					...
					$("tr.table_header").eq(0).find(".addSideLine").each(function(){
					...
					// 테이블 데이터 ROW 추가
					if (parseInt($policyStaticMap.ui.settingInfo["idx_type"]) == 3) { // 시설분석형 / 값이 2Row
						// 시설분석형인 경우 값이 Multi Row 이기 때문에 2번째 값부터는 1번째 값의 우측으로 배치한다.
						
						var maxRowSpan = 1;	// 가정: 값 이외의 헤더는 rowSpan이 동일하다
						var contentData = [];
						
						$("tr.table_header").eq(0).closest("tbody").find("tr:not(.table_header)").each(function(){
					...
					var excelDataElement = document.createElement("input");
					excelDataElement.type = "hidden";
					excelDataElement.name = "isLocal";
					excelDataElement.value = (location.hostname === "localhost" || location.hostname === "127.0.0.1") ? "Y" : "N";
					myForm.appendChild(excelDataElement);
					
					var url = "/view/map/excelDown";
				...
				// 정렬을 위해 그리드 컬럼 헤더를 클릭함.
				// 2018.06.13	ywKim	추가 [v180611] - 엑셀다운/정렬 기능 추가
				$("body").on("click", ".addSideLine.table_header", function() {
					
					var text = $(this).text();
					var headerType = $(this).attr('data-th');
					var sortType = "desc";
					
					if ($(this).attr('data-st')) {
						sortType = $(this).attr('data-st');
					}
					...
					$policyStaticCombineMap.ui.setChart(false, headerType, sortType);
				});
			},
