...
(function(W, D) {
	...
	$(document).ready(function() {
		...
		//mng_s 20200220 e-지방지표의 데이터보드에서 표로 볼때 익스에서 않나오는 현상때문에 함수 재정의
		...
		//mng_s 20220112 이진호
		//location.href.match('/interactiveMap/population?') 삭제
		//SGIS4_1124_공유데이터인 경우 다중시계열 x 시작
		//if(location.href.match('/interactiveMap/population?') || location.href.match('interactiveMap/sharedata?') || location.href.match('interactiveMap/bookmark?')){
		if(location.href.match('interactiveMap/sharedata?') || location.href.match('interactiveMap/bookmark?')){
			$("#multiTimeSeries").hide();
		}
		//SGIS4_1124_공유데이터인 경우 다중시계열 x 끝
		//mng_e 20220112 이진호
	});
	...
	$interactiveDataBoard.ui = {
			...
			fixedLegend : null,
			dataSumValue : 0,
			rollBackCnt : 0,		//20190315 leekh 시계열 집계구 플레이시 오류 수정
...
//					$("#viewTimeSeriesData_dt_area a").removeClass("on");
				...
				//2020년수정변경 시작 (ggm)
				if($interactiveMap.ui.chkIfInteractive() && options.params["is_non_self"] == "Y" && options.params.map.boundLevel == "2") {		//20200417 수정 (ggm)
					// 비자치구를 포함한 시로 2레벨 조회일 때
					this.reqAdmListForBorough(res, options);	//행정동 리스트가져오기
				}else{
			...
			updateInfo : function(res, options) {
				...
				for(var i = 0; i < options.params.param.length; i ++) {
					if(options.params.param[i].key == "year") {
						if(options.params.param[i].value != "9016"){
				...
				if (year.length > 0) {
					title = options.params.title + " (" + year + " " + addText +  ")";
				}else {
					title = options.params.title;
				}
				//2020년수정변경 시작 (ggm)
				var paramsAdmNm = options.params.adm_nm;
				if($interactiveMap.ui.chkIfInteractive()) {		//20200417 수정 (ggm)					
					if(options.params["is_non_self"] == "Y" && options.params.map.boundLevel == "1") {
						// zoom 4 -> 5 이면
						paramsAdmNm = options.params.map.curSidoNm;
					}
				}				
				//2020년수정변경 끝				
...
//				$("#dataBoardArea").html("지역 : " + options.params.adm_nm);
				//2020년수정변경 시작 (ggm)
				$(".dataBoardArea").html("지역 : " + paramsAdmNm);
				//2020년수정변경 끝				
...
//				$("#fullDataBoardTitle").find("#dataBoardArea").html("지역 : " + options.params.adm_nm);
				//2020년수정변경 시작 (ggm)
				$("#fullDataBoardTitle").find(".dataBoardArea").html("지역 : " + paramsAdmNm);
				//2020년수정변경 끝				
...
//					$("#fullDataBoardTitle").find("#dataBoardOrigin").html("출처 : " + origin);
					...
					$interactiveMap.ui.multiTimeorigin = origin; // SGIS4_1028_다중시계열_출처추가
				}
			...
			updateInfoKosis : function(title, value) {
				...
				if( $interactiveLeftMenu.ui.curSelectedStatsType == 'ecountry' ){
					//$("#fullDataBoardTitle").find(".dataBoardOrigin").html("출처 : KOSIS (<a style='color: blue; text-decoration:underline;' href='javascript:$interactiveMapEcountry.kosisLinked();'>통계표보기</a>)");
					$(".dataBoardStats").html("출처 : KOSIS (<a style='color: blue; text-decoration:underline;' href='javascript:$interactiveMapEcountry.kosisLinked();'>통계표보기</a>)");
				} else {
			...
			updateTargetAreaChartKosis : function(res, title) {
				...
				if(res.length > 0) {
					for(var i = 0; i < res.length; i ++) {
						...
						if(i == res.length - 1 && elem.UNIT != undefined && elem.UNIT != "" && elem.UNIT != null) {
							sumDataValue = sumDataValue.toFixed(1) + "	(" + elem.UNIT + ")";
//							$('#th_result_val').text("값("+ elem.UNIT +")");
							$('#th_result').text("값("+ elem.UNIT +")");
						}else if(elem.UNIT == "" || elem.UNIT == undefined || elem.UNIT == null){
							$('#th_result').text('값');
						}
					}
				}
			...
			updateTargetAreaTable : function(res, options) {
				...
				/*
				var dataListRollBack;		//leekh 20190315 집계구 _1 쌓이는 현상 수정을 위한 변수
				
				if(dataListRollBack != undefined){
					dataListRollBack = dataList;		// //leekh 20190315 집계구 _1 쌓이는 현상 수정 
				}
				*/
				...
				if(res.result2 == null){
					...
					resLength = res.result.length + res.result2.length;;
				}
				
				//for(var i = 0; i < res.result.length + res.result2.length; i ++) {
				for(var i = 0; i < resLength; i ++) {	//20190318 갤러리에서 조회했을때 오류 수정
					var elem = [];
					if(i<res.result.length){
						...
						if(elem.adm_cd.length > 7){
							if($interactiveDataBoard.ui.rollBackCnt == 0){	////20190315 leekh 시계열 집계구 플레이시 오류 수정
					...
					if(res.id == "API_0301") {	//인구통계총괄
						if (res.showData == "tot_ppltn") {
							...
							$("#dataBoardStatsSum").html("");
							//$("#dataBoardStatsSum").html("합계 : " + appendCommaToNumber($interactiveDataBoard.ui.dataSumValue));//2019-03-14 박길섭
							retDataList.push(parseFloat(elem.avg_fmember_cnt));
				...
				$interactiveDataBoard.ui.rollBackCnt++;		//20190315 leekh 시계열 집계구 플레이시 오류 수정
				...
				if(res.result2 != undefined){
					result2RollBack = res.result2;		// //leekh 20190315 집계구 _1 쌓이는 현상 수정 
				}
				...
				for(var  i = 0; i < dataList.length; i ++) {
					...
					//mng_s 20180817 행정구역 그리드일 경우 항목 및 집계구번호에서 항목을 없애고 격자번호로 표기
				...
				for(var  i = 0; i < dataList.length; i ++) {
					...
					if( typeof value != 'undefined' && !isNaN( value ) ){
						if( Number.isInteger( value ) ){
							value = appendCommaToNumber( parseInt( value ) );
						} else {
							value = appendCommaToNumber( value.toFixed(2) );// + '' + value
						}
					} else {
						value = 0;
					}
					...
					if(location.href.match('interactiveMap/ecountryView') == -1){
			...
			updatePieChart : function(options) {
				...
				for (var i=0; i<options.params.param.length; i++) {
					...
					if(options.params.param[i].key == "is_zoom_lvl4"){
						p_is_zoom_lvl4 = options.params.param[i].value;
					}else if(options.params.param[i].key == "is_non_self"){
						p_is_non_self = options.params.param[i].value;
					}
				}
				
				if(options.params.api_id != "API_0303" && p_is_zoom_lvl4 == "" && p_is_non_self == ""){
					if(options.params["is_zoom_lvl4"] == "Y"){
						sopPortalPieChartDrawObj.addParam("is_zoom_lvl4", "Y");				
					}
					if(options.params["is_non_self"] == "Y"){
						sopPortalPieChartDrawObj.addParam("is_non_self", "Y");				
					}
				}
				//20200427 수정 끝
			 ...
			 * @history 	 :
			 */
			timeSeriesInit : function(params) {		/// param.maxyear 		srv_dt_statbaseyearinfo 테이블에서 가져옴	leekh
				
				console.log("[interactiveDataBoard.js] timeSeriesInit() 호출");
				console.log("[interactiveDataBoard.js] 데이터보드의 시계열 조희시 년도가 않보일때는 srv_dt_statbaseyearinfo 테이블을 수정해야함.");
				console.log("[interactiveDataBoard.js] 데이터보드의 시계열 조희시 년도가 않보일때는 srv_dt_statbaseyearinfo 테이블을 수정해야함.");
				console.log("[interactiveDataBoard.js] 데이터보드의 시계열 조희시 년도가 않보일때는 srv_dt_statbaseyearinfo 테이블을 수정해야함.");
				console.log("[interactiveDataBoard.js] 데이터보드의 시계열 조희시 년도가 않보일때는 srv_dt_statbaseyearinfo 테이블을 수정해야함.");
				console.log("[interactiveDataBoard.js] 데이터보드의 시계열 조희시 년도가 않보일때는 srv_dt_statbaseyearinfo 테이블을 수정해야함.");
...
//				var dataBoard = this.mapData[this.map_id].dataBoard;
				...
				if(params.api_id == "API_0308" && params.filter == "forestry_cnt") {
					...
					//mng_s 20211019 이진호, maxYear 값 변경 / 2021 센서스 반영
					//maxYear = date.getFullYear()-2;
					maxYear = 2020;
					//mng_e 20211019 이진호
...
//				}
				...
				if((params.api_id == "API_0304") || (params.api_id == "API_0301" && params.filter == "corp_cnt")) {
					...
					for(var year = minYear; year < date.getFullYear(); year ++) {
						...
						//mng_s 20201027 이진호, 2020센서스 데이터 추가, year=="2019"
						}else if(year == "2016"||year == "2017"||year == "2018" ||year == "2019"){	//leekh 20191024 2016년부턴 1년 단위라서 센서스데이터 반영시 1년씩 추가해준다
							if(params.api_id == "API_0307" || params.api_id == "API_0308" || params.api_id == "API_0309" || params.api_id == "API_0310"  ) {
								//농가 임가 어가는 2016년을 추가하지 않는다
								//농가 임가 어가는 2017년을 추가하지 않는다
							}else{
								dataBoard.selectedYearArray.push(year);
							}
						}
			...
			updateTimeSeries : function(options) {
				//srvLogWrite("C0", "03", "04", "00", "", "");		//데이터 보기 - 차트
				var dataBoard = this.mapData[this.map_id].dataBoard;
				var date = new Date();
				var minYear = 2000;		//최소년도 기본 2000년
				//var maxYear = date.getFullYear()-2;		//재작년도를 maxYear로 잡는다.
				
				//mng_s 20211020 이진호 maxYear 을 현재년도 -1로 수정
				...
				//mng_s 20211019 이진호, 2021 센서스 반영 (농림어업 2020년도 들어옴에 따른 maxYear 수정) 및 조건문에  options.params.filter 조건 추가
				if(options.params.api_id == "API_0307" && options.params.filter == "farm_cnt"){
					//maxYear= date.getFullYear()-2;
					maxYear= 2020;
				}
				//mng_e 20211019 이진호
				...
				if(options.params.api_id == "API_0310" && options.params.filter == "population"){
					maxYear= 2020;
				}
				
				//mng_s 20210510 이진호
				//농림어업총조사 > 임가 minYear 값 조정
				//fullDataBoard 의 시계열에서 년도가 2000년부터 나와서 minYear 값을 2005 로 조정하였음
				if(options.params.api_id == "API_0308" && options.params.filter == "forestry_cnt") {
					maxYear = 2020;
				}
				...
				if (options.params.param != undefined && options.params.param.length > 0) {
					for (var i=0; i<options.params.param.length; i++) {
						...
						} else if (options.params.param[i].key == "edu_level"){
							key = options.params.param[i].key;
						} else if (options.params.param[i].key == "mrg_state"){
							key = options.params.param[i].key;
						} else if (options.params.param[i].key == "ocptn_type"){
							key = options.params.param[i].key;
							
						//mng_s 20220112 이진호, '인구주택총조사-주택조건에서 건축년도 포함' 및 '결합조건'일 때 시계열 조회 못하도록 수정
						} else if (options.params.param[i].key == "combine_base"){
							isTimeSerieseHide = true;
						} else if (options.params.param[i].key == "const_year"){
							isTimeSerieseHide = true;
						}
						//mng_e 20220112 이진호
				...
				if((options.params.api_id == "API_0304") || (options.params.api_id == "API_0301" && options.params.filter == "corp_cnt")) {
					...
					// 사업체의 경우 데이터보드의 시계열 조회 2015년 항목 삭제		// leekh 전국 사업체조사 시계열은 srv_dt_statbaseyearinfo에서 수정
						...
						if(base_year <= 2005) {
							...
							maxYear = "2005";
							dashText = "2006년 이후 자료를 보시기 위해서는 조사년도를 다시 선택해 주십시오. (2000~2005: 8차 산업분류, 2006~2016: 9차 산업분류, 2017년~현재 : 10차)";
						} else if(base_year > 2005 && base_year <= 2016) {
							minYear = "2006";
							maxYear = "2016";
							dashText = "2005년 이전 자료를 보시기 위해서는 조사년도를 다시 선택해 주십시오. (2000~2005: 8차 산업분류, 2006~2016: 9차 산업분류, 2017년~현재 : 10차)";
						} else if(base_year > 2016) {
							swt10ChaYn = true;
							minYear = "2016";
							
							//mng_s 20200227 이진호 - maxYear을 동적으로 받기 위해 수정
							//maxYear은 이제 common.js에서 companyDataYear의 값을 따라감
							maxYear = companyDataYear;
							//mng_e 20200227 이진호
							
							dashText = "2016년 이전 자료를 보시기 위해서는 조사년도를 다시 선택해 주십시오. (2000~2005: 8차 산업분류, 2006~2016: 9차 산업분류, 2017년~현재 : 10차)";
						}
					}
					// mng_s 2017. 12. 05 j.h.Seok
					else {
					...
					for(var year = minYear; year <= maxYear; year ++) {
						...
						var checkYear = year
						if(swt10ChaYn && year == "2016"){
							checkYear = "9016";
						}
						
						html += "<li id='li_y"+year+"'>";
						html += "	<input type='checkbox' id='y"+year+"' value='"+checkYear+"' class='year_input' />";
						html +=	"   <input type='radio' name='fixed_legend' value='"+checkYear+"' class='year_radio' style='display:block;'/>";	
						html += "	<div id='div_y"+year+"'>"+year+"</div>";
						html += "</li>";
						
						//데이터보드 확장일 경우
						html2 += "<li id='full_li_y"+year+"'>";
						html2 += 	year + "<input type='hidden' id='full_li_y"+year+"' value='"+checkYear+"' />";
						html2 += "</li>";
					...
					*/
					...
					//mng_s 20211020 이진호, 2021 센서스 반영
					//농림어업총조사 > 임가 > 가구별 성별 선택 및 해당 가구원 연령 선택 후 조회 시  데이터보드 확장 화면에서 시계열 버튼 2000년도 보이는 문제 수정
					if(options.params.api_id == "API_0310" && options.params.filter == "population" && options.params.title.substr(0,2)== "임가") {
						minYear = 2005;
					}
					//총조사 주요지표에서 임가 또는 임가인구_계(명) 선택 조회 시 2000년도 시계열 버튼이 보이는 문제 수정
					if((options.params.api_id == "API_0301" && options.params.filter == "imga_cnt") || (options.params.api_id == "API_0301" && options.params.filter == "imga_ppltn")) {
						minYear = 2005;
					}
					///mng_e 20211020 이진호
					
					//mng_s 20200320 이진호
					//인구주택총조사에서 인구조건과 가구조건에서
					//인구조건에 2010년 이하 선택 후 교육정도별 또는 혼인정도별의 조건을 주면 데이터 보드에 2000년도 부터 2010년까지만 출력되게 수정
					//가구조건에서 2010년 이하 선택 후 점유형태 조건을 주면 데이터 보드에 2000년도 부터 2010년 까지만 출력되게 수정
					for(var year = minYear; year < date.getFullYear(); year ++) {
						if((year % 5) == 0) {
							if(key == "edu_level" || key == "mrg_state" || key == "ocptn_type"){
								if(year <= 2010) {
									viewMaxYear = year;
									html += "<li id='li_y"+year+"'>";
							...
							//mng_s 20201027 이진호, 2020년 센서스 추가, year=="2019"
						}else if((year % 5) != 0 && year == "2016" || year=="2017" || year=="2018" || year=="2019"){
							var option_filter = options.params.filter;
							//mng_s 20211018 이진호, 2021 센서스 반영 및 총조사 주요지표 > 농가~해수면어가 지표선택시 > 데이터보드 > 2016,2017, 2018, 2019 년 시계열 버튼 안나오게 수정
							//mng_s 20210506 이진호, 대화형 통계지도 > 농림어업 총조사 > 풀데이터 보드 일 때 시계열 년도가 2016,2017,2018,2019 가 나오면 안되는데 나와서 수정
							//if 조건문에 farm_cnt(농가), forestry_cnt(임가), fishery_cnt(어가) 아닐 때 추가
							if(key != "edu_level" && key != "mrg_state" && key != "ocptn_type" && option_filter != "farm_cnt" && option_filter != "forestry_cnt" && option_filter != "fishery_cnt" 
									&& option_filter != "nongga_cnt" && option_filter != "nongga_ppltn" && (year != "2000" && option_filter != "imga_cnt" && option_filter != "imga_ppltn") && option_filter != "naesuoga_cnt" && option_filter != "naesuoga_ppltn" && option_filter != "haesuoga_cnt" && option_filter != "haesuoga_ppltn"
										&& options.params.api_id != "API_0310"){
								//2019 센서스 데이터 작업추가(year==2018추가)
								console.log("interactiveDataBoard.js maxYear2 : " + maxYear);
								viewMaxYear = year;
								html += "<li id='li_y"+year+"'>";
								html += "	<input type='checkbox' id='y"+year+"' value='"+year+"' class='year_input' />";
				...
				html += "	</ul>";
				html += "	<div style='color:#26415c; font-size:15px;position:absolute;top:130px;margin-left:140px;width:300px'>"+dashText+"</div>";
				$("#tableTimeSeries").append(html);
				
				//mng_s 20210518 이진호, 
				//사업체 테마업종 분류가 아니면서 2005년 이하 검색시 시계열 조회 하단 문구(사업체 차수 설명) 위치 수정
				if(!isThemeCd && base_year <= 2005){
					$("#tableTimeSeries").children('div').css({
						'margin-left' : '130px',
						'top' : '65px'
					});
				}
				...
				if($("#y2016").val() == "9016"){
					$("#div_y2016").mouseover(function(e){		//과장님 지시사항임
						//$(this).append('<span id="tooltip9016">시계열 조회를 위해 10차 산업분류로 재분류 하였음</span>')
						
					//	var html = "<table style='margin-top:-50px; margin-left:60px; width:130px; background-color:white; position:absolute;' id='tooltip9016'>";
					//	html += "<tr><td  style='font-size: 12px; font-weight:bold; color:#3792DE;'>시계열 조회를 위해 10차산업분류로 재분류 하였음</td></tr>";
					//	html += "</table>"
						
						var html = "<span style='height: 52px; border: 1px solid lightgray; font-size:12px; margin-top:-50px; margin-left:20px; width:130px; background-color:white; position:absolute;' id='tooltip9016'></br>시계열 조회를 위해 10차</br>산업분류로 재분류 하였음</span>";
							
						$("#li_y2016").append(html);
						
						//$(this).attr('title', '시계열 조회를 위해 10차 산업분류로 재분류 하였음');
					}).mousemove(function(e){
				//		$("#tooltip9016").css('top', e.pageY + 10);
				//		$("#tooltip9016").css('left', e.pageX + 10);
					}).mouseout(function(){
						$("#tooltip9016").remove();
				...
				for(var i = 0; i < dataBoard.selectedYearArray.length; i ++) {
					for(var x = 0; x < dataBoard.yearArray.length; x ++) {
						if(dataBoard.selectedYearArray[i] == dataBoard.yearArray[x] || (dataBoard.selectedYearArray[i] == "9016" && dataBoard.yearArray[x] == "2016" )) {
							
							
							
							if(dataBoard.selectedYearArray[i] == "9016"){
								$("#li_y2016").show();
								$("#li_y2016").find("input").prop("checked", true);
							}
...
//				(dataBoard.selectSeriesYear == undefined || dataBoard.selectSeriesYear == "")) {
				if (base_year != undefined && base_year != "") {
					var selYear = base_year;
					if(base_year == "9016"){
						selYear = "2016";
					}
					
					$("#li_y"+selYear).addClass("on");
...
//				$("#fullDataBoardTitle").find("#dataBoardStats").html("");
			...
			//2020년수정변경 시작 (ggm)
			/**
			 * @name         : reqAdmListForBorough
			 * @description  : 비자치구 관련 행정동정보 가져오기(reqAdmList와 동일하고 sgg_cd 사용만 다름) 
			 * @date         : 2020. 04. 08.
			 * @param	     : res, options  
			 * @history 	 :
			 */
			reqAdmListForBorough : function(res, options) {
					...
					if (res.result[0].adm_cd.length > 7) {
						if (this.map.curPolygonCode == 3) {
							if (options.params.adm_cd != null && options.params.adm_cd != undefined) {
								$.ajax({
									type: "POST",
									url : contextPath + "/ServiceAPI/map/admAddressListForBorough.json",
									data : {
										sido_cd : options.params.adm_cd.substring(0,2),
										sgg_cd : options.params.adm_cd.substring(2,5),
										base_year : bndYear,
										is_non_self : options.params.is_non_self
									},
									async : false,
										...
										if (tmpRes.errCd == "0") {
											...
											for (var i=0; i<res.result.length; i++) {
												for(var k=0; k<admNmaList.length; k++) {													
													//지역코드 5번째 자리를 '0'으로 바꿔서 비교
													//if (res.result[i].adm_cd.substring(0,7) == options.params.adm_cd +  admNmaList[k].emdong_cd) {
													if ((res.result[i].adm_cd.substring(0,4) + "0" + res.result[i].adm_cd.substring(5,7)) == options.params.adm_cd +  admNmaList[k].emdong_cd) {
				...
				if (res.result != undefined && res.result.length > 0) {
					if (res.result[0].adm_cd.length > 7) {
						if (this.map.curPolygonCode == 3) {
							...
							} else { //mng_s 20200221 통계로에서 대화형 통계지도로 넘오왔을때 통계표에서 동이름이 undefined로 찍혀서 수정
									...
									$.ajax({
										...
										success: function(tmpRes) {
											if (tmpRes.errCd == "0") {
												...
												for (var i=0; i<res.result.length; i++) {
													for(var k=0; k<admNmaList.length; k++) {
														if (res.result[i].adm_cd.substring(0,7) == options.params.adm_cd.substring(0,5) +  admNmaList[k].emdong_cd) {
														//if (res.result[i].adm_cd.substring(0,7) == options.params.adm_cd) {
															res.result[i]["adm_nm"] =  admNmaList[k].emdong_nm + "_" + (i+1);
															res.result[i]["_dataIdx"] = (i+1);
															break;
														}
			...
			setUIEvent : function() {
				...
				body.on("click",".interactiveDataBoard",function(){
					...
					if(!ck){
						...
						//$(".sop-right").stop().animate({"right":"558px"},200);//20년수정반영 - 밑에 새로운 작업 시작하기 이전 코드
						//200423수정 시작(6번. 지도 추가시 ICON 이상동작 해결)
						if($('div[id^=view].sceneBox.on').length===1) {
						    $(".sop-right").stop().animate({"right":"558px"},200);
						} else if ($('div[id^=view].sceneBox.on').length===2) {
							let viewBigNumber = '';    
							$('div[id^=view].sceneBox.on').each(function(a,b){
                               viewBigNumber = $(b);

							});
                            console.log(viewBigNumber);
                            viewBigNumber.find('.sop-top.sop-right').stop().animate({"right":"558px"},200);
						} 
						//200423수정 끝(6번. 지도추가 시 ICON 이상동작 해결)
						$("#normalDataBoard").show();
						$("#fullDataBoard").hide();
						$(this).addClass("on").stop().animate({"right":"426px"},200);//20년수정반영
						
					}else{
						var full = $(".dataSideBox").hasClass("full");
						
						//mng_s 20210420 이진호, grid 모드에서는 데이터 보드에서 '해당 지역 내 데이터 보기' 표를 hide 처리한다
						var grid = $(".bnd_grid_radio").hasClass("on");
						if(grid){
							$("#dssChart03").hide();
						}
						//mng_e 20210420 이진호
						
						//mng_s 20210420 이진호, grid가 켜져 있지 않을 때만 full 모드로 되게 조건 수정
						if(!grid && !full){
							...
							if($interactiveLeftMenu.ui.curSelectedStatsType == "kosis"
								...
								$(".sop-right").stop().animate({"right":"-1200px"},0);//200423수정 (14번. 데이터보드 확장시 아이콘 정렬 결정 - 사라지게)
								$("#fullDataBoard").show();
				...
				body.on("click",".typeBox>a",function(){ 
					...
					if(ck%2){
						...
						srvLogWrite("C0", "03", "01", "00", "", "");		//데이터 보기 - 차트
						...
						srvLogWrite("C0", "03", "02", "00", "", "");		//데이터 보기 - 표
				...
				body.on("click",".btn_excelDownload", function(){
					...
					srvLogWrite("C0", "03", "03", "00", "", "");		//데이터 보기 - 표 다운로드
					...
					$(this).parents(".compareBox").eq(0).find(".tables").eq(0).find("div").find("table tr").each(function(){
						//mng_s 20200318 이진호
						//대화형 통계지도 데이터 보드에서 엑셀 다운시 N/A 값은 안나오게 설정
						//만약 엑셀에 N/A 값을 표기하려면 2717 줄 if 문 수정하면 됨
						var contentData = []; 
						var tempContentData = [];
						$(this).find("td").each(function(i){
							...
							if(content.indexOf(",") > -1 ){
								content = parseFloat(content.replace(/,/gi, ""));
							} else if(content.indexOf("%") > -1){ //mng_s 20211028 이진호, 상위지역 액셀 다운 안되는 문제 수정
								content = parseFloat(content.replace(/,/gi, ""));
							} //mng_e 20211028 이진호
							
							tempContentData.push(content);
						});
						
						if(tempContentData.indexOf("N/A") == -1 ){
							contentData.push(tempContentData);
						}
						//mng_e 20200318 이진호
				...
				body.on("click",".btn_clockTypeSetting",function(){ 
					...
					$(".btn_clockTypeMts").css("top","145px"); //SGIS_4 추가(다중시계열 버튼 위치조정)
					
					//mng_s 20211029 이진호
					//시계열 세팅 버튼 클릭 시 ok버튼과 다중시계열 버튼이 안보여서 css 조정
					$("#tableTimeSeries").css("min-height", "155px");
					//mng_e 20211029 이진호
				...
				body.on("click",".btn_clockTypeOk",function(){
					...
					//mng_s 20211029 이진호
					//시계열 확인 버튼 클릭 시 min-height 값 원복되게
					$("#tableTimeSeries").css("min-height", "");
					//mng_e 20211029 이진호
					...
					$(".yearList li").each(function(i){
						...
						if(!ck){
							...
							if($(this).find("input").val() == "2017"){
								
								//dataBoard.selectedYearArray.push("9016");
							}
					...
					$(".btn_clockTypeMts").css("top","80px"); //SGIS_4 (다중시계열 버튼 위치조정)
			    });
				
				//시계열 플레이/스탑 버튼
				body.on("click", ".btn_clockTypePlay", function() {
					$interactiveDataBoard.ui.rollBackCnt = 0;	////20190315 leekh 시계열 집계구 플레이시 오류 수정
					//mng_s
				...
				body.on("click", "#tableTimeSeries div", function() {
					srvLogWrite("C0", "03", "04", "00", "", "");		//데이터 보기 - 시계열 조회
				...
				//SGIS_4 시작
				body.on("click", "#multiTimeSeries", function(){
					$("#chooseYearList").empty();
					...
					for(var i=0; i < dataBoard.yearArray.length; i++){
						for(var j=0; j < dataBoard.selectedYearArray.length; j++){
							if(dataBoard.selectedYearArray[j] == dataBoard.yearArray[i] 
								...
								yearList.push(parseInt(dataBoard.yearArray[i]));//SGIS4_1124 수정
							}
						}
					}
					/*
					$.each(yearList, function(index, item){
						html += "<li id='li_y"+item+"'>";
						html += "	<input type='checkbox' id='y"+item+"' value='"+item+"' class='year_input' name='multiTimeRange'/>";
						html += "	<div id='div_y"+item+"'>"+item+"</div>";
						html += "</li>";
					});
					
					$("#chooseYearList").append(html);
					...
					*/
					...
					if(yearList.length < 4){
						for(var i=0; i<yearList.length; i++){
							selectYears[i] = yearList[i];
						}
					}else{
						selectYears.push(yearList[(yearList.length-4)]);
						selectYears.push(yearList[(yearList.length-3)]);
						selectYears.push(yearList[(yearList.length-2)]);
						selectYears.push(yearList[(yearList.length-1)]);
					}
					...
					if($interactiveMultiTimeSeries.ui.isFirstView){
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_4", 3);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_5", 4);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_6", 5);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_7", 6);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_8", 7);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_9", 8);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_10", 9);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_11", 10);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_12", 11);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_13", 12);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_14", 13);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_15", 14);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_16", 15);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_17", 16);
						/*SGIS4_1124 추가 시작*/
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_18", 17);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_19", 18);
						$interactiveMultiTimeSeries.ui.createMap("mapRgn_20", 19);
						/*SGIS4_1124 추가 끝*/
					}
