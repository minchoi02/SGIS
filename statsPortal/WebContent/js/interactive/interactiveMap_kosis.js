...
//				$interactiveLeftMenu.ui.setParams($interactiveLeftMenu.ui.curSelectedStatsType, "");
			...
			checkSubMenu : function(subObj) {
				if(subObj.childCount > 0) {
					...
					$("#kosisDetailDiv").stop().animate({"left":"0"},200);
				} else {
...
//							interactiveMapKosis.checkSubMenu(node);
							...
							//$interactiveLeftMenu.event.stepCloseAnimate(3, "pass");//박길섭 수정 
			...
			setStaticDataFields : function(subObj) {
				...
				html += "	<select title='세부항목' id='kosisDetail' class='kosisDetail' name='kosisDetail' style='font-size:13px;width:100px;'>";
...
//			},
			...
			getKosisDataList : function(params, zoom, gisSe, atdrc_yn, map) {
				...
				srvLogWrite("C0", "02", "05", "02", "", shareParams.title);
...
//				this.map.valPerSlice = this.calculateLegend(arData);
			...
			kosisSearch : function(pagenum) {
				...
				//$interactiveLeftMenu.event.stepCloseAnimate(3, "pass"); //2019-03-05 박길섭 
				
				var searchword = $("#kosisSearchText").val();
				srvLogWrite("C0", "02", "05", "01", "", searchword);
	...
	(function() {
		$class("kosis.serviceApi.KosisDataStaticFields.api").extend(sop.portal.absAPI).define({
			onSuccess : function(status, res) {
				var result = res.result;
				if(res.errCd == "0") {
					console.log("kosis.serviceApi.KosisDataStaticFields.api success");
					//9월 서비스
//					$(".sideQuick.sq02").stop().animate({"left":"840px"},200);
					$("#kosisDetailDiv").stop();
					$("#kosisDetailDiv").stop().animate({"left":"360px"},{duration:200 , start : function(){
