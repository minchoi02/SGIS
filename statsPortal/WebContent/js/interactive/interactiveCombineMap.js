...
(function(W, D) {
	...
	$(document).ready(
		function() {				
			...
			//mng_s 20210329 이진호
			//지도겹처보기 초기 실행 시 사업체POI (지표) 항목 리스트가 노출되는 문제 수정
			$(".rqListBox.rq01").css('right', '-1200px');
			//mng_e 20210329 이진호
	...
	$combineMap.ui = {
					...
					if (data[i].geojson == null && data[i].type == "MY") {
						...
						for (var z=0; z<data[i].geojson.length; z++) {
							for (var k=0; k<data[i].geojson[z].features.length; k++) {
								...
								if (param.isKosis||param.isEcountry) {
						...
						for (var k=0; k<data[i].geojson.features.length; k++) {
							...
							if (param.isKosis) {
								map.dataType = "kosis";
								param.info[3] = data[i].id;
							} else if(param.isEcountry){
								map.dataType = "ecountry";
								param.info[3] = data[i].id;
							} else {
								map.dataType = "normal";
								for (var m=0; m<param.info.length; m++) {
			...
			createInfoTooltip : function(event, data, type, map) {
				...
				//사용자데이터가 아닐경우만
				if (data.info[0] && data.info[0].api_id && data.info[0].api_id != "API_MYDATA") {
					for (var i=0; i<$combineMap.ui.layerGroup.length; i++) {
				...
				if (type != "polygon") {
					if (type == "data") {
						if (data.info.length > 0) {
							for (var i=0; i<$combineMap.ui.layerGroup.length; i++) {
								var layer = $combineMap.ui.layerGroup[i];
								if (data.isKosis||data.isEcountry) {
	...
	$combineMap.event = {
			...
			setUIEvent : function() {
				...
				// djlee 수정
				$(".btnScroll").find("#mCSB_2_container").css("width", "270px");
