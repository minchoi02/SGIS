...
(function(W, D) {
	...
	$interactiveMapApi.request = {
			...
			openApiTotalPopulation : function(params) {
					...
					*/
					...
					for (var i = 0; i < params.param.length; i++) {
						if(params.param[i].key == 'year'){
							grid_log_year =  params.param[i].value;
						}
					}
					...
					sopOpenApiTotalPopulationObj.addParam("filter", params.filter);
				}
				...
				if(zeroLevelYn){
					//2020년수정변경 시작 (ggm)
					if($interactiveMap.ui.chkIfInteractive() && !params.map.isInnerMapShow2 && (params.is_zoom_lvl4 == "Y" || params.is_non_self == "Y") || params.map.isInnerMapShow3){//SGIS4_0107_행정구역 그리드 BSCA추가
						console.log("contextPath + this.API_0301_1_URL = " + contextPath + this.API_0301_1_URL);
						sopOpenApiTotalPopulationObj.addParam("is_zoom_lvl4", params.is_zoom_lvl4);		//4시군구 처리용
						sopOpenApiTotalPopulationObj.addParam("is_non_self", params.is_non_self);		//4시군구 처리용
						
						sopOpenApiTotalPopulationObj.request({
							method : "GET",
							async : async, //9월 서비스
							url : contextPath + this.API_0301_1_URL,	//20200427 수정 (ggm)
							options : {
								params : params,
								url : this.API_0301_1_URL,
							}
						});						
					}else{					
						console.log("openApiPath + this.API_0301_URL = " + openApiPath + this.API_0301_URL);
						sopOpenApiTotalPopulationObj.request({
							method : "GET",
							async : async, //9월 서비스
			...
			openApiSearchPopulation : function(params) {
				...
				//2020년수정변경 시작 (ggm)
				if($interactiveMap.ui.chkIfInteractive() && !params.map.isInnerMapShow2 && (params.is_zoom_lvl4 == "Y" || params.is_non_self == "Y") || params.map.isInnerMapShow3){//SGIS4_0107_행정구역 그리드 BSCA 적용 수정
					sopOpenApiSearchPopulationObj.addParam("is_zoom_lvl4", params.is_zoom_lvl4);	//4시군구 처리용
					sopOpenApiSearchPopulationObj.addParam("is_non_self", params.is_non_self);		//4시군구 처리용
			...
			openApiHouseHold : function(params) {
				...
				//2020년수정변경 시작 (ggm)
				if($interactiveMap.ui.chkIfInteractive() && !params.map.isInnerMapShow2 && (params.is_zoom_lvl4 == "Y" || params.is_non_self == "Y") || params.map.isInnerMapShow3){	//SGIS4_0107_행정구역 그리드 BSCA적용 수정
				
					sopOpenApiHouseHoldObj.addParam("is_zoom_lvl4", params.is_zoom_lvl4);	//4시군구 처리용
					sopOpenApiHouseHoldObj.addParam("is_non_self", params.is_non_self);		//4시군구 처리용
					sopOpenApiHouseHoldObj.request({
						method : "GET",
						async : async,	//9월 서비스
						url : contextPath + this.API_0305_1_URL,	//20200427 수정 (ggm)
						options : {
							params : params,
							url : this.API_0305_1_URL,
						}
					});					
				}else{
					sopOpenApiHouseHoldObj.request({
			...
			openApiHouseHoldMember : function(params) {
				...
				if(params.map.isInnerMapShow3) {
					var bnd_grid = "bnd_grid";
					sopOpenApiHouseHoldMemberObj.addParam("bnd_grid", bnd_grid);
					sopOpenApiHouseHoldMemberObj.addParam("zoom", params.map.zoom);
					sopOpenApiHouseHoldMemberObj.addParam("adm_cd", encodeURIComponent(params.adm_cd));
					
					//mng_s 20220315 이진호
					//그리드일때만 parameter에 filter 추가
					//아래 if문 밖에 있는 filter 추가가 여기로 이동한 것
					sopOpenApiHouseHoldMemberObj.addParam("filter", params.filter); // 가구인지 인구인지 확인하기위해서 파라미터 추가
					//mng_e 20220315 이진호
				}
				
				//mng_s 20220315 이진호
				//그리드일때만 parmeter 추가 하도록 변경(기존소스 주석처리 후 위로 이동)
				//그리드 아닐 때 parmter에 filter 가 추가 되어 412err 발생
				//sopOpenApiHouseHoldMemberObj.addParam("filter", params.filter); // 가구인지 인구인지 확인하기위해서 파라미터 추가
				//mng_e 20220315 이진호
			...
			openApiFarmHouseHold : function(params) {
				...
				if(params.map.isInnerMapShow3) {
					var bnd_grid = "bnd_grid";
					sopOpenApiFarmHouseHoldObj.addParam("bnd_grid", bnd_grid);
					sopOpenApiFarmHouseHoldObj.addParam("zoom", params.map.zoom);
					sopOpenApiFarmHouseHoldObj.addParam("adm_cd", encodeURIComponent(params.adm_cd));
				}
				// SGIS4_0107 추가 끝
				//시계열조회시 로딩아이콘 숨기기
				//9월 서비스
				...
				if($interactiveMap.ui.chkIfInteractive() && !params.map.isInnerMapShow2 && (params.is_zoom_lvl4 == "Y" || params.is_non_self == "Y") || params.map.isInnerMapShow3){	//SGIS4_0107_행정구역 그리드 BSCA 적용 수정
					...
					sopOpenApiFarmHouseHoldObj.addParam("is_zoom_lvl4", params.is_zoom_lvl4);		//4시군구 처리용
					sopOpenApiFarmHouseHoldObj.addParam("is_non_self", params.is_non_self);			//4시군구 처리용
					sopOpenApiFarmHouseHoldObj.request({
						method : "GET",
						async : async,	//9월 서비스
						url : contextPath + this.API_0307_1_URL,	//20200427 수정 (ggm)
						options : {
							params : params,
							url : this.API_0307_1_URL,
						}
					});					
				}else{
					sopOpenApiFarmHouseHoldObj.request({
			...
			openApiForestryHouseHold : function(params) {
				...
				//2020년수정변경 시작 (ggm)
				if($interactiveMap.ui.chkIfInteractive() && !params.map.isInnerMapShow2 && (params.is_zoom_lvl4 == "Y" || params.is_non_self == "Y") || params.map.isInnerMapShow3){	//SGIS4_0107_행정구역 그리드 BSCA 적용 수정
					sopOpenApiForestryHouseHoldObj.addParam("is_zoom_lvl4", params.is_zoom_lvl4);		//4시군구 처리용
					sopOpenApiForestryHouseHoldObj.addParam("is_non_self", params.is_non_self);			//4시군구 처리용
					sopOpenApiForestryHouseHoldObj.request({
						method : "GET",
						async : async,	//9월 서비스
						url : contextPath + this.API_0308_1_URL,	//20200427 수정 (ggm)
						options : {
							params : params,
							url : this.API_0308_1_URL,
						}
					});
				}else{
					sopOpenApiForestryHouseHoldObj.request({
			...
			openApiFisheryHouseHold : function(params) {
				...
				//2020년수정변경 시작 (ggm)
				if($interactiveMap.ui.chkIfInteractive() && !params.map.isInnerMapShow2 && (params.is_zoom_lvl4 == "Y" || params.is_non_self == "Y") || params.map.isInnerMapShow3){	//SGIS4_0107_행정구역 그리드 BSCA 적용 수정
					sopOpenApiFisheryHouseHoldObj.addParam("is_zoom_lvl4", params.is_zoom_lvl4);		//4시군구 처리용
					sopOpenApiFisheryHouseHoldObj.addParam("is_non_self", params.is_non_self);			//4시군구 처리용
					sopOpenApiFisheryHouseHoldObj.request({
						method : "GET",
						async : async,	//9월 서비스
						url : contextPath + this.API_0309_1_URL,	//20200427 수정 (ggm)
						options : {
							params : params,
							url : this.API_0309_1_URL,
						}
					});					
				}else{
					sopOpenApiFisheryHouseHoldObj.request({
			...
			openApiCompany : function(params) {
				...
				if($interactiveMap.ui.chkIfInteractive() && !params.map.isInnerMapShow2 && (params.is_zoom_lvl4 == "Y" || params.is_non_self == "Y") || params.map.isInnerMapShow3){ //SGIS4_0107_행정구역 그리드 BSCA 적용 수정	
					...
					if(api_url == this.API_0304_URL){
						...
						//SGIS4_0107_행정구역그리드는 총조사 안보게.. 시작
						if(params.map.isInnerMapShow3){
							api_url = this.API_0304_1_URL;
						}else{
					...
					sopOpenApiCompanyObj.addParam("is_zoom_lvl4", params.is_zoom_lvl4);			//4시군구 처리용
					sopOpenApiCompanyObj.addParam("is_non_self", params.is_non_self);			//4시군구 처리용
			...
			openApiHouse : function(params) {
				...
				//2020년수정변경 시작 (ggm)
				if($interactiveMap.ui.chkIfInteractive() && !params.map.isInnerMapShow2 && (params.is_zoom_lvl4 == "Y" || params.is_non_self == "Y") || params.map.isInnerMapShow3){ //SGIS4_0107_행정구역그리드 BSCA 적용 수정
					sopOpenApiHouseObj.addParam("is_zoom_lvl4", params.is_zoom_lvl4);		//4시군구 처리용
					sopOpenApiHouseObj.addParam("is_non_self", params.is_non_self);		//4시군구 처리용					
...
//			},
			...
			openApiItemCombine : function(params) {
				...
				var methodType = "POST";
				if(params.noneParams != undefined){ //SGIS_4 추가(if문 추가)
				...
				//2020년수정변경 시작 (ggm)
				if($interactiveMap.ui.chkIfInteractive() && !params.map.isInnerMapShow2 && (params.is_zoom_lvl4 == "Y" || params.is_non_self == "Y") || params.map.isInnerMapShow3){	//SGIS4_0107_행정구역 그리드 BSCA적용 수정
					if(api_url == openApiPath + $interactiveMapApi.request.API_0302_URL){
						api_url = contextPath + this.API_0302_1_URL;		//20200427 수정 (ggm)
					}else if(api_url == openApiPath + $interactiveMapApi.request.API_0305_URL){
						api_url = contextPath + this.API_0305_1_URL;		//20200427 수정 (ggm)
					}else if(api_url == openApiPath + $interactiveMapApi.request.API_0306_URL){
						api_url = contextPath + this.API_0306_1_URL;		//20200427 수정 (ggm)
					}
					
					sopOpenApiItemCombineObj.addParam("is_zoom_lvl4", params.is_zoom_lvl4);	//4시군구 처리용
					sopOpenApiItemCombineObj.addParam("is_non_self", params.is_non_self);		//4시군구 처리용					
				}
				//2020년수정변경 끝
			...
			setStatsData : function (res, options, isContry) {
				...
				if (isContry != undefined && isContry)  {
					...
					//2020년수정변경 시작 (ggm)
					if($interactiveMap.ui.chkIfInteractive() && (!map.isInnerMapShow2 && !map.isInnerMapShow3) && map.boundLevel == "1" && (params["is_non_self"] && params["is_non_self"] == "Y") && (params.adm_cd && params.adm_cd.length > 1)){		//20200417 수정 (ggm)
						// zoom 4 -> 5 일때
						res["pAdmCd"] = params.adm_cd.substring(0, 2);
					}else{
...
//					$("#noticeTextPopup0"+(parseInt(map.id)+1)).hide();
				...
				//설명문구
				if(map.isInnerMapShow2 || map.isInnerMapShow3){
				...
				if ($interactiveMap.ui.searchBtnType == "normal") {	
					...
					if (dataBoard.isTimeSeriesPlay) {
						...
						if (params.view_type == "TS") {
							...
							if (map.boundLevel == "1") {
								//2020년수정변경 시작 (ggm)
								if($interactiveMap.ui.chkIfInteractive() && (!map.isInnerMapShow2 && !map.isInnerMapShow3) && map.zoom == 4){	//20200417 수정 (ggm)
									map.openApiReverseGeoCodeForBorough(map.center);
								}else{
...
//						$("#timeSeriesDiv").hide();
			 ...
			 * @param @param options
			 */
			setMtsStatsData : function (res, options, isContry) {
				console.log(res);
				var result = res.result;
				var params = options.params;
				var map = params.map;
				...
				map.setmultiTimeStatsData(res, options, map);
			},
			/* SGIS_4 다중시계열 끝*/
					...
					onSuccess : function(status, res, options) {
						...
						switch (parseInt(res.errCd)) {
							case 0:
								...
								}else if(options.params.type == "mts"){//SGIS_4 다중시계열 추가
									$interactiveMapApi.request.setMtsStatsData(res, options, isContry);//SGIS_4 다중시계열 추가
								}else {
							...
							case -100:
								if (map.selectedBoundMode == "multi") {
									...
									//mng_s 20220112 이진호
									//다중시계열 동작 시 하나라도 검색결과가 없을 때 로딩화면만 계속 보이므로
									//loading을 false로 설정하여 흘러가게 수정
									if(options.params.type=="mts"){
										//var yearMapDivId = $("#mapRgn_"+map.id+1).parent().parent().parent().attr('id');
										//var year = $("#"+yearMapDivId).children(".MapYearselectbox.YearSelectBox").children().val();
										messageAlert.open(
												"알림", 
												"검색결과가 존재하지 않습니다.",
												function done() {
					...
					onSuccess : function(status, res, options) {	
						...
						switch (parseInt(res.errCd)) {
							case 0:
								...
								if (map.selectedBoundMode == "multi") {
									$interactiveMapApi.request.setMultiStatsData(res, options, isContry);	
								}if(options.params.type == "mts"){//SGIS_4 다중시계열 추가
									$interactiveMapApi.request.setMtsStatsData(res, options, isContry);	//SGIS_4 다중시계열 추가
								}else {
									$interactiveMapApi.request.setStatsData(res, options, isContry);		
								}
		...
		$class("sop.openApi.itemcombine.api").extend(sop.portal.absAPI).define(
				{
					onSuccess : function(status, res, options) {	
						if (res.errCd == "0") {
							...
							//map.drawControl.removeOverlay(); //SGIS_4 다중시계열 수정
							...
							//SGIS_4 다중시계열 수정 시작
							if(options.params.type == "mts"){
								$interactiveMapApi.request.setMtsStatsData(res, options, isContry);	
							}else{
								map.drawControl.removeOverlay();
								$interactiveMapApi.request.setStatsData(res, options, isContry);
							}
...
//							$interactiveMap.ui.setShareInfo(options, "normal", map.id);
							//2021년 SGIS고도화 4차 수정 시작
							if(map.shareInfo.setShareInfo != null){
								map.shareInfo.setShareInfo(options, "normal", map.id);
							}
							//2021년 SGIS고도화 4차 수정 끝
