...
(function(W, D) {
	...
	$policyWriteMapRightMenu.ui = {
			...
			showStatsPanel : function() {
				$("#depth1Menu2").stop().animate({"left": "-80px"}, 200);
				$(".quickBox2.step02").stop().animate({"left":"-280px"}, 200);
				$(".quickBox2.step03").stop().animate({"left":"-280px"},200);

				$(".quickBox2.step01").stop().animate({"left":"0px"}, 200);
			},
			/**
			 * 
			...
			setDetailStatsPanel : function(type) {
				...
				if(inx!=8){
					...
					$("#depth1Menu2").find("a").removeClass("on");					// 2019.01.22	ywKim	변경
//					$("#depth1Menu2").find("li").removeClass("on");
					$("#depth1Menu2").find("li:eq("+inx+")>a").addClass("on");		// 2019.01.22	ywKim	변경
//					$("#depth1Menu2").find("li:eq("+inx+")").addClass("on");
				...
				$(".quickBox2.step01").stop().animate({"left":"-220px"}, 200);
				$(".quickBox2.step03").stop().animate({"left":"-280px"},200);
				$("#depth1Menu2").stop().animate({"left": "0px"}, 200);
				$(".quickBox2.step02").stop().animate({"left":"80px"}, 200);
				$(".quickBox2.step03").stop().animate({"left":"360px"},200);
...
//				$(".quickBox2.step04").stop().animate({"left":"0px"},200);
				...
				if (inx == 3) {
					$(".stepClose2.3depth_close").stop().animate({"right":"-304px"}, 200);
				} else {
					$(".stepClose2.3depth_close").stop().animate({"right":"-25px"}, 200);
				}
			...
			doReqStatsData : function() {
				if (this.btnValidationCheck()) {
					...
					srvLogWrite( "E0", "05", "12", "00", "", $policyWriteMap.ui.getLogParams( true ) ); // jrj 로그 > 통계지표 선택 - 시설분석형
	...
	$policyWriteMapRightMenu.request = {
			...
			reqThemeCode : function(depth) {
				$.ajax({
					...
					success : function(res) {
						...
						var tmpSubData = {};
						for (var i=0; i<result.length; i++) {
							//var classCd = result[i].theme_cd.substring(0,2); 	// 2020년 SGIS고도화 3차(테마코드) - 기존 테마코드 대분류 생성법 주석처리
							var classCd = result[i].b_theme_cd;					// 2020년 SGIS고도화 3차(테마코드) - 새로운 테마코드 대분류를 적용한 코드로 대체
							var rootData = {};
							var subData = {};
			 ...
			 * @history :
			 */
			reqUserDataList : function(pageNum) {
				$.ajax({
						...
						switch(parseInt(res.errCd)) {
							case 0:
								...
								if (result.totalcount > 5) {
									var htmlPage = "<br><br><br><div id='myDataPaging2' class='pagenation' align='center' style='width: 100%;'><span class='pages'></span></div>";
									$("#myDataListTablePage2").html(htmlPage);
								}
								
								$policyWriteMapRightMenu.request.myDataPaging( 
										result.totalcount, 
										$policyWriteMapRightMenu.ui.myPageNum );
			...
			myDataPaging : function(totalCount, currentIndex) {
				...
				$('#myDataPaging2 .pages').paging({
					...
					onclick : function(e, page) {
						$policyWriteMapRightMenu.ui.myPageNum = page;
						$policyWriteMapRightMenu.request.reqUserDataList(page);
					}
				});
			}
	...
	$policyWriteMapRightMenu.event = {
			...
			setUIEvent : function() {
				...
				body.on("click", ".easytree-check", function() {
					...
					$(".quickBox2.step03").stop().animate({"left":"360px"},200);	// 2019.01.22	ywKim	변경
//					$(".quickBox2.step03").stop().animate({"left":"560px"},200);
				});
...
//					$(".quickBox2.step03").stop().animate({"left":"560px"},200);
			...
			stepCloseAnimate : function(inx, type, ts){
			    ...
			    $("#depth1Menu2").stop().animate({"left": "-80px"}, 200);	// 2019.01.22	ywKim	추가
			},
