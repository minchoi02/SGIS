...
(function(W, D) {
	...
	$policyWriteCombineMap.ui = {
			...
			initCombineMap : function(delegate) {
				...
				var arParamList = $policyWriteMapLeftMenu.ui.arParamList;
				var params = "";
				$.each( arParamList, function( i, item ){
					params += ( i!=0 ? "," : "지표:" )+item.title+"("+item.origin+")";
				});
				
				srvLogWrite( "E0", "05", "15", "02", "", $policyWriteMap.ui.getLogParams() ); // jrj 로그 > 통계지표 선택 - 시설분석형
			},
									 ...
									 
							{
								...
								func : function(opt) {
									if ($policyWriteCombineMap.ui.validatationPolicyMap()) {
										srvLogWrite( "E0", "05", "16", "01", "", $policyWriteMap.ui.getLogParams() ); // jrj 로그 > 융합결과 데이터 차트, 표
			...
			/**
			 * @name         : doUpdateTempPolicyMap
			 * @description  : 
			 * @date         :  
			 * @author	     : 
			 * @history 	 :
			 * 		2018.06.25	ywKim	변경: 비공개저장 기능 추가 [v180618] - 연령구간 구체화
			 * 							- pSrvYn, srvYn
			 * @param pSrvYn : srv_yn 값 ('P': 비공개, other : 공개)
			 */
			doUpdateTempPolicyMap : function(pSrvYn) {
				var id = $(".preViewIframe").attr("id");
				var category_id = id.split("-")[0];
				var idx_id = id.split("-")[1];
				var srvYn = (typeof pSrvYn === 'undefined') ? "Y" : pSrvYn;
				var params = {
						category_id : category_id,
						idx_id : idx_id,
						srv_yn : srvYn
				...
				srvLogWrite( "E0", "05", "17", "00", ( srvYn == "Y" ? "공개" : "비공개" ) , $policyWriteMap.ui.getLogParams() ); // jrj 로그 > 정책통계지도 공개
...
//			},
			...
			showpreViewPopup : function(data) {
				...
				if (idx_id != undefined) {
					...
					// 2018.06.25	ywKim	추가: 비공개 연령을 선택한 경우 [비공개 저장] 버튼 활성화
					if ($policyWriteMap.ui.isPrivate && $policyWriteMap.ui.isPrivate == 'Y') {
						$(".pcBtn2 a:nth-child(2)").hide();
						$(".pcBtn2 a:nth-child(3)").show();
					} else {
						$(".pcBtn2 a:nth-child(2)").show();
						$(".pcBtn2 a:nth-child(3)").hide();
					}					
				}
			},
			...
			regCheckParams : function(isTemp) {
				...
				for (var i=0; i<leftMapInfo.length; i++) {
					...
					var origin = "";
						...
						origin = leftMapInfo[i].origin; //원본소스
	...
	$policyWriteCombineMap.event = {
			...
			setUIEvent : function() {
				...
				$("body").on("click", ".compareBox .typeBox>a", function() {
					...
					srvLogWrite( "E0", "05", "15", ( $( ".typeBox a.on" ).index() == 0 ? "03" : "04" ), "", $policyWriteMap.ui.getLogParams() ); // jrj 로그 > 융합결과 데이터 차트, 표
				});
