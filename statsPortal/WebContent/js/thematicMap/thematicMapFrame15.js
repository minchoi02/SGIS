...
(function(W, D) {
	W.$thematicMapFrame15 = W.$thematicMapFrame15 || {};

	$(document).ready(
		function() {	
			//mng_s 화면 크기 조정 - full 20210421 djlee
			if(document.location.href.indexOf("type=full") > 1 ){
				$thematicMapFrame15.ui.doMaxSize(1);
			}
			//mng_e
			
			$thematicMapFrame15.params = getAllParameter();
			$thematicMapFrame15.ui.createMap("mapRgn_1", 0);
			$thematicMapFrame15.event.setUIEvent();
			var mapNavi1 = new mapNavigation.UI();
			mapNavi1.create("mapNavi_1", 1, $thematicMapFrame15.ui);	
			//메뉴에서 카타고리 정보 가져온다.
			$thematicMapFrame15.request.getCategory();	
	...
	$thematicMapFrame15.ui = {
			...
			addCovidMarker : function(result){
				...
				$.each(result,function(cnt,node){
					...
					marker.addTo($thematicMapFrame15.ui.markers);
					var html = "";
			...
			removeCovidMarker : function(){
				var map = this.mapList[this.curMapId];
				if($thematicMapFrame15.ui.markers !=null){
					$thematicMapFrame15.ui.markers.removeFrom(map.gMap);
				}
				this.markers = null;
			},
			//코로나추가 20200722 주용민
			...
			closeWin : function (cookieName, value, expiredays) {
				if($("#coronaPop").find("input[name='corona_close']").is(":checked")) {
					$thematicMapFrame15.ui.setCookie(cookieName, value, expiredays);
				}
			...
			doAnalysisShareInfo : function(data) {
				if (data.type == "bookmark") {
					...
					map.openInitStatData($thematicMapFrame15.params, function() {
						...
						$thematicMapFrame15.ui.changeDataMode();
						...
						$thematicMapFrame15.ui.changeDataMode2();
					});
				}
			},
			...
			createMap : function(id, seq) {
				...
				map.createMap($thematicMapFrame15, id, {
				...
				legend.initialize($thematicMapFrame15.ui);
				map.legend = legend;
				legend.createLegend();
				//작업부분 끝
				
				//코로나 추가 20200729 jrj
				if( $thematicMapFrame15.params.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw" ){
				...
				//공유
				var shareInfo = new share.shareInfo(map, $thematicMapFrame15.ui);
				map.shareInfo = shareInfo;
				$thematicMapFrame15.params["url"] = "/view/thematicMap/thematicMapMain";
				map.shareInfo.setThematicMapShareInfo($thematicMapFrame15.params, "15");
				...
				if (window.parent.$thematicMapMain) {
					$thematicMapFrame15.ui.doAnalysisShareInfo(window.parent.$thematicMapMain.param);
				}else {
					$thematicMapFrame15.request.getStatsThemeMapList($thematicMapFrame15.params.id);
				}
					...
					html2canvas($(divId), {
	                    ...
	                    proxy: contextPath+"/ServiceAPI/community/html2canvasproxy.jsonp",
		 				onrendered: function(canvas) {
	                    	...
	                    	var popup = $thematicMapFrame15.ui.reportPopup.$reportForm.ui;
	        				if($thematicMapFrame15.ui.poiInfoArray.length < 1) {
	        					popup.setData(deepCopy(dataList), options, $thematicMapFrame15.ui.dataGeoJson);
	        				} else {
	        					...
	        					if(window.parent.$thematicMapMain.param.stat_thema_map_id == "5FLM8BXNH320200521093949554ioKEsoOUGI"){
	        						popup.setData(deepCopy(dataList), options, $thematicMapFrame15.ui.dataGeoJson, null);
	        					}else{
	        						popup.setData(deepCopy(dataList), options, $thematicMapFrame15.ui.dataGeoJson, $thematicMapFrame15.ui.poiInfoArray);
	        					}
	        					// mng_e 20200529 김건민
	        				}
...
//					&& $publicDataBoard.ui.chartDataList[map.id] == null
				...
				$thematicMapFrame15.ui.reportPopup = 
					window.open("/js/thematicMap/report/15/reportForm.html", "reportPrint","width=850, height=700, scrollbars=yes");
			},
...
//					$("header").css({"height":"104px", "width":"970px"}); 
			...
			doCombineMap : function(type) {
				...
				if (isCombine) {
					$thematicMapFrame15.ui.combinePopup = 
						window.open(
			...
			mapLoad : function() {
				...
				var popup = $thematicMapFrame15.ui.combinePopup.$combineMap.ui;
			...
			doAddMap : function(type) {				
				...
				mapNavi.create("mapNavi_" + (createMapId+1), createMapId+1, $thematicMapFrame15.ui);
				
				var sceneInx = $(".sceneBox.on").length; 
				$(".sceneBox").eq(createMapId).show().addClass("on");
				if(sceneInx==1){ 
					$(".sceneBox").stop().animate({"width":"50%"},200, function() {
						for (var i=0; i<$thematicMapFrame15.ui.mapList.length; i++) {
							if ($thematicMapFrame15.ui.mapList[i] != null) {
								$thematicMapFrame15.ui.mapList[i].update();
							}
						}
					});
...
//								}
							...
							if(data.properties.adm_nm !== undefined){
								...
								if (tmpData.showData != undefined && tmpData.showData.length > 0) {
									...
									$thematicMapFrame15.ui.mapList[0].selectedAdmCd = tmpData.adm_cd;
...
//									}
			...
			requestOpenApi : function(options) {
				...
				if 	    (api_id == "API_0301") $thematicMapFrame15Api.request.openApiTotalPopulation(options);
				else if (api_id == "API_0302") $thematicMapFrame15Api.request.openApiSearchPopulation(options);
				else if (api_id == "API_0303") $thematicMapFrame15Api.request.openApiInderstryCode(options);
				else if (api_id == "API_0304") $thematicMapFrame15Api.request.openApiCompany(options);
				else if (api_id == "API_0305") $thematicMapFrame15Api.request.openApiHouseHold(options);
				else if (api_id == "API_0306") $thematicMapFrame15Api.request.openApiHouse(options);
				else if (api_id == "API_0307") $thematicMapFrame15Api.request.openApiFarmHouseHold(options);
				else if (api_id == "API_0308") $thematicMapFrame15Api.request.openApiForestryHouseHold(options);
				else if (api_id == "API_0309") $thematicMapFrame15Api.request.openApiFisheryHouseHold(options);
				else if (api_id == "API_0310") $thematicMapFrame15Api.request.openApiHouseHoldMember(options);
				...
				if(shareInfo == null) {
					...
					if (shareInfo.checkShare("BMARK", srvType)) {
						...
						var params = {
								...
								"mapType" : window.parent.$thematicMapMain.param.mapType,
								"select_base_year" : select_base_year,
								"iframe_url" : window.location.protocol+"//"+window.location.host+"/view/thematicMap/thematicMapFrame15"
							};
			...
			doCancel : function(type) {
	 		...
	 		doClearMap : function(type) {
				...
				$(".thematicCharts").hide();
	 			...
	 			var map1 = $thematicMapFrame15.ui.mapList[0];//20년수정반영 ($thematicMapFrame04 => 05 로 수정)
				map1.legend.numberData = false;
				map1.checkShowCaption();
	 		},
			//20년수정반영 끝 (초기화  메서드)
	...
	// map event callback
	// ==============================//
	$thematicMapFrame15.callbackFunc = {
			...
			didMapZoomEnd : function(event, map) {
				...
				if (map.zoom < 10 && map.isInnerMapShow) {
					$thematicMapFrame15.ui.doInnerMap(map.id+1);
				}
			...
			didMapDropEnd : function(event, source, layer, data, map) {
				...
				$thematicMapFrame15.ui.curMapId = map.id;
				$thematicMapFrame15Api.request.combineFailCnt = 0;
				...
				if (id.split("-")[0] == "kosis") {
					...
					index = id.split("-")[1];
//					$thematicMapFrame15.ui.changeBtnLineColor("kosis", tmpParamList,
//							index, map);
					...
					$thematicMapFrame15.ui.searchBtnType = "normal";
					api_id = id.split("-")[0];
					index = id.split("-")[1];
					$interactiveLeftMenu.ui.updateSearchBtnEffect(id, map.id);
					...
					for (var i = 0; i < tmpParamList.length; i++) {
						if (tmpParamList[i].idx == index) {
							...
							$thematicMapFrame15.ui.dropBtnInfo[map.id] = tmpParamList[i];
							break;
						}
					}
				}
				...
				if (map.selectedBoundMode == "multi") {
					for (var i = 0; i < map.selectedBoundList.length; i++) {
						...
						$thematicMapFrame15.ui.curAdmCode = adm_cd;
						map.curAdmCd = adm_cd;
	
						if (tmpParam != null) {
							var params = $thematicMapFrame15.ui.reqSetParams(tmpParam, adm_cd, adm_nm, api_id, map);
							$thematicMapFrame15.ui.curDropParams[map.id] = params;
							$thematicMapFrame15.ui.requestOpenApi(params);
						}
					}
				} else {
					...
					$thematicMapFrame15.ui.curAdmCode = data.adm_cd;
					map.curAdmCd = data.adm_cd;
	
					if (tmpParam != null) {
						var params = $thematicMapFrame15.ui.reqSetParams(tmpParam,data.adm_cd, data.adm_nm, api_id, map);
						//console.log(params);
						//시계열 초기값 세팅
			...
			didMapDoubleClick : function(btn_id, tmpParam) {
				var map = $thematicMapFrame15.ui.mapList[$thematicMapFrame15.ui.curMapId];				
				var index = null;
				var adm_cd = "";
				var adm_name= "";
				var api_id = "";
				...
				if (id.split("-")[0] == "kosis") {
					...
					// 버튼 시각 효과
					$interactiveLeftMenu.ui.updateSearchBtnEffect(id, map.id);
					$thematicMapFrame15.ui.dropBtnInfo[map.id] = tmpParam;
				}
			...
			didMouseOverPolygon : function(event, data, type, map) {	
				if (type != "polygon") {
					...
					$thematicMapFrame15.ui.createInfoTooltip(event, data, type, map);
				}
			},
			...
			didSelectedPolygon : function(event, data, type, map) {
				if (type == "data") {
					...
					if ($thematicMapFrame15.ui.buildPopup != null) {
						$thematicMapFrame15.ui.buildPopup.close();
					}
					...
					$thematicMapFrame15.ui.buildPopup = 
						window.open(
				...
				$(".tb_close").click(function(){ 
					...
					if (sceneInx == 1) {
						...
						$(".sceneBox.on").each(function() {
							...
							tmpView.push(id);
							if (id == $thematicMapFrame15.ui.curMapId) {
								isSameView = true;
							}
						});						
						...
						if (!isSameView) {
							if (tmpView[0] < tmpView[1]) {
								$thematicMapFrame15.ui.curMapId = tmpView[0];
							}else {
								$thematicMapFrame15.ui.curMapId = tmpView[1];
							}
							
							var id = "view" + ($thematicMapFrame15.ui.curMapId + 1);
							switch($thematicMapFrame15.ui.curMapId) {
								case 0:
...
/*				$(".tb_mapAdd").click(function(){ 
				...
				$(".sceneBox").click(function(){
					...
					if (sceneInx > 1) {
						...
						if (id == "view1") {
							$thematicMapFrame15.ui.curMapId = 0;
							$(this).find(".toolBar").css("background", "#0070c0");
						}else if (id == "view2") {
							$thematicMapFrame15.ui.curMapId = 1;
							$(this).find(".toolBar").css("background", "#9ed563");
						}else {
							$thematicMapFrame15.ui.curMapId = 2;
			...
			refreshThemaList : function(select_obj) {
				...
				$thematicMapFrame15.getCategoryList.getMenuList(categoryId);
			}
			// mng_e 2019. 11. 20 j.h.Seok 이용자 중심 서비스 개편 - 정렬 방식 추가
			
			
	};
			$thematicMapFrame15.Popup = {
					show : function () {
			...
			$thematicMapFrame15.request = {
				//카타고리 정보를 가져온다.
				getCategory : function () {
					$statsPotal.api.thematicMap.getCategory({
						method : 'POST',
						success : $thematicMapFrame15.response.successCateList
					});
				},
				...
				successCateList : function (stats, res) {
					...
					if (res.errCd === 0) {
						...
						for ( var i = 0; i < categoryList.length; i++) {							
							//var html = "<li style='background:url("+categoryList[i].category_icon_url+");background-size:30px 30px;background-repeat:no-repeat;background-position:left'><a href='javascript:$thematicMapFrame15.getCategoryList.getMenuList(\""+categoryList[i].thema_map_category+"\");'>"+categoryList[i].category_nm+"</a></li>";
							//$('.qmIcon01').append(html);
							var html = '<li><a data-id="'+categoryList[i].thema_map_category+'"  href="javascript:$thematicMapFrame15.getCategoryList.getMenuList(\''+categoryList[i].thema_map_category+'\');">'+categoryList[i].category_nm+'</a></li>';
							$('.themul').append(html);
							
//							html = '<li><a data-id="'+categoryList[i].thema_map_category+'"  href="javascript:$thematicMapFrame15.getCategoryList.getMenuList(\''+categoryList[i].thema_map_category+'\');" title="'+categoryList[i].category_nm+'"><span>'+categoryList[i].category_nm+'</span></a></li>';
			...
			$thematicMapFrame15.getCategoryList = {
					getMenuList : function (thema_map_category){
						...
						$.ajax({
							  ...
							  success: function(res) {
								...
								html += 'onchange="javascript:$thematicMapFrame15.event.refreshThemaList(this)";>';
...
//								html += '<a id="sort_tooltip" class="theme_sort_tooltip" title="' + sortTooltipText + '"><img src="/img/ico/ico_help05.png" width="20" height="20" alt="정렬방식 도움말 팝업"/></a>';
								...
								$thematicMapFrame15.setCategoryList.setCategoryListHtml(res);
...
//       		 </li>
			...
			$thematicMapFrame15.setCategoryList = {
				...
				$class("sop.portal.themeMapParamList.api").extend(sop.portal.absAPI).define(
						{
							onSuccess : function(status, res, options) {
								...
								if (result.param_info) {
									$thematicMapFrame15.ui.paramInfo = JSON.parse(res.result.themeParamInfoList[0].param_info);
									$thematicMapFrame15.ui.paramInfo.paramInfo["type"] = "bookmark"
									$thematicMapFrame15.ui.doAnalysisShareInfo($thematicMapFrame15.ui.paramInfo.paramInfo);
								}
							},
