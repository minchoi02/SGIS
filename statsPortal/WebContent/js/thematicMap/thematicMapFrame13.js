...
(function(W, D) {
	W.$thematicMapFrame13 = W.$thematicMapFrame13 || {};

	$(document).ready(
		function() {	
			//mng_s 화면 크기 조정 - full 20210421 djlee
			if(document.location.href.indexOf("type=full") > 1 ){
				$thematicMapFrame13.ui.doMaxSize(1);
			}
			//mng_e
			
			$thematicMapFrame13.params = getAllParameter();
			$thematicMapFrame13.ui.createMap("mapRgn_1", 0);
			$thematicMapFrame13.event.setUIEvent();
			var mapNavi1 = new mapNavigation.UI();
			mapNavi1.create("mapNavi_1", 1, $thematicMapFrame13.ui);	
			//메뉴에서 카타고리 정보 가져온다.
			$thematicMapFrame13.request.getCategory();	
	...
	$thematicMapFrame13.ui = {
			...
			addLibraryMarker : function(result){
				...
				$.each(result,function(cnt,node){
					...
					marker.addTo($thematicMapFrame13.ui.markers);
			...
			removeLibraryMarker : function(){
				var map = this.mapList[this.curMapId];
				if($thematicMapFrame13.ui.markers !=null){
					$thematicMapFrame13.ui.markers.removeFrom(map.gMap);
				}
				this.markers = null;
			},
			//mng_e 20171212 주용민
			...
			doAnalysisShareInfo : function(data) {
				if (data.type == "bookmark") {
					...
					map.openInitStatData($thematicMapFrame13.params, function() {
						...
						switch (data.region_boundary) {
							...
							case "1" :
								...
								setTimeout(function() {
									$thematicMapFrame13.ui.mapList[0].changeRegionBound();
								}, 1000);
								break;
							case "2" :
						...
						$thematicMapFrame13.ui.changeDataMode2();
					});
				}
			},
			...
			createMap : function(id, seq) {
				...
				map.createMap($thematicMapFrame13, id, {
				...
				legend.initialize($thematicMapFrame13.ui);
				map.legend = legend;
				legend.createLegend();
				...
				//공유
				var shareInfo = new share.shareInfo(map, $thematicMapFrame13.ui);
				map.shareInfo = shareInfo;
				$thematicMapFrame13.params["url"] = "/view/thematicMap/thematicMapMain";
				map.shareInfo.setThematicMapShareInfo($thematicMapFrame13.params, "13");
				...
				if (window.parent.$thematicMapMain) {
					$thematicMapFrame13.ui.doAnalysisShareInfo(window.parent.$thematicMapMain.param);
				}else {
					$thematicMapFrame13.request.getStatsThemeMapList($thematicMapFrame13.params.id);
				}
					...
					html2canvas($(divId), {
	                    ...
	                    proxy: contextPath+"/ServiceAPI/community/html2canvasproxy.jsonp",
		 				onrendered: function(canvas) {
	                    	...
	                    	var popup = $thematicMapFrame13.ui.reportPopup.$reportForm.ui;
	                    	options["stat_thema_map_id"] = window.parent.$thematicMapMain.param.stat_thema_map_id; 
	        				if($thematicMapFrame13.ui.poiInfoArray.length < 1) {
	        					popup.setData(deepCopy(dataList), options, $thematicMapFrame13.ui.dataGeoJson);
	        				} else {
	        					popup.setData(deepCopy(dataList), options, $thematicMapFrame13.ui.dataGeoJson, $thematicMapFrame13.ui.poiInfoArray);
	        				}
		 				}
		 			});
				}else{
...
//					&& $publicDataBoard.ui.chartDataList[map.id] == null
				...
				$thematicMapFrame13.ui.reportPopup = 
					window.open("/js/thematicMap/report/13/reportForm.html", "reportPrint","width=850, height=700, scrollbars=yes");
			},
					...
					if (data.info.length > 0) {
						...
						if(data.info[2] == "kosis") {
							...
							if (tmpData.showData != undefined && tmpData.showData.length > 0) {
								var filterName = "";
								$thematicMapFrame13.ui.mapList[0].selectedAdmCd = tmpData.adm_cd;
...
//											}
						...
						html += "<tr><td class='admName'>"
							 ...
							 + "<tr style='height:5px'></tr>";	
						//20년수정반영 시작 (백지도 관련)
						if($thematicMapFrame13.ui.mapList[0].mapMode != 'white'){
			...
			requestOpenApi : function(options) {
				...
				if 	    (api_id == "API_0301") $thematicMapFrame13Api.request.openApiTotalPopulation(options);
				else if (api_id == "API_0302") $thematicMapFrame13Api.request.openApiSearchPopulation(options);
				else if (api_id == "API_0303") $thematicMapFrame13Api.request.openApiInderstryCode(options);
				else if (api_id == "API_0304") $thematicMapFrame13Api.request.openApiCompany(options);
				else if (api_id == "API_0305") $thematicMapFrame13Api.request.openApiHouseHold(options);
				else if (api_id == "API_0306") $thematicMapFrame13Api.request.openApiHouse(options);
				else if (api_id == "API_0307") $thematicMapFrame13Api.request.openApiFarmHouseHold(options);
				else if (api_id == "API_0308") $thematicMapFrame13Api.request.openApiForestryHouseHold(options);
				else if (api_id == "API_0309") $thematicMapFrame13Api.request.openApiFisheryHouseHold(options);
				else if (api_id == "API_0310") $thematicMapFrame13Api.request.openApiHouseHoldMember(options);
				...
				if(shareInfo == null) {
					...
					if (shareInfo.checkShare("BMARK", srvType)) {
						...
						var params = {
								...
								"theme" : window.parent.$thematicMapMain.param.theme,
								"mapType" : window.parent.$thematicMapMain.param.mapType,
								"iframe_url" : window.location.protocol+"//"+window.location.host+"/view/thematicMap/thematicMapFrame13"
							};
			...
			doClearMap : function(type) {
				...
				$(".thematicCharts").hide();
	 			...
	 			var map1 = $thematicMapFrame13.ui.mapList[0];//20년수정반영 $thematicMapFrame04 => 03으로 수정)
				map1.legend.numberData = false;
				map1.checkShowCaption();
	 		},
			//20년수정반영 끝 (초기화 메서드)
	...
	// map event callback
	// ==============================//
	$thematicMapFrame13.callbackFunc = {
			...
			didMapZoomEnd : function(event, map) {
				...
				if (map.zoom < 10 && map.isInnerMapShow) {
					$thematicMapFrame13.ui.doInnerMap(map.id+1);
				}
			...
			didMapDropEnd : function(event, source, layer, data, map) {
				...
				$thematicMapFrame13.ui.curMapId = map.id;
				$thematicMapFrame13Api.request.combineFailCnt = 0;
				...
				if (id.split("-")[0] == "kosis") {
					...
					index = id.split("-")[1];
//					$thematicMapFrame13.ui.changeBtnLineColor("kosis", tmpParamList,
//							index, map);
					...
					$thematicMapFrame13.ui.searchBtnType = "normal";
					api_id = id.split("-")[0];
					index = id.split("-")[1];
					$interactiveLeftMenu.ui.updateSearchBtnEffect(id, map.id);
					...
					for (var i = 0; i < tmpParamList.length; i++) {
						if (tmpParamList[i].idx == index) {
							...
							$thematicMapFrame13.ui.dropBtnInfo[map.id] = tmpParamList[i];
							break;
						}
					}
				}
				...
				if (map.selectedBoundMode == "multi") {
					for (var i = 0; i < map.selectedBoundList.length; i++) {
						...
						$thematicMapFrame13.ui.curAdmCode = adm_cd;
						map.curAdmCd = adm_cd;
	
						if (tmpParam != null) {
							var params = $thematicMapFrame13.ui.reqSetParams(tmpParam, adm_cd, adm_nm, api_id, map);
							$thematicMapFrame13.ui.curDropParams[map.id] = params;
							$thematicMapFrame13.ui.requestOpenApi(params);
						}
					}
				} else {
					...
					$thematicMapFrame13.ui.curAdmCode = data.adm_cd;
					map.curAdmCd = data.adm_cd;
	
					if (tmpParam != null) {
						var params = $thematicMapFrame13.ui.reqSetParams(tmpParam,data.adm_cd, data.adm_nm, api_id, map);
						//console.log(params);
						//시계열 초기값 세팅
			...
			didMapDoubleClick : function(btn_id, tmpParam) {
				var map = $thematicMapFrame13.ui.mapList[$thematicMapFrame13.ui.curMapId];				
				var index = null;
				var adm_cd = "";
				var adm_name= "";
				var api_id = "";
				...
				if (id.split("-")[0] == "kosis") {
					...
					// 버튼 시각 효과
					$interactiveLeftMenu.ui.updateSearchBtnEffect(id, map.id);
					$thematicMapFrame13.ui.dropBtnInfo[map.id] = tmpParam;
				}
			...
			didMouseOverPolygon : function(event, data, type, map) {	
				if (type != "polygon") {
					...
					$thematicMapFrame13.ui.createInfoTooltip(event, data, type, map);
				}
			},
			...
			didSelectedPolygon : function(event, data, type, map) {
				if (type == "data") {
					...
					if ($thematicMapFrame13.ui.buildPopup != null) {
						$thematicMapFrame13.ui.buildPopup.close();
					}
					...
					$thematicMapFrame13.ui.buildPopup = 
						window.open(
				...
				$(".tb_close").click(function(){ 
					...
					if (sceneInx == 1) {
						...
						$(".sceneBox.on").each(function() {
							...
							tmpView.push(id);
							if (id == $thematicMapFrame13.ui.curMapId) {
								isSameView = true;
							}
						});						
						...
						if (!isSameView) {
							if (tmpView[0] < tmpView[1]) {
								$thematicMapFrame13.ui.curMapId = tmpView[0];
							}else {
								$thematicMapFrame13.ui.curMapId = tmpView[1];
							}
							
							var id = "view" + ($thematicMapFrame13.ui.curMapId + 1);
							switch($thematicMapFrame13.ui.curMapId) {
								case 0:
...
/*				$(".tb_mapAdd").click(function(){ 
				...
				$(".sceneBox").click(function(){
					...
					if (sceneInx > 1) {
						...
						if (id == "view1") {
							$thematicMapFrame13.ui.curMapId = 0;
							$(this).find(".toolBar").css("background", "#0070c0");
						}else if (id == "view2") {
							$thematicMapFrame13.ui.curMapId = 1;
							$(this).find(".toolBar").css("background", "#9ed563");
						}else {
							$thematicMapFrame13.ui.curMapId = 2;
			...
			refreshThemaList : function(select_obj) {
				...
				$thematicMapFrame13.getCategoryList.getMenuList(categoryId);
			}
			// mng_e 2019. 11. 20 j.h.Seok 이용자 중심 서비스 개편 - 정렬 방식 추가
	};
	
			$thematicMapFrame13.Popup = {
					show : function () {
...
//						var errorMsg = "<p>데이터 로딩중입니다. 잠시만 기다려주세요.</p>";
			...
			$thematicMapFrame13.request = {
				//카타고리 정보를 가져온다.
				getCategory : function () {
					$statsPotal.api.thematicMap.getCategory({
						method : 'POST',
						success : $thematicMapFrame13.response.successCateList
					});
				},
				...
				successCateList : function (stats, res) {
					...
					if (res.errCd === 0) {
						...
						for ( var i = 0; i < categoryList.length; i++) {	
							//djlee 2018-10-31 변경
//							var html = "<li style='background:url("+categoryList[i].category_icon_url+");background-size:30px 30px;background-repeat:no-repeat;background-position:left'><a href='javascript:$thematicMapFrame13.getCategoryList.getMenuList(\""+categoryList[i].thema_map_category+"\");'>"+categoryList[i].category_nm+"</a></li>";
							var html = '<li><a data-id="'+categoryList[i].thema_map_category+'" href="javascript:$thematicMapFrame13.getCategoryList.getMenuList(\''+categoryList[i].thema_map_category+'\');">'+categoryList[i].category_nm+'</a></li>';
							$('.themul').append(html);
//							var html = "<li style='background:url("+categoryList[i].category_icon_url+");background-size:30px 30px;background-repeat:no-repeat;background-position:left'><a href='javascript:$thematicMapFrame13.getCategoryList.getMenuList(\""+categoryList[i].thema_map_category+"\");'>"+categoryList[i].category_nm+"</a></li>";
//							html = '<li><a data-id="'+categoryList[i].thema_map_category+'" href="javascript:$thematicMapFrame13.getCategoryList.getMenuList(\''+categoryList[i].thema_map_category+'\');" title="'+categoryList[i].category_nm+'"><span>'+categoryList[i].category_nm+'</span></a></li>';
			...
			$thematicMapFrame13.getCategoryList = {
					getMenuList : function (thema_map_category){
						...
						$.ajax({
							  ...
							  success: function(res) {
								...
								html += 'onchange="javascript:$thematicMapFrame13.event.refreshThemaList(this)";>';
...
//								html += '<a id="sort_tooltip" class="theme_sort_tooltip" title="' + sortTooltipText + '"><img src="/img/ico/ico_help05.png" width="20" height="20" alt="정렬방식 도움말 팝업"/></a>';
								...
								$thematicMapFrame13.setCategoryList.setCategoryListHtml(res);
...
//       		 </li>
			$thematicMapFrame13.setCategoryList = {
				...
				$class("sop.portal.themeMapParamList.api").extend(sop.portal.absAPI).define(
						{
							onSuccess : function(status, res, options) {
								...
								if (result.param_info) {
									$thematicMapFrame13.ui.paramInfo = JSON.parse(res.result.themeParamInfoList[0].param_info);
									$thematicMapFrame13.ui.paramInfo.paramInfo["type"] = "bookmark"
									$thematicMapFrame13.ui.doAnalysisShareInfo($thematicMapFrame13.ui.paramInfo.paramInfo);
								}
							},
