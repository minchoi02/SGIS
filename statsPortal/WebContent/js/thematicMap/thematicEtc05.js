...
//	$("#selectValue").val("leftValue");
	...
	if(window.parent.$thematicMapMain.param.stat_thema_map_id == "wPsSdFX8Wt20210520161423833UZjHClj5U3"){
		$(".sqListBox").css("width","340px");
		$(".sqTabs").css("width","330px");
		$("#caracc_data_type,#caracc_type").show();
	}
	//보행자 교통사고 추가
	
	//코로나추가 20200722 주용민
	if(window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw"){
		$("#covid_thema_type,#covid_data_type,#covid_hr").show();
		$("#covid_thema_type a").removeClass("on");
		$("#covidThema1").addClass("first on");
		$("#covidThema2").addClass("last");
		$("#covidThema1").css({"background":"#1778cc","border":"1px solid #1778cc"});
	...
	if(window.parent.$thematicMapMain.param.stat_thema_map_id == "kmOpHLH5cK202106100936161097g5G9nLCFE"){
		$("#covid_thema_type,#covid_vacc_type,#vacc_type,#covid_hr").show();
		$("#covidChartDiv").show();
		$(".dataSideBox").css("height","550px");
		$(".covidDbTabs").css("display","table");
		$(".thematicCharts").css("margin-top","70px");
		$("#covid_thema_type a").removeClass("on");
		$("#covidThema1").addClass("first");
		$("#covidThema2").addClass("last on");
		$("#covidThema2").css({"background":"#1778cc","border":"1px solid #1778cc"});
		$("#region_boundary").hide();
	}

	//코로나추가 20200730 jrj
	$("body").on("click", "#covidPlay", function(){
		var that = $thematicMapFrame05.ui.mapList[0];
		...
		$thematicMapFrame05.playCovidTimer = setInterval(function(){
			playCovidTimer();
		}, 2000);
	});
	...
	//보행자 교통사고 발생현황추가
	$('body').on('click', "#caracc_type a", function(){
		$("#caracc_type a").removeClass("on");
		$(this).addClass("on");
		if($("#caracc_occur").hasClass("on")){
//			$thematicMapFrame05.ui.mapList[0].getThemaMapData('car_accident_data','00');	
			$thematicMapFrame05.ui.mapList[0].changeRegionBound();
		}else{
			$thematicMapFrame05.Popup.show();
			$thematicMapFrame05.ui.mapList[0].gMap.eachLayer(function(layer){
				if (layer.feature) {
					layer.remove(); 
				}
			});
			$thematicMapFrame05.ui.mapList[0].carAccident();
		}
	});
	//보행자 교통사고 발생현황추가
	...
	// 코로나 예방접종현황 추가
	$('body').on('click',"#covid_vacc_type a", function(){
		$("#covid_vacc_type a").removeClass("on");
		$(this).addClass("on");
		if($("#vacc_patient").hasClass("on")){
			$("#covidChartDiv").show();
			$("#vacc_type,#stat_sel,#base_year,#map_type,#data_type").show();
			$("#dataBoard,.dataSideBox").show();
			$(".covidDbTabs").show();
			$(".sop-right").stop().animate({"right":"405px"},200);
			if( !$(".interactiveDataBoard").hasClass("on") ){
				$(".interactiveDataBoard").click();
			}
			$thematicMapFrame05.ui.mapList[0].getThemaMapData('covid19_vacc_data','00');
			$thematicMapFrame05.ui.mapList[0].gMap.setZoom(1);
		}else if($("#vacc_hospital").hasClass("on")){
			$("#covidChartDiv").hide();
//			$("#vacc_type,#vacc_type2,#stat_sel,#base_year,#map_type,#data_type,#region_boundary").hide();
			$("#vacc_type,#stat_sel,#base_year,#map_type,#data_type,#region_boundary").hide();
			$thematicMapFrame05.ui.mapList[0].gMap.eachLayer(function(layer){
				if (layer.feature) {
					layer.remove(); 
				}
			});
			$thematicMapFrame05.ui.mapList[0].layerRemove = true;
	    	if ($thematicMapFrame05.ui.mapList[0].legend.circleMarkerGroup != null) {
	    		for (var i=0; i<$thematicMapFrame05.ui.mapList[0].legend.circleMarkerGroup.length;i++){
		    		var marker = $thematicMapFrame05.ui.mapList[0].legend.circleMarkerGroup[i];
		    		marker.remove();
		    	}
	    		$thematicMapFrame05.ui.mapList[0].legend.circleMarkerGroup = null;
	    	}
	    	$thematicMapFrame05.ui.mapList[0].gMap.setMaxZoom(11);
			$thematicMapFrame05.ui.mapList[0].covidVacc();
	    	
			$("#dataBoard,.dataSideBox").hide();
...
//			$("#vacc_type,#vacc_type2,#stat_sel,#base_year,#map_type,#data_type,#region_boundary").hide();
	...
	$('body').on('click',"#vacc_type a", function(){
		$("#vacc_type a").removeClass("on");
		$(this).addClass("on");
		$thematicMapFrame05.ui.mapList[0].getThemaMapDataboardData($thematicMapFrame05.ui.mapList[0].tmpAdmCd);
	});
//	$('body').on('click',"#vacc_type2 a", function(){
...
//	});
    ...
    $('body').on('click','.popup_tab li',function(){
    	...
    	$(this).addClass("on");
        if($(".con1").hasClass("on")){
        	$("#coronaVaccPop,#coronaPop").css("display","none");
        	$(".con1").addClass("on");
        	$("#coronaPop,#general_cnt").show();
        }else if($(".con2").hasClass("on")){
        	$("#coronaVaccPop,#coronaPop").css("display","none");
        	$(".con2").addClass("on");
        	$("#coronaVaccPop,#vacc_cnt").show();
        }
      });
	// 코로나 예방접종현황 추가
	
	//보행자 차량사고현황 추가
	$('body').on('click',"#caracc_data_type a", function(){
		$("#top_bottom_li a.last").trigger("click");
		$thematicMapFrame05.ui.mapList[0].topBottomState = "OFF";
		$("#top_bottom_li").hide();
		$("#caracc_data_type a").removeClass("on");
		$(this).addClass("on");
		if($("#caracc_occur").hasClass("on")){
			$("#stat_sel").show();
			$("#map_type").show();
			...
			$("#region_boundary a").removeClass("on");
			$("#region_boundary a:eq(0)").addClass("on");
			$("#region_boundary").show();

			$("#dataBoard,.dataSideBox, .thematicCharts").show();
			$(".sop-right").stop().animate({"right":"405px"},200);
			if( !$(".interactiveDataBoard").hasClass("on") ){
				$(".interactiveDataBoard").click();
			}
		...
		}else if($("#caracc_occur_point").hasClass("on")){
			$thematicMapFrame05.Popup.show();
			$("#stat_sel").hide();
			$("#region_boundary").hide();
			$("#map_type").hide();
	...
	$("#quar").click(function(){
		...
		$thematicMapFrame05.ui.mapList[0].getThemaMapDataboardData("99");
		$thematicMapFrame05.ui.mapList[0].selectedAdmCd = "99";
		$thematicMapFrame05.ui.mapList[0].tmpAdmCd = "99";
		$thematicMapFrame05.ui.mapList[0].monthChange = true;
		$thematicMapFrame05.ui.mapList[0].makeCovidChart();
		thematicCharts("99");
		$(".covidDbTabs").hide();
		$(".thematicCharts").css("margin-top","10px"); 
		$(".dataSideBox").css("height","500px");
		$(".highcharts-subtitle").empty().text("검역");
		$thematicMapFrame05.ui.mapList[0].monthChange = false;
	});
	...
	$("body").on("click", "#covidStop", function(){
		...
		clearInterval( $thematicMapFrame05.playCovidTimer );
		$thematicMapFrame05.ui.mapList[0].covidPlayYn = false;
	});
	...
	$('body').on('click', "#covid_data_type a", function(){
		...
		if($("#covid_patient").hasClass("on")){
			...
			//코로나추가 20200729 jrj
			$thematicMapFrame05.ui.mapList[0].tmpAdmCd = "00";
			$thematicMapFrame05.ui.mapList[0].selectedAdmCd = "00";
			$thematicMapFrame05.ui.mapList[0].getThemaMapData('covid19_status','00');
			...
			$thematicMapFrame05.ui.mapList[0].gMap.setZoom(1);	//코로나추가 20200728 jrj
			$thematicMapFrame05.ui.mapList[0].gMap.setMaxZoom(1);	//코로나추가 20200728 jrj
			
			$thematicMapFrame05.ui.mapList[0].isFirstDraw = true;
	    	...
	    	$thematicMapFrame05.ui.mapList[0].gMap.setMaxZoom(11);	//코로나추가 20200728 jrj
			$thematicMapFrame05.ui.mapList[0].covid();
	...
	$('body').on('click', "#covid_thema_type a", function(){
		$("#covid_thema_type a").removeClass("on");
		$(this).addClass("on");
		$(this).css({"background":"#1778cc","border":"1px solid #1778cc"});
	});
//코로나추가 20200722 주용민	
	//통계선택에서 경계를 초기화한다.
...
function playCovidTimer(){
	if(window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw" || window.parent.$thematicMapMain.param.stat_thema_map_id == "kmOpHLH5cK202106100936161097g5G9nLCFE"){
		...
		if( that.covidPlayMonth == "01" && temp[0] == "12" ){
			that.covidPlayYear = Number( that.thema_stat_data_base_year ) - 1;
			$("#select_base_year").val( that.covidPlayYear );
			that.covidMonth();
		} else if( that.covidPlayMonth == "12" && temp[0] == "01" ) {
			that.covidPlayYear = that.thema_stat_data_base_year;
			$("#select_base_year").val( that.covidPlayYear );
			that.covidMonth();
		}

		that.covidPlayMonth = temp[0];
...
function thematicCharts(selectedAdmCd, isFirst){
	if(window.parent.$thematicMapMain.param.stat_thema_map_id == "uHaX8JJqyh20201014110114231lgMgsJnINP"){
		...
		if($("#selectValue").val() == "leftValue"){
			$(".thematicBotText").empty().append("출처 : " + $thematicMapFrame05.ui.mapList[0].left_sep_source).css("margin-top","-5px").after("<p class='clink' style='font-size:12px;width:420px;text-align:center;'>상세통계표 보기 : <a style='font-size:12px;text-decoration:underline;color:blue;' href='https://kosis.kr/statHtml/statHtml.do?orgId=350&tblId=DT_35001_A089&vw_cd=MT_ZTITLE&list_id=350_35001_6&seqNo=&lang_mode=ko&language=kor&obj_var_id=&itm_id=&conn_path=MT_ZTITLE' target='_blank'>국가통계포털(KOSIS)</a></p>");
	...
	//20년수정반영 시작
	if($thematicMapFrame05.ui.mapList[0].chartReload != undefined && !$thematicMapFrame05.ui.mapList[0].chartReload) {
		$thematicMapFrame05.ui.mapList[0].chartReload = true;
		return false;
	}
	//20년수정반영 끝
...
//			var tempTitle = "["+$("#covid_month >option:selected").val() + "월 " + $("#covid_day >option:selected").val() + "일 00시 기준] ";			
			var tempTitle = "["+$("#select_base_year >option:selected").val() + "년 " + $("#covid_month >option:selected").val() + "월 " + $("#covid_day >option:selected").val() + "일 00시 기준] ";
		}  else {
//			var tempTitle = "["+ map.covidLastMonth + "월" + map.covidLastDay + "일 00시 기준]";
			...
			var tempTitle = "["+ tempYear + "년 " + tempMonth + "월" + tempDate + "일 00시 기준]";
		}
		if( window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw"){
			...
			left_sep_chart_title = '<span style="font-size:12px;">'+tempTitle+'</span><br/>' + $("#vacc_type a.on").text() + " " + left_sep_chart_title + " (" + map.left_sep_unit + ")";
			right_sep_chart_title = '<span style="font-size:12px;">'+tempTitle+'</span><br/>' + $("#vacc_type a.on").text() + " " + right_sep_chart_title + " (" + map.right_sep_unit + ")";
		}
	}
	...
	for(var i=0;i<map.dataBoardData.length;i++){
		...
		if(map.dataBoardData[i].adm_cd == selectedAdmCd){
			if($(".select_quarter").hasClass("on")){
				...
				//코로나추가 20200722 주용민
				}else if(window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw" || window.parent.$thematicMapMain.param.stat_thema_map_id == "kmOpHLH5cK202106100936161097g5G9nLCFE"){
					// 20210302 김건민 (2020년 월  과 2021년 월을 구분하기 위해서 수정함)
					//xAxisCat.push(map.dataBoardData[i].base_month+'월');
					xAxisCat.push("'"+ map.dataBoardData[i].base_year.substr(2,2)+'년 ' +map.dataBoardData[i].base_month+'월');
					// 20210320 김건민
					$("#covidDbTabs_toggle1").removeClass("on");
					$("#covidDbTabs_toggle2").addClass("on");
				//코로나추가 20200722 주용민										
	...
	if (selectedAdmCd == "00") {
		...
		if(window.parent.$thematicMapMain.param.stat_thema_map_id == "uHaX8JJqyh20201014110114231lgMgsJnINP"){
			...
			left_sep_chart_title = $('#select_base_year option:selected').val() + "년 "+ $("#infection_list a.on").text() + " " + left_sep_chart_title;
			right_sep_chart_title = $('#select_base_year option:selected').val() + "년 "+ $("#infection_list a.on").text() + " " + right_sep_chart_title;
		}else if(window.parent.$thematicMapMain.param.stat_thema_map_id == "8mjCr1kWql20201123140825772vKMjQsNcRw"){
			...
			left_sep_chart_title = $('#select_base_year option:selected').val() + "년 "+ $("#caracc_type a.on").text() + " " + $("#leftValue").text();
		}
	}else{
		if(window.parent.$thematicMapMain.param.stat_thema_map_id == "QNj43PFUT220190612100733746ocaFOXLaj3"){
			if($(".select_quarter").hasClass("on")){
				...
				left_sep_chart_title = "연도별 " + left_sep_chart_title;				
				right_sep_chart_title = "연도별 " + right_sep_chart_title;
			}
		}else{
			if(window.parent.$thematicMapMain.param.stat_thema_map_id == "uHaX8JJqyh20201014110114231lgMgsJnINP"){
				left_sep_chart_title = "연도별 신규 "+ $("#cancer_list a.on").text() +" "+ left_sep_chart_title;
				right_sep_chart_title =  "연도별 누적 "+ $("#cancer_list a.on").text() +" " + right_sep_chart_title;
			}else if(window.parent.$thematicMapMain.param.stat_thema_map_id == "MmLSKu8PgQ20201022173339083sOVy6YCGQj"){
				left_sep_chart_title =  "연도별 "+ $("#infection_list a.on").text() + " " + left_sep_chart_title;
				right_sep_chart_title =  "연도별 "+ $("#infection_list a.on").text() + " " + right_sep_chart_title;
			}else if(window.parent.$thematicMapMain.param.stat_thema_map_id == "8mjCr1kWql20201123140825772vKMjQsNcRw"){
				left_sep_chart_title =  "연도별 "+ $("#bike_type a.on").text() + " " + $("#leftValue").text();
				right_sep_chart_title =  "연도별 "+ $("#bike_type a.on").text() + " " + $("#rightValue").text();
			}else	if(window.parent.$thematicMapMain.param.stat_thema_map_id == "RZ3pr7Maou20201106160851389D0RGtYCGpW"){
				left_sep_chart_title =  "연도별 "+ $("#weather_type1 a.on").text() + " " + $("#leftValue").text();
				right_sep_chart_title =  "연도별 "+ $("#weather_type2 a.on").text() + " " + $("#rightValue").text();
			}else if(window.parent.$thematicMapMain.param.stat_thema_map_id == "wPsSdFX8Wt20210520161423833UZjHClj5U3"){
				left_sep_chart_title =  "연도별 "+ $("#caracc_type a.on").text() + " " + $("#leftValue").text();
			}else{
				left_sep_chart_title = "연도별 " + left_sep_chart_title;				
				right_sep_chart_title = "연도별 " + right_sep_chart_title;
			}
...
//			            categories: xAxisCat,
	...
	//		            yAxis: 1,
									 ...
									 //20년수정반영 시작
										point: {
											events: {
												click: function () {
													if($thematicMapFrame05.ui.mapList[0].base_year_list.includes(this.category.replace("년",""))){
														$thematicMapFrame05.ui.mapList[0].isChartClick = true;
														$('select[id=select_base_year]').val(this.category.replace("년",""));
														$thematicMapFrame05.ui.mapList[0].isDataboardReset = false;
														$thematicMapFrame05.ui.mapList[0].chartReload = false;
														$thematicMapFrame05.ui.mapList[0].changeRegionBound();
													}
												}
											}
	...
	//		            yAxis: 1,
					...
					var rotaionValue = 0
					var ydistance = 0
					if (Math.max.apply(null, retDataList) >= 1000 && retDataList.length > 13){
						rotaionValue = -45; 
						ydistance = -15;
					}
					$('.thematicCharts').highcharts({
	...
	//						         max : retDataList[0] * 1.3
								 ...
								 tooltip: {
									 ...
									 useHtml : true,
									 formatter : function( evt){ 
										 // 20210302 김건민 (코로나19 발생 현황에서 신규 데이터보드 월별 차트, 지역별 차트 수정함.)
										 if(window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw"){
											 var isShow = false;
											 var etcText = ""; 
											//var title = this.points[0].series.name;
											 ...
											 var html = "";
											 html += "<span style='font-size:9px;'>"+"20"+this.x + "</span><br/>";
											 html += "<span>" + title;
											 html += " : " + "<b>" + appendCommaToNumber(this.y) + " (" + map.right_sep_unit + ")"+"</b></span>";
										 }else{
											 var isShow = false;
											 var etcText = ""; 
											 var title = this.points[0].series.name;
...
//		            yAxis: 1,
	...
	//		            max : retDataList[0] * 1.3
					...
					tooltip: {
						...
						useHtml : true,
						formatter : function( evt){ 
							// 20210302 김건민 (코로나19 발생 현황에서  데이터보드 월별 차트, 지역별 차트 수정함.)
							 if(window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw"){
								 var isShow = false;
								 var etcText = ""; 
								//var title = this.points[0].series.name;
								 var title = map.right_sep_chart_title;
...
function sideEvent(){
	...
	$('#tableTimeSeries').on("click","li>div",function(){
	    ...
	    $thematicMapFrame05.ui.mapList[0].isChartClick = true;
		$thematicMapFrame05.ui.mapList[0].isDataboardReset = true;
        $thematicMapFrame05.ui.mapList[0].chartReload = true;
        //$thematicMapFrame05.ui.mapList[0].changeRegionBound();
			...
			if(theme){
				$(".themul > li").each(function(index , elem){
					if(theme == $(elem).find("a").data("id")){
						...
						$thematicMapFrame05.getCategoryList.getMenuList(theme);
						$(elem).find("a").click();
					}
				});
...
//		}
		...
		if($(this).attr('id') == "color"){
			$("#dataMode").val("color");
			if(!(window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw" || 
					window.parent.$thematicMapMain.param.stat_thema_map_id == "RZ3pr7Maou20201106160851389D0RGtYCGpW"  || 
					window.parent.$thematicMapMain.param.stat_thema_map_id == "kmOpHLH5cK202106100936161097g5G9nLCFE"))
			$("#region_boundary").show();
			//코로나추가 20200722 주용민
			$("#data_type").show();
		}
...
//		if(window.parent.$thematicMapMain.param.stat_thema_map_id == "wPsSdFX8Wt20210520161423833UZjHClj5U3"){
//			$thematicMapFrame05.ui.mapList[0].getThemaMapBaseYear('car_accident_data');
//		}
	...
	body.on("click",".sqList #top_bottom_li a", function(){
		...
		if(obj[0].id == "top_bottom_off") {
			...
			$("#top_bottom_select2").hide();
			$("#top_bottom_select_count2").val("10");
		} else {
			$("#top_bottom_type").show();
			if(window.parent.$thematicMapMain.param.stat_thema_map_id == "wPsSdFX8Wt20210520161423833UZjHClj5U3"){
				$("#top_bottom_select2").show();
				$("#top_bottom_type_both").hide();
				$("#top_bottom_type a").removeClass("on");
				$("#top_bottom_type a:eq(0)").addClass("on");
				if(!($(".interactiveDataBoard").hasClass("on"))){
    ...
    */
	...
	body.on("click", ".themul li>a", function(){   
		if(window.parent.$thematicMapMain.param.stat_thema_map_id == "kmOpHLH5cK202106100936161097g5G9nLCFE"){
			$("label[data-id='sAXkcVzk5V202007141335257355ued9032uw']").addClass("on");
		}
...
function initTopBottomButtons() {
	...
	$thematicMapFrame05.ui.mapList[0].topBottomState = "OFF";
	if(window.parent.$thematicMapMain.param.stat_thema_map_id == "wPsSdFX8Wt20210520161423833UZjHClj5U3"){
		$thematicMapFrame05.ui.mapList[0].topBottomType = "top";
	}else{
		$thematicMapFrame05.ui.mapList[0].topBottomType = "both";
	}
	$thematicMapFrame05.ui.mapList[0].topBottomCount = "10";
}
//mng_e 2019. 11. 21 j.h.Seok 이용자 중심 서비스 개편 - 상하위 10개 표출
