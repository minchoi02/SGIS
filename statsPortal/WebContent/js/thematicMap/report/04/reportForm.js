	...
	$reportForm.ui = {
				...
				setMainReportInfo : function(data) {
					...
					if (this.mapType == "thematicMap") {
						if(data.param.stat_thema_map_id == "FlM5JUWj0T20181120214718029IpoBrl4UVw") {/** TODO : stat_thema_map_id */
							var mapDataId = "";
							$.each(window.opener.$('#stat_sel').find('a'), function(i) {
								if($(this).hasClass('on')) {
									mapDataId = $(this).context.id;
								}
							});
							if(mapDataId == "dataType00")
								mainTitle = "시도별 귀농/귀촌/귀어 인구 현황" + "("+data.data.result[0].base_year+")";//전체
							if(mapDataId == "dataType01")
								mainTitle = "시도별 귀농 인구 현황" + "("+data.data.result[0].base_year+")";//귀농
							if(mapDataId == "dataType02")
								mainTitle = "시도별 귀어 인구 현황" + "("+data.data.result[0].base_year+")";//귀어
							if(mapDataId == "dataType03")
								mainTitle = "시도별 귀촌 인구 현황" + "("+data.data.result[0].base_year+")";//귀촌
						}
						else {
...
//								var feature = dataGeoJson[i]._layers[k].feature;
							...
							if (feature.info.length > 0) {
								var tmpValue = null;
								var tmpRightValue = null;
								info = feature.info[0];
								tmpValue = info[info.showData];
								tmpRightValue = info['right_data_val'];
								value = parseFloat(tmpValue);
								rightValue = parseFloat(tmpRightValue);
...
//					}
				...
				drawGrid : function(data,param) {
					...
					var themaMapId = param.param.stat_thema_map_id;
					if(themaMapId == "FlM5JUWj0T20181120214718029IpoBrl4UVw") {/** TODO : stat_thema_map_id */ //귀농귀촌귀어 추가
						var gridList = gridList.sort(function (a, b) {
							return b.right_value - a.right_value 
						});
						var rightValue = gridList[0].right_value;
						var gridList = gridList.sort(function (a, b) {
							return b.value - a.value 
						});
						for (var i=0; i<gridList.length; i++) {
							html += "<tr>";
			 				...
			 				if(gridList[i].right_value == 0) {
			 					html += "<td class='ar'><div class='valueBox'><span class='bg' style='width: 0%; background:rgba(76,139,253,1); '></span><span class='txt'>N/A</span></td>";
			 				} else if(gridList[i].right_value < 0) {
			 					html += "<td class='ar'><div class='valueBox'><span class='bg' style='width:"+(gridList[i].right_value/rightValue*100)+"%;'></span><span class='txt'>"+appendCommaToNumber(gridList[i].right_value)+"</span></td>";
			 				}else{
			 					html += "<td class='ar'><div class='valueBox'><span class='bg' style='width:"+(gridList[i].right_value/rightValue*100)+"%; background:rgba(76,139,253,1); '></span><span class='txt'>"+appendCommaToNumber(gridList[i].right_value)+"</span></td>";
			 				}
			 				html += "</tr>";
						}
						$("#mask").hide();
