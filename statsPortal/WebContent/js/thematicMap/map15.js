	...
	sMap = {
			...
			map : function() {
				...
				this.showTopBottomDataOnly = function(state, refresh) {
					...
					$thematicMapFrame15.ui.dataGeoJson = [];
...
//					}
				...
				this.addPolygonGeoJson = function (obj, type, opt) {
					...
					if (type == "polygon" || type == "build") {
						...
						$thematicMapFrame15.ui.dataGeoJson.push(geojson);
								...
								if (feature.info[x].showData) {
									for (param in feature.info[x]) {
										...
										if (param == feature.info[x].showData) {
											...
											if((window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw" || window.parent.$thematicMapMain.param.stat_thema_map_id == "MmLSKu8PgQ20201022173339083sOVy6YCGQj" )  
...
//													layer.setCaption({title:appendCommaToNumber(layer.feature.info[0][param])+ " ("+layer.feature.info[0].unit + ")", color:"white", showAllZoomLevel: false});
				...
				this.setPolygonDataGeojson = function (geoData, type, index, options, callback) {
					...
					$thematicMapFrame15.Popup.close();
					// 기존 경계 지우기
					if (this.geojson) {
					...
					if (geoData && geoData.combine && this.data.length > 0) {
						...
						if ($("#selectValue2").val()!="auto" && this.legend.selectType =="bubble"){
							$thematicMapFrame15.ui.mapList[0].multiLayerControl.dataGeojson = $thematicMapFrame15.ui.dataGeoJson;							
						}
						if($("#dataMode").val() == "pieChart"){
...
//								_marker.bindInfoWindow(res.result[i].x_coord + ", " + res.result[i].y_coord);
				...
				this.changeStatThemaId = function(position, refresh) {
					if($thematicMapFrame15.params.stat_thema_map_id === 'uDwMJtEFMv202012011643473308MxEquqzIL') {
						// 통근통학 인구변화
						$("#change_stat_thema_left").text("시군구내");
						$("#change_stat_thema_middle").text("타시군구");
						$("#change_stat_thema_right").text("대중교통");
						
						if(position === 'left') {
							// 시군구내
							that.thema_map_data_id = 'kosis_samenss_sgg_in_atndf_atndsk_ppltn_cnt_per';
							that.left_sep_chart_title = '시군구내 통근통학 인구수(명)';
							that.right_sep_chart_title = '시군구내 통근통학 인구 비율(%)';
						} else if(position === 'middle') {
							// 타시군구
							that.thema_map_data_id = 'kosis_in_otr_sgg_atndf_atndsk_ppltn_cnt_per';
							that.left_sep_chart_title = '타시군구 통근통학 인구수(명)';
							that.right_sep_chart_title = '타시군구 통근통학 인구 비율(%)';
						} else if(position === 'right') {
							// 대중교통
							that.thema_map_data_id = 'kosis_tfcmn_atndf_atndsk_ppltn_cnt_per';
							that.left_sep_chart_title = '대중교통 통근통학 인구수(명)';
							that.right_sep_chart_title = '대중교통 통근통학 인구 비율(%)';
						}
					} else if($thematicMapFrame15.params.stat_thema_map_id === 'vtGnsKvxDD20201208103200888xDu9wqLppt') {
						// 노지과수(4종) 재배면적 변화
						// 기본 3개 항목 외에 추가 항목 있으므로 ID 값 확인 필요 (포도 - middle1)
						$("#stat_sel").hide();
						
						$("#change_stat_thema_middle1").show();
						
						$("#change_stat_thema span").text("통계선택");
						
						$("#change_stat_thema_left").text("사과(ha)");
						$("#change_stat_thema_middle").text("배(ha)");
						$("#change_stat_thema_middle1").text("포도(ha)");
						$("#change_stat_thema_right").text("복숭아(ha)");
						
						if(position === 'left') {
							// 사과
							$("#selectValue").val("leftValue");
							that.thema_map_data_id = 'kosis_apple_pear_ctvt_area';
						} else if(position === 'middle') {
							// 배
							...
							// 포도
							$("#selectValue").val("leftValue");
							that.thema_map_data_id = 'kosis_grape_peach_ctvt_area';
						} else if(position === 'right') {
							// 복숭아
							$("#selectValue").val("rightValue");
							that.thema_map_data_id = 'kosis_grape_peach_ctvt_area';
						...
						that.left_sep_chart_title = '연도별 재배면적(ha)';
						that.right_sep_chart_title = '연도별 재배면적(ha)';
					} else if($thematicMapFrame15.params.stat_thema_map_id === 'DpwHKoIME020201208110725432IJM0KuKKGK') {
						// 노지채소(6종) 재배면적 변화
						// 기본 3개 항목 외에 추가 항목 있으므로 ID 값 확인 필요 (마늘 - middle1, 양파 - middle1, 배추 - middle1)
						$("#stat_sel").hide();
						
						$("#change_stat_thema_middle1").show();
						$("#change_stat_thema_middle2").show();
						$("#change_stat_thema_middle3").show();
						
						$("#change_stat_thema span").text("통계선택");
						
						$("#change_stat_thema_left").text("고추(ha)");
						$("#change_stat_thema_middle").text("파(ha)");
						$("#change_stat_thema_middle1").text("마늘(ha)");
						$("#change_stat_thema_middle2").text("양파(ha)");
						$("#change_stat_thema_middle3").text("배추(ha)");
						$("#change_stat_thema_right").text("무(ha)");
						
						$("#change_stat_thema").css("display", "table-cell");
						$("#change_stat_thema_middle2").addClass("last");
						$("#change_stat_thema_middle3").addClass("first");
						
						if(position === 'left') {
							// 고추
							$("#selectValue").val("leftValue");
							that.thema_map_data_id = 'kosis_redpepper_scallion_ctvt_area';
						} else if(position === 'middle') {
							// 파
							$("#selectValue").val("rightValue");
							that.thema_map_data_id = 'kosis_redpepper_scallion_ctvt_area';
							...
							// 마늘
							$("#selectValue").val("leftValue");
							that.thema_map_data_id = 'kosis_garlic_onion_ctvt_area';
						} else if(position === 'middle2') {
							// 양파
							$("#selectValue").val("rightValue");
							that.thema_map_data_id = 'kosis_garlic_onion_ctvt_area';
						} else if(position === 'middle3') {
							// 배추 (left-right 잘 볼것)
							$("#selectValue").val("rightValue");
							that.thema_map_data_id = 'kosis_daikon_cabbage_ctvt_area';
						} else if(position === 'right') {
							// 무 (left-right 잘 볼것)
							$("#selectValue").val("leftValue");
							that.thema_map_data_id = 'kosis_daikon_cabbage_ctvt_area';
					...
					if(refresh) {
//						$thematicMapFrame15.Popup.show();
						that.isReloadMode = true;
						that.openApiReverseGeoCode(that.center, that.bnd_year);
					} else {
						$("#change_stat_thema_left").addClass("on");
						that.openInitStatData($thematicMapFrame15.params);
					}
				},
				// mng_e 2020. 12. 01 j.h.Seok 통계주제도 통합
...
//								$(".helperText span").text(res.result.getThemaStatsInfo[0].title);
								...
								if (result.left_sep_nm !=null && result.left_sep_unit !=null && result.left_sep_nm.length > 0 && result.left_sep_unit.length > 0){
									var leftHtml= '<a href="javascript:srvLogWrite(\'B0\',\'03\',\'02\',\'01\',\''+result.title+'\',\'\');$thematicMapFrame15.Popup.show();$thematicMapFrame15.ui.changeLeftRightValue(1)" class="first last" id="leftValue">'+ result.left_sep_nm+'('+result.left_sep_unit+')</a>'
								...
								if (result.right_sep_nm !=null && result.right_sep_unit!=null && result.right_sep_nm.length > 0 && result.right_sep_unit.length > 0){
									var rightHtml= '<a href="javascript:srvLogWrite(\'B0\',\'03\',\'02\',\'02\',\''+result.title+'\',\'\');$thematicMapFrame15.Popup.show();$thematicMapFrame15.ui.changeLeftRightValue(2)" class="last" id="rightValue">'+ result.right_sep_nm+'('+result.right_sep_unit+')</a>'
								...
								if (result.add_data_disp_yn != null && result.add_data_disp_yn == "Y") {
									...
									var rightHtml= '<a href="javascript:$thematicMapFrame15.Popup.show();$thematicMapFrame15.ui.changeLeftRightValue(3)" class="last" id="etcValue">'+ result.sep_map_left_sep_nm+'('+result.sep_map_left_sep_unit+')</a>'
									$('#stat_sel').append(rightHtml);			
								}
								...
								if(window.parent.$thematicMapMain.param.stat_thema_map_id == "QNj43PFUT220190612100733746ocaFOXLaj3"){
									...
									$('#map_type').append('<a href="javascript:$thematicMapFrame15.ui.changeDataMode()" class="last" id="pieChart">파이차트</a>');
...
//									$("#sido").hide();
								...
								if(that.thema_legend_type == '3') {
									that.getLegendValues($thematicMapFrame15.params);
								} else {
									$('#themaInit_'+that.legend.id).trigger("click");
								}
...
//								}
								...
								var output = "";
								//코로나추가 20200722 주용민
								if(window.parent.$thematicMapMain.param.stat_thema_map_id != "QNj43PFUT220190612100733746ocaFOXLaj3" && window.parent.$thematicMapMain.param.stat_thema_map_id != "onb6f4rRh320190902160751679iQUr3aVwTT" && window.parent.$thematicMapMain.param.stat_thema_map_id != "sAXkcVzk5V202007141335257355ued9032uw"){
								//코로나추가 20200722 주용민
									output = '<span>년도선택</span>';
									output += '<select id="select_base_year" onchange="javascript:srvLogWrite(\'B0\',\'03\',\'03\',\'00\',$(\'.helperText span\').text(),$(\'#select_base_year option:selected\').val()+\'년\');$thematicMapFrame15.Popup.show();$thematicMapFrame15.ui.mapList[0].changeRegionBound()">';
...
//									that.getThemaMapData("cancer_data","00",cancer_type_lv.substr(-1,1));
								...
								if(window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw"){
									...
									covidHtml += '<input id="select_base_year" type="hidden" value="'+list[0]+'">';
									covidHtml += '<select id="covid_month" onchange="javascript:$thematicMapFrame15.ui.mapList[0].covidDay();$thematicMapFrame15.ui.mapList[0].changeRegionBound(true);">';
									
									for(var i=0;i<list_month.length-1;i++){
										covidHtml += '<option value='+list_month[i]+'>';
										covidHtml += list_month[i]+"월";
										covidHtml += '</option>';
									}
									covidHtml += '<option selected="true" value='+list_month[list_month.length-1]+'>'+list_month[list_month.length-1]+"월"+'</option>';
									
									$("#base_year").append(covidHtml);
									
									that.covidLastMonth = list_month[list_month.length-1];
									that.covidDay();
								}
								//코로나추가 20200722 주용민
								//카드 사용금액
...
//									output4 += '<select id="select_base_year" onchange="javascript:$thematicMapFrame15.ui.mapList[0].changeRegionBound()">';
									output4 += '<select id="select_base_year" onchange="javascript:$thematicMapFrame15.ui.mapList[0].makeMonth();$thematicMapFrame15.ui.mapList[0].changeRegionBound();">';
...
//								if(window.parent.$thematicMapMain.param.stat_thema_map_id == "onb6f4rRh320190902160751679iQUr3aVwTT"){
//									output4 = '<span>월별선택</span>';
//									output4 += '<select id="select_base_year" onchange="javascript:srvLogWrite(\'B0\',\'03\',\'03\',\'00\',$(\'.helperText span\').text(),$(\'#select_base_year option:selected\').val()+\'년\');$thematicMapFrame15.Popup.show();$thematicMapFrame15.ui.mapList[0].changeRegionBound()">';
//									
//									for(var i=0;i<list.length-1;i++){
//										output4 += '<option value='+list[i]+'>';
...
//								}
								...
								if(window.parent.$thematicMapMain.param.stat_thema_map_id == "QNj43PFUT220190612100733746ocaFOXLaj3"){
									...
									output += '<a onclick="$thematicMapFrame15.ui.mapList[0].periodClick(\'select_base_year\')" class="first select_base_year" style="cursor:pointer;">년도</a>';
									output += '<a onclick="$thematicMapFrame15.ui.mapList[0].periodClick(\'select_quarter\')" class="select_quarter" style="cursor:pointer;">분기</a>';
									output += '<a onclick="$thematicMapFrame15.ui.mapList[0].periodClick(\'select_monthly\')" class="last select_monthly" style="cursor:pointer;">월</a>	';
									output += '<br/><select id="select_base_year" style="margin-left:66px;" onchange="javascript:srvLogWrite(\'B0\',\'03\',\'03\',\'00\',$(\'.helperText span\').text(),$(\'#select_base_year\').val()+\'년\');javascript:$(\'#select_quarter option:last\').prop(\'selected\',true);javascript:$(\'#select_monthly option:last\').prop(\'selected\',true);$thematicMapFrame15.Popup.show();$thematicMapFrame15.ui.mapList[0].changeRegionBound()">';
									...
									$.ajax({
										...
										success: function(res) {
											if (res.errCd == "0") {
												...
												output2 += '<select id="select_quarter" style="margin-left:5px;display:none;" onchange="$thematicMapFrame15.ui.mapList[0].changeRegionBound();">';
//												output2 += '<option value="0">분기</option>';
									...
									$.ajax({
										...
										success: function(res) {
											if (res.errCd == "0") {
												var result = res.result;
												output3 += '&nbsp;<select id="select_monthly" style="margin-left:5px;display:none;" onchange="$thematicMapFrame15.ui.mapList[0].changeRegionBound();">';
...
//					that.tmpAdmCd = adm_cd;
					...
					//코로나추가 20200722 주용민
					if(window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw"){
						$thematicMapFrame15.ui.removeCovidMarker();
												...
												for(var i=0;i<result.length;i++){
													...
													general_cnt_html = "" 
														...
														//+"    <html><head></head><body><main>                                                                                             "
														   ...
														   +"                <h2 style='font-size:14px;text-align:center;'>" + result[i].base_month +"." + result[i].base_day + ". 00시 기준, 01.03. 이후 누계</h2>                                                                               "
														   +"            </div>                                                                                                      "
														   +"            <section class='cont1'>                                                                                     "
														   ...
														   +"                            <div style='font-size:20px;'  style='margin:0; padding:0; box-sizing: border-box; word-break:keep-all;' class='num'>" + result[i].incdec + "</div>                                                                   "
														   +"                        </div>                                                                                          "
														   +"                    </div>                                                                                              "
														   +"                </div>                                                                                                  "
														   ...
														   +"                            <div style='font-size:20px;'   style='box-sizing: border-box;'  class='num'>" + result[i].localocccnt + "</div>                                                                    "
														   +"                        </div>                                                                                          "
														   +"                    </div>                                                                                              "
														   ...
														   +"                            <div style='font-size:20px;'   style='box-sizing: border-box;'  class='num'>" + result[i].overflowcnt + "</div>                                                                   "
														   +"                        </div>                                                                                          "
														   +"                    </div>                                                                                              "
														   +"                </div>                                                                                                  "
														   +"            </section>                                                                                                  "
														   ...
														   +"                        <div style='font-size:14px; color:white;' class='plus'>" + result[i].defcnt_pm + corona_img + " </div>                        "
														   +"                    </li>                                                                                               "
														   ...
														   +"                        <div style='font-size:14px; color:white;' class='plus'>" + result[i].isolclearcnt_pm + corona_img2 + " </div>                        "
														   +"                    </li>                                                                                               "
														   ...
														   +"                        <div style='font-size:14px; color:white;' class='plus'>" + result[i].isolingcnt_pm + corona_img3 + " </div>                        "
														   +"                    </li>                                                                                               "
														   ...
														   +"                        <div style='font-size:14px; color:white;' class='plus'>" + result[i].deathcnt_pm + corona_img4 + " </div>                        "
														   +"                    </li>                                                                                               "
														   +"                </ul>                                                                                                   "
														   +"            </section>                                                                                                  "
														   ...
														   +"                    <input type='checkbox' name='corona_close' value='OK' id='corona_close' onclick=\"$thematicMapFrame15.ui.closeWin('coronaPop', 'N', 1);\"><label for='corona_close'>1일간 이 창을 더 이상 열지 않음</label>         "
														   +"                </div>                                                                                                  "
														   +"                <button     class='fRight'  onclick=\"$thematicMapFrame15.ui.closeWin('coronaPop', 'N', 1);\">창닫기<img src='/images/corona/btn_close2.png' alt='close'>                       "
														   +"                </button>                                                                                               "
														   +"                                                                                                                        "
														   +"            </div>                                                                                                      "
														   +"            <button     class='cls'  onclick=\"$thematicMapFrame15.ui.closeWin('coronaPop', 'N', 1);\"><img src='/images/corona/btn_close.png' alt='close' ></button>                            "
														   +"        </div>                                                                                                          "
														   +"    </main></body></html>                                                                                                          "
														   ;
												}
...
//										                pointFormat: '<span style="color:{point.color}">{point.name}</span>: <b>{point.y:.2f}%</b><br/>'
				...
				this.changeRegionBound = function( monthChange ){
					...
					var map1 = $thematicMapFrame15.ui.mapList[0];
					...
					that.markers.clearLayers();
					that.poiAdmCd = "";
					$thematicMapFrame15.ui.poiInfoArray = [];
...
//						$("#select_quarter,#select_monthly").hide();
				...
				this.periodClick = function(that){
					...
					$("#select_quarter,#select_monthly").hide();
					pId.show();
					$thematicMapFrame15.Popup.show();$thematicMapFrame15.ui.mapList[0].changeRegionBound();
				};
					...
					$.ajax({
						...
						success: function(res) {
							if (res.errCd == "0") {
								...
								output5 += '<select id="select_base_year_2" onchange="javascript:$thematicMapFrame15.ui.mapList[0].changeRegionBound()">';
					...
					$.ajax({
						...
						success: function(res) {
							if (res.errCd == "0") {
								...
								covidHtml += '<select id="covid_day" onchange="javascript:$thematicMapFrame15.ui.mapList[0].changeCovidDay()">';
				...
				this.covid = function(){
					var part = $("#covid_type a.on").text();
						$.ajax({
							...
							success: function(res) {
								if (res.errCd == "0") {
									$thematicMapFrame15.ui.removeCovidMarker();
									$thematicMapFrame15.ui.addCovidMarker(res.result.detailInfo);
								}
							} ,
							dataType: "json",
							error:function(e){}  
					...
					this.dateCalc = function(){
						...
						if( ( selYear == curYear ) && ( selMonth == curMonth ) && ( that.covidMonthLastDay != lastDate ) ){
							...
							startDt = curDt.getFullYear()+''+startMonth+''+startDay;
							startTime = Date.UTC( curDt.getFullYear(), startMonth-1, startDay );
						} else {
							that.covidPrevMonth = "";
							that.covidPrevDay = "";
						}
					...
					this.makeCovidChart = function(){
						if( ( !$thematicMapFrame15.ui.mapList[0].covidPlayYn && that.monthChange ) || that.isFirstDraw ){
...
//									adm_cd : that.selectedAdmCd,
									...
									covid_month_val : that.covidPlayMonth,
									gubun : 'chart'
								},
								success: function(res) {
									if (res.errCd == "0") {
										...
										function covidChartClick( e, idx, xAxis ){
											...
											$("#covid_month").val( category.split(".")[0] );
											that.covidDay();
											var $day = $("#covid_day option[value="+ category.split(".")[1] +"]").prop("selected","selected");
											that.changeRegionBound();
											
											$thematicMapFrame15.playCovidIdx = $day.index(); 
										}
										...
										$("#covidChart").highcharts({
											...
											tooltip : {
												enabled:true,
												formatter : function(){
													return this.x+'<br/><b>확진자 : '+this.y+'명</b>';
												}
											},
											plotOptions : {
												series : {
		...
		$class("sop.openApi.ReverseGeoCode.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				...
				if(that.thema_map_data_id == ""){
					console.log($thematicMapFrame15.params);
						...
						//코로나추가 20200722 주용민
							...
							if(window.parent.$thematicMapMain.param.stat_thema_map_id == "5FLM8BXNH320200521093949554ioKEsoOUGI" && $("#selectValue2").val()=="auto"){
								...
								if (!that.isReloadMode && that.lastGeojsonInfo != null && that.lastGeojsonInfo.adm_cd == curSidoCd) {
									...
									//코로나추가 20200729 jrj
									if( !( window.parent.$thematicMapMain.param.stat_thema_map_id == "sAXkcVzk5V202007141335257355ued9032uw" && $("#covid_hospital").hasClass("on") ) ){
...
//						that.AreaAndStatsApis(options.adm_cd, pageNum + 1);
					...
					if (poiList.length > 0) {
						for ( var i = 0; i < poiList.length; i++) {
							...
							$thematicMapFrame15.ui.poiInfoArray.push(tempList);
