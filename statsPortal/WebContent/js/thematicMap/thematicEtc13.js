...
//	$("#selectValue").val("leftValue");
	...
	if($("#heritage_type").is(":visible"))
	srvLogWrite("B0","08","01","01",window.parent.$thematicMapMain.themaInfo.title,"");
...
//  
    ...
    //2019-03-19 박길섭 끝
    $(".thematicTopText").hide();
    $(".dataSideContents").css("padding-top","20px");
...
function thematicCharts(selectedAdmCd){
	...
	var map = $thematicMapFrame13.ui.mapList[0];
...
//	        subtitle: {text: map.adm_nm}, //2017.03.21 그래프 타이틀 이슈 - 전국 그래프에서 타이틀 보임
	        colors: [
	                 '#3BBEE3', '#E91E63'
	        ],
	        xAxis: [{
...
function stepCloseAnimate(inx){
    ...
    $(".sideQuick.sq02").removeClass("on");//박길섭 추가
	...
	body.on("click", ".sideQuick.sq02,.stepClose", function(){//20년수정반영(.stepClose추가)
		...
		if($(this).hasClass("on")){
			...
			if(theme){
				...
				var on = $(this).hasClass("on");
				if(!on){
					$(".sideQuick.sq02").stop().animate({"left":"0"},100);
					$(".quickBox.step01").stop().animate({"left":"0"},100);
					//$(".shadow").show(); 
					$(this).find("span").hide();
...
//					$(this).removeClass("on").css("width","90px");
	...
	body.on("click",".sqList #map_type a", function(){
		...
		if($("#selectValue2").val() == "6"){
			var region_boundary = $(".sqList #region_boundary a").parents("li").eq(0).find(".on")[0].id;	// 지역경계값
			if(region_boundary == "autoRegion"){
				$("#selectValue2").val("auto");
			} else if(region_boundary == "sido"){
				$("#selectValue2").val("1");
			} else if(region_boundary == "sigungu"){
				$("#selectValue2").val("2");
			}
		...
		if($(this).attr('id') == "color"){
			...
			// mng_s 2019. 11. 21 j.h.Seok 이용자 중심 서비스 개편 - 상하위 10개 표출
			$("#selectValue2").val("auto");
			$("#region_boundary").find("a").removeClass("on");
			$("#region_boundary").find("a").eq(0).addClass("on");
			...
			$("#pieChartFlag").val("on");
		} else if($(this).attr('id') == "heatMapGrid"){
			// mng_s 2019. 11. 21 j.h.Seok 이용자 중심 서비스 개편 - 상하위 10개 표출
			$("#selectValue2").val("auto");
			$("#region_boundary").find("a").removeClass("on");
			$("#region_boundary").find("a").eq(0).addClass("on");
			...
			$("#dataMode").val("heatMapGrid");
			$("#heatMapFlag").val("on");
		} else if($(this).attr('id') == "networkData"){
			// mng_s 2019. 11. 21 j.h.Seok 이용자 중심 서비스 개편 - 상하위 10개 표출
			initTopBottomButtons();
			// mng_e 2019. 11. 21 j.h.Seok 이용자 중심 서비스 개편 - 상하위 10개 표출
...
function initTopBottomButtons() {
	...
	$thematicMapFrame13.ui.mapList[0].topBottomState = "OFF";
	$thematicMapFrame13.ui.mapList[0].topBottomType = "both";
	$thematicMapFrame13.ui.mapList[0].topBottomCount = "10";
}
//mng_e 2019. 11. 21 j.h.Seok 이용자 중심 서비스 개편 - 상하위 10개 표출
