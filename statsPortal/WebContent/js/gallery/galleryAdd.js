...
(function(W,D){
	...
	$galleryAdd = {
			...
			insertDataValidate : function(){
				...
				if( $("#gallery_title").val() =="" || $("#applicationContent").val() =="" || ( $galleryAdd.imageArray.length < 1 ) ){
					return false;
				}else{
					return true;
				}
			...
			insertDataValidate2 : function(){
				...
				if( $("#gallery_title").val() =="" || $("#applicationContent").val() =="" ){
					return false;
				}else{
					return true;
				}
			...
			gallerySaveForm : function(){
				...
				//mng_s 20211118 방화벽 정책에 의해 <img src 가 공격으로 인식되어서 헥사로 변경해서 넘김
			...
			selectBookMarkList : function(){
			 ...
			 onBlockUIPopup : function(){
				...
				goApplyData : function() {
					window.location.href = "/contents/shortcut/shortcut_05_03_step01.jsp";
				},
				
				//OpenAPI 신청
				goApplyOpenApi : function() {
					//mng_s 20220307 이진호
					//개발지원센터 개편에 따른 url 변경
					//window.open(developApiPath+"/html/openApi/app/myApp.html", '_blank');
					window.open(developApiPath+"/html/newOpenApi/app/myApp.html", '_blank');
					//mng_e 20220307 이진호
				},
