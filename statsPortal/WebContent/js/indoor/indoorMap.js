...
(function (W, D) {
	...
	sMap = {
			...
			map : function() {
				...
				this.addMarker = function (utmkPoint, options, decilist_serial, infoMsg) {		
					var marker = null;
					var visible = false;
					...
					if (this.gMap) {
						...
						if(decilist_serial != null && decilist_serial != '') {
							marker.on("click", function(e){
								$indoorMapView.event.clickMaker(decilist_serial);
							});
							marker.addTo(this.gMap).bindInfoWindow(infoMsg);
						}
				...
				this.addCustomMarker = function (x, y, html, decilist_serial, infoMsg) {
					var marker = null;
					if(this.gMap) {
						var icon = new sop.DivIcon({
							html:html,
							className: "div_icon",
							iconSize: [100, 50],
							iconAnchor: [11,11],
							infoWindowAnchor: [1,-5]
						});
						
						marker = sop.marker([x, y], {icon:icon});
...
//					var color 		= "#BBBBBB";
					...
					//신규 색상 추가
					else if(type == "new_company_theme_10") {
						fill = true;
						fillColor = "blue";
					}
					else if(type == "new_company_theme_20") {
						fill = true;
						fillColor = "white";//2019-04-11 박길섭
					}
					else if(type == "new_company_theme_30") {
						fill = true;
						fillColor = "gray";//2019-04-11 박길섭
					}
					else if(type == "new_company_theme_3136") {
						fill = true;
						fillColor = "white";//2019-04-11 박길섭
					}//2020-01-12 박길섭 시작
					else if(type == "new_company_theme_40") {
						fill = true;
						fillColor = "#D7D7D7";
					}//2020-01-12 박길섭 끝
					else if(type == "new_company_theme_99") {
						fill = true;
						fillColor = "white";
					}
