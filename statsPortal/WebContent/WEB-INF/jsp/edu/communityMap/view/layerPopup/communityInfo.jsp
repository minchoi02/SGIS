	...
	<h2>
		...
		<c:choose>
			<c:when test="${communityMapInfo.temp_save_yn=='Y' }">
				<img src="${pageContext.request.contextPath}/img/community/cm_detail_icon_temp.png" alt="임시함께하는지도">
			</c:when>
			<c:when test="${s>n }">
				<img src="${pageContext.request.contextPath}/img/community/cm_detail_icon_wait.png" alt="대기함께하는지도">
			</c:when>
			<c:otherwise>
				<c:if test="${communityMapInfo.is_hot=='Y' }">
					<img src="${pageContext.request.contextPath}/img/community/cm_detail_icon_hot.png" alt="Hot함께하는지도">
				</c:if>
				<c:if test="${communityMapInfo.is_new=='Y' }">
					<img src="${pageContext.request.contextPath}/img/community/cm_detail_icon_new.png" alt="New함께하는지도">
				</c:if>
			</c:otherwise>
		</c:choose>
	</h2>
	<span class="cm_property">
		<c:choose>
			...
			<c:when test="${communityMapInfo.cmmnty_partcptn_grant_yn=='M' }">
				<a href="javascript:void(0)" title="개설자가 함께하는지도 회원들의 아이디와 비밀번호를 미리 저장하고, 회원들은 지정받은 아이디와 비밀번호로 의견을 등록할 수 있습니다. 로그인이 필요 없습니다." style="cursor: default;">
...
<%-- 		<img src="${pageContext.request.contextPath}/img/community/cm_detail_icon_map.png" alt="정책연계지도"> --%>
	...
	<input type="hidden" id="is_new" value="${communityMapInfo.is_new}"/>
	<input type="hidden" id="is_hot" value="${communityMapInfo.is_hot}"/>
	...
	<c:choose>
		...
		<c:otherwise>
			<button class="btn_comment" id="poi-register-join-button" name="btnPrtcpnt" type="button">함께하는지도 참여하기</button>
		</c:otherwise>
	</c:choose>
	<script>
