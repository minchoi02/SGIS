...
<%
	...
	if(loginYn.equals("N")) {
		session.setAttribute("returnUrl", "/contents/shortcut/shortcut_05_03_past_year.jsp");out.print("<!DOCTYPE html PUBLIC '-//W3C//DTD XHTML 1.0 Transitional//EN' 'http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd'>");
		...
		out.print("</head>");
		out.print("<body>");
		out.print("<script type='text/javascript'> alert('로그인 후 이용할 수 있습니다.'); location.href='/view/member/login_new?returnPage=//sgis.kostat.go.kr/contents/shortcut/shortcut_05_03_past_year.jsp'; </script> ");
	}
	else {
	...
	<head>
		...
		<script>
			...
			function onChange_coa_year(what_year) {
				...
				/*
				if( what_year == "2018" ) {
					jQuery("#txt_data_info").html(" [2018년 6월 30일 경계기준]");
				} else {
			...
			jQuery(function(){
				...
				<%
				    if("2020".equals(census_output_area_year) || "".equals(census_output_area_year)) { //최초로딩시 및 2018 선택시 경계기준표시(나중에 년도 바뀌면 이부분 수정요망)
			    %>
							...
							<div class="table-type">
								<table class="table-style type02">
									...
									<tbody>
										<tr style="display: none;">
											...
											<td>
												<span class="radio-area">
														<input type="radio" class="radio" name="census_output_data_type" id="census_output_data_type1" value="1" checked="checked"/>
											...
											<td>
												<span class="select-wrap">
													<select name="census_output_area_year" id="census_output_area_year" style="width: 250px;" title="집계구 기준년도" onChange="onChange_coa_year(document.getElementById('census_output_area_year').value);">
														<option value="2020" <%if("2020".equals(census_output_area_year)) { %>selected="selected"<%} %>>2020</option>
														<option value="2019_4" <%if("2019_4".equals(census_output_area_year)) { %>selected="selected"<%} %>>2019</option>
														<option value="2018_4" <%if("2018_4".equals(census_output_area_year)) { %>selected="selected"<%} %>>2018</option>
														<option value="2017" <%if("2017".equals(census_output_area_year)) { %>selected="selected"<%} %>>2017</option>
														<option value="2016" <%if("2016".equals(census_output_area_year)) { %>selected="selected"<%} %>>2016</option>
										...
										<tr>
											<th scope="row"><label>시도/시군구</label></th>
											<td>
