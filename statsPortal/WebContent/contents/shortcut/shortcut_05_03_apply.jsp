...
if( session.getAttribute("sec_mem_id") != null &&  session.getAttribute("sec_mem_id").equals((String)session.getAttribute("member_id")) && (NowTime-preInsTime) < 300    ) {
	...
	try {
		...
		sgis_census_req_email  = request.getParameter("email_id") + "@" + request.getParameter("email_addr");;
		...
		//fileName = request.getFilesystemName(formName);
			...
			if (privacy02.equals("")) { // 개인정보 노출 점검
				if(aT.equals("INS")) {
					...
					for(int i = 0; i < sgis_census_id.length; i++){
						...
						// // 2017.11.06 [개발팀] 추가
						//lData.setString("sgis_census_detail_data_id", sgis_census_detail_data_id[i]); //2021년 SGIS4_자료제공 밑으로 이동
						...
						if(sgis_census_id[i].equals("2") && (sgis_census_data_id[i].equals("gd_001") || sgis_census_data_id[i].equals("gd_002") || sgis_census_data_id[i].equals("gd_003") 
								|| sgis_census_data_id[i].equals("gd_004") || sgis_census_data_id[i].equals("gd_005")  )){ //격자경계 데이터 추가할 경우 
							//년도,시도,시군구,세부자료명은 필요없지만 db에서 is not null 조건 때문에 임의로 값을 넣어놓음
							lData.setString("years", "-");
							lData.setString("sido", "-");
							lData.setString("sigungu", "-");
						}else if(sgis_census_id[i].equals("5") && (sgis_census_data_id[i].equals("1") || sgis_census_data_id[i].equals("2") || sgis_census_data_id[i].equals("3") 
								|| sgis_census_data_id[i].equals("4") )){
							lData.setString("years", years[i]);
							if(sido[i].equals("--") ){ // sido -- 일경우는 substring 오류 발생으로 조건문 추가
								lData.setString("sido", "-");
								lData.setString("sigungu", "-");
							}else{
							...
							if(sido[i].equals("--") ){ // sido -- 일경우는 substring 오류 발생으로 조건문 추가
								lData.setString("sido", sido[i].substring(1));
								}else{
									lData.setString("sido", sido[i].substring(4));	
								}	
								lData.setString("years", years[i]);
				...
				/***************************************/
					...
					if((status.equals("A") || status.equals("B")) && !aT.equals("")) {
						...
						for(int i = 0; i < sgis_census_id.length; i++){
							...
							if(sgis_census_id[i].equals("5") && sgis_census_data_id[i].equals("0")){//격자경계 데이터 추가할 경우 
								//년도,시도,시군구,세부자료명은 필요없지만 db에서 is not null 조건 때문에  "-" 값을 넣음
								lData.setString("years", "-");
								lData.setString("sido", "-");
								lData.setString("sigungu", "-");
							}else if(sgis_census_id[i].equals("5") && (sgis_census_data_id[i].equals("1") || sgis_census_data_id[i].equals("2") || sgis_census_data_id[i].equals("3") 
