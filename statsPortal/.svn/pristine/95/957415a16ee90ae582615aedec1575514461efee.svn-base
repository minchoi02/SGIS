...
/*	주석: 1. 불필요 팝업화면의 이벤트, 2.read이벤트는 workRoadEvent로 이동  - 2018.09.18	ywKim	변경
...
*/	
	...
	$tsMap.ui = {
			...
			toolTipDataLength : "2", // 툴팁 표현값 구분자
			 ...
			 * @description  : 오늘의 구인현황 지도 조회
			 * @date         : 2018. 10. 01 
			 * @author	     : 현재훈
			 * @history 	 :
			 */
			getTodayStatusMapData : function(btn_id, adm_cd, detail_type) {// used
				$workRoad.ui.log("$tsMap.ui.getTodayStatusMapData - begin");
		                                      		
				getTodayStatusMapData(btn_id, adm_cd, detail_type,"");
			},
			/**
			 * 
			 * @name         : getTodayStatusMapData
			 ...
			 * @author	     : 현재훈
			 * @history 	 :
			 * 		2019.01.15	ywKIm	변경: 파라미터 추가 (search_type, date_range)
			 * @param	search_type	: 카테고리 데이터 유형 (D,W,M)
			 *			date_range	: 카테고리의 일 구간 (W인 경우만 적용)
			 */
			getTodayStatusMapData : function(btn_id, adm_cd, detail_type, today, search_type, date_range, data_today) {// used
				$workRoad.ui.log("$tsMap.ui.getTodayStatusMapData.2 - begin");

				var lvAdmCd = adm_cd;
				var map = $tsMap.ui.mapList[$tsMap.ui.curMapId];
				map.SidoComboVal = adm_cd;	//콤보박스 선택 지역 값
				map.xComboVal = $tsMain.ui.tsMain_coor_x;	//콤보박스 선택 x값
				map.yComboVal = $tsMain.ui.tsMain_coor_y;	//콤보박스 선택 y값
				...
				todayText = today;
				dataParams.today = today;	// 2019.01.15	ywKIm	변경
				
				if(data_today != null) {
					dataParams.data_today = data_today;		// 2019.05.14[한광희] 일자리 정보 누락 처리(값이 Null일 경우 전일 데이터 사용):정보를 가지고 있는 날짜를 담은 변수 추가					
				} else {
					dataParams.data_today = today;		// 2019.05.14[한광희] 일자리 정보 누락 처리(값이 Null일 경우 전일 데이터 사용):정보를 가지고 있는 날짜를 담은 변수 추가
				}
...
//				}								
				...
				/** 2019.06.19[한광희] 오늘의 구인현황 기간조회 기능 활성화에 따른 기능 수정 START */
				if(search_type == "M" || search_type == "W") {
					detail_type = "NEW-COR";
				}
				/** 2019.06.19[한광희] 오늘의 구인현황 기간조회 기능 활성화에 따른 기능 수정 END */
				...
				tmpArParams.push({	key: "search_type", value: (search_type != undefined) ? search_type : "D" });	// 2019.01.15	ywKim	추가
				tmpArParams.push({	key: "data_today",	value: dataParams.data_today	});	// 2019.05.14[한광희] 일자리 정보 누락 처리(값이 Null일 경우 전일 데이터 사용):정보를 가지고 있는 날짜를 담은 변수 추가

				// 2019.04.18 접근log 생성
		        srvLogWrite('D0', '02', '01', '10', 'sido_cd:'+dataParams.sido_cd+', '
						            			  + 'sgg_cd:'+dataParams.sgg_cd+', '
						            			  + 'charts_type:'+$("input:radio[name='charts_type']:checked").val()+', '
			                                      + 'search_type:'+(search_type != undefined ? search_type : 'D')
			                                      , JSON.stringify(tmpArParams));
			                                      		
				// 단위 설정
				//2019-01-21 (전체 조회 시) 지도에 마우스 오버시 나오는 툴팁에 구인자수 추가. (구분값 2이면 데이터 추가)
				var filterParam = "right_data_val";
				var filterParam2 = "";
				var unit = "명";
				var unit2 = "";
				if(detail_type.indexOf("COR") > -1){
					...
					if($tsMap.ui.toolTipDataLength == "2") {
						filterParam2 = "right_data_val2";
						unit2 = "명";
						tmpArParams.push({	key: "show_data2",	value: filterParam2	});
						tmpArParams.push({	key: "unit2",	value: unit2	});
						$tsMap.ui.toolTipDataLength = "1";
					}
				}
				...
				console.log("[tsMap.js] getTodayStatusMapData 호출");
//				console.log("[tsMap.js] getTodayStatusMapData btn_id[" + btn_id);
//				console.log("[tsMap.js] getTodayStatusMapData tmpParam[" + tmpParam + "/" + tmpParam[0].params[1].key);
...
//				console.log("[tsMap.js] getTodayStatusMapData() $tsMap.ui.dropBtnInfo[map.id] [" + $tsMap.ui.dropBtnInfo[map.id]);
								
				// share정보 초기화
				$tsMap.noReverseGeoCode = false;
				$tsMap.ui.curMapId = map.id;
				$tsMapApi.request.combineFailCnt = 0;
				
				//공유 (나의 데이터에서는 shareInfo가 null)
				/*var shareInfo = new share.shareInfo(map, $tsMap.ui);
...
//				console.log("[tsMap.js] getTodayStatusMapData() btn_id[" + btn_id);
//				console.log("[tsMap.js] getTodayStatusMapData() atdrcYn[" + atdrcYn);
//				console.log("[tsMap.js] getTodayStatusMapData() map.selectedBoundMode[" + map.selectedBoundMode);
									
				map.bnd_year = map.bnd_year;
				$tsMap.ui.searchBtnType = "normal";
				api_id = id.split("-")[0];
				index = id.split("-")[1];

				// 버튼 시각 효과
				//$tsMapFrame.ui.updateSearchBtnEffect(id, map.id);
				$tsMap.ui.dropBtnInfo[map.id] = tmpParam;
				$tsMap.ui.setDataType(map.id, "census");				
				...
				//20181121 손원웅 map 전국 레벨 center값을 위한 초기화(commonMap.js 경계레벨 설정부분에서 mapMove 하면 무한루프...)
				if ($("#current-sido-select").val() == "99"){ // 20201202 이금은, '전체'일 경우만 center이동
                    map.mapMove([989674,1818313],2);
				}				
				...
				if (tmpParam != null) {				
					var params = $tsMap.ui.reqSetParams(tmpParam, adm_cd, adm_nm, api_id, map);
					
					//시계열 초기값 세팅
					//$tsDataBoard.ui.timeSeriesInit(params);
					$tsMap.ui.curDropParams[map.id] = params;
					$tsMap.ui.requestOpenApi(params);
					
					if ( map.isInnerMapShow2 ) {
						$tsMap.ui.requestGridLegend(params); //mng_s 그리드 범례요청
					}
				}
				map.setBoundSelectedMoode(null);
				map.mapBtnInfo.setFixedBoundBtn(false);															
				
				//2019-01-10 지도 조회시 텍스트 표시
				if($tsMap.ui.curMapId != undefined && $tsMap.ui.curMapId != null) {
					// 변수 선언
					var lvHtml = "오늘의 구인 현황  :  ";
					var lvDetailTypeName = "";
					var lvSidoName = "";
					var lvTodayName = "";
					
					// 데이터 처리
					if(detail_type != undefined && detail_type != null) {
						if(detail_type == "ALL-PSN") lvDetailTypeName = "전체구인자수";
						else if(detail_type == "ALL-COR") lvDetailTypeName = "전체구인업체수";
						else if(detail_type == "NEW-PSN") lvDetailTypeName = "신규구인자수";
						else if(detail_type == "NEW-COR") lvDetailTypeName = "신규구인업체수";
					}
					if(lvAdmCd != undefined && lvAdmCd != null) {
						$("#current-sido-select option").each(function() {
							if($(this).val() == lvAdmCd) {
								lvSidoName = $(this).text();
								return false;
							}
						});
					}
						...
						if (search_type != undefined && search_type != null) {
							if (search_type == "W") {
								if (date_range != undefined && date_range != null) {
									lvTodayName = $workRoad.util.weekWithSign(today, ".") + " (" + date_range + ")";
								} else {
									lvTodayName = $workRoad.util.weekWithSign(today, ".");
								}
							} else if (search_type == "M") {
								lvTodayName = $workRoad.util.monthWithSign(today, ".");
							} else {
								lvTodayName = $workRoad.util.dateWithSign(today, ".");
							}
						} else {
							lvTodayName = $workRoad.util.dateWithSign(today, ".");
						}
//						if(today.length == 8) lvTodayName = today.substr(0,4)+"."+today.substr(4,2)+"."+today.substr(6,2);
//						else if(today.length == 6) lvTodayName = today.substr(0,4)+"."+today.substr(4,2);
//						else lvTodayName = today;
					}
					
					// 갈무리
					if(lvDetailTypeName != "" && (lvSidoName != "" || lvTodayName != "")) lvDetailTypeName += " / ";
					if(lvSidoName != "" && lvTodayName != "") lvSidoName += " / ";
					
					// HTML 적용
					lvHtml += lvDetailTypeName + lvSidoName + lvTodayName;
					$("#view"+(Number($tsMap.ui.curMapId)+1)+" #wrmTitle").html(lvHtml);
				}
			},
			 ...
			 * @history 	 :
			 */
			getFirstMapDataLoad : function(btn_id, today) {// used
				$workRoad.ui.log("$tsMap.ui.getFirstMapDataLoad - begin");
				...
				tmpArNoneParams.push("");
				var tmpArParams = new Array();
				tmpArParams.push({	key: "year",	value: today.substring(0, 4)});// 하드코딩 제거 - 2018.11.08	ywKim	변경
				tmpArParams.push({	key: "today",	value: today});// 하드코딩 제거 - 2018.11.08	ywKim	변경
//				tmpArParams.push({	key: "year",	value: '2016'});// 하드코딩 제거 - 2018.11.08	ywKim	변경
//				tmpArParams.push({	key: "today",	value: '20180101'});// 하드코딩 제거 - 2018.11.08	ywKim	변경
				tmpArParams.push({	key: "sido_cd",	value: "99"	});
				tmpArParams.push({	key: "sgg_cd",	value: "999"	});
				tmpArParams.push({	key: "detail_type",	value: "ALL-COR"	});
				
				// 2019.04.18 접근log 생성
		        srvLogWrite('D0', '02', '01', '10', 'sido_cd:99, '
						            			  + 'sgg_cd:999, '
						            			  + 'charts_type:'+$("input:radio[name='charts_type']:checked").val()+', '
		        		                          + 'search_type:D'
			                                      , JSON.stringify(tmpArParams));
				...
				console.log("[tsMap.js] getFirstMapDataLoad 호출");
				console.log("[tsMap.js] getFirstMapDataLoad btn_id[" + btn_id);
				console.log("[tsMap.js] getFirstMapDataLoad tmpParam[" + tmpParam + "/" + tmpParam[0].params[1].key);
				...
				console.log("[tsMap.js] getFirstMapDataLoad() $tsMap.ui.dropBtnInfo[map.id] [" + $tsMap.ui.dropBtnInfo[map.id]);
				...
				console.log("[tsMap.js] getFirstMapDataLoad() btn_id[" + btn_id);
				console.log("[tsMap.js] getFirstMapDataLoad() atdrcYn[" + atdrcYn);
				console.log("[tsMap.js] getFirstMapDataLoad() map.selectedBoundMode[" + map.selectedBoundMode);
							...
							if (admList.length > 0) {
								...
								for (var i=0; i<admList.length; i++) {
									if (tmpParam != null) {
										...
										var params = $tsMap.ui.reqSetParams(tmpParam, admList[i], adm_nm, api_id, map, "gibgae");
										$tsMap.ui.curDropParams[map.id] = params;
										$tsMap.ui.requestOpenApi(params);
									}
								}
			...
			setDataType : function(mapId, type) {// used
				$workRoad.ui.log("$tsMap.ui.setDataType - begin");
				console.log("[tsMap.js] setDataType() type [" + type);
				...
				if (this.dataTypeList != null && 
					...
					if (type == "userData") {
						$tsMapFrame.ui.curSelectedStatsType = "userData";
					}
			...
			createMap : function(id, seq) {// used
				$workRoad.ui.log("$tsMap.ui.createMap - begin");
				
				var map = new sMap.map();
				map.createMap($tsMap, id, {
				...
				legend.initialize($tsMap.ui);
				map.legend = legend;
				legend.createLegend();
				legend.legendType = "auto";
				//작업부분 끝
				
				var btnInfo = new tsMapBtnInfo.btnInfo(map, $tsMap.ui);
//				var btnInfo = new interactiveMapBtnInfo.btnInfo(map, $tsMap.ui);
				map.mapBtnInfo = btnInfo;
				btnInfo.createUI({
					intrPoiControl : false,			// 본PRJ에서 필요없음 - 2019.01.07	ywKim	변경
					intrSettingControl : false,		// 본PRJ에서 필요없음 - 2019.01.07	ywKim	변경 - ☆ false로 설정할 경우 다른 소스에서 문제가 생긴다. (tsMap.js, tsMapApi.js, tsMapBtn.js > map.drawControl.removeOverlay();)
...
//				$galleryAdd.delegate = this;
				...
				if(gv_screen == "full") $tsMap.ui.doMaxSize(1);
			...
			doMaxSize : function(type) {// used
				$workRoad.ui.log("$tsMap.ui.doMaxSize - begin");
				...
				console.log("\n\n[1]:::::* $('.tb_sizing').attr('title'): "+$(".tb_sizing").attr("title"));
			},
			...
			doClearMap : function(type, isStatsInfoClear) {// used
				$workRoad.ui.log("$tsMap.ui.doClearMap - begin");
				...
				if (this.mapList.length > 0) {
					...
					if($tsMapFrame.event.marker != null){
						$tsMapFrame.event.marker.remove();
					}
				...
				//$mydataDataBoard.ui.delegateSetting($tsMap.ui);			//나의데이터 세팅
				//$publicDataBoard.ui.delegateSetting($tsMap.ui);			//공공데이터 세팅
				
				//데이터보드 초기화
				$tsDataBoard.ui.reset(this.curMapId);
				//$publicDataBoard.ui.remove(this.curMapId);
				//$mydataDataBoard.ui.remove(this.curMapId);
				
				//초기화한 지도정보와 매핑되는 버튼 Effect 수정
				$tsMapFrame.ui.updateSearchBtnEffect("", this.curMapId);
			},
			...
			doShare : function(type) {
				$workRoad.ui.addFnc01List("$tsMap.ui.doShare");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				this.curMapId = parseInt(type)-1;
			...
			doBookMark : function(type, srvType) {
				$workRoad.ui.addFnc01List("$tsMap.ui.doBookMark");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				
				console.log("[tsMap.js] doBookMark() 호출");
				
				this.curMapId = parseInt(type)-1;
				var shareInfo = this.mapList[this.curMapId].shareInfo;
				var map = this.mapList[this.curMapId];
				
				console.log("[tsMap.js] doBookMark() srvType [" + srvType);
...
//				
//				console.log("[tsMap.js] doInnerMap 호출 type [" + type + "] isShow [" + isShow +"]");
//				
//				this.curMapId = parseInt(type)-1;
//				var map = this.mapList[this.curMapId];
...
//				
//				console.log("[tsMap.js] doInnerMap2 호출 type [" + type + "] isShow [" + isShow +"]");
//				
//				this.curMapId = parseInt(type)-1;
//				var map = this.mapList[this.curMapId];
...
//				
//				console.log("[tsMap.js] doInnerMap3 호출 type [" + type + "] isShow [" + isShow +"]");
//				
//				this.curMapId = parseInt(type)-1;
//				var map = this.mapList[this.curMapId];
...
//			},
			...
			doCombineMap : function(type) {
				$workRoad.ui.addFnc01List("$tsMap.ui.doCombineMap");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var isCombine = true;
				var cnt = 0;
				for (var i=0; i<this.mapList.length; i++) {
					if (this.mapList[i] != null) {
			 ...
			 */
			mapLoad : function() {
				$workRoad.ui.addFnc01List("$tsMap.ui.mapLoad");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var data = [];
				for (var i=0; i<this.mapList.length; i++) {
					if (this.mapList[i]) {
						var map = this.mapList[i];
				...
				var popup = $tsMap.ui.combinePopup.$combineMap.ui;
				popup.setData(data);
			},
			 ...
			 */
			reportLoad : function() {
				$workRoad.ui.addFnc01List("$tsMap.ui.reportLoad");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var map = this.mapList[this.curMapId];
				...
				function getChartSvgData(id){
					$workRoad.ui.addFnc01List("$tsMap.ui.reportLoad.[getChartSvgData]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
	                    ...
	                    proxy: contextPath+"/ServiceAPI/community/html2canvasproxy.jsonp",
		 				onrendered: function(canvas) {
	                    	...
	                    	var options = {
    	 					...
    	 					var popup = $tsMap.ui.reportPopup.$reportForm.ui;
    	 					popup.delegate = $tsMap.ui;
    	 					popup.map = map;
			...
			reportDataSet : function(type) {
				$workRoad.ui.addFnc01List("$tsMap.ui.reportDataSet");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				this.curMapId = parseInt(type)-1;
				var map = this.mapList[this.curMapId];
				if (map.dataGeojson == null && 
					map.multiLayerControl.dataGeojson == null) {
			...
			doAddMap : function(type) {// used
				$workRoad.ui.log("$tsMap.ui.doAddMap - begin");
...
//				// 2019.03.13 접근log 생성
//				var view_cnt = 1;
//				if(isMap1ContentShow) ++view_cnt;
//				if(isMap2ContentShow) ++view_cnt;
//				if(isMap3ContentShow) ++view_cnt;
//				srvLogWrite('D0', '02', '03', '01', '', 'VIEW'+(view_cnt)+' OPEN');
				...
				if(sceneInx==1){ 
					$(".sceneBox").stop().animate({"width":"50%"},200, function() {
						for (var i=0; i<$tsMap.ui.mapList.length; i++) {
							if ($tsMap.ui.mapList[i] != null) {
								$tsMap.ui.mapList[i].update();
							}
						}
					});
					...
					$( ".sceneBox" ).on("resize", function() {
						$workRoad.ui.addFnc01List("$tsMap.ui.doAddMap.event.[1]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
						for (var i=0; i<$tsMap.ui.mapList.length; i++) {
							if ($tsMap.ui.mapList[i] != null) {
								$tsMap.ui.mapList[i].update();
							}
				...
				mapNavi.create("mapNavi_" + (createMapId+1), createMapId+1, $tsMap.ui);
...
//				});				
				...
				// 2018.11.27	ywKim	[v181116]
				// 지도 컨트롤 버튼 (우측 상단)
				$(".tsMap #view1 .sop-top.sop-right").find(".info").eq(0).css("display", "none");
				$(".tsMap #view1 .sop-top.sop-right").find(".info").eq(1).css("display", "none");
				$(".tsMap #view2 .sop-top.sop-right").find(".info").eq(0).css("display", "none");
				$(".tsMap #view2 .sop-top.sop-right").find(".info").eq(1).css("display", "none");
				$(".tsMap #view3 .sop-top.sop-right").find(".info").eq(0).css("display", "none");
				$(".tsMap #view3 .sop-top.sop-right").find(".info").eq(1).css("display", "none");
			},
			...
			doRemoveMap : function(type) {// used
				$workRoad.ui.log("$tsMap.ui.doRemoveMap - begin");
				
//				// 2019.03.13 접근log 생성
//				srvLogWrite('D0', '02', '03', '02', '', 'VIEW'+(view_cnt)+' CLOSE');
				...
				$tsDataBoard.ui.reDraw(this.curMapId);
			},
			...
			doAnalysisShareInfo : function (type, data) {
				$workRoad.ui.addFnc01List("$tsMap.ui.doAnalysisShareInfo");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				if (type.length == 0 || data.length == 0) {
					var mapNavi1 = new mapNavigation.UI();
					mapNavi1.firstBoolean = true;
					mapNavi1.create("mapNavi_1", 1, $tsMap.ui);	
					...
					if (type == "mainIndexView" 	  ||
						...
						if  (type.indexOf("View") != -1) {
							setTimeout(function() {
								$tsMapFrame.ui.setDetailStatsPanel(type.split("View")[0]);
							}, 200);
						}
					}
					return false;
				...
				switch(typeList[type]) {
					...
					case 14:
						$tsMapFrame.ui.curSelectedStatsType = "publicData";
...
//							$(".tb_radio").hide();	// 2018.11.12	ywKim	변경: 사업체 전개도 사용 안함
							$("#rotationTip").hide();
							$("#tuto_start_btn").hide();
							$(".helperText").hide();
				...
				if(!isKosis) {
					...
					$tsMapFrame.ui.setRevertParams(tmpShareInfo, "share");
					...
					for (var i=0; i<admCdList.length; i++) {
						$tsMapApi.request.openApiShareForStats(tmpParams, admCdList[i]);
					}
				}
				
				
				map.curDropCd = shareInfo.param_info.paramInfo.adm_cd;
				$tsMap.ui.dropBtnInfo[map.id] = $tsMapFrame.ui.arParamList[0];
			...
			doDone : function(type) {
				$workRoad.ui.addFnc01List("$tsMap.ui.doDone");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var map = this.mapList[this.curMapId];
				if (type == "settings") {
			...
			doCancel : function(type) {
				$workRoad.ui.addFnc01List("$tsMap.ui.doCancel");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				if (type == "settings") {
			...
			updateSearchTitle : function(title, unit, mapId) {// used
				$workRoad.ui.log("$tsMap.ui.updateSearchTitle - begin");
			...
			shareToKakaoStory : function() {
				$workRoad.ui.addFnc01List("$tsMap.ui.shareToKakaoStory");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var shareInfo = this.mapList[this.curMapId].shareInfo;
...
//			},
			...
			createInfoTooltip : function(event, data, type, map) {// used
				$workRoad.ui.log("$tsMap.ui.createInfoTooltip - begin");
				
				//2019-01-21 (전체 조회 시) 지도에 마우스 오버시 나오는 툴팁에 구인자수 추가. (너비 자동 조절을 위해 width auto 추가)
				var html = "<table style='margin:10px; width:auto;'>";
				var searchYear = "";
				var that = this;
				var showName = {
									"tot_ppltn" : "총인구",
...
//								20190320 손원웅 수정
								...
								var dateVal = "";
								var search_type = "";	// 2019.06.17[한광희] 기간조회 기능 활성화에 따른 변수 추가
								
								for(var i = 0; i < this.curDropParams[map.id].param.length; i ++) {
									/** 2019.06.17[한광희] 기간조회 기능 활성화에 따른 수정 START */
									if("search_type" == this.curDropParams[map.id].param[i].key) {
										search_type = this.curDropParams[map.id].param[i].value;
									}
									/** 2019.06.17[한광희] 기간조회 기능 활성화에 따른 수정 END */
									if("today" == this.curDropParams[map.id].param[i].key){
										dateVal = this.curDropParams[map.id].param[i].value;
									}
								}
								...
								for (var i = 0; i < data.info.length; i++) {
									...
									if ($wrmTodayStatus.ui.selected_search_type == "W") {
//										searchYear = $workRoad.util.weekWithSign($wrmTodayStatus.ui.selected_today, ".");
										searchYear = $workRoad.util.weekWithSign(dateVal, ".");		// 2019.06.17[한광희] 오늘의 구인현황 기간조회 기능 활성화에 따른 수정 dateWithSign→weekWithSign
									} else if ($wrmTodayStatus.ui.selected_search_type == "M") {
//										searchYear = $workRoad.util.monthWithSign($wrmTodayStatus.ui.selected_today, ".");
										searchYear = $workRoad.util.monthWithSign(dateVal, ".");	// 2019.06.17[한광희] 오늘의 구인현황 기간조회 기능 활성화에 따른 수정 dateWithSign→monthWithSign
									} else {
										//2019-01-03 지도 화면에 마우스 오버시 "2018"로 뜨고 있음 (YYYY.MM.DD)로 변경 필요.
//										searchYear = $workRoad.util.dateWithSign($wrmTodayStatus.ui.selected_today, ".");
										searchYear = $workRoad.util.dateWithSign(dateVal, ".");
									}
									...
									if (tmpData.showData != undefined && tmpData.showData.length > 0) {
										...
										//2019-01-21 (전체 조회 시) 지도에 마우스 오버시 나오는 툴팁에 구인자수 추가. (showData2 존재시 툴팁에 추가 적용)
										var value = 0;
										var value2 = 0;
										var showData = tmpData.showData;
										var showData2 = tmpData.showData2;
										var unit = tmpData.unit;
										var unit2 = tmpData.unit2;
										if (parseFloat(tmpData[showData]) < 5 && 
												showData != "avg_age" &&
												showData != "ppltn_dnsty" &&
												showData != "aged_child_idx" && 
												showData != "oldage_suprt_per" &&
												showData != "juv_suprt_per" && 
												showData != "tot_suprt_per" &&
											// 2016. 03. 24 j.h.Seok modify
												showData != "tot_worker" &&
												showData != "avg_fmember_cnt" &&
											...
											if (sop.isInnerMapShow2) { //그리드 이면
												...
												value = appendCommaToNumber(tmpData[showData]);
												if(showData2 != "") value2 = appendCommaToNumber(tmpData[showData2]);
											} else {
										...
										if (value != "N/A") {
											...
											if (sop.isInnerMapShow2) { //그리드 이면
												...
												if(value == "5미만") { //그리드이면 5미만으로 표시
													...
													if(Number(tmpData[showData]) < Number(map.grid_legend_new[0][1])) {
														value = appendCommaToNumber(map.grid_legend_new[0][1]) + " 이하";
													} else if(Number(map.grid_legend_new[0][1]) <  Number(tmpData[showData]) && Number(tmpData[showData]) <= Number(map.grid_legend_new[0][2]) ) {
														value = appendCommaToNumber(map.grid_legend_new[0][1]) + "~" + appendCommaToNumber(map.grid_legend_new[0][2]) + " 이하";
													} else if(Number(map.grid_legend_new[0][2]) <  Number(tmpData[showData]) && Number(tmpData[showData]) <= Number(map.grid_legend_new[0][3]) ) {
														value = appendCommaToNumber(map.grid_legend_new[0][2]) + "~" + appendCommaToNumber(map.grid_legend_new[0][3]) + " 이하";
													} else if(Number(map.grid_legend_new[0][3]) <  Number(tmpData[showData]) && Number(tmpData[showData]) <= Number(map.grid_legend_new[0][4]) ) {
														value = appendCommaToNumber(map.grid_legend_new[0][3]) + "~" + appendCommaToNumber(map.grid_legend_new[0][4]) + " 이하";
													} else if(Number(map.grid_legend_new[0][4]) <  Number(tmpData[showData]) && Number(tmpData[showData]) <= Number(map.grid_legend_new[0][5]) ) {
														value = appendCommaToNumber(map.grid_legend_new[0][4]) + "~" + appendCommaToNumber(map.grid_legend_new[0][5]) + " 이하";
													} else if(Number(map.grid_legend_new[0][5]) <  Number(tmpData[showData]) && Number(tmpData[showData]) <= Number(map.grid_legend_new[0][6]) ) {
														value = appendCommaToNumber(map.grid_legend_new[0][5]) + "~" + appendCommaToNumber(map.grid_legend_new[0][6]) + " 이하";
													} else if(Number(map.grid_legend_new[0][6]) <  Number(tmpData[showData]) && Number(tmpData[showData]) <= Number(map.grid_legend_new[0][7]) ) {
														value = appendCommaToNumber(map.grid_legend_new[0][6]) + "~" + appendCommaToNumber(map.grid_legend_new[0][7]) + " 이하";
													} else if(Number(map.grid_legend_new[0][7]) <  Number(tmpData[showData]) && Number(tmpData[showData]) <= Number(map.grid_legend_new[0][8]) ) {
														value = appendCommaToNumber(map.grid_legend_new[0][7]) + "~" + appendCommaToNumber(map.grid_legend_new[0][8]) + " 이하";
													} else if(Number(map.grid_legend_new[0][8]) <  Number(tmpData[showData]) && Number(tmpData[showData]) <= Number(map.grid_legend_new[0][9]) ) {
														value = appendCommaToNumber(map.grid_legend_new[0][8]) + "~" + appendCommaToNumber(map.grid_legend_new[0][9]) + " 이하";
													} else if(Number(map.grid_legend_new[0][9]) <  Number(tmpData[showData])  ) {
												...
												if(showData2 != "") {
													//mng_s
													if(value2 == "5미만") { //그리드이면 5미만으로 표시
														value2 = "5미만";
													} else {
														//map.grid_legend_new[0][0]는 더미값 0.01 이므로 사용않함.
														if(Number(tmpData[showData2]) < Number(map.grid_legend_new[0][1])) {
															value2 = appendCommaToNumber(map.grid_legend_new[0][1]) + " 이하";
														} else if(Number(map.grid_legend_new[0][1]) <  Number(tmpData[showData2]) && Number(tmpData[showData2]) <= Number(map.grid_legend_new[0][2]) ) {
															value2 = appendCommaToNumber(map.grid_legend_new[0][1]) + "~" + appendCommaToNumber(map.grid_legend_new[0][2]) + " 이하";
														} else if(Number(map.grid_legend_new[0][2]) <  Number(tmpData[showData2]) && Number(tmpData[showData2]) <= Number(map.grid_legend_new[0][3]) ) {
															value2 = appendCommaToNumber(map.grid_legend_new[0][2]) + "~" + appendCommaToNumber(map.grid_legend_new[0][3]) + " 이하";
														} else if(Number(map.grid_legend_new[0][3]) <  Number(tmpData[showData2]) && Number(tmpData[showData2]) <= Number(map.grid_legend_new[0][4]) ) {
															value2 = appendCommaToNumber(map.grid_legend_new[0][3]) + "~" + appendCommaToNumber(map.grid_legend_new[0][4]) + " 이하";
														} else if(Number(map.grid_legend_new[0][4]) <  Number(tmpData[showData2]) && Number(tmpData[showData2]) <= Number(map.grid_legend_new[0][5]) ) {
															value2 = appendCommaToNumber(map.grid_legend_new[0][4]) + "~" + appendCommaToNumber(map.grid_legend_new[0][5]) + " 이하";
														} else if(Number(map.grid_legend_new[0][5]) <  Number(tmpData[showData2]) && Number(tmpData[showData2]) <= Number(map.grid_legend_new[0][6]) ) {
															value2 = appendCommaToNumber(map.grid_legend_new[0][5]) + "~" + appendCommaToNumber(map.grid_legend_new[0][6]) + " 이하";
														} else if(Number(map.grid_legend_new[0][6]) <  Number(tmpData[showData2]) && Number(tmpData[showData2]) <= Number(map.grid_legend_new[0][7]) ) {
															value2 = appendCommaToNumber(map.grid_legend_new[0][6]) + "~" + appendCommaToNumber(map.grid_legend_new[0][7]) + " 이하";
														} else if(Number(map.grid_legend_new[0][7]) <  Number(tmpData[showData2]) && Number(tmpData[showData2]) <= Number(map.grid_legend_new[0][8]) ) {
															value2 = appendCommaToNumber(map.grid_legend_new[0][7]) + "~" + appendCommaToNumber(map.grid_legend_new[0][8]) + " 이하";
														} else if(Number(map.grid_legend_new[0][8]) <  Number(tmpData[showData2]) && Number(tmpData[showData2]) <= Number(map.grid_legend_new[0][9]) ) {
															value2 = appendCommaToNumber(map.grid_legend_new[0][8]) + "~" + appendCommaToNumber(map.grid_legend_new[0][9]) + " 이하";
														} else if(Number(map.grid_legend_new[0][9]) <  Number(tmpData[showData2])  ) {
															value2 = appendCommaToNumber(map.grid_legend_new[0][9]) + " 초과";
														}
													}
												}
											}
											...
											html += "<td class='statsData'>"+title+value+" ("+unit+")";
											if(showData2 != "") html += " / "+value2+" ("+unit2+")";
											html += "</td>";
										}else {
											html += "<td class='statsData'>"+title+value;
											if(showData2 != "") html += " / "+value2;
											html += "</td>";
										}
						...
						if (sop.isInnerMapShow2 || sop.isInnerMapShow3) { //그리드 이면
							...
							if (filterName.length > 0) {
								...
								title = todayText.substring(0,4)+"."+todayText.substring(4,6)+"."+todayText.substring(6,8) + " : ";
							}
							
							if($tsMap.ui.mapList[0].dataType == "kosis" && $tsMap.ui.mapList[0].dataForCombine[0].UNIT != undefined){
								html += "<td class='statsData'>"+title+" 0("+$tsMap.ui.mapList[0].dataForCombine[0].UNIT+")</td>";
							} else {
								html += "<td class='statsData'>"+title+" N/A (구인정보 없음)</td>";
							}
						}
				...
				//2019-01-21 (전체 조회 시) 지도에 마우스 오버시 나오는 툴팁에 구인자수 추가. (너비 자동 조절로 인한 주석)
			...
			requestOpenApi : function(options) {// used
				$workRoad.ui.log("$tsMap.ui.requestOpenApi - begin");
				...
				console.log("[tsMap.js] requestOpenApi() api_id [" + api_id);
				
				/*if 	    (api_id == "API_0301") $tsMapApi.request.openApiTotalPopulation(options);
				else if (api_id == "API_0302") $tsMapApi.request.openApiSearchPopulation(options);
				else if (api_id == "API_0303") $tsMapApi.request.openApiInderstryCode(options);
				else if (api_id == "API_0304") $tsMapApi.request.openApiCompany(options);
				else if (api_id == "API_0305") $tsMapApi.request.openApiHouseHold(options);
				else if (api_id == "API_0306") $tsMapApi.request.openApiHouse(options);
				else if (api_id == "API_0307") $tsMapApi.request.openApiFarmHouseHold(options);
				else if (api_id == "API_0308") $tsMapApi.request.openApiForestryHouseHold(options);
				else if (api_id == "API_0309") $tsMapApi.request.openApiFisheryHouseHold(options);
				else if (api_id == "API_0310") $tsMapApi.request.openApiHouseHoldMember(options);
				else if (api_id == "API_4011") $tsMapApi.request.openApiItemCombine(options);*/				
				if (api_id == "API_WORK") $tsMapApi.request.openApiWorkData(options);	// 구인현황조회 추가
				pageCallReg();
			...
			requestGridLegend : function(options) {
				$workRoad.ui.addFnc01List("$tsMap.ui.requestGridLegend");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				
				console.log("[tsMap.js] requestGridLegend() 호출");
				
				var api_id = options.api_id;
				
				console.log("[tsMap.js] requestGridLegend() api_id [" + api_id);
				
				//주의할점 경고!!! 실제 페이지를 불러올때 아래 함수가 없으면 맵이 정상작동을 않함. 
				//개발중이어서 다 만들지 않은 상태에서 테스트 하다보니 지도를 드래그 했을때 격자를 못그림
				//각 서비스별로 나눌 필요가 없어서 하나로 사용한다.
				$tsMapApi.request.gridLegendTotalPopulation(options);
			...
			reqSetParams : function (tmpParam, adm_cd, adm_nm, api_id, map, type) {// used
				$workRoad.ui.log("$tsMap.ui.reqSetParams - begin");
				
				adm_cd = "00"; // 구인현황 조회 추가
				console.log("[tsMap.js] reqSetParams() 호출 adm_cd [" + adm_cd);
				...
				var params = {
						...
						origin : (tmpParam[0].origin != undefined) ? tmpParam[0].origin : "",	// 출처 - 2018.11.08	ywKim	추가
						adm_cd : adm_cd,
						adm_nm : adm_nm,						
						api_id : api_id,
						map : map,
	...
	$tsMap.callbackFunc = {
			...
			didMapMoveStart : function(event, map) {// used
				$workRoad.ui.log("$tsMap.callbackFunc.didMapMoveStart - begin");
			},

			// 맵이동 종료시, 콜백 호출
			didMapMoveEnd : function(event, map) {// used
				$workRoad.ui.log("$tsMap.callbackFunc.didMapMoveEnd - begin");
				console.log("[tsMap.js] didMapMoveEnd() $tsMap.ui.dropBtnInfo[map.id] [" + $tsMap.ui.dropBtnInfo[map.id]);
				console.log("[didMapMoveEnd]$tsMap.ui.mapList[0].data : " + $tsMap.ui.mapList[0].data.length);
				/*
				...
				if (poiControl.isOpenPOI && 
					...
					console.log("[tsMap.js] didMapMoveEnd 테마poi조회");
				...
				if (poiControl.isOpenPOI && 
						...
						console.log("[tsMap.js] didMapMoveEnd 사업체poi조회");
			...
			didMapZoomStart : function(event, map) {
				$workRoad.ui.addFnc01List("$tsMap.callbackFunc.didMapZoomStart");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			},
			
			/*	2018-11-08 줌 이벤트 삭제하기로 함_시작
			
			// 맵 줌 종료 시, 콜백 호출
			didMapZoomEnd : function(event, map) {
				
				console.log("[tsMap.js] didMapZoomEnd 호출");
				console.log("[tsMap.js] didMapZoomEnd() map.zoom [" + map.zoom);
...
//				}
			...
			2018-11-08 줌 이벤트 삭제하기로 함_끝
			*/
			
			
			// 드랍종료 시, 콜백 호출
			didMapDropEnd : function(event, source, layer, data, map) {
				$workRoad.ui.addFnc01List("$tsMap.callbackFunc.didMapDropEnd");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				if( map.isInnerMapShow2!=undefined && map.isInnerMapShow2 ) { //mng_s 그리드일 경우
					//그리드는 드랍 이벤트 없음
				} else {
					console.log("[tsMap.js] didMapDropEnd 호출");
					console.log("[tsMap.js] source.prop(\"id\") [" + source.prop("id") );
					...
					//공유 (나의 데이터에서는 shareInfo가 null)
					var shareInfo = new share.shareInfo(map, $tsMap.ui); //share.shareInfo()는 bookmarkAndShareInfo.js에 있는듯 하다.
					map.shareInfo = shareInfo;
					map.shareInfo.shareUrlInfo = [];
					map.dropInfo = null;
					
					if ($tsMap.ui.curDropParams == undefined) {
						$tsMap.ui.curDropParams = [];
					}
					
					var id = $("#" + source.prop("id")).find("a").attr("id");
					var atdrcYn = $("#" + source.prop("id")).find(".atdrc_yn").html();
					var tmpParamList = deepCopy($tsMapFrame.ui.arParamList);
					
					console.log("[tsMap.js] didMapDropEnd() id [" + id );
					console.log("[tsMap.js] didMapDropEnd() atdrcYn [" + atdrcYn );
					...
					if (id.split("-")[0] == "kosis") {
						...
						$tsMap.ui.setDataType(map.id, "kosis");
						$tsMap.ui.dropBtnInfo[map.id] = tmpParamList[i];
						$tsMap.ui.dropBtnInfo[map.id]["isKosis"] = true;
					...
					if (map.selectedBoundMode == "multi") {
						...
						if (tmpParam != null) {
							var params = $tsMap.ui.reqSetParams(tmpParam, data.adm_cd, data.adm_nm, api_id, map);
			...
			didMapDoubleClick : function(btn_id, tmpParam) {
				$workRoad.ui.addFnc01List("$tsMap.callbackFunc.didMapDoubleClick");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var btn_id = "API_0301-0";
				...
				console.log("[tsMap.js] didMapDoubleClick 호출");
				console.log("[tsMap.js] didMapDoubleClick btn_id[" + btn_id);
				console.log("[tsMap.js] didMapDoubleClick tmpParam[" + tmpParam);
				...
				console.log("[tsMap.js] didMapDoubleClick() $tsMap.ui.dropBtnInfo[map.id] [" + $tsMap.ui.dropBtnInfo[map.id]);
				...
				console.log("[tsMap.js] didMapDoubleClick() btn_id[" + btn_id);
				console.log("[tsMap.js] didMapDoubleClick() atdrcYn[" + atdrcYn);
				console.log("[tsMap.js] didMapDoubleClick() map.selectedBoundMode[" + map.selectedBoundMode);
			...
			didMouseOverPolygon : function(event, data, type, map) {// used
				$workRoad.ui.log("$tsMap.callbackFunc.didMouseOverPolygon - begin");
				...
				if (type != "polygon") {
					if( map.isInnerMapShow2!=undefined && map.isInnerMapShow2 ) { //mng_s 그리드일 경우 
						...
						if (type == "data") {
								$tsDataBoard.ui.selectChartData(data.properties, data.dataIdx, map.id);
								map.legend.selectLegendRangeData(event.target.options.fillColor);
						}
						if (data.info.length > 0) { //데이터가 있을 경우만 툴팁을 보여준다.
							$tsMap.ui.createInfoTooltip(event, data, type, map);
						}
			...
			didMouseOutPolygon : function(event, data, type, map) {// used
				$workRoad.ui.log("$tsMap.callbackFunc.didMouseOutPolygon - begin");
			...
			didSelectedPolygon : function(event, data, type, map) {// used
				$workRoad.ui.log("$tsMap.callbackFunc.didSelectedPolygon - begin");
				
				if (type == "data") {
					//2019-01-02 지도에서 지역 선택시 조회처리. (전체인 경우만)
					if($("#current-sido-select").val() == "99" && data.properties != undefined) {
						var lvAdmCd = data.properties.adm_cd;
						$("#current-sido-select option").each(function(index, element){
							if($( element ).val() == lvAdmCd) {
								$("#current-sido-select").val(lvAdmCd);
					 ...
					 *  지도에서 지역(시군구) 선택시 조회처리.(전체인 경우만 해당)
					 */
					if($("#current-sgg-select").val() == "999" && data.properties != undefined) {
						var lvAdmCd = data.properties.adm_cd.substring(2,5);
						$("#current-sgg-select option").each(function(index, element){
							if($( element ).val() == lvAdmCd) {
								$("#current-sgg-select").val(lvAdmCd);
					...
					if ($tsMap.ui.buildPopup != null) {
						$tsMap.ui.buildPopup.close();
					}
			...
			didDrawCreate : function(event, type, map) {
				$workRoad.ui.addFnc01List("$tsMap.callbackFunc.didDrawCreate");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				if (map.curPolygonCode == "1") {
					...
					$tsMapApi.request.userAreaBoundInfo(area, type, map.curPolygonCode, event, map);	
				}
			...
			didFinishedMultidata : function(dataList, admCdList, map) {
				$workRoad.ui.addFnc01List("$tsMap.callbackFunc.didFinishedMultidata");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var admNameList = [];
				...
				if($tsMapFrame.ui.curSelectedStatsType == "userData") {
					return;
				}
				
				$tsDataBoard.ui.updateDataBoardMulti(dataList, options);
			}
	...
	$tsMap.event = {
			...
			setUIEvent : function() {// used
				$workRoad.ui.log("$tsMap.event.setUIEvent - begin");
				
//				Kakao.init('167fc6abf0eb4717e1f3de7895a0152a');
				
				$("#openShare").click(function() {
					$workRoad.ui.addFnc01List("$tsMap.event.setUIEvent.[1]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					
				});
				
				var isClose = false;
				$(".tb_close").click(function(){// used
					$workRoad.ui.log('$tsMap.event.setUIEvent.[$(".tb_close").click]');
					...
					if (sceneInx == 1) {
						...
						if (!isSameView) {
							if (tmpView[0] < tmpView[1]) {
								$tsMap.ui.curMapId = tmpView[0];
							}else {
								$tsMap.ui.curMapId = tmpView[1];
							}
							
							var id = "view" + ($tsMap.ui.curMapId + 1);
							switch($tsMap.ui.curMapId) {
								case 0:
				...
				$(".sceneBox").click(function(){// used
					$workRoad.ui.log('$tsMap.event.setUIEvent.[$(".sceneBox").click]');
					...
					var curMapId = $tsMap.ui.curMapId;
					var dataType = $tsMap.ui.dataTypeList[curMapId];		//census, kosis, publicData, userData
					
					//$mydataDataBoard.ui.delegateSetting($tsMap.ui);			//나의데이터 세팅
					//$publicDataBoard.ui.delegateSetting($tsMap.ui);			//공공데이터 세팅

					//다른 분할맵을 선택할 경우
					if(curMapId != $tsDataBoard.ui.map_id) {
						if(dataType == "census" || dataType == "kosis") {
							//현재 선택된 맵으로 데이터보드 다시 그리기
							$tsDataBoard.ui.reDraw(curMapId, dataType);
							...
							$tsDataBoard.ui.reset(curMapId);
						}
					}
					$tsDataBoard.ui.map_id = curMapId;		//현재 선택된 맵 아이디 저장
					
					//Left Menu 통계표출 연동
//					$tsMapFrame.ui.showNumberSetting();
			    });
...
//					
//					$tsMapFrame.ui.isInnerMapShow2 = false;
//					
//					//좌측메뉴 보이기
...
//					
//					$tsMapFrame.ui.isInnerMapShow2 = true;
//					sop.isInnerMapShow2 = true;
//					
//					$tsMap.ui.doClearMap(1); //지도 초기화
//					
//					//좌측메뉴 숨기기
...
//					$(".btn_legendSetting").hide(); //범례 환경설정 버튼 하이드
//					
//					console.log("[tsMap.js] $tsMapFrame.ui.searchbtnCnt [" + $tsMapFrame.ui.searchbtnCnt);
//					//그리드로 넘어오기전 생성된 버튼을 제거한다. 그리드에서 서비스 하지 않는 버튼이 있기 때문에
//					if($tsMapFrame.ui.searchbtnCnt!=undefined && $tsMapFrame.ui.searchbtnCnt>0) {
//						for(i=0; i<$tsMapFrame.ui.searchbtnCnt; i++) {
//							$tsMapFrame.ui.deleteSearchBtn(i);
//						}
//					}
//					
...
//					
//					$tsMapFrame.ui.isInnerMapShow3 = false;
//					
//					//좌측메뉴 보이기
//					$("#gridHideLeftBtn01").show();
//					$("#gridHideLeftBtn02").show();
...
//					
//					$tsMapFrame.ui.isInnerMapShow3 = true;
...
//					$(".btn_legendSetting").hide(); //범례 환경설정 버튼 하이드
//					
//					console.log("[tsMap.js] bnd_grid 행정구역 그리드 $tsMapFrame.ui.searchbtnCnt [" + $tsMapFrame.ui.searchbtnCnt);
//					//그리드로 넘어오기전 생성된 버튼을 제거한다. 그리드에서 서비스 하지 않는 버튼이 있기 때문에
//					if($tsMapFrame.ui.searchbtnCnt!=undefined && $tsMapFrame.ui.searchbtnCnt>0) {
//						for(i=0; i<$tsMapFrame.ui.searchbtnCnt; i++) {
//							$tsMapFrame.ui.deleteSearchBtn(i);
...
//			    });	
				...
				$("#dataSlider_item").slider({
			        ...
			        slide: function( event, ui ) {  //ui.value
						$workRoad.ui.addFnc01List("$tsMap.event.setUIEvent.[4]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
