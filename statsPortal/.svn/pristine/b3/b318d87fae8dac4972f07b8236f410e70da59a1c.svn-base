...
(function(W, D) {
	...
	$(document).ready(
		function() {
			...
			//srvLogWrite("C0", "01", "01", "00", "", "");		//대화형통계지도 메인 뷰
		...
		//	commonPopupObj.openWin("interactive_laypopup");
			...
			//6레벨일 경우 top,left 를 50% 로 설정하면 주변 지역이 조회되는 경우가 있어서 top,left 조절
			//해상도에 따라 분기처리
			if (iHeight >=1080||iWidth >=1920) { //1920*1080
				for(var i = 1; i <=3; i++){
					...
					//보고서 출력시 중심점 마커 표시x
					$("#div_target" + i).attr('data-html2canvas-ignore', 'true');
				}
			}else{ //1920*1080 아래
				for(var i = 1; i <=3; i++){
					$('#div_target'+i).css({
			...
			//mng_s djlee 20210430 화면 크기 조정 - full
	...
	$interactiveMap.ui = {
			...
			createMap : function(id, seq) {
				...
				map.createMap($interactiveMap, id, {
					...
					//mng_s 20200518 이진호
					//대화형통계지도에서 zoomSliderControl 사용할 수 있도록 설정
...
//					$("header").css({"height":"104px", "width":"970px"}); 
			...
			doClearMap : function(type, isStatsInfoClear, callYN) {			//20200511 수정 (ggm)
				srvLogWrite("C0", "07", "06", "00", "", "");	//초기화
				$interactiveMap.ui.searchMarkerClear();
				
				$interactiveMap.noReverseGeoCode = false;
				this.curMapId = parseInt(type)-1;
				var clearFlag = false;						//20200511 수정 (ggm)
				if (this.mapList.length > 0) {
					var map = this.mapList[this.curMapId];
					if (isStatsInfoClear == undefined || isStatsInfoClear) {
						clearFlag = true;					//20200511 수정 (ggm)
						map.clearDataOverlay(callYN);		//20200511 수정 (ggm)
					}
				...
				//20200511 수정 시작 (ggm)
				if (clearFlag) {
					$mydataDataBoard.ui.isStatsInfoClear = false;
				}
				//20200511 수정 끝				
			...
			doInnerMap : function(type, isShow) {
				...
				if (isShow) {
					...
					$(".sop-overlay-pane").show();//사업체 전개도 꺼도 보이는 오류 해결 2019-04-04 박길섭
					...
					$("#schNmImg").hide();
					$("#searchHelp").hide();
					$(".inputSearchKeyboard").hide();
					closeKeyboard();
					
					//mng_s 20200727 이진호
					//행정구역 그리드 보기가 활성화 되고 줌슬라이더 활성화 시 줌 슬라이더의 위치 수정
					$("#view1").find("[id^='zoomBtn_']").click(function(){
						$("#view1").find(".sop-control-zoomSlider.sop-bar.sop-control").css("top", "118px");
					});
					//mng_e 20200727 이진호
					
					//mng_s 20201112 이진호
					//줌슬라이더가 먼저 활성화 된 상태로 사업체 전개도 활성화 시 줌 슬라이더 위치 수정
					if($("#view1").find(".sop-control-zoomSlider.sop-bar.sop-control").css("display") != "none"){
						$("#view1").find(".sop-control-zoomSlider.sop-bar.sop-control").css("top", "118px");
					}
					//mng_e 20201112 이진호
					
					//mng_s 20201112 이진호
					//사업체 전개도 활성화 시 '행정구역단위 그리드 보기' 버튼 숨김 & 전체화면 버튼 생성
					$("#view1").find(".bnd_grid_radio").css("display", "none");
					$("#view1").find("#btnList_1").children("ul").css({"display": "", "margin-right":"-190px"});
					$("#btnList_"+type).find(".tb_mapAdd").hide(); //맵추가
					$("#btnList_"+type).find("#btn_reset").hide(); //맵리셋
					...
					$(".sop-overlay-pane").hide();//2019-04-04 박길섭
					...
					$("#schNmImg").show();
					$("#searchHelp").show();
					$(".inputSearchKeyboard").show();
					
	                                //mng_s 20191114 이금은 
	                                var view_seq = Number($(".sceneBox.on").attr("id").substring(4,5));
	                                $interactiveMap.ui.doAddMap();
	                                $interactiveMap.ui.doRemoveMap(1); 
	                                $interactiveMap.ui.doAddMap(1);
	                                $(".toolBar .tb_right ul li").eq(6).hide();
	                                $(".viewTitle").hide();		
	                                $interactiveMap.ui.doRemoveMap(view_seq+1);	
	                                //mng_e 20191114
	                
	                //mng_s 20201112 이진호    
                    $("#view1").find(".bnd_grid_radio").css("display", "");
                    $("#view1").find("#btnList_1").children("ul").css({"display": "", "margin-right":"0px"});
                    $("#btnList_"+type).find(".tb_mapAdd").show(); //맵추가
					$("#btnList_"+type).find("#btn_reset").show(); //맵리셋
					$("#btnList_"+type).find(".grid_radio").show(); //그리드 하이드
			...
			doInnerMap2 : function(type, isShow) {
				...
				if (isShow) {
					...
					*/
					...
					map.mapBtnInfo.controlShow("setting");
					*/
					$(".sop-overlay-pane").hide();//사업체 전개도 꺼도 보이는 오류 해결 2019-04-04 박길섭
			...
			doInnerMap3 : function(type, isShow) {
				...
				//그리드 활성화시 위치/지표 검색 창을 숨긴다.	leekh
				if(isShow){
					
					//mng_s 20210420 이진호, dataBoard가 on 이면 데이터 보드 닫아준다
					var dataBoard_on = $('.interactiveDataBoard').hasClass('on');
					if(dataBoard_on){
						$(".stepClose-data").trigger('click');
					}
					//mng_e 20210420 이진호
					...
					//mng_s 20201112 이진호
					//줌슬라이더가 먼저 활성화 된 상태로 행정구역 그리드 활성화 시 줌 슬라이더 위치 수정
					if($("#view1").find(".sop-control-zoomSlider.sop-bar.sop-control").css("display") != "none"){
						$("#view1").find(".sop-control-zoomSlider.sop-bar.sop-control").css("top", "118px");
					}
					//mng_e 20201112 이진호
				...
				if (isShow) {
					srvLogWrite("C0", "10", "01", "00", "", "");	//그리드 뷰
					
					//2019-03-15 박길섭 시작
					$(".bnd_grid_radio").addClass("on");
					if($("#companyBtn>a").hasClass("on")){//전국 사업체조사를 누르고 그리드 켜기 누를시 에러 해결
						//SGIS4_1124_행정구역그리드 사업체조사 추가 시작
						//211025_수정(행정구역 그리드 BSCA 반영 X)
						/*
						$("#buttonMakeBtn").show();
						$("#buttonMakeBtn").text("사업체조사 버튼생성");
						$(".bottom .stepClose").stop().animate({"right":"-25px"},200);
						$(".buttonBar.ui-draggable").stop().animate({"left":"360px"},200);
						*/
						$("#buttonMakeBtn").hide();
						$("#menuAutoClose2Lev").hide();
						$(".bottom .3depth_close").stop().animate({"right":"-304px"},200);
						//211025_수정(행정구역 그리드 BSCA 반영 X)
						//SGIS4_1124_행정구역그리드 사업체조사 추가 끝
						//2019-03-19 박길섭 시작
						if($("#gridCompanyThemaHide").hasClass("on")){
							$interactiveLeftMenu.ui.companyTab('industry');
						}
						//2019-03-19 박길섭 끝
					}
					...
					*/
					...
					$("#btnList_"+type).find(".tb_report").hide(); //리포트 하이드
					//2019-03-07 박길섭 시작
					$(".stat-infor").hide();//전체 화면,URL 공유하기,즐겨찾기로 저장하기 등등 하이드	
					$(".search-infor").hide();//집계구 위치 찾기 하이드
					$(".interactive-list>li").eq(3).hide();//kosis 하이드
					$(".interactive-list>li").eq(4).hide();//공공데이터 하이드
					$(".interactive-list>li").eq(5).hide();//나의 데이터 하이드
					$(".nav-list>li").eq(2).hide();//kosis 하이드
					$(".nav-list>li").eq(4).hide();//kosis 하이드
					$(".nav-list>li").eq(5).hide();//공공데이터 하이드
					$(".nav-list>li").eq(6).hide();//나의 데이터 하이드
					//2019-03-07 박길섭 끝
					//2019-03-15 박길섭 시작
					//행정그리드에서 나오지 않는 항목 선택하고 행정그리드 활성화 클릭시 총조사 주요지표 활성화
					if($(".nav-list>li").eq(2).find("a").hasClass("on")||$(".nav-list>li").eq(4).find("a").hasClass("on")||$(".nav-list>li").eq(5).find("a").hasClass("on")||$(".nav-list>li").eq(6).find("a").hasClass("on")){
						$("#mainIndexBtn").trigger("click");
					}
					//2019-03-15 박길섭 끝
					//mng_s 범례 설정버튼 show/hide
					$(".btn_legendSetting").hide(); //범례 환경설정 버튼 하이드
					...
					*/
					//SGIS4_1124_수정 끝
					//$(".bottom .3depth_close").stop().animate({"right":"-304px"},200);
					//SGIS_4 행정구역그리드 사업체조사 추가 끝
				}else {
					$(".bnd_grid_radio").removeClass("on");//2019-03-15 박길섭
					//$("#btnList_"+type).find("ul").show();
					/*
					$("#btnList_"+type).find(".tb_mapAdd").show();
					$("#btnList_"+type).find(".tb_radio").show(); //사업체 전개도 하이드
			...
			doCombineMap : function(type) {
				srvLogWrite("C0", "07", "02", "00", "", "");	//지도겹쳐보기
				var isCombine = true;
				var cnt = 0;
				for (var i=0; i<this.mapList.length; i++) {
					if (this.mapList[i] != null) {
			...
			reportLoad : function() {
				...
				if (this.dataTypeList[map.id] == undefined || this.dataTypeList[map.id] == null) {
					...
					if(this.dropBtnInfo[0] == undefined || this.dropBtnInfo[0].filterParam == null) {
						if( map.dataType == 'ecountry' ){
							this.dataTypeList[map.id] = "ecountry";
						} else {
				...
				if (this.dataTypeList[map.id] != undefined && this.dataTypeList[map.id] != null) {
					switch (menuType[this.dataTypeList[map.id]]) {
						case 0:
							...
							switch (subMenu[$publicDataBoard.ui.mapData[map.id].type]) {
								...
								case 2:
									...
									origin = "철도 노선별 관리기관, 지하철 이용현황(2017)";
									break;
								case 3:
									adm_nm = "";
									poiData = options.result.target.info;
						...
						case 4:
							...
							param = ( this.dropBtnInfo[map.id] ? this.dropBtnInfo[map.id] : {} );
							param.isKosis = false;
							param.isEcountry = true;
							param.title = $interactiveMapEcountry.curSelectedTitle;
				...
				// mng_s SseOk 2019.03.06 보고서 확인용 start
//				//2017.02.22 이미지캡쳐 수정
//				//==================================================================================================================================//
//				setTimeout(function() {
//					html2canvas($(divId), {
...
//				//==================================================================================================================================//
				...
				//==================================================================================================================================//
				setTimeout(function() {
					//20190306 leekh 크롬 업데이트에 의해 보고서 분기처리함
					var agent = navigator.userAgent.toLowerCase();
					if ( (navigator.appName == 'Netscape' && navigator.userAgent.search('Trident') != -1) || (agent.indexOf("msie") != -1) ) {
						html2canvas($(divId), {
							logging: true,
				...
				//==================================================================================================================================//
				// mng_s SseOk 2019.03.06 보고서 확인용 end
			...
			reportDataSet : function(type) {
				srvLogWrite("C0", "07", "09", "00", "", "");		//보고서 보기
				this.curMapId = parseInt(type)-1;
				var map = this.mapList[this.curMapId];
				if (map.dataGeojson == null && 
					map.multiLayerControl.dataGeojson == null) {
			...
			doAddMap : function(type) {
				...
				srvLogWrite("C0", "07", "01", "00", "", "");	//다중뷰
				
				$(".interactNavi").css({"left" : "225px"});
				...
				//mng_e
				for (var i=0; i<this.mapList.length; i++) {
					if($("#view" + parseInt(i+1) ).find(".zoomSliderBtn").hasClass("on")){
						$("#view" + parseInt(i+1)).find(".sop-control-zoomSlider.sop-bar.sop-control").css('display', '');
					}
				}
				
				
				apiLogWrite2("A0","A16","다중뷰","없음","00","없음");
				resetDataBoardAndIconsPosition();//20200424 수정 (6번. 지도 추가시 ICON 이상동작 추가 수정);
			},
			...
			doRemoveMap : function(type) {
				$(".interactNavi , #mapNavi_2 ").css({"left" : "165px"});
...
*/				
                ...
                //개발팀 수정본 20200703
				//resetDataBoardAndIconsPosition();//20200424 수정 (6번. 지도 추가시 ICON 이상동작 추가 수정);
				//운영팀 svn 소스 20200703 추후 문제발생시 검토요망
				for (var i=0; i<this.mapList.length; i++) {
					if($("#view" + parseInt(i+1) ).find(".zoomSliderBtn").hasClass("on")){
						$("#view" + parseInt(i+1)).find(".sop-control-zoomSlider.sop-bar.sop-control").css('display', '');
					}
			...
			doAnalysisShareInfo : function (type, data) {
				console.log("doAnalysisShareInfo");
				//통합검색에 의해 생성된 bookmark로 들어온 데이터일 경우 leekh 조회하지 않게 변경
				if(data != "" && $interactiveMap.ui.bookmarkType == null){
					var imsiData = new Array();
					if(data.param_info != null){
						imsiData["param_info"] = JSON.parse(data.param_info);
						if(imsiData.param_info.paramInfo.adm_cd == "9999999"){
							//http://localhost:8080/view/map/interactiveMap/bookmark?id=oKL8rKwFEu20190904154855616CLnqIwJEpp#

							//mng_s 20201028 이진호
							//header 검색으로 검색하여 대화형 통계지도에 접근시 경계년도를 최신으로 받게 수정
							var map = this.mapList[this.curMapId];
							data.param_info=data.param_info.replace('"bnd_year":"2019"','"bnd_year":"'+map.bnd_year+'"');
							//mng_e 20201028 이진호
							
							$interactiveMap.ui.bookmarkType = type;
							$interactiveMap.ui.bookmarkData = data;
							type = "mainIndexView";
							data = "";
							
						//mng_s 20201029 이진호
						//대화형 통계지도는 e지방지표를 제외한 모든 항목의 경계년도가 최신으로 받아야 하기 때문에 else 문 추가
						}else{
							if(data.api_call_url != "/view/ecountry/getData"){
								var map = this.mapList[this.curMapId];
								data.param_info=data.param_info.replace('"bnd_year":"2019"','"bnd_year":"'+map.bnd_year+'"');
					...
					$interactiveMap.ui.bookmarkType = null;
				}
	            ...
	            setTimeout(function() {
	            	...
	            	//mng_s 20200525 이진호 
	            	//통계검색 시 UI 깨짐 오류 해결
	            	if(type == "bookmark" && $interactiveMap.ui.bookmarkData == null && location.href.match("interactiveMap/bookmark")){
	            		$("#org_left_menu").css("left", "-80px");		//북마크일때 1덱스 안보이게	20190703 leekh
	            	}else if(type == "bookmark" && $interactiveMap.ui.bookmarkData == null){
	            		$("#org_left_menu").css("left", "0px"); //대화형 통계지도에서 통계검색 시 보이게
	            	}
	            	//mng_e 20200525 이진호
				...
				if (type.length == 0 || data.length == 0) {
					...
					if (type == "mainIndexView" 	  ||
						...
						if  (type.indexOf("View") != -1) {
							setTimeout(function() {
								$interactiveLeftMenu.ui.setDetailStatsPanel(type.split("View")[0]);		//여기서 설정
							}, 200);
						}
					}
					return false;
				...
				if(type != "kosis" && data.param_info != undefined && type != "ecountry"){ 
					this.isShare = true;
				...
				//200423수정 시작 (ggm)
				var apiInfoForBorough = {
						"API_0301" : $interactiveMapApi.request.API_0301_1_URL,
						"API_0302" : $interactiveMapApi.request.API_0302_1_URL,
						"API_0304" : $interactiveMapApi.request.API_0304_1_URL,
						"API_0305" : $interactiveMapApi.request.API_0305_1_URL,
						"API_0306" : $interactiveMapApi.request.API_0306_1_URL,
						"API_0307" : $interactiveMapApi.request.API_0307_1_URL,
						"API_0308" : $interactiveMapApi.request.API_0308_1_URL,
						"API_0309" : $interactiveMapApi.request.API_0309_1_URL,
						"API_0310" : $interactiveMapApi.request.API_0310_1_URL
				};
				//200423수정 끝				
				...
				var url, title, unit, isKosis = false, isEcountry = false, api_id, center, zoomlevel, showData, paramInfo = {};
				var admCdList = [];
				...
				switch(typeList[type]) {
					...
					case 12:
						...
						if (data.param_info.type == "userData") {
							...
							//20200427 수정 시작 (ggm)
							if((url == "/OpenAPI3/stats/population.json" || url == $interactiveMapApi.request.API_0301_1_URL) && data.param_info.api_id == "API_0304") {
							//20200427 수정 끝	
							...
							if (data.param_info.isEcountry) { //e-지방지표
								data.param_info["api_id"] = "ecountry";
								$interactiveMapEcountry.curSelectedTitle = data.param_info.title;
								$interactiveMapEcountry.ecountry_data_year = data.param_info.paramInfo.prid_value;
								$interactiveMapEcountry.ecountry_data_year_nm = data.param_info.paramInfo.year_nm;
							}
						...
						//20200511 수정 시작 (ggm)
						if(url == apiInfoForBorough[data.param_info.api_id]){
//							var prevMapInfo = data.param_info["mapInfo"];
//							if(prevMapInfo != undefined && prevMapInfo != null){
//								var zLvl = prevMapInfo["zoomlevel"];
//								if(zLvl == 4){
//									data.param_info.paramInfo["is_zoom_lvl4"] = "Y";
//								}
//								
//								var prevParamInfo = data.param_info["paramInfo"];
//								if(prevParamInfo != undefined && prevParamInfo != null){
//									var areaCd = prevParamInfo["adm_cd"];
//									if(zLvl == 5 && $interactiveMap.ui.chkIfNonSelf(areaCd) > -1){
//										data.param_info.paramInfo["is_non_self"] = "Y";
//									}
//								}
//							}
							var prevParamInfo = data.param_info["paramInfo"];
							if(prevParamInfo != undefined && prevParamInfo != null){
								var areaCd = prevParamInfo["adm_cd"];
								var lowSch = prevParamInfo["low_search"];
								if(areaCd != undefined && areaCd != null){									
									if(areaCd.length == 2){
										if(lowSch == "1"){
											data.param_info.paramInfo["is_zoom_lvl4"] = "Y";
										}
									}else if(areaCd.length == 5){
										if($interactiveMap.ui.chkIfNonSelf(areaCd) > -1){
											data.param_info.paramInfo["is_non_self"] = "Y";
											if(lowSch == "0"){
												data.param_info.paramInfo["is_zoom_lvl4"] = "Y";
											}
						...
						}else if(apiInfoForBorough[data.param_info.api_id] != undefined){
							var prevMapInfo = data.param_info["mapInfo"];
							if(prevMapInfo != undefined && prevMapInfo != null){
								var zLvl = prevMapInfo["zoomlevel"];
								if(zLvl == 4){
									data.param_info.mapInfo.zoomlevel = 5;
								}
							}							
						}						
						//20200511 수정 끝
					...
					case 10:
						...
						//20200511 수정 시작 (ggm)
						if($interactiveMap.ui.chkIfInteractiveOnlyForApiId(api_id)){							
							if(map.zoom == 4 && (data["adm_cd"] != undefined && data["adm_cd"] != null && data["adm_cd"].length == 5)){
								var tmpAdmCd = data["adm_cd"].substring(0, 4) + "0";
								if($interactiveMap.ui.chkIfNonSelf(tmpAdmCd) > -1){
									data.adm_cd = tmpAdmCd;
								}								
							}							
						}
						//20200511 수정 끝						
						...
						//200423수정 시작 (ggm)
						//4시군 자치시 관련하여 url, zoomlevel 값을 변경
						if($interactiveMap.ui.chkIfInteractiveOnlyForApiId(api_id)){			//20200427 수정 : case2~10은 4시군 관련 개선대상 api (ggm)
							var isChgUrl = false;
							if(zoomlevel == 4){
								//위(기존 로직)에서 계산한 조회할 zoom 값이 4 이면(이 함수에선 무조건 한단계 아래로 zoom 하므로)
								paramInfo["is_zoom_lvl4"] = "Y";
								isChgUrl = true;
							}
							if(map.zoom == 4 && $interactiveMap.ui.chkIfNonSelf(data.adm_cd) > -1){
								paramInfo["is_non_self"] = "Y";
								zoomlevel = 5;
								isChgUrl = true;
							}
							
							if(isChgUrl){
								url = apiInfoForBorough[apiInfo[typeList[type]-2].id];
							}
						}						
						//200423수정 끝
						break;
...
//						this.mapList[mapId].setZoom(zoom);
					...
					case 15 : 
						...
						$interactiveMapEcountry.searchmode = true;
						$interactiveMapEcountry.map = map;
						
						$interactiveMapEcountry.list_id = data.list_id;
						$interactiveMapEcountry.tbl_id = data.tbl_id;
						$interactiveMapEcountry.adm_cd = data.adm_cd;
						$interactiveMapEcountry.atdrc_yn = data.atdrc_yn;
						$interactiveMapEcountry.map.atdrc_yn = ( data.atdrc_yn == "Y" ? "1" : "0" );
						$interactiveMapEcountry.base_item_id = data.base_item_id;
						$interactiveMapEcountry.add_item_id = data.add_item_id;
						$interactiveMapEcountry.prd_id = data.prd_id;
						$interactiveMapEcountry.prid_value = data.prid_value;
						
						// j.h.Seok
						if(data.adm_cd == undefined || data.adm_cd == null) {
							$interactiveMapEcountry.adm_cd = '00';
						}
						
						var url = "/view/ecountry/getData";
						var zoom = "0";
						
						var title = decodeURIComponent(data.title);
						$interactiveMapEcountry.curSelectedTitle = title;
						$interactiveMapEcountry.kosis_select_menu_text = title;
						
						//전국경계
						if($interactiveMapEcountry.adm_cd == null || $interactiveMapEcountry.adm_cd == "") {
							zoom = "2";
							$interactiveMapEcountry.adm_cd = "00";
						} else {
							var adm_cd_len = ( $interactiveMapEcountry.adm_cd ? $interactiveMapEcountry.adm_cd.length : 1 );
							
							if( adm_cd_len <= 2 ){
								zoom = "1";
							} else if( adm_cd_len == 5 ){
								zoom = "4";
						...
						admCdList.push($interactiveMapEcountry.adm_cd);
						...
						options.push({
							key : "base_item_nm",
							value : $interactiveMapEcountry.ecountry_base_item_nm
						});
						options.push({
							key : "adm_cd",
						...
						options.push({
							key : "title",
							value : $interactiveMapEcountry.curSelectedTitle
						});
						
						$interactiveMapEcountry.searchInit( options, map );
			...
			informationPopOpen : function() {
				if($("#notice_mini_pop").css("display") == "none") {
					...
					//mng_s 20211029 이진호, 유의사항 팝업 드래그로 이동 가능하게 수정 (주무관님 요청)
					$('#notice_mini_pop').draggable();
					//mng_e 20211029 이진호
			...
			createInfoTooltip : function(event, data, type, map) {
				...
				var html = "<table style='margin:10px;' id='toolTip'>";
				var searchYear = "";
				var that = this;
				var showName = {
									"tot_ppltn" : "총인구",
				...
				if(this.curDropParams[map.id] != undefined) {
					for(var i = 0; i < this.curDropParams[map.id].param.length; i ++) {
						if (this.curDropParams[map.id].param[i].key == "year") {
							...
							if(this.curDropParams[map.id].param[i].value == "9016"){
								searchYear = "2016년";
							}
...
//							searchYear = year + "년 ";
				...
				if (type == "data") {
					if (data.info.length > 0) {
						...
						//kosis
						if(data.info[2] == "kosis" || data.info[2] == "ecountry") {
							...
							var itemtext = map.itemtext;

							if( typeof itemtext != 'undefined' && itemtext != 'undefined' && itemtext !== undefined && itemtext != '' ){
								html += "<tr><td class='statsData'>";
								html += itemtext;
								html += "</td></tr>";
							}
							
							if( data.info[2] == "ecountry" ){
								var base_item_nm = $interactiveMapEcountry.ecountry_base_item_nms[ $interactiveMapEcountry.tbl_id_linked ];
								searchYear = base_item_nm + ' ';
							}
								...
								for (var i = 0; i < data.info.length; i++) {
									...
									if (tmpData.showData != undefined && tmpData.showData.length > 0) {
										...
										if (parseFloat(tmpData[tmpData.showData]) < 5 && 
											...
											//tmpData.showData != "employee_cnt" // mng_s 20210422 이진호, 종사자 수 도 N/A 표기
											) {
											...
											} else if (sop.isInnerMapShow3) {
												//211025_수정(행정구역 그리드 BSCA 반영 X)
												value = appendCommaToNumber(tmpData[tmpData.showData]); //행정구역 그리드 이면 5미만도 표시해 준다
												//SGIS4_1124_행정구역 그리드 BSCA 적용 시작
												/*
												if ( 1<= parseFloat(tmpData[tmpData.showData]) <= 4 ){
													value = "N/A"; //mng_s 20180831 1~4까지는 N/A로 표시하기로함
												} 
												if ( parseFloat(tmpData[tmpData.showData]) == 0 ){
													//value = "0"; //mng_s 20180831 
													value = "N/A"; //mng_s 20180831 일단 N/A로 표시하는데 추후 0인 데이터는 쿼리단에서 분기해야될듯하다.
												}
												*/
												//211025_수정(행정구역 그리드 BSCA 반영 X) 끝
												//SGIS4_1124_행정구역 그리드 BSCA 적용 끝
										...
										if (value != "N/A") {
											...
											if (sop.isInnerMapShow3) { //행정구역 그리드 이면
												...
												*/
												// 총조사제외, 전체 제외, 500m/100m격자만 적용
												//if((data.info[0].statType == null || data.info[0].statType == "undefined" || "tot" != data.info[0].statType) && "API_0301" != data.info[0].api_id && (map.zoom == 8 || map.zoom == 9 || map.zoom == 10 || map.zoom == 11 || map.zoom == 12 || map.zoom == 13)){//SGIS4_1124_BSCA적용 추가(zoom 레벨 8,9,10,11,12,13은 구간으로 표시)
													//SGIS4_0107_수정 시작
													/*
													if( Number( tmpData[tmpData.showData])  < Number(map.legend.valPerSlice[0][1])) {
														value = appendCommaToNumber(map.legend.valPerSlice[0][1]) + " 이하";
													} else if(Number(map.legend.valPerSlice[0][1]) <  Number(tmpData[tmpData.showData]) && Number(tmpData[tmpData.showData]) <= Number(map.legend.valPerSlice[0][2]) ) {
														value = appendCommaToNumber(map.legend.valPerSlice[0][1]) + "~" + appendCommaToNumber(map.legend.valPerSlice[0][2]) + " 이하";
													} else if(Number(map.legend.valPerSlice[0][2]) <  Number(tmpData[tmpData.showData]) && Number(tmpData[tmpData.showData]) <= Number(map.legend.valPerSlice[0][3]) ) {
														value = appendCommaToNumber(map.legend.valPerSlice[0][2]) + "~" + appendCommaToNumber(map.legend.valPerSlice[0][3]) + " 이하";
													} else if(Number(map.legend.valPerSlice[0][3]) <  Number(tmpData[tmpData.showData]) && Number(tmpData[tmpData.showData]) <= Number(map.legend.valPerSlice[0][4]) ) {
														value = appendCommaToNumber(map.legend.valPerSlice[0][3]) + "~" + appendCommaToNumber(map.legend.valPerSlice[0][4]) + " 이하";
													} else if(Number(map.legend.valPerSlice[0][4]) <  Number(tmpData[tmpData.showData]) && Number(tmpData[tmpData.showData]) <= Number(map.legend.valPerSlice[0][5]) ) {
														value = appendCommaToNumber(map.legend.valPerSlice[0][4]) + "~" + appendCommaToNumber(map.legend.valPerSlice[0][5]) + " 이하";
													} else if(Number(map.legend.valPerSlice[0][5]) <  Number(tmpData[tmpData.showData]) && Number(tmpData[tmpData.showData]) <= Number(map.legend.valPerSlice[0][6]) ) {
														value = appendCommaToNumber(map.legend.valPerSlice[0][5]) + "~" + appendCommaToNumber(map.legend.valPerSlice[0][6]) + " 이하";
													} else if(Number(map.legend.valPerSlice[0][6]) <  Number(tmpData[tmpData.showData]) && Number(tmpData[tmpData.showData]) <= Number(map.legend.valPerSlice[0][7]) ) {
														value = appendCommaToNumber(map.legend.valPerSlice[0][6]) + "~" + appendCommaToNumber(map.legend.valPerSlice[0][7]) + " 이하";
													} else if(Number(map.legend.valPerSlice[0][7]) <  Number(tmpData[tmpData.showData]) && Number(tmpData[tmpData.showData]) <= Number(map.legend.valPerSlice[0][8]) ) {
														value = appendCommaToNumber(map.legend.valPerSlice[0][7]) + "~" + appendCommaToNumber(map.legend.valPerSlice[0][8]) + " 이하";
													} else if(Number(map.legend.valPerSlice[0][8]) <  Number(tmpData[tmpData.showData]) && Number(tmpData[tmpData.showData]) <= Number(map.legend.valPerSlice[0][9]) ) {
														value = appendCommaToNumber(map.legend.valPerSlice[0][8]) + "~" + appendCommaToNumber(map.legend.valPerSlice[0][9]) + " 이하";
													} else if(Number(map.legend.valPerSlice[0][9]) <  Number(tmpData[tmpData.showData])  ) {
														value = appendCommaToNumber(map.legend.valPerSlice[0][9]) + " 초과";
													}
													*/
													//SGIS4_0107_수정 끝
												//}//SGIS4_1124_BSCA적용 추가(zoom 레벨 8,9,10,11,12,13은 구간으로 표시)
											}
											
											//211025_수정(행정구역 그리드 BSCA 반영 X) 끝
											/*
											if (sop.isInnerMapShow3 && parseFloat(tmpData[tmpData.showData]) == 0 ) {
											...
											*/
											...
											//mng_s 20210421 이진호, 행정구역단위 그리드 모드에서 N/A 값일 때 분기 처리
						...
						if (sop.isInnerMapShow2 || sop.isInnerMapShow3) { //그리드 이면
							...
							if(( $interactiveMap.ui.mapList[0].dataType == "kosis" || $interactiveMap.ui.mapList[0].dataType == "ecountry" ) 
								&& $interactiveMap.ui.mapList[0].dataForCombine[0].UNIT != undefined){
								html += "<td class='statsData'>"+title+" 0("+$interactiveMap.ui.mapList[0].dataForCombine[0].UNIT+")</td>";
							} else {
					...
					html += "<tr><td class='admName'>" + bd_nm + "</td></tr>" 
					     ...
					     /*+  "<tr><td class='statsData'>" +lowest + " ~ "+ highest +"</td></tr>" *///2019-03-27 박길섭
					     +	"<tr style='height:5px;'></tr>" 
			...
			requestOpenApi : function(options) {
				...
				for (var i = 0; i < options.param.length; i ++) {
					...
					//200423수정 시작 (ggm)
					} else if(options.param[i].key == "is_zoom_lvl4" || options.param[i].key == "is_non_self") {	// TODO: 무조건 삭제할지, 아래 조건에 따라 삭제할지 확인	
					//200423수정 끝
					} else {
				...
				//20200417 수정 시작 (ggm)
				//데이터보드(시계열 조회)에서 호출 시 기존값 사용
				if(options["view_type"] != "TS"){					
					//비자치구 관련 개선 대상만으로 한정되도록 조건 설정: 이후 두 변수 할용 시에 "Y" 인지 비교					
					//200423수정 시작 (ggm)
					if($interactiveMap.ui.chkIfInteractive() && !options.map.isInnerMapShow2 && !options.map.isInnerMapShow3){
						if($interactiveMap.ui.chkIfZoomLvl4(options.map.zoom, options.map.boundLevel) == "Y"){
							options["is_zoom_lvl4"] = "Y";
						}
						if(options.map.zoom == 4 && $interactiveMap.ui.chkIfNonSelf(options['adm_cd']) > -1){
							options["is_non_self"] = "Y";
						}
					}						
					//200423수정 끝
				}
			 ...
			 * 
			 * @name         : chkIfZoomLvl4
			 * @description  : zoom 4레벨 조회여부 확인
			 * @date         : 2020. 04. 02. 
			 * @author	     : 
			 * @history 	 :
			 * @param 		 : curZoomNo, lowSearch
			 */
			chkIfZoomLvl4 : function(curZoomNo, lowSearch) {				
				var rstVal = "N";
				...
				console.log("[interactiveMap.js] chkIfZoomLvl4(): >>>>> " + rstVal);
			...
			/**
			 * 
			 * @name         : chkIfInteractive
			 * @description  : 4시군구 관련 기능적용 대상여부 확인
			 * @date         : 2020. 04. 16. 
			 * @author	     : 
			 * @history 	 :
			 * @param		 :
			 */
			chkIfInteractive : function() {				
				var rstVal = false;		// 기능적용 대상여부
				var isClick = false;	// 서브메뉴가 클릭되었는지
				
				$(".nav-list li").each(function(index , elem){
					if($(elem).find("a").hasClass("on")){
						isClick = true;
						// 총조사주요지표, 인구주택총조사, 농림어업총조사, 전국사업체조사 일 경우 true
						if($(elem).data("key") == "mainIndex" || $(elem).data("key") == "populationHouse" || $(elem).data("key") == "3f" || $(elem).data("key") == "company") {
							rstVal = true;
							console.log("[interactiveMap.js] chkIfInteractive(): 메뉴선택 >>>>> " + rstVal);
						}
				...
				//대화형통계지도 서브메뉴 선택 상태가 아닐 때(URL공유하기 등으로 들어와서 ..) 
				if(!isClick){
					var dropParam = $interactiveMap.ui.curDropParams[$interactiveMap.ui.curMapId];
					if(dropParam != null && dropParam != undefined){
						var pApiId = dropParam["api_id"];
						if(this.isInteractiveApiIdList.indexOf(pApiId) > -1){
							rstVal = true;
							console.log("[interactiveMap.js] chkIfInteractive(): drop정보 체크 >>>>> " + rstVal);
						}							
					}					
					if(!rstVal){
						var map = this.mapList[$interactiveMap.ui.curMapId];
						if(map.dataForCombine != null && map.dataForCombine != undefined){
							var pApiId = map.dataForCombine["id"];
							if(this.isInteractiveApiIdList.indexOf(pApiId) > -1){
								rstVal = true;
								console.log("[interactiveMap.js] chkIfInteractive(): 통계정보 체크 >>>>> " + rstVal);
							}							
						}						
					}	
				}
				
				//대화형통계지도 서브메뉴 선택 상태가 아니고 드랍도 발생하지 않았는데, 행정경계 API 호출 시 통계정보 체크
//				if(!isClick && !rstVal){					
//					if(arguments.length >= 2 && arguments[0] == "chkData"){						
//						var mapSeq = arguments[1];
//						var map = this.mapList[mapSeq];
//						if(map.dataForCombine != null && map.dataForCombine != undefined){
//							var pApiId = map.dataForCombine["id"];
//							if(this.isInteractiveApiIdList.indexOf(pApiId) > -1){
//								rstVal = true;
...
//				}
				...
				console.log("[interactiveMap.js] chkIfInteractive(): >>>>> " + rstVal);

				return rstVal;
			},
			
			chkIfInteractiveOnlyForApiId : function(pApiId) {				
				var rstVal = false;
				if(this.isInteractiveApiIdList.indexOf(pApiId) > -1){
					rstVal = true;
				}				
				console.log("[interactiveMap.js] chkIfInteractiveOnlyForApiId(): >>>>> " + rstVal);
			 ...
			 * 
			 * @name         : chkIfNonSelf
			 * @description  : 비자치구를 포함한 시(코드)여부 확인
			 * @date         : 2020. 04. 16. 
			 * @author	     : 
			 * @history 	 :
			 * @param		 :
			 */
			chkIfNonSelf : function(pAdmCd) {				
				var rstVal = -1;
				
				if(pAdmCd != null && pAdmCd != undefined){
					rstVal = this.nonSelfGoverningList.indexOf(pAdmCd);
				}else{
					rstVal = this.nonSelfGoverningList.indexOf(this.curAdmCode);
				}				
				console.log("[interactiveMap.js] chkIfNonSelf(): >>>>> " + rstVal + "(param:" + pAdmCd + ")");
			...
			reqSetParams : function (tmpParam, adm_cd, adm_nm, api_id, map, type) {
				...
				var year = tmpParam.year
				
				
				
				
				var filterParamGb = tmpParam.filterParam;
				
				//총조사 주요지표
				if(filterParamGb == "tot_ppltn"){					//총인구 통계조
					if(!map.isInnerMapShow3){
						srvLogWrite("C0", "02", "01", "01", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
					}else{
						srvLogWrite("C0", "10", "02", "01", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);		//총인구 통계조회
					}
				}else if(filterParamGb == "avg_age"){				//평균나이 통계조회
					srvLogWrite("C0", "02", "01", "02", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
				}else if(filterParamGb == "ppltn_dnsty"){			//인구밀도 통계조회
					srvLogWrite("C0", "02", "01", "03", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
				}else if(filterParamGb == "aged_child_idx"){		//노령화지수 통계조회
					srvLogWrite("C0", "02", "01", "04", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
				}else if(filterParamGb == "oldage_suprt_per"){		//노년부양비 통계조회
					srvLogWrite("C0", "02", "01", "05", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
				}else if(filterParamGb == "juv_suprt_per"){			//유년부양비 통계조회
					srvLogWrite("C0", "02", "01", "06", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
				}else if(filterParamGb == "avg_fmember_cnt"){		//평균 가구원 통계조회
					srvLogWrite("C0", "02", "01", "07", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
				}else if(filterParamGb == "tot_house"){				//주택 통계조회
					if(!map.isInnerMapShow3){
						srvLogWrite("C0", "02", "01", "08", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
					}else{
						srvLogWrite("C0", "10", "02", "03", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);		//그리드 주택통계조회
					}
				}else if(filterParamGb == "corp_cnt"){		
					//사업체수 통계조회
					if(!map.isInnerMapShow3){
						if(tmpParam.title == "사업체수"){
							srvLogWrite("C0", "02", "01", "09", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
						}else {
							srvLogWrite("C0", "02", "04", "01", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
						}
					}else{
						if(tmpParam.title == "사업체수"){
							srvLogWrite("C0", "10", "02", "04", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
						}else{
							srvLogWrite("C0", "10", "04", "01", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
						}
						
					}
				}
				//인구주택 총조사
				else if(filterParamGb == "population"){//인구조건 2종 검색설정 및 통계조회
					if(!map.isInnerMapShow3){
						if(tmpParam.names != undefined){//SGIS_4 다중시계열 관련(if문 추가)
							if(tmpParam.names[0] == "농가"){
								srvLogWrite("C0", "02", "03", "04", tmpParam.title,  "year:" + year + ", adm_cd:" + adm_cd);
							}else if(tmpParam.names[0] == "임가"){
								srvLogWrite("C0", "02", "03", "04", tmpParam.title,  "year:" + year + ", adm_cd:" + adm_cd);
							}else if(tmpParam.names[0] == "어가"){
								srvLogWrite("C0", "02", "03", "04", tmpParam.title,  "year:" + year + ", adm_cd:" + adm_cd);
							}else{
								var paramStr = "";
								for(var i=0; i<tmpParam.names.length; i++){
									if(i != 0){
										paramStr = paramStr + ", " + tmpParam.names[i];
									}else{
										paramStr = tmpParam.names[i];
									}
								}
								srvLogWrite("C0", "02", "02", "01", tmpParam.title, "year:" + year + ", " + paramStr + ",adm_cd:" + adm_cd);
							}
						}else{//SGIS_4 다중시계열 관련(if문 추가)
							//다중시계열 관련
							//srvLogWrite("C0", "02", "03", "04", tmpParam.title,  "year:" + year + ", adm_cd:" + adm_cd);
						}//SGIS_4 다중시계열 관련(if문 추가)
					}else{
						srvLogWrite("C0", "10", "03", "01", tmpParam.title, "year:" + year + ", " + paramStr + ",adm_cd:" + adm_cd);
					}
					
				}else if(filterParamGb == "household_cnt"){//가구조건 검색설정 및 통계조회
					if(!map.isInnerMapShow3){
						/*
						 * tmpParam.params[0].value
						 * 01 : 1세대 가구
						 * 02 : 2세대 가구
						 * 03 : 3세대 가구
						 * 04 : 4세대 가구
						 ...
						 * B0 : 혈연가구
						 */
						var houseHoldParam = tmpParam.params[0].value;
						houseHoldParam  = replaceAll2("01", "1세대 가구", houseHoldParam );
						houseHoldParam  = replaceAll2("02", "2세대 가구", houseHoldParam );
						houseHoldParam  = replaceAll2("03", "3세대 가구", houseHoldParam );
						houseHoldParam  = replaceAll2("04", "4세대 가구", houseHoldParam );
						houseHoldParam  = replaceAll2("A0", "1인 가구", houseHoldParam );
						houseHoldParam  = replaceAll2("B0", "혈연가구", houseHoldParam );
						if(houseHoldParam == "0"){
							houseHoldParam = "다중선택안함";
						}
						
						srvLogWrite("C0", "02", "02", "02", houseHoldParam, "year:" + year + ", adm_cd:" + adm_cd);
					}else{
						srvLogWrite("C0", "10", "03", "02", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd + " ," + tmpParam.title);
					}
				}else if(filterParamGb == "house_cnt"){	//주택조건
					if(!map.isInnerMapShow3){
						srvLogWrite("C0", "02", "02", "03", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
					}else{
						srvLogWrite("C0", "10", "03", "03", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd + " ," + tmpParam.title);
					}
				
				}else if(filterParamGb == "data_cnt"){	//결합조건
					
					srvLogWrite("C0", "02", "02", "04", tmpParam.title,  "year:" + year + ", adm_cd:" + adm_cd + ", 결합조건:" + tmpParam.title);
					
				}else if(filterParamGb == "farm_cnt"){	//농림어업총조사 - 농가
					srvLogWrite("C0", "02", "03", "01", tmpParam.title,  "year:" + year + ", adm_cd:" + adm_cd);
				}else if(filterParamGb == "forestry_cnt"){	//농림어업총조사 -임가
					srvLogWrite("C0", "02", "03", "02", tmpParam.title,  "year:" + year + ", adm_cd:" + adm_cd);
				}else if(filterParamGb == "fishery_cnt"){	//농림어업총조사 -어가
					srvLogWrite("C0", "02", "03", "03", tmpParam.title,  "year:" + year + ", adm_cd:" + adm_cd);
				}else if(filterParamGb == "employee_cnt" ){	//산업분류 사업체 통계조회
					srvLogWrite("C0", "02", "04", "01", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
				}else if(filterParamGb == "tot_worker" ){	//산업분류 사업체 통계조회
					if(tmpParam.params[0].key == "theme_cd"){
						srvLogWrite("C0", "02", "04", "02", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
					}else{
						srvLogWrite("C0", "02", "04", "01", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
					}
				}else if(filterParamGb == "tot_family" ){	//산업분류 사업체 통계조회
					if(map.isInnerMapShow3){
						srvLogWrite("C0", "10", "02", "02", tmpParam.title, "year:" + year + ", adm_cd:" + adm_cd);
					}
				}
				...
				var params = {
						...
						type : type, //SGIS_4 추가
						is_zoom_lvl4 : tmpParam.is_zoom_lvl4, //SGIS4_1124_다중시계열4시군 적용 관련
						is_non_self : tmpParam.is_non_self //SGIS4_1124_다중시계열4시군 적용 관련
				};	
				if(tmpParam.isLowSearch != "Y" || tmpParam.isLowSearch == null || tmpParam.isLowSearch == undefined || tmpParam.isLowSearch == "undefined"){//SGIS4_1124_lowSeach적용 관련 추가
				params.param.push({
					key : "low_search",
					value : low_search
				});
				}//SGIS4_1124_lowSeach적용 관련 추가
				
				return params;
			},
			/*SGIS_4 다중시계열 관련 시작*/
			colseListPopup : function(){
				$("#multiTimeSeriesPopup").hide();
			},
			/*
			setMultiTimeSeriesData : function(){
				...
				if(ck <= 4){
					...
					if($("input:checkbox[name=multiTimeRange]").is(":checked") == true) {
						$("input[name=multiTimeRange]:checked").each(function(i){
							selectYears.push($(this).val());
						});
					}
					$interactiveMap.ui.multiTimeList = selectYears;
					$interactiveMap.ui.multiTimeDataSum = $interactiveDataBoard.ui.dataSumValue;
					
					$("#multiTimeSeriesPopup").show();
					
					$interactiveMultiTimeSeries.ui.setMultiTimeSeries();
					//window.open("/view/map/interactiveMultiTimeSeries", "width=1709px ,heigth=723px");
				}else{
...
*/
				...
				if($interactiveMap.ui.bookmarkType!= null){
					setTimeout(function(){
						...
						var type = $interactiveMap.ui.bookmarkType;
						var data = $interactiveMap.ui.bookmarkData;
						...
						data.param_info = replaceAll2("989674,1818313", coordX + "," +coordY , data.param_info)
						data.param_info = replaceAll2("9999999",adm_cd , data.param_info)
						
						
						
					//	imsiData["param_info"] = JSON.parse(data.param_info);;

						$interactiveMap.ui.doAnalysisShareInfo(type, data);
			...
			didMapZoomEnd : function(event, map) {
				...
				if(map.isInnerMapShow3) {
					...
					*/
					...
					//211025_수정(행정구역 그리드 BSCA 반영 X)
					//SGIS4_1124_500m격자 추가 끝
					$("#grid_title_1").html("<span style='font-weight:bold;'>지도레벨 : " + map.zoom + "</span>(0~13), &nbsp;&nbsp;<span style='font-weight:bold;'> 격자레벨 : " + grid_level + "</span>(100km/10km/1km/100m)");//211025_수정(행정구역 그리드 BSCA 반영 X)
				}
...
//				}
				...
				if (map.zoom < 10 && map.isInnerMapShow) {
					messageConfirm.open(
			    			 "알림", 
			    			 "해당 레벨부터는 사업체전개도를 볼 수 없습니다.<br>" +
			    			 "계속하기 버튼을 누르면 다시 사업체전개도를 볼 수 있습니다.",
			    			 btns = [
								{
								    title : "그만보기",
								    fAgm : null,
								    ...
								    func : function(opt) {
								    	$interactiveMap.ui.doInnerMap(map.id+1);
										$(".grid_radio").show();
										$(".tb_right").find("ul").show();
										$(".tb_right").css("margin-right", "0px");
										map.mapBtnInfo.controlShow("poi");
										map.mapBtnInfo.controlShow("setting");
										$(".tb_radio").css("background","url(/img/bg/bg_tbradio_on.png)");
										map.openApiBuildArea(map.bounds);											
								 ...
								 },
			    			     {
								   ...
								   func : function(opt) {
										$(".grid_radio").hide();
										$(".tb_right").find("ul").hide();
										$(".tb_right").css("margin-right", "5px");
										map.mapBtnInfo.controlHide("poi");
										map.mapBtnInfo.controlHide("setting");
										map.setZoom(10);//2019-04-03 박길섭
								   }
			    			     }   
			    			 ]
			    	);
				...
				//2017.06.26 [개발팀] 공공데이터 추가 - 대전-세종간 통행정보
				if (publicDataBoard.type != "cctv") {
					if (map.zoom < 9 && publicDataBoard.options.map != null && $publicDataBoard.ui.isShow && publicDataBoard.options.markerGroup.length > 0) {
				    			 ...
				    			 btns = [
									{
									    ...
									    func : function(opt) {
									    	...
									    	//mng_s 20210720 이진호
									    	//그만보기를 눌러도 POI가 클리어 되지 않는 현상 수정
									    	poiControl.clearPOI();
									    	//mng_e 20210720 이진호
									    }
									 },
			...
			didMapDropEnd : function(event, source, layer, data, map) {
				$interactiveMap.ui.searchMarkerClear();		//leekh 검색에 의한 마커 클리어
				if( map.isInnerMapShow2!=undefined && map.isInnerMapShow2 ) { //mng_s 그리드일 경우
					//그리드는 드랍 이벤트 없음
				} else {
					...
					if (map.isFixedBound) {
//						map.setBoundSelectedMoode("multi");
						map.setBoundSelectedMoode(null);		//20190521 지도 고정 조회시 multi로 조회하지 않기로 함
					...
					if (id.split("-")[0] == "kosis") {
						...
						$interactiveMap.ui.curBtnId = $(source).find('a').eq(0).attr('id');
						...
						return;
					} else if( id.split("-")[0] == "ecountry" ) {	//e-지방지표
						$interactiveMap.ui.curBtnId = $(source).find('a').eq(0).attr('id');
						...
						for (var i = 0; i < tmpParamList.length; i++) {
							if (tmpParamList[i].idx == index) {
								$interactiveMapEcountry.curSelectedTitle = tmpParamList[i].title;
								$interactiveMapEcountry.ecountry_data_year_nm = tmpParamList[i].year_nm;
								selParams.push(tmpParamList[i].params);
								break;
							}
						}
						
						var ecountryYear = null;
						for (var k=0; k<selParams[0].length; k++) {
							if (selParams[0][k].key == "ecountry_data_year") {
								ecountryYear = selParams[0][k].value;
								if (ecountryYear.length > 4) {
									ecountryYear = ecountryYear.substring(0,4);
								}
								break;
							}
						}
						if (ecountryYear != null) {
							$interactiveMap.ui.curDropParams[map.id] = {
									param : [{
										key : "year",
										value : ecountryYear
						...
						$interactiveMap.ui.setDataType(map.id, "ecountry");
						$interactiveMap.ui.dropBtnInfo[map.id] = tmpParamList[i];
						$interactiveMap.ui.dropBtnInfo[map.id]["isEcountry"] = true;
						$interactiveMapEcountry.reqSetEcountryParam(selParams, tempAdmCd, atdrcYn, map);
						$interactiveLeftMenu.ui.updateSearchBtnEffect(id, map.id);
						return;	
					} else {
			...
			didMapDoubleClick : function(btn_id, tmpParam) {
				...
				if (id.split("-")[0] == "kosis") {
					...
					$interactiveMap.ui.curBtnId = $('#'+btn_id).attr('id');
					...
					//더블클릭으로 kosis 조회시 보고서 안나오는 문제 해결 leekh 20190902 start
					var tmpParamList = deepCopy($interactiveLeftMenu.ui.arParamList);
				...
				if (map.selectedBoundMode == "multi") {
					...
					if (adm_cd.length == 0 && !map.isInnerMapShow2) { //mng_s
						...
						if( 0 < $(".tb_radio").css("background").indexOf("bg_tbradio_off")){
							...
							messageAlert.open("알림", "초기화 버튼을 클릭하여 경계를 조회해주세요.");  	// 화면에 경계가 그려지지 않았을 경우에 나타남. 예) 최초 화면 로딩시 등
							return;
						}
						// mng_e 20191114 이금은
	...
	$interactiveMap.event = {
			...
			setUIEvent : function() {
				...
				//Kakao.init('c5009ab618c0964819f69049d2ccf84e'); //김책임님 키 2021버전 js SDK
...
/*				$(".tb_mapAdd").click(function(){ 
				...
				$(".sceneBox").click(function(){
					...
					if(curMapId != $interactiveDataBoard.ui.map_id) {
						if(dataType == "census" || dataType == "kosis" || dataType == "ecountry") {
							//현재 선택된 맵으로 데이터보드 다시 그리기
				...
				$(".grid_radio .fr").click(function(){
					...
					$("#multiTimeSeries").hide(); //SGIS_4 추가
				...
				$(".bnd_grid_radio .fl").click(function(){ 
					//$(".bnd_grid_radio").css("background","url(/img/bg/bg_bnd_gridradio_on.png)"); //십자 모양의 그리드 버튼
					$(".bnd_grid_radio").css("background","url(/img/bg/bg_gridradio_on.png)"); //사각형 모양의 그리드 버튼
				...
				$(".bnd_grid_radio .fr").click(function(){
					//$(".bnd_grid_radio").css("background","url(/img/bg/bg_bnd_gridradio_off.png)"); //십자 모양의 그리드 버튼
					$(".bnd_grid_radio").css("background","url(/img/bg/bg_gridradio_off.png)"); //사각형 모양의 그리드 버튼
					...
					$("#gridHideCombineBtn").hide(); //인구주택총조사의 결합조건 탭 하이드
					$("#gridHideShowNumberBtn").hide(); //통계표출 하이드
					$("#org_left_menu").hide(); //좌측메뉴 하이드
					$("#bnd_grid_left_menu").show(); //행정구역 그리드용 좌측메뉴 보이기
					$("#org_left_menu_chg").hide(); //좌측메뉴 하이드
					$("#bnd_grid_left_menu_chg").show(); //행정구역 그리드용 좌측메뉴 보이기
					...
					//mng_s 20201204 이진호
					//행정단위 그리드 보기에서 주요지표 년도를 2000년으로 설정시 mainIndex_box2 hide 처리
					...
					//SGIS4_1124_행정구역 그리드 BSCA 및 표준산업분류 목록, 테마업종탭 show 시작
					//211025_수정(행정구역 그리드 BSCA 반영 X)
					
					//mng_s 20211214 운영반영 후 테스트해본 결과 412에러(정의되어있지 않은 파라미터)발생하여 주석 해제함
					//SGIS4_0107_수정 시작
					/*
					$("#gridCompanyThemaHide").hide();//전국사업체조사의 테마업종탭 하이드
					$("#gridCompanyKsscHide").hide();//전국사업체조사의 표준산업분류목록 버튼 하이드
					...
					*/
					//SGIS4_0107_수정 끝
					//mng_e 20211214 운영반영 후 테스트해본 결과 412에러발생하여 주석 해제함
					
					
					//211025_수정(행정구역 그리드 BSCA 반영 X) 끝
					//SGIS4_1124_행정구역 그리드 BSCA 및 표준산업분류 목록, 테마업종탭 show 끝
					//데이터보드쪽 하이드
					$(".dataBoardArea").hide();
					$("#dataBoardStatsSum").hide();
					...
					$("#interactiveTooltip").hide(); //왼쪽 메뉴 상단의 도움말 아이콘 하이드
					$("#bnd_grid_ingu_title").attr("title", "<p>인구총조사 항목 중 인구&middot;가구&middot;주택 각 항목으로 통계 조회가 가능합니다.</p>");
					$("#bnd_grid_population_title").attr("title", "&lt;p&gt;인구총조사 항목 중 상세조건 설정에 따른 인구통계 조회가 가능합니다.&lt;br /&gt;- 성별 : 남성/여성&lt;br /&gt;- 연령 : &lt;span style=&quot;line-height: 20.8px;&quot;&gt;조사시점 현재 실제로 태어난 사실상의 만 나이&lt;/span&gt;&lt;br /&gt;");
					$("#bnd_grid_gagu_title").attr("title", "인구총조사 항목 중 상세조건 설정에 따른 가구통계 조회가 가능합니다.</br>- 세대구성 : 일반가구에 한하여 가구주와 그 가족의 친족관계에 따라 구분");
					
					$(".btn_clockTypeSetting").hide(); //시계열 설정버튼
					//$(".btn_clockTypeSetting").show();
					$(".btn_clockTypePlay").hide(); //시계열 플레이버튼 하이드
					$(".btn_clockTypeOk").hide(); //시계열 플레이버튼 하이드
					$("#multiTimeSeries").hide(); //SGIS_4 추가
					$(".btn_legendSetting").hide(); //범례 환경설정 버튼 하이드
					...
					*/
					...
					//SGIS4_1124_농업어업총조사 추가 시작
					//가구원 기준으로 검색!!
					// SGIS4_0107 수정 시작
					/*
					$('input:checkbox[id="rt01"]').attr("checked", true);
					$("#API_0310 >div:eq(0)").find(".txt01").html("");
					$("#API_0310 >div:eq(1)").find(".txt01").html("");
					$("#rt01").html("가구원 성별(필수)");
					$("#rt01").attr("title", "가구원 성별(필수)");
					$("#rt01").hide();
					$("#3fGenderTab").find("span").html("가구원 성별(필수)");
					$("#3f_gender01").closest("div").show();
					$("#3fGenderTab").addClass("on");
					*/
					// SGIS4_0107 수정 끝
					//SGIS4_1124_농업어업총조사 추가 끝
