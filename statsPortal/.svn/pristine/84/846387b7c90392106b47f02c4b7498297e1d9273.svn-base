				...
				<div class="WriteBox">
					...
					<c:if test="${communityMapInfo.cmmnty_partcptn_grant_yn=='P' }">
						<ul>
							...
							<li>비밀번호는 함께하는지도 개설자가 설정한 비밀번호이므로,<br> 비밀번호는 개설자에게 확인하시기 바랍니다.</li>
						</ul>
					</c:if>
					<script>
						...
						$("#type-password-check-button").click(function(){
							if(!$("#${popupId } input[name=pw]").prop("readonly")){
								...
								$.ajax({
									url: contextPath+"/view/edu/"+$communityMapCommon.ss_school_level+"/community/check/password",
									data:data,
									async: true,
									type: 'POST',
									dataType: "json",
