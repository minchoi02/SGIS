...
(function(W, D) {
	...
	$communityLeftMenu.ui.poiRegist = {
		...
		popup : function(){
			if(!$("#choose-map").parent(".write_position").hasClass("disabled")){
				initAddress("popup");
				window.open(contextPath+$communityMapCommon.edu_url+"/popup/joso","pop","width=570,height=420, scrollbars=yes");
			}
		},
		/**
	...
	$communityLeftMenu.event.poi = {
		setUIEvent : function(){
			...
			$("#layer-popup-poi-regist-form").heumValidation({autoValidation:false},function(errors){
				...
				if(errors.length>0){
					...
					if($communityView.ui.curMarker==null){
						...
						var cmmnty_poi_id = $communityView.ui.curMarker.cmmnty_poi_id;
						
						url = contextPath+"/ServiceAPI/community/communityPoiModify.json";
						formData.append("cmmnty_poi_id", cmmnty_poi_id);
					...
					var params = "title:"+$(popupId+" input[name=title]").val();
					params += ",reg_lc:"+$(popupId+" input[name=reg_lc]").val();
					params += ",opinion_state:"+$(popupId+" textarea[name=opinion_state]").val();
					params += ",symbol:"+$(popupId+" input[name=symbol]:checked").val();
					params += ",loc_x:"+$communityLeftMenu.ui.curAddressCenter[0];
					params += ",loc_y:"+$communityLeftMenu.ui.curAddressCenter[1];
							...
							if(res.errCd=="0"){
								if(res.result.success){
									...
									$communityView.ui.mapMarksLists(function(){
										...
										if( cmmnty_poi_id ){
											params = "cmmnty_poi_id:"+cmmnty_poi_id + "," + params;
											srvLogWrite( "H0", "03", "03", "05", $communityMapCommon.getParameter("cmmnty_map_id"), params ); //jrj 로그
										} else {
											srvLogWrite( "H0", "03", "05", "00", $communityMapCommon.getParameter("cmmnty_map_id"), params ); //jrj 로그
										}
			...
			$("#poi-search-form").submit(function(errors){
				...
				srvLogWrite( "H0", "03", "03", "02", $communityMapCommon.getParameter("cmmnty_map_id"), $("#searchWord").val() ); //jrj 로그
							...
							ul.append(
								$("<li/>").append(
									$("<a/>",{"href":"#","data-id":node.cmmnty_poi_id,"data-x":node.x_loc,"data-y":node.y_loc}).click(function(){
										...
										srvLogWrite( "H0", "03", "03", "03", 
												$communityMapCommon.getParameter("cmmnty_map_id"), "cmmnty_poi_id:"+node.cmmnty_poi_id ); //jrj 로그
									}).append(
