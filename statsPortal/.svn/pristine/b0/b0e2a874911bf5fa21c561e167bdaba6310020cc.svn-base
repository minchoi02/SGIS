...
(function(W, D) {
	...
	$houseAnalysisMap.leftmenu = {
		...
		closeAnimate : function(id){
			...
			if(id == "look-abode"){
				$(".quickBox").stop().animate({"left":"-345px"},time);//20년수정반영
				$('div#recommendAreaFind').css('display', 'none');
				$('div#toSee').css('display', 'block');
			}else{
				$(".quickBox").stop().animate({"left":"-478px"},time);//20년수정반영
				$('div#recommendAreaFind').css('display', 'block');
				$('div#toSee').css('display', 'none');
			}
		},
		/**
		...
		setSubmitTooltipContent:function(){
			...
			if(!$("#ideal-type>a").hasClass("on")){
				if($("#look-abode-box").is(":visible")){
					...
					$('ul.thematic.nav-list.house-list').find('a').removeClass('on');
					$('ul.thematic.nav-list.house-list').find('li#look-abode').find('a').addClass('on');
				}else{
					...
					$('ul.thematic.nav-list.house-list').find('a').removeClass('on');
					$('ul.thematic.nav-list.house-list').find('li#search-recommend_le').find('a').addClass('on');
				}
		...
		getStandardAreaList: function(sido_cd, sgg_cd) {
			if(sido_cd == "00"){
				...
				$("#search-recommend-box .IndexSelect li.sub-class span.bagic > strong").html(" 중");//비교지역 중요도 표시 
				$("#search-recommend-box .IndexSelect li.sub-class a").data("asis-order","2");//비교지역 중요도 표시 
				$("#search-recommend-box .IndexSelect li.sub-class a").attr("data-asis-order","2");//비교지역 중요도 표시 
			}else{
				$.ajax({
					method: "POST",
					async: true,
