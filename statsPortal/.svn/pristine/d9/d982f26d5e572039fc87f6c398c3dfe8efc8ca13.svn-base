	...
	$resultGallery = {
			...
			insertGallery : function(){
				...
				if(AuthInfo.authStatus == true){
					...
					var linkUrl = "https://sgis.kostat.go.kr/view/gallery/resultGallery";
					messageConfirm.open(
			    			 "알림", 
			    			 shareMessage,
			    			 btns = [
			...
			if(res.errCd == "0") {
					...
					if(galleryImgListItem[0]){
						...
						if(paramInfo.refFileList){
							...
							for(var i = 0; i < paramInfo.refFileList.length; i ++) {
								...
								fileListHtml += '<a onclick="srvLogWrite(\'J0\',\'04\',\'10\',\'00\',\'\',\'\');" href="/upload/gallery/refFile/' + paramInfo.refFileList[i].saveName + '" class="t02">'+paramInfo.refFileList[i].fileName+'</a>';
							}
				...
				if(galleryPollDetailListItem.length > 0 ){
					...
					if (galleryPollDetailListItem[0].survey_surv_end_dt != undefined) {
						...
						if(btDay >= -1){
							if(totalVoteYn){
								galleryPollDetailHtml += '<a href="javascript:srvLogWrite(\'J0\',\'04\',\'05\',\'00\',\'\',\'\'); $resultGallery.clickVoteInfo(\'R\')" class="btnVote">다시 투표하기</a>';
							}else{
								galleryPollDetailHtml += '<a href="javascript:srvLogWrite(\'J0\',\'04\',\'05\',\'00\',\'\',\'\'); $resultGallery.clickVoteInfo(\'\')" class="btnVote">투표하기</a>';
							}
						}else{
