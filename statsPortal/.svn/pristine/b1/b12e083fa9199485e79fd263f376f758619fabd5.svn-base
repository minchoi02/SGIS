...
function startTutorial() {
	...
	$(window).resize(function() {
		...
		if( $houseAnalysisMap.ui.tutoIndex == 5 ){
			$("#bt_06").css("top", $("#tuto06").offset().top);
		}
		if( $houseAnalysisMap.ui.tutoIndex == 17 ){
			$("#bt_18").css("top", $("#tuto18").offset().top);
		}
...
/*	apiLogWrite2("A0", "A28", "대화형통계지도 튜토리얼", "(NULL)", "(NULL)", "(NULL)");*/
	...
	function pointInfo(i, clicked) {
		...
		case 1:
			$(pointId).animate({
				top : posi.top + (height * 2)+70,
				left : posi.left + width
			}, 500, "", function() {
				$(this).animate({
					top : posi.top + (height * 2) + 80,
					left : posi.left + width
				}, 500, "", function() {
					pointInfo(i, false);
				});
	...
	$("#bt_03").click(function() {
		if($(window).height()>=1080||$(window).width()>=1920){
			...
			//mng_s 20200424 이진호, 튜토리얼 현행화/ 위치 수정
			//$("#bt_04").css("top","464px");
			$("#bt_04").css("top","435px");
			//mng_e 20200424 이진호
	...
	$("#bt_10").click(function() {
		...
		$("#tutorialText").append("<div class=\"title\">"
				...
				+ "<strong style=\"color:#ee7c1a;  font-weight:bold;\">‘통계메뉴’</strong> 버튼을 클릭해 주세요.</p></div>");
		posiInfo(10);
	});
	$("#bt_11").click(function() {
		...
		if(btChk == 0 ){
			...
			$("#bt_12").show();
			$("#bt_12").css('top','238px');
			$("input:checkbox[name='stand_area_check']").prop("checked",true);
			...
			$("#tutorialText").append("<div class=\"title\">"
					...
					+ "</div>"
					+ "<div class=\"content\">"
					+ "<p><strong style=\"color:#ee7c1a;  font-weight:bold;\">‘살고싶은지역’</strong>에서 이사 가고자 하는 지역을 선택할 수 있습니다.</p>" 
					+ "<p><strong style=\"color:#ee7c1a;  font-weight:bold;\">‘비교지역’</strong>은 살고싶은지역과 비교하여 통계를 살펴볼 지역으로, "
					+ "현재 살고 있는 지역으로 설정하거나 <strong style=\"color:#ee7c1a;  font-weight:bold;\">'전국'</strong>으로 설정하시면 됩니다.<br>"
					+ "여기서는 비교지역을 <strong style=\"color:#ee7c1a;  font-weight:bold;\">'전국'</strong> 으로 두고 살고싶은지역을 "
...
///////////////////////////////////// 추천지역찾기 루트 1 시작
	...
	$("#bt_13").click(function() {
		$("input:checkbox[name='stand_area_check']").prop("checked",false);
	...
	$("#bt_20").click(function() {
		...
		$("#tutorialText").append("<div class=\"title\">"
				...
				+ "</div>"
				+ "<div class=\"content\">"
				+ "<p>상단의 방사형 그래프에서는 살고싶은지역의 7개 각 카테고리별 점수를 비교지역(전국)과 비교하여 지역의 특성을 파악할 수 있습니다.<br>" 
	...
	$("#bt_25").click(function() {
		...
		$("#tutorialText").append("<div class=\"title\">"
				...
				+ "<strong style=\"color:#ee7c1a;  font-weight:bold;\">‘통계메뉴’</strong>를 선택해 주세요.</p></div>");
		posiInfo(25);
	});
...
///////////////////////////////////// 추천지역찾기 루트 2 끝	
	...
	$("#bt_28")	.click(function() {
		$("#search-recommend_le>a").removeClass("on");
