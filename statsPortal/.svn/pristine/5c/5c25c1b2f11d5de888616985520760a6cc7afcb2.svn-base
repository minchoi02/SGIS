...
(function(W, D) {
	...
	$ssaJobQualityDetailPopup.ui = {
		...
		SearchPopupData : function(){
			...
			srvLogWrite('D0', '05', '03', '03', '', slwParams);
						...
						$('#ssaJobQualityDetailChart').highcharts({
							...
							plotOptions: {								
								series: {
									...
									//states: { },
									//2020-04-20 [곽제욱] 저임근 근로자 비율 Map에 표출하기 위한 변경 START
						            cursor: (dataParams.Link_id == "I3306" || dataParams.Link_id == "I3306_1" || dataParams.Link_id == "E3303") ? 'pointer' : "",
						            //2020-04-20 [곽제욱] 저임근 근로자 비율 Map에 표출하기 위한 변경 END
						            point: {
						                events: {						                	
						                    click: function (e) {						                    	
						                    	...
						                    	//2020-04-20 [곽제욱] 저임근 근로자 비율 Map에 표출하기 위한 변경 START
						                    	if(dataParams.Link_id == "I3306" || dataParams.Link_id == "I3306_1" || dataParams.Link_id == "E3303") {
		                    					//2020-04-20 [곽제욱] 저임근 근로자 비율 Map에 표출하기 위한 변경 END
						                    		$ssaJobQualityDetailPopup.ui.selectDataBoardMap(this.category);
						                    		
													//0190311 손원웅 추가_지도 연도 세팅
												...
												if($ssaJobQuality.ui.SearchParam.Link_id == "I3306_1"){
													lvItem = "I3306";
												}else if($ssaJobQuality.ui.SearchParam.Link_id == "I3306"){
													lvItem = "I3305";
												}else{
													lvItem = $ssaJobQuality.ui.SearchParam.Link_id;
												}
...
//								console.log("SearchPopupMap - DataInfo data : " + JSON.stringify(data));
								...
								// 2019.08.22 이금은 this_Mon 으로 변경
								var this_Mon = $workRoad.util.dateWithSign($ssaJobQualityDetailPopup.ui.CurrentDate[0].max_prd_de, "."); //max데이터 월 또는 현재 월
								var title    = data.stat_nm + " (" + this_Mon + ")";
								$("#ssaDataBoard #ssaStatPath").html(data.stat_path);
								$("#ssaDataBoard #ssaTitle").html(title);					// 제목
								$("#ssaDataBoard #ssaOrigin").html(data.colct_source);		// 출처
						...
						//2020-04-20 [곽제욱] 저임근 근로자 비율, 평균임금총액, 총 근로시간 Map에 표출하기 위한 변경 START 
						if(dataParams.Link_id == "I3306" || dataParams.Link_id == "I3306_1" || dataParams.Link_id == "E3303" 
							|| dataParams.Link_id == "E3301" || dataParams.Link_id == "E3307"){
							//2020-04-20 [곽제욱] 저임근 근로자 비율, 평균임금총액, 총 근로시간 Map에 표출하기 위한 변경 END
//							20190311 손원웅 추가_지도 연도 세팅
							$ssaMap.ui.getTodayStatusMapData("","","",This_Mon);	//지도 연도 세팅
							$ssaMap.ui.ssaJobQualityOpenApiBoundarySido("2016");	//최신년도 데이터가 2016...
						//2020-04-20 [곽제욱] 저임근 근로자 비율, 평균임금총액, 총 근로시간 Map에 표출하기 위한 변경 START 
						} else if(dataParams.Link_id == "E3309" || dataParams.Link_id == "I3308"){
						//2020-04-20 [곽제욱] 저임근 근로자 비율, 평균임금총액, 총 근로시간 Map에 표출하기 위한 변경 END
...
//								Tmpdata2 = Tmpdata;
						...
						//2020-04-20 [곽제욱] 저임금 근로자 비율, 평귬임금총액, 총근로시간 데이터보드 변경 START
						if(dataParams.Link_id == "I3306" || dataParams.Link_id == "I3306_1" || dataParams.Link_id == "E3303" 
							|| dataParams.Link_id == "E3301" || dataParams.Link_id == "E3307"){
						//2020-04-20 [곽제욱] 저임금 근로자 비율, 평귬임금총액, 총근로시간 데이터보드 변경 END
