...
//				$("#fullDataBoardTitle").find("#dataBoardArea").hide();
				$("#fullDataBoardTitle").find(".dataBoardArea").hide();
//				$("#fullDataBoardTitle").find("#dataBoardOrigin").html("출처 : KOSIS(<a href='javascript:todayStatusMapEtc.kosisLinked();'>통계표보기</a>)");
				$("#fullDataBoardTitle").find(".dataBoardOrigin").html("출처 : KOSIS(<a href='javascript:todayStatusMapEtc.kosisLinked();'>통계표보기</a>)");
//				$("#dataBoardStats").html("출처 : KOSIS(<a href='javascript:todayStatusMapEtc.kosisLinked();'>통계표보기</a>)");
				$(".dataBoardStats").html("출처 : KOSIS(<a href='javascript:todayStatusMapEtc.kosisLinked();'>통계표보기</a>)");
			},
			 ...
			 * @history 	 :
			 */
			updateTargetAreaChart : function(target_mon) {
				//데이터보드 차트 카테고리 세팅
				...
				for(var i=0; i<$ssaDetailPopup.ui.DetailData.length; i++){
					if(target_mon == $ssaDetailPopup.ui.DetailData[i].prd_de && "00" != $ssaDetailPopup.ui.DetailData[i].c1){ //20181112 계 삭제
						$ssaDetailPopup.ui.DB_Chart_Category.push($ssaDetailPopup.ui.DetailData[i].c1_nm);

						var TmpCategory = {};
						TmpCategory.c1_nm = $ssaDetailPopup.ui.DetailData[i].c1_nm;
						TmpCategory.c1 = $ssaDetailPopup.ui.DetailData[i].c1;
						TmpCategory.dt = $ssaDetailPopup.ui.DetailData[i].dt;
						
						$ssaDetailPopup.ui.DB_Chart_Category_CD[Chk_Category_Cnt2++] = TmpCategory;
						
						var TmpResult = {};
						TmpResult.adm_cd = $ssaDetailPopup.ui.DetailData[i].c1;
						TmpResult.adm_nm = $ssaDetailPopup.ui.DetailData[i].c1_nm;
						
						Chk_Category[Chk_Category_Cnt++] = TmpResult;
					}
				}
				
				var that = Array.prototype.slice.call($ssaDetailPopup.ui.DataBoardChart);
				var that_Category = $ssaDetailPopup.ui.DB_Chart_Category;
				var that_Category_CD = $ssaDetailPopup.ui.DB_Chart_Category_CD;
				var Chart_series = [];
				
				for(var i=0; i<that.length; i++){
					if(that[i].name == target_mon){
						Chart_series.push(that[i]);
					}
				}
				
				//Y축 Label
				var detailDataList = $ssaDetailPopup.ui.DetailData;
				$ssaDataBoard.ui.yLabel = "";
				for(var i = 0; i < detailDataList.length; i ++) {
					if(detailDataList[i].unit_nm != undefined && detailDataList[i].unit_nm != null) {
						$ssaDataBoard.ui.yLabel = detailDataList[i].unit_nm;
						break;
					}
				}
				...
				if(Chart_series.length > 0 && Chart_series[0].data != undefined && Chart_series[0].data != null && Chart_series[0].data.length > 0) {
					for(var i = 0; i < Chart_series[0].data.length; i ++) {
						var title = that_Category[i];
						//20181210 제주도만 지역명이 달라서 조건 추가_손원웅
						if(title == "제주도"){
							title = "제주특별자치도";
						}
						
						dataList.push({ item : title, itemCd : that_Category_CD[i].c1, value : Chart_series[0].data[i]});
					}
				}
				
				//정렬
				var tempList = $ssaDataBoard.Util.tableDataSortValue(dataList);
				//_dataIdx 추가
				for(var i=0; i<tempList.length; i++){
					tempList.splice(i,1,{item : tempList[i].item, itemCd : tempList[i].itemCd, value : tempList[i].value, _dataIdx : (i)});
				}
				
				$ssaDataBoard.ui.tempIdxList = tempList;
				
				var tmpList = [];
				var last_categories = [];
				for(var i=0; i<tempList.length; i++){
					tmpList.push(tempList[i].value);
					last_categories.push(tempList[i].item);
				}
				
				var last_chart_series = [{
						data : tmpList,
						name : (Chart_series.length > 0) ? Chart_series[0].name : ""
				}];
				
				
				$('#ssaDataBoard #ssaJobStatusDataBoardChart').highcharts({
					chart: {
...
//					    height: '530'
					...
					colors: ['#2951f2'/*,'#ff0000', '#f79339', '#ffc000', '#92d050', '#00b0f0', '#0000FF', '#7030a0'
				               , '#880015', '#B97A57', '#FFAEC9', '#FFC90E', '#B5E61D', '#99D9EA', '#7092BE'*/],
					tooltip: {
						valueDecimals: 2,
						formatter: function () {
			        		return 	'<span>' + this.point.category + '</span><br/>' +
		        					'<span style="color:' + this.series.color + '">' + this.series.name + '</span>: <b style="font-family:NanumBarunGothic;">' + $workRoad.util.addComma(this.point.y) + '</b>';
		                }
			        },
					title: { text: '' },
					subtitle: { text: '' },
					...
					xAxis: {
						categories: last_categories//$ssaDetailPopup.ui.DB_Chart_Category,
						//title: { text: '갱신주기' }
					},
					yAxis: {
						min: 0, 
						...
						labels: { 
							overflow: 'justify',
							formatter: function () {
								var lvYLabel = $ssaDataBoard.ui.yLabel;
								if(lvYLabel != "") return $workRoad.util.addComma(this.value,lvYLabel);
								else return $workRoad.util.addComma(this.value);
				            }
						}
					}, 
					plotOptions: {								
											...
											map.dataGeojson.eachLayer(function(layer) {
												if (layer.feature) {
													...
													if (layer.feature.dataIdx == idx-1) {
														...
														$ssaMap.callbackFunc.didMouseOverPolygon(e, layer.feature, layer.options.type, map);
													}
												}
											});
										}
											...
											map.dataGeojson.eachLayer(function(layer) {
												if (layer.feature) {
													...
													if (tmpTitle == title) {
														...
														for(var i=0; i<tempList.length; i++){
															if(title == tempList[i].item){
																layer.feature.dataIdx = tempList[i]._dataIdx;
															}
														}
...
//				        verticalAlign: 'middle'
					...
					series: last_chart_series//Chart_series,
				});
				
				//그래프 라벨선택 시, 해당 레이어 mouseover
				//9월 서비스
				$("#ssaDataBoard #targetCharts").find('.highcharts-axis-labels text').click(function(){
					//집계구
				...
				if(res.result != undefined) {
					for(var i = 0; i < res.result.length; i ++) {
						...
						//조건결합시
							...
							retDataList.push(parseFloat(elem.right_data_val));	//구인현황추가
						}
					}
				}
				...
				//해당지역 데이터 보기 차트 (일반)
				$('#ssaJobStatusDataBoardChart').highcharts(
				{
					chart : { 
						type : 'column',height: height, width:500
					},
					exporting: { enabled: false },
					title : {
						text : ''
					...
					xAxis : {
						categories: $ssaDetailPopup.ui.DB_Chart_Category,
						labels : { 
							rotation : 0,
					...
					series : $ssaDetailPopup.ui.DataBoardChart[ {
						name : titleText,
						data : retDataList,
						color : '#2951f2'
					} ]
...
*/
			...
			/**
			 * 
			 * @name         : updateTargetAreaJobGrowthChart
			 * @description  : 해당 지역 데이터 보기 차트
			 * @date         : 2015. 10. 28. 
			 * @author	     : 김성현
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaJobGrowthChart : function(target_mon) {
				var that = $ssaJobGrowthDetailPopup.ui.DataBoardChart;
				var Chart_series = [];
				...
				for(var i=0; i<that.length; i++){
					if(that[i].name == target_mon){
						...
						Chart_series.push(tempThat);
					}
				}
				
				// 정렬
				var detailDataList = $ssaJobGrowthDetailPopup.ui.DetailData;
				var category = $ssaJobGrowthDetailPopup.ui.DB_Chart_Category;
				var data = Chart_series[0].data;
				var tempList = [];
				/*{
					c1: "00"
					c1_nm: "전국"
				...
				for(var i = 0; i < detailDataList.length; i++) {
					if(target_mon == detailDataList[i].prd_de) {
						try {
							var tempMap = {
								category : (detailDataList[i].c1_nm == "제주도") ? "제주특별자치도" : detailDataList[i].c1_nm
								,data : parseFloat((detailDataList[i].dt == "-") ? 0 : detailDataList[i].dt)
								,item : (detailDataList[i].c1_nm == "제주도") ? "제주특별자치도" : detailDataList[i].c1_nm
								,itemCd : detailDataList[i].c1
								,value : parseFloat((detailDataList[i].dt == "-") ? 0 : detailDataList[i].dt)
							};
							if(tempMap.category != "전국" && tempMap.category != "계" && tempMap.category != "기타") tempList.push(tempMap);
						} catch(e) { }
					}
				}
				category = [];
				data = [];
				tempList.sort(function(a, b) { // 내림차순
					return a.data > b.data ? -1 : a.data < b.data ? 1 : 0;
				});
				//_dataIdx 추가
				for(var i=0; i<tempList.length; i++){
					tempList.splice(i,1,{item : tempList[i].item, itemCd : tempList[i].itemCd, value : tempList[i].value, category : tempList[i].category, data : tempList[i].data, _dataIdx : (i)});
				}
				$ssaDataBoard.ui.tempIdxList = tempList;
				for(var i = 0; i < tempList.length; i++) {
					var tempCategory = tempList[i].category;
					var tempData = tempList[i].data;
					category.push(tempCategory);
					data.push(tempData);
				}
				Chart_series[0].data = data;
...
//					    height: '530'
					...
					colors: ['#2951f2'/*, '#f79339', '#ffc000', '#92d050', '#00b0f0', '#0000FF', '#7030a0'
				               , '#880015', '#B97A57', '#FFAEC9', '#FFC90E', '#B5E61D', '#99D9EA', '#7092BE'*/],
	               tooltip: {
						valueDecimals: 2,
						formatter: function () {
...
//				        verticalAlign: 'middle'
			...
			/**
			 * 
			 * @name         : updateTargetAreaJobQualityChart
			 * @description  : 해당 지역 데이터 보기 차트
			 * @date         : 2015. 10. 28. 
			 * @author	     : 김성현
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaJobQualityChart : function(target_mon) {
				var that = $ssaJobQualityDetailPopup.ui.DataBoardChart;
				var Chart_series = [];
				...
				// 정렬
				var detailDataList = $ssaJobQualityDetailPopup.ui.DetailData;
				var category = $ssaJobQualityDetailPopup.ui.DB_Chart_Category;
				var data = Chart_series[0].data;
				var tempList = [];
				/*
					c1: "00"
...
//				        verticalAlign: 'middle'
			...
			/**
			 * 
			 * @name         : updateTargetAreaJobQualityChartInvert
			 * @description  : 해당 지역 데이터 보기 차트
			 * @date         : 2015. 10. 28. 
			 * @author	     : 김성현
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaJobQualityChartInvert : function(target_mon) {
				var PC_Series = [];
				var category = $ssaJobQualityDetailPopup.ui.DataBoardCategory;
				var PushNm = $ssaJobQualityDetailPopup.ui.DB_Chart_Category;
				var data = $ssaJobQualityDetailPopup.ui.DataBoardChart;
				...
				for(var i = 0; i < category.length; i++) {
					if(data.length > i) {
						var lvCategory = category[i];
						var lvData = data[i];
						if(lvData != undefined && lvData != null && lvData != "0" && lvData != "-") {
							temp_flag = true;
						}
						if(temp_flag == true) {
							temp_category.push(lvCategory);
				...
				category = temp_category.reverse();
				...
				$ssaDataBoard.ui.tempIdxList = tempList;
				for(var i = 0; i < tempList.length; i++) {
					category.push(tempList[i].category);
					data.push(tempList[i].data);
				}
				
				PC_Series.push({
					name: PushNm,
					//data: [0,0,0,0,0,0,5.7,5.7,5.8,5.9,6,0],
					data: data,
					tooltip: {
					   valueSuffix: ''
					}
...
//				        verticalAlign: 'middle'
			...
			/**
			 * 
			 * @name         : updateTargetAreaEconomicSituationChart
			 * @description  : 해당 지역 데이터 보기 차트
			 * @date         : 2015. 10. 28. 
			 * @author	     : 김성현
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaEconomicSituationChart : function(target_mon) {
				var that = $ssaEconomicSituationDetailPopup.ui.DataBoardChart;
				var Chart_series = [];
				...
				// 정렬
				var detailDataList = $ssaEconomicSituationDetailPopup.ui.DetailData;
				var category = $ssaEconomicSituationDetailPopup.ui.DB_Chart_Category;
				var data = Chart_series[0].data;
				var tempList = [];
				/*
					c1: "25"
...
//				        verticalAlign: 'middle'
			...
			/**
			 * 
			 * @name         : updateTargetAreaEconomicSituationChartInvert
			 * @description  : 해당 지역 데이터 보기 차트
			 * @date         : 2015. 10. 28. 
			 * @author	     : 김성현
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaEconomicSituationChartInvert : function(target_mon) {
				var PC_Series = [];
				var category = $ssaEconomicSituationDetailPopup.ui.DataBoardCategory;
				var PushNm = $ssaEconomicSituationDetailPopup.ui.DB_Chart_Category;
				var data = $ssaEconomicSituationDetailPopup.ui.DataBoardChart;
...
//				        verticalAlign: 'middle'
			...
			/**
			 * 
			 * @name         : updateTargetAreaLifeQualityChart
			 * @description  : 해당 지역 데이터 보기 차트
			 * @date         : 2015. 10. 28. 
			 * @author	     : 김성현
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaLifeQualityChart : function(target_mon) {
				var PC_Series = [];
				var category = $ssaLifeQualityDetailPopup.ui.DataBoardCategory;
				var PushNm = $ssaLifeQualityDetailPopup.ui.DB_Chart_Category;
				var data = $ssaLifeQualityDetailPopup.ui.DataBoardChart;
...
//						20190319 손원웅 추가
				...
				//Y축 Label
				var detailDataList = $ssaLifeQualityDetailPopup.ui.DetailData;
				$ssaDataBoard.ui.yLabel = "";
				for(var i = 0; i < detailDataList.length; i ++) {
					if(detailDataList[i].unit_nm != undefined && detailDataList[i].unit_nm != null) {
						$ssaDataBoard.ui.yLabel = detailDataList[i].unit_nm;
				            ...
				            point: {
				                events: {						                	
				                    click: function (e) {						                    	
				                    	...
				                    	//$$$ssaLifeQualityDetailPopup.ui.selectDataBoardMap(this.category);								
				                    }
				                }
				            },					            				                    
				            marker: {
...
//				        verticalAlign: 'middle'
			...
			/** 2020-04-27 [곽제욱] 일자리통계분석 - 삶의질 - 데이터보드(차트) 시도별 표출 START */
			/**
			 * 
			 * @name         : updateTargetAreaLifeQualityChartInvert
			 * @description  : 해당 지역 데이터 보기 차트(시도별)
			 * @date         : 2020. 04. 24. 
			 * @author	     : 곽제욱
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaLifeQualityChartInvert : function(target_mon) {
				var that = $ssaLifeQualityDetailPopup.ui.DataBoardChart;
				var Chart_series = [];
				...
				// 정렬
				var detailDataList = $ssaLifeQualityDetailPopup.ui.DetailData;
				var category = $ssaLifeQualityDetailPopup.ui.DB_Chart_Category;
				var data = Chart_series[0].data;
				var tempList = [];
				for(var i = 0; i < detailDataList.length; i++) {
					if(target_mon == detailDataList[i].prd_de) {
...
//				        verticalAlign: 'middle'
			...
			/** 2020-04-27 [곽제욱] 일자리통계분석 - 삶의질 - 데이터보드(차트) 시도별 표출 END */
			 ...
			 * @history 	 :
			 */
			updateTargetAreaTable : function(target_mon) {
				$("#barChartTable").show();
				if($ssaDetailPopup.ui.DataBoardChart.length > 0){
					var TableData = $ssaDetailPopup.ui.DataBoardChart;	//차트 & 테이블 데이터
					var TableCategory = $ssaDetailPopup.ui.DB_Chart_Category;	//카테고리 (지역)
					var Table_Val = "";
					
					//2019-01-23 (125~128) 데이터 없는거 차트에서 제거.
					var arry_Category = [
						$ssaDetailPopup.ui.Category[0].mon0,
						$ssaDetailPopup.ui.Category[0].mon1,
						$ssaDetailPopup.ui.Category[0].mon2,
						$ssaDetailPopup.ui.Category[0].mon3,
						$ssaDetailPopup.ui.Category[0].mon4,
						$ssaDetailPopup.ui.Category[0].mon5,
						$ssaDetailPopup.ui.Category[0].mon6,
						$ssaDetailPopup.ui.Category[0].mon7,
						$ssaDetailPopup.ui.Category[0].mon8,
						$ssaDetailPopup.ui.Category[0].mon9,
						$ssaDetailPopup.ui.Category[0].mon10,
						$ssaDetailPopup.ui.Category[0].mon11
					];
					
					var html = "";
					if($ssaJobStatus.ui.SearchParam.Link_id == "I3116") {
						html += "<tr>";
						html += "	<th scope='col' style='width:13%;'>항목</th>";
						
						//2019-01-23 (125~128) 데이터 없는거 차트에서 제거.
						for(var i = 0; i < arry_Category.length; i++) {
							for(var j = 0; j < TableData.length; j++) {
								if(arry_Category[i].replace(/년 /g,"0").replace(/분기/g,"") == TableData[j].name) {
									html += "	<th scope='col'>"+arry_Category[i]+"</th>";
								}
							}
						}
//						html += "	<th scope='col'>"+$ssaDetailPopup.ui.Category[0].mon0+"</th>";
//						html += "	<th scope='col'>"+$ssaDetailPopup.ui.Category[0].mon1+"</th>";
//						html += "	<th scope='col'>"+$ssaDetailPopup.ui.Category[0].mon2+"</th>";
//						html += "	<th scope='col'>"+$ssaDetailPopup.ui.Category[0].mon3+"</th>";
//						html += "	<th scope='col'>"+$ssaDetailPopup.ui.Category[0].mon4+"</th>";
//						html += "	<th scope='col'>"+$ssaDetailPopup.ui.Category[0].mon5+"</th>";
//						html += "	<th scope='col'>"+$ssaDetailPopup.ui.Category[0].mon6+"</th>";
//						html += "	<th scope='col'>"+$ssaDetailPopup.ui.Category[0].mon7+"</th>";
//						html += "	<th scope='col'>"+$ssaDetailPopup.ui.Category[0].mon8+"</th>";
//						html += "	<th scope='col'>"+$ssaDetailPopup.ui.Category[0].mon9+"</th>";
//						html += "	<th scope='col'>"+$ssaDetailPopup.ui.Category[0].mon10+"</th>";
//						html += "	<th scope='col'>"+$ssaDetailPopup.ui.Category[0].mon11+"</th>";
						html += "</tr>";
					} else {
						/*html += "<tr>";
						html += "	<th scope='col' style='width:13%;'>항목</th>";
						if(TableData.length > 0) html += "	<th scope='col'>"+TableData[0].name+"</th>";
						if(TableData.length > 1) html += "	<th scope='col'>"+TableData[1].name+"</th>";
						if(TableData.length > 2) html += "	<th scope='col'>"+TableData[2].name+"</th>";
						if(TableData.length > 3) html += "	<th scope='col'>"+TableData[3].name+"</th>";
						if(TableData.length > 4) html += "	<th scope='col'>"+TableData[4].name+"</th>";
						if(TableData.length > 5) html += "	<th scope='col'>"+TableData[5].name+"</th>";
						if(TableData.length > 6) html += "	<th scope='col'>"+TableData[6].name+"</th>";
						if(TableData.length > 7) html += "	<th scope='col'>"+TableData[7].name+"</th>";
						if(TableData.length > 8) html += "	<th scope='col'>"+TableData[8].name+"</th>";
						if(TableData.length > 9) html += "	<th scope='col'>"+TableData[9].name+"</th>";
						if(TableData.length > 10) html += "	<th scope='col'>"+TableData[10].name+"</th>";
						if(TableData.length > 11) html += "	<th scope='col'>"+TableData[11].name+"</th>";
						html += "</tr>";*/
					...
					for(var i = 0; i < TableData.length; i++) {
						var Table_name = TableData[i].name;
						if(Table_name == target_mon) {
							var Table_Data = TableData[i].data;
							var Table_Category = TableData[i].category;
							var tempList = [];
							for(var j = 0; j < Table_Category.length; j++) {
								try {
									var tempMap = {
										 category : Table_Category[j]
										,data : Table_Data[j]
									};
									if(tempMap.category != "전국" && tempMap.category != "계" && tempMap.category != "기타") tempList.push(tempMap);
								} catch(e) { }
							...
							tempList.sort(function(a, b) { // 내림차순
								if(typeof a.data == "object") {
									return a.data.y > b.data.y ? -1 : a.data.y < b.data.y ? 1 : 0;
								}
								else {
									return a.data > b.data ? -1 : a.data < b.data ? 1 : 0;
								}
							...
							for(var i = 0; i < tempList.length; i++) {
								var tempCategory = tempList[i].category;
								TableCategory.push(tempCategory);
							}
							break;
						}
					}
					
					for(var  i = 0; i < TableCategory.length; i ++) {
						
						html += "<tr>";
						html += "	<td class='al' style='text-align:center;background: #f1f1f1;'>"+((TableCategory[i] == "제주도") ? "제주특별자치도" : TableCategory[i])+"</td>";
						
						for(var  j = 0; j < TableData.length; j++) {
							var Table_Data = [];
							var Table_Data_Idx = i;
							var Table_Category = TableData[j].category;
							Table_Data.push(TableData[j].data);
							var Table_name = TableData[j].name;
							
							for(var k = 0; k < Table_Category.length; k++) {
								if(TableCategory[i] == Table_Category[k]) {
									Table_Data_Idx = k;
									break;
								}
							}
							
							if(typeof Table_Data[0][Table_Data_Idx] == "object"){
								Table_Val = Table_Data[0][Table_Data_Idx].y;
							}else{
								Table_Val = Table_Data[0][Table_Data_Idx];
							}
							
							if(target_mon == Table_name){
								html += "	<td style='text-align:center;background: #f1f1f1;'>"+((Table_Val != undefined) ? Table_Val : 0)+"</td>";
							}else{
								html += "	<td style='text-align:center;'>"+((Table_Val != undefined) ? Table_Val : 0)+"</td>";
							}
						}
						
						//2019-01-23 (125~128) 데이터 없는거 차트에서 제거.
						/*if(TableData.length < 12) {
							for(var k = TableData.length; k < 12; k++) {
			...
			/**
			 * 
			 * @name         : updateTargetAreaJobGrowthTable
			 * @description  : 해당 지역 데이터 보기 표 업데이트
			 * @date         : 2015. 10. 29. 
			 * @author	     : 김성현
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaJobGrowthTable : function(target_mon) {
				if($ssaJobGrowthDetailPopup.ui.DataBoardChart.length > 0){
					var TableData = $ssaJobGrowthDetailPopup.ui.DataBoardChart	//차트 & 테이블 데이터
					var TableCategory = $ssaJobGrowthDetailPopup.ui.DB_Chart_Category	//카테고리 (지역)
					...
					for(var i = 0; i < TableData.length; i++) {
						var Table_Data = TableData[i].data;
						var Table_Data_Flag = false;
						for(var j = 0; j < Table_Data.length; j++) {
							if((isNaN(Table_Data[j]) ? 0 : Table_Data[j]) != 0) {
								Table_Data_Flag = true;
								break;
							}
						}
						if(Table_Data_Flag == true) {
							html += "	<th scope='col'>"+TableData[i].name+"</th>";
						}
					}
					
//					html += "	<th scope='col'>"+TableData[0].name+"</th>";
//					html += "	<th scope='col'>"+TableData[1].name+"</th>";
//					html += "	<th scope='col'>"+TableData[2].name+"</th>";
//					html += "	<th scope='col'>"+TableData[3].name+"</th>";
//					html += "	<th scope='col'>"+TableData[4].name+"</th>";
//					html += "	<th scope='col'>"+TableData[5].name+"</th>";
//					html += "	<th scope='col'>"+TableData[6].name+"</th>";
//					html += "	<th scope='col'>"+TableData[7].name+"</th>";
//					html += "	<th scope='col'>"+TableData[8].name+"</th>";
//					html += "	<th scope='col'>"+TableData[9].name+"</th>";
//					html += "	<th scope='col'>"+TableData[10].name+"</th>";
//					html += "	<th scope='col'>"+TableData[11].name+"</th>";
					html += "</tr>";
					...
					for(var  i = 0; i < TableCategory.length; i ++) {
						...
						for(var  j = 0; j < TableData.length; j++) {
							...
							//2019-01-23 (125~128) 데이터 없는거 차트에서 제거.
							var Table_Data = TableData[j].data;
							var Table_Data_Flag = false;
							for(var k = 0; k < Table_Data.length; k++) {
								if((isNaN(Table_Data[k]) ? 0 : Table_Data[k]) != 0) {
									Table_Data_Flag = true;
									break;
								}
							}
							if(Table_Data_Flag == false) {
								continue;
							}
							...
							//isNaN인 경우 0으로 변경
							if(target_mon == Table_name){
								html += "	<td style='text-align:center;background: #f1f1f1;'>"+(isNaN(Table_Data[Table_Data_Idx]) ? 0 : Table_Data[Table_Data_Idx])+"</td>";
							}else{
								html += "	<td style='text-align:center;'>"+(isNaN(Table_Data[Table_Data_Idx]) ? 0 : Table_Data[Table_Data_Idx])+"</td>";
							}
						}
						html += "</tr>";
					}
			...
			/**
			 * 
			 * @name         : updateTargetAreaJobQualityTable
			 * @description  : 해당 지역 데이터 보기 표 업데이트
			 * @date         : 2015. 10. 29. 
			 * @author	     : 김성현
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaJobQualityTable : function(target_mon) {
				if($ssaJobQualityDetailPopup.ui.DataBoardChart.length > 0){
					var TableData = $ssaJobQualityDetailPopup.ui.DataBoardChart	//차트 & 테이블 데이터
					var TableCategory = $ssaJobQualityDetailPopup.ui.DB_Chart_Category	//카테고리 (지역)
...
//					html += "	<th scope='col'>"+TableData[11].name+"</th>";
					...
					for(var  i = 0; i < TableCategory.length; i ++) {
						...
						for(var  j = 0; j < TableData.length; j++) {
							...
							//isNaN인 경우 0으로 변경
							if(target_mon == Table_name){
								html += "	<td style='text-align:center;background: #f1f1f1;'>"+(isNaN(Table_Data[Table_Data_Idx]) ? 0 : $workRoad.util.addComma(Table_Data[Table_Data_Idx]))+"</td>";
							}else{
								html += "	<td style='text-align:center;'>"+(isNaN(Table_Data[Table_Data_Idx]) ? 0 : $workRoad.util.addComma(Table_Data[Table_Data_Idx]))+"</td>";
							}
						}
						html += "</tr>";
					}
			...
			/**
			 * 
			 * @name         : updateTargetAreaJobQualityTableInvert
			 * @description  : 해당 지역 데이터 보기 표 업데이트
			 * @date         : 2015. 10. 29. 
			 * @author	     : 김성현
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaJobQualityTableInvert : function(target_mon) {
				if($ssaJobQualityDetailPopup.ui.DataBoardChart.length > 0){
					var TableData = $ssaJobQualityDetailPopup.ui.DataBoardTable;	//차트 & 테이블 데이터
					var TableCategory = $ssaJobQualityDetailPopup.ui.DataBoardCategory;	//카테고리 (지역)
					var TableName = $ssaJobQualityDetailPopup.ui.DB_Chart_Category;
					...
					for(var  i = 0; i < TableCategory.length; i ++) {
						...
						for(var  j = 0; j < TableData.length; j++) {
							...
							if(target_mon == Table_name){
								html += "	<td style='text-align:center;background: #f1f1f1;'>"+Table_Data[i]+"</td>";
							}else{
								html += "	<td style='text-align:center;'>"+Table_Data[i]+"</td>";
							}
						}
						html += "</tr>";
					}
					...
					html += "<tr>";
					html += "	<th scope='col' style='width:13%;'>항목</th>";
					html += "	<th scope='col'>"+TableName+"</th>";
					html += "</tr>";
					
					// 정렬
					//2019-01-23 (125~128) 데이터 없는거 차트에서 제거.
					var fromIdx = 0;
					var toIdx = TableData.length;
					for(var i = 0; i < TableData.length; i++) {
						var lvData = TableData[i].data;
						if(lvData != undefined && lvData != null && lvData != "0" && lvData != "-") {
							break;
						}
						else {
					...
					tempList.sort(function(a, b) { // 내림차순
						try {
							return parseFloat(a.data) > parseFloat(b.data) ? -1 : parseFloat(a.data) < parseFloat(b.data) ? 1 : 0;
						} catch(e) {
							return a.data > b.data ? -1 : a.data < b.data ? 1 : 0;
						}
					});
					...
					for(var  i = 0; i < tempList.length; i ++) {
//						20190318 손원웅 수정
						if(tempList[i].data != 0){
							html += "<tr>";
							html += "	<td class='al' style='text-align:center;background: #f1f1f1;'>"+tempList[i].name+"</td>";
							html += "	<td style='text-align:center;'>"+(isNaN(tempList[i].data) ? 0 : $workRoad.util.addComma(tempList[i].data))+"</td>";
							html += "</tr>";
						}
					}
			...
			/**
			 * 
			 * @name         : updateTargetAreaEconomicSituationTable
			 * @description  : 해당 지역 데이터 보기 표 업데이트
			 * @date         : 2015. 10. 29. 
			 * @author	     : 김성현
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaEconomicSituationTable : function(target_mon) {
				if($ssaEconomicSituationDetailPopup.ui.DataBoardChart.length > 0){
					var TableData = $ssaEconomicSituationDetailPopup.ui.DataBoardChart	//차트 & 테이블 데이터
					var TableCategory = $ssaEconomicSituationDetailPopup.ui.DB_Chart_Category	//카테고리 (지역)
...
//								html += "	<td style='text-align:center;'>"+(isNaN(Table_Data[Table_Data_Idx]) ? 0 : Table_Data[Table_Data_Idx])+"</td>";
			...
			/**
			 * 
			 * @name         : updateTargetAreaEconomicSituationTableInvert
			 * @description  : 해당 지역 데이터 보기 표 업데이트
			 * @date         : 2015. 10. 29. 
			 * @author	     : 김성현
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaEconomicSituationTableInvert : function(target_mon) {
				if($ssaEconomicSituationDetailPopup.ui.DataBoardChart.length > 0){
					var TableData = $ssaEconomicSituationDetailPopup.ui.DataBoardTable;	//차트 & 테이블 데이터
					var TableCategory = $ssaEconomicSituationDetailPopup.ui.DataBoardCategory;	//카테고리 (지역)
					var TableName = $ssaEconomicSituationDetailPopup.ui.DB_Chart_Category;
					...
					for(var  i = 0; i < tempList.length; i ++) {
						html += "<tr>";
						html += "	<td class='al' style='text-align:center;background: #f1f1f1;'>"+tempList[i].name+"</td>";
						html += "	<td style='text-align:center;'>"+(isNaN(tempList[i].data) ? 0 : tempList[i].data)+"</td>";
						html += "</tr>";
					}
			...
			/**
			 * 
			 * @name         : updateTargetAreaLifeQualityTable
			 * @description  : 해당 지역 데이터 보기 표 업데이트
			 * @date         : 2015. 10. 29. 
			 * @author	     : 김성현
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaLifeQualityTable : function(target_mon) {
				if($ssaLifeQualityDetailPopup.ui.DataBoardTable.length > 0){
					var TableData = $ssaLifeQualityDetailPopup.ui.DataBoardTable;	//차트 & 테이블 데이터
					var TableCategory = $ssaLifeQualityDetailPopup.ui.DataBoardCategory;	//카테고리 (지역)
					var TableName = $ssaLifeQualityDetailPopup.ui.DB_Chart_Category;
...
//						20190319 손원웅 추가
			...
			/** 2020-04-27 [곽제욱] 일자리통계분석 - 삶의질 - 데이터보드(표) 시도별 표출 START */
			/**
			 * 
			 * @name         : updateTargetAreaLifeQualityTableInvert
			 * @description  : 해당 지역 데이터 보기 표 업데이트
			 * @date         : 2020. 04. 27. 
			 * @author	     : 곽제욱
			 * @param	: res, options
			 * @history 	 :
			 */
			updateTargetAreaLifeQualityTableInvert : function(target_mon) {
				if($ssaLifeQualityDetailPopup.ui.DataBoardChart.length > 0){
					var TableData = $ssaLifeQualityDetailPopup.ui.DataBoardChart	//차트 & 테이블 데이터
					var TableCategory = $ssaLifeQualityDetailPopup.ui.DB_Chart_Category	//카테고리 (지역)
			...
			/** 2020-04-27 [곽제욱] 일자리통계분석 - 삶의질 - 데이터보드(표) 시도별 표출 END */
			//mng_s 20180115 leekh N/A 0처리 함수
			dataBoardSum : function(val) {
				if(val == "N/A"){
					val = 0;
				}
				$ssaDataBoard.ui.dataSumValue += parseFloat(val);
			},
			...
			updateTargetAreaTableKosis : function(res) {
				...
				//정렬
				var dataList = $ssaDataBoard.Util.tableDataSort(dataList);
			...
			/*updatePieChart : function(options) {
				...
				var sopPortalPieChartDrawObj = new sop.portal.ssa.pieChartDraw.api();
				for (var i=0; i<options.params.param.length; i++) {
...
//				var dataBoard = this.mapData[this.map_id].dataBoard;
				var dataBoard = this.mapData[$ssaMap.ui.curMapId].dataBoard;
				var date = new Date();
				...
				if (options.params.param != undefined && options.params.param.length > 0) {
					for (var i=0; i<options.params.param.length; i++) {
						if (options.params.param[i].key == "year") {
							...
							$ssaMap.ui.curDropParams.time_year = base_year;
				...
				for(var i = 0; i < dataBoard.yearArray.length; i ++) {
					$("#li_y"+dataBoard.yearArray[i]).on('click', function() {
						...
						if(!$(".btn_clockTypeSetting").hasClass("on")) {
							$ssaDataBoard.ui.selectTimeSeries(options, $(this).find("input"));
						}
					});
				}
...
//					this.delegate.shareUrlInfo = [];
					...
					for(var i = 0; i < options.params.param.length; i ++) {
						if(options.params.param[i].key == "year") {
							...
							$ssaMap.ui.curDropParams.time_year = $(yearObj).val();
						}
					}
					...
					$ssaMap.ui.requestOpenApi(options.params);
				}
			},
			...
			doReqTimeSeries : function() {
				...
				if (dataBoard.selectedYearArray.length > 0) {
					if (dataBoard.timeSeriesCnt == dataBoard.selectedYearArray.length) {
						$ssaMapApi.request.mask.close(); //9월 서비스
			...
			timerSeriesPlay : function() {
				//9월 서비스
				var dataBoard = $ssaDataBoard.ui.mapData[$ssaDataBoard.ui.map_id].dataBoard;
				var map = $ssaDataBoard.ui.map;
				...
				if (dataBoard.timeSeriesPushData[dataBoard.timeSeriesCnt].res.errCd == "0") {
					...
					//범례고정일 경우
					if ($ssaDataBoard.ui.isFixedLegend) {
						if (map.dataGeojson) {
							map.dataGeojson.remove();
							map.removeCaption();
							map.dataGeojson = null;
						}
						map.legend.valPerSlice = $ssaDataBoard.ui.fixedLegend;
					...
					$ssaDataBoard.ui.updateDataBoard(res, options);
					$ssaMap.ui.curDropParams[map.id] = options.params;
					dataBoard.timeSeriesCnt++;
			...
			selectChartData : function(properties, index, map_id) {
				var charts = $("#ssaDataBoard #ssaJobStatusDataBoardChart").highcharts();
				
				if($("#ssaDataBoard #ssaJobStatusDataBoardChart").is(":visible")) {
...
//				dataBoard.timer = 0;
				...
				$(".btn_clockTypePlay").css("background-image", "url(/images/workRoad/ico/ico_dbPlay.png)");
			},
			...
			reset : function(map_id) {
				$(".dataBoardDiv").hide();
				//일반 데이터보드
				$(".dataBoardNotice").show();
				$("#ssaDataBoard #ssaTitle").html("");		//통계명
				$("#ssaDataBoard #ssaOrigin").html("");		//출처
				//$(".dataBoardStats").html("");		//통계
				$("#ssaDataBoard #ssaJobStatusDataBoardChart").empty();		//표 차트
				...
				$("#fullDataBoardTitle").find("#ssaDataBoard #ssaTitle").html("");
				$("#fullDataBoardTitle").find("#ssaDataBoard #ssaOrigin").html("");
				$("#fullDataBoardTitle").find("#ssaDataBoard #ssaArea").html("");
			...
			toggleDataboard : function(pVisible) {
				var $this = $('#ssaDataBoard .workRoadDataBoard');
				var ck = false;
//				var ck = $(this).hasClass("on");
				...
				//2019-01-29 데이터보드 너비 조정.
				if(!ck){
					var full = $ssaDataBoard.ui.boardFull;
					if(!full){
						//일반 데이터보드
						...
						$("#fullDataBoard").hide();
						$this.addClass("on").stop().animate({"right":"425px"},200);
						$(".workRoad .today-data.view dd").css("width","452px");
						...
						$("#normalDataBoard").show();
						$("#fullDataBoard").hide();
						$this.addClass("on").stop().animate({"right":"675px"},200);
						$(".workRoad .today-data.view dd").css("width","652px");
						$(".sop-right").stop().animate({"right":"758px"},200);//2019-06-12 박길섭
					}
				}else{
	...
	$ssaDataBoard.event = {
			...
			setUIEvent : function() {
				...
				body.on("click",".dataSideBox .bar>a",function(){
					...
					$(".workRoadDataBoard").removeClass("on").stop().animate({"right":"0"},200);
				...
				//데이터보드 열고 닫기
				body.on("click","#ssaDataBoard .workRoadDataBoard",function(){
					//2019-01-29 데이터보드 너비 조정.
					$ssaDataBoard.ui.toggleDataboard(true);
//					var ck = $(this).hasClass("on");
//					if(!ck){
//						//일반 데이터보드
//						$(".dataSideBox").stop().animate({"right":"50px"},200);
//						$("#normalDataBoard").show();
//						$("#fullDataBoard").hide();
//						$(this).addClass("on").stop().animate({"right":"476px"},200);
//						
//					}else{
//						var full = $(".dataSideBox").hasClass("full");
//						if(!full){
...
//					}
					...
					if(!ck){
						...
						if(!full){
							...
							if($ssaMapFrame.ui.curSelectedStatsType == "kosis"
	            ...
	            body.on("click",".typeBox>a",function(){ 
	               ...
	               if(ck%2){
	                  ...
	                  $("#barChartTable").hide(); //2020-07-22 [곽제욱] 데이터보드-표보기 오류 수정
	                  ...
	                  $(".dataSideBox").removeClass("full");
	                  $ssaDataBoard.ui.toggleDataboard(true);
	                  $ssaDataBoard.ui.boardFull = false;
	                  
	                  // 2019.03.13 접근log 생성
	                  srvLogWrite('D0', '05', '06', '01', '', $("#ssaDataBoard #ssaStatPath").text());
	               }else{
	                  ...
	                  $("#barChartTable").show(); //2020-07-22 [곽제욱] 데이터보드-표보기 오류 수정
	                  //2019-01-29 데이터보드 너비 조정.
	                  $(".dataSideBox").addClass("full");
	                  $ssaDataBoard.ui.toggleDataboard(true);
	                  $ssaDataBoard.ui.boardFull = true;
	                  
	                  // 2019.03.13 접근log 생성
	                  srvLogWrite('D0', '05', '06', '02', '', $("#ssaDataBoard #ssaStatPath").text());
	               }
	             });
				//데이터보기 엑셀 다운로드
				body.on("click",".btn_excelDownload", function(){
					...
					// 2019.03.13 접근log 생성
					srvLogWrite('D0', '05', '06', '03', '',  $("#ssaDataBoard #ssaStatPath").text());
					...
					$(this).parents(".compareBox").eq(0).find(".tables").eq(0).find("div").find("table tr").each(function(){
					 ...
					 var url = "/view/workRoad/excelDown";
				...
				body.on("click",".btn_clockTypeSetting",function(){ 
					...
					if ($(".btn_clockTypeLegend").text() == "on") {
						$(".year_input").hide();
						$(".year_radio").show();
						if ($ssaDataBoard.ui.fixedLegendYear != null) {
							$(".year_radio").each(function() {
								if ($(this).val() == $ssaDataBoard.ui.fixedLegendYear) {
									$(this).prop("checked");
								}
							});
						}
				...
				body.on("click", ".btn_clockTypePlay", function() {
					...
					if ( sop.isInnerMapShow2 || sop.isInnerMapShow3 ) { //그리드이면
						...
						if($(this).text() == "play") {
							that = $ssaMap.ui;
							$(this).text("stop");
							$(this).css("background-image", "url(/images/workRoad/ico/ico_dbStop.png)");
							//시계열 플레이시작.
							...
							$(this).text("play");
							$(this).css("background-image", "url(/images/workRoad/ico/ico_dbPlay.png)");
							$("#div_y2016").click();
						}
						...
						if($(this).text() == "play") {
							...
							dataBoard.isTimeSeriesPlay = true;
							//9월 서비스
							$ssaDataBoard.ui.map.isDrop = true;
							$ssaDataBoard.ui.map.undoDropLayerBounds();
							$ssaDataBoard.ui.map.setFixedBoundLevel(true);
							$ssaDataBoard.ui.doReqTimeSeries();
							...
							clearInterval(dataBoard.timer);
							$ssaDataBoard.ui.map.setFixedBoundLevel(false); //9월 서비스
						}
					}
				...
				body.on("click", ".btn_clockTypeLegend", function() {
					if($(this).text() == "off") {
						$(this).text("on");
						$(this).css("background-image", "url(/images/workRoad/ico/ico_fixed_legend_on.png)");
						$(".year_input").hide();
						$(".year_radio").show();
						...
						$(".year_radio").each(function() {
							...
							if (radio) {
								$ssaDataBoard.ui.fixedLegendYear = $(this).val(); 
							}
						})
						$ssaDataBoard.ui.isFixedLegend = true;
						
					}else {
						$(this).text("off");
						$(this).css("background-image", "url(/images/workRoad/ico/ico_fixed_legend_off.png)");
						$(".year_input").show();
						$(".year_radio").hide();
						$ssaDataBoard.ui.isFixedLegend = false;
					}
				});
			...
			dataBoardOpen : function() {
				...
				if(!$(".workRoadDataBoard").hasClass("on")) {
					$(".workRoadDataBoard").click();
					$("#dataBoard").show();
				}
			}
	};
...
//					return b.value - a.value;
			...
			tableDataSortValue : function(data) {
				var totalSum = 0;
	...
	/*********** 원형차트 레벨별 표출 Start **********/
	/*(function() {
	    $class("sop.portal.ssa.pieChartDraw.api").extend(sop.portal.absAPI).define({
