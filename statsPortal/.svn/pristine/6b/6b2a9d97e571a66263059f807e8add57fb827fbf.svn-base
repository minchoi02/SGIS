...
//													layer.setCaption({title:appendCommaToNumber(layer.feature.info[0][param])+ " ("+layer.feature.info[0].unit + ")", color:"white", showAllZoomLevel: false});
				...
				this.setPolygonDataGeojson = function (geoData, type, index) {
					...
					$thematicMapFrame03.Popup2.close();
...
//								  $(".helperText span").text(res.result.getThemaStatsInfo[0].title);
								  ...
								  if (result.left_sep_nm !=null && result.left_sep_unit !=null && result.left_sep_nm.length > 0 && result.left_sep_unit.length > 0){
									  var leftHtml= '<a href="javascript:srvLogWrite(\'B0\',\'03\',\'02\',\'01\',\''+result.title+'\',\'\');$thematicMapFrame03.Popup.show();$thematicMapFrame03.ui.changeLeftRightValue()" class="first last" id="leftValue">'+ result.left_sep_nm+'('+result.left_sep_unit+')</a>'
								  ...
								  if (result.right_sep_nm !=null && result.right_sep_unit!=null && result.right_sep_nm.length > 0 && result.right_sep_unit.length > 0){
									  var rightHtml= '<a href="javascript:srvLogWrite(\'B0\',\'03\',\'02\',\'02\',\''+result.title+'\',\'\');$thematicMapFrame03.Popup.show();$thematicMapFrame03.ui.changeLeftRightValue()" class="last" id="rightValue">'+ result.right_sep_nm+'('+result.right_sep_unit+')</a>'
								...
								//시계열년도고정유무가 Y이면, 기준경계년도에 맞는 경계를 표출한다.
								  ...
								  if(window.parent.$thematicMapMain.param.stat_thema_map_id == "mtjKxt2Zkb20171109101132537k7SYSI50kn"){
									  $('#map_type').append('<a href="javascript:$thematicMapFrame03.ui.changeDataMode()" class="" id="pieChart">파이차트</a>');
									  $('#map_type').append('<a onclick="srvLogWrite(\'B0\',\'06\',\'03\',\'00\',\''+window.parent.$thematicMapMain.themaInfo.title+'\',\'\')"; href="javascript:$thematicMapFrame03.ui.changeDataMode()" class="last" id="heatMapGrid">열지도</a>');
...
//									  $('#region_boundary').append('<a href="javascript:$thematicMapFrame03.Popup.show(); $thematicMapFrame03.ui.mapList[0].changeRegionBound()" class="first on" id="autoRegion">자동</a>');
//									  $('#region_boundary').append('<a href="javascript:$thematicMapFrame03.Popup.show(); $thematicMapFrame03.ui.mapList[0].changeRegionBound();" id="sido" style="width:90px;">시도별 전국지도</a>');
//									  $('#region_boundary').append('<a href="javascript:$thematicMapFrame03.Popup.show(); $thematicMapFrame03.ui.mapList[0].changeRegionBound();" id="sigungu" style="width:100px;">시군구별 전국지도</a>');
//									  $('#region_boundary').append('<a href="javascript:$thematicMapFrame03.Popup.show(); $thematicMapFrame03.ui.mapList[0].changeRegionBound();" id="eupmyundong" class="last" style="width:100px;">읍면동별 전국지도</a>');
									  
									  $('#map_type').append('<a href="javascript:srvLogWrite(\'B0\',\'10\',\'02\',\'00\',\''+window.parent.$thematicMapMain.themaInfo.title+'\',\'\');$thematicMapFrame03.Popup.show(); $thematicMapFrame03.ui.mapList[0].changeRegionBound();" class="last" id="networkData" style="width:100px;">네트워크 데이터</a>');
									  $("#bubble").hide();
								  ...
								  if(window.parent.$thematicMapMain.param.stat_thema_map_id == "yFwtuGMFxt20160629174353420HvLDEFvqyu"){
									  $('#region_boundary').append('<a href="javascript:srvLogWrite(\'B0\',\'07\',\'01\',\'00\',\''+window.parent.$thematicMapMain.themaInfo.title+'\',\'\');$thematicMapFrame03.ui.mapList[0].changeRegionBound()" id="library" class="last" style="width:70px;">POI</a>');
									  $("#sido").attr('class', '');
								  }
								  //mng_e 20171212 주용민
				...
				this.maxZoomType = function(){
				  if(that.max_expnsn_level=="01"){
					  ...
					  if(that.poi_disp_yn) {
						  // mng_s 20200208 김건민 위성지도 일떄 읍면동까지 내려가게 수정함.
						  if(that.mapMode == "settlite"){
					...
					/** 통계 세부 데이터 조회 끝 **/
					 ...
					 /**
						...
						this.changeRegionBound = function(){
							...
							// 20210923 김건민 (크롬에서 문화재관리지도 기능이 안되게 alert만 띄우게 처리함.)
							if($("#selectValue2").val()=="4"){
								if(navigator.userAgent.toLowerCase().indexOf("chrome") != -1){
									$("#heritage").removeClass("on");
									alert("문화재공간정보서비스 특성상 Internet explorer에서만 문화재지도가 정상적으로 표출됩니다. IE에서만 이용 가능합니다.");
									return;
								}
							}
							// 20210923 김건민
							...
							// mng_s 2017. 12. 13 kdb
								...
								//문화재 현황에 등록문화재 추가
								// 20210916 김건민 (IE에서만 문화제관리지도 클릭 시 실행되게 기능 수정함.)
...
//							}
							...
							$.ajax({
								...
								success: function(res) {
									if (res.errCd == "0") {
										$thematicMapFrame03.Popup.close();
										// mng_s 20220208 김건민 (위성지도 관련된 대해서 수정함.)
										if(that.mapMode == "settlite"){
										   that.gMap.setMaxZoom(18);
										}else{
										   that.gMap.setMaxZoom(12);
						...
						this.heritage = function(type){
							...
							//var imageUrl = "http://sgis.kostat.go.kr/checkKey.do?key=hfRAJvveRqpdP8cx&domain=http://gis-heritage.go.kr/";
							//var imageUrl = "/checkKey.do?key=hfRAJvveRqpdP8cx&domain=http://gis-heritage.go.kr/";
							
							//mng_s 20210907 gpki인증서가 브라우저에서 밴당해서 프록시로 구성함 프록시가 제우스 라이브러리때문에 않돌아가서 원복함
							//var imageUrl = "/gis_heritage/checkKey.do?key=hfRAJvveRqpdP8cx&domain=http://gis-heritage.go.kr/";
							
							//var imageUrl = "http://sgis.kostat.go.kr/o2mapweb_i/services"; //proxy를 o2mapweb_i로 설정하고 이걸로 날리면 될것 같다 아래것도 ?format으로 변경요망
							...
							//imageUrl = "/proxy_heritage2.jsp?" + encodeURI(imageUrl);
							...
							/*
							var imageUrl2 = "http://sgis.kostat.go.kr/o2mapweb_i/services"; //proxy를 o2mapweb_i로 설정하고 이걸로 날리면 될것 같다 아래것도 ?format으로 변경요망
							
							imageUrl2 += "?format=image/png&version=1.3.0&transParent=true&displayOutsideMaxExtent=null";
							
							imageUrl2 += "&crs=EPSG:9020203&exceptions=INIMAGE&service=WMS&request=GetMap";
							imageUrl2 += "&LAYERS=" + heritageApiList[$("#heritageMode").val()];
							imageUrl2 += "&styles=default";
							imageUrl2 += "&bBox=" + imageBounds._southWest.x + ", " + imageBounds._southWest.y + ", " + imageBounds._northEast.x + ", " + imageBounds._northEast.y;
							imageUrl2 += "&width=" + $('#mapRgn_1').width() + "&height=" + $("#mapRgn_1").height();
							
							imageUrl2 = "/proxy_heritage2.jsp?" + encodeURI(imageUrl2);
							
							switch($("#heritageMode").val()){
								case "0" : that.imageLayer0 = new sop.imageOverlay(imageUrl2, imageBounds); that.gMap.addLayer(that.imageLayer0); that.imageLayer0.bringToFront(); break;
								case "1" : that.imageLayer1 = new sop.imageOverlay(imageUrl2, imageBounds); that.gMap.addLayer(that.imageLayer1); that.imageLayer1.bringToFront(); break;
							}
							*/
...
//									$thematicMapFrame03.Popup2.show();
//								}else{
//									that.getThemaMapData(that.thema_map_data_id,curSidoCd , "100"); //원본
//								}
