...
(function(W, D) {
	...
	var menuCurrentPageIndex = 0;		//Menu 현재 페이지
	var themeCurrentPageIndex = 0;		//통계주제도 현재 페이지
	var kosisCurrentPageIndex = 0;		//KOSIS 현재 페이지	
	var emptyRequestCnt = 0;
	
	var statsMeCatalogCurrentPageIndex = 0;		// 2020.04.06[한광희] My통계로 현재 페이지 추가
	...
	$searchList = {
		...
		statsMeCatalogParam : [],		// 2020.04.06[한광희] My통계로 검색 파라미터 추가
		map : null,
		resultData : null,
		...
		//Menu 페이징 처리
		menuPaging : function(totalCount, currentIndex) {
			var pageSize = 5;						
			var totalPage = Math.ceil( totalCount / pageSize);			
			$('#menuPaging .pages').paging({
				current:currentIndex+1,
				max:totalPage,
				itemClass : 'page',
				itemCurrent : 'current',
				...
				last : '&gt;&gt;',
				onclick:function(e,page){
					menuCurrentPageIndex = page-1;
					$searchList.openApiMenu($searchList.searchKeyword, page-1);
				}
			});
		},
...
//		},
		...
		// MENU 검색
		openApiMenu : function(searchword, pagenum) {
			var sopOpenApiMenuObj = new sop.openApi.menusearch.api();
			sopOpenApiMenuObj.addParam("accessToken", accessToken);
			sopOpenApiMenuObj.addParam("searchword", encodeURIComponent(encodeURIComponent(searchword)));
			sopOpenApiMenuObj.addParam("pagenum", pagenum);
			sopOpenApiMenuObj.request({
		        method : "GET",
		        async : true,
		        url : openApiPath+"/OpenAPI3/search/menu.json",
		        options : {
		        	btntype : "search",
		        	api_id : "API_0505",
		        	params : {
			...
			this.openApiMenuParam = [];
			this.openApiMenuParam.push(searchword);
			this.openApiMenuParam.push(pagenum);
		},
...
//		},
		...
		openApiReverseGeoCode : function (division, url, title) {
			...
			if(division != null && division == "sop"){
				srvLogWrite('A0', '06', '04', '00', '', '');
			} else if(division != null && division == "kosis"){
				srvLogWrite('A0', '06', '06', '00', '', '');
			}
		...
		/** 2020.04.06[한광희] SGIS 포털 검색 시 My통계로 카탈로그 검색 END */
		// My통계로 카탈로그 검색
		statsMeCatalog : function(searchword, pagenum) {
			var sopStatsMeCatalogObj = new sop.openApi.statsMeCatalogSearch.api();
			sopStatsMeCatalogObj.addParam("searchword", encodeURIComponent(searchword));
			sopStatsMeCatalogObj.addParam("resultCnt", 5);
			sopStatsMeCatalogObj.addParam("pagenum", pagenum);
			sopStatsMeCatalogObj.request({
		        method : "POST",
		        async : true,
		        url : contextPath+"/ServiceAPI/statsMe/map/getPotalStatsMeCatalogList.json",
		        options : {
		        	btntype : "search",
		        	api_id : "115100",
		        	params : {
			...
			this.statsMeCatalogParam = [];
			this.statsMeCatalogParam.push(searchword);
			this.statsMeCatalogParam.push(pagenum);
		},
		
		// My통계로 카탈로그 페이징 처리
		statsMeCatalogPaging : function(totalCount, currentIndex) {
			var pageSize = 5;
			var totalPage = Math.ceil( totalCount / pageSize);			
			$('#statsMeCatalogPaging .pages').paging({
				current:currentIndex+1,
				max:totalPage,
				itemCurrent : 'current',
				format : '{0}',
				...
				last : '&gt;&gt;',
				onclick:function(e,page){
					statsMeCatalogCurrentPageIndex = page-1;
					$searchList.statsMeCatalog($searchList.searchKeyword, page-1);
				}
			});
		},
		/** 2020.04.06[한광희] SGIS 포털 검색 시 My통계로 카탈로그 검색 END */
		...
		checkRequestEmptyCnt : function( totCnt ) {
			$searchList.searchCnt++;
			
			console.log("[searchList.js] totCnt[" + totCnt + "] $searchList.totCnt[" +$searchList.totCnt);
			
    		$searchList.totCnt = ( $searchList.totCnt + totCnt );
    		
    		console.log("[searchList.js] $searchList.searchCnt[" + $searchList.searchCnt + "] $searchList.totCnt[" +$searchList.totCnt);
    		
    		//mng_s 20200529 이진호
    		//$searchList.searchCnt 의 수가 기존 8에서 7로 줄어듬에 따라 조건문 수정
			//if( $searchList.searchCnt == 8 && $searchList.totCnt == 0 ){
    		if( $searchList.searchCnt == 7 && $searchList.totCnt == 0 ){
	...
	(function() {
	    $class("sop.openApi.relsearch.api").extend(sop.portal.absAPI).define({
	        onSuccess : function(status, res, options) {	        	
            	...
            	if( res.errCd != "-401" ) {
            		var len = ( res.result ? res.result.length : 0 );
            		$searchList.checkRequestEmptyCnt( len );
	...
	(function() {
	    $class("sop.openApi.sopsearch.api").extend(sop.portal.absAPI).define({
	        onSuccess : function(status, res, options) {
            	...
            	if( res.errCd != "-401" ){
            		var len = ( res.result ? res.result.resultdata.length : 0 );
            		$searchList.checkRequestEmptyCnt( len );
	...
	/*********** Menu 검색 Start **********/
	(function() {
	    $class("sop.openApi.menusearch.api").extend(sop.portal.absAPI).define({
	        onSuccess : function(status, res, options) {
            	$("#menuListTable").empty();
            	...
            	if( res.errCd != "-401" ){
            		...
            		if(res.errCd == "0") {
            			...
            			if(result.totalcount > 5){
            				var htmlPage = "<br><br><br><div id='menuPaging' class='pagenation searchPagenation' align='center' style='width: 100%;'><span class='pages'></span></div>";
            				$("#menuListTablePage").html(htmlPage);
            			}            		
            			$searchList.menuPaging(result.totalcount, menuCurrentPageIndex);
            		...
            		accessTokenInfo();
        			setTimeout($searchList.openApiMenu($searchList.openApiMenuParam[0], $searchList.openApiMenuParam[1]), 500);
            	}
	        },
	        onFail : function(status) {
	        }
	...
	(function() {
	    $class("sop.openApi.themeticsearch.api").extend(sop.portal.absAPI).define({
	        onSuccess : function(status, res, options) {
            	...
            	if( res.errCd != "-401" ){
            		var len = ( res.result ? res.result.themeMapInfoList.length : 0 );
            		$searchList.checkRequestEmptyCnt( len );
            			...
            			if(result.themeMapInfoListCount > 0) {
            				...
            				for(var i = 0; i < result.themeMapInfoList.length; i++) {
            					...
            					if(elem.thema_map_type == '02') {   
            						...
            						html += "		<a style='margin-left:20px;' href='javascript:logWriteAndMove(\"A0\", \"06\", \"05\", \"00\", \"\", \"\", \"/view/thematicMap/thematicMapMainOld?theme="+elem.category+"&stat_thema_map_id="+elem.stat_thema_map_id+"&mapType="+elem.thema_map_type+"\", false);'>" + elem.category_nm + " &gt; " + elem.title + "</a>";
            					} else {
            						...
            						html += "		<a style='margin-left:20px;' href='javascript:logWriteAndMove(\"A0\", \"06\", \"05\", \"00\", \"\", \"\", \"/view/thematicMap/thematicMapMain?theme="+elem.category+"&stat_thema_map_id="+elem.stat_thema_map_id+"&mapType="+elem.thema_map_type+"\", false);'>" + elem.category_nm + " &gt; " + elem.title + "</a>";
	    ...
	    $class("sop.openApi.geocode.api").extend(sop.portal.absAPI).define({
	        onSuccess : function(status, res) {	        	
	        	if( res.errCd != "-401" ){
	        		...
	        		var html = "<div class='search_Result_fail'>검색중입니다...</div>";
	        		...
	        		//Menu 검색
	        		if(arrayKey.length < 2) {
	        			$searchList.openApiMenu($searchList.sKeyword, menuCurrentPageIndex);
	        		} else {
	        			$searchList.openApiMenu($searchList.searchKeyword, menuCurrentPageIndex);
	        		}
	        		...
	        		/** 2020.04.06[한광희] SGIS 포털 검색 시 My통계로 카탈로그 검색 추가 START */
		            // My통계로 검색
		            if(arrayKey.length < 2) {
		            	$searchList.statsMeCatalog($searchList.sKeyword, statsMeCatalogCurrentPageIndex);
		            } else {
						      $searchList.statsMeCatalog($searchList.searchKeyword, statsMeCatalogCurrentPageIndex);
		            }
		            /** 2020.04.06[한광희] SGIS 포털 검색 시 My통계로 카탈로그 검색 추가 END */
...
//		            			$("#poiPaging").css("top", "575px");
	        			...
	        			for(var  i = 0; i < result.resultdata.length; i ++) {
	        				...
	        				html += "<div><li><a href='javascript:srvLogWrite(\"A0\", \"06\", \"01\", \"00\", \"\", \"\");$searchList.poiClick(" + i + ");' onmouseover='$searchList.marker_tooltipshow(" + i + ");return false;' onmouseout='$searchList.marker_tooltiphide(" + i + ");return false;'>" + result.resultdata[i].nm +"</a></div>";	
		...
		$class("sop.openApi.ReverseGeoCodeSearch.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				...
				if( res.errCd != "-401" ){
					...
					/*
					if(res.result.resultdata != undefined) {
						var len = ( res.result ? res.result.resultdata.length : 0 );
	            		$searchList.checkRequestEmptyCnt( len );
					}
					*/
...
//					   "&title="+options.title;
	...
	(function() {
	    $class("sop.openApi.statsMeCatalogSearch.api").extend(sop.portal.absAPI).define({
	        onSuccess : function(status, res, options) {
            	$("#statsMeCatalogListTable").empty();
            	if( res.errCd != "-401" ){
            		var len = ( res.result ? res.result.statsMeCatalogList.length : 0 );
            		$searchList.checkRequestEmptyCnt( len );
            		
            		if(res.errCd == "0") {
            			var result = res.result;
            			// My통계로
            			if(result.statsMeCatalogCount > 0) {
            				var html = ""; 
            				html += "<div class='search_result_list'>";
            				html += "<p>My통계로(路) 검색결과</p>";
            				html += "<ul>";
            				
            				for(var i = 0; i < result.statsMeCatalogList.length; i++) {
            					var elem = result.statsMeCatalogList[i];            			
            					html += "<li id='statsMeCatalogList"+i+"'>";
            					html += "<a style='margin-left:20px;' href='javascript:logWriteAndMove(\"N0\", \"08\", \"12\", \"00\", \"(" +elem.stat_data_id+ ")" +elem.stat_data_srv_nm+ "\", \"\", \"/view/statsMe/potalSearchStatsMeMap?stat_data_id="+elem.stat_data_id+"&stat_data_srv_nm="+elem.stat_data_srv_nm+"\", false);'>" + elem.stat_data_srv_nm + "</a>";
            					
            					html += "</li>";
            				}
            				html += "</ul>";
            				html += "</div>";	            		            
            				$("#statsMeCatalogListTable").html(html);
            				
            				if(result.statsMeCatalogCount > 5){
            					var htmlPage = "<br><br><br><div id='statsMeCatalogPaging' class='pagenation searchPagenation' align='center' style='width: 100%;'><span class='pages'></span></div>";
            					$("#statsMeCatalogListTablePage").html(htmlPage);
            				}            		
            				$searchList.statsMeCatalogPaging(result.statsMeCatalogCount, statsMeCatalogCurrentPageIndex);
            			}
            		} 
            	} else {
            		accessTokenInfo();
        			  setTimeout($searchList.statsMeCatalog($searchList.statsMeCatalogParam[0], $searchList.statsMeCatalogParam[1]), 500);
            	}
	        },
	        onFail : function(status) {
	        }
