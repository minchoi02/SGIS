...
public class Forestryhousehold extends AbsQuery<List> {
	...
	protected void optimizeParameterMap(Map mapParameter) throws ApiException {
		...
		if (area_type.equals("0")) {
			...
			//20200427 수정 시작 : is_non_self가 Y이면 시도로 처리 (ggm)
			String is_non_self  = (String) mapParameter.get("is_non_self");
			if(adm_cd != null && "Y".equals(is_non_self)) {
			...
			if(sido_cd == null){
				mapParameter.put("sido", "\'NULL\'");
			}else{
				mapParameter.put("sido", sido_cd);
			}
			if(sgg_cd == null){
				mapParameter.put("sigungu", "\'NULL\'");
			}else{
				mapParameter.put("sigungu", sido_cd+""+sgg_cd);
			}
			if(emdong_cd == null){
				mapParameter.put("emdong", "\'NULL\'");
			}else{
				mapParameter.put("emdong", sido_cd+""+sgg_cd+""+emdong_cd);
			}
	...
	enum OptionParam {
		...
		,log_qntty_to
		,bnd_year
		, is_zoom_lvl4		//20200427 수정 (ggm)
		, is_non_self		//20200427 수정 (ggm		
	}
