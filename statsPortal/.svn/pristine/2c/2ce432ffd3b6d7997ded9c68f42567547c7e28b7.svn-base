...
//			$houseAnalysisMap.idealtype.step1init();
		...
		setSearchItem : function(){
			...
			var idealTypeTemp="";
			$("#ideal-type-step1 a[data-type=radio][data-search-item=true].M_on,#ideal-type-step1 input[data-search-item=true]:checked").each(function(idx){
				var _item = $("#ideal-type-step1 a[data-type=radio][data-search-item=true].M_on,#ideal-type-step1 input[data-search-item=true]:checked")[idx];

				if( _item ){
					idealTypeTemp += $( _item ).attr('data-title')+",";
				} else {
					idealTypeTemp += ",";
				}
			...
			idealTypeTemp = idealTypeTemp + $("#ideal-type-sido-select").find("option[value='"+$('#ideal-type-sido-select').val()+"']").text()+" "+$("#ideal-type-sgg-select").find("option[value='"+$('#ideal-type-sgg-select').val()+"']").text();
			srvLogWrite('F0','02','04','01',idealTypeTemp,'');
