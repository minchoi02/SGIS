...
function dragImageOn(){
	...
	$("#div_mapImage").droppable({
		...
		drop:function(e,ui){	
			...
			if( $galleryAdd.removeIconFlag != 0 ) { //아이콘 삭제시 idx가 겹치는 문제 발생으로 flag처리함.
				var idx = $galleryAdd.iconArray.length + $galleryAdd.removeIconFlag; 
			} else {
			...
			//var canvas_top = $(".mapWrap").height();
			//var canvas_width = $(".mapWrap").width();
			...
			//var positionTop = Number(ui.position.top);
			//var positionLeft = Number(ui.position.left);
			
			var imgSrc, imgSrcSplit;
			
			var className = ui.draggable.attr("class");
			
			///sgis_edu/resource/images/icon_stk01.png
			...
			if (className.indexOf("imgIcon") != -1 || className.indexOf("imgTextArea") != -1) {
				...
				if( $galleryAdd.removeIconFlag != 0 ) { //아이콘 삭제시 idx가 겹치는 문제 발생으로 flag처리함.
					for (var i=0; i<= $galleryAdd.removeIconFlag +20; i++) {
						if($galleryAdd.iconArray[i] != undefined ) {
							if ("ico_" + $galleryAdd.iconArray[i].seqId == ui.draggable.attr('id')) {
								
								var positionTop = Number((ui.position.top)/canvas_top*100);
								var positionLeft = Number(ui.position.left/canvas_width*100);
								
								$galleryAdd.iconArray[i].positionTop = positionTop;
								$galleryAdd.iconArray[i].positionLeft = positionLeft;
								break;
							}
						}
					}
...
(function(W,D){
	...
	$galleryAdd = {
			...
			deleteCaptureImage : function(){
				...
				deleteHist.request({
					method : "POST",
					async : false,
					url : contextPath + "/ServiceAPI/edu_gallery/DeleteGalleryHist.json",
					options : {}
				});
			},
            	...
            	setTimeout(function() {
            		if( Object.prototype.toString.call(targetId) === '[object Array]' ) {
            			...
            			for (var i=0; i<targetId.length; i++) {
    						...
    						var capture = html2canvas($(targetId)[0], {
    							logging: true,
    		                    useCORS: false,
    		                    proxy: contextPath+"/ServiceAPI/community/html2canvasproxy.jsonp"
			...
			*/
			...
			//교안 이미지 등록 시 목록 선택 하이라이트 표시하기
			galleryItemAdd2: function(amugerna,hist_id) {
				
				//이미지가 서버에 없으면 오류처리한다.
				var test_img = "/upload/gallery/galleryView/" + hist_id + ".png";
				var http = $.ajax({
					type:"HEAD",
					url : test_img,
					async: false
				...
				if(http.status != 200) {
					messageAlert.open("알림", "이미지가 존재하지 않아 등록할 수 없습니다.");
					$("#gallery_bookmark_pop tr").removeClass("on");
					$("#reg_chosen_img").attr("disabled", true);
					return false;
				} else if(http.status == 200) {
					$("#reg_chosen_img").attr("disabled", false);
				}
				
				$("#gallery_bookmark_pop tr").click( function() {
						$("#gallery_bookmark_pop tr").removeClass("on");
						$(this).addClass("on");
					} 
				);
				...
				$("#reg_chosen_img").off("click").on("click", function() { //이벤트 버블링때문에 삭제 할때마다 한번씩 더 늘어나서 이렇게 처리함
		        	$galleryAdd.galleryItemAdd(amugerna,hist_id);
				});
			},
			
			//선택한 이미지 등록하기 클릭시 전처리 후 작동
			...
			galleryItemAddLocalFile : function(amugerna,hist_id){
				...
				paramInfo.type = "bookMark";
				//paramInfo.fileName = $.trim(hist_id)+".png";
				paramInfo.fileName = $.trim(hist_id);
				object.paramInfo = paramInfo;
			...
			showGalleryImage : function(o){
				...
				*/
				...
				if( $galleryAdd.removeIconFlag != 0 ) { //아이콘 삭제시 idx가 겹치는 문제 발생으로 flag처리함.
					for (var i=0; i<= $galleryAdd.removeIconFlag +20; i++) {
						if($galleryAdd.iconArray[i] != undefined && $galleryAdd.iconArray[i].type =="2"){
							$galleryAdd.iconArray[i].exp = $("#ico_" + $galleryAdd.iconArray[i].seqId +" > div > textArea").val();
						}
					}
				} else {
					for(var i = 0; i < $galleryAdd.iconArray.length; i ++){
				...
				image.onerror = function() {
					$("#mapImage").css("background-image",'url("/img/pic/pic_testmap02.jpg")');
					$("#mapImage").css({"width" : "710px", "margin-left" : "0px"});
				};
				//==================================END===================================//
			},
			/**
			...
			writeGalleryUseCase : function(){
				location.href="/view/edu_gallery/writeGalleryUseCase";
			},
			...
			removeIcon : function(idx){
				...
				console.log("removeIcon idx [" + idx);
				
				$galleryAdd.removeIconFlag = $galleryAdd.removeIconFlag + 20;
				...
				for(var i =0; i < $galleryAdd.iconArray.length; i++){
					...
					if(object.seqId == idx){
						...
						$galleryAdd.iconList.splice(i,1);
					}
				}
				*/
				...
				if( $galleryAdd.removeIconFlag != 0 ) { //아이콘 삭제시 idx가 겹치는 문제 발생으로 flag처리함.
					for (var i=0; i<= $galleryAdd.removeIconFlag +20; i++) {
						...
						if(object != undefined && object.seqId == idx){
							$galleryAdd.iconArray.splice(i,1);
							$galleryAdd.iconList.splice(i,1);
						}
					}
			...
			//교안 만들기에서 아이콘을 모두 없애는 함수
			removeIconAll : function(){
				
				if($galleryAdd.removeIconFlag == 0 ) {
					for(var i =0; i < $galleryAdd.iconArray.length; i++){
						$("#ico_"+i).remove();
					}
					$galleryAdd.iconArray = []; //배열 초기화
					$galleryAdd.iconList = [];
				} else {
					for(var i =0; i <= $galleryAdd.removeIconFlag + 20; i++){
						$("#ico_"+i).remove();
			...
			saveFormSubmit : function(){
				...
				//leekh 제목과 내용만 체크하도록 수정
				var dataValidate = $galleryAdd.insertDataValidate2();
		    			 ...
		    			 btns = [
							{
							    ...
							    func : function(opt) {
									...
									$("#fileForm").ajaxForm({
										async : false,
										type : "POST",
										url : "/view/edu_gallery/galleryAdd",
										...
										complete : function(data){
											if (location.href.indexOf("thematicMap") != -1) {
												window.parent.location.href = "/view/edu_gallery/collectionGallery";
											}else {
												window.location.href = "/view/edu_gallery/collectionGallery";
											}
										}
			...
			imageUploadAndEdit : function(){
				...
				$galleryAdd.galleryItemAddLocalFile('',JSON.parse(jsonParam).preViewImg.saveFileName);
			},
			...
			gallerySaveForm : function(){
				...
				*/
				...
				var img_file_nm = $galleryAdd.imageArray[0].fileName;
				
				//console.log(img_file_nm);
				...
				*/
				...
				console.log($(":input:radio[name=private]:checked").val());
				//debugger;
				
				//iconParam = iconParam.replaceAll("<img src", "would_it_be_passed");
				iconParam = string_to_utf8_hex_string(iconParam);
				...
				$("#gallerySaveForm").ajaxForm({
					...
					data : {
						...
						param : jsonParam,
						//iconParam : encodeURIComponent(iconParam),
						iconParam : iconParam,
						teach_pwd : $("#teach_pwd").val(),
						open_yn : $(":input:radio[name=private]:checked").val(),
						param_school_grade : $("#hidden_ss_school_grade").val()
					...
					error : function(xhr,textStatus,error){
						console.log("등록오류 체크 xhr.responseText [" + xhr.responseText + "]");
						if(xhr.responseText != "") {
							messageAlert.open("알림", xhr.responseText);
						}
												
						alertFlag = xhr.responseText;
					},
					complete : function(data){
			...
			gallerySaveForm2 : function(){
				...
				*/
				...
				preViewHtml += '                <h3 class="lnbTi">' + $("#gallery_title").val() + '</h3>                                    '   ;
				...
				//preViewHtml += '                            <div class="stk stk01" style="top:10%; left:50%"></div>                '     ;
				//preViewHtml += '                            <div class="stk stk02" style="top:20%; left:50%"></div>                '     ;
				//preViewHtml += '                            <div class="stk stk03" style="top:50%; left:50%">                      '     ;
				//preViewHtml += '                                <p>입력한 메모가 출력되는 곳입니다.</p>                                  '     ;
				//preViewHtml += '                            </div>                                                                 '     ;
				...
				var galleryImgIconListItem = $galleryAdd.iconArray;
				
				var icon_html = "";
				for(var i = 0; i < galleryImgIconListItem.length; i++) {
					//var imgIconSrc = "/sgis_edu/resource/images/"+galleryImgIconListItem[i].icon_nm;
					var html ="";
					if("icon_stk01.png" == galleryImgIconListItem[i].srcFileName){
						html = '<div class="mapIconBox" name="ico"  style="z-index:20000;position:absolute;top:'+Number(Number(galleryImgIconListItem[i].positionTop) ) +'%;left:'+Number(Number(galleryImgIconListItem[i].positionLeft) )+'%">';
						html +='<img src="';
						html +="/sgis_edu/resource/images/img_stk01.png";
						html +='" style="width:28px;height:34px;" />';
						html +="</div>";
					} else if("icon_stk02.png" == galleryImgIconListItem[i].srcFileName){
						...
						html +='" style="width:184px;height:153px;" />';
						html +="</div>";
					}else if("icon_stk03.png" == galleryImgIconListItem[i].srcFileName){
						html = '<div class="mapIconBox" name="ico" id="ico_'+i+'" style="z-index:20000;position:absolute;top:'+Number(Number(galleryImgIconListItem[i].positionTop) ) +'%;left:'+Number(Number(galleryImgIconListItem[i].positionLeft) )+'%">'
						html +='<div class="mal01" style="background-image: url(/sgis_edu/resource/images/img_stk03.png); width:215px; height:130px; padding:15px 15px 30px 15px;" >';
				...
				//preViewHtml += '                            <button class="btn cmntBtn">댓글보기 <span>0</span></button>             '    ;
				preViewHtml += '                        </div>                                                                     '     ;
				preViewHtml += '                    </article>                                                                     '     ;
				preViewHtml += '                </section>                                                                         '     ;
				preViewHtml += '            </main>                                                                                '     ;
				...
				$("#gallerySaveForm").ajaxForm({
					...
					complete : function(data){
						$galleryAdd.writeGalleryHide();
						//debugger;
						window.location.href = "/view/edu_gallery/resultGallery?param_ss_page_info=T";
			...
			selectBookMarkList : function(){
				var selectBook = new sop.portal.selectBookMarkList.api.edu();
				selectBook.request({
					method: "POST",
					async : false,
	...
	//교안 이미지 등록 화면
	(function(){
		$class("sop.portal.selectBookMarkList.api.edu").extend(sop.portal.absAPI).define({
			onSuccess : function(status, res, options){
				switch(parseInt(res.errCd)){
					case 0 : 
						...
						$("#bookMarkList2 >tbody> tr").html('');
						var resList2 = res.result.resultList2;
						for(var i = 0 ; i < resList2.length; i++ ){
							var object = resList2[i];
							
							//console.log(JSON.parse(object.param_info).fileName);
							var file_name_id = JSON.parse(object.param_info).fileName;
							//console.log("first [" + file_name_id);
							if ( file_name_id.indexOf(".png") != -1) {
								file_name_id = file_name_id.substring(0, file_name_id.indexOf(".png"));
							}
							//console.log("second [" + file_name_id);
							
							var html = "<tr>";
							html +='<td><img src="/upload/gallery/galleryView/'+$.trim(file_name_id)+'.png" onerror="/img/pic/pic_testmap02.jpg" width="30" height="20" /></td>';
							html +='<td class="al"><a href="javascript:void(0)" class="gwon02" onclick="$galleryAdd.galleryItemAdd2(\''+object.title+'\',\''+file_name_id+'\')">'+object.title+'</a></td>'
							html +='<td>'+object.reg_dt.substring(0,4)+'.'+object.reg_dt.substring(5,7)+'.'+object.reg_dt.substring(8,10)+'</td>';
							html +='</tr>';
							$("#bookMarkList2 > tbody").append(html);
						}
						...
						for(var i = 0 ; i < resList.length; i++ ){
							...
							html +='<td class="al"><a href="javascript:void(0)" class="gwon02" onclick="$galleryAdd.galleryItemAdd2(\''+object.hist_nm+'\',\''+object.hist_id+'\')">'+object.hist_nm+'</a></td>'
				...
				switch(parseInt(res.errCd)){
					case 0 : 
						...
						$galleryAdd.doCapture(options.captureTargetId, hist_id, function(data) {
							...
							$.ajax({
							    url: contextPath +"/ServiceAPI/edu_gallery/urlMakeBase64.json",
							    data: formData,
							    type: 'POST',
							    contentType: false,
							    processData: false,
