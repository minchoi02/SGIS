...
//	setCookie("confirmMsg", "done", 365);
	...
	srvLogWrite( "H0", "01", "04", "00", "", iWidth+"x"+iHeight ); // jrj 로그 >> 튜토리얼
	...
	$(window).on('resizeEnd',function() {
				...
				if (iWidth_ > 1920 || iHeight_ > 1080 || iWidth_ < 800
					...
					if (warnMsg == 1) {
						...
						alert("함께하는지도 화면으로 돌아가겠습니다.");
						location.href = $communityMapCommon.edu_url+"/together_list";
					}
				}
			});
//	apiLogWrite2('', '', '함께하는지도', '없음', '00', '없음'); //"(NULL)", "(NULL)", "(NULL)"
	$("#tuto_start_btn").hide();
	var posi = "";
	var width = 0;
	var height = 0;
	...
	if(urlPath == '/view/community/intro?tutorial_mode'){
		...
		$("#tutorialText").append("<div class=\"title\">"
				+ "<p><span style=\"margin-left:5px;\">함께하는지도 첫 사용을 환영합니다!</span></p>"
				+ "</div>"
				+ "<div class=\"content\">"
...
//		fullMode(document.documentElement);	
		...
		$("#comm_8").click(function(){
			$("#comm_8").hide();
			posiInfo(-1);
			location.href = $communityMapCommon.edu_url+"/together_list?tutorial_mode_2";
		});	
		...
		$("#comm_32").click(function(){
			...
			$("#tutorialText .title, #tutorialText .content, #tutorialText #next").empty();
			$("#tutorialText").append("<div class=\"title\">"
					+ "<p><span style=\"margin-left:5px;\">함께하는지도 튜토리얼을 마치신것을 축하합니다.!</span></p>"
					+ "</div>"
					+ "<div class=\"content\">"
					+ "<p><strong style=\"color:#ff0000;  font-weight:bold;\">(10초 후에 '함께하는지도' 으로 이동합니다.)</p></div>");
			posiInfo(-1);
			setTimeout(function() {
				location.href = $communityMapCommon.edu_url+"/together_list";
			}, 10000);
