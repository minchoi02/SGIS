...
//							//params.region = "sgg";
		...
		setTotSurvData : function(p_map, p_region, p_type, p_sido_cd, p_sgg_cd, p_emdong_cd, p_surv_id, p_callback) {			
			//변수 선언
				...
				//api
				if($totSurvMap.ui.selectedC1) l2 = $totSurvMap.ui.selectedC1;
				else l2 = "000";
				var api_params = {
			...
			$.ajax({
				...
				success: function(res) {
					...
					//정보 저장
					$totSurvMap.ui.mapStatsData[res[0].tbl_id] = result;
					*/
					
					var itm_id_list = api_params.char_itm_id_list.split(',');
					var res2 = [];
					var isNew = true;
					if(itm_id_list.length>1){
						for(var i = 0; i< res.length; i++){
							if(res[i].OV_L1_KOR != "읍부" && res[i].OV_L1_KOR != "동부" && res[i].OV_L1_KOR != "면부") {
								if(res[i].OV_L1_ID == '03' || res[i].OV_L1_ID == '04' ||  res[i].OV_L1_ID == '05'){
									continue;
								}
								isNew = true;
								for(var j = 0; j<res2.length; j++){
									if(res2[j].adm_cd==res[i].OV_L1_ID){
										res2[j].dt = Number(res2[j].dt) + Number(res[i].DTVAL_CO)
										isNew = false;
										break;
									}
								}
								if(isNew){
									res2.push({
										dt : Number(res[i].DTVAL_CO) ,
										unit_nm : '가구',
										region_nm : res[i].OV_L1_KOR,
										surv_id : res[i].TBL_ID,
										adm_cd : res[i].OV_L1_ID,
										itm_nm : res[i].CHAR_ITM_NM
									});
								}
							}
						}
						...
						for(var i = 0; i< res.length; i++){
							if(res[i].OV_L1_KOR != "읍부" && res[i].OV_L1_KOR != "동부" && res[i].OV_L1_KOR != "면부") {
								if(res[i].ADM_KOR == '읍부' || res[i].ADM_KOR == '면부' ||  res[i].ADM_KOR == '동부'){
									continue;
								}
								res2.push({
									dt : res[i].DTVAL_CO ,
					...
					$totSurvMap.ui.mapStatsData[lv_tot_surv_id+"_"+p_type+"_"+p_region+"_"+lv_adm_cd] = res2;
					//콜백함수 호출
