...
(function(W, D) {
	...
	$technicalBizMapApi.request = {
			...
			openApiSggCompanyCnt : function(themeCd, themeNm, map, type, callback) {
				...
				// mng_s 20190403 김건민
				if(type == '0'){
					srvLogWrite('G2', '09', '02', '00', '', '사업체');
				}else{
					srvLogWrite('G2', '09', '02', '00', '', '종사자');
				}
				// mng_e 20190403 김건민
			...
			openApiGetAllLq : function (themeCd,themeNm,type,base_region,year,map,standard,callback){
				...
				// mng_s 20190404 김건민
				if(standard == 'company'){
					srvLogWrite('G2', '11', '02', '00', '', '업종별 입지계수 사업체');
				}else{
					srvLogWrite('G2', '11', '02', '00', '', '업종별 입지계수 종사자');
				}
				// mng_e 20190404 김건민
			},
			...
			openApiGetSggLq : function(params,themeCd,map,callback){
				...
				// mng_s 20190402 김건민
				if(map.zoom == '2'){
					apiLogWrite2("T0", "T07", "시도 입지계수 조회", '시도 입지계수 조회', map.zoom, "전국");
					srvLogWrite('G2', '11', '06', '00', '', '시도 입지계수 조회');
				}else{
					apiLogWrite2("T0", "T07", "시군구 입지계수 조회", '시군구 입지계수 조회', map.zoom, "전국");
					srvLogWrite('G2', '11', '07', '00', '', '시군구 입지계수 조회');
				}
				// mng_e 20190402 김건민
			...
			openApiPoiCompanyDensity : function(type, params, map, callback) {
				...
				// mng_s 20190403 김건민
				if(type == '2'){
					srvLogWrite('G2', '10', '05', '00', '', '시도 업종밀집도 조회');
				}else if(type == '3'){
					srvLogWrite('G2', '10', '06', '00', '', '시군구 업종밀집도 조회');
				}
				// mng_e 20190403 김건민
			...
			openApiTechSidoCompanyInfo : function(map) {
				...
				srvLogWrite('G2', '08', '02', '00', '', '시도별 현황 사업체');
			...
			openApiTechSidoWorkerInfo : function(map){
				apiLogWrite2("T0", "T01", "시도별 기술업종 현황", "bnd_year=" + map.bnd_year , map.zoom, "전국");
				srvLogWrite('G2', '08', '02', '00', '', '시도별 현황 종사자');
			...
			openApiSupplyRegionInfo : function(params, map, callback) {
				...
				if(params.menuType== "supply"){
					if(params.type == '2'){
						...
						srvLogWrite('G2', '13', '03', '00', '', '시도 지원시설 조회');
					}else if(params.type == '3'){
						apiLogWrite2("T0", "T04", "지원시설 조회", "menuType="+ params.menuType + "&year="+ params.year, map.zoom, "전국");
						srvLogWrite('G2', '13', '04', '00', '', '시군구 지원시설 조회');
					}
					//apiLogWrite2("T0", "T04", "지원시설 조회", "menuType="+ params.menuType + "&year="+ params.year, map.zoom, "전국");
					...
					if(params.type == '2'){
						...
						srvLogWrite('G2', '14', '03', '00', '', '시도 산업단지 조회');
					}else if(params.type == '3'){
						apiLogWrite2("T0", "T05", "산업단지 조회", "menuType="+ params.menuType + "&year="+ params.year, map.zoom, "전국");
						srvLogWrite('G2', '14', '04', '00', '', '시군구 산업단지 조회');
					}
									...
									map.geojson.eachLayer(function(layer) {
										...
										for (var i=0; i<result.length; i++) {
											...
											if (tmpResult.sido_cd == layer.feature.properties.adm_cd) {
												...
												console.log(tmpResult.techbiz_list.length);
							...
							case 0:
								...
								for (var i=0; i<result.length; i++) {
									...
									if (result[i].lct_type == "4") {//창업투자회사
										...
										html +=	"<a href='"+result[i].url+"' target='_blank' class='btnType02 w200' title='새창으로 열림' style='display:none;'>벤처창업입지114 이동</a>";
									}
