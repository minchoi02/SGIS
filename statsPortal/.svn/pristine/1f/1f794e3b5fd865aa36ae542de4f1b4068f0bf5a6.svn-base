...
(function(W, D) {
	...
	$vjMap.ui = {
		...
		curJobLocationMarker : null,		// 구인정보상의 근무지 위치 마커
		callbackForMarker : null,			// 마커에 대한 상세정보를 확인할 수 있는 콜백함수
		jobCountLayer : null,				// 시도별 전체 구인건수에 대한 마커 레이어 - 2019.01.30	ywKim	추가

		/**
		 * @name         : markerTooltip_click
		 * @description  : 마커의 툴팁을 클릭시 호출됨
		 * 					구인정보 상세화면으로 이동
		 * @date         : 2018. 12. 14
		 * @author	     : ywKim
		 * @history 	 :
		 */
		markerTooltip_click : function(pSender, pJoNo, pPtX, pPtY) {// used
			$workRoad.ui.log("$vjMap.ui.markerTooltip_click - begin");

			$(pSender).closest(".sop-infowindow").remove();

			if (typeof $vjMap.ui.callbackForMarker == "function") {
				$vjMap.ui.callbackForMarker(pJoNo, pPtX, pPtY);
			}
		},
		/**
		 * @name         : clearJobLocationMarker
		 * @description  :
		 * @date         : 2019.01.25
		 * @author	     : ywKim
		 * @history 	 :
		 */
		clearAllJobLocationMarker : function() {// used
			$workRoad.ui.log("$vjMap.ui.clearAllJobLocationMarker - begin");
		...
		/**
		 * @name         : markAllJobLocation
		 * @description  : 구인정보 목록의 모든 근무지 위치에 마킹
		 * 					다수일때 속도 느림
		 * @date         : 2018. 10. 30.
		 * @author	     : ywKim
		 * @history 	 :
		 * 	2019.01.30	ywKim	변경: 지도 중심 이동 및 줌 설정 기능 추가
		 * @params	pDataList	: [[pt_x, pt_y] ... ]
		 *			pSidoCd		: 선택된 시도 코드
		 *			pSggCd		: 선택된 시군구 코드
		 *			pX			: 선택된 행정구역의 지도 중심 좌표
		 *			pY			: 선택된 행정구역의 지도 중심 좌표
		 */
		markAllJobLocation : function(pDataList, pSidoCd, pSggCd, pX, pY, pCallback) {// used
			$workRoad.ui.log("$vjMap.ui.markAllJobLocation - begin");

			$vjMap.ui.clearJobCountMarker();
			$vjMap.ui.clearAllJobLocationMarker();

			$vjMap.ui.callbackForMarker = pCallback;

			var map = this.mapList[this.curMapId];
			var markerList = this.mapList[this.curMapId].markers;
//			20190313 손원웅 수정_잘못 된 위치로 맵 이동 함...
//			// 지도 중심 이동 및 줌 설정 - 2019.01.30	ywKim	 추가
//			if (pX != null && pY != null) {
//				if (pSggCd != null && pSggCd != "999") {
//					map.mapMove([pX, pY], 7);	// 군, 구
//				}
//				else if (pSidoCd != null) {
//					var doAreaCode = ["31", "32", "33", "34", "35", "36", "37", "38"];// 경기도,강원도,충청남북도,전라남북도,경상남북도
//					if (doAreaCode.indexOf(pSidoCd) >= 0) {
//						map.mapMove([pX, pY], 3);	// 시
//					} else {
//						map.mapMove([pX, pY], 5);	// 도
//					}
//				}
//			}
			map.mapMove([0, 0], 13);	// 2020.07.23[한광희] 사람인 추가로 인한 조회 수정
			
			for(var  i = 0; i < pDataList.length; i ++) {
				var data = pDataList[i];

				if (data.pt_x == undefined || data.pt_y == undefined) {
					console.log("★★★ Warning ★★★");
					console.log("$vjMap.ui.markAllJobLocation(...) : 구인정보의 위치정보를 알 수 없습니다.(pt_x or pt_y is undefined.) (" + data.jo_no + "," + data.corp_nm);
					continue;
				}

				var myIcon = sop.icon({
	                iconUrl: "/images/workRoad/marker/markerIcon_01.png", // 20200413 이금은 마커아이콘 변경
	                iconSize:     [43, 58],	                
	                iconAnchor:   [22, 53],
				...
				var contents = "";
				contents += "<table style='text-align:left;width:auto;white-space: nowrap;word-break:break-all;padding:5px;'>";
				contents += "	<tbody>";
				contents += "		<tr>";
				contents += "			<th style='text-align: left; word-break:break-all;width:30%;padding:5px;color: #3792de;font-size:14px; cursor: pointer;' onclick='javascript:$vjMap.ui.markerTooltip_click(this, \"" + data.jo_no + "\", " + data.pt_x + ", " + data.pt_y + ");'>";
				contents += "				<strong>" + data.corp_nm + "</strong>";
				contents += "			</th>";
				contents += "			<td></td>";
				contents += "		</tr>";
				contents += "	</tbody>";
	            ...
	            var marker = sop.marker([data.pt_x, data.pt_y], {icon: myIcon});

				marker.usrJo_no = data.jo_no;
				marker.usrPt_x = data.pt_x;
				marker.usrPt_y = data.pt_y;
				marker.bindInfoWindow(contents);
				markerList.addLayer(marker);

//				marker.on("click",function(e){
//					if (typeof pCallback == 'function') {
//						pCallback(this.usrJo_no, this.usrPt_x, this.usrPt_y);
//					}
//                });
...
//				});
		 ...
		 * @history 	 :
		 */
		clearJobLocation : function() {// used
			$workRoad.ui.log("$vjMap.ui.clearJobLocation - begin");

			var map = this.mapList[this.curMapId];

			if ($vjMap.ui.curJobLocationMarker != null) {
				map.gMap.removeLayer($vjMap.ui.curJobLocationMarker);
				$vjMap.ui.curJobLocationMarker = null;
			}
		},
		/**
		 * @name         : markJobLocation
		 * @description  : 구인정보상의 근무지 위치에 마킹
		 * @date         : 2018. 10. 30
		 * @author	     : ywKim
		 * @history 	 :
		 * @params	pPosition	: [x, y]
		 */
		markJobLocation : function (pPosition, pMovingYn) {// used
			$workRoad.ui.log("$vjMap.ui.markJobLocation - begin");

			$vjMap.ui.clearJobCountMarker();
			$vjMap.ui.clearJobLocation();

			var map = this.mapList[this.curMapId];
			//20190218 손원웅_조건 수정(pPosition[0].length -> undefined 로 출력...)
//			if (pPosition != undefined && pPosition.length == 2 && pPosition[0].length > 0 && pPosition[1].length > 0) {
			if (pPosition != undefined && pPosition.length == 2) {
				var myIcon = sop.icon({
	                iconUrl: '/images/workRoad/marker/selected.png',
	//                shadowUrl: '/img/idm/bus-shadow.png',
	                iconSize:     [43, 58],
	//                shadowSize:   [32, 32],
	                iconAnchor:   [22, 53],
	//                shadowAnchor: [5, 0],
	//                popupAnchor:  [0, -100]//[-3, -76]
	            });

	            var marker = sop.marker(pPosition, {icon: myIcon/*, zIndexOffset : 10001*/});
	            marker.addTo(map.gMap);
	            $vjMap.ui.curJobLocationMarker = marker;

	            if (pMovingYn == undefined || pMovingYn == true) {
	            	this.mapList[this.curMapId].mapMove(pPosition, this.mapList[this.curMapId].zoom);
	            }
			} else {
				alert("좌표가 존재하지 않습니다.");
			}
			...
			// 2019.03.13 접근log 생성
			srvLogWrite('D0', '03', '06', '00', '', '');
		},
		/**
		 * @name         : clearJobCountMarker
		 * @description  : 시도별 구인건수 마커 레이어 지우기
		 * @date         : 2019.01.30
		 * @author	     : ywKim
		 * @history 	 :
		 * @params
		 */
		clearJobCountMarker : function() {
			var map = this.mapList[this.curMapId];

			if ($vjMap.ui.jobCountLayer != null) {
				map.gMap.removeLayer($vjMap.ui.jobCountLayer);
				$vjMap.ui.jobCountLayer = null;
			}
		},
		/**
		 * @name         : markJobCount
		 * @description  : 시도별 구인건수 마커 표시
		 * @date         : 2019.01.25
		 * @author	     : ywKim
		 * @history 	 :
		 * @params
		 */
		markJobCount : function(pDataList, pSidoCd, pX, pY, pClick_Callback) {// used
			$workRoad.ui.log("$vjMap.ui.markJobCount - begin");
			...
			/* if (pSidoCd != null) {
				...
				map.mapMove([989674, 1818313], 2);	// 전국
			} */
			/** 2020.07.23[한광희] 사람인 추가로 인한 조회 수정 END */

			$.each(pDataList, function(index, data) {
				var html  = "";
				html += "<div>";
				html +=	    "<span style='text-align:center;width:61px;height:61px;font-size:11px;font-weight:bold;color:#fff;line-height:61px;position:absolute;'>" + data.cnt + "</span>";
				html += 	"<img src='/images/workRoad/marker/markercluster-7.png' style='width:61px;height:61px;' />";
				html += "</div>";
				...
				var pt = [data.x_coor, data.y_coor];
				var marker = sop.marker(pt, {
					icon : markerIcon
				});

				if (pSidoCd == null && data.sido_cd != undefined) {
					marker.sido_cd = data.sido_cd;
				} else if (pSidoCd != null) {
					marker.sido_cd = pSidoCd;
				}
				if (data.sgg_cd != undefined) {
					marker.sgg_cd = data.sgg_cd;
				}
				if (data.x_coor != undefined) {
					marker.x_coor = data.x_coor;
				}
				if (data.y_coor != undefined) {
					marker.y_coor = data.y_coor;
				}

//				marker.info = industryChartData[k];
				tmpMarkerGroup.addLayer(marker);
//				marker.addTo(map.gMap);
				$vjMap.ui.jobCountLayer = tmpMarkerGroup;

				marker.on("click", function() {
					if (typeof pClick_Callback == "function") {
						
						/** 2020.07.23[한광희] 사람인 추가로 인한 조회 수정 START */
						if(this.sgg_cd != undefined){
							$wrmViewJobs.ui.sggPT = [this.x_coor, this.y_coor];
						} else {
							$wrmViewJobs.ui.sidoPT = [this.x_coor, this.y_coor];
						}
						/** 2020.07.23[한광희] 사람인 추가로 인한 조회 수정 END */
						
						pClick_Callback(this.sido_cd, this.sgg_cd, this.x_coor, this.y_coor);
					}
				});
			});
			...
			// 행정구역 영역표시 - 2019.01.30	ywKim	추가
			// ToDo: 색상지도 추가해야함.
			map.openApiBoundarySido("2016", null);
		},
		...
		setDataType : function(mapId, type) {
			$workRoad.ui.addFnc02List("$vjMap.ui.setDataType");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//			}
		...
		createMap : function(id, seq) {// used
			$workRoad.ui.log("$vjMap.ui.createMap - begin");

			var map = new sMap.map();
			map.createMap($vjMap, id, {
				center : [ 989674, 1818313 ],
				zoom : 2, // 9->8->2	2019.01.30	ywKim	 변경
			...
			legend.initialize($vjMap.ui);
			map.legend = legend;
			legend.createLegend();
			legend.legendType = "auto";					// 2018.12.27	ywKim	변경
			//작업부분 끝

			var btnInfo = new vjMapBtnInfo.btnInfo(map, $vjMap.ui);
//					var btnInfo = new interactiveMapBtnInfo.btnInfo(map, $vjMap.ui);
			...
			//공유
//					var shareInfo = new share.shareInfo(map, $vjMap.ui);
...
//					$galleryAdd.delegate = this;
			...
			// 범례 (좌측하단)
			$(".vjMap .sop-bottom.sop-left").css("display", "none");
		...
		doMaxSize : function(type) {// used
			$workRoad.ui.log("$vjMap.ui.doMaxSize - begin");
		...
		doClearMap : function(type, isStatsInfoClear) {
			$workRoad.ui.addFnc02List("$vjMap.ui.doClearMap");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도

//			$vjMap.noReverseGeoCode = false;
...
//			$("#helper_"+viewId).show();
//
//			// 임시 - 2018.10.29	ywKim	주석
////			$mydataDataBoard.ui.delegateSetting($vjMap.ui);			//나의데이터 세팅
////			$publicDataBoard.ui.delegateSetting($vjMap.ui);			//공공데이터 세팅
//
//			// 임시 - 2018.10.29	ywKim	주석
...
//			$interactiveLeftMenu.ui.updateSearchBtnEffect("", this.curMapId);
		...
		doShare : function(type) {
			$workRoad.ui.addFnc02List("$vjMap.ui.doShare");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//			}
		...
		doBookMark : function(type, srvType) {
			$workRoad.ui.addFnc02List("$vjMap.ui.doBookMark");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//
//						//2017.02.22 이미지캡쳐 수정
//						var captureTargetId = ".vjMap #mapRgn_"+type;
//
//						//2016.10.25 lbdms 캡쳐를 위한 정보 수정
//						if (shareData[i].params != undefined) {
//							if (shareData[i].params.mapInfo != undefined) {
...
//				},
		...
		doCombineMap : function(type) {
			$workRoad.ui.addFnc02List("$vjMap.ui.doCombineMap");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//						/*$mydataDataBoard.ui.mapData[map.id].options.rowDataArray.length != 0 || // 임시 - 2018.10.29	ywKim	주석*/
//						markerCnt > 0) {
//						cnt++;
//					}
//				}
...
//			}
		 ...
		 */
		mapLoad : function() {
			$workRoad.ui.addFnc02List("$vjMap.ui.mapLoad");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//
//			}
//			var popup = $vjMap.ui.combinePopup.$combineMap.ui;
//			popup.setData(data);
		},
		 ...
		 */
		reportLoad : function() {
			$workRoad.ui.addFnc02List("$vjMap.ui.reportLoad");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도

//			var map = this.mapList[this.curMapId];
//			var mapType = "interactiveMap";
//			var divId = ".vjMap #mapRgn_" + (map.id + 1);
//
...
//				$workRoad.ui.addFnc02List("$vjMap.ui.getChartSvgData");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//	 							mapData : data
//	 					};
//	 					var popup = $vjMap.ui.reportPopup.$reportForm.ui;
//	 					popup.delegate = $vjMap.ui;
//	 					popup.map = map;
...
//			//==================================================================================================================================//
		...
		reportDataSet : function(type) {
			$workRoad.ui.addFnc02List("$vjMap.ui.reportDataSet");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//				window.open("/js/common/report/reportForm.html", "reportPrint","width=850, height=700, scrollbars=yes");
		...
		doAddMap : function(type) {
			$workRoad.ui.addFnc02List("$vjMap.ui.doAddMap");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도

//			$("#rotationTip,#tuto_start_btn,.grid_radio,.tb_radio").hide();
//
//			var isMap1ContentShow =  $(".vjMap #mapRgn_1").is(":visible");
////			var isMap2ContentShow =  $(".vjMap #mapRgn_2").is(":visible");	// 2018.11.12	ywKim	변경: 맵2,3 사용안함
////			var isMap3ContentShow =  $(".vjMap #mapRgn_3").is(":visible");
...
//			}
//
//			var sceneInx = $(".vjMap .sceneBox.on").length;
//			$(".vjMap .sceneBox").eq(createMapId).show().addClass("on");
//			if(sceneInx==1){
//				$(".vjMap .sceneBox").stop().animate({"width":"50%"},200, function() {
//					for (var i=0; i<$vjMap.ui.mapList.length; i++) {
//						if ($vjMap.ui.mapList[i] != null) {
//							$vjMap.ui.mapList[i].update();
//						}
//					}
//				});
//				$(".vjMap .sceneRela").css({"border-left":"3px solid #000"});
//				$(".vjMap .sceneRela").eq(0).css({"border-left":"0"});
//			}else if(sceneInx==2){
//				$(".vjMap .sceneBox").css({"position":"absolute"});
//				$(".vjMap .sceneBox").stop().animate({"width":"800px", "height":"500px"},200, function() {
//					for (var i=0; i<$vjMap.ui.mapList.length; i++) {
//						if ($vjMap.ui.mapList[i] != null) {
//							$vjMap.ui.mapList[i].update();
//						}
//					}
//				});
//				$(".vjMap .sceneRela").css({"border-left":"0"});
//				$(".vjMap .resizeIcon").show();
//				$(".vjMap .sceneBox").each(function(i){
//					$(this).css({"z-index":parseInt(10-i), "border":"3px solid #333"})
//					.animate({"top":parseInt(50*(i+1))+"px", "left":parseInt(150*(i+1))+"px"},200);
//				});
//				$( ".vjMap .sceneBox" ).draggable({containment: ".containerBox>.rela"}).resizable();
//				$( ".vjMap .sceneBox" ).on("resize", function() {
//					for (var i=0; i<$vjMap.ui.mapList.length; i++) {
//						if ($vjMap.ui.mapList[i] != null) {
//							$vjMap.ui.mapList[i].update();
//						}
...
//			mapNavi.create("mapNavi_" + (createMapId+1), createMapId+1, $vjMap.ui);
//
//			//범례고정 show/hide
//			//9월 서비스
//			for (var i=0; i<this.mapList.length; i++) {
...
////			$(".vjMap .sceneBox.on").each(function(index) {
...
//			//mng_e
		...
		doRemoveMap : function(type) {
			$workRoad.ui.addFnc02List("$vjMap.ui.doRemoveMap");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//			}
//
//			$(".vjMap .sceneBox").eq(this.curMapId).removeClass("on").hide();
//			var sceneInx = $(".vjMap .sceneBox.on").length;
//			if(sceneInx==1){
//				$(".vjMap .sceneBox").stop().animate({"width":"100%"},200, function() {
//					for (var i=0; i<$vjMap.ui.mapList.length; i++) {
//						if ($vjMap.ui.mapList[i] != null) {
//							$vjMap.ui.mapList[i].update();
//						}
...
////				$(".interactiveDataBoard").show();		// 임시 - 2018.10.29	ywKim	주석
//				$(".vjMap .sceneBox.on").eq(0).find(".vjMap .sceneRela").css({"border-left":"0px"});
//				$(".vjMap .sceneBox.on").each(function(index) {
...
//				});
//
//				$(".vjMap .sceneBox").draggable("destroy").resizable("destroy").css({"position":"static", "border":"0", "height":"100%"});
//				$(".vjMap .sceneBox.on").eq(1).find(".vjMap .sceneRela").css({"border-left":"3px solid #000"});
//			}
//
//			//범례고정 show/hide
//			//9월 서비스
...
//
//			$(this).hide();
//			$(".vjMap .resizeIcon").hide();
//			$(".interactiveView").each(function(i){
//				$(this).text("VIEW"+parseInt(i+1));
//			});
//
//			var isMap1ContentShow =  $(".vjMap #mapRgn_1").is(":visible");
//			var isMap2ContentShow =  $(".vjMap #mapRgn_2").is(":visible");
//			var isMap3ContentShow =  $(".vjMap #mapRgn_3").is(":visible");
//
//			if (isMap1ContentShow & isMap2ContentShow & isMap3ContentShow) {
//				messageAlert.open("알림", "지도는 3개까지만 생성할 수 있습니다.")
...
////			$interactiveDataBoard.ui.reDraw(this.curMapId);
		...
		doAnalysisShareInfo : function (type, data) {
			$workRoad.ui.addFnc02List("$vjMap.ui.doAnalysisShareInfo");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도

//			if (type.length == 0 || data.length == 0) {
//				var mapNavi1 = new mapNavigation.UI();
//				mapNavi1.firstBoolean = true;
//				mapNavi1.create("mapNavi_1", 1, $vjMap.ui);
//
...
//					$vjMapApi.request.openApiShareForStats(tmpParams, admCdList[i]);
//				}
//			}
//
//
//			map.curDropCd = shareInfo.param_info.paramInfo.adm_cd;
//			$vjMap.ui.dropBtnInfo[map.id] = $interactiveLeftMenu.ui.arParamList[0];
...
//			return true;
		...
		doDone : function(type) {
			$workRoad.ui.addFnc02List("$vjMap.ui.doDone");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//			$("#"+type).hide();
		...
		doCancel : function(type) {
			$workRoad.ui.addFnc02List("$vjMap.ui.doCancel");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//			$("#"+type).hide();
		...
		updateSearchTitle : function(title, unit, mapId) {
			$workRoad.ui.addFnc02List("$vjMap.ui.updateSearchTitle");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//			$("#title_" + viewId).show();
		...
		shareToKakaoStory : function() {
			$workRoad.ui.addFnc02List("$vjMap.ui.shareToKakaoStory");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//				},
		...
		createInfoTooltip : function(event, data, type, map) {
			$workRoad.ui.addFnc02List("$vjMap.ui.createInfoTooltip");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//						}
//
//						if($vjMap.ui.mapList[0].dataType == "kosis" && $vjMap.ui.mapList[0].dataForCombine[0].UNIT != undefined){
//							html += "<td class='statsData'>"+title+" 0("+$vjMap.ui.mapList[0].dataForCombine[0].UNIT+")</td>";
//						} else {
...
//				.css("padding-left", "5px");
		...
		requestOpenApi : function(options) {
			$workRoad.ui.addFnc02List("$vjMap.ui.requestOpenApi");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//
//			if 	    (api_id == "API_0301") $vjMapApi.request.openApiTotalPopulation(options);
//			else if (api_id == "API_0302") $vjMapApi.request.openApiSearchPopulation(options);
//			else if (api_id == "API_0303") $vjMapApi.request.openApiInderstryCode(options);
//			else if (api_id == "API_0304") $vjMapApi.request.openApiCompany(options);
//			else if (api_id == "API_0305") $vjMapApi.request.openApiHouseHold(options);
//			else if (api_id == "API_0306") $vjMapApi.request.openApiHouse(options);
//			else if (api_id == "API_0307") $vjMapApi.request.openApiFarmHouseHold(options);
//			else if (api_id == "API_0308") $vjMapApi.request.openApiForestryHouseHold(options);
//			else if (api_id == "API_0309") $vjMapApi.request.openApiFisheryHouseHold(options);
//			else if (api_id == "API_0310") $vjMapApi.request.openApiHouseHoldMember(options);
//			else if (api_id == "API_4011") $vjMapApi.request.openApiItemCombine(options);
//			pageCallReg();
		...
		requestGridLegend : function(options) {
			$workRoad.ui.addFnc02List("$vjMap.ui.requestGridLegend");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//			$vjMapApi.request.gridLegendTotalPopulation(options);
//
//			pageCallReg(); //그리드 페이지 통계
		...
		reqSetParams : function (tmpParam, adm_cd, adm_nm, api_id, map, type) {
			$workRoad.ui.addFnc02List("$vjMap.ui.reqSetParams");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
$vjMap.callbackFunc = {
		...
		didMapMoveStart : function(event, map) {// used
			$workRoad.ui.log("$vjMap.callbackFunc.didMapMoveStart - begin");
		},

		// 맵이동 종료시, 콜백 호출
		didMapMoveEnd : function(event, map) {// used
			$workRoad.ui.log("$vjMap.callbackFunc.didMapMoveEnd - begin");

//			console.log("[interactiveMap.js] didMapMoveEnd 호출");
//			console.log("[interactiveMap.js] didMapMoveEnd() $vjMap.ui.dropBtnInfo[map.id] [" + $vjMap.ui.dropBtnInfo[map.id]);
//
...
//				if($vjMap.ui.dropBtnInfo[map.id] != null && $vjMap.ui.dropBtnInfo[map.id]!=undefined) {
...
//			}
		...
		didMapZoomStart : function(event, map) {
			$workRoad.ui.addFnc02List("$vjMap.callbackFunc.didMapZoomStart");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
		},

		// 맵 줌 종료 시, 콜백 호출
		didMapZoomEnd : function(event, map) {// used
			$workRoad.ui.log("$vjMap.ui.didMapZoomEnd - begin");
			$workRoad.ui.log("$vjMap.ui.didMapZoomEnd - map.zoom [" + map.zoom + "]");
...
////			}
		...
		didMapDropEnd : function(event, source, layer, data, map) {
			$workRoad.ui.addFnc02List("$vjMap.callbackFunc.didMapDropEnd");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//				// share정보 초기화
//				$vjMap.noReverseGeoCode = false;
//				$vjMap.ui.curMapId = map.id;
//				$vjMapApi.request.combineFailCnt = 0;
//
//				//공유 (나의 데이터에서는 shareInfo가 null)
//				var shareInfo = new share.shareInfo(map, $vjMap.ui); //share.shareInfo()는 bookmarkAndShareInfo.js에 있는듯 하다.
//				map.shareInfo = shareInfo;
//				map.shareInfo.shareUrlInfo = [];
//				map.dropInfo = null;
//
//				if ($vjMap.ui.curDropParams == undefined) {
//					$vjMap.ui.curDropParams = [];
//				}
//
...
//					}
//
//					$vjMap.ui.setDataType(map.id, "kosis");
//					$vjMap.ui.dropBtnInfo[map.id] = tmpParamList[i];
//					$vjMap.ui.dropBtnInfo[map.id]["isKosis"] = true;
...
//					$vjMap.ui.setDataType(map.id, "census");
//					$vjMap.ui.searchBtnType = "normal";
//					api_id = id.split("-")[0];
//					index = id.split("-")[1];
//					$interactiveLeftMenu.ui.updateSearchBtnEffect(id, map.id);
//
...
//									if (tmpParam != null) {
//										var params = $vjMap.ui.reqSetParams(tmpParam, admList[i], adm_nm, api_id, map, "gibgae");
//										$vjMap.ui.curDropParams[map.id] = params;
//										$vjMap.ui.requestOpenApi(params);
//									}
//								}
//							}
//
...
//
//								if (tmpParam != null) {
//									var params = $vjMap.ui.reqSetParams(tmpParam, adm_cd, adm_nm, api_id, map);
//									$vjMap.ui.curDropParams[map.id] = params;
//									$vjMap.ui.requestOpenApi(params);
//								}
//							}
...
//				} else {
//					if (tmpParam != null) {
//						var params = $vjMap.ui.reqSetParams(tmpParam, data.adm_cd, data.adm_nm, api_id, map);
//
//						//시계열 초기값 세팅
////						$interactiveDataBoard.ui.timeSeriesInit(params);	// 임시 - 2018.10.29	ywKim	주석
//						$vjMap.ui.curDropParams[map.id] = params;
//						$vjMap.ui.requestOpenApi(params);
//					}
...
//			}
		...
		didMapDoubleClick : function(btn_id, tmpParam) {
			$workRoad.ui.addFnc02List("$vjMap.callbackFunc.didMapDoubleClick");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//
//			var map = $vjMap.ui.mapList[$vjMap.ui.curMapId];
//			var index = null;
//			var adm_cd = "";
//			var adm_name= "";
//			var api_id = "";
//
//			console.log("[interactiveMap.js] didMapDoubleClick() $vjMap.ui.dropBtnInfo[map.id] [" + $vjMap.ui.dropBtnInfo[map.id]);
//
//			// share정보 초기화
//			$vjMap.noReverseGeoCode = false;
//			$vjMap.ui.curMapId = map.id;
...
//				// 버튼 시각 효과
//				$interactiveLeftMenu.ui.updateSearchBtnEffect(id, map.id);
//				$vjMap.ui.dropBtnInfo[map.id] = tmpParam;
//				$vjMap.ui.setDataType(map.id, "census");
//			}
//
//			if (map.selectedBoundMode == "multi") {
...
//
//					if ( map.isInnerMapShow2 ) {
//						$vjMap.ui.requestGridLegend(params); //mng_s 그리드 범례요청
//					}
//				}
//				map.setBoundSelectedMoode(null);
//				map.mapBtnInfo.setFixedBoundBtn(false);
...
//			}
		...
		didMouseOverPolygon : function(event, data, type, map) {// used
			$workRoad.ui.log("$vjMap.callbackFunc.didMouseOverPolygon - begin");
//			$workRoad.ui.addFnc02List("$vjMap.callbackFunc.didMouseOverPolygon");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//					if (type == "data") {
////							$interactiveDataBoard.ui.selectChartData(data.properties, data.dataIdx, map.id);// 임시 - 2018.10.29	ywKim	주석
//							map.legend.selectLegendRangeData(event.target.options.fillColor);
//					}
//					if (data.info.length > 0) { //데이터가 있을 경우만 툴팁을 보여준다.
//						$vjMap.ui.createInfoTooltip(event, data, type, map);
//					}
...
//			}
		...
		didMouseOutPolygon : function(event, data, type, map) {// used
			$workRoad.ui.log("$vjMap.callbackFunc.didMouseOutPolygon - begin");
//			$workRoad.ui.addFnc02List("$vjMap.callbackFunc.didMouseOutPolygon");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//			}
		...
		didSelectedPolygon : function(event, data, type, map) {
			$workRoad.ui.addFnc02List("$vjMap.callbackFunc.didSelectedPolygon");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도

//			if (type == "data") {
//
//			}else if (type == "build") {
//				if ($vjMap.ui.buildPopup != null) {
//					$vjMap.ui.buildPopup.close();
//				}
//
//				var top = $(".vjMap #mapRgn_"+(map.id+1)).offset().top + 100;
//				var left = $(".vjMap #mapRgn_"+(map.id+1)).offset().left +
//						   $(".vjMap #mapRgn_"+(map.id+1)).width()/2 - 400/2;
//
//				$vjMap.ui.buildPopup =
//					window.open(
...
//			}
		...
		didDrawCreate : function(event, type, map) {
			$workRoad.ui.addFnc02List("$vjMap.callbackFunc.didDrawCreate");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//				});
//			}else {
//				$vjMapApi.request.userAreaBoundInfo(area, type, map.curPolygonCode, event, map);
//			}
		...
		didFinishedMultidata : function(dataList, admCdList, map) {
			$workRoad.ui.addFnc02List("$vjMap.callbackFunc.didFinishedMultidata");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//			}
//
////			$interactiveDataBoard.ui.updateDataBoardMulti(dataList, options);// 임시 - 2018.10.29	ywKim	주석
		}
...
$vjMap.event = {
		...
		setUIEvent : function() {// used
			$workRoad.ui.log("$vjMap.ui.setUIEvent - begin");

//					Kakao.init('167fc6abf0eb4717e1f3de7895a0152a');

			$("#openShare").click(function() {
				$workRoad.ui.addFnc02List("$vjMap.event.setUIEvent.[#openShare]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
...
//					$(".vjMap .sceneBox.on").each(function() {
//						var id = parseInt($(this).attr("id").split("view")[1])-1;
//						tmpView.push(id);
//						if (id == $vjMap.ui.curMapId) {
//							isSameView = true;
//						}
//					});
//
...
//							$vjMap.ui.curMapId = tmpView[0];
//						}else {
//							$vjMap.ui.curMapId = tmpView[1];
//						}
//
//						var id = "view" + ($vjMap.ui.curMapId + 1);
//						switch($vjMap.ui.curMapId) {
//							case 0:
//								$("#"+id).find(".vjMap .toolBar").css("background", "#0070c0");
//								break;
//							case 1:
//								$("#"+id).find(".vjMap .toolBar").css("background", "#9ed563");
//								break;
//							case 2:
//								$("#"+id).find(".vjMap .toolBar").css("background", "#ff0066");
//								break;
//						}
//					}
//				}
//		    });
			...
			// 맵 영역 (인터렉티브 바, 툴바, 지도) 클릭시
			// 멀티맵 기능 (활성 맵 선택 표시)
			$(".vjMap .sceneBox").click(function(){// used
				$workRoad.ui.log('$vjMap.event.setUIEvent.[$(".vjMap .sceneBox").click] - begin');

//				var sceneInx = $(".vjMap .sceneBox.on").length;
//				var id = $(this).attr("id");
//				if (sceneInx > 1) {
//					if (!isClose) {
//						$(".vjMap .sceneBox").find(".vjMap .toolBar").css("background", "#ffffff");
//					}
//					if (id == "view1") {
//						$vjMap.ui.curMapId = 0;
//						$(this).find(".vjMap .toolBar").css("background", "#0070c0");
//					}else if (id == "view2") {
//						$vjMap.ui.curMapId = 1;
//						$(this).find(".vjMap .toolBar").css("background", "#9ed563");
//					}else {
//						$vjMap.ui.curMapId = 2;
//						$(this).find(".vjMap .toolBar").css("background", "#ff0066");
//					}
//					$(".vjMap .sceneBox").find(".tb_mapAdd").parent().show();
//
//					if (sceneInx == 3) {
//						$(".vjMap .sceneBox").find(".tb_mapAdd").parent().hide();
//						$(".vjMap .sceneBox").css({"z-index":"8", "border":"2px solid #333"});
//						$(this).css({"z-index":"10"});
//
//					}
//				}else {
...
//				isClose = false;
//
//				var curMapId = $vjMap.ui.curMapId;
//				var dataType = $vjMap.ui.dataTypeList[curMapId];		//census, kosis, publicData, userData
//
//				// 임시 - 2018.10.29	ywKim	주석
////				$mydataDataBoard.ui.delegateSetting($vjMap.ui);			//나의데이터 세팅
////				$publicDataBoard.ui.delegateSetting($vjMap.ui);			//공공데이터 세팅
...
//				    });
			...
			$("#dataSlider_item").slider({
		        ...
		        slide: function( event, ui ) {  //ui.value
					$workRoad.ui.addFnc02List('$vjMap.event.setUIEvent.[$("#dataSlider_item").slider]');	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
