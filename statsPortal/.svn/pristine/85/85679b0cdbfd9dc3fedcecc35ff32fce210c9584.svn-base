...
public class SearchListPoi extends AbsQuery< List > {
	
	private static final Log logger = LogFactory.getLog( SearchListPoi.class );
	...
	public List executeAPI( HttpServletRequest req, HttpServletResponse res, String trId ) throws AbsException
	{
		...
		try
		{
			...
			if(mapParameter.containsKey("sufid")) {
				if(mapParameter.get("sufid") != null) {
					String sufids[] = mapParameter.get("sufid").toString().split(",");				
					if(sufids.length > 0) {
						mapParameter.put("sufid", sufids);
					}
				}
			}
	...
	{
		return "catchmentArea.selectSearchListPoi";
	}
