...
(function(W, D) {
	...
	$(document).ready(function() {
		...
		//mng_s 20210514 이진호
		//대화형통계지도 > 공공데이터 > 데이터보드 > 임의영역 반경 내 주요 정보 목록 > 출처년도를 동적으로 변경되게 수정 (common.js의 센서스 데이터 년도와 사업체 년도)
		$("#userAreaDiv").children('.origin_txt_2').html('출처 : 통계청 , 인구주택총조사,' + dataYear +' / 통계청, 전국사업체조사, ' + companyDataYear);
		//mng_e 20210514 이진호
	...
	$publicDataBoard.ui = {
			...
			updatePublicData : function(type) {
				...
				if (type == "cctv") {
					...
					this.map.markers2.clearLayers();		//클러스트 해지함. leekh 20190314 주석 풀어서 복구
					setTimeout(function() {
					...
					if(this.map.zoom != 10) {
						...
						this.map.markers.clearLayers();
						this.map.markers2.clearLayers();		//클러스트 해지함. leekh 20190314 주석처리해서 복구
			...
			reqPoi : function() {
				...
				//mng_s 20210401 이진호
				//사업체POI가 지도에 존재하면 사업체 POI를 지운 후 공공데이터 POI 생성
				var btn_id = $interactiveMap.ui.mapList[$interactiveMap.ui.curMapId].mapBtnInfo.id;
				$("#poiInit_"+btn_id).trigger("click"); // 지도초기화 버튼 트리거
				//mng_e 20210401 이진호
			...
			requestMetroPplninfo : function () {
				// 20190401 leekh 대화형통계지도 공공데이터 지하철 승하차 인구 정보 관련 로그 삽입. 혹시 다른곳에서 이부분에 로그 삽입이 있다면 url로 분기처리 해야함
				srvLogWrite("C0", "02", "06", "01", "", "");
			...
			metroWeekPplnChartDraw : function(result) {
				...
				//mng_s 20210323 이진호
				//요일평균 승하차인원 정보가 나오지 않는 오류 수정
				sopPortalMetroWeekPplnDrawObj.addParam("station_nm", result.target.info.station_nm);
				//mng_e 20210323 이진호
			...
			reset : function(map_id) {
				...
				if (mapData.type != "cctv") {
					for(var i = 0; i < mapData.options.markerGroup.length; i ++) {
						...
						//mng_s 20210331 이진호
						//버스정류장 POI를 생성하고 초기화 버튼을 눌러도 POI가 지워지지 않는 현상 수정
	...
	$class("sop.portal.metroPplnPoi.api").extend(sop.portal.absAPI).define({
		onSuccess : function (status, res, options) {
			...
			if (res.errCd == "0") {
				...
				//초기화
				//mng_s 20210323 이진호
				//초기화 수행후 mapData type이 metro로 되도록 수정
				$publicDataBoard.ui.reset(map.id);
				$publicDataBoard.ui.mapData[$publicDataBoard.ui.map_id].type = "metro";
		...
		onSuccess : function (status, res, options) {
			...
			if (res.errCd == "0") {
				...
				for(var i = 0; i < result.length; i ++) {
					...
					_marker.info = result[i];
					_marker.addTo(map.markers2);		//클러스트 해지함. leekh 20190314 클러스트 복구시 markers로 수정		
					...
					_marker.on("click", function(result) {
						...
						$publicDataBoard.ui.circleInfo(result);
						//인포윈도우 열기
					//	_marker.openInfoWindow();				//클러스트 해지함. leekh 20190314 주석 풀어서 복
						//결과 리스트 저장
						publicDataBoard.options.result = result;
					});
