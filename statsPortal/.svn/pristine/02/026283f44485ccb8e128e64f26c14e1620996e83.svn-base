	...
	function mapClick(){
		...
		body.on("click",".sop-interactive",function(){ 
			...
			if(!ck){
				...
				if(!full){
					$(".interactiveDataBoard").addClass("on").stop().animate({"right":"480px"},200);
				}else{
...
function stepCloseAnimate(inx) {
	console.log("stepCloseAnimate inx = " + inx);
    var time = 200;
    var fx = '.quickBox';
    ...
    $(fx).queue("step02", function() {
        ...
        $('.interactiveBar h3').animate({ "left": "0" }, time);
...
function eventLeftAttach() {
	
	console.log("123123123123123eventLeftAttach");
    var body = $("body");
    ...
    body.on("click", ".sideQuick.sq02", function() {
    	console.log(".sideQuick.sq02 click");
        var on = $(this).hasClass("on");
        console.log("on = " + on);
        if (!on) {
        	console.log("on");
...
//    body.off("click", ".stepBox label");
    body.on("click", ".stepBox label", function() {
    	console.log("stepBox label click");
        var cls = $(this).parents("ul").eq(0).attr("class");
        if (cls != "dbTypeCk") {
            var ck = $(this).hasClass("on");
            if (!ck) {
