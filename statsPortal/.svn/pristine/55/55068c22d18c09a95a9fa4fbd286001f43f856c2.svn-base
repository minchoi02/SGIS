					...
					if(res[0].ADM_CD == "00") {
						...
						for(var i=0; i<res.length; i++) {
							if($totSurvMain.ui.selectedArea.length == 2) {
								$totSurvMap.ui.checkIsAtdrc(res[i].ADM_CD);
				...
				if (type == "data") {
					if(data.info[0].adm_cd.length == 7 ){
						...
						if($totSurvMap.ui.mapToggleId != data.info[0].adm_cd) {
							...
							else if(selectedThema == "경제"){ //2021-09-06 [이영호] 경총 추가
								$ecnmyDash.polygonSelectArea = data.properties.adm_cd; 
							}
							...
							// 선택한 지역 하이라이트
							// 2020-11-02 [곽제욱] 타일차트 색변경 추가 START
							if(selectedThema == "경제") {		// 2021-09-08 [이영호] 경총 지도 선택시 타일차트 하이라이트
								//원래 색으로 초기화
								for(var i=0; i<$totSurvMap.ui.mapStatsData[$totSurvMap.ui.selectedTblId].length; i++) {
									for(var j=0; j<$totSurvMain.ui.tilePerColor.length; j++){
										if($totSurvMain.ui.tilePerColor[j].adm_cd == $totSurvMap.ui.mapStatsData[$totSurvMap.ui.selectedTblId][i].adm_cd){
											$("#areaEcnmyY2015 g.highcharts-level-group-1 rect:eq(" + i +")").attr("fill", $totSurvMain.ui.tilePerColor[j].color);
										}
									}
								}
								//선택된 지역 하이라이트
								if(data.info[0].adm_cd != "" && data.info[0].adm_cd != "00") {
									for(var i=0; i<$totSurvMap.ui.mapStatsData[$totSurvMap.ui.selectedTblId].length; i++) {
										if($totSurvMap.ui.mapStatsData[$totSurvMap.ui.selectedTblId][i].adm_cd == data.info[0].adm_cd) {
											$("#areaEcnmyY2015 g.highcharts-level-group-1 rect:eq(" + i +")").attr("fill", "#0086c6")
										}
									}
								}
							} else {
							...
							if($totSurvMap.ui.mapToggleId != "00") {
								if(selectedThema == "경제") {		// 2021-09-08 [이영호] 경총 지도 선택시 타일차트 하이라이트
									if(data.info[0].adm_cd != "" && data.info[0].adm_cd != "00") {
										for(var i=0; i<$totSurvMap.ui.mapStatsData[$totSurvMap.ui.selectedTblId].length; i++) {
											if($totSurvMap.ui.mapStatsData[$totSurvMap.ui.selectedTblId][i].adm_cd == $totSurvMap.ui.mapToggleId) {
												for(var j=0; j<$totSurvMain.ui.tilePerColor.length; j++){
													if($totSurvMain.ui.tilePerColor[j].adm_cd == $totSurvMap.ui.mapToggleId){
														$("#areaEcnmyY2015 g.highcharts-level-group-1 rect:eq(" + i +")").attr("fill", $totSurvMain.ui.tilePerColor[j].color);
													}
												}
											}
							...
							if(selectedThema == "경제"){ //2021-09-06 [이영호] 경총 추가								
