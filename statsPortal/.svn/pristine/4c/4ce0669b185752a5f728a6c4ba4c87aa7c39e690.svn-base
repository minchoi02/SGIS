...
(function(W, D) {
	...
	$policyWritePopup.ui = {
			...
			goMyDataPage : function() {
				srvLogWrite( "E0", "05", "01", "00", "", "" ); // jrj 로그 > 나의 데이터 만들기
...
//							$("#pSggList option:eq("+index+")").before($("<option/>",{text:sidoNode.sgg_nm,value:sidoNode.sgg_list.join(","),"x-coor":op.data("coor-x"),"y-coor":op.data("coor-y"), "admCd":sidoNode.adm_cd}));
							$("#pSggList option:eq("+index+")").before($("<option/>",{text:sidoNode.sgg_nm,value:sidoNode.sgg_list.join(","),"x-coor":sidoNode.x_coor,"y-coor":sidoNode.y_coor, "admCd":sidoNode.adm_cd}));
						}
					});
				}
			},
			...
			/**
			 * @name             : setAutonomouseRegionList
			 * @description      : 자치구정보를 설정한다.
			 * @date             : 2019.01.03
			 * @author	         : ywKim
			 * @history 	     :
			 * @param sidoCd     : 시도코드
			 */
			setAutonomouseRegionList : function(sidoCd) {
				$("#pSggList").empty();
				
				if($psmCombine.ui.atdrcList[sidoCd]){
					$.each($psmCombine.ui.atdrcList[sidoCd], function(sidoCnt, sidoNode){
						$("#pSggList").append($("<option/>", {
							text:sidoNode.sgg_nm,
							value:sidoNode.sgg_list.join(","),
							"x-coor":sidoNode.x_coor,
							"y-coor":sidoNode.y_coor,
							"admCd":sidoNode.adm_cd}));
					});
				}
				
				var sort = $("#pSggList option").sort(function(a, b) {
					return a.text > b.text ? 1 : -1; 
				});
				
				$("#pSggList").empty();
				$("#pSggList").append(sort);
				var html = "<option value='' admCd='"+sidoCd+"'>전체</option>";
				$("#pSggList").prepend(html);
				$("#pSggList>option:first").attr("selected","selected");
			},
	...
	$policyWritePopup.request = {
			...
			getSggList: function(sidoCd, callback) {
				// 자치구여부 체크시 - 2018.01.03	ywKim	추가
				if ($("#atdrcYn").is(":checked")) {
					$policyWritePopup.ui.setAutonomouseRegionList(sidoCd);
					return;
				}
	...
	$policyWritePopup.event = {
			setUIEvent : function() {
				...
				$("body").on("click", ".idxType", function() {
					...
					var idxNm = ( $policyWritePopup.ui.selectedIdx == 1 ? "수요변화형" : ( $policyWritePopup.ui.selectedIdx == 2 ? "통계연산형" : "시설분석형") );
					srvLogWrite( "E0", "05", "04", "00", idxNm, "" ); // jrj 로그 > 작성타입 선택
				...
				$("body").on("change", "#pSidoList", function() {
					...
					srvLogWrite( "E0", "05", "05", "00", obj.text(), "" ); // jrj 로그 > 대상지역 선택
				});
				...
				$("body").on("change", "#pSggList", function() {
					...
					srvLogWrite( "E0", "05", "05", "00", obj.text(), "시도:"+$("#pSidoList option:selected").text() ); // jrj 로그 > 대상지역 선택
				...
				$("body").on("change", "#pEmdList", function() {
					...
					srvLogWrite( "E0", "05", "05", "00", obj.text(), "시도:"+$("#pSidoList option:selected").text() + ",시군구:" + $("#pSggList option:selected").text() ); // jrj 로그 > 대상지역 선택
				});
				...
				$("body").on("click", "#bordArea li>a", function() {
					...
					srvLogWrite( "E0", "05", "06", "00", $( this ).text(), "" ); // jrj 로그 > 데이터 집계 경계 선택
					
					switch(id) {
						case "sggBord": //시군구 경계
							$policyWritePopup.ui.boundLevel = 1;
							$policyWritePopup.ui.tot_type = 2;
							break;
						case "emdBord": //읍면동 경계
							switch (admCd.length) {
								case 2:
									$policyWritePopup.ui.boundLevel = 2;
									$policyWritePopup.ui.tot_type = 3;
									break;
								case 5:
									$policyWritePopup.ui.boundLevel = 1;
									$policyWritePopup.ui.tot_type = 3;
