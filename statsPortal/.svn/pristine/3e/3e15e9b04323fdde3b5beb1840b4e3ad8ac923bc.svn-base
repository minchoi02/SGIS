...
(function(W, D) {
	...
	$policyWriteMapLeftMenu.ui = {
			...
			sqlListBoxLeft: "0px",	//
			class_deg : null,
			myPageNum : 1,	//나의데이터 현재 페이지
			gvAgeRadio : null,	//인구조건 bf val
			gvAgeRadio2 : null,	//결합조건 bf val
			 ...
			 * @author	     : 김성현
			 * @history 	 :
			 * 		2018.06.25	ywKim	변경 : 자주사용하는연령 목록 적용 [v180618] - 연령구간 구체화
			...
			setParams : function(curSelectedStatsType, api_id, idx) {
				...
				var yearListId = null;
				var origin = null;
				var isPrivate = 'N';			// 비공개 저장여부 (Y: 공개하지 않고 작성자만 볼 수 있다) - 2018.06.25	ywKim	추가: 비공개 저장 여부 Flag [v180618] - 연령구간 구체화
...
//								else if ($("#populationAgeTabFavorite").hasClass("on")) {// 2018.06.25	ywKim	추가: 자주사용하는 연령 목록에서 선택 [v180618] - 연령구간 구체화
//									var favoriatAge = $("#populationAgeFavorite").val();
//									var favoriatAgeText = $("#populationAgeFavorite option:selected").text();
									//20190306 손원웅 수정
									var favoriatAge = $("input[name=favoriteAge_radio]:radio:checked").val();
									var favoriatAgeTextId = $("input[name=favoriteAge_radio]:radio:checked").attr("id");
									var favoriatAgeText = $("label[for="+favoriatAgeTextId+"]").text();
									var favoriatAgeArr = favoriatAge.split('-'); 
									var favoriatAgeFrom = parseInt(favoriatAgeArr[0]);
									var favoriatAgeTo = parseInt(favoriatAgeArr[1]);
								
									isPrivate = favoriatAgeArr[2];
									
									tmpArParamName.push(favoriatAgeText);
									
									tmpArParams.push({
										key : "age_from",
										value : favoriatAgeFrom
									});
									tmpArParams.push({
										key : "age_to",
										value : favoriatAgeTo
...
//								else if ($("#populationAgeTab_combine_favorite").hasClass("on")) {// 2018.06.25	ywKim	추가: 자주사용하는 연령 목록에서 선택 [v180618] - 연령구간 구체화
//									var favoriatAge = $("#populationAge_combine_favorite").val();
//									var favoriatAgeText = $("#populationAge_combine_favorite option:selected").text();
									//20190306 손원웅 수정
									var favoriatAge = $("input[name=favoriteAge_combine_radio]:radio:checked").val();
									var favoriatAgeTextId = $("input[name=favoriteAge_combine_radio]:radio:checked").attr("id");
									var favoriatAgeText = $("label[for="+favoriatAgeTextId+"]").text();
									var favoriatAgeArr = favoriatAge.split('-'); 
									var favoriatAgeFrom = parseInt(favoriatAgeArr[0]);
									var favoriatAgeTo = parseInt(favoriatAgeArr[1]);
...
//								}
					...
					case "3f": //농림어업총조사
						switch (api_id) {
							case "API_0310":
								...
								} else if ($("#3fAgeTabFavorite").hasClass("on")) {// 2018.06.25	ywKim	추가: 자주사용하는 연령 목록에서 선택 [v180618] - 연령구간 구체화
									var favoriatAge = $("#3fAgeFavorite").val();
									var favoriatAgeText = $("#3fAgeFavorite option:selected").text();
									var favoriatAgeArr = favoriatAge.split('-'); 
									var favoriatAgeFrom = parseInt(favoriatAgeArr[0]);
									var favoriatAgeTo = parseInt(favoriatAgeArr[1]);
					...
					case "company": //전국사업체조사
						switch (api_id) {
							case "API_0304-b": //산업분류
								...
								else if($("#company_year").val() > 2005 && $("#company_year").val() < 2017) {
									companyClassType = "9";
								}
								else if($("#company_year").val() >= 2017) {
									companyClassType = "10";
								}
								//산업분류 트리일 경우
				...
				this.arParamList[idx] = {
					isPrivate: isPrivate,			// 2018.06.25	ywKim	추가: 비공개 저장 여부 파라미터 [v180618] - 연령구간 구체화
					type : statsType,
			 ...
			 * 
			 * @name         : showStatsPanel
			 * @description  : 메뉴 목록 열기 (사이드바, 2Depth 이하 메뉴를 모두 닫고)
			 * @date         : 2019.01.22 
			 * @author	     : ywKim
			 * @history 	 :
			 */
			...
			setDetailStatsPanel : function(type) {
				//2019-05-03 [김남민] 커피전문점 선택 후 좌측 메뉴 자동으로 닫혔을 경우 다시 통계메뉴를 클릭하면 세부지표 선택 메뉴가 호출되어야 함. (05-07 추가수정) START
				$policyWriteMapLeftMenu.ui.ctgr_type = type;
				//2019-05-03 [김남민] 커피전문점 선택 후 좌측 메뉴 자동으로 닫혔을 경우 다시 통계메뉴를 클릭하면 세부지표 선택 메뉴가 호출되어야 함. (05-07 추가수정) END
				...
				if(inx!=8){
					...
					$("#depth1Menu").find("a").removeClass("on");					// 2019.01.22	ywKim	변경
//					$("#depth1Menu").find("li").removeClass("on");
					$("#depth1Menu").find("li:eq("+inx+")>a").addClass("on");		// 2019.01.22	ywKim	변경
//					$("#depth1Menu").find("li:eq("+inx+")").addClass("on");
					...
					$("#depth1Menu").find("a").removeClass("on");					// 2019.01.22	ywKim	변경
//					$("#depth1Menu").find("li").removeClass("on");
					$("#depth1Menu").find("li:eq("+3+")>a").addClass("on");			// 2019.01.22	ywKim	변경
...
//				$(".quickBox.step04").stop().animate({"left":"0px"},200);
				...
				// 우측 닫기 버튼 위치 조정 - 2019.01.22	ywKim	추가
				if (inx == 3) {
					$(".stepClose.3depth_close").stop().animate({"right":"-304px"}, 200);
				} else {
					$(".stepClose.3depth_close").stop().animate({"right":"-25px"}, 200);
				}
...
//						$("#kosisDetailDiv").stop().animate({"left":"560px"},200);
			...
			populationTab : function(type) {
				...
				switch (tabType[type]) {
					case 0:
						...
						this.bfRadio(1, this.gvAgeRadio, null);
						break;
					case 1:
						$("#API_0305").show();
					...
					case 3:
						...
						this.bfRadio(2, null, this.gvAgeRadio2);
			...
			tripleFTab : function(type) {
				...
				if(inx == 2) {		//어가일 경우만 해수면,내수면 선택
					...
					$("#3f_year").append("<option value='2000'>2000년</option>");
			...
			companyTab : function(type) {
				...
				if(tabType[type] == 0) {		//산업분류
					...
					$(".stepClose.3depth_close").stop().animate({"right":"-304px"}, 200);	// 우측 닫기 버튼 위치 조정 - 2019.01.22	ywKim	추가
					...
					$(".stepClose.3depth_close").stop().animate({"right":"-25px"}, 200);	// 우측 닫기 버튼 위치 조정 - 2019.01.22	ywKim	추가
				}
			},
			...
			companyClassView : function(type) {
				...
				$(".sideQuick.sq02").stop().animate({"left":"840px"},200);
				$("#companyClassSearchDiv").stop().animate({"left":"0px"},200);
				$("#companyClassListDiv").stop().animate({"left":"360px"},200);	// 리디자인에 따른 위치  변경 - 2019.01.22	ywKim
			...
			bndYearSelectbox : function() {
				...
				if (parseInt(dataYear) > 2015) {
					for (var year=parseInt(dataYear); year>2015; year--) {
						//주요지표 조사년도
						$("#mainIndex_year").append("<option value="+year+">"+year+"년</option>");
						
						//인구조건 조사년도
						$("#population_year").append("<option value="+year+">"+year+"년</option>");
					
						//가구조건 조사년도
						$("#household_year").append("<option value="+year+">"+year+"년</option>");
						
						//주택조건 조사년도
						$("#house_year").append("<option value="+year+">"+year+"년</option>");
						
						//인구+가구+주택 결합조건 조사년도
						$("#population_year_combine").append("<option value="+year+">"+year+"년</option>");
					}
					for (var year=parseInt(dataYear); year>=2000; year--) {
						if((year % 5) == 0) {
							...
							//농림어업 조사년도
							$("#3f_year").append("<option value="+year+">"+year+"년</option>");
				...
				if(parseInt(companyDataYear) <= 2016) {
					$("#company_year").append("<option value='2017'>2017년</option>");
				}
				//2019-04-22 [김남민] 정책통계지도 > 작성하기 > 전국사업체조사 > 조사년도 2017년도 추가. END
				...
				for(var year = parseInt(companyDataYear); year >= 2000; year --) {
					//사업체 조사년도
					$("#company_year").append("<option value="+year+">"+year+"년</option>");
					
					//사업체 주요지표 조사년도
					$("#mainIndex_corp_year").append("<option value="+year+">"+year+"년</option>");
					
					//2018.01.09 [개발팀]
					//사업체 테마 조사년도 설정
					if (year > 2005) {
						$("#company_year_theme").append("<option value="+year+">"+year+"년</option>");
					}
			...
			btnValidationCheck : function(curSelectedStatsType, api_id) {
				switch (curSelectedStatsType) {
					...
					case "3f": //농림어업총조사
						...
						if($("#3fGenderTab").hasClass("on") || $("#3fAgeTab").hasClass("on") || $("#3fAgeTabFavorite").hasClass("on")) {// 2018.06.25	ywKim	변경: 자주사용하는 연령 적용[v180618] - 연령구간 구체화
//						if($("#3fGenderTab").hasClass("on") || $("#3fAgeTab").hasClass("on")) {
			...
			//20190307 손원웅 추가
			bfRadio : function(idx, gar, gar2) {
				if(idx == 1){
					//결합조건 세팅
					this.gvAgeRadio2 = $("input[name=favoriteAge_combine_radio]:radio:checked").attr("id");
					if(gar != null){
						$("label[for='"+gar+"']").addClass("on");
						$("input:radio[id='"+gar+"']").prop("checked", true);
					} 
				}else if(idx == 2){
					//인구조건 세팅
					this.gvAgeRadio = $("input[name=favoriteAge_radio]:radio:checked").attr("id");
					if(gar2 != null){
						$("label[for='"+gar2+"']").addClass("on");
						$("input:radio[id='"+gar2+"']").prop("checked", true);
					} 
				}
	...
	$policyWriteMapLeftMenu.request = {
			...
			/**
			 * 
			 * @name         : getFavoriteAgeList / 자주사용하는 연령 목록 가져오기
			 * @description  : 관리자가 설정한 자주사용한 연령 목록을 가져온다.
			 * 				   사용 통계 항목 : 인구주택총조사, 농림어업총조사
			 * @date         : 2018.06.21 
			 * @history 	 :
			 * 		2018.06.21	ywKim	신규 [v180618] - 연령구간 구체화
			 * @author	     : ywKim
			 * @param 		 : 
			 * @history 	 :
			 */
			getFavoriteAgeList : function() {
				//2019-05-03 [김남민] 없는 테이블 조회 호출 막기. START
				return;
				//2019-05-03 [김남민] 없는 테이블 조회 호출 막기. END
				$.ajax({
					url : contextPath + "/ServiceAPI/policyWrite/getFavoriteAgeList.json",
					type : "POST",
					async : true,
					dataType : "json",
					beforeSend: function() {},
					success : function(res) {
						if (res.errCd == 0) {
							$policyWriteMapLeftMenu.event.setFavoriteAge(res.result);
						}
					},
					error : function(data){
					}
			...
			myDataList : function(pageNum) {
				$.ajax({
					...
					data : {
						"pageNum" : pageNum,
						"resultCount" : "5",
						"tot_type" : $policyWriteMap.ui.settingInfo.tot_type,
						"idx_type" : $policyWriteMap.ui.settingInfo.idxType
					},
					async : true,
					dataType : "json",
					beforeSend: function() {},
					success : function(res) {
						...
						switch(parseInt(res.errCd)) {
							case 0:
								...
								$policyWriteMapLeftMenu.request.myDataPaging( 
										result.totalcount, 
										$policyWriteMapLeftMenu.ui.myPageNum );
			...
			myDataPaging : function(totalCount, currentIndex) {
				...
				$('#myDataPaging .pages').paging({
					...
					last : '&gt;&gt;',
					onclick : function(e, page) {
						$policyWriteMapLeftMenu.ui.myPageNum = page;
						$policyWriteMapLeftMenu.request.myDataList(page);
					}
				});
			}
	};
	...
	$policyWriteMapLeftMenu.event = {
			...
			setUIEvent : function() {
				...
				$policyWriteMapLeftMenu.request.getFavoriteAgeList();// 2018.06.21	ywKim	추가: 자주사용하는 연령 조회 [v180618] - 연령구간 구체화
				...
				//조건 상세설정 선택
				body.on("click",".radioStepBoxPopulationAge label",function(e){
					...
					// 선택된값
					var lvSeletedValue = $(this).prev().val(); 

					// 대상 선택 (인구조건 또는 결합조건 내부)
					//20190307 손원웅 수정_radio 버튼 선택 관련
					var $ageTab = ($root.find("#populationAgeTab").length == 1) ? $root.find("#populationAgeTab") : $root.find("#populationAgeTab_combine");
//					var $ageTabFavorite = ($root.find("#populationAgeTabFavorite").length == 1) ? $root.find("#populationAgeTabFavorite") : $root.find("#populationAgeTab_combine_favorite");
					var $ageTabFavorite = ($root.find("#favoriteAge_stepBox_radio").length == 1) ? $root.find("#favoriteAge_stepBox_radio") : $root.find("#favoriteAge_combine_stepBox_radio");
					// roundTextBox (숨겨진 체크박스 체크)
					$ageTab.removeClass("on");
					$ageTab.children("input").removeAttr("checked");
					$ageTab.next(".joinDefault").hide();
					
					$ageTabFavorite.removeClass("on");
					$ageTabFavorite.children("input").removeAttr("checked");
					$ageTabFavorite.hide();
//					$ageTabFavorite.next(".joinDefault").hide();
					if(lvSeletedValue == "1") {
						$ageTab.addClass("on");
						$ageTab.children("input").attr("checked", "checked");
						$ageTab.next(".joinDefault").show();
					}
					else if(lvSeletedValue == "2") {
						
						var chkId = "";
						var chklen = 0;
						
						$ageTabFavorite.find('li').children("input").each(function(index,Element){
							if(index == 0){
								chkId = Element.id;
							};
							if(Element.checked){
						...
						$ageTabFavorite.addClass("on");
						$ageTabFavorite.children("input").attr("checked", "checked");
						$ageTabFavorite.show();
						
						if(chklen == 0){
							$("label[for='"+chkId+"']").addClass("on");
							$("input:radio[id='"+chkId+"']").prop("checked", true);
						};
//						$ageTabFavorite.addClass("on");
//						$ageTabFavorite.children("input").attr("checked", "checked");
//						$ageTabFavorite.next(".joinDefault").show();
					}
				});
				...
				body.on("click","a.roundTextBox",function(e){
					...
					// 2018.06.21	ywKim	추가: 연령(선택) 체크박스의 라디오기능 [v180618] - 연령구간 구체화
					// 한쌍의 다른 체크박스를 해제한다.
					// 한쌍을 알려주는 속성은 data-pair-id 이며, 자신과 쌍을 이루는 다른 요소의 ID가 설정돼 있다. 
					if (!ck) {
						var $p = $(this).closest('a');
						if ($p) {
							var pairId = $p.attr("data-pair-id");
							if (pairId) {
								var $pairParent = $("#" + pairId);
								if ($pairParent) {
									var ck = $pairParent.hasClass("on");
									if (ck) {
										$pairParent.trigger("click");
									}
								}
							}	
						}
...
//					}
				...
				/* 2018-12-26 데이터통계 작성화면 UI 맞추기 */
				//$("#mCSB_4_container").width("279px");
				//$("#mCSB_5_container").width("279px");
			...
			stepCloseAnimate : function(inx, type, ts){
			    ...
			    $("#depth1Menu").stop().animate({"left": "-80px"}, 200);	// 2019.01.22	ywKim	추가
			},
			
			/*
			 * @name         : setFavoriteAge / 자주사용하는 연령 설정하기
			 * @description  : 자주사용하는 연령에 대한 콤보박스(모두)를 세팅한다.
			 * @date         : 2018.06.21
			 * @author	     : ywKim
			 * @param ageList : 자주사용하는 연령 목록 / DB에서 조회한 레코드를 항목으로 가지고 있음 (age_from, age_to
			 * @history 	 :
			 * 			 2018.06.21	ywKim	신규 [v180618] - 연령구간 구체화
			 */
			setFavoriteAge : function (ageList) {
				$("select[name=favoriteAge]").each(function() {
					for (var i = 0; i < ageList.length; i++) {
						var val = ageList[i].age_from + "-" + ageList[i].age_to + "-" + ageList[i].private_yn;
						var txt = "";
						
						if (ageList[i].age_from !== undefined && ageList[i].age_to !== undefined) {							
							txt = ageList[i].age_nm;
						} else {
							txt = ageList[i].age_nm + " ★★잘못된 값★★";
						}
						
						if (ageList[i].private_yn == "Y") {
							txt += " [비공개]";
						}							
