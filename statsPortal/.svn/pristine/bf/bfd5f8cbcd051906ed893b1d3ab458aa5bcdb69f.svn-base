...
<!--<link href="/css/default.css" rel="stylesheet" type="text/css" /> 190305 방민정수정-->    
		...
		$(function(){
			...
			$.ajax({
				...
				success:function(data){
					srvLogWrite('A0', '06', '02', '00', '', '');
// 					alert("성공");
					console.log(data);
					var resultData = data.result;
					var html ="";
					
					var len = ( data.result ? resultData.post_title_list.length : 0 );
            		$searchList.checkRequestEmptyCnt( len );
					
					if(resultData.post_title_list.length == 0){
							$("#wordDiv").hide();						
					}else{
						$.each(resultData.post_title_list,function(i){
							...
							html += "<p id='word_title_"+i+"' style='margin-bottom:20px;font-size:15px; line-height:16px; font-family:나눔고딕B;color: #333;'>"+ptitle_word+"</p>";
							html += "<p id='word_ex_"+i+"' style='letter-spacing:-0.6px;'>"+ptitle_word_ex+"</p><hr><br>";
							$("#wordDiv div").html(html);
							$("#wordDiv").show();
						});
					}
				},
	...
	<div class="wrapper">
    	...
    	<div id="content">
			...
			<div id="article-wrap">					
				...
				<!-- ( drop_search_result ) -->
				<!-- mng_s 20211229 이진호, PM님 요청으로 안보이게 처리 -->
				...
				<!-- mng_e 20211229 이진호 -->
				<!-- mng_s 20211229 이진호, PM님 요청으로 안보이게 처리 -->				
				<div id="poiListTablePage" style="display: none;"></div>
				<!-- mng_e 20211229 이진호 -->
				<!-- //( 지도 + POI 검색결과 ) -->
				...
				<!-- 2020.04.06[한광희] My통계로 검색결과 START -->
				<div id="statsMeCatalogListTable"></div>
				<div id="statsMeCatalogListTablePage"></div>										
				<br><br><br><br>
				<!-- 2020.04.06[한광희] My통계로 검색결과 END -->
