	...
	interactiveMapBtnInfo = {
		btnInfo	: function(map, delegate) {
			...
			this.createPOIBtn = function() {
				...
				// 2020년 SGIS고도화 3차(테마코드) 시작 - 새로운 테마코드 대분류에 따른 POI 버튼 UI 변화로 인해서 추가된 ajax 요청 (pse)
				var censusThemeInfo = null;
				$.ajax({
				    type: "GET",
				    url : contextPath + "/ServiceAPI/bizStats/themeCdHelper.json",
				    data : {service : "selectAllCensusThemeInfo"},
				    success : function(res) {
				    	censusThemeInfo = res.result;
				    },
				    async: false,
				    dataType : "json"
				});
				// 2020년 SGIS고도화 3차(테마코드) 끝 - 새로운 테마코드 대분류에 따른 POI 버튼 UI 변화로 인해서 추가된 ajax 요청 (pse)
				
				// 2020년 SGIS고도화 3차(테마코드) 시작 - 대분류의 개수에 따라 유연하게 길이를 조정 가능하도록 코드 수정 (pse)
				// POI List 하나당 가로, 세로의 총길이( width + margin ) 값이 46 으로 고정되어 있다.
				// +2를 하는 이유는 맨 처음의 산업분류 버튼과 맨 끝의 초기화 버튼 때문이다.
				var totalWidth = (censusThemeInfo.length+2) * 46; 
				// 2020년 SGIS고도화 3차(테마코드) 끝 - 대분류의 개수에 따라 유연하게 길이를 조정 가능하도록 코드 수정 (pse)
				...
				poiUI.update = function (props) {
					...
					html += 	"<ul class='rqListBox rq01' style='width: "+totalWidth+"px;'>";// 2020년 SGIS고도화 3차(테마코드) 대분류의 개수에 따라 유연하게 길이를 조정 가능하도록 코드 수정
					...
					*/
					...
					// 2020년 SGIS고도화 3차(테마코드) 시작 - 하드 코딩 수정 (pse)
					censusThemeInfo.forEach(function(item,index){
						var b_theme_cd_nm = item.b_theme_cd_nm.length >= 5 ? 		// 문자열이 5글자 이상이 되면 무조건 줄넘김이 생긴다 (ex: 생활서비스). 그러니 5글자 이상일 때
								item.b_theme_cd_nm.substr(0,2) + '<br>' + item.b_theme_cd_nm.substr(2) 	// 앞 2글자 + 줄넘김  + 남은 글자로 변환시킨다.
								: item.b_theme_cd_nm;													// 길이가 5미만이면 그냥 그대로 넣는다.
								
						html += 	"<li><a><span>"+b_theme_cd_nm+"</span></a>";	
						//html += 		"<ol class='rqIcon"+("00"+(index+1)).slice(-2)+"' data-big-theme-cd='"+item.b_theme_cd+"'></ol>";	// 숫자가 1자리면 왼쪽에 0을 붙임.
						html += 		"<ol data-big-theme-cd='"+item.b_theme_cd+"'></ol>";	// 숫자가 1자리면 왼쪽에 0을 붙임.    				
						html += 	"</li>";
					});
					html +=    	"</li>";
					html +=    	"<li id='poiInit_"+that.id+"'><a class='ico_side_Select'><span>초기화</span></a>";
					html +=    	"</li>";
					// 2020년 SGIS고도화 3차(테마코드) 끝 - 하드 코딩 수정 (pse)
					html += "</ul>";
					html += "</div>";
				...
				*/
				...
				// 2020년 SGIS고도화 3차(테마코드) 시작 - 하드코딩 대체 코드 추가 (pse)
				var _this = this;
				censusThemeInfo.forEach(function(item,index){
					_this.addThemeList(item.b_theme_cd,item.s_theme_list);
				});
				// 2020년 SGIS고도화 3차(테마코드) 끝 - 하드코딩 대체 코드 추가 (pse)
...
//							"							</select>" +
			...
			this.createTechPOIBtn = function() {
				...
				this.addEduPoiList();
				this.addCulturePoiList();
				// mng_s 20201221 김건민 (기술업종 현황 산업분류 9차 -> 10차로 수정함)
				this.reqInterstryList(0, "10", null);
				// mng_e 20201221 김건민
			},
			...
			this.createSettingBtn = function(type) {
				...
				setUI.update = function (props) {
					...
					if  (type == "intr") {
						...
						html +=		"<li><a href='javascript:void(0)' class='ico_side_Select multi_bound'><span>다중<br />선택</span></a></li>"; 
					}
...
//					html += "<a class='zoomSliderBtn'><img src='/images/common/sliderbtn.png'></img></a>";
			...
			// 2020년 SGIS고도화 3차(테마코드) 시작 - 하드 코딩 해소를 위한 메서드 선언 (pse)
			/**
			 * 
			 * @name         : addThemeList
			 * @description  : addPOIBtn() 메서드 내에서 사용, POI 버튼 각각에 대한 소분류 li 태그를 forEach로 생성
			 * @date         : 2020. 08. 12. 
			 * @author	     : pse
			 * @history 	 :
			 */
			this.addThemeList = function(b_theme_cd, s_theme_list) {
				var html = "";
				s_theme_list.forEach(function(item,index){
					html += "<li><a name='"+item.theme_cd+"'><img src='/img/ico/ico_"+item.theme_cd+".png' alt='"+item.s_theme_cd_nm+"'/><span style='cursor:pointer;'>"+item.s_theme_cd_nm+"</span></a></li>";	
				});
				$("#poi_" + that.id).find("[data-big-theme-cd='"+b_theme_cd+"']").append(html);
			},
			// 2020년 SGIS고도화 3차(테마코드) 끝 - 하드 코딩 해소를 위한 메서드 선언 (pse)
									...
									btns = [
											{
											    ...
											    func : function(opt) {
											    	...
											    	//mng_s 20210401 이진호 > // 사업체 POI(지표) 호출 시 공공데이터가 있으면 공공데이터 삭제
											    	var publicData_type = $publicDataBoard.ui.mapData[$publicDataBoard.ui.delegate.curMapId].type; // 공공데이터가 있을 경우 metro 또는 busStop , 없으면 '' 로 나옴
											    	if(publicData_type != ''){//지도에 공공데이터 POI 여부 판단
											    		$publicDataBoard.ui.remove($publicDataBoard.ui.delegate.curMapId); // 공공데이터 전체 초기화
											    	}
											    	//mng_e 20210401 이진호
...
//						$("#zoomBtn_" + that.id).css({top : "-265px",left : "-30px"});
				...
				$("#zoomBtn_" + that.id).find(".zoomSliderBtn").click(function(){
					...
					if(!on){
						...
						$("#zoomBtn_" + that.id).parent().children(".sop-control-zoomSlider.sop-bar.sop-control").css({"display": "","top" : "206px","left" : "5px", "z-index" : "99999"}); //zoomSlider none해제 
						$("#zoomBtn_" + that.id).parent().children(".sop-control-zoomSlider.sop-bar.sop-control").attr('data-html2canvas-ignore', 'true');
			...
			//mng_s 20210303 이진호
			//백지도 숨기기 기능 수행 시 zoom level이 0 또는 1 일때 숨기기 기능이 해제 되지 않도록 하기 위해 주석처리
			//김책임님께서 기능이 해제되면 안된다고 하셨음
			//20200611 수정 시작 (ggm)
			...
			//숨기기 기능 포함 수정했습니다.
			this.refreshWhiteMap = function() {
				var white_child_ck = $("#map_" + that.id).find("#white_child").find("a").hasClass("on");
	
				if(that.map.mapMode == "white" /*&& (that.map.zoom == 0 || that.map.zoom == 1)*/){//20210401 수정
					var btnWhite = $("#map_" + that.id).find(".rqListBox>li>a[name='white']");
					if(btnWhite != undefined && btnWhite != null){
						$("#map_" + that.id).find(".rqListBox>li>a[name='white']").removeClass("on");
						btnWhite.trigger("click");
						
						if(white_child_ck){
							that.map.gMap.removeLayer(that.baseTileLayer);
							$("#map_" + that.id).find(".rqListBox>li>a[name='white']").removeClass("on");
							$("#map_" + that.id).find("#white_child").find("a").addClass("on");
						}
					}
				}
			}
