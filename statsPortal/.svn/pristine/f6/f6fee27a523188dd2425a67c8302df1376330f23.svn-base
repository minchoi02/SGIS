...
//					$("header").css({"height":"104px", "width":"970px"}); 
			...
			doClearMap : function(type) {
				...
				//시군구별 기술업종 현황일 경우 (초기화 버튼 때문에 추가함.)
				// mng_s 20200623 김건민 
...
//				$technicalBizLeftMenu.ui.updateSearchBtnEffect("", this.curMapId);
			...
			goLinkIndustryDetailInfo : function(pageNo, index) {
				apiLogWrite2("T0", "T05", "산업단지 조회", "산업단지 상세정보 보기", "00", "전국");
				srvLogWrite('G2', '14', '06', '00', '', '산업단지 상세정보 보기');
				var map = this.mapList[0];
				setTimeout(function() {
					var options = $technicalBizDataBoard.ui.mapData[map.id].options;
					var markerGroup = options.marker;
			...
			doReqAllLq : function(themeCd,themeNm,type,base_region,standard){
				...
				var selectYearList = $("#lqInfoYearSettingList > li , #lqInfoYearSettingList2 > li"); // djlee 20190413 수정
				var selectYear = null;
				for(var i = 0; i < selectYearList.length; i++){
					if($(selectYearList[i]).find("a").hasClass("on")){
						selectYear = $(selectYearList[i]).find("a").data("year");// djlee 20190413 수정
					}
				}
				if(selectYear == null){
...
//				this.doClearMap(map.id+1);
			...
			lqInfoTooltip : function(event,data,map){
				// mng_s 20190404 김건민
				apiLogWrite2("T0", "T07", "지역 선택 팝업창 보기", '지역 선택 팝업창 보기', map.zoom, "전국");
				srvLogWrite('G2', '11', '05', '00', '', '지역 선택 팝업창 보기');
				// mng_e 20190404 김건민
			...
			areaSelectSearch : function(idx){
				switch(idx){
					case 0: 
						// mng_s 20190402 김건민
						srvLogWrite('G2', '12', '03', '07', '', '지역 비교하기');
						// mng_e 20190402 김건민
						//처음 클릭 하였을때 
						$("#searchAreaBox").show();
			...
			setSelectAdmCode : function(admCodeArray, option){
				...
				$technicalBizMapApi.request.searchAdmNm(admCodeArray,'legendPage', function() { 
					...
					// mng_s 20190402 김건민
					$("#compositeChart").attr("href", "javascript:srvLogWrite('G2', '12', '05', '01', '', '종합 통계 정보');$technicalBizMap.ui.areaTotalInfoResult('"+option.techCd+"',"+techCdIdx[option.techCd]+")");
					$("#detailChart").attr("href", "javascript:srvLogWrite('G2', '12', '05', '02', '', '상세 통계 정보');$technicalBizMap.ui.areaDetailInfoResult('"+option.techCd+"',"+techCdIdx[option.techCd]+", 0, 0)");
					// mng_e 20190402 김건민
