(function(W, D) {
	$(document).ready(function() {
		...
		srvLogWrite( "H0", "01", "07", "01", $communityMapCommon.getParameter("post_no"), "" );
	});
	function getDetail(){
		$.ajax({
			type: "POST",
			...
			success: function(res) {
				if(res.errCd=="0"){
					...
					$.each(res.result.summaryList,function(cnt,node){
						...
						$("#notice_list").append(ul,$("<a/>",{"href":contextPath+$communityMapCommon.edu_url+"/notice/list"+addQuery,"class":"anc-btn black qnaboardclose","style":"width:80px; margin-left: 880px;","text":"목록"}));
