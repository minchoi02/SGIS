...
function startTutorial() {
						...
						$("#tutorialText")
								.append(
										"<div class=\"title\">"
												...
												+ "각 총조사 결과 중 이용자가 주로 이용하는 지표를 정리해 놓은 메뉴입니다."
												+ "</p></div>");
					});
	...
	var tutoImg = [ $("#mainIndexImg"), $("#ptotalImg"), $("#searchImg"),
			...
			$("#slide3Img"), $("#themaListImg"), $("#themaRestaurantImg"),
...
//						$(".themul > li").eq(0).addClass("on");
						$(".themul > li").eq(0).trigger("click");
//						$interactiveLeftMenu.ui.setDetailStatsPanel("mainIndex");
						$("#ptotalImg").show();
						posiInfo(1);
					});
	$("#ptotalImg")
			...
			.click(
					function() {
						if (okChk == 1) {
							...
							//mng_s 20210415 이진호, 튜토리얼 현행화
							//$("#dragItemListImg").hide();
							$("#dragItemListImg").css('z-index', '10002');
							//mng_E 20210415 이진호
						...
						$("#tutorialText")
								.append(
										"<div class=\"title\">"
												...
												+ "</div>"
												+ "<div class=\"content\">"
												+ "<p>아래에 <strong style=\"color:#0040ff;  font-weight:bold;\">대전광역시 서구 경계</strong>입니다.<br> "
			...
			.click(
					function() {
						if (dbChk == 1) {
							...
							//mng_s 20210415 이진호, 튜토리얼 현행화
							$("#dataBoardDetailImg").css('height', '800px');
	...
	$("#mainMenuImg").click(function() {
		...
		$("#tutorialText").append("<div class=\"title\">"
								...
								+ "5년에 한 번 시행하는 <strong style=\"color:#0040ff;  font-weight:bold;\">인구주택총조사(2016년 이전은 5년단위), 농림어업총조사</strong> 및<br>"
								+ "매년 시행하는 <strong style=\"color:#0040ff;  font-weight:bold;\">전국사업체조사, 인구주택총조사(2016년이후)</strong> 입니다."
								+ "</p></div>"
								+ "<div class=\"button\" id=\"next\" style=\"left:-6px; top:24px;\">"
								+ "<a href=\"#\"><button type=\"button\" alt=\"다음\" style=\"width:70px; height:28px; margin:0 1px 0 1px; line-height:19px; color: #ffffff; background-color: #457AB0;  font-size: 14px;\">다음</button></a>"
								+ "</div>");
		$("body").on("click",".button",function() {
				...
				$("#tutorialText").append("<div class=\"title\">"
										...
										+ "<strong style=\"color:#0040ff;  font-weight:bold;\">* 집계구: 소지역통계 제공을 위한 최소구역으로 인구 500명 내외 포함(읍면동의 평균 1/29)</strong><br>"
										
										//mng_s 20200422 이진호 / 튜토리얼 현행화 
										//KOSIS 가 없어짐에 따라 KOSIS 소개 문구를 삭제하고 e-지방지표 소개 문구 추가
										...
										+ "<strong style=\"color:#0040ff;  font-weight:bold;\">'e-지방지표'</strong>는 국가통계포털의 자료 중 지역자료가 있는 경우 통계지도로 표시하여 줍니다."
										+ " 표본조사 자료이므로 시도 내지 시군구로만 조회 가능합니다."
										//mng_e 20200422 이진호
	...
	$("#themaRestaurantImg")
			.click(
					function() {
						...
						// 2020년 SGIS고도화 3차(테마코드) 시작 - 기존 코드 주석처리 , 새로운 테마코드에 의해 html id값이 변경되어 이를 반영함 (pse)
						/*
						$("#food").addClass("on");
						$("#foodInfo").css("display", "block");
						*/
						$("a[id='H'][class='subRoundTextBox']").addClass("on");
						$("div[id='H_info'][class='joinDefault']").css("display", "block");
						// 2020년 SGIS고도화 3차(테마코드) 끝 - 기존 코드 주석처리 , 새로운 테마코드에 의해 html id값이 변경되어 이를 반영함 (pse)
						$("#themaCafeImg").show();
						
						//mng_s 20200424 이진호, 튜토리얼 현행화
						if ($(window).height()>=1080||$(window).width()>=1920) {
							$("#mCSB_5_container").css({"top" : "-312px"});
							//$("#themaCafeImg").css({"top" : "620px"});	//  2020년 SGIS고도화 3차(테마코드) - 기존코드 주석처리 (pse)
							
							//mng_s 20210415 이진호, 튜토리얼 현행화 / 위치 수정
							//$("#themaCafeImg").css({"top" : "733px"});		//  2020년 SGIS고도화 3차(테마코드) - 위치 변경 코드 추가(pse)
							$("#themaCafeImg").css({"top" : "700px"});		//  2020년 SGIS고도화 3차(테마코드) - 위치 변경 코드 추가(pse)
							//mng_e 20210415 이진호
							
						}else{
							$("#mCSB_5_container").css({"top" : "-312px"});
							//$("#themaCafeImg").css({"top" : "365px"});	//  2020년 SGIS고도화 3차(테마코드) - 기존코드 주석처리 (pse)
							$("#themaCafeImg").css({"top" : "525px"});		//  2020년 SGIS고도화 3차(테마코드) - 위치 변경 코드 추가(pse)
						}
						//mng_e 20200424 이진호
																	...
																	$(
																			...
																			.append(
																					"<div class=\"title\">"
																							...
																							+ "</div>"
																							+ "<div class=\"content\">"
																							+ "<p><strong style=\"color:#0040ff;  font-weight:bold;\">'집계구'</strong>는 읍면동보다 더 작은 소지역통계를 제공하기 위해 만들어졌으며,<br>"
																							+ "매년 업데이트 됩니다. 붉게 표시된 집계구에 카페가 밀집되어 있습니다.<br>"
						...
						if (showdbChk == 2) {
							...
							if ($(window).height() >= 1080
								...
								$("#dataBoardImgDiv")
										.css(
												{
													"background-image" : "url(/img/tutorial/databoardMainImg.png)",
							...
							$("#tutorialText")
									.append(
											"<div class=\"title\">"
													...
													//mng_s 20210416 이진호, 튜토리얼 현행화
													//+ "대전광역시 카페는 전국 카페의 3.3% 서구 카페는 대전광역시 카페의 31.7%를 둔산2동 카페는 서구 카페의 15.7%를 차지하고 있습니다."
													+ "대전광역시 카페는 전국 카페의 3.4% 서구 카페는 대전광역시 카페의 31.3%를 둔산2동 카페는 서구 카페의 14%를 차지하고 있습니다."
													//mng_e 20210416 이진호
											...
											function() {
												...
												$("#tutorialText")
														.append(
																...
																+ "2000년부터 2016년까지 1년 단위 데이터가 있습니다. 다만 2006년부터 개정된 산업분류가 적용되어 2005년 이전의 자료는<br>"
					...
					function() {
						...
						$("#tutorialText")
								.append(
										"<div class=\"title\">"
												...
												+ "</div>"
												+ "<div class=\"content\">"
												+ "<p>이번에는 <strong style=\"color:#ee7c1a;  font-weight:bold;\">2016년</strong>을 클릭해 보세요.<br>"
												+ "</p></div>");
						if ($(window).height() >= 1080
								|| $(window).width() >= 1920) {
							$("#dataBoardImgDiv")
						...
						if ($(window).height() >= 1080
							...
							$("#toPoint_db3").css({
								...
								//mng_s 20200423 이진호, 튜토리얼 현행화 작업 및 오류 수정
								//포인터 위치 수정
								//"bottom" : "365px"
								"bottom" : "305px"
								//mng_e 20200423 이진호
			...
			.click(
					function() {
						if (cleanChk == 2) {
							...
							posiInfo(-1);
							setTimeout(function() {
								location.href = "/view/map/interactiveMap/mainIndexView";//2019-03-13 박길섭
							}, 10000);
						}
					});
...
function closeTutorial() {
	...
	//mng_s 20210603 이진호, 튜토리얼 종료시 컨펌 동작(확인/취소 눌럿을 때 동작 수정
