				...
				for (var i=0; i<result.length; i++) {
					...
					switch(parseInt($policyWriteMap.ui.settingInfo.idxType)) {
						case 1: //수요변화형지표
							...
							//2018.05.09 yearList 선언 수정
							yearList = JSON.parse(mainData.GROUP_COL_LIST);
							...
							for (var p in yearList) {
								var column = String.fromCharCode(p.charCodeAt(0)).toUpperCase();
							...
							break;
						case 2: //통계연산형지표
						case 3: //시설분석형지표  2018.05.09 case 수정
							//메터데이터 가공
							//나의데이터 구조설계 오류로 인한 가공 필수
							//집계데이터의 경우, 메터데이터 칼럼을 한단계씩 밀어야 함 (POINT의 경우, 변경하지 않아도 됨)
							//예)A -> B, C -> D로 변경 
							...
							//2018.05.23 geoFiled 값 수정
							//나의데이터 컬럼이 원본 E일 경우 DB에 GIOFIELD C로 저장됨.
							//DB에 uploadData.USR_DATA에도 C로 저장됨 +1 할 필요 없음.
							geoFiled = String.fromCharCode(mainData.GIOFIELD.charCodeAt(0)).toUpperCase();

							var column = null;
							var title = null;
							for (var i=0; i<metaData.length; i++) {
								//2018.05.23 컬럼,타이틀 값 수정
								//표출데이터는 3 또는 1로 저장됨.
									...
									if (depth == 0) {
										...
										$policyWriteMapLeftMenu.ui.companyTree = $("#company_TreeBox").easytree({
											...
											iconSelected : function(e, id) {
												...
												window.open(
														"https://kssc.kostat.go.kr:8443/ksscNew_web/kssc/common/ClassificationContentMainTreeListView.do?strCategoryNameCode=001&strCategoryDegree="+ class_deg +"&strCategoryCode="+ id,
