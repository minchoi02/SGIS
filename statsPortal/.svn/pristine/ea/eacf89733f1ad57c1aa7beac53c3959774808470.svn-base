...
(function(W, D) {
	...
	$(document).ready(function() {
		...
		if(document.location.href.indexOf("type=full") > 1 ){
			$thematicMapFrame07.ui.doMaxSize(1);
		}
		//mng_e
...
//					index, map);
	...
	$thematicMapFrame07.event = {
			...
			refreshThemaList : function(select_obj) {
				...
				$thematicMapFrame07.getCategoryList.getMenuList(categoryId);
			}
			...
			successCateList : function (stats, res) {
				...
				if (res.errCd === 0) {
					...
					for ( var i = 0; i < categoryList.length; i++) {							
						...
						var html = '<li><a data-id="'+categoryList[i].thema_map_category+'"  href="javascript:$thematicMapFrame07.getCategoryList.getMenuList(\''+categoryList[i].thema_map_category+'\');">'+categoryList[i].category_nm+'</a></li>';
						$('.themul').append(html);
//						html = '<li><a data-id="'+categoryList[i].thema_map_category+'"  href="javascript:$thematicMapFrame07.getCategoryList.getMenuList(\''+categoryList[i].thema_map_category+'\');" title="'+categoryList[i].category_nm+'"><span>'+categoryList[i].category_nm+'</span></a></li>';
			...
			getMenuList : function (thema_map_category){
				...
				$.ajax({
					...
					success: function(res) {
						...
						html += 'onchange="javascript:$thematicMapFrame07.event.refreshThemaList(this)";>';
