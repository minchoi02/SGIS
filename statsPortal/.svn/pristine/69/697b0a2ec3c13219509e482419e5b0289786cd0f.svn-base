...
(function(W, D) {
				...
				body.on("click",".policyStaticMapDataBoard",function(){
					...
					if(btn.hasClass("disabled")){
						...
						if(!ck){
							...
							//일반 데이터보드
							//2018-12-19 융합 전 데이터보드를 누를 경우 오른쪽 빈 공간이 생김
							$(".dataSideBox").stop().animate({"right":"0px"},200);
							$("#normalDataBoard").show();
							$("#fullDataBoard").hide();
							btn.addClass("on").stop().animate({"right":"425px"},200);
							
							if( $("#typeBoxDB a.on").index() == 0 ){
								srvLogWrite( "E0", "04", "04", "01", $policyStaticMap.ui.settingInfo.policy_idx_nm, $policyStaticMap.ui.getLogParams() ); // jrj 로그 > 데이터보드 차트 보기
							} else {
								srvLogWrite( "E0", "04", "04", "02", $policyStaticMap.ui.settingInfo.policy_idx_nm, $policyStaticMap.ui.getLogParams() ); // jrj 로그 > 데이터보드 표 보기
							}
						}else{
							//if($(".dataSideBox").hasClass("full")){ //20210510 jrj
								close();
							//}
						}
					}
	...
	$policyStaticMapDataBoard.Combine = {
			...
			////////// 2017.09.13 [개발팀] 데이터보드기능수정 START ////////////
				...
				switch (parseInt(grade)) {		//leekh SRV_DT_POLICYMAP_LIST 에서 가져옮
					case 1:
						grade = "통계청";
					...
					case 2:
						...
						break;
					case 3:
						if( $policyStaticMap.ui.settingInfo["idx_id"] == '9221137705201710301546072662443762660' ){
							grade = "보건복지부";
						} else if($policyStaticMap.ui.settingInfo["idx_id"] == '4567814844201710301633013080966785401'){
							grade = "경찰청";
						} else if($policyStaticMap.ui.settingInfo["idx_id"] == '5075653758201711211705104277120963624' || $policyStaticMap.ui.settingInfo["idx_id"] == '3074823551201711211711119354566751530'){
							grade = "행정안전부";
						} else {
							grade = "공공데이터";
						}
				...
				//leekh 출처 정보는 SRV_DT_POLICYMAP_APIPARAM 테이블에서 수정
				var data1 = $policyStaticMapLeftmenu.ui.arrParamList[0];
				var data2 = $policyStaticMapLeftmenu.ui.arrParamList[1];
				...
				switch(parseInt(data1.data_div)) {
					...
					case 5: // LBDMS 데이터
						//출처설정
						$("#dataA_originDB").html(data1.source+""); // leekh 20190227 출처 잘못된 txt 삭제함
						break;
					case 6: // 생활SOC
						$("#dataA_originDB,#dataA_origin").html(data1.source);
						break;
					default: // census 데이터
						//출처설정
				...
				switch(parseInt(data2.data_div)) {
					...
					case 5: // LBDMS 데이터
						...
						break;
					case 6: // 생활SOC
						$("#dataB_originDB").html(data1.source);
						break;
					default: // census 데이터
						...
						if(  sCall_param2.year != 'undefined' && typeof sCall_param2.year != 'undefined' ){
							param2year = (sCall_param2.year.replace(/9016/g,"2016"));
						} else if( sCall_param2.base_year != 'undefined' && typeof sCall_param2.base_year != 'undefined' ){
							param2year = (sCall_param2.base_year.replace(/9016/g,"2016"));
						}

						$("#dataB_originDB").html(data2.source+" ("+ param2year +")");
					...
					switch (parseInt($policyStaticMap.ui.settingInfo["idx_type"])) {
						case 1:  // 수요변화형
							...
							default :
								...
								//2019-04-22 [김남민] 정책통계지도 > 9차(8차), 10차 분리 적용. START
								html +=		"<th class='addSideLine'>"+(sCall_param2.year.replace(/9016/g,"2016"))+"년</th>";
								//2019-04-22 [김남민] 정책통계지도 > 9차(8차), 10차 분리 적용. END
								break;
							}
							break;
			...
			getDemandIndexCombineData : function() {
				...
				for (var i=0; i<multiDataA.length; i++) {
					//2019-04-23 [김남민] 정책통계지도 > 좌측을 Tobe 우측을 Asis로 변경. (try 추가) START
					try{
					...
					//2019-04-23 [김남민] 정책통계지도 > 좌측을 Tobe 우측을 Asis로 변경. (try 추가) END
				}
				return {
					...
					case 1:
						...
						switch(parseInt(data1.data_div)) {
							...
							default :
								...
								//2019-04-22 [김남민] 정책통계지도 > 9차(8차), 10차 분리 적용. START
								series.push({name:(sCall_param2.year.replace(/9016/g,"2016"))+"년", data:tmpDataB, color:'#FFC622'});
								//2019-04-22 [김남민] 정책통계지도 > 9차(8차), 10차 분리 적용. END
								break;
							}
						break;
