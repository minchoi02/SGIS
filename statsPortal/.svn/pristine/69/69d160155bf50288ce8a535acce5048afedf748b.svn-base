...
$(function () {
    ...
    */
	...
	function increYear(){
		if(playWay=="l"&& $("#slider").slider("value") == "1961"){				//미니멈 연도가 되면 역재생 중지
			clearInterval(playIntevar);
		}
		if(playWay=="r"&& $("#slider").slider("value") == "2059"){				//맥시멈 연도가 되면 역재생 중지
			clearInterval(playIntevar);
		}
			if(playWay == "r"){
...
<input type="button" value="이전연도" onclick="nextYear('b')" />
...
<input type="button" value="다음연도" onclick="nextYear('a')" />
