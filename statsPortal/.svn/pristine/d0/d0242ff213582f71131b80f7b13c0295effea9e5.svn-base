...
//	setCookie("confirmMsg", "done", 365);
	...
	srvLogWrite( "E0", "02", "01", "00", "", iWidth+"x"+iHeight ); // jrj 로그 >> 튜토리얼
	...
	$("#btn_05").click(function() {
		if($(window).height()>=1080||$(window).width()>=1920){
			if(btChk == 0){
				...
				//mng_s 20200423 이진호, 튜토리얼 현행화
				//대전 서구에 마우스 오버시 툴팁 추가
				$("#click_map_1").mouseover(function(){
					$("#click_map_1_tooltip").css({
						display : '',
						bottom : '300px',
						left : '685px'
					});
				});
				$("#click_map_1").mouseleave(function(){
					$("#click_map_1_tooltip").hide();
				});
				//mng_e 20200423 이진호
	...
	$("#btn_06").click(function() {
		if($(window).height()>=1080||$(window).width()>=1920){
			if(dbChk == 0){
				...
				//mng_s 20200423 이진호, 튜토리얼 현행화
				//데이터 보드 그래프에 마우스 오버 시 툴팁
				$("#dataBoard_graph").css({
					display : '',
					left : '1442px',
					bottom : '410px'
				});
				$("#dataBoard_graph").mouseover(function(){
					$("#dataBoard_graph_tooltip").css({
						display : '',
						bottom : '460px',
						left : '1700px'
					})
				});
				$("#dataBoard_graph").mouseleave(function(){
					$("#dataBoard_graph_tooltip").hide();
				});
				//mng_e 20200423 이진호
		...
		if($(window).height()>=1080||$(window).width()>=1920){
			if(popChk == 0){
				...
				$("#tutorialText").append("<div class=\"title\">"
						...
						+ "<strong style=\"color:#ee7c1a;  font-weight:bold;\">'통계메뉴'</strong> 버튼을 선택해주세요.</p></div>");
						//mng_e 20210416 이진호
				posiInfo(10);
			}else if(popChk == 4){
				$("#btn_10").hide();
