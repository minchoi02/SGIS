...
(function(W, D) {
	...
	$houseAnalysisMap.chart = {
		...
		spiderChart : function(adm_cd, id, listData, bClassId, categoriTitle){
			...
			function titleMove(e) {
				...
				$("#"+id+" .highcharts-axis-labels>span>span").click(function(){
					srvLogWrite('F0','03','03','00',$(this)[0].innerHTML,'');
		...
		indicatorChart : function(mClassObject){
			...
			setIndicatorData(mClassObject,$houseAnalysisMap.search.isAbode?"00":$houseAnalysisMap.search.getRecommendObject().now_resid_sido_cd+$houseAnalysisMap.search.getRecommendObject().now_resid_sgg_cd,isSetStand,function(standResponse){
				//190219_s 주용민
				if((mClassObject.b_class_idx_id == 'HML0002'||mClassObject.b_class_idx_id == 'HML0003') && (mClassObject.m_class_idx_id == 'HMM0004' ||mClassObject.m_class_idx_id == 'HMM0005' || mClassObject.m_class_idx_id == 'HMM0006' || mClassObject.m_class_idx_id == 'HMM0009' || mClassObject.m_class_idx_id == 'HMM0010' || mClassObject.m_class_idx_id == 'HMM0111')){
					var chartResult,data=[],yearCategory=[];
					var adm_cd = $houseAnalysisMap.search.activeAdmCd;
					if(adm_cd.length <= 5){
						$("#toggle_div_2").hide();
					}else if(adm_cd.length >= 7){
						$.ajax({
							url : contextPath+"/ServiceAPI/house/getYearHouseChart.json",
							type:"POST",
							data: {b_class_idx_id : mClassObject.b_class_idx_id,
							...
							success: function(res){
								chartResult = res.result;
								for(var i=0; i<chartResult.length; i++){
									data.push([chartResult[i].base_year, parseFloat(chartResult[i].value.toFixed(2))]);
									yearCategory.push([chartResult[i].base_year]);
								}
								console.log(data);
								var series=[{data:data}];
								$("#detailChart6_2").highcharts({
									exporting: {
										buttons: {
											contextButton: {
												enabled: false
									...
									tooltip:{
										pointFormat : '{point.name}년: <b>{point.y}</b>'
	...
	//									min:0,
		...
		//mng_s 20180508_김건민
		indicatorChart1 : function(mClassObject){
			
			$("#indicator-navigator_1>li li ").removeClass("navigator");
			$("#indicator-navigator_1>li").removeClass("on_navi");
			$("#indicator-navigator_1>li li ").hide();
			$("#indicator-navigator_1>li,#indicator-navigator_1>li li").removeClass("M_on");
			$("#indicator-navigator_1>li[data-id="+mClassObject.b_class_idx_id+"]").addClass("M_on");
			$("#indicator-navigator_1>li[data-id="+mClassObject.b_class_idx_id+"] a[data-m-class="+mClassObject.m_class_idx_id+"]").parent().addClass("M_on");
			for(var i = 0; i<=$houseAnalysisMap.search.recommend.importance_cd.length; i++){
				$("#indicator-navigator_1>li li a[data-m-class ="+$houseAnalysisMap.search.recommend.importance_cd[i]+"]").parent().show();
				$("#indicator-navigator_1>li li a[data-m-class ="+$houseAnalysisMap.search.recommend.importance_cd[i]+"]").parent().addClass("navigator");
			}
			for(var i=0; i<$("#indicator-navigator_1>li").length; i++){
				if($("#indicator-navigator_1>li:eq("+i+") li").hasClass('navigator')){
					$("#indicator-navigator_1>li:eq("+i+")").show();
					$("#indicator-navigator_1>li:eq("+i+")").addClass("on_navi");
				}else{
					$("#indicator-navigator_1>li:eq("+i+")").hide();
				}
			}
			for(var i=0; i<=$("#indicator-navigator_1>li.on_navi").length; i++){
				$("#indicator-navigator_1>li.on_navi:eq("+i+")").css("width",100/$("#indicator-navigator_1>li.on_navi").length+"%");
			}
			...
			$("#detailChart6-avg-text1,#detailChart6-avg-text2").empty().hide();
			var adm_cd = $houseAnalysisMap.search.activeAdmCd;
			var mapOption = $houseAnalysisMap.databoard.getMapOptions(adm_cd);
			var map = $houseAnalysisMap.ui.mapList[$houseAnalysisMap.ui.curMapId];
			var year = map.bnd_year,searchLevel;
			...
			setAvgText($("#detailChart6-avg-text2"),mClassObject,mapOption,year,searchLevel,"N",isSetStand);
			setIndicatorData(mClassObject,$houseAnalysisMap.search.isAbode?"00":$houseAnalysisMap.search.getRecommendObject().now_resid_sido_cd+$houseAnalysisMap.search.getRecommendObject().now_resid_sgg_cd,isSetStand,function(standResponse){
				//190219_s 주용민
				if((mClassObject.b_class_idx_id == 'HML0002'||mClassObject.b_class_idx_id == 'HML0003') && (mClassObject.m_class_idx_id == 'HMM0004' ||mClassObject.m_class_idx_id == 'HMM0005' || mClassObject.m_class_idx_id == 'HMM0006' || mClassObject.m_class_idx_id == 'HMM0009' || mClassObject.m_class_idx_id == 'HMM0010' || mClassObject.m_class_idx_id == 'HMM0111')){
					var chartResult,data=[],yearCategory=[];
	...
	//									min:0,
				...
				$houseAnalysisMap.api.houseAnalysisOrderLists(searchLevel,mClassObject.b_class_idx_id,mClassObject.m_class_idx_id,year,mapOption.sido_cd,mapOption.sgg_cd,false,function(res){
					...
					if(isSetStand){
						...
						drawIndicatorChart1(title, res.result[0].unit, data,{
							type: 'spline',
			                data: lineData,
			                lineColor : "#0054FF",
			                marker: {
	...
	//mng_s 20180508_김건민
	function drawIndicatorChart1(dataName, unit, data, lineData) {
		var xAxisLabelIndex = 0;
		var xAxisCat = [];
		$.each(data,function(cnt,node){
			xAxisCat.push(node.name.split(";")[0]);
	...
	function setIndicatorData(mClassObject,adm_cd,isSetStand,callback){
		...
		setAvgText($("#detailChart6-avg-text1"),mClassObject,mapOption,year,searchLevel,"Y",isSetStand,callback);
	}
	/**
	 * @name               : setAvgText
	...
	function houseChart(isSido,adm_cd){
		...
		$houseAnalysisMap.api.census.getCensusData("API_0604",{"adm_cd" : adm_cd},
			function(res){
				...
				if(res.errCd=="0"){
					var series = [];
					var categories = ['아파트', '연립/다세대', '단독주택', '오피스텔','기타'];
					var setData = function(data){
						var tempData = new Array();
						...
						tempData.push(parseFloat(data.officetel_per?data.officetel_per:0)); //오피스텔 - 비율
