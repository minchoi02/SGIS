 ...
 * 현재 미사용중이며, /js/common/map.js 사용중에 수정 및 오류 발생 시 사용
 *
 */
			...
			map : function() {
				...
				this.createMap = function (delegate, target, opt) {
					...
					if(document.location.href.match("interactiveMap")){
						...
						if(document.location.href.match('interactiveMap/population')){
							$interactiveLeftMenu.ui.setDetailStatsPanel('populationHouse');
							...
							$interactiveLeftMenu.ui.setDetailStatsPanel('populationHouse');
							$interactiveLeftMenu.ui.populationTab('household');
							$('#div_buttonBar').css('left', '360px');
							$('.stepClose.3depth_close').attr('class', 'stepClose 3depth_close on');
...
//					}
				...
				this.setInnerMap = function(isShow) {
					...
					console.log("[urbanMap.js] setInnerMap 호출");
				...
				this.setInnerMap2 = function(isShow) {
					...
					console.log("[urbanMap.js] setInnerMap2 호출");
				...
				this.setInnerMap3 = function(isShow) {
					...
					console.log("[urbanMap.js] setInnerMap3 호출");
				...
				this.addPolygonGeoJson = function (obj, type, opt) {
					...
					console.log("[urbanMap.js] addPolygonGeoJson() 상위111 호출");
					console.log("[urbanMap.js] addPolygonGeoJson() 상위111 type [" + type);
					...
					if(adm_cd != null && adm_cd != undefined){
						console.log("[urbanMap.js] addPolygonGeoJson() 상위111 adm_cd[" + adm_cd);
						console.log("[urbanMap.js] addPolygonGeoJson() 상위111 adm_cd.length[" + adm_cd.length);
					}
					...
					if ( this.isInnerMapShow2 ) {
						...
						/*
						console.log("[urbanMap.js] addPolygonGeoJson() 상위111 this.legend.legendColor [" + this.legend.legendColor);
					...
					// 줌 종료
						...
						this.gMap.on("zoomend", function (e) {
							...
							// 집계구 경계표출
								...
								console.log("[urbanMap.js] zoomend event 발생시 111 ");
								console.log("[urbanMap.js] zoomend event 발생시 this.isInnerMapShow3 [" + this.isInnerMapShow3 + "]");

								// 이전의 경계레벨이 집계구단위가 아닐 경우
								if (that.curPolygonCode != mapPolygonCode) {

									console.log("[urbanMap.js] zoomend event 발생시 222 ");

									that.curPolygonCode = mapPolygonCode;
									if (!that.isInnerMapShow){
										if (that.isDrop) {

											console.log("[urbanMap.js] zoomend event 발생시 333 openApiBoundaryStatsarea() 호출");
				...
				this.autoDownBoundary = function () {
					...
					console.log("[urbanMap.js] autoDownBoundary() 호출");
					console.log("[urbanMap.js] autoDownBoundary() this.curPolygonCode [" + this.curPolygonCode);
					console.log("[urbanMap.js] autoDownBoundary() this.boundLevel [" + this.boundLevel);
					console.log("[urbanMap.js] autoDownBoundary() this.curDropCd [" + this.curDropCd);
				...
				this.setStatsData = function (type, data, showDataParamName, unit, length) {
					...
					console.log("[urbanMap.js] setStatsData() 호출 interactiveMapApi.js와 이름이 같으므로 주의요망");
				...
				this.openApiBoundaryHadmarea = function (adm_cd, year, low_search, atdrc_yn, callback, bounds){ //, zoomLevel) { 2019-03-11 박길섭
					if ( bounds != null && bounds != undefined && this.isInnerMapShow2) { //mng_s kimjoonha grid this.isInnerMapShow2 조건 추가 20180206
						//console.log("[urbanMap.js] openApiBoundaryHadmarea bounds._southWest.x [" + bounds._southWest.x);
						...
						console.log("[urbanMap.js] sopOpenApiHadmareaObj.request 1111 호출전...");
						...
						//console.log("[urbanMap.js] openApiBoundaryHadmarea that.gMap.getCenter() [" + that.gMap.getCenter());
						console.log("[urbanMap.js] openApiBoundaryHadmarea that.zoom [" + that.zoom);
						console.log("[urbanMap.js] openApiBoundaryHadmarea isInnerMapShow3 adm_cd [" + adm_cd);
						console.log("[urbanMap.js] openApiBoundaryHadmarea isInnerMapShow3 this.curSidoCd [" + this.curSidoCd);
						console.log("[urbanMap.js] openApiBoundaryHadmarea isInnerMapShow3 this.curSiggCd [" + this.curSiggCd);
						console.log("[urbanMap.js] openApiBoundaryHadmarea isInnerMapShow3 this.curDongCd [" + this.curDongCd);
						...
						*/
						...
						console.log("[urbanMap.js] sopOpenApiHadmareaObj.request 행정구역 그리드 호출전...");
				...
				this.openApiReverseGeoCode = function (center, callback) {
					console.log("[urbanMap.js] openApiReverseGeoCode 호출");
					// 2022 SGIS5차 추가
					var accessToken = $urbanMain.ui.getToken();
					// 2022 SGIS5차 추가 끝
				...
				this.openApiReverseGeoCodeGrid = function (center) {
					...
					console.log("[urbanMap.js] openApiReverseGeoCodeGrid 호출");
				...
				this.openApiGridArea = function(bounds) {
					...
					console.log("[urbanMap.js] openApiGridArea() 호출");
					...
					console.log("[urbanMap.js] openApiGridArea() sopOpenApiGridAreaObj.request 111");
					...
					console.log("[urbanMap.js] openApiGridArea() sopOpenApiGridAreaObj.request 222");
				...
				this.setLayerColor = function (feature, layer) {
					...
					if (this.dataType == "normal") {
						var isUserData = ( that.mapBtnInfo && that.mapBtnInfo.delegate && that.mapBtnInfo.delegate.dataTypeList && that.mapBtnInfo.delegate.dataTypeList[ that.id ] == "userData" );
...
//										fillColor : that.legend.legendColor[4]
				...
				this.setPolygonDataGeojson = function (geoData) {
					...
					console.log("[urbanMap.js] setPolygonDataGeojson() 호출");
					console.log("[urbanMap.js] setPolygonDataGeojson() this.geojson [" + this.geojson);
					console.log("[urbanMap.js] setPolygonDataGeojson() this.boundLevel [" + this.boundLevel);
					console.log("[urbanMap.js] setPolygonDataGeojson() geoData.pAdmCd [" + geoData.pAdmCd);
					...
					//20200902 jrj
					if( that.mapBtnInfo && that.mapBtnInfo.delegate &&
							that.mapBtnInfo.delegate.dataTypeList && that.mapBtnInfo.delegate.dataTypeList[ that.id ] == "userData" ){
						that.legend.numberData = true;
						that.checkShowCaption();

						if( !$("#showNumberBtn").hasClass("on") ){
							$("#showNumberBtn").addClass("on").text("on");
						}
					} else {
						that.legend.numberData = $("#showNumberBtn").hasClass("on");
					}
						...
						if (geoData.pAdmCd != undefined) {
							switch(parseInt(this.boundLevel)) {
								case 1:
									...
									console.log("[urbanMap.js] setPolygonDataGeojson() tmpLayerCode [" + tmpLayerCode);
									console.log("[urbanMap.js] setPolygonDataGeojson() this.curPolygonCode [" + this.curPolygonCode);
									console.log("[urbanMap.js] setPolygonDataGeojson() this.isInnerMapShow2 [" + this.isInnerMapShow2);
					...
					console.log("[urbanMap.js] setPolygonDataGeojson() 22222 this.data.length [" + this.data.length);

					// 경계데이터에 통계정보를 병합하고, 경계를 그린다.
					if (this.data.length > 0) {

						console.log("[urbanMap.js] setPolygonDataGeojson() geoData.pAdmCd [" + geoData.pAdmCd +"] this.data[0].pAdmCd [" + this.data[0].pAdmCd);
					...
					console.log("[urbanMap.js] setPolygonDataGeojson() geoData.combine [" + geoData.combine);
				...
				this.combineStatsData = function (boundData, isPass) {//
					...
					console.log("[urbanMap.js] combineStatsData() 호출");
										...
										for (key in that.data[k].result[i]) {
											if (key == that.data[k].showData) {
												...
												if (key === "tot_ppltn" 	 ||	  //총인구
												    ...
												    key === "employee_cnt"		  //종사자수 20210422 이진호 추가
												   ) {
													...
													//mng_s 20210421 이진호, 행정구역 단위 그리드 5미만 값 N/A 처리 하면서 로직 추가
													if ( this.isInnerMapShow3 && that.data[k].result[i][key] == "N/A") {
															that.data[k].result[i][key] = 4;
													}
													//mng_e 20210421 이진호
												}
				...
				this.multiLayerControl = {
					...
					openApiBoundaryHadmarea : function (adm_cd, year, low_search, data, atdrc_yn, callback) {
						...
						console.log("[urbanMap.js] sopOpenApiHadmareaObj.request 2222 호출전...");
						//mng_s 업종별 뜨는 지역만 async를 false로 변경
						var async_setting = true;
						//console.log("[urbanMap.js] sopOpenApiHadmareaObj.request $bizStatsLeftMenu [" + $bizStatsLeftMenu );
								  ...
								  switch(parseInt(res.errCd)) {
									  case 0:
										  ...
										  if (that.multiLayerControl.multiData != null) {
											  ...
											  console.log("[urbanMap.js] openApiBoundaryHadmarea() 111");
										  ...
										  if (callback != undefined && typeof callback === "function") {
											  ...
											  console.log("[urbanMap.js] openApiBoundaryHadmarea() 222");
					...
					openApiBoundaryStatsarea : function (adm_cd, year, data, callback) {
						...
						console.log("[urbanMap.js] openApiBoundaryStatsarea() /OpenAPI3/boundary/statsarea.geojson 호출전 adm_cd [" + adm_cd + "]");
						console.log("[urbanMap.js] openApiBoundaryStatsarea() this.isInnerMapShow3 [" + this.isInnerMapShow3 + "]");
				...
				this.openApiReverseGeoCodeForBorough = function (center, callback) {
					...
					console.log("[urbanMap.js] openApiReverseGeoCodeForBorough 호출");
				...
				this.openApiBoundaryHadmareaForBorough = function (adm_cd, year, low_search, atdrc_yn, callback, bounds){
					...
					console.log("[urbanMap.js] sopOpenApiHadmareaForBoroughObj.request 1111 호출전...");
			...
			miniMap : function() {
				...
				this.addPolygonGeoJson = function (obj, type, opt) {
					...
					console.log("[urbanMap.js] addPolygonGeoJson() 하위222 호출");
	...
	(function () {
		$class("sop.openApi.hadmarea.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				console.log("[urbanMap.js] sop.openApi.hadmarea.api res.errCd [" + res.errCd);
			...
			onFail : function (status) {
				console.log("[urbanMap.js] sop.openApi.hadmarea.api onFail [" + status);
			}
		});
	}());
	...
	(function () {
		$class("sop.openApi.gridarea.api").extend(sop.portal.absAPI).define({
			...
			onSuccess : function (status, res, options) {
				...
				console.log("[urbanMap.js] sop.openApi.gridarea.api 호출");
				console.log("[urbanMap.js] sop.openApi.gridarea.api res.errCd [" + res.errCd);
			...
			onFail : function (status) {
				console.log("[urbanMap.js] sop.openApi.gridarea.api onFail status [" + status);
			}
		});
	}());
	...
	(function () {
		$class("sop.openApi.ReverseGeoCode.api").extend(sop.portal.absAPI).define({
			...
			onSuccess : function (status, res, options) {
				...
				console.log("[urbanMap.js] sop.openApi.ReverseGeoCode.api 호출");
				console.log("[urbanMap.js] sop.openApi.ReverseGeoCode.api res.errCd [" + res.errCd);
				...
				if (res.errCd == "0") {
					...
					console.log("[urbanMap.js] sop.openApi.ReverseGeoCode.api that.isInnerMapShow2[" + that.isInnerMapShow2);
					console.log("[urbanMap.js] sop.openApi.ReverseGeoCode.api that.isInnerMapShow3[" + that.isInnerMapShow3);
						...
						//mng_s grid
							...
							case 5:
								...
								if (that.lastGeojsonInfo != null
										...
										&& that.lastGeojsonInfo.year == that.bnd_year) {
									console.log("[urbanMap.js] sop.openApi.ReverseGeoCode.api 집계구 000 [" );
									console.log("[urbanMap.js] sop.openApi.ReverseGeoCode.api 집계구 that.dataGeojson [" + that.dataGeojson);

									if (that.dataGeojson == null) {
										console.log("[urbanMap.js] sop.openApi.ReverseGeoCode.api 집계구 111 [" );
										that.setPolygonDataGeojson(that.lastGeojsonInfo.geojson);

										//that.openApiBoundaryHadmarea(curSidoCd + curSiggCd + curDongCd, that.bnd_year,  "2", "0", that.boundaryCallback, that.bounds);

									}else {
										console.log("[urbanMap.js] sop.openApi.ReverseGeoCode.api 집계구 222 [" );
										that.clearData();
										//that.openApiBoundaryStatsarea(curSidoCd + curSiggCd + curDongCd, that.bnd_year, that.boundaryCallback); //9월 서비스
										that.openApiBoundaryHadmarea(curSidoCd + curSiggCd + curDongCd, that.bnd_year,  "2", "0", that.boundaryCallback, that.bounds);
									}
								}else {
									console.log("[urbanMap.js] sop.openApi.ReverseGeoCode.api 집계구 333 [" );
									//that.openApiBoundaryStatsarea(curSidoCd + curSiggCd + curDongCd, that.bnd_year, that.boundaryCallback); //9월 서비스
									that.openApiBoundaryHadmarea(curSidoCd + curSiggCd + curDongCd, that.bnd_year,  "2", "0", that.boundaryCallback, that.bounds);
								}
								break;
			...
			onFail : function (status) {
				console.log("[urbanMap.js] sop.openApi.ReverseGeoCode.api onFail [" + status);
			}
		});
	}());
	/** ********* OpenAPI 리버스지오코딩. End ********* */
	...
	(function () {
		$class("sop.openApi.ReverseGeoCodeForBorough.api").extend(sop.portal.absAPI).define({
			...
			onSuccess : function (status, res, options) {
				...
				console.log("[urbanMap.js] sop.openApi.ReverseGeoCodeForBorough.api 호출");
				console.log("[urbanMap.js] sop.openApi.ReverseGeoCodeForBorough.api res.errCd [" + res.errCd);
				...
				if (res.errCd == "0") {
					...
					console.log("[urbanMap.js] sop.openApi.ReverseGeoCodeForBorough.api that.isInnerMapShow2[" + that.isInnerMapShow2);
					console.log("[urbanMap.js] sop.openApi.ReverseGeoCodeForBorough.api that.isInnerMapShow3[" + that.isInnerMapShow3);
			...
			onFail : function (status) {
				console.log("[urbanMap.js] sop.openApi.ReverseGeoCodeForBorough.api onFail [" + status);
			}
		});
	}());
	...
	(function () {
		$class("sop.openApi.hadmareaForBorough.api").extend(sop.portal.absAPI).define({
			onSuccess : function (status, res, options) {
				console.log("[urbanMap.js] sop.openApi.hadmareaForBorough.api res.errCd [" + res.errCd);
				var that = options.target;
				if (res.errCd == "0") {
					options["geojson"] = res;
			...
			onFail : function (status) {
				console.log("[urbanMap.js] sop.openApi.hadmareaForBorough.api onFail [" + status);
			}
		});
	}());
