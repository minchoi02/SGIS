...
Draw.DistanceMeasure = Draw.Feature.extend({
	...
	_endVector: function (e) {
		//mng_s 20211101 이진호
		//지도 거리 측정 시 측정 중 우클릭 하지 않고 초기화 버튼 클릭하여 초기화 시 
		//라인이 초기화 되지만 가이드라인이 계속 남는 현상 발생하여
		//event 유무 분기 처리 후 event가 없으면 가이드라인의 utmk를 초기화 시켜서 해결
		if(e != undefined){
	...
	//mng_s 20220119 이진호
	//거리 단위가 km일때 소숫점 두자리 까지 보여달라는 요청이 들어옴에 따른 수정
	//20220120 이진호, 원복해달라고 요청이 다시옴 --> 추후 소수점 요청 시 주석풀고 기존소스 주석
	_setMeterToKm: function (_meter) {
		if (_meter > 1000) {
			var _kilometer = _meter / 1000;
			return '<font color = "red" > ' +  Math.floor(_kilometer)  + ' </font > km'; 	//소수점 없는 km
			//return '<font color = "red" > ' +  Math.floor(_kilometer*100)/100  + ' </font > km';	//소수점 한자리 km
		} else {
