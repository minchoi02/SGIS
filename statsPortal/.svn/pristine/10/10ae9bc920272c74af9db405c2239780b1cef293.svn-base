...
public class GetDensityCombined extends AbsQuery<Map>{
	...
	public Map executeAPI(HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			Iterator<Map.Entry<String,String>> entries = mapParameter.entrySet().iterator();

			List<Map<String,Object>> selectAreaInfo = null;
			
			// 2019-04-14 djlee 수정 시작
			if(mapParameter.get("base_year")!= null && "9016".equals(mapParameter.get("base_year")+"") && "2017".equals(mapParameter.get("base_year")+"")){
				selectAreaInfo = technicalBizMapService.getDensityCombined10(mapParameter);
			}else {
				selectAreaInfo = technicalBizMapService.getDensityCombined(mapParameter);
			}
