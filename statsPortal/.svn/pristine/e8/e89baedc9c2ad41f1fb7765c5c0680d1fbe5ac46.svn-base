...
(function(W, D) {
	...
	$ssaMap.ui = {
			 ...
			 * @history 	 :
			 */
			getTodayStatusMapData : function(btn_id, adm_cd, detail_type, year_mon) {// used
...
//				20190311 손원웅 추가_지도 연도 세팅
				tmpArParams.push({	key: "year",	value: $workRoad.util.dateWithSign(year_mon, ".")	});
				...
				if(this.curDropParams[map.id] != undefined) {
					for(var i = 0; i < this.curDropParams[map.id].param.length; i ++) {
						if (this.curDropParams[map.id].param[i].key == "year") {
							...
							searchYear = this.curDropParams[map.id].param[i].value;
//							20190311 손원웅 추가
//							searchYear = this.curDropParams[map.id].param[i].value + "년 ";
				...
				$("div.sop-tooltip.sop-zoom-animated.sop-tooltip-right").css("width", "120px"); // 툽팁 너비가 작은 원인 불명으로 강제 너비 늘리기 - 2018.11.30	ywKim	신규
			},
