...
(function(W, D) {
	W.$totSurvDetail = W.$totSurvDetail || {};
	...
	$totSurvDetail.ui = {
		...
		init : function(){
			...
			$totSurvMain.ui.appendContent("/view/totSurv/totSurvDetail/main");
			$totSurvDetail.ui.pageReadyYnFlag = "Y";	// 화면 로딩 후 메뉴 재 조회 방지 플래그
		},
		 ...
		 */
		ready : function(){
			$totSurvDetail.ui.selectTotSurvDataList = []	// 선택한 총조사 목록 초기화
			$totSurvDetail.ui.pageReadyYnFlag = "N";	// 화면 로딩 후 메뉴 재 조회 방지 플래그
			$totSurvMain.ui.tmsPathChange("nationwide", "");	// 상단 헤더 지역명 초기화
			$totSurvDetail.ui.clear();	// 초기화
			
			// 좌측 메뉴 연도 삭제
			$(".yearBtn").remove();
			// 년도및 시도(시군구는 시도 생성 후 생성) 콤보박스 셋팅
			$totSurvDetail.ui.setYearCombo();
			$totSurvDetail.ui.getAreaSido($("#detail_sido option:selected").val());

			// 관심주제 조회
			$totSurvDetail.ui.subThemaList();
			// 데이터 조회
			$totSurvDetail.ui.searchList();
			$("#locationPath").html('<button class="totSurvBI" type="button" onclick="location.href=\'/view/totSurv/populationDash\'"></button>');	// 상단 네비게이션 삭제 //20210225 박은식 상단 네비게이션 BI추가
			$totSurvDetailMap.ui.mapToggleId = "";	// 맵 토클 id
		...
		clear : function(){
			...
			$totSurvDetail.ui.page = 1;	// 페이징 초기화
			// 정렬 초기화
			$totSurvDetail.ui.orderTypeNm = "default";
			$totSurvDetail.ui.orderType = "ASC";
			$totSurvDetail.ui.selectSubThemaList = [];	// 선택한 관심주제 초기화
			$totSurvDetailMap.ui.mapToggleId = "";	// 맵 토클 id
			// 관심주제 이미지 초기화
		...
		detailDivResize : function(){
			...
			if($totSurvMain.ui.pageIndex == 0){
				...
				if($totSurvDetailMap.ui.map != undefined && $totSurvDetailMap.ui.map!=null && $totSurvDetailMap.ui.map!=""){ //20201118 박은식 undefined 조건 추가 (맵 확대, 추소시 에러 표출 수정)
					$totSurvDetailMap.ui.map.update();
				}
				$totSurvDetail.ui.chartDIVReset(survId);
			}
		...
		setYearCombo : function(){
			if($totSurvDetail.ui.totLastYear != ''){
				var startYear = 2015;
				
				$("#selYear").empty();
				
				for(var i=Number($totSurvDetail.ui.totLastYear); i>=startYear; i=i-1){
					if(Number($totSurvDetail.ui.totLastYear) == i){
				...
				// 시도/시군구 조회 기준년도
				$totSurvDetail.ui.bndYear = $("#selYear option:selected").val();
			}
		},
		...
		getAreaSido : function(p_sido_cd) {
			...
			//기존에 저장된 정보 있음
			if($totSurvDetail.ui.areaSidoData[$totSurvDetail.ui.bndYear] != undefined) {
				//시도 목록 추가
				var lvResultList = $totSurvDetail.ui.areaSidoData[$totSurvDetail.ui.bndYear];
				...
				$totSurvDetail.ui.getAreaSgg($("#detail_sido option:selected").val(), "999");
			}
			//기존에 저장된 정보 없음
			else {
				// ajax 시작
				$.ajax({
				    ...
				    async: false,
				    data: {
				    	base_year:$totSurvDetail.ui.bndYear
				    }
				}).done(function (res) { // 완료
					if(res.errCd == "0") {
						//시도 목록 저장
						$totSurvDetail.ui.areaSidoData[$totSurvDetail.ui.bndYear] = res.result.sidoList;
		...
		getAreaSgg : function(p_sido_cd, p_sgg_cd) {
			...
			//기존에 저장된 정보 있음
			if(p_sido_cd != undefined && p_sido_cd != null && p_sido_cd != "" && $totSurvDetail.ui.areaSggData[$totSurvDetail.ui.bndYear+p_sido_cd] != undefined) {
				//시군구 목록 추가
				var lvResultList = $totSurvDetail.ui.areaSggData[$totSurvDetail.ui.bndYear+p_sido_cd];
				for(var i = 0; i < lvResultList.length; i++) {
				...
				$.ajax({
					...
					if(res.errCd == "0") {
						...
						if(p_sido_cd != undefined && p_sido_cd != null && p_sido_cd != "") {
							$totSurvDetail.ui.areaSggData[$totSurvDetail.ui.bndYear+p_sido_cd] = res.result.sggList;
						}
					...
					if(res.errCd == 0){
						...
						if(subThemaData.length > 0){
							$.each(subThemaData, function(key, value){
								$totSurvDetail.ui.subThemaSelect(value.sub_thema);
							});
						}
					}
					else {
		...
		searchList : function( clickYn ){			
			...
			if($totSurvDetail.ui.selectSubThemaList.length > 0){
				var selectSubThema = "";
				$.each($totSurvDetail.ui.selectSubThemaList, function(key, value){
					if($totSurvDetail.ui.selectSubThemaList.length-1 != key){
			...
			dataParams.selAdmCd = selAdmCd;
			// 페이지 관련 변수 설정
			dataParams.page = $totSurvDetail.ui.page;
			// 정렬
			if($totSurvDetail.ui.orderTypeNm != "default"){
				dataParams.orderTypeNm = $totSurvDetail.ui.orderTypeNm;
				dataParams.orderType = $totSurvDetail.ui.orderType;
			} else {
				dataParams.orderType = $totSurvDetail.ui.orderType;
			}
		...
		drawSerchList : function(totalDatalist){
			...
			if(totalDatalist.length > 0){		
					...
					$('#totSurvDataList > tbody').append(
						$("<tr/>", {"id":value.surv_id+"Tr"}).append(
							$("<td/>", {"title":value.thema, "style":"width:11%;"}).append(
								$("<img/>", {"src":"/images/totSurv/"+$totSurvDetail.ui.searchThemaImgFileNm($totSurvDetail.ui.bigThema, value.thema), "class":"w25Img"})
							),
							$("<td/>", {"title":value.sub_thema, "style":"width:11%;"}).append(
								$("<img/>", {"src":"/images/totSurv/icon/"+$totSurvDetail.ui.searchThemaImgFileNm($totSurvDetail.ui.subThema, value.sub_thema), "class":"w25Img"})
							),
							...
							$totSurvDetail.ui.listCheckData(value.surv_id, value.surv_nm);
						}),
						$("<tr/>", {"style":"height:6px;"})
					);
				});
				...
				if($totSurvDetail.ui.viewTypeFalg){
					$totSurvDetail.ui.viewTypeFalg = false;
		...
		createTotSurvDetailDiv : function(){
			//$totSurvMain.ui.loading(false);	// 로딩바
			if($totSurvDetail.ui.selectTotSurvDataList.length > 0){
				if($("#detail_sido option:selected").val()=="99"){
					$totSurvDetail.ui.admNm = "전국";
					$totSurvDetail.ui.admCd = "99";
				} else {
					if($("#detail_sgg option:selected").val()=="999"){
						$totSurvDetail.ui.admNm = $("#detail_sido option:selected").text();
						$totSurvDetail.ui.admCd = $("#detail_sido option:selected").val();
					} else{
						$totSurvDetail.ui.admNm = $("#detail_sido option:selected").text()+" "+$("#detail_sgg option:selected").text();
						$totSurvDetail.ui.admCd = $("#detail_sido option:selected").val()+$("#detail_sgg option:selected").val();
					}
				}
				
				$("#totSurvDetailDataDiv").empty();
				
				$.each($totSurvDetail.ui.selectTotSurvDataList, function(key, value){
								...
								$("<div/>", {"class":"detail_titIconBox", "id":totSurvDataList[0]+"btnDiv"}).append(
									...
									$("<button/>", {"class":"detail_titIcon03 on", "title":"지도 접기", "type":"button", "name":"detail_titIcon", "style":"display:none;"}).click(function(){ //20210223 박은식 title명 변경
										if($(".detail_titIcon03").hasClass("on")){
											$totSurvDetail.ui.mapDivToggle(false, totSurvDataList[0]);											
										} else {
											$totSurvDetail.ui.mapDivToggle(true, totSurvDataList[0]);
										}
									}),
									$("<button/>", {"class":"detail_titIcon04 on", "title":"표 접기", "type":"button", "name":"detail_titIcon", "style":"display:none;"}).click(function(){//20210223 박은식 title명 변경
										if($(".detail_titIcon04").hasClass("on")){
											$totSurvDetail.ui.tableDivToggle(false, totSurvDataList[0]);											
										} else {
											$totSurvDetail.ui.tableDivToggle(true, totSurvDataList[0]);
										}
									}),
					...
					// 첫번째 조사표 선택
					if(key == 0){
						$totSurvDetail.ui.createTotSurvDetail(totSurvDataList[0]);
					}
				});
		...
		createTotSurvDetail : function(survId){
			...
			if($("#"+survId+"Detail > div").length == 0){
				$totSurvMain.ui.loading(true);	// 로딩바
				setTimeout(function() {
					$totSurvDetail.ui.createTotSurvSubDetailDiv(survId);
				}, 100);
			} else{
		...
		createTotSurvSubDetailDiv : function(survId){
			...
			$("#"+survId+"Detail").append(
				$("<div/>", {"class":"subDetailContainer", "data-id":survId}).append(
					// 주제별/지역비교
					$("<div/>", {"class":"detail_colWrap"}).append(
						$("<div/>", {"class":"detail_col"}).append(
							$("<div/>", {"class":"detailrow bb pb5" , "style" : "margin-top: -5px;"}).append(
								$("<div/>", {"class":"Detailselectbox"}).append(
									$("<select/>", {"id":survId+"Select", "name":"themaSelectBox"}) //2020.11.03[신예리] width값 삭제
								),
								$("<span/>", {"class":"detailTit-label", "name":"itemSggNm", "text":$totSurvDetail.ui.admNm}).click(function(){									
									$totSurvDetail.ui.areaPopupToggle(true);
								}),
								$("<span/>", {"class":"ColDataSelectTit", "id":"detailChartSelectedItemNm", "Style":"display:none;"}),
								$("<div/>", {"class":"StatusWrap"}).append(
									// 2020.10.29 다른 차트 유형 변경 추가 START
									$("<div/>", {"class":"iconBox"}).append(
										$("<button/>", {"class":"chartBtn", "id":"chartBtn", "title":"차트 유형"}).click(function(){ //20201103 박은식 다른 유형 차트 버튼 title 추가
											chartModal($totSurvDetail.ui.themaChartData[survId], 'itm1', 'cd_nm', 'dt');
									})),
									// 2020.10.29 다른 차트 유형 변경 추가 END
									$("<p/>", {"class":"popTopStatus", "name":survId+"Unit", "id":survId+"ThemaUnit"})
								)
							),
							$("<div/>", {"class":"detail_Graph02 pt5", "id":survId+"ItemChart"})
						),
						$("<div/>", {"class":"detail_col", "id":survId+"AreaRankDiv"}).append(
							$("<div/>", {"class":"detailrow bb pb10"}).append(
								$("<h4/>", {"text":"지역비교"}),
								$("<div/>", {"class":"StatusWrap"}).append(
									$("<span/>", {"class":"popTopStatusData", "id":survId+"TotalArea"}),
									$("<p/>", {"class":"popTopStatus", "id":survId+"AreaRankTitle", "style":"font-weight:700;"}),
									$("<span/>", {"class":"popTopStatusData", "style":"color: #338CDA;", "name":"areaRank", "id":survId+"AreaRank"}), //2020.10.26[신예리] text color 추가
									$("<span/>", {"class":"popTopStatus", "id":survId+"AreaRankSubTitle", "style":"font-weight:700;"}),
									$("<p/>", {"class":"popTopStatus", "name":survId+"Unit", "style":"margin-left:10px;"})
								)
							),
							$("<div/>", {"class":"detail_Graph02", "id":survId+"AreaChart"})
						)
					),
					// 시계열 및 증감율/상위 지역비교
					$("<div/>", {"class":"detail_colWrap"}).append(
						$("<div/>", {"class":"detail_col", "id":survId+"TimeChartDiv"}).append(
							$("<div/>", {"class":"detailrow bb pb10"}).append(
								$("<h4/>", {"text":"시계열 및 증감율"}),
								$("<div/>", {"class":"StatusWrap"}).append(
									$("<p/>", {"class":"popTopStatus", "name":survId+"Unit"})
								)
							),
							$("<div/>", {"class":"detail_Graph02 pt5", "id":survId+"TimeChart"})
						),
						$("<div/>", {"class":"detail_col", "id":survId+"UpperAreaRankDiv"}).append(
								$("<div/>", {"class":"detailrow bb pb10"}).append(
									$("<h4/>", {"text":"상위 지역비교"}),
									$("<div/>", {"class":"StatusWrap"}).append(
										$("<span/>", {"class":"popTopStatusData", "id":survId+"TotalUpperArea"}),
										$("<p/>", {"class":"popTopStatus", "id":survId+"UpperAreaRankTitle", "style":"font-weight:700;"}),
										$("<span/>", {"class":"popTopStatusData", "style":"color: #338CDA;", "name":"areaRank", "id":survId+"UpperAreaRank"}), //2020.10.26[신예리] text color 추가
										$("<span/>", {"class":"popTopStatus", "id":survId+"UpperAreaRankSubTitle", "style":"font-weight:700;"}),
										$("<p/>", {"class":"popTopStatus", "name":survId+"Unit", "style":"margin-left:10px;"})
									)
								),
								$("<div/>", {"class":"detail_Graph02", "id":survId+"UpperAreaChart"})
							)
					),
					// 지도 영역
						...
						$("<div/>", {"class":"detail_col", "style":"overflow:hidden;"}).append( //2020.10.27[신예리] 익스플로러 지도 영역 휠 사용 시 번짐현상 때문에 style 추가
							$("<div/>", {"class":"detailrow bb pb10"}).append(
								...
								$("<button/>", {"type":"button", "class":"detailCol_hide", "title":"영역 숨기기 버튼", "style":"margin-left: auto;"}).click(function(){ //2020.10.27[신예리] 스타일 추가
									$totSurvDetail.ui.mapDivToggle(false, survId);
								})
							),
					...
					// 표 영역
					$("<div/>", {"class":"detail_colWrap", "id":survId+"TableDiv", "style":"display:block;"}).append(
						$("<div/>", {"class":"detail_col"}).append(
							$("<div/>", {"class":"detailrow bb pb10"}).append(
								$("<img/>", {"src":"/images/totSurv/col_icon04.png", "class":"poptiticon"}),
								$("<h4/>", {"id":survId+"MapTit", "style":"width: 70px;"}).append("통계표 보기"), //20210304 박은식 제목 추가
								$("<div/>", {"class":"captionArea", "style":"margin-left: auto;"}).append(
									$("<span/>", {"class":"tit_label", "style":"margin-left: auto;", "name":survId+"Unit"}), //2020.10.27[신예리] span, class 변경
									$("<button/>", {"type":"button", "class":"detailCol_hide", "title":"영역 숨기기 버튼"}).click(function(){
										$totSurvDetail.ui.tableDivToggle(false, survId);
									})
								)
							),
							$("<div/>", {"class":"detail_Graph02 detailTable", "id":survId+"Table", "style":"height:auto;"})
						)
					)
				)
			);
			
			$totSurvDetailMap.ui.createMap(survId+"mapRgn");
			$totSurvDetail.ui.getTotDetailThemaChartList(survId);	// 상세페이지 주제별 차트 목록 조회
		...
		getDataJson : function(survId, item_seq, mode){
			$totSurvDetailMap.ui.selectedSurvId = survId;
			$totSurvDetailMap.ui.selectedItemSeq = item_seq;
			// 맵 정보 조회
			if(mode == "1"){
				var p_map_region = "sido";
				if($totSurvDetail.ui.admCd.length == 2){
					p_map_region = "sido";
				} else {
					p_map_region = "sgg";
				}
				$totSurvDetailMap.ui.drawMapData(p_map_region, "color");				
			} else {
				if($totSurvDetailMap.ui.map != null){
					$totSurvDetailMap.ui.map.update();
				}
			}
			$totSurvDetail.ui.getTotDetailThemaChartDataList(survId, item_seq);	// 주제별 차트 조회
			$totSurvDetail.ui.getTotDetailTmsresChartDataList(survId, item_seq);	// 시계열 차트 조회
			$totSurvDetail.ui.getTotDetailUpperAreaChartDataList(survId, item_seq);	// 상위지역 차트 조회
			$totSurvDetail.ui.getTotDetailAreaRankChartDataList(survId, item_seq);	// 지역비교 차트 조회
			$totSurvDetail.ui.getTotDetailTableDataList(survId, item_seq);	// 표정보 조회
		},
				...
				success: function(res){
					if(res.errCd == 0){
						if(res.result.dataList.length > 0){
							$totSurvDetail.ui.createTable(survId, res.result.dataList, res.result.yearList);
						} else {
							$totSurvDetail.ui.detailDataDivNone(survId+"Table", "table");
						}
					}
					else {
						console.log("failed!");
		...
		createTable : function(survId, dataList, yearList){
			...
			for(var i=0; i<dataList.length; i++) {
				...
				if(dataList[i].cd == $totSurvDetail.ui.chartClickCd){
			...
			$.ajax({
				...
				success: function(res){
					if(res.errCd == 0){
						...
						// 차트 정보 저장
						$totSurvDetail.ui.itemChartInfoDetailList[survId] = res.result.itemChartInfoDetail;
						...
						$totSurvDetail.ui.getDataJson(itemChartInfo[0].surv_id, itemChartInfo[0].item_seq, "1");	// 데이터 조회
					}
					else {
						console.log("failed!");
					}
		...
		setOpenAPIBtnToggle : function(survId, data_cd){			
			...
			if($("#openAPIBtn").css("display") == "block"){
				$totSurvDetail.ui.getOpenAPIAreaSido($("#areaPopup_sido option:selected").val());
				$totSurvDetail.ui.getOpenAPIAreaSgg($("#areaPopup_sido option:selected").val(), $("#areaPopup_sgg option:selected").val());
				$totSurvDetail.ui.getOpenAPIAreaEmdong($("#areaPopup_sido option:selected").val(), $("#areaPopup_sgg option:selected").val(), "00");
			}
		},
		...
		getOpenAPIMapData : function(survId, dataCd){			
			...
			$totSurvDetail.ui.openApiAdmCd = openApiAdmCd;
			
			if(survId == "PH0001"){	// 인구, 주택, 가구
				if(dataCd =="T100"){	// 인구(T100)
					$totSurvDetailMapApi.request.openApiSearchPopulation(openApiAdmCd);
				} else if(dataCd == "T200"){	// 가구(T200)
					$totSurvDetailMapApi.request.openApiHouseHold(openApiAdmCd);
				} else if(dataCd == "T310"){	// 주택(T310)
					$totSurvDetailMapApi.request.openApiHouse(openApiAdmCd);
				}
			} else if(survId == "FS0013" || survId == "FS0315"){	// 농가
				if(dataCd =="T00" || dataCd =="T01"){	// 가구(T00), 가구원총수(T01)
					$totSurvDetailMapApi.request.openApiFarmHouseHold(openApiAdmCd);
				}
			} else if(survId == "FS0013" || survId == "FS0315"){	// 해수면 어업
				if(dataCd =="T01"){	// 가구원총수(T01)
					$totSurvDetailMapApi.request.openApiFisheryHouseHold(openApiAdmCd, "2");
				}
			} else if(survId == "FS0171" || survId == "FS0469"){	// 내수면 어업
				if(dataCd =="T01"){	// 가구원총수(T01)
					$totSurvDetailMapApi.request.openApiFisheryHouseHold(openApiAdmCd, "1");
				}
			} else if(survId == "FS0235" || survId == "FS0532"){	// 임업
				if(dataCd =="T01"){	// 가구원총수(T01)
					$totSurvDetailMapApi.request.openApiForestryHouseHold(openApiAdmCd);
				}
			}
		},
					...
					if(res.errCd == 0){
						...
						$.each(res.result.themaData, function(key, value){
							if(value.subsum_yn == "N"){
								...
								$totSurvDetail.ui.setOpenAPIBtnToggle(survId, value.cd);	// 개방형지도 버튼 토글
							}
						});
						
						$totSurvDetail.ui.themaChartData[survId] = tempData;
						if($totSurvDetail.ui.themaChartData[survId].length > 0){
							$totSurvDetail.ui.themaChartTypeChange($totSurvDetail.ui.themaChartData[survId], survId, "N", "180");							
						} else {
							$totSurvDetail.ui.detailDataDivNone(survId+"ItemChart", "chart");
						}
					}
					else {
						console.log("failed!");
		...
		getTotDetailAreaAtdrcCheck : function(admCd){
			...
			$.ajax({
				...
				if(res.errCd == "0") {
					...
					$totSurvDetail.ui.isAtdrc = res.result.rslt;
			...
			if($totSurvDetail.ui.admCd.length > 2){
				$.ajax({
					...
					success: function(res){
						if(res.errCd == 0){
							if(res.result.dataList.length > 0){ 
								$totSurvDetail.ui.upperAreaChartData[survId] = res.result.dataList;
								// 랭킹 설정
								...
								$totSurvDetail.ui.upperAreaAvgDt[survId] = res.result.avgDt;
								
								setUpperAreaChart($totSurvDetail.ui.upperAreaChartData[survId], survId, "N", "180");
							} else {
								$totSurvDetail.ui.detailDataDivNone(survId+"UpperAreaChart", "chart");
							}
						}
						else {
							console.log("failed!");
				...
				success: function(res){
					if(res.errCd == 0){
						...
						if(res.result.tmsresData.length > 1){ 
							$totSurvDetail.ui.tmsresChartData[survId] = res.result.tmsresData;
							setTmsresChart($totSurvDetail.ui.tmsresChartData[survId], survId, "N", "180");
						} else {
							$totSurvDetail.ui.detailDataDivNone(survId+"TimeChart", "chart");
						}
					}
					else {
						console.log("failed!");
		...
		getTotDetailAreaRankChartDataList : function(survId, item_seq){
			...
			$.ajax({
				...
				success: function(res){
					if(res.errCd == 0){
						if(res.result.dataList.length > 0){ 
							$totSurvDetail.ui.areaRankChartData[survId] = res.result.dataList;
							//20201130 박은식 해당지역 데이터 유무체크 로직 추가 START
							var localDataCheck = false;
							for(var i=0;i<res.result.dataList.length;i++){
								if(res.result.dataList[i].region_cd == $totSurvDetail.ui.admCd){
									localDataCheck = true;
									break;
								}
							}
							if($totSurvDetail.ui.admCd != "99"){
								if(localDataCheck == true){
									...
									if(res.result.level == "sido"){
										...
										if($totSurvDetail.ui.isAtdrc == true){
							...
							if(localDataCheck == true || $totSurvDetail.ui.admCd == "99"){
								$totSurvDetail.ui.areaAvgDt[survId] = res.result.avgDt;
								
								setAreaRankChart($totSurvDetail.ui.areaRankChartData[survId], survId, "N", "180");
							} else {
								$totSurvDetail.ui.detailDataDivNone(survId+"AreaChart", "chart");
							}
		...
		chartMouseOver : function(obj, color){
        	...
        	if($totSurvDetailMap.ui.chartToggleYn=="N"){
		...
		chartMouseOut : function(obj, color){
			if($totSurvMap.ui.selectedObj[0] != obj[0]){
	    		if($totSurvDetailMap.ui.chartToggleYn == "Y"){
		...
		chartDataClear : function(){
			$totSurvDetail.ui.themaChartData = [];	// 주제별 차트
			$totSurvDetail.ui.tmsresChartData = [];	// 시계열 및 증감율
			$totSurvDetail.ui.upperAreaChartData = [];	// 상위지역 비교 차트
			$totSurvDetail.ui.upperAreaAvgDt = []; 	// 상위지역 평균 값
			$totSurvDetail.ui.areaRankChartData = []; 	// 지역비교
			$totSurvDetail.ui.areaAvgDt = [];	// 지역비교 평균 값
		},
		...
		chartDIVReset : function(survId){
			// 주제별 차트
			if($totSurvDetail.ui.themaChartData[survId] != undefined && $totSurvDetail.ui.themaChartData[survId].length != 0){
				$totSurvDetail.ui.themaChartTypeChange($totSurvDetail.ui.themaChartData[survId], survId, "Y", "180");							
			} else {
				$totSurvDetail.ui.detailDataDivNone(survId+"ItemChart", "chart");
			}
			// 시계열 차트
			if($totSurvDetail.ui.tmsresChartData[survId] != undefined && $totSurvDetail.ui.tmsresChartData[survId].length != 0){ 
				setTmsresChart($totSurvDetail.ui.tmsresChartData[survId], survId, "Y", "180");
			} else {
				$totSurvDetail.ui.detailDataDivNone(survId+"TimeChart", "chart");
			}
			// 상위지역비교 차트
			if($totSurvDetail.ui.upperAreaChartData[survId] != undefined && $totSurvDetail.ui.upperAreaChartData[survId].length != 0){
				setUpperAreaChart($totSurvDetail.ui.upperAreaChartData[survId], survId, "Y", "180");
			} else {
				$totSurvDetail.ui.detailDataDivNone(survId+"UpperAreaChart", "chart");
			}
			// 지역비교 차트
			if($totSurvDetail.ui.areaRankChartData[survId] != undefined && $totSurvDetail.ui.areaRankChartData[survId].length != 0){
				setAreaRankChart($totSurvDetail.ui.areaRankChartData[survId], survId, "Y", "180");
			} else {
				$totSurvDetail.ui.detailDataDivNone(survId+"AreaChart", "chart");
			}
		},
		...
		themaChartItmClick : function(obj, d, color, contents){
			...
			if(obj[0] != $totSurvMap.ui.selectedObj[0]){
				...
				// 차트 토글여부 Y
				$totSurvDetailMap.ui.chartToggleYn = "Y";
				
				// 코드 값 셋팅
				if(d.itm_cd != ""){
					$totSurvDetail.ui.chartClickItmCd = d.itm_cd;
				}
				if(d.c1 != ""){
					$totSurvDetail.ui.chartClickC1 = d.c1;
				}
				if(d.c2 != ""){
					$totSurvDetail.ui.chartClickC2 = d.c2;
				}
				if(d.c3 != ""){
					$totSurvDetail.ui.chartClickC3 = d.c3;
				}
				if(d.disp_data_type != ""){
					$totSurvDetail.ui.dispDataType = d.disp_data_type;
				}
				if(d.cd != ""){
					$totSurvDetail.ui.chartClickCd = d.cd;
				}
				
				// 비자치구에서 선택했을 경우
				var tempRegionCd = $totSurvDetail.ui.admCd;
	    		$totSurvDetail.ui.getTotDetailAreaAtdrcCheck(tempRegionCd);		
				...
				$totSurvDetailMap.ui.drawMapData(p_map_region, "color");	// 맵 정보 조회
				$totSurvDetail.ui.getTotDetailTmsresChartDataList(survId, item_seq);	// 시계열 차트 조회
				$totSurvDetail.ui.getTotDetailUpperAreaChartDataList(survId, item_seq);	// 상위지역 차트 조회
				$totSurvDetail.ui.getTotDetailAreaRankChartDataList(survId, item_seq);	// 지역비교 차트 조회
		...
		chartSelectedItemClear : function(){
			// 차트 클릭 관련 초기화
			$totSurvDetailMap.ui.chartToggleYn = "N";
			$totSurvDetail.ui.chartClickItmCd = "";
			$totSurvDetail.ui.chartClickC1 = "";
			$totSurvDetail.ui.chartClickC2 = "";
			$totSurvDetail.ui.chartClickC3 = "";
			$totSurvDetail.ui.chartClickCd = "";
			$totSurvDetail.ui.dispDataType = "";
		...
		chartSelectedSave : function(target, data, color, index, resizeYn, title){
			if(resizeYn == "Y"){
				...
				$totSurvDetail.ui.themaChartItmClick( $("#"+target).find(".eventGroup").eq(index), data, color, title);
			}
		},
	};
	...
	$totSurvDetail.event = {
		 ...
		 */
		setUIEvent : function(){
			console.log("■$totSurvDetail.event.setUIEvent() called.");
			var body = $("body");
			...
			body.on("change", "#detail_sido", function() {
				...
				setTimeout(function() {
					...
					$totSurvDetail.ui.my_sido_cd = $("#detail_sido option:selected").val();
					
					var param = $("#selYear option:selected").val()+"|"+$totSurvDetail.ui.my_sido_cd+"|"+($totSurvDetail.ui.my_sgg_cd?$totSurvDetail.ui.my_sgg_cd:"99");
					srvLogWrite("P0", "09", "04", "01", param, "");
					
					// 시군구 조회
					$totSurvDetail.ui.getAreaSgg($totSurvDetail.ui.my_sido_cd);
					// 관심주제 조회
					$totSurvDetail.ui.subThemaList();
					// 데이터 조회
					$totSurvDetail.ui.searchList();					
			...
			body.on("change", "#detail_sgg", function() {
				...
				setTimeout(function() {
					$totSurvDetail.ui.clear();	// 초기화
					$totSurvDetail.ui.my_sgg_cd = $("#detail_sgg option:selected").val();
					
					var param = $("#selYear option:selected").val()+"|"+$totSurvDetail.ui.my_sido_cd+"|"+($totSurvDetail.ui.my_sgg_cd?$totSurvDetail.ui.my_sgg_cd:"99");
					srvLogWrite("P0", "09", "04", "01", param, "");
					
					$totSurvDetail.ui.getTotDetailAreaAtdrcCheck($("#detail_sido option:selected").val()+$("#detail_sgg option:selected").val());	// 비자치구여부 조회
					// 관심주제 조회
					$totSurvDetail.ui.subThemaList();
					// 데이터 조회
					$totSurvDetail.ui.searchList();					
			...
			body.on("change", "#selYear", function() {
				...
				setTimeout(function() {
					...
					if(selYear ==3 || selYear==5 || selYear == 10){
						$totSurvDetail.ui.bndYear =  $totSurvDetail.ui.totLastYear;
					}
					else{
						$totSurvDetail.ui.bndYear = selYear;
					}
					
					$totSurvDetail.ui.getAreaSido($("#detail_sido option:selected").val());
					$totSurvDetail.ui.getAreaSgg($("#detail_sido option:selected").val(), $("#detail_sgg option:selected").val());
			...
			body.on("click", ".sideCol-column > ul > li", function() {
				...
				if($(this).hasClass("on")){
					...
					$totSurvDetail.ui.selectSubThemaList.splice($totSurvDetail.ui.selectSubThemaList.indexOf(selSubThema), 1);
				}
				else{
					$(this).addClass("on");
					// 목록에서 추가
					var selSubThema = $(this).children().prop("title") ;   // 선택된 소제목명
					$totSurvDetail.ui.selectSubThemaList.push(selSubThema);
				}
				...
				// 데이터 조회
				$totSurvDetail.ui.searchList( true );
			});
			...
			body.on("click", ".detailListHeader", function(){
				// 정렬 설정
				$totSurvDetail.ui.orderTypeNm = $(this).attr("data-id");
				$totSurvDetail.ui.orderType = $(this).attr("data-type");
				
				// 정렬 변경
				if($totSurvDetail.ui.orderType == "ASC"){
			...
			body.on("change", "select[name='themaSelectBox']", function(){
				...
				if($("#areaPopup_sgg option:selected").val() != "999"){
					$totSurvDetail.ui.admCd = $("#areaPopup_sido option:selected").val()+$("#areaPopup_sgg option:selected").val();
				} else {
					$totSurvDetail.ui.admCd = $("#areaPopup_sido option:selected").val();
				}
				
				setTimeout(function() {
					$totSurvDetail.ui.setOpenAPIBtnToggle(tempSurvId, tempDataCd);	// 개방형지도 버튼 토글
					$totSurvDetail.ui.getDataJson(tempSurvId, tempItemSeq, "1");	// 데이터 조회
				}, 100);
			});		
			
			// 지역선택 팝업 x 버튼 클릭
			body.on("click", "#commonTotSurv_detailSidoselectPop_close", function(){
				$totSurvDetail.ui.areaPopupToggle(false);
			});
			...
			body.on("change", "#areaPopup_sido", function() {
				...
				setTimeout(function() {
					// 시군구 조회
					$totSurvDetail.ui.getAreaPopupSido($("#areaPopup_sido option:selected").val());
					$totSurvDetail.ui.getAreaPopupSgg($("#areaPopup_sido option:selected").val(), "999");
				}, 100);
			});
			...
			body.on("change", "#areaPopup_sgg", function() {
				$totSurvMain.ui.loading(true);	// 로딩바
				$totSurvDetail.ui.getTotDetailAreaAtdrcCheck($("#areaPopup_sido option:selected").val()+$("#areaPopup_sgg option:selected").val());	// 비자치구여부 조회
			});
			...
			body.on("click", "#commonTotSurv_detailSidoselectPop_ok", function() {
				...
				if($totSurvMain.ui.pageIndex == 0){
					$totSurvDetail.ui.areaPopupToggle(false);	// 지역선택 팝업 숨김
					$totSurvMain.ui.loading(true);	// 로딩바
					...
					setTimeout(function() {
						// 데이터 조회
						srvLogWrite("P0", "09", "05", "03", $totSurvDetailMap.ui.selectedSurvId, "sido_cd="+$("#areaPopup_sido").val()+",sgg_cd="+($("#areapopup_sgg").val()?$("#areapopup_sgg").val():""));
						$totSurvDetail.ui.getDataJson($totSurvDetailMap.ui.selectedSurvId, $totSurvDetailMap.ui.selectedItemSeq, "1");					
					}, 100);
				}
			...
			body.on("click", "#openAPIBtn", function(){
				...
				if(!$(this).hasClass("on")){
					...
					if($totSurvMain.ui.getCookie('mapPopTit') != "true"){
    			    	...
    			    	$('.CloseWin').hide();
    			    	// 개방형 지도 데이터 조회
    					$totSurvDetail.ui.getOpenAPIMapData(survId, dataCd);
	    			}
					$('.mapInfo').show();
					/** 개방형지도 설명 팝업 END */
				} else {
			...
			body.on("change", "#OpenAPI_sido", function(){
				// 시군구 조회
				$totSurvDetail.ui.getOpenAPIAreaSgg($(this).val(), "999");
			...
			body.on("change", "#OpenAPI_sgg", function(){
				// 읍면동 조회
				$totSurvDetail.ui.getOpenAPIAreaEmdong($("#OpenAPI_sido option:selected").val(), $(this).val(), "00");
         ...
         if(resizeYn == "Y"){
             ...
             .attr("fill", function(d,i) { return colors[i]})
         	 .on("mouseover", function(d){
         		$totSurvDetail.ui.createChartTool(d.surv_year, d.cd_nm, d.dt.replace(/\B(?=(\d{3})+(?!\d))/g,","), d.unit, tool, -20, -100);
         		$totSurvDetail.ui.chartMouseOver($(this), "#576574");
         	 })
         	 .on("mouseout", function(){
         		 tool.css("display", "none");
         		$(this).attr("fill", function(d,i) { return colors[i]})
         		$totSurvDetail.ui.chartMouseOut($(this), "#576574");
			 })
			 .on("click", function(d){
				 ...
				 $totSurvDetail.ui.themaChartItmClick($(this), d, "#576574", d.surv_year+"년 "+d.cd_nm);
			 })
	...
	var g = svg.selectAll(".pie")
	 ...
	 if(resizeYn == "Y"){
		 g.append("path")
		    ...
		    .on("mouseover", function(d, i){
		    	var toolData = data[i];
		    	$totSurvDetail.ui.createChartTool(toolData.surv_year, toolData.cd_nm, numberFormat(toolData.dt), toolData.unit, tool, -20, -100);
	     	  	$totSurvDetail.ui.chartMouseOver($(this), "#576574");
		    })
		    ...
		    .on("click", function(d, i){
				...
				$totSurvDetail.ui.themaChartItmClick($(this), toolData, "#576574", toolData.surv_year+"년 "+toolData.cd_nm);
    		})
...
function setTmsresChart(data, target, resizeYn, height){
	...
	svg.append("g")
		.attr("style", "pointer-events: none;")
		  ...
		  .join("circle")
		.on("mouseover", function(d){
			$totSurvDetail.ui.createChartTool(d.surv_year, "증감율", (d.incORdec == 0) ? "-" : d.incORdec.toFixed(2), (d.incORdec == 0) ? "" : "%", tool, -20, -90);
		})
		.on("mouseout", function(){
			tool.css("display", "none");
		})
...
function setUpperAreaChart(data, target, resizeYn, height){
         ...
         if(resizeYn == "Y"){
             ...
             .attr("fill", function(d){
            	 ...
            	 if(d.region_cd.length == 2){
            		 color = (d.region_cd == $totSurvDetail.ui.admCd.substring(0,2)) ?  "#637FF2" : "#54D1D1";
            	 } else {
            		 color = (d.region_cd == $totSurvDetail.ui.admCd.substring(0,4)+"0") ?  "#637FF2" : "#54D1D1";
            	 }
            	 return color })
         	 .on("mouseover", function(d){
         		$totSurvDetail.ui.createChartTool(d.surv_year, d.region_nm, d.dt.replace(/\B(?=(\d{3})+(?!\d))/g,","), d.unit, tool, -20, -100);
         		$totSurvDetail.ui.chartMouseOver($(this), "#576574");
         	 })
         	 .on("mouseout", function(){
         		 tool.css("display", "none");
  ...
  svg.append("g").attr("style", "color:#878A89; font-size:11px;")
	...
	var dtMax = d3.max(data, function(d){ return Number(d.dt) });
	var dtAve = Number($totSurvDetail.ui.upperAreaAvgDt[target]);
	...
	var xAxisEvent = function(g){ return g.data(data)
		...
		.on("mouseover", function(d){
			...
			$totSurvDetail.ui.createChartTool(d.surv_year, "평균", $totSurvDetail.ui.upperAreaAvgDt[target].replace(/\B(?=(\d{3})+(?!\d))/g,","), d.unit, tool, -20, -100);
		})
		.on("mouseout", function(){
			$(this).css("opacity", "0");
     		 tool.css("display", "none");
...
function setAreaRankChart(data, target, resizeYn, height){
         ...
         if(resizeYn == "Y"){
             ...
             .attr("fill", function(d){
            	 ...
            	 if(d.region_cd.length == 2){
            		 ...
            		 color = (d.region_cd == $totSurvDetail.ui.admCd) ?  "#637FF2" : "#54D1D1";
            	 }
            	 return color })
         	 .on("mouseover", function(d){
         		$totSurvDetail.ui.createChartTool(d.surv_year, d.region_nm, d.dt.replace(/\B(?=(\d{3})+(?!\d))/g,","), d.unit, tool, -20, -100);
  ...
  svg.append("g").attr("style", "color:#878A89; font-size:12px;")
	...
	var dtMax = d3.max(data, function(d){ return Number(d.dt) });
	var dtAve = Number($totSurvDetail.ui.areaAvgDt[target]);
	...
	var xAxisEvent = function(g){ return g.data(data)
		...
		.on("mouseover", function(d){
			$(this).css("opacity", "0.3");
			$totSurvDetail.ui.createChartTool(d.surv_year, "평균", $totSurvDetail.ui.areaAvgDt[target].replace(/\B(?=(\d{3})+(?!\d))/g,","), d.unit, tool, -20, -100);
		})
		.on("mouseout", function(){
			$(this).css("opacity", "0");
     		 tool.css("display", "none");
