...
function resizeColor(c01, c02, cls, max) { // 범례구간설정 색지정
	...
	for (var i = 0; i < arrColor.length; i++) {
		$(cls).prepend("<td ='background:" + arrColor[i] + ";'></td>");
	}
}
...
function sideEvent() {
	...
	body.on("click", ".dataSideBox .bar>a", function() {
		...
		$(".info-job.choice-list").stop().animate({ "right": "5px" }, 200);
	...
	body.on("click", ".interactiveDataBoard", function() {
		...
		if (!ck) {
			...
			$(".info-job.choice-list").stop().animate({ "right": "610px" }, 200);
			if (!full) {
				$(this).addClass("on").stop().animate({ "right": "475px" }, 200);
			} else {
	...
	$("body").on("click",".switchBox",function(){
		...
		if(ck){
			...
			$(this).find(".ball").stop().animate({"left":"64px"},200,'easeOutExpo');
			$(this).find(".txt").stop().animate({"left":"4px"},200,'easeOutExpo');
			introTable = setInterval(function(){
				fnObj.introTable();
			},_timer);
		}else{
			$(this).addClass("off");
			$(this).find(".ball").stop().animate({"left":"2px"},200,'easeOutExpo');
			$(this).find(".txt").stop().animate({"left":"30px"},200,'easeOutExpo');
			clearInterval(dintroTable);
		}
	});
	/* body.on("click", ".dataSideBox .bar>a", function() {
		 $(".dataSideBox").stop().animate({ "right": "-1500px" }, 200);
...
function quickEvent() {
	...
	body.on("click", ".sqTabs a#showNumberBtn", function() {
		var on =  $(this).toggleClass("on");
		
		if (!on) {
			$(".sqTabs a#showNumberBtn").html("on");
			$(this).removeClass("on");
		}else {
			$(this).addClass("on");
			$(".sqTabs a#showNumberBtn").html("on");
			$(".sqTabs a#showNumberBtn").css({"background":"#00bcd4", "color":"#fff", "width":"30px","height": "30px"});
		}
	});
	...
	body.on("click", ".ul-area > li", function() {
		var on = $(this).hasClass("on");
		if (!on) {
			$(".quickBox.step01").stop().animate({ "left": "-244px" }, 200);
			$(".quickBox.step05").stop().animate({ "left": "140px" }, 200);
	...
	body.on("click", ".stepClose", function() {
		...
		$(".expanded").stop().animate({ "left": "-80px" }, 200);
		$(".quickBox.step05").stop().animate({ "left": "-780px" }, 200);
	});
	...
	body.on("click", ".normalBox .ul-area > li", function() {
		$(this).parent("li").index();
		$(this).parents("ul").eq(0).attr("class");
		$(".normalBox .ul-area >li ").removeClass("on");
		...
		$(".quickBox.step05").stop().animate({"left":"280px"},200);
