	...
	$reportFormEvent.UI = {
				...
				savePDF : function() {
					var scrollPos;
		 			...
		 			$.each( $("#wrap .pntPage>div:visible:not(.pntBtn)"), function( i, item ){
		 				if( $( item ).height() > 0 ){
		 					
			 				var item_h = ( $(item).offset().top + $(item).height() );
			 				...
			 				console.log('===== ', items[idx].item, items[idx].h );
		 				
		 					if( item_h > split_h ){
		 						var top = ( split_h - items[ idx-1 ].h );
		 						
		 						console.log('===== top >>  ', top, items[ idx-1 ] );
		 						
		 						$( items[ idx ].item ).addClass("mt-free").data("mt-free",$( item ).css("margin-top") );
		 						$( items[ idx ].item ).css("margin-top", top + 'px' ); //css("border", "1px solid red");
		 						split_h = ( split_h + pdf_h );
		 					}
