...
(function(W, D) {
	...
	srvLogWrite( "H0", "02", "01", "00", "", "" ); //jrj 로그
	...
	$communityForm = {
		ui:{
			 ...
			 */
			setSelectList : function( element, name, noLog ){
				var spanColor,textWidth,cn;
				if(name==="itemPoi"){
					...
					spanColor = "background: #d379ff;border: #9641a7 solid 1px;";
					textWidth = "width: 140px;";
					cn = "LBDMS";
				}

				if( !( name === "stats" ) && !noLog ){
					srvLogWrite( "H0", "02", "02", "03", ( cn == "LBDMS" ? "" : element.data("title") ), cn ); //jrj 로그
				}
	...
	$communityForm.event = {	
		...
		setUIEvent: function() {
			...
			//경계 고정 버튼 클릭
			$communityForm.ui.rq07 = $("#zoom_" + map.mapBtnInfo.id).find(".rightQuick.rq07");
			$communityForm.ui.rq07.click();
			...
			//비밀번호 변경 취소 버튼 클릭시
			$("#change-password-cancel-button").click(function( evt ){
				evt.stopPropagation();
			...
			$("#poi-tab a").click(function(){
				...
				$(this).parent("li").addClass("M_on");
				$("#mydata-list,#theme-list,#lbdms-list").hide();
				$("#"+$(this).data("id")).show();
				return false;
			});
			...
			$("body").on("change","input[name=stats]:checkbox",function(){
				if($(this).is(":checked")){
					if($("input[name=stats]:checkbox:checked").length>5){
						...
						if( $communityForm.ui.rq07.hasClass("on") ){
							$communityForm.ui.rq07.click();
						}
						
						var apiCode = ( $(this).closest("ul").attr("id") === "map-history" ? "02" : "01" );
						srvLogWrite( "H0", "02", "02", apiCode, $(this).data("title"), ""); //jrj 로그
			...
			$("input[name=mydata]:checkbox").change(function(){
				var self = this;
				if( $(self).is(":checked") && $("input[name=lbdms]:checkbox:checked").length > 0 ){
					$communityMapCommon.confirm(
							"알림","나의데이터와 LBDMS 데이터는 둘 중 한건만 선택 가능합니다. 추가된 LBDMS 데이터를 삭제하시겠습니까?",[{
								title : "확인",
								func : function(opt) {
									if( $communityForm.ui.rq07.hasClass("on") ){
										$communityForm.ui.rq07.click();
									...
									$.each($communityMap.ui.lbdmsMarkers,function(cnt,node){
										node.remove();
									});
									$communityMap.ui.lbdmsMarkers = [];
									
									$("input[name=lbdms]:checkbox").prop("checked", false);
									$("#select-list>li[data-type=lbdms]").remove();
									
									$("input[name=mydata]:checkbox").prop("checked",false);
									$(self).prop("checked",true);
									$communityMapApi.request.getMyData($(self).val())
									$communityForm.ui.setSelectList($(self),"mydata");
			...
			//lbdms 체크박스 변경 이벤트
			$("input[name=lbdms]:checkbox").change(function(){
				var self = this;
				
				if( $(self).is(":checked") && $("input[name=mydata]:checkbox:checked").length > 0 ){
					$communityMapCommon.confirm(
							"알림","나의데이터와 LBDMS 데이터는 둘 중 한건만 선택 가능합니다. 추가된 나의데이터를 삭제하시겠습니까?",[{
								title : "확인",
								func : function(opt) {
									if( $communityForm.ui.rq07.hasClass("on") ){
										$communityForm.ui.rq07.click();
									...
									$("input[name=lbdms]:checkbox").prop("checked",false);
									$(self).prop("checked",true);
									$communityMapApi.request.getLbdmsData($(self).val());
									$communityForm.ui.setSelectList($(self),"lbdms");
									if($("#theme-list .current-theme-list>img.M_on").length>0){
										$communityMap.ui.getCompanyPoi($("#theme-list .current-theme-list>img.M_on").data("id"));
									}
								}
			...
			$("#community-form").heumValidation({isReturnCallback:true},function(errors){
				...
				if(errorArray.length>0){
					...
					formData.append("cmmnty_from_ce", $communityMapCommon.ss_school_grade);
					
					if( $communityMapCommon.getParameter("cmmnty_map_id") && communityMapInfo.temp_save_yn !== "Y" ){
						formData.append("cmmnty_map_id",$communityMapCommon.getParameter("cmmnty_map_id"));
						
						modify = true;
						srvLogWrite( "H0", "03", "07", "01", $communityMapCommon.getParameter("cmmnty_map_id"), "" ); //jrj 로그
					}
					...
					$.ajax({
						...
						success: function(res) {
							...
							if(res.errCd=="0"){
								if( !modify ){
									srvLogWrite( "H0", "02", ( $("input[name=temp]").val() === "Y" ? "03" : "04" ), "00", res.result.cmmnty_map_id, "" ); //jrj 로그
								}
								...
								$communityMapCommon.alert("알림","등록되었습니다",function(){
									if($communityMapCommon.hasText($("input[name=temp]").val())&&$("input[name=temp]").val()=="Y"){
										...
										location.href=contextPath+$communityMapCommon.edu_url+"/together_list"+addQuery
									}else{
										location.href=contextPath+$communityMapCommon.edu_url+"/together_view?cmmnty_map_id="+res.result.cmmnty_map_id+$communityMapCommon.addQuery(["cmmnty_map_id"])
									}
								});
							}else{
								$("#community-form input[name=temp]").remove();
				...
				if(res.errCd=="0"||res.errCd=="-100"){
					if(res.result.summaryList.length>0){
						...
						$.each(res.result.summaryList,function(cnt,node){
							...
							if( node && node.path_nm ){
								imagePath = node.path_nm.substring(0,node.path_nm.lastIndexOf("/")); 
								imageName = node.path_nm.substring(node.path_nm.lastIndexOf("/")+1);
							}
				...
				if(res.errCd == "0") {
					if(res.result.success){
						$communityMapCommon.alert("알림", "삭제되었습니다",function(){
							location.href = contextPath+$communityMapCommon.edu_url+"/together_list";
						});
					}else{
