(function(W, D) {
	...
	$(document).ready(function() {
		srvLogWrite('T0','02','03','02','E','contents_id='+contents.contentsId);
	...
	contents = {
		...
		makeContents : function () {
			...
			$.ajax({
				...
				//콘텐츠 정보 
				$(".menuTi").html("<a href='/view/edu/ele/classList?thema_id="+res.result.contents.thema_id+"'>"+res.result.contents.thema_nm+"</a>");
				...
				//퀴즈없을때 
					...
					if("" != res.result.nextContents && null != res.result.nextContents){
						$(".btnPage01.next").attr('href', "javascript:logWriteAndMove('/view/edu/ele/classDetail?contents_id="+res.result.nextContents.contents_id+"','T0','02','03','05','E','contents_id="+res.result.nextContents.contents_id+"')" );
						$('.btnPage01.next i').text('다음 스토리');
						if("Y" == res.result.nextContents.thema_next){
							$(".nextContents").html("다음 주제로 가기");
							$(".btnPage01.next i").html("퀴즈 풀고 <br/>다음 주제로");
						}
					}else{
				...
				//다음콘텐츠
				if("" != res.result.nextContents && null != res.result.nextContents){
					$(".nextContents").attr("onclick","javascript:logWriteAndMove('/view/edu/ele/classDetail?contents_id="+res.result.nextContents.contents_id+"','T0','02','03','05','E','contents_id="+res.result.nextContents.contents_id+"')");
					...
					$(".nextContents").attr("onclick","location.href='/view/edu/ele/classList?thema_id=E01'");
		...
		makeRecommend : function(){
			...
			$.each(contents.recommendList, function(i, val){
				html+="<li><a id='tag_"+val.contents_id+"' href=\"javascript:logWriteAndMove('/view/edu/ele/classDetail?contents_id="+val.contents_id+"','T0','02','03','06',\'"+contents.schoolGrade+"\',\'contents_id="+val.contents_id+"')\" >";
