...
<%
/**************************************************************************************************************************
* Program Name	 : 함께하는지도 Left메뉴 JSP	
...
<div class="quickBox step01" style="width: 430px;">
	<div class="subj">
		<span>함께하는지도</span> <a href="${introUrl }" class="HomeCommunity">함께하는지도 홈</a>
	</div>
	<div id="community-left-menu" class="scrollBox" style="width:430px;">
		<div class="menu-box MapInfoBox">
			<div class="cm_info">
				<jsp:include page="/view/edu/${ss_school_level}/community/layerPopup/communityInfo"/>
				<jsp:include page="/view/edu/${ss_school_level}/community/layerPopup/communityPoi"/>
				<jsp:include page="/view/edu/${ss_school_level}/community/layerPopup/communityNotice"/>
				<c:choose>
					<c:when test="${member_id==communityMapInfo.usr_id }">
						<div class="cm_info_s cm_admin">
							...
							<ul class="admin_menu">
								<li class="am_modify">
									<c:url var="formUrl" value="/view/edu/${ss_school_level}/community/together_form">
										<c:forEach var="parameterItem" items="${param }">
											<c:param name="${parameterItem.key }" value="${parameterItem.value }"/>
										</c:forEach>
									</c:url>
									<c:url var="makeUrl" value="/view/edu/${ss_school_level}/community/auth">
										<c:param name="returnPage" value="${formUrl }"/>
									</c:url>
									<a href="${makeUrl }">정보수정</a>
								</li>
								<li class="am_upload">
									<c:url var="uploadUrl" value="/view/edu/${ss_school_level}/community/together_upload">
										<c:forEach var="parameterItem" items="${param }">
											<c:param name="${parameterItem.key }" value="${parameterItem.value }"/>
										</c:forEach>
									</c:url>
					...
					<c:otherwise>
						<c:if test="${(communityMapInfo.regist_yn=='Y'||communityMapInfo.regist_yn=='W')&&communityMapInfo.cmmnty_partcptn_grant_yn=='Y'&&communityMapInfo.approval_distinct=='A'}">
							<div class="cm_info_s">
								<a href="javascript:$communityLeftMenu.ui.outCommunity();" class="cn_tab_btn no_tab" style="background-image: none;">함께하는지도 탈퇴</a>
							</div>
						</c:if>
					</c:otherwise>
				</c:choose>
	...
	<div id="layer-popup-box">
		<jsp:include page="/view/edu/${ss_school_level}/community/layerPopup/communityPoiRegist"/>
		<jsp:include page="/view/edu/${ss_school_level}/community/layerPopup/communityPoiView"/>
		<jsp:include page="/view/edu/${ss_school_level}/community/layerPopup/communityNoticeRegist"/>
		<jsp:include page="/view/edu/${ss_school_level}/community/layerPopup/communityNoticeView"/>
		<c:if test="${member_id==communityMapInfo.usr_id }">
			<c:choose>
				<c:when test="${communityMapInfo.cmmnty_partcptn_grant_yn=='Y' }">
					<jsp:include page="/view/edu/${ss_school_level}/community/layerPopup/communityMemberManager"/>
				</c:when>
				<c:when test="${communityMapInfo.cmmnty_partcptn_grant_yn=='M' }">
					<jsp:include page="/view/edu/${ss_school_level}/community/layerPopup/communityMtypeMemberManager"/>
				</c:when>
			</c:choose>
		</c:if>
	</div>
	...
	<div class="bottom" id="hideClose"><a href="javascript:void(0)" class="stepClose on">닫기</a></div><!--20년수정반영-->
</div>