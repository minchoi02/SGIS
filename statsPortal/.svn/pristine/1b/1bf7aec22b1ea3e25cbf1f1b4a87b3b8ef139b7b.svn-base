...
if( session.getAttribute("sec_mem_id") != null &&  session.getAttribute("sec_mem_id").equals((String)session.getAttribute("member_id")) && (NowTime-preInsTime) < 300    ) { //운영용
//if( session.getAttribute("sec_mem_id") != null &&  session.getAttribute("sec_mem_id").equals((String)session.getAttribute("member_id")) && (NowTime-preInsTime) < 3    ) { //로컬 테스트용
	...
	try {
		...
		sgis_census_req_tel = request.getParameter("sgis_census_req_tel") == null ? "000" : request.getParameter("sgis_census_req_tel");
