...
function timeTotalPopulationChart(data, target, resizeYn, height){
  //기본셋팅
	...
	var line = d3.line()
				 .x(function(d){ return x(d.surv_year) + x.bandwidth() / 2 })
				 .y(function(d){ return y2(d.dt) });
	...
	var x1 = d3.scaleLinear()
			   .domain([0, d3.max(data, function(d){ return d.dt })])
	...
	svg.append("g")
	    .selectAll("circle")
	   	.data(data)
	    .join("circle")
	   .attr("fill", "#fff") //2020.09.22[신예리] background color 값 변경
	   .attr("stroke", color) //2020.09.22[신예리] stroke 추가
	   .attr("stroke-width", 2) //2020.09.22[신예리] stroke width 추가
	   .attr("style", "pointer-events: none;") //2020.09.22[신예리] mouse event 제거
	   .attr("cx", function(d){ return x(d.surv_year) + x.bandwidth() / 2 })
	   .attr("cy", function(d){ return y2((target == 'timeRankPopulationChart') ? d.rank : d.dt) })	   
	   .attr("r", 4)
	   .attr("radius", 1)
	   .attr("d", arc);
	...
	svg.append("g")
		...
		.join("rect")
		 .on("mouseover", function(d){
			...
			.attr("style", "fill-opacity:0.1;cursor:pointer;") //2020.10.23[신예리] 마우스 커서 추가
			if(target == "tiemGenderPopulationChart"){
				$populationTms.ui.createChartTool(d.surv_year, d.itm_nm, "비율", d.dt.replace(/\B(?=(\d{3})+(?!\d))/g,","), "", tool, 10, -30); //20201013 박은식 %제거 
			} else {
				$populationTms.ui.createChartTool(d.surv_year, d.itm_nm, "", d.dt.replace(/\B(?=(\d{3})+(?!\d))/g,","), "명", tool, 10, -30);
			}
		 })
		 .on("mouseout", function(){
			 tool.css("display", "none");
			 d3.select(this).attr("fill", "#468442")
			.attr("style", "fill-opacity:0;cursor:pointer;") //2020.10.23[신예리] 마우스 커서 추가
		 })
		.attr("x", function(d){ return (x(d.surv_year) + x.bandwidth() / 2)-10 })
		.attr("y", 5)
		.attr('width', 20)
		.attr('height', 120)
	...
	svg.append("g").attr("style", "color:#878A89; font-size:11px;")
		...
		.attr("y", function(d) { return y2(Number(d.dt))-10 })
...
function timeSidoPopulationChart(data, target, resizeYn, height){
	...
	var x = d3.scaleBand()
				.domain(d3.range(data.length))
				.range([margin.left, width - margin.right]) // 20201015 박은식 range 범위 변경
				.padding(0.4)
				
	var y = d3.scaleLinear()
				.domain([0, d3.max(data, function(d){ return (target == 'timeRankPopulationChart') ? (area.length == 2) ?  Number(18)* 1.1 : Number(290)* 1.1 : d.dt* 1.2} )]).nice()
				.range([height - margin.bottom, margin.top])

	var xAxis = function(g){ return g.attr("transform", 'translate(0,'+(height - margin.bottom)+')') // 20201015 박은식 지표범위 위치 이동
									 .call(d3.axisBottom(x)
											 .tickFormat(function(i){ return data[i].surv_year })
											 .tickSizeOuter(0))
				}
	...
	if(resizeYn == "Y"){
		svg.append("g")
			...
			.join("rect")
			.on("mouseover", function(d){
				if(target == "timeRankPopulationChart"){
					var title = ($totSurvMain.ui.selectedArea.length == 2) ? "광역시도 중" : "시군구 중";
					$populationTms.ui.createChartTool(d.surv_year, title, " 랭킹", d.rank, "", tool, -20, -100);
				} else {
					$populationTms.ui.createChartTool(d.surv_year, d.itm_nm, "", d.dt.replace(/\B(?=(\d{3})+(?!\d))/g,","), "명", tool, -20, -100);
				}
				$populationTms.ui.chartMouseOver($(this), "#576574") //2020.09.22[신예리] 마우스오버 컬러 변경
		  })
		  .on("mouseout", function(){
			  tool.css("display", "none")
			  $populationTms.ui.chartMouseOut($(this), $totSurvMain.ui.tempColor)
		  })
			.attr("fill", function(d,i){ return color[i] })
			.attr("color", function(d,i){ return color[i] })
			.attr("x", function(d, i){ return (x(i)+(x.bandwidth()/2) -20) }) //20201111 박은식 시계열 bar 차트  width값 수정
			.attr("y", function(d){ return y(0) })
			.attr("width", 40) //20201111 박은식 시계열 bar 차트  width값 수정
			.attr("height",  function(d){ return y(0) - y((target == 'timeRankPopulationChart') ? (area.length == 2) ?  (Number(18) - Number(d.rank)) : (Number(290) - Number(d.rank)) : d.dt) })
			.attr("y", function(d){ return y((target == 'timeRankPopulationChart') ? (area.length == 2) ? (Number(18) - Number(d.rank)) : (Number(290) - Number(d.rank)) : d.dt) });
		...
		svg.append("g").attr("style", "color:#878A89; font-size:11px;").attr("class", "chartData")
			.selectAll("text")
			.data(data)
			.join("text")
			.attr("text-anchor", "middle")
			.attr("x", function(d, i){ return (x(i))+ x.bandwidth() / 2 })
			.attr("y", function(d) { return y(Number((target=="timeForeignPopulationChart") ? d.dt: (area.length == 2) ? (Number(18) - Number(d.rank)) :(Number(290) - Number(d.rank))))-10 })
			.text( function(d){ return (target=="timeForeignPopulationChart") ? d.dt.replace(/\B(?=(\d{3})+(?!\d))/g,",") : d.rank })
...
function timeRankChart(data, target, resizeYn, height){
	...
	var y2 =  d3.scaleLinear()
				.domain([ 0, (area.length == 2) ? 20 : 300])
				.rangeRound([height - margin.bottom, margin.top]);
	
	var line = d3.line().x(function(d){ return (x(d.surv_year) + (Number(x.bandwidth()) / 2))})
						.y(function(d){ return y2((area.length == 2) ? (Number(18) - Number(d.rank)) : (Number(290) - Number(d.rank))) });
	...
	if(path.node() != null) {		
		...
		.attr("cx", function(d){ return x(d.surv_year) + x.bandwidth() / 2 })
		.attr("cy", function(d){ return y2((area.length == 2) ? (Number(18) - Number(d.rank)) : (Number(290) - Number(d.rank))) })
		.attr("r", 4);
		...
		svg.append("g").attr("style", "color:#878A89; font-size:11px;").attr("class", "chartData")
			...
			.join("text")
			.attr("text-anchor", "middle")
			.attr("x", function(d){ return x(d.surv_year) + x.bandwidth() / 2 })
			.attr("y", function(d) { return y2(Number((area.length == 2) ? (Number(18) - Number(d.rank)) :(Number(290) - Number(d.rank))))-10 })
		...
		.join("rect")
		 .on("mouseover", function(d){
			...
			.attr("style", "fill-opacity:0.1;cursor:pointer;") //2020.10.23[신예리] 마우스 커서 추가
			var title = ($totSurvMain.ui.selectedArea.length == 2) ? "광역시도 중" : "시군구 중";
			$populationTms.ui.createChartTool(d.surv_year, title, " 랭킹", d.rank, "", tool, 10, -30);
		 })
		 .on("mouseout", function(){
			 tool.css("display", "none");
			 d3.select(this).attr("fill", "#468442")
 ...
 // create a g element ('group element') into which we will place the bars for the male population
	...
	if(resizeYn == 'Y'){
		...
		gM.selectAll('rect')
		  ...
		  .on("mouseover", function(d){
				$populationTms.ui.createChartTool(d.surv_year, d.age, " 남자 ", d.males.replace(/\B(?=(\d{3})+(?!\d))/g,","), "명", tool, 80, -30);
				$populationTms.ui.chartMouseOver($(this), "#576574") //2020.09.22[신예리] 마우스오버 컬러 변경
		  })
		  .on("mouseout", function(){
			  tool.css("display", "none");
	...
	if(resizeYn == 'Y'){
		...
		gF.selectAll('rect')
		  ...
		  .append('rect')
		  .on("mouseover", function(d){
				$populationTms.ui.createChartTool(d.surv_year, d.age, "인구", d.females.replace(/\B(?=(\d{3})+(?!\d))/g,","), "명", tool, -200, -30);
				$populationTms.ui.chartMouseOver($(this), "#576574") //2020.09.23[신예리] 마우스오버 컬러 변경
		  })
		  .on("mouseout", function(){
			  tool.css("display", "none");
			  $populationTms.ui.chartMouseOut($(this), $totSurvMain.ui.tempColor)
		...
		gF.selectAll('rect')
		  ...
		  .style("cursor", "pointer") //2020.10.23[신예리] 마우스 커서 추가
		  .on("mouseover", function(d){
				$populationTms.ui.createChartTool(d.surv_year, d.age, " 여자 ", d.females.replace(/\B(?=(\d{3})+(?!\d))/g,","), "명", tool, -200, -30);
				$populationTms.ui.chartMouseOver($(this), "#BC5757") //2020.09.23[신예리] 마우스오버 컬러 변경
		  })
		  .on("mouseout", function(){
			  tool.css("display", "none");
			  $populationTms.ui.chartMouseOut($(this), $totSurvMain.ui.tempColor)
