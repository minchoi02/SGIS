...
(function(W, D) {
	...
	/*$(document).ready(
			function() {	
				...
				*//** ********* kosis start ********* *//*
				//KOSIS 메뉴설정
				if (saMapEtc.isFirstLoading) {
					saMapEtc.getKosisItemRoot();
					saMapEtc.isFirstLoading = false;
				}
				...
				$("#kosisPeriod").change(function(){
					...
					var tmpYear = null;
					var tmpPeriodData = saMapEtc.kosis_data_period_array;
	...
	saMapEtc = {
			...
			reqSetKosisParam : function(selParams, tempAdmCd, atdrc_yn, map) {
				$workRoad.ui.addFnc03List("saMapEtc.reqSetKosisParam");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				console.log("[saMapEtc] reqSetKosisParam call");
				var switchVal = 1;
				
				// 2016.03.22 j.h.Seok
				// 2016.03.29 j.h.Seok
//				if(saMapEtc.gis_se != null || saMapEtc.gis_se != undefined) {
//					switchVal = parseInt(saMapEtc.gis_se);
//				}
											...
											btns = [
												{
													...
													func : function(opt) {
														...
														saMapEtc.doReqKosisData(switchVal, selParams, tempAdmCd, atdrc_yn, map);
													}
												},
			...
			doReqKosisData : function(switchVal, selParams, tempAdmCd, atdrc_yn, map) {
				$workRoad.ui.addFnc03List("saMapEtc.doReqKosisData");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				console.log("[saMapEtc] doReqKosisData call");
				...
				switch(switchVal) {
					case 1:
						if(admCdLen > 1) {
							if (map.isFixedBound) {
								setTimeout(function() {
									saMapEtc.getKosisDataList(selParams, 1, 1, map);
								}, 500)
							}else {
								messageConfirm.open(
										"알림",
										...
										btns = [
												{
												    ...
												    func : function(opt) {
												    	map.setZoom(1);
														setTimeout(function() {
															saMapEtc.getKosisDataList(selParams, 1, 1, atdrc_yn, map);
														}, 500)
												    }
												 },
												 
							    			     {
												   ...
												   disable : false,
												   func : function(opt) {
													   $saSubMenu.ui.updateSearchBtnEffect("", map.id);
												   }
							    			     }   
							    			 ]
								);
					...
					case 2:
						if(admCdLen > 2) {
							if (map.isFixedBound) {
								setTimeout(function() {
									saMapEtc.getKosisDataList(selParams, 3, tempAdmCd.substring(0, 2), atdrc_yn, map);
								}, 500);
							}else {
								messageConfirm.open(
										"알림",
							...
							if(map.zoom <= 1) {
								...
								map.setZoom(3);
								setTimeout(function() {
									saMapEtc.getKosisDataList(selParams, 3, tempAdmCd, atdrc_yn, map);
								}, 500);
							}
						}
						break;
					...
					case 3:
						if(admCdLen > 5) {
							if (map.isFixedBound) {
								setTimeout(function() {
									saMapEtc.getKosisDataList(selParams, 4, tempAdmCd.substring(0, 5), atdrc_yn, map);
								}, 500);
							}else {
								messageConfirm.open(
										"알림",
							...
							if(map.zoom <= 1) {
								...
								map.setZoom(4); //5->4
								setTimeout(function() {
									saMapEtc.getKosisDataList(selParams, 4, tempAdmCd, atdrc_yn, map);
								}, 500);
							}
						}
						break;
					...
					default:
						saMapEtc.getKosisDataList(selParams, null, tempAdmCd, atdrc_yn, map);
						break;
					}
			},
			...
			setKosisParams : function() {
				$workRoad.ui.addFnc03List("saMapEtc.setKosisParams");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				console.log(saMapEtc.curSelectNode);
...
//				$saSubMenu.ui.setParams($saSubMenu.ui.curSelectedStatsType, "");
				$saSubMenu.ui.setParams("kosis", "");
				
				this.curSelectedTitle = btnText;
				$saSubMenu.ui.createSearchBtn("kosis", saMapEtc.atdrc_yn);
				$saSubMenu.ui.searchbtnCnt++;
				
				$saSubMenu.event.stepCloseAnimate(1, "check"); //9월 서비스
			},
			...
			checkSubMenu : function(subObj) {
				$workRoad.ui.addFnc03List("saMapEtc.checkSubMenu");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				if(subObj.childCount > 0) {
					if(subObj.children[0].id == subObj.id + "_progress") {
						saMapEtc.getKosisItemSub(subObj.listId);
					}
					//9월 서비스
					...
					saMapEtc.getKosisStaticDataField();
				}
			},
			...
			setKosisItemOnTree : function(subObj) {
				$workRoad.ui.addFnc03List("saMapEtc.setKosisItemOnTree");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var result = [];
				...
				this.tree = $("#kosisStatsTree").easytree({
					...
					allowActivate: true,
		            disableIcons: true,
		            building:$saSubMenu.event.kosisTreeWidth,
		            stateChanged:$saSubMenu.event.kosisTreeWidth,
		            toggled : $saSubMenu.event.kosisTreeWidth,
		            toggled : function(event, nodes, node) {
						if (node.childCount == null) {
							...
							if(node.id != "no_item_progress" && node.children[0].id == node.id + "_progress") {
								saMapEtc.getKosisItemSub(node.listId);
							}
						} else {
							//$saSubMenu.event.stepCloseAnimate(3, "pass"); 
							saMapEtc.checkSubMenu(node);
							saMapEtc.curSelectNode = node;	//현재 선택된 노드
							saMapEtc.atdrc_yn = node.atdrc_yn;
						}
					},
					selected : function(node) {
						//최하위 노드
						if (node.childCount == 0) {
							saMapEtc.curSelectNodeCheck = true;
//							saMapEtc.checkSubMenu(node);
							saMapEtc.curSelectNode = node;	//현재 선택된 노드
							saMapEtc.atdrc_yn = node.atdrc_yn;
						} else {	//상위 노드
							saMapEtc.curSelectNodeCheck = false;
							$saSubMenu.event.stepCloseAnimate(3, "pass"); 
						}
					}
				})
			...
			appendKosisItemOnTree : function(subObj, options) {
				$workRoad.ui.addFnc03List("saMapEtc.appendKosisItemOnTree");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				// 2016. 03. 28 j.h.Seok modify
				if(subObj.length < 1) {
				...
				for(var i=0; i < subObj.length; i++)
				{
					...
					switch(tempGisSe) {
					case 1:
						tmpObj.text = tmpObj.text + "<img src='/images/workRoad/ico/kosis_gis_se_sido.png' width='4px' height='13px' />";
						break;
						
					case 2:
						tmpObj.text = tmpObj.text + "<img src='/images/workRoad/ico/kosis_gis_se_sido.png' width='4px' height='13px' />" 
							+ "<img src='/images/workRoad/ico/kosis_gis_se_sgg.png' width='4' height='13' />";
						break;
						
					case 3:
						tmpObj.text = tmpObj.text + "<img src='/images/workRoad/ico/kosis_gis_se_sido.png' width='4px' height='13px' />" 
							+ "<img src='/images/workRoad/ico/kosis_gis_se_sgg.png' width='4' height='13' />" 
							+ "<img src='/images/workRoad/ico/kosis_gis_se_adm.png' width='4' height='13' />";
						break;
					}
			...
			setStaticDataFields : function(subObj) {
				$workRoad.ui.addFnc03List("saMapEtc.setStaticDataFields");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				// 세부항목
			...
			exchangeText : function(detail, tempObj) {
				$workRoad.ui.addFnc03List("saMapEtc.exchangeText");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var text = "";
				if(detail.length > 2) {
			...
			setStaticDataFieldsForSearchList : function(subObj, options) {
				$workRoad.ui.addFnc03List("saMapEtc.setStaticDataFieldsForSearchList");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				// 세부항목
				var dataItems = subObj.dataItem;
				var selectDataItem = [];
				for(var i = 0; i < dataItems.length; i++) {
				...
				for(var i = 0; i < options.length; i++) {
					...
					if(temp.key == "org_id") {
						...
						title = temp.value;
						if(title == undefined || title == "undefined" || title == null || title == "null" && saMapEtc.table_name != null) {
							title = saMapEtc.table_name;
				...
				if (saMapEtc.kosis_data_item_detail != null
					...
					tempparams.kosis_data_item_detail = saMapEtc.kosis_data_item_detail;
				} else {
					tempparams.kosis_data_item_detail = " ";
				}
				tempparams.kosis_data_item = saMapEtc.kosis_data_item;
				tempparams.kosis_data_period = saMapEtc.kosis_data_period;
				tempparams.kosis_data_year = saMapEtc.kosis_data_year;
				tempparams.gis_se = adm_cd;
				
				tempparams.obj_var_id = saMapEtc.kosis_obj_var_id;
				tempparams.field_id = saMapEtc.kosis_field_id;
...
//				tmpParams.param_info = JSON.stringify(tmpParams.param_info);
			...
			setDataFieldList : function(subObj, html) {
				$workRoad.ui.addFnc03List("saMapEtc.setDataFieldList");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				// 세부항목
				var dataField = subObj.dataFieldList;
				var selectDataItem = [];
				...
				$("#kosisOrigin").html("출처 : KOSIS(<a href='javascript:saMapEtc.kosisLinked();'>통계표보기</a>)"); //9월 서비스
				
				$("#kosisPeriod").change(function(){
					$workRoad.ui.addFnc03List("saMapEtc.setDataFieldList.event.[1]");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
					$("#kosisYear")
						.find("option")
						.remove()
						.end();
			...
			setResultDataOnMap : function() {
				$workRoad.ui.addFnc03List("saMapEtc.setResultDataOnMap");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var that = this;
				var tempObj = saMapEtc.kosis_result_data;
...
//					this.map.mapInfo.updateLegendRangeKosis(this.kosis_result_data);
			...
			kosisLinked : function() {
				$workRoad.ui.addFnc03List("saMapEtc.kosisLinked");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				if(saMapEtc.tbl_id_linked == null || saMapEtc.tbl_id_linked == undefined) {
					saMapEtc.tbl_id_linked = saMapEtc.tbl_id;
				}
				var url = "http://kosis.kr/statHtml/statHtml.do?";
				url += "orgId=";
				url += saMapEtc.org_id;
				url += "&tblId=";
				url += saMapEtc.tbl_id_linked;
				url += "&conn_path=I2";
			...
			getTableName : function(tbl_id) {
				$workRoad.ui.addFnc03List("saMapEtc.getTableName");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			getKosisItemRoot : function() {
				$workRoad.ui.addFnc03List("saMapEtc.getKosisItemRoot");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			getKosisItemSub : function(list_id) {
				$workRoad.ui.addFnc03List("saMapEtc.getKosisItemSub");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			getKosisStaticDataField : function() {
				$workRoad.ui.addFnc03List("saMapEtc.getKosisStaticDataField");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var kosisStatsStaticField = new kosis.serviceApi.KosisDataStaticFields.api();
				kosisStatsStaticField.addParam("org_id", saMapEtc.org_id);
				kosisStatsStaticField.addParam("tbl_id", saMapEtc.tbl_id);
				kosisStatsStaticField.addParam("obj_var_id", saMapEtc.kosis_obj_var_id);
			...
			getKosisDetailOption : function(options) {
				$workRoad.ui.addFnc03List("saMapEtc.getKosisDetailOption");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var kosisDetailOption = new kosis.serviceApi.kosisDetailOption.api();
				kosisDetailOption.addParam("org_id", saMapEtc.org_id);
				kosisDetailOption.addParam("list_id", saMapEtc.tbl_id);
...
//			},
			...
			getKosisStaticDataFieldForSearchList : function(options) {
				$workRoad.ui.addFnc03List("saMapEtc.getKosisStaticDataFieldForSearchList");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
			...
			getKosisDataList : function(params, zoom, gisSe, atdrc_yn, map) {
				$workRoad.ui.addFnc03List("saMapEtc.getKosisDataList");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				if(gisSe < 2) {
					saMapEtc.pAdmCd = "00";
				} else {
					saMapEtc.pAdmCd = gisSe;
				}
				...
				for(var i = 0; i < params.length; i++) {
					...
					if(params[i].key == "tbl_id") {
						saMapEtc.tbl_id_linked = params[i].value; 
					}
				}
				...
				var shareParams = {
					param : params,
					title : saMapEtc.curSelectedTitle,
					map : map,
					isKosis : true
				};
			...
			setKosisStatsData : function (receiveMap, logOptions) {
				$workRoad.ui.addFnc03List("saMapEtc.setKosisStatsData");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				var tempData = {};
				tempData.kosis = true;
				if(saMapEtc.pAdmCd == "1") {
					saMapEtc.pAdmCd = "00";
				}
				tempData.pAdmCd = saMapEtc.pAdmCd;
...
//				}
			...
			combineKosisStatsData : function(boundData) {
				$workRoad.ui.addFnc03List("saMapEtc.combineKosisStatsData");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				...
				tempData = [];
				tempData = tempList;
				saMapEtc.kosis_result_data = tempData;
			...
			setLegendForKosisStatsData : function(arData) {
				$workRoad.ui.addFnc03List("saMapEtc.setLegendForKosisStatsData");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
//				this.map.valPerSlice = this.calculateLegend(arData);
			...
			dataFieldClose : function() {
				$workRoad.ui.addFnc03List("saMapEtc.dataFieldClose");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				$("#kosisArea").hide();
				/*messageConfirm.open(
						"행정구역통계 설정 종료",
			...
			checkKosisBtn : function() {
				$workRoad.ui.addFnc03List("saMapEtc.checkKosisBtn");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var isKosis = false;
			...
			kosisSearch : function(pagenum) {
				$workRoad.ui.addFnc03List("saMapEtc.kosisSearch");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				//2016.09.05 9월 서비스
			...
			kosisSearchResult : function(result) {
				$workRoad.ui.addFnc03List("saMapEtc.kosisSearchResult");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				saMapEtc.kosis_search_result_data = result.resultdata;
				
				var html = "";
				html += '<div style="text-align: center;">';
				html += '	<a href="javascript:saMapEtc.kosisTreeShow();" class="btnStyle01">KOSIS 목록으로</a>';
				html += '</div>';
				...
				for(var i = 0; i < result.resultdata.length; i ++) {
					...
					html +=			"<td title='"+elem.menu_path+"'><a style='font-size:13px;' href = 'javascript:saMapEtc.kosisClick(";
					html += 		i;
				...
				saMapEtc.kosisPaging(result.totalcount, saMapEtc.kosisCurrentPageIndex);
			...
			kosisTreeShow : function() {
				$workRoad.ui.addFnc03List("saMapEtc.kosisTreeShow");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				$("#kosisStatsTree").show();
			...
			kosisPaging : function(totalCount, currentIndex) {
				$workRoad.ui.addFnc03List("saMapEtc.kosisPaging");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				var pageSize = 10;
				...
				$('#kosis_SearchBox_paging .pages').paging({
					...
					onclick:function(e,page){
						saMapEtc.kosisCurrentPageIndex = page-1;
						saMapEtc.kosisSearch(page-1);
					}
				});
			},
			
			kosisClick : function(index) {
				$workRoad.ui.addFnc03List("saMapEtc.kosisClick");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
				// 2016. 03. 23 j.h.Seok 수정
				var tempData = saMapEtc.kosis_search_result_data[index];
			...
			openApiReverseGeoCode : function (division, url, title) {
				$workRoad.ui.addFnc03List("saMapEtc.openApiReverseGeoCode");	// 임시 - 2019.01.10	ywKim	추가: 사용 안하는 함수 찾는 용도
	...
	if ($workRoad.ui.isSetCallback('saMapEtc')) return;
		...
		$class("kosis.serviceApi.kosisTableName.api").extend(sop.portal.absAPI).define({
			onSuccess : function(status, res) {
				...
				if(res.errCd == "0") {  	
					...
					saMapEtc.table_name = result[0].TBL_NM;
...
//					var result = res.result.rootItems;
					...
					saMapEtc.setKosisItemOnTree(result);
				} else if(res.errCd == "-401") {
					accessTokenInfo();
				} else {
					messageAlert.open("알림", res.errMsg);
...
//					var result = res.result.items;
					var result = res.result.kosis_menu;
					saMapEtc.appendKosisItemOnTree(result, options);
				} else if(res.errCd == "-401") {
					accessTokenInfo();
				} else {
					messageAlert.open("알림", res.errMsg);
		...
		$class("kosis.serviceApi.KosisDataStaticFields.api").extend(sop.portal.absAPI).define({
			onSuccess : function(status, res) {
				...
				if(res.errCd == "0") {
					...
					saMapEtc.setStaticDataFields(result);
					saMapEtc.tbl_id_linked = saMapEtc.tbl_id;
				} else if(res.errCd == "-401") {
					accessTokenInfo();
				} else {
				...
				if(res.errCd == "0") {
					if(result.kosis_detail_option == undefined || result.kosis_detail_option.length < 1) {
						...
						if( result.kosis_detail_option[0] == null ){
							...
							saMapEtc.kosis_obj_var_id = result.kosis_detail_option[0].obj_var_id;
							saMapEtc.kosis_field_id = result.kosis_detail_option[0].field_id;
	...
	(function() {
		$class("kosis.serviceApi.KosisDataStaticFieldsForSearchList.api").extend(sop.portal.absAPI).define({
			onSuccess : function(status, res, options) {
				var result = res.result;
				if(res.errCd == "0") {
					saMapEtc.setStaticDataFieldsForSearchList(result, options);
				} else if(res.errCd == "-401") {
					accessTokenInfo();
				} else {
					messageAlert.open("알림", res.errMsg);
		...
		$class("kosis.serviceApi.kosisDataList.api").extend(sop.portal.absAPI).define({
			onSuccess : function(status, res, options) {
				...
				if(res.errCd == "0") {
					...
					if (map.selectedBoundMode == "multi") {
						saMapEtc.setKosisStatsData(map, options);
						...
						saMapEtc.setLegendForKosisStatsData(tmpData);
						for (var i=0; i<result.length; i++) {
							map.multiLayerControl.reqBoundary(result[i].CODE, result[i], atdrc_yn, function(res) {
								var geoData = saMapEtc.combineKosisStatsData(res);
					...
					//데이터보드 업데이트
//					$interactiveDataBoard.ui.updateDataBoardKosis(res, saMapEtc.curSelectedTitle);
	    ...
	    $class("kosis.openApi.kosissearch.api").extend(sop.portal.absAPI).define({
	        onSuccess : function(status, res, options) {
	            ...
	            if(res.errCd == "0") {
            		...
            		//KOSIS
	            	if(res.errCd == "0") {
	            		saMapEtc.kosisSearchResult(result);
	            	}
	            } else if(res.errCd == "-401") {
			//9월 서비스
	    ...
	    $class("kosis.serviceApi.kosisCodeMapping.api").extend(sop.portal.absAPI).define({
	        onSuccess : function(status, res, options) {
	            if(res.errCd == "0") {
	            	console.log(res);
	            	var result = res.result;
	            	saMapEtc.kosisResultProcess(result.sop_codes, options);
	            } else if(res.errCd == "-401") {
	            	accessTokenInfo();
	            	setTimeout(saMapEtc.kosisSearch(0), 500);
	            } else if (res.errCd == "-100"){
	            	messageAlert.open("알림", res.errMsg);
	            } else {
	                //messageAlert.open("알림", res.errMsg);
