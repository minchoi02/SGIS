	...
	$communityIntro.event = {
		...
		setUIEvent: function() {
			...
			$("#community-search-form").submit(function(){
				...
				srvLogWrite( "H0", "01", "02", "00", "", search_word );//jrj 로그
			...
			$("#community-sort-form").submit(function(){
				...
				srvLogWrite( "H0", "01", "03", "00", "", $("#community-second-sort option:selected").text() ); //jrj 로그
						...
						$.each(res.result.summaryList,function(cnt,node){
							...
							$("#community-list").append(
								$("<div/>",{"class":"sum"}).append(
									...
									$("<h3/>",{"text":node.cmmnty_map_nm,"title":node.cmmnty_map_nm}),
//									$("<p/>",{"text":node.intrcn}),
