...
(function(W, D) {
	...
	$(window).load(function() {
		setTimeout(function() {
			if (window.opener.$thematicMapFrame13 != undefined) {
				window.opener.$thematicMapFrame13.ui.reportLoad();
			}
		}, 500);
	});
	...
	$reportForm.ui = {
				...
				setData : function(data, options, dataGeoJson, poiJson) {
					...
					//네트워크 주제도일때
					if(options["stat_thema_map_id"] == "2qAx0jvYOk20180802165500441EHhhaQZQaK" || options["stat_thema_map_id"] == "3SnEYaTafC20181127142830568kSyMYbSg3S"){
						$("#gridArea").hide();
						$("#pntLegend").hide();
						$("#network_legend").show();
						$("#mask").hide();
						poiJson = "";
						if(options["stat_thema_map_id"] == "3SnEYaTafC20181127142830568kSyMYbSg3S"){
						...
						if(options["stat_thema_map_id"] == "OuQf1ZhcWo20190822091422257GkbDsfsZHi"){
					...
					if(data.param.corp_class_cd == "99999"){
						$("#poiInfoTable").hide();
					}else{
				...
				setLegend : function(data, options) {
					...
					if (window.opener.$thematicMapFrame13 != undefined) {
						map = window.opener.$thematicMapFrame13.ui.mapList[data.id];
					}
...
//					}
				...
				drawPOIGrid : function(data) {
					...
					for (var i=0; i<gridList.length; i++) {
						html += "<tr>";
		 				...
		 				html += 	"<td class='al' >"+gridList[i].adm_addr+"</td>";
				...
				drawGrid : function(data,param) {
					...
					var total = parseFloat(data.total).toFixed(2);//2019-02-28 박길섭
					var unit = data.unit;
...
//					$("#tAdmName").html(adm_nm);
					...
					$(".pntDataBox td.al").css("text-align","right");//2019-02-28 박길섭
