...
%> 
<a href="javascript:void(0)" class="interactiveDataBoard disabled"style="z-index:9999;">데이터보드</a><!--20년수정반영-->

<div class="dataSideBox"style="z-index:9998;"><!--200423수정 (11번. 살고싶은 우리동네 데이터 보드 위에 ICON 올라와 있음, z-index 추가)-->
	<div class="bar">
	  			...
	  			<dl class="dscList">
	   				...
	   				<dt style="display:none;">
	   					<a href="javascript:void(0)" id="data-board-dt-1">
	   						<span class="areaSpan"></span> 추천지역 리스트 <img title="추천된 10개 지역을 보실 수 있으며, 같은 순위일 때는 행정구역 순서대로 나타납니다."  alt="추천된 10개 지역을 보실 수 있으며, 같은 순위일 때는 행정구역 순서대로 나타납니다." src="/img/ico/ico_tooltip02.png" style="position: absolute;padding: 5px ;" />
   						</a>
 					</dt>
	   				<dd style="display:none;">
		   				<div class="List" id="interestList"></div>
		   				<div class="List" id="interestList2"></div>
	   				</dd>
	   				<!-- mng_s 20180508_김건민 -->
	   				<dt style="display:none;">
	   					<a href="javascript:void(0)" id="data-board-dt-3">
	   						<span class="areaSpan"></span>설정된 지표별 현황 보기
	   					</a>
	   				</dt>
	   				<dd style="display:none;">
		   				<div class="IndexStatusBar2" style="width:100%;">
	   						<ul id="indicator-navigator_1">
		   						<c:forEach var="map" items="${mlsfcLists }" varStatus="depth">
									<c:forEach var="depthName1" items="${map.key }">
										<c:if test="${mlsfcLists[depthName1].info.recmd_region_search_disp_yn=='Y'}">
											<li data-id="${depthName1 }">
												<a href="#">${mlsfcLists[depthName1].info.b_class_idx_nm }</a>
												<ul>
													<c:forEach var="indicator" items="${mlsfcLists[depthName1].indicator }" varStatus="status">
														<c:forEach var="mlsfc" items="${indicator.key }">
															<li title="${fn:replace(fn:escapeXml(mlsfcLists[depthName1].indicator[mlsfc].abbrev_exp), cn, br) }">
															   <a href="#" data-b-class="${mlsfcLists[depthName1].indicator[mlsfc].b_class_idx_id }" data-m-class="${mlsfcLists[depthName1].indicator[mlsfc].m_class_idx_id }">${mlsfcLists[depthName1].indicator[mlsfc].m_class_idx_nm }</a>
															</li>
														</c:forEach>
													</c:forEach>
												</ul>
		   				...
		   				<!-- 190219_s 주용민 -->
						<div id="toggle_div_1" class="on" style="display:none;float:right;margin-right:15px;">
							<img src="/img/house/house_toggle1.png" alt="토글버튼" style="cursor:pointer;"/>
						</div>		   				
		   				<div id="detailChart4-chart">
		   					<div id="detailChart6" style="width: 100%;height: 210px;padding-top:32px;"></div>
<!-- 		   					<div id="detailChart6" style="width: 100%;height: 210px;"></div> -->
							<!-- 차트정보 S -->
							<div class="Chart_info">
							  <ul id="detailChart6-avg-text1">
							  </ul>
							  <ul id="detailChart6-avg-text2">
							  </ul>
							</div>
		   					<div id="detailChart6_1" style="width: 100%;height: 294px;display:none;"></div>
							<!-- 차트정보 E -->		   					
		   				</div>
						<!-- 190219_e 주용민 -->
	   				</dd>
	   				...
	   				<dd style="display:none;">
						...
						<div id="detailChart1" style="display:none;"></div>
	   					...
	   					<!-- 190219_s 주용민 -->
						<div id="toggle_div_2" class="on" style="display:none;float:right;margin-right:15px;">
							<img src="/img/house/house_toggle1.png" alt="토글버튼" style="cursor:pointer;"/>
						</div>
	   					<div id="detailChart3-chart">
		   					<div id="detailChart5" style="width: 100%;height: 210px;padding-top:32px;"></div>
<!-- 		   					<div id="detailChart5" style="width: 100%;height: 210px;"></div> -->
							...
							<div class="Chart_info">
							  ...
							  <ul id="detailChart5-avg-text2">
		   					...
		   					<div id="detailChart6_2" style="width: 100%;height: 294px;display:none;"></div>
							<!-- 차트정보 E -->		   					
	   					</div>
	   					<!-- 190219_e 주용민 -->
	   				</dd>
	   				...
	   				<dd style="display:none;">
						<div id="detailChart4" style="padding-right:5px;">
							<div class="ThisAreaInfo">
								<ul>
									<c:set var="censusDataYear" value="2020"/>
									<c:set var="companyDataYear" value="2019"/><!-- 사업체조사 -->
									...
									<li><a class="Title" href="#" data-type="house">주택종류</a>
										<div class="chart">
											<div class="radio">
												...
												<label><input type="radio" name="color-map-house" data-option='{"api":"API_0306","filter":"house_cnt","showData":"house_cnt","data_nm":"오피스텔","unit":"호"}' data-parameters='{"year":"${censusDataYear }","house_type":"06"}' /> 오피스텔</label>
												<label><input type="radio" name="color-map-house" data-option='{"api":"API_0306","filter":"house_cnt","showData":"house_cnt","data_nm":"기타","unit":"호"}' data-parameters='{"year":"${censusDataYear }","house_type":"99"}' /> 기타</label>
											</div>
											<div id="house-chart" class="radio-chart"></div>
											<p class="origin_txt_2" style="padding:0;">거처유형별 가구비율&nbsp;&nbsp;출처 : ${censusDataYear } 인구주택총조사</p>
										</div>
									</li>
									<%
