			...
			setTileMapChart : function(target, width, height, surv_id){
				...
				if($totSurvMain.ui.selectedLevel != "3" || $totSurvMap.ui.isAtdrc){
					...
					if(regionCd.length==2){
						regionCd = 'up:'+regionCd
					}	
					
					var param = {
						surv_year_list: year							// 수록시점
							, org_id_list: "101"							// 조직번호
								, tbl_id_list: "DT_1FO15108"						// 통계표 ID
			    		...
			    		success:function(res){
							if (res) {
								...
								for(var i=0; i<data.length; i++) {
									if(data[i]["OV_L1_KOR"] == '동부' || data[i]["OV_L1_KOR"] == '읍부' || data[i]["OV_L1_KOR"] == '면부' || data[i]["OV_L1_KOR"] == '전국'){
										res.splice(i,1);
										i--;
									}
								}
								...
								var mindomain = Number(data[0].DTVAL_CO);
								var maxdomain = Number(data[data.length-1].DTVAL_CO);
