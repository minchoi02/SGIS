...
(function(W, D) {
	...
	console.log("[totSurv] legendInfo.js");
		...
		legendInfo : function(map) {
			...
			this.initialize = function(delegate) {
				...
				if (this.delegate != undefined && this.delegate.namespace != undefined && this.delegate.namespace === "houseAnalysisMap") {
					...
					// 2020-11-04 [곽제욱] 총조사시각화 주제별 범례색깔 변경 START
					if($totSurvMain.ui.selectedThema == "인구" || $totSurvMain.ui.selectedThema == "가구" || $totSurvMain.ui.selectedThema == "주택" || $totSurvMain.ui.selectedThema == "상세"){					
					...
					// 2020-11-04 [곽제욱] 총조사시각화 주제별 범례색깔 변경 END
				}
			};
			...
			if (valPerSlice.length < this.lv && that.legendType == "auto") {
				...
				for(var i = 1; i<valPerSlice.length;i++){ //2016.04.04 범례
					if(value <= valPerSlice[i]){
						...
						if($inMoreDetailMap.ui.selectedObj[0] != "" && $inMoreDetailMap.ui.selectedObj[0] != null){
				...
				// 2020-10-26 [곽제욱] 선택한 아이템이 존재할경우 범례 색 변경 START
				if($inMoreDetailMap.ui.selectedObj != "" && $inMoreDetailMap.ui.selectedObj != null){
			...
			body.on("click", "#btn_legend_"+that.id, function(){ 
				...
				if(ing=="hide"){  
					...
					if ($("#typeArea_"+that.id).attr("class") == "ring") {
						...
						//$("#legendColor_"+that.id).show();	//2021-09-09 [이영호] 최윤정 주무관 요청으로 숨김
					...
					if ($("#typeArea_"+that.id).attr("class") == "ring") {
						$(".legendLine").hide();
							  ...
							  $("#typeArea_"+that.id).attr("class") == "jum") {
						//$("#legendColor_"+that.id).hide();	//2021-09-09 [이영호] 최윤정 주무관 요청으로 숨김
					}
				}	
			});	
