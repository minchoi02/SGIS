...
function mapClick(){
	...
	body.on("click",".sop-interactive",function(){ 
		...
		if(!ck){
			...
			if(!full){
				$(".interactiveDataBoard").addClass("on").stop().animate({"right":"0"},200);
			}else{
				$(".interactiveDataBoard").addClass("on").stop().animate({"right":"0"},200);
			}
...
function thematicCharts_new(selectedAdmCd, isFirst, type){
	...
	//		            yAxis: 1,
		...
		$('.dscList_new').show();
		$('.area-tit_new').find('span').text(map.adm_nm);
		...
		if(type == 'total') {
			...
			$('.title_new').text(map.dataBoardData[0].base_year + '년 ' + title);
			map.dataBoardData = map.dataBoardData.sort(function(a, b){
			...
			$('#barCharts01').highcharts({
				chart: {
		            type: 'bar', width:150, height:460, marginLeft:0
		        },
		        colors: ['#5b3fb2','#ffffff'],
		        tooltip: {
					enable : true,
					shared: true,
					useHtml : true,
			...
			$('#barCharts02').highcharts({
		        chart: {
		            type: 'bar', width:150, height:460, marginLeft:2 //, margin:[0,0,0,100]
		        },
...
function sideEvent(){
	...
	body.on("click",".interactiveDataBoard",function(){ 
		...
		if(!ck){
			...
			if(!full){
				$(this).addClass("on").stop().animate({"right":"335px"},200);
			}else{
				$(this).addClass("on").stop().animate({"right":"335px"},200);
			}
