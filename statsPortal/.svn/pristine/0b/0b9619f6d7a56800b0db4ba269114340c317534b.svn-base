...
(function(W, D) {
	...
	sLegendInfo = {
		legendInfo : function(map) {
			...
			//SGIS_4 추가 시작
			var fixed_legend_val4 = null; 
			var fixed_legend_val5 = null;
			var fixed_legend_val6 = null;
			var fixed_legend_val7 = null;
			//SGIS_4 추가 끝
...
//					var colorList2 = getCalculColor("#a501b7", "#7c02cd", that.lv);
			...
			body.on("click", "#btn_legend_"+that.id, function(){ 
				...
				if(ing=="hide"){  
					...
					if ($("#typeArea_"+that.id).attr("class") == "ring") {
						...
						if(!map.isMtsNoMapBorder){//SGIS4_1124 추가
						$("#legendColor_"+that.id).show();
						}//SGIS4_1124 추가
			...
			$("body").on("click",".colorck li>a",function(){
				...
				if(dataid =="colorSetting_"+that.id){
					...
					if (that.selectType == "dot") {
						...
						console.log("[legendInfo.js ] changeBubbleColor 호출 ");
	     ...
	     * @author	     : 
	     */
		this.legendCopy2 = function(targetId) {
			var legendCopyLength = this.delegate.mapList.length;
			if(legendCopyLength > 4){
				legendCopyLength =  7
			}else{
				legendCopyLength += 3
			}
			...
			if (this.legendList == null) {
				this.legendList = [];
				for (var i=3; i<legendCopyLength; i++) {
					var map = this.delegate.mapList[i];
					var legendInfo = {
							legendType : map.legend.legendType,
							lv : map.legend.lv,
			...
			if(curPage.match(matchStr)){ //mng_s 20180105 정책맵일경우
				...
				//mng_s 20171016 VIEW가 여러개일 때 범례 고정관련 수정
				if (targetId==3) {
					this.data = sLegendInfo.legendInfo.fixed_legend_val4;
				} else if (targetId==4) {
					this.data = sLegendInfo.legendInfo.fixed_legend_val5;
				} else if (targetId==5) {
					this.data = sLegendInfo.legendInfo.fixed_legend_val6;
				} else if (targetId==6){
					this.data = sLegendInfo.legendInfo.fixed_legend_val7;
				}
			}
			this.legendColor1 = targetLegend.legendColor1;
			this.legendColor2 = targetLegend.legendColor2;
		...
		this.createLegend = function() {
			...
			$("#reverseBtn_"+that.id).click(function(){
				...
				that.map.setLegendColor();
				//SGIS4_1124_시계열 관련 이벤트 시작
				if(map.isMtsNoMapBorder){
					var map01 = $interactiveMultiTimeSeries.ui.getMap(4);
					var map02 = $interactiveMultiTimeSeries.ui.getMap(5);
					var map03 = $interactiveMultiTimeSeries.ui.getMap(6);
					var legendMapList  = [];
					legendMapList.push(map01);
					legendMapList.push(map02);
					legendMapList.push(map03);
					that.mtsReverseBtnEvent(legendMapList);
				}
				//SGIS4_1124_시계열 관련 이벤트 끝
			});	
		...
		//SGIS4_1124_다중시계열 시작 _reversBtn 이벤트관련 
		//map.id 3에만 범례가 있어서... 이벤트 생성
		this.mtsReverseBtnEvent = function(legendList){
			$.each(legendList, function(index, item){
				var that = item.legend;
				if(that.reverseOn==false){
					that.reverseOn=true;
				}else{
					that.reverseOn=false;
