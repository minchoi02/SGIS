...
<head>
	...
	<script>
		...
		var defaultLbdms;
		<c:if test="${fn:trim(communityMapInfo.lbdms)!=''}">
			defaultLbdms = "${communityMapInfo.lbdms}";
		</c:if>
	</script>
	...
	<script src="${pageContext.request.contextPath}/sgis_edu/resource/js/communityMap/view/communityDataBoard.js"></script>
	
	<script src="${pageContext.request.contextPath}/sgis_edu/resource/js/communityMap/view/communityLeftMenu.js"></script>
	<script src="${pageContext.request.contextPath}/sgis_edu/resource/js/communityMap/view/layerPopup/communityPoi.js"></script>
	<script src="${pageContext.request.contextPath}/sgis_edu/resource/js/communityMap/view/layerPopup/communityNotice.js"></script>
	<c:if test="${member_id==communityMapInfo.usr_id }">
	<script src="${pageContext.request.contextPath}/sgis_edu/resource/js/communityMap/view/layerPopup/communityMember.js"></script>
	</c:if>
	
	
	<script src="${pageContext.request.contextPath}/sgis_edu/resource/js/communityMap/view/communityView.js"></script>
	...
	<script src="${pageContext.request.contextPath}/sgis_edu/resource/js/communityMap/communityTutorial.js"></script>
	...
	$(document).ready(function(){
		...
		//mng_s 20200421 이진호
		//함께하는지도 게시물 통계리스트 text색상 변경
		$('.sqListBox.sq03 .sqList ul li a .text').css('color', 'black');
		//mng_e 20200421 이진호
...
<body>
	<div id="wrap" style="height: 473px;">
		<header> 
		...
		<div class="containerBox">
			<div class="rela">
						...
						<div class="toolBar">
							...
							<div class="tb_right" id="btnList_1">
								<ul>
									...
									<li><a onclick="javascript:$communityMap.ui.reportDataSet();" class="tb_report" style="cursor:pointer;" title="보고서 보기"><img src="${pageContext.request.contextPath}/img/ico/ico_toolbars09.png" alt="보고서출력"/></a></li>
								</ul>
							</div>
						</div>
				...
				<c:choose>
					<c:when test="${fn:length(communityMapInfo.stats)>0||fn:length(communityMapInfo.itemList)>0}">
				    	...
				    	<div class="sqListBox sq03" style="left:430px;">
				    		<div class="sqTabs">
				    			...
				    			<c:if test="${fn:trim(communityMapInfo.lbdms)!=''}">
									<span>기본자료</span>
					    			<a href="javascript:$communityLeftMenu.ui.defaultLbdms();" id="defaultLbdmsBtn" class="on">on</a>
				    			</c:if>
				    		...
				    		<div id="searchBtnResultRgn" class="sqList" style="overflow-y: auto; height:200px !important;">
					    			...
					    			<div id="history-list-box">
						    			<ul id="history-list">
						    				<c:forEach var="stat" items="${communityMapInfo.recommend }" varStatus="status">
						    					<c:if test="${stat.map_type ne 'THEME'}">
				    			...
				    			<c:if test="${fn:length(communityMapInfo.itemList)>0 }">
					    			<h3 class="ListTitle">기본 POI</h3> <!--  style="margin-top:60px;" -->
					    			<div id="poi-list-box">
						    			<ul id="poi-list">
					...
					<c:when test="${fn:trim(communityMapInfo.lbdms)!='' }">
						<div class="sqListBox sq03 onlyMydata">
							<div class="sqTabs">
								<span>기본자료</span>
				    			<a href="javascript:$communityLeftMenu.ui.defaultLbdms();" id="defaultLbdmsBtn" class="on">on</a>
				...
				<div class="leftArea">
					...
					<jsp:include page="/view/edu/${ss_school_grade}/community/communityLeftMenu"/>
				</div>
				<div id="dataBoard">
					<jsp:include page="/view/edu/${ss_school_grade}/community/communityDataBoard"></jsp:include>
				</div>
			</div>
		</div>
