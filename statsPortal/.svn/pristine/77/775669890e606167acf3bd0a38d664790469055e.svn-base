...
//				this.updateInfo(res, options);		//해당지역 조회 정보
				...
				if ((res.id == "API_0301" || res.id == "4011" || res.pAdmCd == "00") || this.map.boundLevel != "1") { //9월 서비스
					...
					$("#tsDataBoard #tsStats").closest('dl').hide();
					$("#fullDataBoardTitle").find("#tsDataBoard #tsStats").show();	//2016.03.18 수정
				}else {
					...
					$("#tsDataBoard #tsStats").closest('dl').hide();
					$("#fullDataBoardTitle").find("#tsDataBoard #tsStats").hide();
				}
			},
			...
			updateDataBoardMulti : function(res, options) {
				...
				$("#tsDataBoard #tsTitle").html(options.params.title);	// 제목
				$("#fullDataBoardTitle").find("#tsDataBoard #tsTitle").html("제목 : " + options.params.title);
			 ...
			 */
			updateInfo : function() {
				$workRoad.ui.selectJobStatsDataInfo("I0001"
						, function(data) {
							var title = "";//data.stat_nm + " (" + $workRoad.util.dateWithSign($wrmTodayStatus.ui.today, ".") + ")";
							
							// 일,주,월 기준 표기법 - 2019.01.15	ywKim	변경
							if ($wrmTodayStatus.ui.selected_search_type == "W") {
								title = data.stat_nm + " (" + $workRoad.util.weekWithSign($wrmTodayStatus.ui.selected_today, ".") + ")";
							} else if ($wrmTodayStatus.ui.selected_search_type == "M") {
								title = data.stat_nm + " (" + $workRoad.util.monthWithSign($wrmTodayStatus.ui.selected_today, ".") + ")";
							} else {
								//2018-01-03 지도 화면에 마우스 오버시 "2018"로 뜨고 있음 (YYYY.MM.DD)로 변경 필요.
								title = data.stat_nm + " (" + $workRoad.util.dateWithSign($wrmTodayStatus.ui.selected_today, ".") + ")";
							}
							var sido_nm = ($wrmTodayStatus.ui.sido_nm.length > 0) ? $wrmTodayStatus.ui.sido_nm : "전국";
							
							/** 
							 * 2019-04-30[한광희] 오늘의 구인형황 시군구 선택시 데이터보드 지역 명칭 설정 시작
							 */
							var sgg_nm = ($wrmTodayStatus.ui.sgg_nm.length > 0) ? $wrmTodayStatus.ui.sgg_nm : "";
							var adm_nm = "";
							
							if(sido_nm == "전체") {
								adm_nm = sido_nm;
								$wrmTodayStatus.ui.sgg_nm = "";
							} else {
								if(sgg_nm != "" && sgg_nm != "전체") {
							...
							/** 
							 * 2019-04-30[한광희] 오늘의 구인형황 시군구 선택시 데이터보드 지역 명칭 설정 종료
							 */
							
							
							$("#tsDataBoard #tsTitle").html(title);						// 제목
							$("#tsDataBoard #tsOrigin").html(data.colct_source);		// 출처
							// $("#tsDataBoard #tsArea").html(sido_nm);	// 지역
							$("#tsDataBoard #tsArea").html(adm_nm);	// 2019.05.13[한광희] 지역(시군구) 선택시 데이터보드 지역항목에 시군구 명칭 설정
						}, function(err) {
							alert(err);
						});				
			},
...
//				$("#tsDataBoard #tsTitle").html(title);						// 제목
//				$("#tsDataBoard #tsOrigin").html(options.params.origin);	// 출처
//				$("#tsDataBoard #tsArea").html(options.params.adm_nm);		// 지역
////				$("#tsDataBoard #tsStats").html("통계 :");													//통계는 Pie차트를 통해서 입력
//				$("#fullDataBoardTitle").find("#tsDataBoard #tsTitle").html("제목 : " + title);
//				$("#fullDataBoardTitle").find("#tsDataBoard #tsArea").html("지역 : " + options.params.adm_nm);
////				$("#fullDataBoardTitle").find("#tsDataBoard #tsStats").html("통계 :");						//통계는 Pie차트를 통해서 입력
...
//					$("#tsDataBoard #tsOrigin").show();
//					$("#tsDataBoard #tsOrigin").html(origin);	// 출처
//					$("#fullDataBoardTitle").find("#tsDataBoard #tsOrigin").show();
//					$("#fullDataBoardTitle").find("#tsDataBoard #tsOrigin").html("출처 : " + origin);
//				}
//				
//			},			
			/**
			...
			updateInfoKosis : function(title, value) {
				$("#fullDataBoardTitle").find("#tsDataBoard #tsTitle").html("제목 : " + title);
				$("#tsDataBoard #tsTitle").html(title);	// 제목
				$("#tsDataBoard #tsOrigin").html("");
				$("#tsDataBoard #tsArea").html("");
				$("#tsDataBoard #tsSum").html("");
				$("#fullPieChart").hide();
				$(".fullPieLegend").hide();
				$("#fullTableTimeSeries").hide();
				$("#fullDataBoardTitle").find("#tsDataBoard #tsOrigin").show();
				$("#fullDataBoardTitle").find("#tsDataBoard #tsArea").hide();
				$("#fullDataBoardTitle").find("#tsDataBoard #tsOrigin").html("출처 : KOSIS(<a href='javascript:todayStatusMapEtc.kosisLinked();'>통계표보기</a>)");
				$("#tsDataBoard #tsStats").html("출처 : KOSIS(<a href='javascript:todayStatusMapEtc.kosisLinked();'>통계표보기</a>)");
			},
			...
			updateTargetAreaChart : function(res, options) {
				...
				$("#tsDataBoard #targetCharts").css("height", height+"px");
				$("#tsDataBoard #fullTargetChart").css("width", width+"px");
				...
				$('#tsDataBoard #targetCharts').highcharts(
				{
					chart : { 
											...
											map.dataGeojson.eachLayer(function(layer) {
												if (layer.feature) {
													...
													if (layer.feature.dataIdx == idx-1) {
														...
														$tsMap.callbackFunc.didMouseOverPolygon(e, layer.feature, layer.options.type, map);
													}
												}
											});
										}
				...
				//해당지역 데이터 보기 차트 (Full)
				$('#tsDataBoard #fullTargetChart').highcharts({
			        chart : { 
				...
				//9월 서비스
				$("#tsDataBoard #targetCharts").find('.highcharts-axis-labels text').click(function(){
					//집계구
...
//				$("#tsDataBoard #wrapperChartScroll").css("height", (height+50)+"px");
				$("#tsDataBoard #targetCharts").css("height", height+"px");
			...
			updateTargetAreaTable : function(res, options) {
				...
				for(var i = 0; i < res.result.length + res.result2.length; i ++) {
					...
					if(res.id == "API_0301") {	//인구통계총괄
						if (res.showData == "tot_ppltn") {
							...
							$("#tsDataBoard #tsSum").html(appendCommaToNumber($tsDataBoard.ui.dataSumValue)); 
					...
					//조건결합시
						...
						this.dataBoardSum(elem.right_data_val);	//구인현황추가
						$("#tsDataBoard #tsSum").html(appendCommaToNumber($tsDataBoard.ui.dataSumValue)+res.unit);
						//retDataList.push(parseFloat(elem.data_cnt));
						retDataList.push(parseFloat(elem.right_data_val));	//구인현황추가
					}
				}
				...
				console.log("retDataList : " + JSON.stringify(retDataList));
				console.log("addrDataList : " + JSON.stringify(addrDataList));
				...
				console.log("retDataList3 : " + JSON.stringify(retDataList));
				console.log("addrDataList3 : " + JSON.stringify(addrDataList));
				console.log("addrCdList3 : " + JSON.stringify(addrCdList));
				//정렬
				var dataList = $tsDataBoard.Util.tableDataSort(dataList);
			...
			dataBoardSum : function(val) {
				...
				$tsDataBoard.ui.dataSumValue += parseFloat(val);
			},
			...
			updateTargetAreaTableKosis : function(res) {
				...
				console.log("retDataList1 : " + JSON.stringify(retDataList));
				console.log("addrDataList1 : " + JSON.stringify(addrDataList));
				console.log("addrCdList1 : " + JSON.stringify(addrCdList));
				//정렬
				var dataList = $tsDataBoard.Util.tableDataSort(dataList);
			...
			updateTargetAreaTableMulti : function(res, options) {
				...
				console.log("retDataList2 : " + JSON.stringify(retDataList));
				console.log("addrDataList2 : " + JSON.stringify(addrDataList));
				console.log("addrCdList2 : " + JSON.stringify(addrCdList));
				//정렬
				var dataList = $tsDataBoard.Util.tableDataSort(dataList);
			...
			/*updatePieChart : function(options) {
				...
				var sopPortalPieChartDrawObj = new sop.portal.ts.pieChartDraw.api();
				for (var i=0; i<options.params.param.length; i++) {
			...
			timeSeriesInit : function(params) {
				...
				console.log("[tsDataBoard.js] timeSeriesInit() 호출");
				
//				var dataBoard = this.mapData[this.map_id].dataBoard;
				var dataBoard = this.mapData[$tsMap.ui.curMapId].dataBoard;
				var date = new Date();
...
//				}
			...
			updateTimeSeries : function(options) {
				...
				console.log("tsDataBoard.js @@@@@@@@@@@@@@@@@@@@@@@@@@@@ maxYear1 : " + maxYear);
				...
				console.log("tsDataBoard.js @@@@@@@@@@@@@@@@@@@@@@@@@@@@ maxYear2 : " + maxYear);
				//총조사 사업체일 경우 기본 최소년도는 2000년, 재작년도를 maxYear로 잡는다.
				...
				console.log("tsDataBoard.js @@@@@@@@@@@@@@@@@@@@@@@@@@@@ maxYear3: " + maxYear);
				
				if(options.params.api_id == "API_0307"){
					maxYear= date.getFullYear()-2;
				}
				
				console.log("tsDataBoard.js @@@@@@@@@@@@@@@@@@@@@@@@@@@@ maxYear4 : " + maxYear);
				...
				if (options.params.param != undefined && options.params.param.length > 0) {
					for (var i=0; i<options.params.param.length; i++) {
						if (options.params.param[i].key == "year") {
							...
							$tsMap.ui.curDropParams.time_year = base_year;
				...
				console.log("tsDataBoard.js @@@@@@@@@@@@@@@@@@@@@@@@@@@@ maxYear5 : " + maxYear);
				...
				for(var i = 0; i < dataBoard.yearArray.length; i ++) {
					$("#li_y"+dataBoard.yearArray[i]).on('click', function() {
						...
						if(!$(".btn_clockTypeSetting").hasClass("on")) {
							$tsDataBoard.ui.selectTimeSeries(options, $(this).find("input"));
						}
					});
				}
...
//					this.delegate.shareUrlInfo = [];
					...
					for(var i = 0; i < options.params.param.length; i ++) {
						if(options.params.param[i].key == "year") {
							...
							$tsMap.ui.curDropParams.time_year = $(yearObj).val();
						}
					}
					...
					$tsMap.ui.requestOpenApi(options.params);
				}
			},
			...
			doReqTimeSeries : function() {
				...
				if (dataBoard.selectedYearArray.length > 0) {
					if (dataBoard.timeSeriesCnt == dataBoard.selectedYearArray.length) {
						$tsMapApi.request.mask.close(); //9월 서비스
			...
			timerSeriesPlay : function() {
				//9월 서비스
				var dataBoard = $tsDataBoard.ui.mapData[$tsDataBoard.ui.map_id].dataBoard;
				var map = $tsDataBoard.ui.map;
				...
				if (dataBoard.timeSeriesPushData[dataBoard.timeSeriesCnt].res.errCd == "0") {
					...
					//범례고정일 경우
					if ($tsDataBoard.ui.isFixedLegend) {
						if (map.dataGeojson) {
							map.dataGeojson.remove();
							map.removeCaption();
							map.dataGeojson = null;
						}
						map.legend.valPerSlice = $tsDataBoard.ui.fixedLegend;
					...
					$tsMap.ui.curDropParams[map.id] = options.params;
					dataBoard.timeSeriesCnt++;
			...
			selectChartData : function(properties, index, map_id) {
				...
				var charts = $("#tsDataBoard #targetCharts").highcharts();
				
				if($("#tsDataBoard #targetCharts").is(":visible")) {
...
//							$("#tsDataBoard #wrapperChartScroll").mCustomScrollbar("scrollTo", $("#tsDataBoard #targetCharts").find('.highcharts-axis-labels text').eq(scrollIdx));
...
//							charts.series[0].data[this.lastChatIndex].update({ color: '#2951f2' });
//							$("#tsDataBoard #wrapperChartScroll").mCustomScrollbar("scrollTo", $("#tsDataBoard #targetCharts").find('.highcharts-axis-labels text').eq(lastIndx-1));
						}
					}
				}
				Highcharts.setOptions({
...
//				dataBoard.timer = 0;
			...
			reset : function(map_id) {
				...
				$("#tsDataBoard #tsTitle").html("");		//제목
				$("#tsDataBoard #tsArea").html("");			//지역
				$("#tsDataBoard #tsStats").html("");		//통계
				$("#tsDataBoard #targetCharts").empty();		//표 차트
				...
				$("#fullDataBoardTitle").find("#tsDataBoard #tsTitle").html("");
				$("#fullDataBoardTitle").find("#tsDataBoard #tsOrigin").html("");
				$("#fullDataBoardTitle").find("#tsDataBoard #tsArea").html("");
				$("#fullDataBoardTitle").find("#tsDataBoard #tsStats").html("");
				
				$("#fullTableTimeSeries").html("");		//조회년도
				$("#tsDataBoard #fullTargetChart").empty();				//막대 차트
	...
	$tsDataBoard.event = {
			 ...
			 */	
			setUIEvent : function() {
				console.log("■$tsDataBoard.event.setUIEvent() called.");
				
//				var body = $("body");
				
				//투명도 설정 바
				$("#tsDataSideBox #dataSlider").slider({
			    	range: "min",
			        min: 5,
			        max: 10,
			        value: 10,
			        slide: function( event, ui ) {  //ui.value
			        	$("#tsDataSideBox, #tsDataBoardToggle").css("opacity", ui.value*0.1);
				    }
			    });
				$("#tsDataSideBox, #tsDataBoardToggle").css( "opacity", $("#tsDataSideBox #dataSlider").slider( "value" ) );
				
				//닫기 버튼
				$workRoad.event.set("click","#tsDataSideBox .bar>a",function(){ 
					$("#tsDataSideBox").removeClass("full");
					$("#tsDataSideBox").stop().animate({"right":"-1500px"},200);
					$("#tsDataBoardToggle").removeClass("on").stop().animate({"right":"0"},200);
					$(".sop-right").stop().animate({"right":"0px"},200);//2019-06-12 박길섭
				});
				
				//4개 탭 열고 닫기
				$workRoad.event.set("click","#tsDataBoard .dscList dt>a",function(){
				...
				//데이터보드 열고 닫기
				$workRoad.event.set("click","#tsDataBoardToggle",function(){
					
					var ck = $(this).hasClass("on");
					if(!ck){
						//일반 데이터보드
						$(".sop-right").stop().animate({"right":"558px"},200);//2019-06-12 박길섭
						$("#tsDataSideBox").stop().animate({"right":"0px"},200);
//						$("#tsDataSideBox").stop().animate({"right":"50px"},200);
						$("#tsDataBoard").show();
						$("#fullDataBoard").hide();
						...
						// 2019.03.13 접근log 생성
						srvLogWrite('D0', '02', '04', '01', '',  $("#tsDataBoard #tsStatPath").text());
						
					}else{
						
						// 데이터보드 닫기 - 2019.01.16	ywKim	변경 : full databoard 사용안함
						$("#tsDataSideBox").removeClass("full");
						$("#tsDataSideBox").stop().animate({"right":"-1500px"},200);
						$(this).removeClass("on").stop().animate({"right":"0"},200);
						$(".sop-right").stop().animate({"right":"0px"},200);//2019-06-12 박길섭
						// old - 2019.01.16
//						var full = $("#tsDataSideBox").hasClass("full");
//						if(!full){
//							//KOSIS, 공공데이터, 나의데이터일 경우 확장 데이터보드를 열지 않는다.
//							if($tsDataBoard.ui.curSelectedStatsType == "kosis"
//										|| $tsDataBoard.ui.curSelectedStatsType == "publicData"
//										|| $tsDataBoard.ui.curSelectedStatsType == "userData") {
//								
//								//데이터보드 닫기
//								$("#tsDataSideBox").removeClass("full");
//								$("#tsDataSideBox").stop().animate({"right":"-1500px"},200);
...
//						}
				...
				//표보기, 차트보기
				$workRoad.event.set("click","#tsDataBoard .typeBox>a",function(){ 
...
//						$(this).parents(".compareBox").find(".censusChart").show();
						$('#tsDataBoard #wrapperChartScroll').show();
...
//						$(this).parents(".compareBox").find(".censusChart").hide();
						$('#tsDataBoard #wrapperChartScroll').hide();
						...
						// 2019.03.13 접근log 생성
						srvLogWrite('D0', '02', '04', '02', '', $("#tsDataBoard #tsStatPath").text());

					}
			    });
				//데이터보기 엑셀 다운로드
				$workRoad.event.set("click","#tsDataBoard .btn_excelDownload", function(){
					...
					$(this).parents(".compareBox").eq(0).find(".tables").eq(0).find("div").find("table tr").each(function(){
					 ...
					 // 2019.03.13 접근log 생성
					 srvLogWrite('D0', '02', '04', '03', '', $("#tsDataBoard #tsStatPath").text());
				});
				
				//시계열 세팅 버튼
				$workRoad.event.set("click","#tsDataBoard .btn_clockTypeSetting",function(){ 
					$(this).addClass("on");
					$(".yearList li").show();
					...
					if ($(".btn_clockTypeLegend").text() == "on") {
						$(".year_input").hide();
						$(".year_radio").show();
						if ($tsDataBoard.ui.fixedLegendYear != null) {
							$(".year_radio").each(function() {
								if ($(this).val() == $tsDataBoard.ui.fixedLegendYear) {
									$(this).prop("checked");
								}
							});
						}
				...
				//시계열 확인
				$workRoad.event.set("click","#tsDataBoard .btn_clockTypeOk",function(){
					var dataBoard = $tsDataBoard.ui.mapData[$tsDataBoard.ui.map_id].dataBoard;
					...
					$(".yearList li").each(function(i){
						...
						if(!ck){
							...
							console.log("[tsDataBoard.js] $(this).find(\"input\").val() [" + $(this).find("input").val());
						}
					});
				...
				//시계열 플레이/스탑 버튼
				$workRoad.event.set("click", "#tsDataBoard .btn_clockTypePlay", function() {
					...
					if ( sop.isInnerMapShow2 || sop.isInnerMapShow3 ) { //그리드이면
						...
						if($(this).text() == "play") {
							...
							dataBoard.isTimeSeriesPlay = true;
							//9월 서비스
							$tsDataBoard.ui.map.isDrop = true;
							$tsDataBoard.ui.map.undoDropLayerBounds();
							$tsDataBoard.ui.map.setFixedBoundLevel(true);
							...
							clearInterval(dataBoard.timer);
							$tsDataBoard.ui.map.setFixedBoundLevel(false); //9월 서비스
						}
					}
				...
				//시계얄-범례고정 on/off
				$workRoad.event.set("click", "#tsDataBoard .btn_clockTypeLegend", function() {
					if($(this).text() == "off") {
						$(this).text("on");
						...
						$(".year_radio").each(function() {
							...
							if (radio) {
								$tsDataBoard.ui.fixedLegendYear = $(this).val(); 
							}
						})
						$tsDataBoard.ui.isFixedLegend = true;
						...
						$(".year_input").show();
						$(".year_radio").hide();
						$tsDataBoard.ui.isFixedLegend = false;
					}
				});
				
				//시계열-범례고정 버튼 선택
				$workRoad.event.set("click", "#tsDataBoard .year_radio", function() {
					$tsDataBoard.ui.fixedLegendYear = $(this).val(); 
				});
				
				
				//시계열 년도 div 클릭
				$workRoad.event.set("click", "#tsDataBoard #tableTimeSeries div", function() {
...
//				$("#tsDataBoard #wrapperChartScroll").mCustomScrollbar({axis:"xy",advanced: { autoExpandHorizontalScroll: true }});
				$("#tsDataBoard #wrapperFullChartScroll").mCustomScrollbar({axis:"x",advanced: { autoExpandHorizontalScroll: true }});
			},
			...
			dataBoardOpen : function() {
				...
				if(!$("#tsDataBoardToggle").hasClass("on")) {
					$("#tsDataBoardToggle").click();
					$("#dataBoard").show();
				}
			}
	};
