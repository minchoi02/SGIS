...
<title>SGIS에듀 자유게시판 | 통계지리정보서비스</title>
<script>
var nowPage = "<%=nowPage %>";
...
function getEduBoardModView(){
	 ...
	 jQuery.ajax({
	 		...
	 		success:function(data){
	 			...
	 			if(data.result.getEduBoardView == undefined){
	 				alert("잘못된 경로입니다.");
	 				window.location.href = '/edu/jsp/board/edu_board_list.jsp';
	 			}else{
		 			if("<%= member_id%>" != data.result.getEduBoardView.reg_member_id){
		 				alert("잘못된 경로입니다.");
		 				window.location.href = '/edu/jsp/board/edu_board_list.jsp';
		 			}
	 			}
