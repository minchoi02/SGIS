...
(function(W, D) {
	...
	console.log("[administStats] legendInfo.js");
	...
	sLegendInfo = {
		legendInfo : function(map) {
			...
			this.defaultStartLegendColor = ["#ffd75d", "#a0b880", "#cccccc", "#cccccc", "#cccccc", "#fdf3d6", "#cccccc"];
			...
			//this.selectedLegendColor = ["#DEEAF8", "#CDDDEE", "#B3C9E2", "#9DB5CF", "#8CA2BB", "#7B8FA5", "#6A7B8D", "#576574", "#47525E", "#323D4A"]; // 2020-10-26 [곽제욱] 항목 선택시 범례 색 변경 추가(level 10인경우)
			this.selectedLegendColor = ["#CCCCD6", "#AAABBB", "#888AAB", "#66699B", "#44488A", "#22277A", "#00076A", "#000551"]; // 2020-10-26 [곽제욱] 항목 선택시 범례 색 변경 추가
				...
				if (this.delegate != undefined && this.delegate.namespace != undefined && this.delegate.namespace === "houseAnalysisMap") {
					...
					switch ($administStatsMain.ui.selectedThema) {
						case "신혼부부":
							...
							break;
						case "주택소유":
							this.legendColor1 = "#0E4000";
							this.legendColor2 = "#CCCCCC";
							break;
						case "중·장년층":
							this.legendColor1 = "#230064";
							this.legendColor2 = "#CCCCCC";
							break;
						case "귀농·귀어·귀촌":
							this.legendColor1 = "#CB5B1B";
							this.legendColor2 = "#CCCCCC";
							break;
						case "통계더보기":
							break;
			    				...
			    				if(info.isHouseMap){
			    					...
			    					if($houseAnalysisMap.ui.indicatorStatMap){
			    						if(info.m_class_idx_id=="HMM0023"){
			    							dataNameText = "보육시설 대비 5세이하 신혼부부 수";
			    						}else if(info.m_class_idx_id=="HMM0024"){
			    							dataNameText = "병의원 및 약국 대비 신혼부부 수";
			    						}else if(info.m_class_idx_id=="HMM0025"){
			    							dataNameText = "노인복지시설 대비 65세 이상 신혼부부 수";
			    						}else if(info.m_class_idx_id=="HMM0026"){
			    							dataNameText = "사회복지시설 대비 신혼부부 수";
			    						}
			    					}
	    ...
	    this.getDataName = function(showData) {
	    	var showName = {
					"tot_ppltn" : "총신혼부부",
					"tot_ppltn_male" : "총신혼부부(남자)",
					"tot_ppltn_fem" : "총신혼부부(여자)",
					"avg_age" : "평균나이",
					...
					"tot_house" : "총주택",
					"nongga_cnt" : "농가(가구)",
					"nongga_ppltn" : "농가(신혼부부)",
					"imga_cnt" : "임가(가구)",
					"imga_ppltn" : "임가신혼부부",
					"naesuoga_cnt" : "내수면총어가",
					"naesuoga_ppltn" : "해수면어가신혼부부",
					"haesuoga_cnt" : "해수면총어가",
					"haesuoga_ppltn" : "해수면어가신혼부부",
					"employee_cnt" : "종사자수"
			};
	    ...
	    this.showNumberData = function(){
	    	...
	    	srvLogWrite("S0","02","01","01",$("#showData_"+that.id + ">span").text(),""); //jrj 로그
		...
		this.calNegativeLegend = function(arData) {
			...
			if (!this.isNegative) {
				...
				// 2020-11-04 [곽제욱] 행정통계시각화 주제별 범례색깔 변경 START
				if($administStatsMain.ui.selectedThema == "신혼부부" || $administStatsMain.ui.selectedThema == "가구" || $administStatsMain.ui.selectedThema == "주택" || $administStatsMain.ui.selectedThema == "상세"){
					var colorList2 = getCalculColor("#ffd75d", "#cd1103", that.lv);
				} else if($administStatsMain.ui.selectedThema == "농업"){
					var colorList2 = getCalculColor("#F5A020", "#CB5B1B", that.lv); //2020.11.17[신예리] 컬러 변경
				} else if($administStatsMain.ui.selectedThema == "임업"){
					var colorList2 = getCalculColor("#D4D05D", "#4F7001", that.lv); //2020.11.17[신예리] 컬러 변경
				} else if($administStatsMain.ui.selectedThema == "어업"){
					if($fisheryDash.fisheryType=="sea"){
						var colorList2 = getCalculColor("#60A8C2", "#1A4351", that.lv);
					} else {
						var colorList2 = getCalculColor("#BE86C9", "#360D3E", that.lv);
					}
				} else {
					var colorList2 = getCalculColor("#ffd75d", "#cd1103", that.lv);
				}
				// 2020-11-04 [곽제욱] 행정통계시각화 주제별 범례색깔 변경 END
...
//			console.log("[legendInfo.js] setUserLegend() this.map.isInnerMapShow3 [" + this.map.isInnerMapShow3);
		...
		this.getColor = function(value, valPerSlice) {
			...
			if (valPerSlice.length < this.lv && that.legendType == "auto") {
				...
				var returnLevel=0;
				for(var i = 1; i<valPerSlice.length;i++){
					if(value <= valPerSlice[i]){
						if($administStatsMain.ui.selSliceId != "" && $administStatsMain.ui.selSliceId != null){
							/* 2021-09-10 선택한 색 변경 추가 */
								if ($administStatsMap.ui.selMapColor != "" && $administStatsMap.ui.selMapColor != null) {
									const mapColor = $administStatsMap.ui.selMapColor;
									return [$.trim(getCalculColor("#CCCCCC", mapColor, that.lv)[i-1]), i-1];
								}else{
									return [$.trim(that.legendColor[i-1]), i-1];
								}
						} else {
							return [$.trim(that.legendColor[i-1]), i-1];
						}
				...
				if($administStatsMap.ui.selMapColor != "" && $administStatsMap.ui.selMapColor != null){
					return [$.trim(that.selectedLegendColor[that.lv-1]), that.lv];
				} else {
					return [$.trim(that.legendColor[that.lv-1]), that.lv];
				}
			}
		};
		...
		this.legendEvent = function(){
			...
			body.on("click","#showData_"+that.id, function(){
...
//						srvLogWrite("C0", "08", "04", "00", "", cls);		//지도타입 4종
			...
			body.on("click","#lgTypeList_"+that.id+">li>a", function(){
				$("#lgTypeList_"+that.id+">li>a").click();
			});
...
//					srvLogWrite("C0", "08", "05", "00", "", "");		//단계설정 3종
		...
		this.createLegend = function() {
			...
			infoLegend.update = function(props) {
				...
				this._div.innerHTML = '<div class="legendRing legendBox max" id="legendBox_'+that.id+'" data-ing="max" style="display: none;">' // 2020-12-11 [곽제욱] data-html2canvas-ignore=true 제거
																																				// 2022-02-08 style="display: none; 추가
						...
						*/
						...
						//mng_s 20210326 이진호, 범례 설정 버튼 안보이게 수정(display none 추가)

						+'<a href="javascript:void(0)" id="btn_legendSetting_'+that.id+'" class="btn_legendSetting" style="display:none;">설정</a>'
						//mng_e 20210326 이진호

						+'</div>'
						+'</div>'
						+'<a id="showData_'+that.id+'" href="javascript:void(0)" data-type="data" data-subj="통계치 팁()" title="통계치" class="a_show_data '+(that.numberData?"on":"")+'"><span class="span_show_data">통계치 '+(that.numberData?"on":"off")+'</span></a>'
...
//	console.log("[legendInfo.js] legendColor() obj [" + obj);
	...
	if($("#grid_lg_color_2").closest("li").length > 0){
		$("#grid_lg_color_2").closest("li").remove();
	}
	if($("#grid_lg_color_4").closest("li").length > 0){
		$("#grid_lg_color_4").closest("li").remove();
	}
	if($("#grid_lg_color_5").closest("li").length > 0){
		$("#grid_lg_color_5").closest("li").remove();
		switch (gv_mode) {
			case "newly":
				break;
			case "house":
				$("#grid_lg_color_0").attr("class", "");
				$("#grid_lg_color_1").attr("class", "on");
				$("#grid_lg_color_3").attr("class", "");
				$($("#grid_lg_color_0").closest("li")).insertAfter($("#grid_lg_color_1").closest("li"));
				$($("#grid_lg_color_3").closest("li")).insertAfter($("#grid_lg_color_1").closest("li"));
				break;
			case "middl":
				$("#grid_lg_color_0").attr("class", "");
				$("#grid_lg_color_1").attr("class", "");
				$("#grid_lg_color_3").attr("class", "on");
				$($("#grid_lg_color_0").closest("li")).insertAfter($("#grid_lg_color_3").closest("li"));
				$($("#grid_lg_color_1").closest("li")).insertAfter($("#grid_lg_color_3").closest("li"));
				break;
		}
	}
