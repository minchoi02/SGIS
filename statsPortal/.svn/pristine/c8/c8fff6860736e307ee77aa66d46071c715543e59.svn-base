	...
	$houseAnalysisMap.ui = {
		...
		createMap: function(id, seq) {
			...
			map.gMap.whenReady(function() {
				map.createHeatMap();
				map.mapMove([map.gMap.getCenter().x,map.gMap.getCenter().y],1);
			});
...
//				$("header").css({"height":"104px", "width":"970px"}); 
				$(".global_nav, .searchArea, .headerEtc, .gnb, .headerContents form,.headerContents h1").show(); // 2020.10.22 '.global_nav, .searchArea,' 추가 
...
//			$("header").css({"height":"10px", "width":"100%"}); 
			$(".global_nav, .searchArea,  .headerEtc, .gnb, .headerContents form,.headerContents h1").hide(); // 2020.10.22 '.global_nav, .searchArea,' 추가
		...
		doReset : function(callback) {
			this.doClearMap(function(){
				if($houseAnalysisMap.ui.doRemoveFlag){
					...
					$houseAnalysisMap.ui.mapList[$houseAnalysisMap.ui.curMapId].markers2.clearLayers();
					$houseAnalysisMap.ui.mapList[$houseAnalysisMap.ui.curMapId].markers3.clearLayers();
					$(".school,.apt").prop("checked",false);
				}
		...
		doAnalysisShareInfo : function (type, data) {
			...
			if (dataArr != undefined) {
				...
				case 0 :
					...
					var importance_asis_val = paramInfoArr.importance_asis_val.split(',');	//살고싶은지역 가중치
...
//										return false;
		...
		,resizeLayer : function(){
			var clientHeight = window.innerHeight; // 현재 화면의 지도 영역의 높이
			
			if( clientHeight < 650 ){
				$("#scrollDiv").mCustomScrollbar();
				$(".LikeTown").css("height", ( clientHeight - 50 ) + "px");
				$("#scrollDiv").css("height", ( clientHeight - 100 ) + "px");
			} else {
				if( $(".LikeTown").height() < 720 ){
					$(".LikeTown").css("height", ( $(".LikeTown").height() + 50 ) + "px");
					$("#scrollDiv").css("height", ( $("#scrollDiv").height() + 50 ) + "px");
				}
			}
		}
