...
//			logger.info(sido_cd+" "+sgg_cd+" "+emdong_cd+" "+adm_length+" "+low_search);
			...
			if("Y".equals(mapParameter.get("is_non_self"))) {
				...
				if (adm_cd_nonSelf != null) {
					//is_non_self가 "Y"일때 호출조건 3가지: adm_length(5) & low_search(1), adm_length(5) & low_search(0), adm_length(13) & low_search(2)  
					if (("5".equals(adm_length) && "1".equals(low_search)) && adm_cd_nonSelf.length() >= 2) {
						adm_cd_nonSelf = adm_cd_nonSelf.substring(0, 2);		// 기존(시도 -> 시군구) 조회인것처럼
					}else if ((("5".equals(adm_length) && "0".equals(low_search)) || ("13".equals(adm_length) && "2".equals(low_search))) && adm_cd_nonSelf.length() >= 4) {
