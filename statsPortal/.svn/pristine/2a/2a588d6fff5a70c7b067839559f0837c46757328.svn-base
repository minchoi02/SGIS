...
(function(W, D) {
	...
	$houseAnalysisMap.search = {
		...
		setTitle : function(adm_nm,stats_name){
			if(this.isIndicator===true||this.isCensus===true){
				if($("#look-select-type").is(":checked")){
					$(".helperText").css("top","-8px");
					$(".icon-circle").css("top","20px");
				}
				else{
					$(".helperText").css("top","0px");
					$(".icon-circle").css("top","13px");
				}
				$(".interactiveBar>.helperText>.icon-circle").css("display","block");//2019-03-13 박길섭
			...
			search :function(){
				...
				if(!$("#look-select-location").is(":checked")&&!$("#look-select-type").is(":checked")){
					...
					if($houseAnalysisMap.search.mapStat.indicator.getSearchLevel($houseAnalysisMap.search.mapStat.indicator.adm_cd)>compareLevel){
						...
						var tempRegion = $("#current-sido-select").find("option[value='"+$('#current-sido-select').val()+"']").text()+" "+$("#current-sgg-select").find("option[value='"+$('#current-sgg-select').val()+"']").text()
						if(!($("#look-select-location").is(":checked"))){
							tempRegion = "";
						}
						var tempNm = bClassInfoList[$houseAnalysisMap.search.mapStat.indicator.b_class_idx_id].indicator[$houseAnalysisMap.search.mapStat.indicator.m_class_idx_id].m_class_idx_nm;
						if($houseAnalysisMap.search.mapStat.indicator.b_class_idx_id == "HML0001"){
							srvLogWrite('F0','02','02','01',tempNm,tempRegion);
						}else if($houseAnalysisMap.search.mapStat.indicator.b_class_idx_id == "HML0002"){
							srvLogWrite('F0','02','02','02',tempNm,tempRegion);
						}else if($houseAnalysisMap.search.mapStat.indicator.b_class_idx_id == "HML0003"){
							srvLogWrite('F0','02','02','03',tempNm,tempRegion);
						}else if($houseAnalysisMap.search.mapStat.indicator.b_class_idx_id == "HML0004"){
							srvLogWrite('F0','02','02','04',tempNm,tempRegion);
						}else if($houseAnalysisMap.search.mapStat.indicator.b_class_idx_id == "HML0005"){
							srvLogWrite('F0','02','02','05',tempNm,tempRegion);
						}else if($houseAnalysisMap.search.mapStat.indicator.b_class_idx_id == "HML0006"){
							srvLogWrite('F0','02','02','06',tempNm,tempRegion);
						}else if($houseAnalysisMap.search.mapStat.indicator.b_class_idx_id == "HML0007"){
							srvLogWrite('F0','02','02','07',tempNm,tempRegion);
						}
					}
				}else{//지역만 선택되어있을때
					abs.onBlockUIPopup();
		...
		recommend : {
			...
			inter_resid_sido_cd:null,//살고싶은 지역 시도 코드
			inter_resid_sgg_cd:null,//살고싶은 지역 시군구 코드
			inter_name:null,//살고싶은 지역 지역 명칭
			...
			search : function(){
				...
				$(".interactiveBar>.helperText>.icon-circle").css("display","none");
				this.init();
...
//					$houseAnalysisMap.search.recommend.importance_asis_val.push($(this).data("asis-order"));
						...
						if(res.errCd=="0"){
							if(res.features.length>0){
								...
								$.each(res.features,function(cnt,node){
									...
									if(cnt<=4){
										$("#interestList").append(
											$("<div/>",{"style":"position: relative;"+(res.features.length>5?"width:100%;float:left;":"")}).append(
												$("<a/>",{
													href:"#",
	...
	//												"style":"width:calc(100% - "+($houseAnalysisMap.ui.hasText(naver)?"55":"35")+"px)"
													...
													srvLogWrite('F0','03','01','00',$(this).innerHTML,'');
													$houseAnalysisMap.search.getRecommendObject().init();
													var element = $(this);
													$houseAnalysisMap.ui.doClearMap(function(){
		...
		idealType : {
			...
			search : function(){
				var puzzleTemp="";
				$("#ideal-type-dropzone>div.M_on").each(function(cnt){
					puzzleTemp += $("div.ui-droppable.M_on")[cnt].children[1].innerText+",";
				});
				srvLogWrite('F0','02','04','02',puzzleTemp.substr(0,puzzleTemp.length-1),'');
				$(".interactiveBar>.helperText>.ico-circle").css("display","none");
				//$("#ideal-type-close-button").removeClass("on");
				//우리동네수정 시작
				var poiId=$(".sop-top.sop-right>.sop-control").attr("id");
				if($("#"+poiId).find(".rightQuick.rq01").hasClass("on")){
					$("#"+poiId).find(".rightQuick.rq01").trigger("click");
				}
				//우리동네수정 끝
					...
					if($houseAnalysisMap.ui.hasText(this.parameters.adm_cd)&&this.parameters.adm_cd!="00"){
						...
						$houseAnalysisMap.api.census.getCensusData(this.api,this.parameters,function(res){
							$houseAnalysisMap.ui.doClearMap(function(){
								...
								//190219_s 주용민
								$houseAnalysisMap.search.lastMapOption.zoom = 10;
								//190219_e 주용민
			...
			indicator : {//지표
				...
				search : function(callback){
					...
					if(mClassObject.disp_level==1){
						...
						//190219_s 주용민
						map.gMap.setMaxZoom(10);
						//190219_e 주용민
					}
					this.adm_cd = mapOption.adm_cd;
					console.log("역추적 2222222222222" + year);
