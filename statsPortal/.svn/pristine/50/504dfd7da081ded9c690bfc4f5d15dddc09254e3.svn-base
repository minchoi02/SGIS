	...
	collectionGallery = {
			...
			$("body").on("click",".writeGalleryListItem li", function(){
	    		...
	    		if(!$(this).parents("ul").hasClass("openSelect")){
	    			if($(this).hasClass("statsGalleryDialog")){
	    				...
	    				$('#dialogDiv').load("/view/edu_gallery/statsGalleryDialog.jsp");
	    			}else if($(this).hasClass("useGalleryDialog")){
	    				$('#dialogDiv').attr("class", "useGalleryDialog");
	    				$('#dialogDiv').load("/view/edu_gallery/useGalleryDialog.jsp");
	    			}else if($(this).hasClass("sgisCensusReq")){
	    				$('#dialogDiv').attr("class","sgisCensusReq")
	    				$('#dialogDiv').load("/view/edu_gallery/sgisCensusReq.jsp");
	    			}
		...
		selectGalleryList : function () {
			...
			sopPortalGalleryObj.request({
				method : "POST",
				async : false,
				url : contextPath + "/ServiceAPI/edu_gallery/galleryListAll.json"
			});
		...
		shareInfoOpen : function(){
			...
			var linkUrl = domain + "/view/edu_gallery/resultGallery?shareDataId="+collectionGallery.selectDataId+"&srvType="+srvType;
...
//		},
		...
		serarchTagName : function(searchStr){
			var linkUrl ="/view/edu_gallery/resultGallery?searchTagName="+searchStr;
				...
				if(res.errCd == "0") {
					...
					if("useGalleryDialog" == $('#dialogDiv').attr("class")){
						...
						if(galleryImgListItem[0]){
							...
							if(!paramInfo.preViewImg){
								var preViewImgHtml = "";	
								preViewImgHtml += '<img src="/img/common/testimg01.png" width="100%" height="100%" onerror="this.src=\'/img/common/testimg01.png\'" />';
								preViewImgHtml +='<a href="/img/common/testimg01.png" class="gdDownload"></a>';
								$(".gdMapArea").html(preViewImgHtml);
							}
						}
					}
