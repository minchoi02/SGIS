...
public class HadmareaForBorough extends AbsQuery<List> {

	private static final Log logger = LogFactory.getLog( HadmareaForBorough.class );
	...
	*/
	...
	public List executeAPI( HttpServletRequest req, HttpServletResponse res, String trId ) throws AbsException
	{
		...
		try
		{
			...
			System.out.println("[HadmareaForBorough.java] zoom [" + zoom);
			...
			//2020년수정변경 시작 (ggm)
			//이 클래스는 비자치구를 포함하는 시일때만 호출된다.(zoom 4일때와 zoom4에서 2레벨로 조회할때)
			if(mapParameter.get("low_search") != null && "2".equals(mapParameter.get("low_search"))) {
				String sgg_cd_nonSelf = (String) mapParameter.get( "sgg_cd" );
				if (sgg_cd_nonSelf != null && sgg_cd_nonSelf.length() >= 2) {
			...
			//			System.out.println("[HadmareaForBorough.java] goem.asText() [" + goem.asText());
	...
	protected void optimizeParameterMap( Map mapParameter ) throws Exception
	{
		...
		System.out.println("[SOPOpenAPI3] [HadmareaForBorough.java] [ adm_cd = " + adm_cd + " ]" );
	...
	{
		return "boundary.hadmareaForBorough";
	}
