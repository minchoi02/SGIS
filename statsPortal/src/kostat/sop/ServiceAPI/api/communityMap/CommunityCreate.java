...
public class CommunityCreate extends AbsQuery<HashMap<String,Object>> {
	...
	enum MustParam{
		cmmnty_map_nm,
		cmmnty_from_ce //에듀 초,중,고,포탈 구분 값
	}
	...
	public HashMap<String,Object> executeAPI(
		...
		try {
			...
			if(community!=null&&community.get("usr_id").equals(login_id)){
				...
				System.out.println("========================== " + mapParameter.get("temp") );
				System.out.println("========================== " + community.get("temp_save_yn") );
				
				if( (!"Y".equals( mapParameter.get("temp")) ) && "Y".equals( community.get("temp_save_yn") ) ) {
					attack = false;
				}
			}
			...
			//srv_dt_cmmnty_map
			if( attack ){
				String defenceAutometicAttack = session.selectOne("communityMap.defenceAutometicAttack", mapParameter);		//X면 통과 Y또는 null이면 실패	글은 1분이상 지나야 등록가능함
				System.out.println(defenceAutometicAttack);
				if(defenceAutometicAttack != null){
					if("N".equals(defenceAutometicAttack)){
						throw new ApiException("짧은시간동안에 연속해서 등록하실 수 없습니다");
					}
				}
			}
			//자동화공격 방지 end
			
			if( update && mapParameter.get("cmmnty_map_id") == null ){
				throw new ApiException("세션 오류가 발생하였습니다.");
			} else {
	...
	private boolean validation(
		...
		String[] lbdms = request.getParameterValues("lbdms");//lbdms POI
		...
		String grant_yn =  getParameter(mapParameter,"grant_yn");//의견등록 참여방법
		...
		if(temp==null||temp.equals("N")){
			...
			if(lbdms!=null&&lbdms.length>1){
				throw new ApiException("LBDMS 데이터는 최대 하나만 선택 가능합니다");
			}
			if( ( mydata!=null && mydata.length>1 ) && ( lbdms!=null && lbdms.length>1 ) ){
				throw new ApiException("LBDMS 데이터는 최대 하나만 선택 가능합니다");
			}
			...
			if("Y".equals(pwdyn)){
				...
				if( !"P".equals(grant_yn) ){
					mapParameter.put("pw", null);
				}
			}
		} else {
			if( pw != null && !"".equals( pw ) && "Y".equals(pwdyn) && "P".equals(grant_yn) ){
				BCryptPasswordEncoder pwbcrypt = new BCryptPasswordEncoder();
				mapParameter.put("pw", pwbcrypt.encode(pw));
			}
		}
	...
	private boolean formCommunity(
		...
		if("Y".equals(parameters.get("temp"))||parameters.get("cmmnty_map_atch_file_id")!=null||!(Boolean)fileResult.get("error")){
			...
			if(!hasError){
				...
				session.delete("communityEtc.deleteCmmntyLbdmsList",parameters);
				session.delete("communityEtc.deleteCmmntyItemList",parameters);
				String[] stats = request.getParameterValues("stats");//통계
				String[] mydata = request.getParameterValues("mydata");//마이데이터
				String[] itemPoi = request.getParameterValues("itemPoi");//산업분류 POI
				String[] lbdms = request.getParameterValues("lbdms");//LBDMS POI
				boolean insertStats = true,insertMydata=true,isertItemPoi=true,insertLbdms=true;
				...
				if(insertLbdms&&lbdms!=null){
					HashMap<String, Object> obj = new HashMap<String, Object>();
					obj.put("cmmnty_map_id", parameters.get("cmmnty_map_id"));
					obj.put("list", lbdms[0]);
					session.insert("communityEtc.insertCmmntyLbdmsList", obj);
				}
			}
		}else{
