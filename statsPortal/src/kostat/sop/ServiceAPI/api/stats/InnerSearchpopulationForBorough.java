...
import java.util.ArrayList;/*SGIS4_0107 추가*/
import java.util.HashMap;/*SGIS4_0107 추가*/
...
import kostat.sop.ServiceAPI.api.catchmentArea.ServiceAreaBSCA;/*SGIS4_0107 추가*/
...
* 대화형통계지도에서 비자치구 관련 조회시에만 호출
* 
* <pre>
* input : innersearchpopulationForBorough.json/xml
* output : json/xml
...
public class InnerSearchpopulationForBorough extends AbsQuery< List >
{
	private static final Log logger = LogFactory.getLog( InnerSearchpopulationForBorough.class );
	...
	public List executeAPI( HttpServletRequest req, HttpServletResponse res, String trId ) throws AbsException
	{
		...
		try
		{
			...
			if(zoom != null && !"".equals(zoom) && !"bnd_grid".equals(bnd_grid)) { //mng_s 그리드이면
				...
				if(("0".equals(gender) && age_from ==null) || ("0".equals(gender) && "01".equals(age_from) && "21".equals(age_to))){
					statType = "tot";
					mapParameter.put("statType" , statType);
				}
				
				result2 = session.selectList( "stats.innersearchpopulation_bnd_grid", mapParameter );//211029_SGIS_4 수정
				
				if(result2 != null && result2.size() > 0) {
					for(int i=0; i<result2.size(); i++) {
						HashMap<String, Object> kwMap = new HashMap<>();
						kwMap = ServiceAreaBSCA.getTotalSum2((HashMap) result2.get(i), "pop_" ,5 ,null);
						if(kwMap != null) {
							result.add(kwMap);
						}
					}
...
//			}
	...
	{
		return "stats.innersearchpopulationForBorough";
	}
