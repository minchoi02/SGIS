...
public class RecommendAreaLists extends AbsQuery<List<?>> {
	...
	public List<?> executeAPI(HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			//System.out.println("[RecommendAreaLists.java] importance_disp_lev_split.length [" + importance_disp_lev_split.length);
			logger.debug("[RecommendAreaLists.java] importance_disp_lev_split.length [" + importance_disp_lev_split.length);
			...
			while(tempCnt < importance_disp_lev_split.length){
				if("3".equals(importance_disp_lev_split[tempCnt])){
					...
					//System.out.println("[RecommendAreaLists.java] dispLev [" + dispLev);
					logger.debug("dispLev [" + dispLev);
			...
			logger.debug("house.getMaxOrder ==> mapParameter [" + mapParameter);
			
			//mng_s 20200115 기존 소스에 mapParameter가 빠져 있어서 추가함.
			mapParameter.put("max_order_no", session.selectOne("house.getMaxOrder", mapParameter));
			logger.debug("house.getMaxOrder ==> selectOne [" + session.selectOne("house.getMaxOrder", mapParameter));
			//logger.info("house.getMaxOrder ==> selectOne [" + session.selectOne("house.getMaxOrder"));
			//System.out.println("[RecommendAreaLists.java] selectOne [" + session.selectOne("house.getMaxOrder"));
