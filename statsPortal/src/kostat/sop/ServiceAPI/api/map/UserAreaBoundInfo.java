...
private static final Log logger = LogFactory.getLog(UserAreaBoundInfo.class);
	...
	public List executeAPI(HttpServletRequest req, HttpServletResponse res,
		...
		try {
			...
			//mng_s 20190826 소방, 응급, 경찰서만 4레벨,5레벨에서 비자치구로 가지고 오기
			//쿼리에서 보면 코드값이 1~5 인데 소방에서 요청하면 code가 100으로 날라간다. 그래서 여기서 치환한번 해본다.
			if("rectangle".equals(type) && "100".equals(code)) {
				mapParameter.put( "base_year", bnd_year );
				mapParameter.put( "code", "2" );
			}
