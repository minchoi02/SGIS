...
public class TechnicalBizMapController {
	...
	public ModelAndView technicalBizLeftMenu(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//생활업종 툴팁(재개발로 인하여 생활업종쪽 툴팁 표출 필요)
			Map mapParameterBiz = new HashMap();
			Map tooltipInfoBiz = new HashMap();
			mapParameterBiz.put("menu_class_cd", "B0");
			List tooltipListBiz = mapService.selectTooltipInfo(mapParameterBiz);
			
			for (int i=0; i<tooltipListBiz.size(); i++) {
				Map mapTec = (HashMap)tooltipListBiz.get(i);
				...
				tooltipInfoBiz.put(classCd + ttpId, exp);
			}
			paramInfo.put("tooltipListBiz", tooltipInfoBiz);
