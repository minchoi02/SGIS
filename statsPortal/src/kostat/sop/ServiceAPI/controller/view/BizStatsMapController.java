...
public class BizStatsMapController {
	...
	public ModelAndView bizStatsMap(HttpServletRequest request, HttpServletResponse response) {
		...
		List<Map<String, Object>> allThemeCdInfo = themeCdCommon.selectAllThemeCdInfo();
...
//					}
	...
	public ModelAndView bizStatsLeftMenu(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//기술업종 툴팁(재개발로 인하여 기술업종쪽 툴팁 표출 필요)
			Map mapParameterTec = new HashMap();
			Map tooltipInfoTec = new HashMap();
			mapParameterTec.put("menu_class_cd", "E0");
			List tooltipListTec = mapService.selectTooltipInfo(mapParameterTec);
			
			for (int i=0; i<tooltipListTec.size(); i++) {
				Map mapTec = (HashMap)tooltipListTec.get(i);
				String classCd = (String)mapTec.get("menu_class_cd");
				String ttpId = (String)mapTec.get("ttip_id");
				String exp = (String)mapTec.get("ttip_exp");
				tooltipInfoTec.put(classCd + ttpId, exp);
			}
			paramInfo.put("tooltipListTec", tooltipInfoTec);
		}catch (AbsAPIException e) {
			logger.error(e);
			throw e;
		} catch (IllegalArgumentException e) {
	...
	public ModelAndView bizStatsHelper(HttpServletRequest request, HttpServletResponse response) {
		// 2020년 SGIS고도화 3차(테마코드) 시작  - 기술업종 통계지도에서도 이 JSP를 jsp:include 한다. 어쩔 수 없이 아래와 같은 if문을 추가한다. (pse)
		if(request.getAttribute("allThemeCdInfo") == null ) {
			List<Map<String,Object>> allThemeCdInfo = themeCdCommon.selectAllThemeCdInfo();
			request.setAttribute("allThemeCdInfo", allThemeCdInfo);
		}
		// 2020년 SGIS고도화 3차(테마코드) 끝 (pse)
